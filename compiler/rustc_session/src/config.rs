...
pub fn build_session_options(early_dcx: &mut EarlyDiagCtxt, matches: &getopts::Matches) -> Options {
    ...
    if unstable_opts.assumptions_on_binders {
        // Only warn when `-Znext-solver` was explicitly given a value that doesn't enable
        // the solver globally.
        let no_next_solver = matches
            .opt_strs("Z")
            .iter()
            .any(|arg| *arg == "next-solver=no" || *arg == "next-solver=coherence");
        if no_next_solver {
        ...
        unstable_opts.next_solver = NextSolverConfig { globally: true };
    }
