...
use rustc_target::spec::{
    Architecture, CodeModel, DebuginfoKind, PanicStrategy, RelocModel, RelroLevel, SanitizerSet,
...
fn validate_commandline_args_with_session_available(sess: &Session) {
    ...
    if sess.opts.unstable_opts.fixed_x18 && sess.target.arch == Architecture::AArch64 {
    ...
    if sess.opts.unstable_opts.branch_protection.is_some()
        && sess.target.arch != Architecture::AArch64
    {
    ...
    if sess.opts.unstable_opts.function_return != FunctionReturn::default() {
        if !matches!(sess.target.arch, Architecture::X86 | Architecture::X86_64) {
    ...
    if sess.opts.cg.soft_float {
        if sess.target.arch == Architecture::Arm {
