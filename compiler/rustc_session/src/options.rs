...
mod desc {
    ...
    pub(crate) const parse_next_solver_config: &str =
        "either `globally` (when used without an argument) or `no` (default)";
...
pub mod parse {
    ...
    pub(crate) fn parse_next_solver_config(slot: &mut NextSolverConfig, v: Option<&str>) -> bool {
        if let Some(config) = v {
            *slot = match config {
                "no" => NextSolverConfig { globally: Some(false) },
                "globally" => NextSolverConfig { globally: Some(true) },
                _ => return false,
            };
        } else {
            *slot = NextSolverConfig { globally: Some(true) };
        }
