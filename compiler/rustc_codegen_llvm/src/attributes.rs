...
use rustc_target::spec::{
    Architecture, FramePointer, SanitizerSet, StackProbeType, StackProtector,
};
...
pub(crate) fn inline_attr<'ll, 'tcx>(
    ...
    match inline {
        ...
        InlineAttr::Never => {
            if tcx.sess.target.arch != Architecture::AmdGpu {
...
fn backchain_attr<'ll>(cx: &SimpleCx<'ll>, sess: &Session) -> Option<&'ll Attribute> {
    if sess.target.arch != Architecture::S390x {
        return None;
    }
...
pub(crate) fn llfn_attrs_from_instance<'ll, 'tcx>(
    ...
    if codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::NAKED) {
        ...
        {
            assert!(sess.target.arch == Architecture::AArch64);
            if bti {
