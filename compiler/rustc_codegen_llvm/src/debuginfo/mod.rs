...
impl<'ll, 'tcx> DebugInfoBuilderMethods<'tcx> for Builder<'_, 'll, 'tcx> {
    ...
    /// Temporarily annotate move/copy operations with debug info for profiling.
    ///
    /// This creates a temporary debug scope that makes the move/copy appear as an
    /// inlined call to `compiler_move<T, SIZE>()` or `compiler_copy<T, SIZE>()`.
    /// The provided closure is executed with this temporary debug location active.
    ///
