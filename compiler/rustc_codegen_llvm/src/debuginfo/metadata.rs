...
fn build_pointer_or_reference_di_node<'ll, 'tcx>(
    ...
    let pointee_type_di_node = match pointee_type.kind() {
        ...
        ty::Slice(element_type) => spanned_type_di_node(cx, *element_type, span),
        ...
        _ => spanned_type_di_node(cx, pointee_type, span),
    };
...
pub(crate) fn spanned_type_di_node<'ll, 'tcx>(
    ...
    let DINodeCreationResult { di_node, already_stored_in_typemap } = match *t.kind() {
        ...
        ty::RawPtr(pointee_type, _) | ty::Ref(_, pointee_type, _) => {
            build_pointer_or_reference_di_node(cx, t, pointee_type, unique_type_id, span)
        }
        ...
        {
            build_pointer_or_reference_di_node(cx, t, t.expect_boxed_ty(), unique_type_id, span)
        }
