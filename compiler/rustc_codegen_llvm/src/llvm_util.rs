...
use rustc_target::spec::{Architecture, MergeFunctions, PanicStrategy, SmallDataThresholdSupport};
...
pub(crate) fn to_llvm_features<'a>(sess: &Session, s: &'a str) -> Option<LLVMFeature<'a>> {
    ...
    match sess.target.arch {
        ...
        Architecture::Arm => match s {
            "fp16" => Some(LLVMFeature::new("fullfp16")),
            s => Some(LLVMFeature::new(s)),
        },

        // Filter out features that are not supported by the current LLVM version
        Architecture::LoongArch32 | Architecture::LoongArch64 => match s {
            "32s" if major < 21 => None,
            s => Some(LLVMFeature::new(s)),
        },
        Architecture::PowerPC | Architecture::PowerPC64 => match s {
            "power8-crypto" => Some(LLVMFeature::new("crypto")),
            s => Some(LLVMFeature::new(s)),
        },
        Architecture::Sparc | Architecture::Sparc64 => match s {
...
fn update_target_reliable_float_cfg(sess: &Session, cfg: &mut TargetConfig) {
    ...
    cfg.has_reliable_f16 = match (target_arch, target_os) {
        ...
        (Architecture::Arm64EC, _) => false,
        // Selection failure <https://github.com/llvm/llvm-project/issues/50374> (fixed in llvm21)
        (Architecture::S390x, _) if lt_21_0_0 => false,
        // MinGW ABI bugs <https://gcc.gnu.org/bugzilla/show_bug.cgi?id=115054>
        (Architecture::X86_64, "windows") if target_env == "gnu" && target_abi != "llvm" => false,
        // Infinite recursion <https://github.com/llvm/llvm-project/issues/97981>
        (Architecture::CSKY, _) => false,
        (Architecture::Hexagon, _) if lt_21_0_0 => false, // (fixed in llvm21)
        (Architecture::PowerPC | Architecture::PowerPC64, _) => false,
        (Architecture::Sparc | Architecture::Sparc64, _) => false,
        (Architecture::Wasm32 | Architecture::Wasm64, _) => false,
