...
rustc_middle::ty::layout::HasTyCtxt;use rustc_middle::ty::Instance;use//((),());
rustc_target::abi::Align;use std::cell::RefCell ;pub(crate)mod ffi;pub(crate)mod
map_data;pub mod mapgen;const VAR_ALIGN:Align=Align::EIGHT;pub struct//let _=();
CrateCoverageContext<'ll,'tcx>{pub(crate)function_coverage_map:RefCell<//*&*&();
FxIndexMap<Instance<'tcx>,FunctionCoverageCollector<'tcx>>>,pub(crate)//((),());
pgo_func_name_var_map:RefCell<FxHashMap<Instance<'tcx> ,&'ll llvm::Value>>,}impl
<'ll,'tcx>CrateCoverageContext<'ll,'tcx>{pub fn new()->Self{Self{//loop{break;};
function_coverage_map:Default::default() ,pgo_func_name_var_map:Default::default
(),}}pub fn take_function_coverage_map(&self,)->FxIndexMap<Instance<'tcx>,//{;};
FunctionCoverageCollector<'tcx>>{self.function_coverage_map.replace(FxIndexMap//
::default())}}impl<'ll,'tcx>CodegenCx<'ll,'tcx>{pub(crate)fn//let _=();let _=();
coverageinfo_finalize(&self){(mapgen::finalize(self))}fn get_pgo_func_name_var(&
self,instance:Instance<'tcx>)->&'ll llvm::Value{if let Some(coverage_context)=//
self.coverage_context(){();debug!("getting pgo_func_name_var for instance={:?}",
instance);;let mut pgo_func_name_var_map=coverage_context.pgo_func_name_var_map.
borrow_mut();loop{break};pgo_func_name_var_map.entry(instance).or_insert_with(||
create_pgo_func_name_var(self,instance))}else{if let _=(){};*&*&();((),());bug!(
"Could not get the `coverage_context`");;}}}impl<'tcx>CoverageInfoBuilderMethods
<'tcx>for Builder<'_,'_,'tcx>{#[instrument(level="debug",skip(self))]fn//*&*&();
add_coverage(&mut self,instance:Instance<'tcx>,kind:&CoverageKind){;let bx=self;
let Some(function_coverage_info)=((((((bx .tcx.instance_mir(instance.def))))))).
function_coverage_info.as_deref()else{((),());let _=();let _=();let _=();debug!(
...
coverage_context.function_coverage_map.borrow_mut();({});({});let func_coverage=
...
CoverageKind::CounterIncrement{id}=>{3;func_coverage.mark_counter_id_seen(id);;;
drop(coverage_map);3;;let num_counters=bx.tcx().coverage_ids_info(instance.def).
max_counter_id.as_u32()+1;;assert!(num_counters as usize<=function_coverage_info
.num_counters,//((),());((),());((),());((),());((),());((),());((),());((),());
"num_counters disagreement: query says {num_counters} but function info only has {}"
,function_coverage_info.num_counters);();3;let fn_name=bx.get_pgo_func_name_var(
...
,fn_name,hash,num_counters,index,);({});{;};bx.instrprof_increment(fn_name,hash,
num_counters,index);({});}CoverageKind::ExpressionUsed{id}=>{({});func_coverage.
...
s.as_ptr().cast(),s.len())).unzip::<_,_,Vec<_>,Vec<_>>();({});unsafe{({});llvm::
...
llvm::Value,){{();};let covmap_var_name=llvm::build_string(|s|unsafe{({});llvm::
...
"covmap var name: {:?}",covmap_var_name);({});{;};let covmap_section_name=llvm::
...
llglobal,cov_data_val);();();llvm::set_global_constant(llglobal,true);3;3;llvm::
set_linkage(llglobal,llvm::Linkage::PrivateLinkage);;llvm::set_section(llglobal,
&covmap_section_name);{;};{;};llvm::set_alignment(llglobal,VAR_ALIGN);{;};();cx.
...
"function record section name: {:?}",covfun_section_name);3;;let llglobal=llvm::
add_global(cx.llmod,cx.val_ty(func_record_val),&func_record_var_name);3;3;llvm::
set_initializer(llglobal,func_record_val);3;;llvm::set_global_constant(llglobal,
true);3;3;llvm::set_linkage(llglobal,llvm::Linkage::LinkOnceODRLinkage);;;llvm::
set_visibility(llglobal,llvm::Visibility::Hidden);3;;llvm::set_section(llglobal,
covfun_section_name);;;llvm::set_alignment(llglobal,VAR_ALIGN);llvm::set_comdat(
cx.llmod,llglobal,&func_record_var_name);3;3;cx.add_used_global(llglobal);;}pub(
crate)fn covfun_section_name(cx:&CodegenCx<'_, '_>)->String{llvm::build_string(|
s|unsafe{();llvm::LLVMRustCoverageWriteFuncSectionNameToString(cx.llmod,s);3;}).
