...
EventId{3;let pass_name=profiler.get_or_alloc_cached_string(pass_name);;;let mut
...
{;let demangled_ir_name=rustc_demangle::demangle(part).to_string();;let ir_name=
profiler.get_or_alloc_cached_string(demangled_ir_name);({});{;};components.push(
StringComponent::Value(SEPARATOR_BYTE));3;;components.push(StringComponent::Ref(
...
llvm_pass_event_kind,event_id));;}fn after_pass_callback(&mut self){;self.stack.
...
let llvm_self_profiler=&mut*(llvm_self_profiler as*mut LlvmSelfProfiler<'_>);3;;
