...
use rustc_target::spec::{
    Architecture, CodeModel, FloatAbi, RelocModel, SanitizerSet, SplitDebuginfo, TlsModel,
};
...
pub(crate) fn target_machine_factory(
    ...
    let float_abi = if sess.target.arch == Architecture::Arm && sess.opts.cg.soft_float {
        llvm::FloatAbi::Soft
    } else {
