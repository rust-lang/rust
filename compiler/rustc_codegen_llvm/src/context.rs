...
use rustc_target::spec::{
    Architecture, HasTargetSpec, RelocModel, SmallDataThresholdSupport, Target, TlsModel,
};
...
pub(crate) unsafe fn create_module<'ll>(
    ...
    if llvm_version < (21, 0, 0) {
        if sess.target.arch == Architecture::Nvptx64 {
            // LLVM 21 updated the default layout on nvptx: https://github.com/llvm/llvm-project/pull/124961
            target_data_layout = target_data_layout.replace("e-p6:32:32-i64", "e-i64");
        }
        if sess.target.arch == Architecture::AmdGpu {
    ...
    if llvm_version < (22, 0, 0) {
        if sess.target.arch == Architecture::Avr {
    ...
    if matches!(sess.target.arch, Architecture::RiscV32 | Architecture::RiscV64)
        && !llvm_abiname.is_empty()
    {
