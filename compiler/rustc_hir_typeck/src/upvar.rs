...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn perform_2229_migration_analysis(
        ...
        struct MigrationError<'tcx> {
            migration_message: String,
            need_migrations: Vec<NeededMigration>,
            tcx: TyCtxt<'tcx>,
            body_id: hir::BodyId,
            closure_hir_id: HirId,
            closure_head_span: Span,
            migration_string: String,
            migrated_variables_concat: String,
        }

        impl<'a, 'tcx> rustc_errors::Diagnostic<'a, ()> for MigrationError<'tcx> {
            fn into_diag(
                self,
                ...
                let mut lint = rustc_errors::Diag::new(dcx, level, migration_message);
                    ...
                    for lint_note in diagnostics_info.iter() {
                        match &lint_note.captures_info {
                            UpvarMigrationInfo::CapturingPrecise {
                                ...
                                let cause_span = tcx.hir_span(*capture_expr_id);
                        ...
                        if lint_note.reason.drop_order {
                            let drop_location_span = drop_location_span(tcx, closure_hir_id);
                ...
                let closure_span = tcx.hir_span_with_body(closure_hir_id);
                let mut closure_body_span = {
                    // If the body was entirely expanded from a macro
                    // invocation, i.e. the body is not contained inside the
                    // closure span, then we walk up the expansion until we
                    // find the span before the expansion.
                    let s = tcx.hir_span_with_body(body_id.hir_id);
                    s.find_ancestor_inside(closure_span).unwrap_or(s)
                };

                if let Ok(mut s) = tcx.sess.source_map().span_to_snippet(closure_body_span) {
                    if s.starts_with('$') {
                        // Looks like a macro fragment. Try to find the real block.
                        if let hir::Node::Expr(&hir::Expr {
                            kind: hir::ExprKind::Block(block, ..),
                            ..
                        }) = tcx.hir_node(body_id.hir_id)
                        {
                            ...
                            if let Ok(snippet) = tcx.sess.source_map().span_to_snippet(block.span) {
