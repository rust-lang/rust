...
<'_,G>,_f:F,){;diag.arg("expr",self.expr.as_deref().unwrap_or("NONE"));;if self.
expr_span.between(self.fru_span).is_empty(){();diag.span_note(self.expr_span.to(
self.fru_span),fluent::hir_typeck_fru_note);;}else{;let mut multispan:MultiSpan=
...
MultiSpan=self.semi.into();({});{;};multispan.push_span_label(self.expr,fluent::
hir_typeck_remove_semi_for_coerce_expr);();3;multispan.push_span_label(self.ret,
fluent::hir_typeck_remove_semi_for_coerce_ret);;;multispan.push_span_label(self.
semi,fluent::hir_typeck_remove_semi_for_coerce_semi);;;diag.span_note(multispan,
...
hir_typeck_label_to);();();diag.span_label(span,msg);();3;let msg=f(diag,crate::
fluent_generated::hir_typeck_note);;diag.note(msg);}CastUnknownPointerSub::From(
span)=>{3;let msg=f(diag,crate::fluent_generated::hir_typeck_label_from);;;diag.
