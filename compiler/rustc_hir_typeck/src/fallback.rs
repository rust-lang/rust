...
impl<'tcx> FnCtxt<'_, 'tcx> {
    ...
    fn lint_never_type_fallback_flowing_into_unsafe_code(
        ...
        let unsafe_infer_vars = unsafe_infer_vars.get_or_init(|| {
            let unsafe_infer_vars = compute_unsafe_infer_vars(self, self.item_id);
    ...
    fn lint_obligations_broken_by_never_type_fallback_change(
            ...
            && let [never_error, ..] = never_errors.as_mut_slice()
        {
            ...
            self.tcx.emit_node_span_lint(
                ...
                self.tcx.local_def_id_to_hir_id(self.item_id),
                self.tcx.def_span(self.item_id),
...
fn compute_unsafe_infer_vars<'a, 'tcx>(
    ...
    let body = fcx.tcx.hir_body_owned_by(item_id);
    ...
    debug!(?res, "collected the following unsafe vars for {item_id:?}");
