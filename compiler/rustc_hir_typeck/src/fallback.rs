...
impl<'tcx> FnCtxt<'_, 'tcx> {
    /// Performs type inference fallback, setting [`FnCtxt::never_type_fallback_has_occurred`]
    ...
    ///
    /// Sets [`FnCtxt::never_type_fallback_has_occurred`] if never type fallback
    ...
    fn fallback_if_possible(
        ...
        let fallback = match ty.kind() {
            ...
            _ if diverging_fallback.contains(&ty) => {
                self.never_type_fallback_has_occurred.set(true);
