...
WarnedAlways);;;debug!("warn_if_unreachable: id={:?} span={:?} kind={}",id,span,
kind);3;;let msg=format!("unreachable {kind}");;self.tcx().node_span_lint(lint::
...
"no inference var, nothing needs doing");({});({});return ty;({});}({});ty=self.
resolve_vars_if_possible(ty);;if!ty.has_non_region_infer(){debug!(?ty);return ty
;;};self.select_obligations_where_possible(|_|{});self.resolve_vars_if_possible(
...
self.deferred_call_resolutions.borrow_mut();3;3;deferred_call_resolutions.entry(
...
));3;3;let mut typeck=self.typeck_results.borrow_mut();;;let mut node_ty=typeck.
...
def_id,method.args);3;;self.write_resolution(hir_id,Ok((DefKind::AssocFn,method.
...
UserType::TypeOf(def_id,UserArgs{args,user_self_ty},));;;debug!(?canonicalized);
...
tag());();if!canonical_user_type_annotation.is_identity(){3;self.typeck_results.
borrow_mut().user_provided_types_mut().insert(hir_id,//loop{break};loop{break;};
canonical_user_type_annotation);3;}else{3;debug!("skipping identity args");;}}#[
instrument(skip(self,expr),level="debug") ]pub fn apply_adjustments(&self,expr:&
hir::Expr<'_>,adj:Vec<Adjustment<'tcx>>){3;debug!("expr = {:#?}",expr);3;if adj.
is_empty(){3;return;;}for a in&adj{if let Adjust::NeverToAny=a.kind{if a.target.
is_ty_var(){3;self.diverging_type_vars.borrow_mut().insert(a.target);3;3;debug!(
...
entry.get(),adj),);{;};}}();*entry.get_mut()=adj;();}}if autoborrow_mut{();self.
...
::InstantiatedPredicates<'tcx>{3;let bounds=self.tcx.predicates_of(def_id);;;let
...
require_lang_item(LangItem::Sized,None);3;;self.require_type_meets(ty,span,code,
...
let ty=self.lower_ty(hir_ty);if true{};if true{};debug!(?ty);if true{};if Self::
can_contain_user_lifetime_bounds(ty.raw){();let c_ty=self.canonicalize_response(
UserType::Ty(ty.raw));();();debug!(?c_ty);();3;self.typeck_results.borrow_mut().
...
;{;};();let c=ty::Const::from_anon_const(self.tcx,anon_const.def_id);();();self.
...
param_def_id:DefId)->ty::Const<'tcx>{{;};let did=hir_ct.def_id;{;};{;};self.tcx.
feed_anon_const_type(did,self.tcx.type_of(param_def_id));();3;let ct=ty::Const::
from_anon_const(self.tcx,did);3;;self.register_wf_obligation(ct.into(),self.tcx.
...
traits::ObligationCause::new(span,self.body_id,code);3;;self.register_predicate(
...
scope_tree=self.tcx.region_scope_tree(def_id);;;let rvalue_scopes={rvalue_scopes
::resolve_rvalue_scopes(self,scope_tree,def_id)};3;;let mut typeck_results=self.
typeck_results.borrow_mut();3;3;typeck_results.rvalue_scopes=rvalue_scopes;3;}#[
...
);({});{;};let coroutines=std::mem::take(&mut*self.deferred_coroutine_interiors.
borrow_mut());();();debug!(?coroutines);();for&(expr_def_id,body_id,interior)in 
...
"Failed to unify coroutine interior type");;;let mut obligations=ok.obligations;
obligations.extend(self. fulfillment_cx.borrow_mut().drain_unstalled_obligations
(&self.infcx));3;;let obligations=obligations.into_iter().map(|o|(o.predicate,o.
cause)).collect();3;3;debug!(?obligations);3;3;self.typeck_results.borrow_mut().
...
;3;if!errors.is_empty(){;self.adjust_fulfillment_errors_for_expr_obligation(&mut
errors);3;;let errors_causecode=errors.iter().map(|e|(e.obligation.cause.span,e.
root_obligation.cause.code().clone())).collect::<Vec<_>>();();3;self.err_ctxt().
report_fulfillment_errors(errors);if true{};let _=||();if true{};if true{};self.
...
fulfillment_cx.borrow_mut().select_where_possible(self);3;if!result.is_empty(){;
mutate_fulfillment_errors(&mut result);let _=();let _=();let _=();let _=();self.
adjust_fulfillment_errors_for_expr_obligation(&mut result);();3;self.err_ctxt().
...
TypeAndMut<'tcx>{();let ret_ty=method.sig.output();3;ret_ty.builtin_deref(true).
...
bool{;let self_ty=self.shallow_resolve(self_ty);;debug!(?self_ty);match*self_ty.
...
formal_ret);();();let ret_ty=expected_ret.only_has_type(self)?;();if formal_ret.
...
return None;{;};}}}{;};let expect_args=self.fudge_inference_if_ok(||{();let ocx=
...
instantiate(self.tcx,args);;;self.write_args(hir_id,args);self.write_resolution(
hir_id,Ok((def_kind,def_id)));({});({});let code=match lang_item{hir::LangItem::
...
qpath,hir_id,span);3;;let(ty,qself,item_segment)=match*qpath{QPath::Resolved(ref
opt_qself,path)=>{3;return(path.res,opt_qself.as_ref().map(|qself|self.lower_ty(
qself)),path.segments,);3;}QPath::TypeRelative(ref qself,ref segment)=>{;let ty=
...
return(def,Some(ty),slice::from_ref(&**item_segment));{();};}({});let item_name=
...
TraitMissingMethod,|_err|{},);({});}r}).or_else(|error|{{;};let guar=self.dcx().
...
;;if result.is_ok(){;self.register_wf_obligation(ty.raw.into(),qself.span,traits
::WellFormed(None));;};self.write_resolution(hir_id,result);(result.map_or(Res::
...
peel_refs());();3;let prev_call_mutates_and_returns_unit=||{self.typeck_results.
...
let mut sp=MultiSpan::from_span(path_segment.ident.span);3;3;sp.push_span_label(
...
format!("`{}`",segment.ident),_=>"its receiver".to_string(),}),);{();};{();};let
...
,_=>bug!("instantiate_value_path on {:?}",res),};;;let mut user_self_ty=None;let
...
ty_adt_def().unwrap();3;;user_self_ty=Some(UserSelfTy{impl_def_id:adt_def.did(),
self_ty:self_ty.raw});3;;is_alias_variant_ctor=true;;}Res::Def(DefKind::AssocFn|
DefKind::AssocConst,def_id)=>{3;let assoc_item=tcx.associated_item(def_id);;;let
...
.len()==1{3;let self_ty=self_ty.expect("UFCS sugared assoc missing Self").raw;;;
user_self_ty=Some(UserSelfTy{impl_def_id:container_id,self_ty});3;}}}}_=>{}};let
indices:FxHashSet<_>=(generic_segments.iter()).map(|GenericPathSegment(_,index)|
index).collect();();3;let generics_has_err=self.lowerer().prohibit_generic_args(
...
infer_args_for_err=FxHashSet::default();{();};{();};let mut explicit_late_bound=
ExplicitLateBound::No;;for&GenericPathSegment(def_id,index)in&generic_segments{;
...
(e);();}}3;let has_self=generic_segments.last().is_some_and(|GenericPathSegment(
...
non_enum_variant().ctor.unwrap();3;;let vis=tcx.visibility(ctor_def_id);;if!vis.
...
Res::Def(DefKind::Ctor(CtorOf::Struct,ctor_kind),ctor_def_id);3;3;let user_args=
Self::user_args_for_adt(ty);;;user_self_ty=user_args.user_self_ty;(new_res,Some(
...
"Self { /* fields */ }",Applicability::HasPlaceholders,);3;}}};let reported=err.
emit();;;return(Ty::new_error(tcx,reported),res);}}}else{(res,None)};let def_id=
...
())},};3;;struct CtorGenericArgsCtxt<'a,'tcx>{fcx:&'a FnCtxt<'a,'tcx>,span:Span,
generic_segments:&'a[GenericPathSegment], infer_args_for_err:&'a FxHashSet<usize
>,segments:&'tcx[hir::PathSegment<'tcx>],}3;;impl<'tcx,'a>GenericArgsLowerer<'a,
...
segments[index].args{3;return(Some(data),self.segments[index].infer_args);3;}}3;
...
=>{*&*&();let tcx=self.fcx.tcx();*&*&();if has_default&&is_host_effect{self.fcx.
...
GenericArg<'tcx>{;let tcx=self.fcx.tcx();;match param.kind{GenericParamDefKind::
...
infer_args_for_err,segments,},)});3;3;self.write_user_type_annotation_from_args(
...
type_of(def_id);();();assert!(!args.has_escaping_bound_vars());();3;assert!(!ty.
skip_binder().has_escaping_bound_vars());3;3;let ty_instantiated=self.normalize(
...
,),);{();};}}}({});debug!("instantiate_value_path: type of {:?} is {:?}",hir_id,
ty_instantiated);;;self.write_args(hir_id,args);(ty_instantiated,res)}pub(crate)
...
param_env=self.param_env;;;let bounds=self.instantiate_bounds(span,def_id,args);
...
borrow_mut());3;match result{Ok(normalized_ty)=>normalized_ty,Err(errors)=>{;let
guar=self.err_ctxt().report_fulfillment_errors(errors);3;3;return Ty::new_error(
...
,ctxt:BreakableCtxt<'tcx>,f:F,)->(BreakableCtxt<'tcx>,R){3;let index;3;{;let mut
enclosing_breakables=self.enclosing_breakables.borrow_mut();*&*&();*&*&();index=
enclosing_breakables.stack.len();;;enclosing_breakables.by_id.insert(id,index);;
enclosing_breakables.stack.push(ctxt);3;}3;let result=f();3;3;let ctxt={;let mut
enclosing_breakables=self.enclosing_breakables.borrow_mut();();();debug_assert!(
enclosing_breakables.stack.len()==index+1);({});({});enclosing_breakables.by_id.
...
true;();();break;3;}}_=>(),}3;expr_id=parent_expr.hir_id;3;}contained_in_place}}
