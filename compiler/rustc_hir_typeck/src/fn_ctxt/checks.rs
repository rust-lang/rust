...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn report_arg_errors(
        ...
        let mut arg_mistake_ctxt = FnCallDiagCtxt::new(
            self,
            compatibility_diagonal,
            formal_and_expected_inputs,
            provided_args,
        ...
        if let Some(err) = arg_mistake_ctxt.check_wrap_args_in_tuple() {
            return err;
        }

        if let Some(fallback_error) = arg_mistake_ctxt.ensure_has_errors() {
        ...
        // It is what it is.
        if let Some(err) = arg_mistake_ctxt.filter_out_invalid_arguments()
            && arg_mistake_ctxt.errors.is_empty()
        {
            // We're done if we found errors, but we already emitted them.
            return err;
        }

        assert!(!arg_mistake_ctxt.errors.is_empty());
        ...
        if let Some(err) = arg_mistake_ctxt.check_single_incompatible() {
            return err;
        }
        ...
        arg_mistake_ctxt.maybe_optimize_extra_arg_suggestion();

        let mut err = arg_mistake_ctxt.initial_final_diagnostic();
        arg_mistake_ctxt.suggest_confusable(&mut err);

        // As we encounter issues, keep track of what we want to provide for the suggestion.

        // FIXME(scrabsha): where should we put this comment? :3
        ...
        let (mut suggestions, labels, suggestion_text) =
            arg_mistake_ctxt.labels_and_suggestion_text(&mut err);

        arg_mistake_ctxt.label_generic_mismatches(&mut err);
        arg_mistake_ctxt.append_arguments_changes(&mut suggestions);
        ...
        arg_mistake_ctxt.label_fn_like(
            &mut err,
            fn_def_id,
            arg_mistake_ctxt.callee_ty,
            call_expr,
            None,
            None,
            &arg_mistake_ctxt.matched_inputs,
            &arg_mistake_ctxt.formal_and_expected_inputs,
            arg_mistake_ctxt.call_metadata.is_method,
            tuple_arguments,
        );
        ...
        if let Some(suggestion_message) =
            ...
            && !arg_mistake_ctxt.call_is_in_macro()
        {
            let (suggestion_span, suggestion_code) = arg_mistake_ctxt.suggestion_code();
