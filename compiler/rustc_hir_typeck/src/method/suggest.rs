...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn impl_into_iterator_should_be_iterator(
        ...
        match *ty.peel_refs().kind() {
            ty::Param(param) => {
                let generics = self.tcx.generics_of(self.item_id);
    ...
    fn suggest_use_shadowed_binding_with_method(
        ...
        impl<'a, 'tcx> LetVisitor<'a, 'tcx> {
            ...
            fn is_sub_scope(&self, sub_id: hir::ItemLocalId, super_id: hir::ItemLocalId) -> bool {
                let scope_tree = self.fcx.tcx.region_scope_tree(self.fcx.item_id);
        ...
        {
            let body = self.tcx.hir_body_owned_by(self.item_id);
    ...
    fn suggest_method_not_found_because_of_unsatisfied_bounds(
        ...
        let mut ty_span = match rcvr_ty.kind() {
            ty::Param(param_type) => {
                Some(param_type.span_from_generics(self.tcx, self.item_id.to_def_id()))
            }
                    ...
                    (self_ty.kind(), parent_pred.kind().skip_binder())
                {
                    let node = match p.trait_ref.self_ty().kind() {
                        ty::Param(_) => {
                            ...
                            Some(self.tcx.hir_node_by_def_id(self.item_id))
                        }
                        ty::Adt(def, _) => {
    ...
    fn suggest_calling_field_as_fn(
        ...
        if let Some((field, field_ty)) = field_receiver {
            let scope = tcx.parent_module_from_def_id(self.item_id);
        ...
        F: FnOnce(Vec<String>, Vec<String>, Span),
    {
        ...
        let scope = self.tcx.parent_module_from_def_id(self.item_id);
    ...
    fn suggest_traits_to_import(
        ...
        if !candidates.is_empty() {
            ...
            if let Some(param) = param_type {
                let generics = self.tcx.generics_of(self.item_id.to_def_id());
