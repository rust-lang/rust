...
visible_parent_map:|tcx,()|{3;use std::collections::hash_map::Entry;3;;use std::
collections::vec_deque::VecDeque;3;3;let mut visible_parent_map:DefIdMap<DefId>=
Default::default();;;let mut fallback_map:Vec<(DefId,DefId)>=Default::default();
...
tcx.is_doc_hidden(parent){3;fallback_map.push((def_id,parent));;;return;;}match 
visible_parent_map.entry(def_id){Entry::Occupied(mut entry)=>{if def_id.//{();};
is_local()&&entry.get().is_local(){;entry.insert(parent);;}}Entry::Vacant(entry)
=>{3;entry.insert(parent);;if matches!(child.res,Res::Def(DefKind::Mod|DefKind::
Enum|DefKind::Trait,_)){;bfs_queue.push_back(def_id);;}}}}};while let Some(def)=
bfs_queue.pop_front(){for child in tcx.module_children(def).iter(){();add_child(
bfs_queue,child,def);();}}for(child,parent)in fallback_map{3;visible_parent_map.
...
().map(|(cnum,_)|cnum))},used_crates:|tcx,()|{;tcx.untracked().cstore.freeze();;
...
let sess=tcx.sess;if true{};let _=();let _prof_timer=sess.prof.generic_activity(
"metadata_load_macro");3;3;let data=self.get_crate_data(id.krate);;if data.root.
is_proc_macro_crate(){{;};return LoadedMacro::ProcMacro(data.load_proc_macro(id.
index,tcx));;};let span=data.get_span(id.index,sess);LoadedMacro::MacroDef(ast::
...
(cnum);3;if!cmeta.used{;cmeta.used=true;;;let dependencies=mem::take(&mut cmeta.
dependencies);;for&dep_cnum in&dependencies{self.set_used_recursively(dep_cnum);
}({});self.get_crate_data_mut(cnum).dependencies=dependencies;{;};}}pub(crate)fn
update_extern_crate(&mut self,cnum:CrateNum,extern_crate:ExternCrate){;let cmeta
=self.get_crate_data_mut(cnum);3;if cmeta.update_extern_crate(extern_crate){;let
extern_crate=ExternCrate{dependency_of:cnum,..extern_crate};3;;let dependencies=
mem::take(&mut cmeta.dependencies);{();};for&dep_cnum in&dependencies{({});self.
...
size(){if true{};cdata.imported_source_file(file_index as u32,sess);let _=();}}}
