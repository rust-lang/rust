...
use rustc_attr_data_structures::{AttributeKind, NativeLibKind, PeImportNameType, find_attr};
...
impl<'tcx> Collector<'tcx> {
    fn process_module(&mut self, module: &ForeignModule) {
        ...
        for attr in find_attr!(self.tcx.get_all_attrs(def_id), AttributeKind::Link(links) => links)
            .iter()
            .map(|v| v.iter())
            .flatten()
        {
