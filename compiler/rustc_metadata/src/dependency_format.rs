...
//! 3. Each dylib can contain more than one library (due to static linking), and can also bring in
...
//!
//! 1. If static linking is required, then require all upstream dependencies to be available as
//!    rlibs. If not, generate an error.
//! 2. If static linking is requested (generating an executable), then attempt to use all upstream
//!    dependencies as rlibs. If any are not found, bail out and continue to step 3.
//! 3. Static linking has failed, at least one library must be dynamically linked. Apply a heuristic
//!    by greedily maximizing the number of dynamically linked libraries.
//! 4. Each upstream dependency available as a dynamic library is registered. The dependencies all
//!    propagate, adding to a map. It is possible for a dylib to add a static library as a
//!    dependency, but it is illegal for two dylibs to add the same static library as a dependency.
//!    The same dylib can be added twice. Additionally, it is illegal to add a static dependency
//!    when it was previously found as a dylib (and vice versa)
//! 5. After all dynamic dependencies have been traversed, re-traverse the remaining dependencies
//!    and add them statically (if they haven't been added already).
//!
