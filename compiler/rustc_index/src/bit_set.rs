...
impl<T: Idx> GrowableBitSet<T> {
    ...
    pub fn new_empty() -> GrowableBitSet<T> {
        GrowableBitSet { domain_size: 0, words: Vec::new(), marker: PhantomData }
    }

    pub fn with_capacity(capacity: usize) -> GrowableBitSet<T> {
        let words = num_words(capacity);
        GrowableBitSet { domain_size: capacity, words: vec![0; words], marker: PhantomData }
    }
...
impl<T: Idx> From<BitSet<T>> for GrowableBitSet<T> {
    fn from(bit_set: BitSet<T>) -> Self {
        let mut new = GrowableBitSet::with_capacity(bit_set.domain_size());
...
impl<R: Idx, C: Idx> BitMatrix<R, C> {
    ...
    pub fn from_row_n(row: &BitSet<C>, num_rows: usize) -> BitMatrix<R, C> {
        ...
        //let words_per_row = num_words(num_columns);
        //assert_eq!(words_per_row, row.words().len()); // FIXME
        BitMatrix {
            num_rows,
            num_columns,
            words: iter::from_fn(|| Some(row.words_wide())).take(num_rows).flatten().collect(),
            marker: PhantomData,
        }
    }
    ...
    pub fn union_row_with(&mut self, with: &BitSet<C>, write: R) -> bool {
        ...
        let (write_start, write_end) = self.range(write);
        let mut changed = false;
        for (read_word, write_index) in iter::zip(with.words_wide(), write_start..write_end) {
            let word = self.words[write_index];
            let new_word = word | read_word;
