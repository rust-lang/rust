...
impl<'a> Iterator for Parser<'a> {
    ...
    fn next(&mut self) -> Option<Piece<'a>> {
        if let Some(&(Range { start, end }, ix, ch)) = self.input_vec.get(self.index) {
            match ch {
                '{' => {
                    self.index += 1;
                    if let Some(&(_, i, '{')) = self.input_vec.get(self.index) {
                        self.index += 1;
                        ...
                        } else if let Some(&(_, _, c)) = self.input_vec.get(self.index) {
                            match c {
                ...
                _ => Some(Piece::Lit(self.string(ix))),
            }
        } else {
            // end of input
            if self.is_source_literal {
...
impl<'a> Parser<'a> {
    ...
    fn consume_pos(&mut self, ch: char) -> Option<(Range<usize>, usize)> {
        if let Some((r, i, c)) = self.input_vec.get(self.index) {
            if ch == *c {
                self.index += 1;
    ...
    fn consume_closing_brace(&mut self, arg: &Argument<'_>) -> Option<Range<usize>> {
        ...
        let (range, description) = if let Some((r, _, c)) = self.input_vec.get(self.index) {
            if *c == '}' {
                self.index += 1;
                return Some(r.clone());
            }
    ...
    fn ws(&mut self) {
        let rest = &self.input_vec[self.index..];
    ...
    fn string(&mut self, start: usize) -> &'a str {
        while let Some((r, i, c)) = self.input_vec.get(self.index) {
            match c {
                '{' | '}' => {
    ...
    fn argument(&mut self) -> Argument<'a> {
        ...
        let position_span = self.index2range(start_ix).start..self.index2range(end_ix).start;
    ...
    fn position(&mut self) -> Option<Position<'a>> {
        if let Some(i) = self.integer() {
            ...
            match self.input_vec.get(self.index) {
                Some((range, _, c)) if rustc_lexer::is_id_start(*c) => {
                    ...
                    if word == "r" {
                        if let Some((r, _, '#')) = self.input_vec.get(self.index) {
                            if self
                                .input_vec
                                .get(self.index + 1)
                                .is_some_and(|(_, _, c)| rustc_lexer::is_id_start(*c))
                            {
                                ...
                                let full_span = start..self.index2range(self.index).start;
    ...
    fn index2pos(&self, index: usize) -> usize {
        if let Some(&(_, pos, _)) = self.input_vec.get(index) { pos } else { self.input.len() }
    }

    fn index2range(&self, index: usize) -> Range<usize> {
    ...
    fn format(&mut self) -> FormatSpec<'a> {
        ...
        // fill character
        if let Some(&(ref r, _, c)) = self.input_vec.get(self.index) {
            if let Some((_, _, '>' | '<' | '^')) = self.input_vec.get(self.index + 1) {
                self.index += 1;
                spec.fill = Some(c);
        ...
        if !havewidth {
            ...
            if spec.width != CountImplied {
                let end = self.index2range(self.index).start;
                spec.width_span = Some(self.index2range(start_ix).start..end);
            }
        }
        ...
        if let Some((range, _)) = self.consume_pos('.') {
            ...
            spec.precision_span = Some(range.start..self.index2range(self.index).start);
        }
        ...
        if self.consume('x') {
            ...
            spec.ty = "?";
            if let Some((r, _, c)) = self.input_vec.get(self.index) {
                match c {
            ...
            spec.ty = self.word();
            if !spec.ty.is_empty() {
                let start = self.index2range(start_ix).start;
                let end = self.index2range(self.index).start;
    ...
    fn count(&mut self) -> Count<'a> {
        if let Some(i) = self.integer() {
            ...
            if word.is_empty() {
                ...
                CountIsName(word, self.index2range(start_ix).start..r.start)
            } else {
                self.index = start_ix;
                CountImplied
            }
    ...
    fn word(&mut self) -> &'a str {
        ...
        let (err_end, end): (usize, usize) = loop {
            if let Some(&(ref r, i, c)) = self.input_vec.get(self.index) {
        ...
        let word = &self.input[self.index2pos(index)..end];
        if word == "_" {
            self.errors.push(ParseError {
                ...
                span: self.index2range(index).start..err_end,
                secondary_label: None,
                suggestion: Suggestion::None,
            });
        }
    ...
    fn integer(&mut self) -> Option<usize> {
        ...
        while let Some(&(_, _, c)) = self.input_vec.get(self.index) {
        ...
        if overflow {
            let overflowed_int =
                &self.input[self.index2pos(start_index)..self.index2pos(self.index)];
            self.errors.push(ParseError {
                description: format!(
                ...
                note: None,
                label: "integer out of range for `usize`".into(),
                span: self.index2range(start_index).start..self.index2range(self.index).end,
                secondary_label: None,
                suggestion: Suggestion::None,
            });
        }
