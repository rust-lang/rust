...
/// Compute if/when loans are invalidated.
pub(super) fn compute_loan_invalidations<'tcx>(
    tcx: TyCtxt<'tcx>,
    body: &Body<'tcx>,
    borrow_set: &BorrowSet<'tcx>,
) -> IndexVec<BorrowIndex, Vec<Location>> {
    let dominators = body.basic_blocks.dominators();
    let mut visitor = LoanInvalidationsGenerator {
        borrow_set,
        tcx,
        body,
        dominators,
        loan_invalidated_at: IndexVec::from_fn_n(|_| Default::default(), borrow_set.len()),
    };
    visitor.visit_body(body);
    visitor.loan_invalidated_at
}
...
struct LoanInvalidationsGenerator<'a, 'tcx> {
    ...
    loan_invalidated_at: IndexVec<BorrowIndex, Vec<Location>>,
}
...
impl<'a, 'tcx> Visitor<'tcx> for LoanInvalidationsGenerator<'a, 'tcx> {
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        ...
        match &terminator.kind {
            ...
            TerminatorKind::Drop { place: drop_place, replace, .. } => {
                let write_kind =
            ...
            TerminatorKind::Yield { value, resume, resume_arg, drop: _ } => {
                ...
                let resume_location = resume.start_location();
                for (i, data) in self.borrow_set.iter_enumerated() {
                    if borrow_of_local_data(data.borrowed_place) {
                        self.loan_invalidated_at[i].push(resume_location);
                    }
                }
            ...
            | TerminatorKind::CoroutineDrop => {
                ...
                for (i, data) in self.borrow_set.iter_enumerated() {
                    if borrow_of_local_data(data.borrowed_place) {
                        self.loan_invalidated_at[i].push(start_location);
                    }
                }
            }
...
impl<'a, 'tcx> LoanInvalidationsGenerator<'a, 'tcx> {
    ...
    fn emit_loan_invalidated_at(&mut self, b: BorrowIndex, l: Location) {
        self.loan_invalidated_at[b].push(l);
    }
