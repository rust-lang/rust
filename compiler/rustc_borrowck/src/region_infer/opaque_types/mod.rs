...
///
/// We then use apply this inferred type to actually check all uses of the opaque.
...
/// This computes the actual hidden types of the opaque type and maps them to their
/// definition site. Outside of registering the computed concrete types this function
/// does not mutate the current borrowck state.
///
/// While it may fail to infer the hidden type and report errors, we always apply
...
/// recheck all uses of the opaque regardless.
fn compute_concrete_opaque_types<'tcx>(
    ...
    map_defining_uses_to_definition_site(root_cx, &rcx, &defining_uses, &mut errors);
    errors
}
...
fn map_defining_uses_to_definition_site<'tcx>(
    ...
    for &DefiningUse { opaque_type_key, ref arg_regions, hidden_type } in defining_uses {
        ...
        let mut visitor = ToArgRegionsFolder::new(rcx, arg_regions);
        let hidden_type = match hidden_type.try_fold_with(&mut visitor) {
