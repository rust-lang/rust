...
use rustc_middle::ty::{self, PlaceholderRegion, RegionVid, Ty, TyCtxt, TypeFoldable};
...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    #[instrument(skip(self, infcx, polonius_output), level = "debug")]
    pub(super) fn solve(
        &mut self,
        infcx: &InferCtxt<'tcx>,
        body: &Body<'tcx>,
    ...
    // FIXME(amandasystems) hash apropriate parts of stuff to help with the deduplication
    ...
    fn try_promote_type_test(
        ...
        // trivial.
        // FIXME(amandasystems) this is probably a bug!
        //assert!(found_outlived_universal_region);
        if !found_outlived_universal_region {
            eprintln!("BUG: no universal region found!!!")
        }
        None
    }
