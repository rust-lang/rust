...
pub(super) fn generate<'a, 'tcx>(
    ...
    let relevant_live_locals = if typeck.tcx().sess.opts.unstable_opts.polonius.is_next_enabled() {
        compute_relevant_live_locals(typeck.tcx(), typeck.body, |r| {
        ...
        compute_relevant_live_locals(typeck.tcx(), typeck.body, |r| free_regions.contains(&r))
    };

    trace::trace(typeck, location_map, flow_inits, move_data, relevant_live_locals);
