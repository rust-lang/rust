...
impl<'tcx> RegionInferenceContext<'tcx> {
    pub(crate) fn get_var_name_and_span_for_region(
        ...
        trace!("get_var_name_and_span_for_region(fr={fr:?})");
        assert!(self.universal_regions().is_universal_region(fr));

        trace!("get_var_name_and_span_for_region: attempting upvar");
    ...
    pub(crate) fn get_upvar_index_for_region(
        ...
        let upvar_index =
            self.universal_regions().defining_ty.upvar_tys().position(|upvar_ty| {
                trace!("get_upvar_index_for_region: upvar_ty={upvar_ty:?}");
                tcx.any_free_region_meets(&upvar_ty, |r| {
                    let r = r.as_var();
                    trace!("get_upvar_index_for_region: r={r:?} fr={fr:?}");
                    r == fr
                })
            })?;
    ...
    pub(crate) fn get_upvar_name_and_span_for_region(
        ...
        trace!("get_upvar_name_and_span_for_region: upvar_hir_id={upvar_hir_id:?}");
    ...
    pub(crate) fn get_argument_index_for_region(
        ...
        let argument_index =
            self.universal_regions().unnormalized_input_tys.iter().skip(implicit_inputs).position(
                |arg_ty| {
                    trace!("get_argument_index_for_region: arg_ty = {arg_ty:?}");
    ...
    pub(crate) fn get_argument_name_and_span_for_region(
        ...
        trace!("get_argument_name_and_span_for_region: argument_local={argument_local:?}");
