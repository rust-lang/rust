...
impl<'cx, 'tcx> MirBorrowckCtxt<'cx, 'tcx> {
    ...
    pub(crate) fn report_use_while_mutably_borrowed(
        ...
        // If the attempted use is in a closure then we do not care about the path span of the place
        // we are currently trying to use we call `var_span_label` on `borrow_spans` to
        ...
        impl<'hir> Visitor<'hir> for ClosureFinder<'hir> {
            ...
            fn visit_expr(&mut self, ex: &'hir hir::Expr<'hir>) {
                ...
                if let hir::ExprKind::Closure(closure) = ex.kind
                    ...
                    // FIXME: actually support such cases (need to figure out how to move
                    // from the capture place to original local)
    ...
    fn report_temporary_value_does_not_live_long_enough(
        ...
        match explanation {
            ...
            | BorrowExplanation::UsedLaterWhenDropped { .. } => {
                ...
                /// `FormatArguments`, which is a special case since it's generated
                /// by the compiler.
        ...
        let (sugg_span, suggestion) = match tcx.sess.source_map().span_to_snippet(args_span) {
            Ok(string) => {
                let coro_prefix = if string.starts_with("async") {
                    // `async` is 5 chars long. Not using `.len()` to avoid the cast from `usize` to
                    // `u32`
                    Some(5)
    ...
    fn annotate_fn_sig(
        ...
        //
        // 1. If there are named arguments, then we should highlight the return type and highlight
        //    any of the arguments that are also references with that lifetime. If there are no
        //    arguments that have the same lifetime as the return type, then don't highlight
        //    anything.
        // 2. The return type is a reference with an anonymous lifetime. If this is the case, then
        //    we can take advantage of (and teach) the lifetime elision rules.
        //
        ...
        //    return type.
        // 3. The return type is not a reference. In this case, we don't highlight anything.
