...
use crate::{
    ClosureRegionRequirements, ConcreteOpaqueTypes, PropagatedBorrowCheckResults,
    YieldDoMirBorrowck, resume_do_mir_borrowck, start_do_mir_borrowck,
};
...
pub(super) struct BorrowCheckRootCtxt<'tcx> {
    ...
    partial_results: FxHashMap<LocalDefId, Option<YieldDoMirBorrowck<'tcx>>>,
    ...
    fn compute_partial_results(&mut self, def_id: LocalDefId) {
        ...
        if !self.partial_results.contains_key(&def_id) {
            ...
            let relies_on_opaques = result.infcx.has_opaque_types_in_storage()
                || !result.yield_compute_regions.deferred_closure_requirements.is_empty();
