...
pub struct SerializedDepGraph<D> {
    ...
    /// Used to lookup names of dep kinds.
    deps: Arc<D>,
    ...
    fn add_to_index(
        ...
        if index.insert(node.hash, idx).is_some() {
            ...
            if node.kind != D::DEP_KIND_NULL && node.kind != D::DEP_KIND_SIDE_EFFECT {
                let name = self.deps.name(node.kind);
                panic!(
...
impl<D: Deps> SerializedDepGraph<D> {
    pub fn empty(deps: &Arc<D>, sess: &Session) -> Self {
        SerializedDepGraph {
    ...
    #[instrument(level = "debug", skip(d, deps, sess))]
    pub fn decode(
        d: &mut MemDecoder<'_>,
        deps: &Arc<D>,
        sess: &Session,
    ) -> Arc<SerializedDepGraph<D>> {
