...
pub trait Emitter: Translate {
    ...
    ///
    /// * If the current `Diagnostic` has only one visible `CodeSuggestion`, we format the `help`
    ///   suggestion depending on the content of the substitutions. In that case, we return the
    ///   modified span only.
    ///
    /// * If the current `Diagnostic` has multiple suggestions, we return the original
    ///   `primary_span` and the original suggestions.
    ...
    fn draw_line(
        ...
        let code: String = source_string
            ...
            .take_while(|ch| {
                ...
                // is. For now, just accept that sometimes the code line will be
                // longer than desired.
...
const OUTPUT_REPLACEMENTS: &[(char, &str)] = &[
    ...
    ('\u{200D}', ""), /* Replace ZWJ with nothing for consistent terminal output of grapheme
                     * clusters. */
