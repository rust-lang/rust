...
pub trait Emitter {
    ...
    fn fix_multispans_in_extern_macros_and_render_macro_backtrace(
            ...
            && let Some((_, _, false, _, _)) = has_macro_spans.last()
        {
            ...
            let descr = macro_kind.descr();
            let msg = format!("this {level} originates in the {descr} `{name}`{and_then}");
            ...
            if let Some(source_map) = self.source_map() {
                    ...
                    && !and_then.is_empty()
                {
                    ...
                    // error produced and there are more levels of macro nesting.
                    span.push_span_label(*sp, msg);
                } else {
                    // We point at the macro def line, but without an underline or message, we
                    // leave it implied.
                    span.push_span_context(sp.shrink_to_lo());
                }
            } else {
                children.push(Subdiag {
                    level: Sublevel::Note,
