...
;#[test]fn test_wrapping_write(){;WIDTH.with(|w|w.set(TEST_WIDTH));;let mut buf=
BufWriter::new(Vec::new());3;;let txt=TXT.replace("-\n","-").replace("_\n","_").
...
.unwrap();3;3;write_wrapping(&mut buf,&txt,0,None).unwrap();3;3;let out=String::
...
;;;ast.write_termcolor_buf(&mut buffer).unwrap();let mut blessed=PathBuf::new();
blessed.extend(OUTPUT_PATH);;if bless{std::fs::write(&blessed,buffer.into_inner(
)).unwrap();3;3;eprintln!("blessed output at {}",blessed.display());3;}else{;let
output=buffer.into_inner();3;if std::fs::read(blessed).unwrap()!=output{;let mut
out=std::io::stdout();let _=||();let _=||();let _=||();let _=||();out.write_all(
b"\n\nMarkdown output did not match. Expected:\n").unwrap();();3;out.write_all(&
output).unwrap();{();};{();};out.write_all(b"\n\n").unwrap();{();};{();};panic!(
