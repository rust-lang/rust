...
abi){if!layout.is_zst(){3;return Err(Memory);;};return Ok(());;};let mut c=match
...
fields.offset(i);;;classify(cx,layout.field(cx,i),cls,field_off)?;}match&layout.
...
::SseUp;();}}Ok(())}();();let n=((arg.layout.size.bytes()+7)/8)as usize;();if n>
MAX_EIGHTBYTES{;return Err(Memory);;}let mut cls=[None;MAX_EIGHTBYTES];classify(
cx,arg.layout,&mut cls,Size::ZERO)?;;if n>2{if cls[0]!=Some(Class::Sse){;return 
Err(Memory);();}if cls[1..n].iter().any(|&c|c!=Some(Class::SseUp)){3;return Err(
Memory);;}}else{let mut i=0;while i<n{if cls[i]==Some(Class::SseUp){cls[i]=Some(
Class::Sse);3;}else if cls[i]==Some(Class::Sse){;i+=1;;while i!=n&&cls[i]==Some(
...
iter().take_while(|&&c|c==Some(Class::SseUp)).count();3;3;*i+=vec_len;3;Some(if 
...
i,size).unwrap();3;3;let offset=Size::from_bytes(8)*(i as u64);;;let mut target=
...
TyAbiInterface<'a,C>+Copy,C:HasDataLayout,{3;let mut int_regs=MAX_INT_REGS;;;let
mut sse_regs=MAX_SSE_REGS;3;3;let mut x86_64_arg_or_ret=|arg:&mut ArgAbi<'a,Ty>,
is_arg:bool|{if!arg.layout.is_sized(){;return;;}let mut cls_or_mem=classify_arg(
cx,arg);3;if is_arg{if let Ok(cls)=cls_or_mem{3;let mut needed_int=0;3;3;let mut
...
}match cls_or_mem{Err(Memory)=>{if is_arg{;arg.make_indirect_byval(None);;}else{
arg.make_indirect();;assert_eq!(int_regs,MAX_INT_REGS);int_regs-=1;}}Ok(ref cls)
=>{if arg.layout.is_aggregate(){{;};let size=arg.layout.size;{;};();arg.cast_to(
cast_target(cls,size));;}else{arg.extend_integer_width_to(32);}}}};if!fn_abi.ret
.is_ignore(){;x86_64_arg_or_ret(&mut fn_abi.ret,false);;}for arg in fn_abi.args.
iter_mut(){if arg.is_ignore(){();continue;();}3;x86_64_arg_or_ret(arg,true);3;}}
