...
//!   need to configure and install a local wasm32-wasi-preview1-threads toolchain.
//!
//! * Second, one of the primary use cases of LLVM's new wasm backend and the wasm support in LLD is
//!   that any compiled language can interoperate with any other. To that the
//!   `wasm32-wasi-preview1-threads` target is the first with a viable C standard library and
//!   sysroot common definition, so we want Rust and C/C++ code to interoperate when compiled to
...
//!    `clang` binary that supports `wasm32-wasi-preview1-threads` and is configured with the
//!    `wasm32-wasi-preview1-threads` sysroot. This will cause Rust code to be linked against the
//!    libc.a that the specified `clang` provides.
//!
