...
fn entry_fn(tcx: TyCtxt<'_>, (): ()) -> Option<(DefId, EntryFnType)> {
    ...
    if tcx.hir().attrs(CRATE_HIR_ID).contains(sym::no_main) {
        return None;
    }
...
fn entry_point_type(ctxt: &EntryContext<'_>, id: ItemId, at_root: bool) -> EntryPointType {
    ...
    if attrs.contains(sym::start) {
        EntryPointType::Start
    } else if attrs.contains(sym::rustc_main) {
...
fn attr_span_by_symbol(ctxt: &EntryContext<'_>, id: ItemId, sym: Symbol) -> Option<Span> {
    let attrs = ctxt.tcx.hir().attrs(id.hir_id());
    attrs.find_by_name(sym).map(|attr| attr.span)
}
