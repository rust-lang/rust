...
pub(crate) fn check_crate(
    ...
    // These could be generated by the compiler rather than called by user code.
    // As such it is possible they are not implicitly added to the `missing`
    // array unless we do so here.
    if items.panic_impl().is_none() {
        items.missing.push(LangItem::PanicImpl);
    }
