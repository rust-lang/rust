...
use rustc_hir::attrs::{
    ...
    InlineAttr, LintAttribute, MirDialect, MirPhase, ReprAttr, SanitizerSet,
};
...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_attributes(
        ...
        for attr in attrs {
            match attr {
                ...
                Attribute::Parsed(AttributeKind::LintAttribute { sub_attrs, .. }) => self.check_lint_attr(hir_id, sub_attrs),
                Attribute::Parsed(
    ...
    fn check_lint_attr(&self, hir_id: HirId, sub_attrs: &[LintAttribute]) {
        for LintAttribute { attr_span, lint_instances, attr_style, .. } in sub_attrs {
            if !lint_instances.iter().any(|id| id.lint_name() == sym::linker_messages) {
                return;
            };
