...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_attributes(
        ...
        for attr in attrs {
            match attr.path().as_slice() {
                ...
                [sym::proc_macro_warning, ..] => {
                    self.check_proc_macro_warning(hir_id, target, attr.span);
                }
                [sym::autodiff, ..] => {
    ...
    fn check_proc_macro_warning(&self, hir_id: HirId, target: Target, attr_span: Span) {
        if target != Target::Static {
            return;
        }
        ...
        if let Err(terr) = ocx.eq(&cause, param_env, expected, actual) {
            let mut diag =
                self.tcx.dcx().create_err(errors::ProcMacroWarningWrongType { span: hir_ty.span });
            infcx.err_ctxt().note_type_err(
                &mut diag,
                &cause,
                None,
