...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_one_parsed_attribute(
        ...
        match attr {
            ...
            AttributeKind::LintAttributes(sub_attrs) => self.check_lint_attr(hir_id, sub_attrs),
    ...
    fn check_lint_attr(&self, hir_id: HirId, sub_attrs: &[LintAttribute]) {
        for LintAttribute { attr_span, lint_instances, attr_style, .. } in sub_attrs {
            let dead_code_lint_in_instances =
                lint_instances.iter().any(|id| id.lint_name() == sym::dead_code_pub_in_binary);
            if !dead_code_lint_in_instances
                && !lint_instances.iter().any(|id| {
            ...
            if hir_id != CRATE_HIR_ID {
                ...
                let note = if dead_code_lint_in_instances
