...
/// `true` when `name` is a non-lifetime generic parameter declared on `generics`.
///
/// Used for `{ident}` placeholders in `#[diagnostic::on_unimplemented]` / `#[diagnostic::on_move]`.
fn generics_has_type_param_named(generics: &hir::Generics<'_>, name: Symbol) -> bool {
    generics.params.iter().any(|p| {
        !matches!(p.kind, GenericParamKind::Lifetime { .. })
            && matches!(p.name, ParamName::Plain(n) if n.name == name)
    })
}
            ...
            match attr {
                ...
                Attribute::Parsed(AttributeKind::OnUnimplemented { span, directive }) => {
                    ...
                    if let Some(directive) = directive.as_deref() {
                        ...
                        {
                            let is_rustc_attr = directive.is_rustc_attr;
                            late_validation::for_each_unknown_diagnostic_format_param(
                                directive,
                                |name| generics_has_type_param_named(generics, name),
                                |argument_name, sp| {
                                    self.tcx.emit_node_span_lint(
                ...
                Attribute::Parsed(AttributeKind::OnMove { span, directive }) => {
                    if let Some(v) =
                        late_validation::validate_diagnostic_on_move_target(&late_ctx, *span)
                    {
                        self.tcx.emit_node_span_lint(
                            MISPLACED_DIAGNOSTIC_ATTRIBUTES,
                            hir_id,
                            ...
                            late_validation::for_each_unknown_diagnostic_format_param(
                                ...
                                |name, sp| {
                                    self.tcx.emit_node_span_lint(
                                        ...
                                        hir_id,
                                        sp,
                                        errors::OnMoveMalformedFormatLiterals { name },
                                    );
                                },
                            );
                        }
