...
use rustc_hir::{
    self as hir, self, AssocItemKind, Attribute, AttributeKind, CRATE_HIR_ID, CRATE_OWNER_ID,
    FnSig, ForeignItem, HirId, Item, ItemKind, MethodKind, Repr, Safety, Target, TraitItem,
};
...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_rustc_pub_transparent(&self, attr_span: Span, span: Span, attrs: &[Attribute]) {
        if !attr::find_attr!(attrs, AttributeKind::Repr(r) => r.contains(&Repr::Transparent))
            .unwrap_or(false)
        {
