...
#[derive(Diagnostic)]
#[diag("`#[diagnostic::on_incomplete_macro_args]` can only be applied to macro definitions")]
struct DiagnosticOnIncompleteMacroArgsOnlyForMacros;
...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_attributes(
        ...
        for attr in attrs {
            match attr {
                ...
                Attribute::Parsed(AttributeKind::OnIncompleteMacroArgs { span, .. }) => {
                    self.check_diagnostic_on_incomplete_macro_args(*span, hir_id, target)
                }
    ...
    /// Checks if `#[diagnostic::on_incomplete_macro_args]` is applied to a macro definition.
    fn check_diagnostic_on_incomplete_macro_args(
        &self,
        attr_span: Span,
        hir_id: HirId,
        target: Target,
        ...
        if !matches!(target, Target::MacroDef) {
            self.tcx.emit_node_span_lint(
                ...
                hir_id,
                attr_span,
                DiagnosticOnIncompleteMacroArgsOnlyForMacros,
            );
        }
    }
