...
impl<'tcx> CheckAttrVisitor<'tcx> {
        ...
        for attr in attrs {
            ...
            match attr {
                ...
                Attribute::Parsed(AttributeKind::Inline(kind, attr_span)) => {
                    self.check_inline(hir_id, *attr_span, kind)
                }
                ...
                Attribute::Parsed(AttributeKind::AllowInternalUnstable(..)) => {
                    self.check_allow_internal_unstable(target, attrs)
                }
    ...
    fn check_inline(&self, hir_id: HirId, attr_span: Span, kind: &InlineAttr) {
    ...
    fn check_doc_alias_value(
        ...
        if let Some(location) = match target {
            ...
            | Target::Closure
            | Target::Impl { .. }
            | Target::WherePredicate => Some(target.singular_name()),
            Target::ExternCrate
            | Target::Use
            | Target::Static
            | Target::Const
    ...
    fn check_must_use(&self, hir_id: HirId, attr_span: Span, target: Target) {
        if let Target::Method(MethodKind::Trait { body: true }) = target {
            let parent_def_id = self.tcx.hir_get_parent_item(hir_id).def_id;
            let containing_item = self.tcx.hir_expect_item(parent_def_id);
            if let hir::ItemKind::Trait(..) = containing_item.kind {
                return; // OK
            }
            ...
            self.tcx.emit_node_span_lint(
                ...
                hir_id,
                attr_span,
                errors::MustUseNoEffect { target: target.singular_name() },
            );
        }
    }
    ...
    fn check_allow_internal_unstable(&self, target: Target, attrs: &[Attribute]) {
        match target {
            Target::Fn => {
                for attr in attrs {
