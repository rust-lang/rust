...
use rustc_hir::{ItemAttributes, OwnerId};
...
fn extract(attrs: &ItemAttributes<'_>) -> Option<Symbol> {
    attrs.with_name(sym::rustc_diagnostic_item).find_map(|attr| attr.value_str())
}
