...
fn maybe_record_as_seed<'tcx>(
    ...
    push_into_worklist: &mut impl FnMut(LocalDefId, ComesFromAllowExpect),
    unsolved_items: &mut Vec<LocalDefId>,
) {
    let allow_dead_code = has_allow_dead_code_or_lang_attr(tcx, owner_id.def_id);
    if let Some(comes_from_allow) = allow_dead_code {
        push_into_worklist(owner_id.def_id, comes_from_allow);
    }
    ...
    match tcx.def_kind(owner_id) {
        DefKind::Enum => {
            if let Some(comes_from_allow) = allow_dead_code {
                ...
                for variant in adt.variants().iter() {
                    push_into_worklist(variant.def_id.expect_local(), comes_from_allow);
                }
            }
        }
        ...
        DefKind::GlobalAsm => {
            ...
            push_into_worklist(owner_id.def_id, ComesFromAllowExpect::No);
        }
        DefKind::Const { .. } => {
...
struct SeedWorklists {
    worklist: Vec<(LocalDefId, ComesFromAllowExpect)>,
    deferred_seeds: Vec<(LocalDefId, ComesFromAllowExpect)>,
...
fn create_and_seed_worklist(tcx: TyCtxt<'_>) -> SeedWorklists {
    ...
    for (id, effective_vis) in tcx.effective_visibilities(()).iter() {
        if effective_vis.is_public_at_level(Level::Reachable) {
            deferred_seeds.push((*id, ComesFromAllowExpect::No));
        }
    }

    let mut push_into_worklist = |def_id, comes_from_allow| match comes_from_allow {
        ComesFromAllowExpect::Yes => deferred_seeds.push((def_id, ComesFromAllowExpect::Yes)),
        ComesFromAllowExpect::No => worklist.push((def_id, ComesFromAllowExpect::No)),
    };
