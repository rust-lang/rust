...
impl<'tcx> ExportableItemCollector<'tcx> {
    ...
    fn add_exportable(&mut self, def_id: LocalDefId) {
        self.catch_exportable_in_mod = true;
    ...
    fn walk_item_with_mod(&mut self, item: &'tcx hir::Item<'tcx>) {
        ...
        self.catch_exportable_in_mod = false;

        intravisit::walk_item(self, item);

        if self.catch_exportable_in_mod || self.in_exportable_mod {
            self.exportable_items.insert(def_id.to_def_id());
        }

        self.catch_exportable_in_mod = old_catch_exportable_in_mod;
...
impl<'tcx> ImplsOrderVisitor<'tcx> {
    fn new(tcx: TyCtxt<'tcx>) -> ImplsOrderVisitor<'tcx> {
        ImplsOrderVisitor { tcx, order: Default::default(), id: 0 }
    }
}
...
/// id to each impl according to the order of elements in the source code. However, there is
/// no "proper" way to get this id, so we use:
///   1. The sequential number of an impl during the `intravisit::Visitor` traversal.
///   2. A new attribute `#[rustc_stable_impl_id]` that outputs the impl id as a compiler error.
///      If the visitor's implementation is changed, the corresponding test will fail. Then
///      you will need to rewrite the implementation of this query to preserve order.
fn stable_order_of_exportable_impls<'tcx>(
    tcx: TyCtxt<'tcx>,
    _: LocalCrate,
) -> &'tcx FxIndexMap<DefId, usize> {
    let mut vis = ImplsOrderVisitor::new(tcx);
    tcx.hir().walk_toplevel_module(&mut vis);
