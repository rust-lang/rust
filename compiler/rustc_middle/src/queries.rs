...
rustc_queries! {
    ...
    query resolver_for_lowering_raw(_: ()) -> (&'tcx Steal<(ty::AstLoweringResolutionContext<'tcx>, Arc<ast::Crate>)>, &'tcx ty::ResolverGlobalCtxt) {
        eval_always
        no_hash
    ...
    query lower_to_hir_delayed(def_id: LocalDefId) -> hir::MaybeOwner<'tcx>  {
        eval_always
        desc { "getting the delayed crate HIR" }
    }

    query get_delayed_child_owner(def_id: LocalDefId) -> hir::MaybeOwner<'tcx>  {
        no_hash
        feedable
