...
impl EffectiveVisibility {
    ...
    fn update(&mut self, vis: Visibility, level: Level, tree: impl DefIdTree) -> bool {
        let mut changed = false;
        for l in Level::all_levels() {
            if l <= level {
                let current_effective_vis = self.at_level_mut(l);
        ...
        if let Some(inherited_effective_vis) = self.effective_vis(parent_id) {
            for l in Level::all_levels() {
                if level >= l {
                    ...
                    changed |= current_effective_vis.update(calculated_effective_vis, l, tree);
                }
            }
        } else {
            ...
            changed |= current_effective_vis.update(Visibility::Public, Level::Direct, tree);
        }
