...
impl<'a> StableHashingContext<'a> {
    ...
    pub fn set_hash_spans_as_parentless(&mut self, hash_spans_as_parentless: bool) {
        self.hashing_controls.hash_spans_as_parentless = hash_spans_as_parentless;
    }
    ...
    #[inline]
    fn hash_span_data(&mut self, span: SpanData, hasher: &mut StableHasher) {
        const TAG_VALID_SPAN: u8 = 0;
        const TAG_INVALID_SPAN: u8 = 1;
        const TAG_RELATIVE_SPAN: u8 = 2;

        if span.parent.is_some() && self.hashing_controls().hash_spans_as_parentless {
            let mut span = span;
            span.parent = None;
            self.hash_span_data(span, hasher);
            return;
        }
