...
fn typetree_from_ty_impl_inner<'tcx>(
    ...
    if let Some(inner) = unwrap_option_nonnull_ptr_like(tcx, ty) {
        dbg!("option of niche!");
        dbg!(&ty);
        // Option<NonNull<T>> and similar types use a niche to encode the Option/None variant,
        // so we can ignore it and return the inner tt directly.
        return typetree_from_ty_impl_inner(tcx, inner, depth + 1, visited, true);
    };
    ...
    if let Some(inner) = is_nonnull(tcx, ty) {
...
fn unwrap_option_nonnull_ptr_like<'tcx>(
    tcx: TyCtxt<'tcx>,
    ty: Ty<'tcx>,
) -> Option<Ty<'tcx>> {
    ...
    if !tcx.is_lang_item(def.did(), LangItem::Option) {
        dbg!("not an Option: {}", ty);
        return None;
    }

    let inner = args.type_at(0);

    // Accepted: Option<&T>, Option<&mut T>, Option<Box<T>>
    // Rejected: Option<*const T>, Option<*mut T>
    if inner.is_ref() || inner.is_box() {
        return inner.builtin_deref(true);
    }

    // Accepted: Option<NonNull<T>>
    if let Some(inner_ty) = is_nonnull(tcx, inner) {
        return Some(inner_ty);
    }
    dbg!("the wrong Option: {}", ty);
