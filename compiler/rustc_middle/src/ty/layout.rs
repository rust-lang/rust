    ...
    fn layout_raw_uncached(&self, ty: Ty<'tcx>) -> Result<&'tcx Layout, LayoutError<'tcx>> {
        ...
        Ok(match *ty.data() {
            ...
            ty::Ref(_, pointee, _) | ty::RawPtr(ty::TypeAndMut { ty: pointee, .. }) => {
                ...
                let metadata = match unsized_part.data() {
                    ty::Foreign(..) => {
                        return Ok(tcx.intern_layout(Layout::scalar(self, data_ptr)));
                    }
...
impl<'tcx> LayoutCx<'tcx, TyCtxt<'tcx>> {
    ...
    fn record_layout_for_printing_outlined(&self, layout: TyAndLayout<'tcx>) {
        ...
        let adt_def = match *layout.ty.data() {
        ...
        + HasParamEnv<'tcx>,
{
    ...
    fn field(this: TyAndLayout<'tcx>, cx: &C, i: usize) -> C::TyAndLayout {
        ...
        cx.layout_of(match *this.ty.data() {
            ty::Bool
            | ty::Char
            | ty::Int(_)
            | ty::Uint(_)
            ...
            ty::Ref(_, pointee, _) | ty::RawPtr(ty::TypeAndMut { ty: pointee, .. }) => {
                ...
                match tcx.struct_tail_erasing_lifetimes(pointee, cx.param_env()).data() {
    ...
    fn pointee_info_at(this: TyAndLayout<'tcx>, cx: &C, offset: Size) -> Option<PointeeInfo> {
        ...
        let pointee_info = match *this.ty.data() {
            ...
            _ => {
                ...
                if let Some(ref mut pointee) = result {
                    if let ty::Adt(def, _) = this.ty.data() {
    ...
    fn new_internal(
        ...
        let extra_args = if sig.abi == RustCall {
            ...
            if let Some(input) = sig.inputs().last() {
                if let ty::Tuple(tupled_arguments) = input.data() {
