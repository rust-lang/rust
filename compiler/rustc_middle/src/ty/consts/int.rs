...
impl ScalarInt {
    ...
    #[inline]
    pub fn to_atomic_ordering(self) -> std::intrinsics::AtomicOrdering {
        use std::intrinsics::AtomicOrdering::*;
        // Here we rely on `AtomicOrdering` having the same values in host and target!
        // We use `u32` as that's how valtrees encode the enum discriminant.
        let val = self.to_u32();
