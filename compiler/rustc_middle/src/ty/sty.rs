...
impl<'tcx> Ty<'tcx> {
    ...
    pub fn find_async_drop_impl_coroutine(self, tcx: TyCtxt<'tcx>) -> Ty<'tcx> {
    ...
    pub fn is_async_drop_in_place_poll_fn(self, tcx: TyCtxt<'_>) -> bool {
        match self.kind() {
            ty::Coroutine(def, ..) => tcx.is_async_drop_in_place_poll_fn(*def),
            _ => false,
        }
    }
