...
pub struct Binder<'tcx, T> {
    ...
    bound_clauses: &'tcx List<ty::Clause<'tcx>>,
}
    ...
    T: TypeVisitable<TyCtxt<'tcx>>,
{
    ...
    pub fn dummy(value: T) -> Binder<'tcx, T> {
        ...
        Binder { value, bound_vars: ty::List::empty(), bound_clauses: ty::List::empty() }
    }
    ...
    pub fn bind_with_vars(value: T, bound_vars: &'tcx List<BoundVariableKind>) -> Binder<'tcx, T> {
        ...
        Binder { value, bound_vars, bound_clauses: ty::List::empty() }
    }
    ...
    pub fn bind_with_vars_and_clauses(
        ...
        Binder { value, bound_vars, bound_clauses }
    }
}
...
impl<'tcx, T> rustc_type_ir::BoundVars<TyCtxt<'tcx>> for ty::Binder<'tcx, T> {
    ...
    fn has_no_bound_vars(&self) -> bool {
        // FIXME(non_lifetime_binders): Should check clauses are empty too?
...
impl<'tcx, T> Binder<'tcx, T> {
    ...
    pub fn bound_clauses(&self) -> &'tcx List<ty::Clause<'tcx>> {
        self.bound_clauses
    }
    ...
    pub fn as_ref(&self) -> Binder<'tcx, &T> {
        Binder {
            ...
            bound_clauses: self.bound_clauses,
        }
    }
        ...
        F: FnOnce(&T) -> U,
    {
        ...
        Binder { value, bound_vars: self.bound_vars, bound_clauses: self.bound_clauses }
    }
    ...
    where
        F: FnOnce(T) -> U,
    {
        let Binder { value, bound_vars, bound_clauses } = self;
        let value = f(value);
        if cfg!(debug_assertions) {
            let mut validator = ValidateBoundVars::new(bound_vars);
            value.visit_with(&mut validator);
        ...
        F: FnOnce(T) -> Result<U, E>,
    {
        ...
        Ok(Binder { value, bound_vars, bound_clauses })
    }
    ...
    {
        Binder::bind_with_vars_and_clauses(value, self.bound_vars, self.bound_clauses)
    }
    ...
    where
        T: TypeVisitable<TyCtxt<'tcx>>,
    {
        // FIXME(non_lifetime_binders): What to do w/ bound clauses here?
        ...
        F: FnOnce(T) -> (U, V),
    {
        ...
        let (u, v) = f(value);
        (
            Binder { value: u, bound_vars, bound_clauses },
            Binder { value: v, bound_vars, bound_clauses },
        )
    }
}
...
impl<'tcx, T> Binder<'tcx, Option<T>> {
    pub fn transpose(self) -> Option<Binder<'tcx, T>> {
        let Binder { value, bound_vars, bound_clauses } = self;
        value.map(|value| Binder { value, bound_vars, bound_clauses })
    }
}
...
impl<'tcx, T: IntoIterator> Binder<'tcx, T> {
    pub fn iter(self) -> impl Iterator<Item = ty::Binder<'tcx, T::Item>> {
        let Binder { value, bound_vars, bound_clauses } = self;
        value.into_iter().map(|value| Binder { value, bound_vars, bound_clauses })
    }
}
