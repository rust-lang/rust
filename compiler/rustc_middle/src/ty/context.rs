...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn with_stable_hashing_context<R>(
        ...
        let hcx = StableHashingContext::new(self.sess, &*definitions, &*self.cstore);
        f(hcx)
    }
    ...
    pub fn with_no_span_stable_hashing_context<R>(
        ...
        let definitions = self.definitions_untracked();
        let hcx = StableHashingContext::ignore_spans(self.sess, &*definitions, &*self.cstore);
        f(hcx)
    }
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn mk_ty(self, st: TyKind<'tcx>) -> Ty<'tcx> {
        self.interners.intern_ty(st, self.sess, &self.definitions.read(), &*self.cstore)
    }
