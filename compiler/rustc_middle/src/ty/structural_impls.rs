...
impl<'tcx> TypeSuperFoldable<'tcx> for Ty<'tcx> {
    fn try_super_fold_with<F: FallibleTypeFolder<'tcx>>(
        ...
        let kind = match *self.kind() {
            ...
            ty::DynStar(trait_ty, region) => {
                ty::DynStar(trait_ty.try_fold_with(folder)?, region.try_fold_with(folder)?)
            }
...
impl<'tcx> TypeSuperVisitable<'tcx> for Ty<'tcx> {
    fn super_visit_with<V: TypeVisitor<'tcx>>(&self, visitor: &mut V) -> ControlFlow<V::BreakTy> {
        match self.kind() {
            ...
            ty::Dynamic(ref trait_ty, ref reg) | ty::DynStar(ref trait_ty, ref reg) => {
