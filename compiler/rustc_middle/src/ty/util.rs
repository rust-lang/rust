...
impl<'tcx> ty::TyS<'tcx> {
    ...
    ///
    /// Note that this implies that `ty` *might* have a destructor attached
    /// if it returns false then then the type has a destructor on some type
    /// without the, attribute `rustc_insignificant_dtor` defined on it.
    ///
    /// Note that this method is used to check for change in drop order in
