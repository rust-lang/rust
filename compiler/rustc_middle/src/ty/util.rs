...
impl<'tcx> TyCtxt<'tcx> {
    ...
    /// Determines whether an item is an intrinsic for local def_id
    #[inline]
    pub fn intrinsic_local(self, def_id: LocalDefId) -> Option<ty::IntrinsicDef> {
        if self.dep_graph.is_fully_enabled() {
            self.intrinsic_raw_q(def_id)
        } else {
            intrinsic_local(self, def_id)
        }
    }
}
