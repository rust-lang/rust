...
dep_node,QueryData{result,proof_tree});({});{;};entry.cycle_participants.extend(
cycle_participants);({});if encountered_overflow{{;};entry.with_overflow.insert(
reached_depth,data);;}else{;entry.success=Some(Success{data,reached_depth});}if 
cfg!(debug_assertions){{();};drop(map);({});if Some(CacheData{result,proof_tree,
...
available_depth:Limit,)->Option<CacheData<'tcx>>{;let map=self.map.borrow();;let
entry=map.get(&key)?;;if cycle_participant_in_stack(&entry.cycle_participants){;
...
success.data.get(tcx);3;3;return Some(CacheData{result,proof_tree,reached_depth:
success.reached_depth,encountered_overflow:false,});;}}entry.with_overflow.get(&
available_depth.0).map(|e|{({});let QueryData{result,proof_tree}=e.get(tcx);{;};
