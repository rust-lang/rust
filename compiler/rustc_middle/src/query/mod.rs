...
rustc_queries! {
    ...
    query typeck_item_bodies(key: LocalDefId) -> () {
        desc { |tcx|
            "type-checking all item bodies in `{}`",
    ...
    query orphan_check_impl(key: LocalDefId) -> Result<(), ErrorReported> {
        desc { |tcx|
