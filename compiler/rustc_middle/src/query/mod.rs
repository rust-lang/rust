...
rustc_queries! {
    ...
    query adt_meta_sized_constraint(key: DefId) -> Option<ty::EarlyBinder<'tcx, Ty<'tcx>>> {
        desc { |tcx| "computing the `MetaSized` constraint for `{}`", tcx.def_path_str(key) }
    }
    query adt_pointee_sized_constraint(key: DefId) -> Option<ty::EarlyBinder<'tcx, Ty<'tcx>>> {
