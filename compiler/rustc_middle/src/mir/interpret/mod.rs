...
impl<'tcx> AllocMap<'tcx> {
    ...
    fn reserve(&self) -> AllocId {
        ...
        AllocId(
            NonZero::new(next_id).expect("Overflow of u64 should be impossible with +1 increments"),
        )
    }
}
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    fn reserve_and_set_dedup(self, alloc: GlobalAlloc<'tcx>, salt: usize) -> AllocId {
        ...
        assert!(
            self.alloc_map
                .alloc_map
                .insert(id.0.get().try_into().unwrap(), alloc_salt.0.clone())
                .is_ok()
        );
    ...
    pub fn try_get_global_alloc(self, id: AllocId) -> Option<GlobalAlloc<'tcx>> {
        self.alloc_map.alloc_map.get(id.0.get().try_into().unwrap()).cloned()
    }
    ...
    pub fn set_alloc_id_memory(self, id: AllocId, mem: ConstAllocation<'tcx>) {
        if let Err(_) = self
            .alloc_map
            .alloc_map
            .insert(id.0.get().try_into().unwrap(), GlobalAlloc::Memory(mem))
        {
            let old = self.try_get_global_alloc(id).unwrap();
    ...
    pub fn set_nested_alloc_id_static(self, id: AllocId, def_id: LocalDefId) {
        if let Err(_) = self
            .alloc_map
            .alloc_map
            .insert(id.0.get().try_into().unwrap(), GlobalAlloc::Static(def_id.to_def_id()))
        {
            let old = self.try_get_global_alloc(id).unwrap();
            bug!("tried to set allocation ID {id:?}, but it was already existing as {old:#?}");
        }
