...
pub enum TerminatorKind<'tcx> {
    ...
    /// MIR building detects possible async drops, and constructs a complete CFG. Async drops await
    /// a coroutine, which means that the current coroutine may be itself dropped while dropping
    /// `place`. That's the 'drop' field, similar to `Yield` terminator.
    ///
    ...
    Drop {
        ...
        /// It is set to `None` after drop elaboration.
