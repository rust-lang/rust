...
fn write_coverage_branch_info(
    ...
    for coverage::MCDCBranchSpan { span, condition_info, markers, decision_depth } in
        mcdc_branch_spans
    {
        writeln!(
            w,
            "{INDENT}coverage mcdc branch {{ condition_id: {:?}, markers: {markers:?}, depth: {decision_depth:?} }} => {span:?}",
