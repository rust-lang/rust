...
use rustc_type_ir::solve::{ExternalConstraintsData, Goal, QueryInput, Response};
use rustc_type_ir::{
...
impl<'a, D: SolverDelegate<Interner = I>, I: Interner> Canonicalizer<'a, D, I> {
    ...
    pub(super) fn canonicalize_query_response(
        ...
        let Response { certainty, var_values, external_constraints } = value;
        ...
        // FIXME: Too verbose and tons of clones. Refactor `ExternalConstraintsData`, maybe?
        ...
        let region_constraints = if region_constraints.has_type_flags(NEEDS_CANONICAL) {
            region_constraints.clone().fold_with(&mut canonicalizer)
        } else {
            region_constraints.clone()
        };
        let opaque_types = if opaque_types.has_type_flags(NEEDS_CANONICAL) {
            opaque_types.iter().map(|opaque| opaque.fold_with(&mut canonicalizer)).collect()
        } else {
            opaque_types.clone()
        };
        let normalization_nested_goals =
            if normalization_nested_goals.has_type_flags(NEEDS_CANONICAL) {
                normalization_nested_goals.clone().fold_with(&mut canonicalizer)
            } else {
                normalization_nested_goals.clone()
            };

        // Filter out irrelevant hidden tys, in a fixed-point iteration to make them less bulky.
        let mut hidden_types_of_opaques_candidates = hidden_types_of_opaques.clone();
        let mut hidden_types_of_opaques = vec![];
        while !hidden_types_of_opaques_candidates.is_empty() {
            let prev_len = hidden_types_of_opaques.len();
            hidden_types_of_opaques_candidates.retain(|bounds @ (hidden_ty, _)| {
                ...
                {
                    hidden_types_of_opaques.push(bounds.clone().fold_with(&mut canonicalizer));
                    false
                } else {
                    true
                }
            });
            if hidden_types_of_opaques.len() == prev_len {
                break;
            }
        }
