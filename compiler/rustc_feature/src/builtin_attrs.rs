...
pub static BUILTIN_ATTRIBUTES: &[Symbol] = &[
    ...
    // These special attributes are added by the compiler when `cfg` and `cfg_attr` attributes are
    // expanded so that subsequent code can tell that conditional compilation occurred.
    //
    // - A `#[cfg(pred)]` with a true predicate is replaced by a `#[cfg_trace]` that records the
    //   parsed predicate. A `#[cfg(pred)]` with a false predicate leaves no trace because there
    //   is no node left to annotate.
    //
    // - A `#[cfg_attr(pred, attrs)]` is replaced by a `#[cfg_attr_trace]` attribute whether the
    //   predicate evaluated true or not (or even failed to parse). The `pred` and `attrs` are not
    //   recorded because they are not needed.
    //
    // The attributes are used for some diagnostics, by rustdoc (for detecting feature usage), and
    ...
    // are treated specially in various places because they must not be observable in any way that
    // could change behaviour. For example, they are hidden from proc macros.
    sym::cfg_trace,
    sym::cfg_attr_trace,
