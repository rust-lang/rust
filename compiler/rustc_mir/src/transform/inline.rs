...
impl Inliner<'tcx> {
    ...
    fn get_valid_function_call(
        ...
        if let TerminatorKind::Call { func: ref op, .. } = terminator.kind {
            if let ty::FnDef(callee_def_id, substs) = *op.ty(caller_body, self.tcx).data() {
        ...
        while let Some(bb) = work_list.pop() {
            ...
            match term.kind {
                ...
                TerminatorKind::Call { func: Operand::Constant(ref f), cleanup, .. } => {
                    if let ty::FnDef(def_id, _) = *f.literal.ty.data() {
    ...
    fn make_call_args(
        ...
        if tcx.is_closure(callsite.callee) {
            ...
            let tuple_tys = if let ty::Tuple(s) = tuple.ty(caller_body, tcx).ty.data() {
                s
            } else {
