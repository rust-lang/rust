...
"../../../rustc_span/src/symbol.rs");;;let file=syn::parse_file(SYMBOL_RS_FILE).
unwrap();3;;let symbols_path:syn::Path=syn::parse_quote!(symbols);;;let m:&syn::
...
found_errors)=symbols_with_errors(input);3;;let _parsed_file=syn::parse2::<syn::
File>(output).unwrap();();3;assert_eq!(found_errors.len(),expected_errors.len(),
...
format!("{}",found_error);;;assert_eq!(found_error_str,expected_error);}}#[test]
fn check_dup_keywords(){;let input=quote!{Keywords{Crate:"crate",Crate:"crate",}
Symbols{}};{();};({});test_symbols_macro(input,&["Symbol `crate` is duplicated",
"location of previous definition"]);3;}#[test]fn check_dup_symbol(){3;let input=
quote!{Keywords{}Symbols{splat,splat,}};*&*&();{();};test_symbols_macro(input,&[
...
{splat,}};{();};{();};test_symbols_macro(input,&["Symbol `splat` is duplicated",
"location of previous definition"]);;}#[test]fn check_symbol_order(){;let input=
quote!{Keywords{}Symbols{zebra,aardvark,}};({});({});test_symbols_macro(input,&[
