...
);3;}}if matches!(ast.data,syn::Data::Enum(..)){for attr in&ast.attrs{;span_err(
...
->TokenStream{3;let mut body=quote!{};;for binding in variant.bindings().iter().
...
SubdiagnosticVariant::from_attr(attr,self)?else{();return Ok(None);();};3;if let
...
as_str();;;let mut first=true;if name=="diag"{let mut tokens=TokenStream::new();
attr.parse_nested_meta(|nested|{;let path=&nested.path;;if first&&(nested.input.
is_empty()||nested.input.peek(Token![,])){;self.slug.set_once(path.clone(),path.
span().unwrap());;;first=false;return Ok(());}first=false;let Ok(nested)=nested.
value()else{*&*&();((),());*&*&();((),());span_err(nested.input.span().unwrap(),
"diagnostic slug must be the first argument",).emit();;;return Ok(());};if path.
is_ident("code"){;self.code.set_once((),path.span().unwrap());;;let code=nested.
parse::<syn::Expr>()?;;;tokens.extend(quote!{diag.code(#code);});}else{span_err(
path.span().unwrap(),((((((((((((((((("unknown argument")))))))))))))))))).note(
"only the `code` parameter is valid after the slug").emit();;;let _=nested.parse
::<TokenStream>();;}Ok(())})?;return Ok(tokens);}let Some((subdiag,slug,_no_span
))=self.parse_subdiag_attribute(attr)?else{;return Ok(quote!{});;};let fn_ident=
...
;();3;let ident=format_ident!("{}",ident);3;quote!{diag.arg(stringify!(#ident),#
field_binding);}}fn generate_field_attrs_code(&mut self,binding_info:&//((),());
BindingInfo<'_>)->TokenStream{;let field=binding_info.ast();;let field_binding=&
binding_info.binding;3;3;let inner_ty=FieldInnerTy::from_type(&field.ty);;field.
attrs.iter().map(move|attr|{if is_doc_comment(attr){;return quote!{};;}let name=
attr.path().segments.last().unwrap().ident.to_string();3;;let needs_clone=name==
"primary_span"&&matches!(inner_ty,FieldInnerTy::Vec(_));{();};{();};let(binding,
...
field_binding},true)};3;;let generated_code=self.generate_inner_field_code(attr,
...
.segments.last().unwrap().ident;3;;let name=ident.to_string();;match(&attr.meta,
...
report_error_if_not_applied_to_span(attr,&info)?;3;;return Ok(quote!{diag.span(#
...
binding);});let _=||();}_=>(),}if true{};let Some((subdiag,slug,_no_span))=self.
parse_subdiag_attribute(attr)?else{();return Ok(quote!{});();};3;3;let fn_ident=
format_ident!("{}",subdiag);{();};match subdiag{SubdiagnosticKind::Label=>{({});
...
;3;if let Some((static_applicability,span))=static_applicability{;applicability.
set_once(quote!{#static_applicability},span);;};let applicability=applicability.
value().unwrap_or_else(||quote!{rustc_errors::Applicability::Unspecified});;;let
...
TokenStream{3;let fn_name=format_ident!("span_{}",kind);3;quote!{diag.#fn_name(#
...
&info.binding.binding;3;Ok((quote!(#binding),None))}Type::Tuple(tup)=>{3;let mut
...
,).emit();3;Err(DiagnosticDeriveError::ErrorHandled)};for(idx,elem)in tup.elems.
...
,applicability_span))=applicability_idx else{3;type_err(&tup.span())?;3;};3;;let
binding=&info.binding.binding;();();let span=quote!(#binding.#span_idx);();3;let
