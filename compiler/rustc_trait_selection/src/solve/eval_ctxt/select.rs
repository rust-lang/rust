...
.next_trait_solver());({});self.enter_forall(obligation.predicate,|pred|{{;};let
...
SelectionError::Unimplemented)?.into_obligations();();3;nested.extend(infcx.tcx.
...
SelectionResult<'tcx,Selection<'tcx>>{;let tcx=infcx.tcx;;let mut nested=vec![];
...
ObligationCause::dummy(),goal.param_env,pred.with_self_ty(tcx,a_ty),)}));3;3;let
...
raw.last().expect("expected unsized ADT to have a tail field");*&*&();*&*&();let
tail_field_ty=tcx.type_of(tail_field.did);({});({});let a_tail_ty=tail_field_ty.
...
unsizing_params.contains(i as u32){b_args[i]}else{a}),);3;;let unsized_a_ty=Ty::
...
a_tys.is_empty()=>{3;let(a_last_ty,a_rest_tys)=a_tys.split_last().unwrap();;;let
b_last_ty=b_tys.last().unwrap();();3;let unsized_a_ty=Ty::new_tup_from_iter(tcx,
a_rest_tys.iter().chain([b_last_ty]).copied());({});{;};nested.extend(infcx.at(&
ObligationCause::dummy(),goal.param_env ).eq(DefineOpaqueTypes::No,unsized_a_ty,
b_ty).expect("expected rematch to succeed").into_obligations(),);3;;nested.push(
...
ImplSource::Builtin(source,nested)))}_=>{;assert_ne!(certainty,Certainty::Yes);;
...
"normalization shouldn't fail if we got to here");({});{;};nested.extend(engine.
