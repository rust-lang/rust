...
#[repr(transparent)]
pub struct RustcSolverDelegate<'tcx>(InferCtxt<'tcx>);

impl<'a, 'tcx> From<&'a InferCtxt<'tcx>> for &'a RustcSolverDelegate<'tcx> {
    fn from(other: &'a InferCtxt<'tcx>) -> Self {
        // SAFETY: `repr(transparent)`
        unsafe { std::mem::transmute(other) }
    }
}

impl<'tcx> Deref for RustcSolverDelegate<'tcx> {
...
impl<'tcx> SolverDelegate for RustcSolverDelegate<'tcx> {
    ...
    fn relate<T: Relate<TyCtxt<'tcx>>>(
        ...
        rhs: T,
    ) -> Result<Vec<Goal<TyCtxt<'tcx>, ty::Predicate<'tcx>>>, NoSolution> {
