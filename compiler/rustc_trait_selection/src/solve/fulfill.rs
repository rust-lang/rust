...
fn convert_solver_error<'tcx, E>(
    infcx: &InferCtxt<'tcx>,
    error: SolverNextSolverError<TyCtxt<'tcx>>,
) -> Option<E>
where
    E: FromSolverError<'tcx, NextSolverError<'tcx>>,
{
    let error = match error {
        SolverNextSolverError::TrueError(obligation) => NextSolverError::TrueError(obligation),
        SolverNextSolverError::Ambiguity(obligation) => {
            NextSolverError::Ambiguity(try_ambiguity_error_for_stalled(infcx, obligation)?)
        }
        SolverNextSolverError::Overflow(obligation) => NextSolverError::Overflow(obligation),
    };

    Some(E::from_solver_error(infcx, error))
}
    ...
    E: FromSolverError<'tcx, NextSolverError<'tcx>>,
{
    ...
    fn collect_remaining_errors(&mut self, infcx: &InferCtxt<'tcx>) -> TraitErrors<E> {
        if !self.core.has_pending_obligations() {
            ...
            let errors: ThinVec<_> = self
                .core
                .collect_remaining_errors(|error| error)
                .into_iter()
                .filter_map(|error| convert_solver_error::<E>(infcx, error))
                .collect();
