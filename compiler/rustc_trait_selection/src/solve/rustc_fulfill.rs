...
///
/// The frontend wrapper keeps rustc-specific snapshot checks, diagnostics,
/// and successful-obligation handling outside of the shared engine.
pub struct FulfillmentCtxt<'tcx, E: 'tcx> {
    core: SolverFulfillmentCtxt<TyCtxt<'tcx>, PredicateObligation<'tcx>>,
...
impl<'tcx> From<SolverNextSolverError<PredicateObligation<'tcx>>> for NextSolverError<'tcx> {
    fn from(error: SolverNextSolverError<PredicateObligation<'tcx>>) -> Self {
        match error {
