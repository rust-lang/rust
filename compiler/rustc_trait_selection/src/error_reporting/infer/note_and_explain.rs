...
```",
    ...
    fn point_at_methods_that_satisfy_associated_type(
        ...
        let methods: Vec<(Span, String)> = items
            ...
            .filter_map(|item| {
                let method = tcx.fn_sig(item.def_id).instantiate_identity().skip_normalization();
        ...
        if let DefKind::Trait | DefKind::Impl { .. } = tcx.def_kind(parent_id) {
            ...
            for assoc_item in assoc_items.in_definition_order() {
                if assoc_item.is_type()
                    ...
                    && let assoc_ty = tcx.type_of(assoc_item.def_id).instantiate_identity().skip_normalization()
