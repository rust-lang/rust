...
pub fn compute_applicable_impls_for_diagnostics<'tcx>(
    ...
    // If our `item_id` has been set (and isn't just from a dummy obligation cause),
    ...
    let item_id = obligation.cause.item_id;
    if item_id != CRATE_DEF_ID {
        let predicates = tcx.predicates_of(item_id.to_def_id()).instantiate_identity(tcx);
        for (pred, span) in
        ...
        let mut err = match bound_predicate.skip_binder() {
            ty::PredicateKind::Clause(ty::ClauseKind::Trait(data)) => {
                ...
                if term.is_some_and(|term| term.as_type().is_some())
                    && let Some(body) = self.tcx.hir_maybe_body_owned_by(obligation.cause.item_id)
                {
