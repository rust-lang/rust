...
pub fn obligations<'tcx>(
    ...
    debug!("wf::obligations({:?}, item_id={:?}) = {:?}", term, item_id, wf.out);

    let result = wf.normalize(infcx);
    debug!("wf::obligations({:?}, item_id={:?}) ~~> {:?}", term, item_id, result);
    Some(result)
}
...
impl<'a, 'tcx> WfPredicates<'a, 'tcx> {
    ...
    fn cause(&self, code: traits::ObligationCauseCode<'tcx>) -> traits::ObligationCause<'tcx> {
        traits::ObligationCause::new(self.span, self.item_id, code)
    }
    ...
    fn add_wf_preds_for_trait_pred(
        ...
        self.out.extend(
            trait_ref
                ...
                .map(|(i, term)| {
                    let mut cause = traits::ObligationCause::misc(self.span, self.item_id);
