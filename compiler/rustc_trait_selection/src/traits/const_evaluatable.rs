    ...
    if tcx.features().generic_const_exprs {
        ...
        match unexpanded_ct.kind() {
            ty::ConstKind::Expr(_) => {
                ...
                Err(NotConstEvaluatable::Error(tcx.sess.delay_bug_unless_error(
                    span,
            ...
            ty::ConstKind::Unevaluated(uv) => {
                ...
                match concrete {
                    Err(ErrorHandled::TooGeneric) => {
                        Err(NotConstEvaluatable::Error(infcx.tcx.sess.delay_bug_unless_error(
                            span,
        ...
        match concrete {
            ...
            Err(ErrorHandled::TooGeneric) => {
                let err = if uv.has_non_region_infer() {
                    ...
                    let guar = infcx.tcx.sess.delay_bug_unless_error(
                        span,
                        "Missing value for constant, but no error reported?",
                    );
