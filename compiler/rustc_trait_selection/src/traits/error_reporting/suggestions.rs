...
fn suggest_restriction(
    ...
    if let Some((bound_str, fn_sig)) =
        fn_sig.zip(projection).and_then(|(sig, p)| match p.self_ty().data() {
...
impl<'a, 'tcx> InferCtxtExt<'tcx> for InferCtxt<'a, 'tcx> {
    fn suggest_restricting_param_bound(
        ...
        let (param_ty, projection) = match self_ty.data() {
    ...
    fn suggest_dereferences(
        ...
        if let ty::Ref(region, base_ty, mutbl) = *real_ty.data() {
    ...
    fn suggest_fn_call(
        ...
        let (def_id, output_ty, callable) = match *self_ty.data() {
    ...
    fn suggest_remove_reference(
        ...
        if let Ok(snippet) = self.tcx.sess.source_map().span_to_snippet(span) {
            ...
            for refs_remaining in 0..refs_number {
                if let ty::Ref(_, inner_ty, _) = suggested_ty.data() {
    ...
    fn suggest_change_mut(
        ...
        if let Ok(snippet) = self.tcx.sess.source_map().span_to_snippet(span) {
            ...
            if let ty::Ref(region, t_type, mutability) = *trait_ref.skip_binder().self_ty().data() {
    ...
    fn suggest_semicolon_removal(
        ...
        let is_empty_tuple =
            |ty: ty::Binder<Ty<'_>>| *ty.skip_binder().data() == ty::Tuple(ty::List::empty());
    ...
    fn suggest_impl_trait(
        ...
        let is_object_safe = match ty.data() {
        ...
        let (last_ty, all_returns_have_same_type, only_never_return) = ret_types.clone().fold(
            ...
            |(last_ty, mut same, only_never_return): (std::option::Option<Ty<'_>>, bool, bool),
             ty| {
                ...
                same &=
                    !matches!(ty.data(), ty::Error(_))
                        && last_ty.map_or(true, |last_ty| {
                            // FIXME: ideally we would use `can_coerce` here instead, but `typeck` comes
                            // *after* in the dependency graph.
                            match (ty.data(), last_ty.data()) {
                ...
                (Some(ty), same, only_never_return && matches!(ty.data(), ty::Never))
            },
        );
        let all_returns_conform_to_trait =
            if let Some(ty_ret_ty) = typeck_results.node_type_opt(ret_ty.hir_id) {
                match ty_ret_ty.data() {
                    ty::Dynamic(predicates, _) => {
    ...
    fn report_closure_arg_mismatch(
        ...
        crate fn build_fn_sig_string<'tcx>(
            ...
            let sig = if let ty::Tuple(inputs) = inputs.data() {
                tcx.mk_fn_sig(
