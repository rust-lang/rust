...
impl<'a, 'tcx> ProofTreeVisitor<'tcx> for AmbiguityCausesVisitor<'a, 'tcx> {
    ...
    fn visit_goal(&mut self, goal: &InspectGoal<'_, 'tcx>) {
        ...
        let predicate_kind = goal.infcx().enter_forall_with_ambiguous_aliases_and_leak_universe(predicate.kind());
        // TODO: should have some "replace ambiguous aliases with infer" thingy on the infcx
