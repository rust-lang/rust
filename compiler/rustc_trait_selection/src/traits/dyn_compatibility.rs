...
fn generics_require_sized_self(tcx: TyCtxt<'_>, def_id: DefId) -> bool {
    ...
    let predicates = predicates.instantiate_identity(tcx).skip_normalization().predicates;
        ...
        ty::AssocKind::Const { name, is_type_const } => {
            ...
            if tcx.features().min_generic_const_args() {
                ...
                let ty = ty::Binder::dummy(
                    tcx.type_of(item.def_id).instantiate_identity().skip_normalization(),
                );
...
fn receiver_for_self_ty<'tcx>(
    ...
    let result = EarlyBinder::bind(receiver_ty).instantiate(tcx, args).skip_normalization();
    debug!(
