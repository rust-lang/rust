...
fn implied_outlives_bounds<'a, 'tcx>(
    ...
    let implied_bounds_result =
        infcx.tcx.implied_outlives_bounds((canonical, force_no_implied_bounds_hack));
...
impl<'tcx> InferCtxt<'tcx> {
    ...
    fn implied_bounds_tys<Tys: IntoIterator<Item = Ty<'tcx>>>(
        ...
        tys.into_iter().flat_map(move |ty| {
            implied_outlives_bounds(self, param_env, body_id, ty, force_no_implied_bounds_hack)
        })
    }
}
