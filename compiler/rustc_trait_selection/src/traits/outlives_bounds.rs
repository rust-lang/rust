...
/// - `item_id`, the item_id to use when normalizing assoc types.
///   Note that this may cause outlives obligations to be injected
///   into the inference context with this body-id.
/// - `ty`, the type that we are supposed to assume is WF.
#[instrument(level = "debug", skip(infcx, param_env, item_id), ret)]
fn implied_outlives_bounds<'a, 'tcx>(
    ...
    let span = infcx.tcx.def_span(item_id);
    ...
    if !constraints.is_empty() {
        ...
        let cause = ObligationCause::misc(span, item_id);
...
impl<'tcx> InferCtxt<'tcx> {
    ...
    fn implied_bounds_tys<Tys: IntoIterator<Item = Ty<'tcx>>>(
        ...
        tys.into_iter().flat_map(move |ty| {
            implied_outlives_bounds(self, param_env, item_id, ty, disable_implied_bounds_hack)
        })
    }
}
