...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
    ...
    pub(super) fn for_each_item_bound<T>(
        ...
        loop {
            ...
            for bound in relevant_bounds.instantiate(self.tcx(), alias_ty.args).skip_normalization()
            {
...
impl<'tcx> SelectionContext<'_, 'tcx> {
        ...
        match self_ty.kind() {
            ...
            ty::Adt(def, args) => {
                if let Some(crit) = def.sizedness_constraint(self.tcx(), sizedness) {
                    ty::Binder::dummy(vec![crit.instantiate(self.tcx(), args).skip_normalization()])
                } else {
                    ty::Binder::dummy(vec![])
                }
            }
        ...
        Ok(match *t.kind() {
            ...
            ty::Alias(ty::AliasTy { kind: ty::Opaque { def_id }, args, .. }) => {
                if self.infcx.can_define_opaque_ty(def_id) {
                    ...
                    ty::Binder::dummy(AutoImplConstituents {
                        types: vec![ty.instantiate(self.tcx(), args).skip_normalization()],
                        assumptions: vec![],
                    })
                }
            }
    ...
    fn match_impl(
        ...
        let trait_ref =
            impl_trait_header.trait_ref.instantiate(self.tcx(), impl_args).skip_normalization();
    ...
    fn impl_or_trait_obligations(
        ...
        let predicates =
            predicates.instantiate_own(tcx, args).map(Unnormalized::skip_normalization);
