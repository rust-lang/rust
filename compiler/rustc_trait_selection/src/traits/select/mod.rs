...
impl<'tcx> SelectionContext<'_, 'tcx> {
    fn sized_conditions(
        ...
        sizedness_conditions(self, obligation, SizedTraitKind::Sized)
    }
    ...
    fn meta_sized_conditions(
        ...
        sizedness_conditions(self, obligation, SizedTraitKind::MetaSized)
    }

    fn pointee_sized_conditions(
        &mut self,
        obligation: &PolyTraitObligation<'tcx>,
    ) -> BuiltinImplConditions<'tcx> {
        sizedness_conditions(self, obligation, SizedTraitKind::PointeeSized)
    }
    ...
    match self_ty.kind() {
        ...
        ty::Adt(def, args) => {
            let constraint = match sizedness {
                SizedTraitKind::Sized => def.sized_constraint(ctx.tcx()),
                SizedTraitKind::MetaSized => def.meta_sized_constraint(ctx.tcx()),
                SizedTraitKind::PointeeSized => def.pointee_sized_constraint(ctx.tcx()),
            };
