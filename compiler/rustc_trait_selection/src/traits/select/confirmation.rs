        ...
        for assoc_type in assoc_types {
            ...
            for bound in self.tcx().item_bounds(assoc_type).transpose_iter() {
                let normalized_bound = normalize_with_depth_to(
                    ...
                    obligation.cause.clone(),
                    obligation.recursion_depth + 1,
                    bound.instantiate(tcx, trait_predicate.trait_ref.args).skip_normalization(),
                    &mut nested,
                );
        ...
        Ok(match (source.kind(), target.kind()) {
            ...
            (&ty::Adt(def, args_a), &ty::Adt(_, args_b)) => {
                ...
                let source_tail = normalize_with_depth_to(
                    ...
                    obligation.cause.clone(),
                    obligation.recursion_depth + 1,
                    tail_field_ty.instantiate(tcx, args_a).skip_normalization(),
                    &mut nested,
                );
                let target_tail = normalize_with_depth_to(
                    ...
                    obligation.cause.clone(),
                    obligation.recursion_depth + 1,
                    tail_field_ty.instantiate(tcx, args_b).skip_normalization(),
                    &mut nested,
                );
