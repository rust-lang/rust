    ...
    fn check_mod(&mut self, cx: &LateContext<'_>, _: &'tcx hir::Mod<'tcx>, id: hir::HirId) {
        ...
        let crate_ident = if let Some(name) = &cx.tcx.sess.opts.crate_name {
            ...
            attr::find_by_name(cx.tcx.hir().attrs(hir::CRATE_HIR_ID), sym::crate_name).and_then(
                |attr| {
                    if let AttrKind::Normal(AttrItem {
                        args: AttrArgs::Eq { eq_span: _, ref value },
                        ..
                    }) = attr.kind
                        && let ast::LitKind::Str(name, ..) = value.kind
                    {
                        ...
                        let sp = cx
                            ...
                            .and_then(|snippet| {
                                ...
                                Some(
                                    value
                                        .span
                                        .with_lo(value.span.lo() + BytePos(left as u32 + 1))
                                        .with_hi(value.span.hi() - BytePos(right as u32)),
                                )
                            })
