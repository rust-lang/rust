...
pub trait LintContext {
    ...
    fn emit_span_lint<S: Into<MultiSpan>>(
        ...
        self.opt_span_lint(lint, Some(span), |lint| {
            let diag = decorator.into_diag(lint.dcx, lint.level());
            lint.merge_with_other_diag(diag);
        });
    }

    /// Emit a lint at `span` from a lazily-constructed lint struct (some type that implements
    /// `Diagnostic`, typically generated by `#[derive(LintDiagnostic)]`).
    fn emit_span_lint_lazy<S: Into<MultiSpan>, L: for<'a> Diagnostic<'a, ()>>(
        &self,
        lint: &'static Lint,
        span: S,
        ...
        self.opt_span_lint(lint, Some(span), |lint| {
            let diag: Diag<'_, ()> = decorator().into_diag(lint.dcx, lint.level());
            lint.merge_with_other_diag(diag);
        });
    }
    ...
    fn emit_lint(&self, lint: &'static Lint, decorator: impl for<'a> Diagnostic<'a, ()>) {
