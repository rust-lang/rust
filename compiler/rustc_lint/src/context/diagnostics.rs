    ...
    match diagnostic {
        ...
        BuiltinLintDiag::AbsPathWithModule(mod_span) => {
            let (replacement, applicability) =
                match ctx.sess().source_map().span_to_snippet(mod_span) {
                    Ok(ref s) => {
        ...
        BuiltinLintDiag::UnusedImports {
            ...
            let test_module_span =
                test_module_span.map(|span| ctx.sess().source_map().guess_head_span(span));
        ...
        BuiltinLintDiag::UnexpectedCfgName(name, value) => {
            ctx.emit_span_lint(lint, span, check_cfg::unexpected_cfg_name(ctx.sess(), name, value));
        }
        BuiltinLintDiag::UnexpectedCfgValue(name, value) => {
            ctx.emit_span_lint(
                lint,
                span,
                check_cfg::unexpected_cfg_value(ctx.sess(), name, value),
            );
        }
        ...
        BuiltinLintDiag::SingleUseLifetime {
            ...
            let suggestion = if let Some(deletion_span) = deletion_span {
                let (use_span, replace_lt) = if elide {
                    let use_span = ctx.sess().source_map().span_extend_while_whitespace(use_span);
                    (use_span, String::new())
                } else {
                    (use_span, "'_".to_owned())
                };
        ...
        BuiltinLintDiag::NamedArgumentUsedPositionally {
            ...
            let (suggestion, name) = if let Some(positional_arg_to_replace) = position_sp_to_replace
            {
                ...
                let span_to_replace = if let Ok(positional_arg_content) =
                    ctx.sess().source_map().span_to_snippet(positional_arg_to_replace)
        ...
        BuiltinLintDiag::RedundantImportVisibility { max_vis, span: vis_span, import_vis } => {
            ctx.emit_span_lint(
                lint,
                span,
                lints::RedundantImportVisibility { span: vis_span, max_vis, import_vis },
            );
        }
