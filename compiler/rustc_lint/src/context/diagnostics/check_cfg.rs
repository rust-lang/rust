...
pub(super) fn unexpected_cfg_name(
    ...
    let code_sugg = if is_feature_cfg && is_from_cargo {
        lints::unexpected_cfg_name::CodeSuggestion::ConsiderDefiningFeatures
    ...
    let meta_sugg = if is_from_cargo {
        let sub = if !is_feature_cfg {
            Some(lints::unexpected_cfg_name::AddBuildRsPrintln {
                build_rs_println: format!("println!(\"cargo::rustc-check-cfg={inst}\");"),
            })
        } else {
            None
        };
        lints::unexpected_cfg_name::InvocationSuggestion::Cargo { sub }
    } else {
        lints::unexpected_cfg_name::InvocationSuggestion::Rustc {
            cmdline_arg: format!("--check-cfg={inst}"),
        }
    };
...
pub(super) fn unexpected_cfg_value(
    ...
    let invocation_sugg = if is_from_cargo {
        let help = if name == sym::feature {
            ...
            Some(lints::unexpected_cfg_value::CargoHelp::UseFeatureOrBuildRs {
                build_rs_println: format!("println!(\"cargo::rustc-check-cfg={inst}\");"),
            })
        } else {
            None
        };
        lints::unexpected_cfg_value::InvocationSuggestion::Cargo(help)
    } else {
        let help = if !is_cfg_a_well_know_name {
            Some(lints::unexpected_cfg_value::RustcHelp {
                cmdline_arg: format!("--check-cfg={inst}"),
            })
        } else {
            None
