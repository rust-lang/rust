...
impl<'tcx> LateLintPass<'tcx> for UnusedResults {
    fn check_stmt(&mut self, cx: &LateContext<'_>, s: &hir::Stmt<'_>) {
        ...
        if let Some(must_use_op) = must_use_op {
            cx.emit_lint(UNUSED_MUST_USE, UnusedOp {
                span: expr.span,
                op: must_use_op,
        ...
        fn emit_must_use_untranslated(
            ...
            match path {
                ...
                &MustUsePath::Closure(span) => {
                    cx.emit_lint(UNUSED_MUST_USE, UnusedClosure {
                        span,
                        count: plural_len,
                        pre: descr_pre,
                        post: descr_post,
                ...
                &MustUsePath::Coroutine(span) => {
                    cx.emit_lint(UNUSED_MUST_USE, UnusedCoroutine {
                        span,
                        count: plural_len,
                        pre: descr_pre,
                        post: descr_post,
                ...
                &MustUsePath::Def(span, def_id, reason) => {
                    cx.emit_lint(UNUSED_MUST_USE, UnusedDef {
                        span,
                        pre: descr_pre,
                        post: descr_post,
                        cx,
...
trait UnusedDelimLint {
    ...
    fn emit_unused_delims(
        ...
        cx.emit_lint(self.lint(), UnusedDelim {
            span: primary_span,
...
impl UnusedImportBraces {
    fn check_use_tree(&self, cx: &EarlyContext<'_>, use_tree: &ast::UseTree, item: &ast::Item) {
        if let ast::UseTreeKind::Nested { ref items, .. } = use_tree.kind {
            ...
            cx.emit_lint(UNUSED_IMPORT_BRACES, UnusedImportBracesDiag {
                span: item.span,
                node: node_name,
            });
        }
