//! This module check that we are not transmuting `&T` to `&mut T` or `&UnsafeCell<T>`, or casting
//! `&T` to `&UnsafeCell<T>` (checking casts from `&T` to `&mut T` is done in reference_casting.rs,
//! because it has some differences).
//!
//! The complexity comes from the fact that we want to lint against this conversion even when the `&T`, the `&mut T`
...
//! in the source type (not a `&mut` reference, since it is perfectly fine to convert `&mut T` to `&UnsafeCell<T>`).
...
use crate::lints::{BuiltinMutablesTransmutes, ConversionBreadcrumbs, UnsafeCellConversions};
...
fn struct_fields<'a, 'tcx>(
    ...
    let field_tys =
        adt_def.non_enum_variant().fields.iter().map(|field| {
            cx.tcx.normalize_erasing_regions(cx.typing_env(), field.ty(cx.tcx, substs))
        });
    // I thought we could panic if the fields shape is not `Arbitrary`, but apparently SIMD vectors are
    // structs but also have primitive layout. So...
...
impl FieldsBreadcrumbs {
    ...
    fn format_unsafe_cell_for_diag<'tcx>(
        ...
        ConversionBreadcrumbs { before_ty, ty: top_ty, after_ty }
    }
    ...
    fn format_for_diag<'tcx>(
        ...
        ConversionBreadcrumbs { before_ty: "", ty, after_ty }
    }
}
...
fn collect_fields<'tcx>(
    ...
    match ty.kind() {
        ...
        &ty::Array(ty, len)
            if let Some(len) = len.try_to_target_usize(cx.tcx)
                && len != 0 =>
        {
                ...
                Mutability::Not => {
                    ...
                    check_unsafe_cells(
                        ...
                        |src_pointee_fields_breadcrumbs, dst_pointee_fields_breadcrumbs| {
                            ...
                            cx.emit_span_lint(
                                UNSAFE_CELL_CONVERSIONS,
                                expr.span,
                                UnsafeCellConversions { from, to, orig_cast: None },
                            );
                        },
                    );
                    CheckOverlapping::No
...
/// Checks for casts via `&*(reference as *const _ as *const _)` and lints.
fn lint_reference_casting<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
    ...
    check_unsafe_cells(cx, dst, src, |src_fields_breadcrumbs, dst_fields_breadcrumbs| {
        ...
        cx.emit_span_lint(UNSAFE_CELL_CONVERSIONS, expr.span, UnsafeCellConversions {
            from,
            to,
            orig_cast,
        });
...
declare_lint! {
    /// The `unsafe_cell_conversions` lint catches transmuting or casting from `&T` to [`&UnsafeCell<T>`]
    ...
    pub UNSAFE_CELL_CONVERSIONS,
    Deny,
    "transmuting &T to &UnsafeCell<T> is error-prone, rarely intentional and may cause undefined behavior"
}

declare_lint_pass!(MutableTransmutes => [MUTABLE_TRANSMUTES, UNSAFE_CELL_CONVERSIONS]);
