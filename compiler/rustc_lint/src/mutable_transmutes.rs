...
use rustc_abi::{FieldIdx, FieldsShape, HasDataLayout as _, Layout, Size};
...
use crate::lints::{BuiltinMutablesTransmutes, ConversionBreadcrumbs, UnsafeCellTransmutes};
...
fn lint_reference_casting<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
    ...
    check_unsafe_cells(cx, dst, src, |src_fields_breadcrumbs, dst_fields_breadcrumbs| {
        ...
        cx.emit_span_lint(
            UNSAFE_CELL_TRANSMUTES,
            expr.span,
            UnsafeCellTransmutes { from, to, orig_cast },
        );
    });
}
