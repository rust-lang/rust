...
use rustc_session::lint::{
    CheckLintNameResult, Level, Lint, LintExpectationId, LintId, StableLintExpectationId,
    TargetLint, UnstableLintExpectationId,
};
...
impl<'tcx> LintLevelsBuilder<'_, LintLevelQueryMap<'tcx>> {
    fn add_id(&mut self, hir_id: HirId) {
        ...
        self.add(self.provider.attrs.get(hir_id.local_id));
    }
}
...
impl<'s> LintLevelsBuilder<'s, TopDown> {
    ...
    pub fn crate_root(
        ...
        builder.add(&parsed_crate_attrs);
        builder
    }
    ...
    LevelSpec<P::LintExpectationId>: Into<LevelSpec>,
{
    ...
    fn simple_add(
        ...
        expect_lint_id: Option<P::LintExpectationId>,
    ) {
        ...
        if self.check_gated_lint(*id, lint.span(), false) {
            self.insert_spec(*id, LevelSpec::new(level, expect_lint_id, src));
        }
    }

    fn add(&mut self, attrs: &[hir::Attribute]) {
            ...
            let level = match kind {
                ...
                LintAttributeKind::Expect => {
                    for lint in lint_instances {
                        ...
                        let attr_index = attr_index.try_into().unwrap();
                        let expectation_id =
                            self.provider.mk_lint_expectation_id(attr_id, attr_index, lint_index);
