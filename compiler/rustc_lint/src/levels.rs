...
impl<'tcx> LintLevelsBuilder<'_, LintLevelQueryMap<'tcx>> {
    ...
    fn inherit_derive_expectations(&mut self, owner: hir::OwnerId) {
        ...
        // This runs for every owner, and everything below `self.provider.attrs` adds query
        // dependencies that incremental rebuilds have to re-verify per owner, so bail as
        // early and as cheaply as possible. Builtin derives always attach
        // `#[automatically_derived]` to the impls they emit, and the owner's attribute map
        // was already fetched to build this query, so this check is free of new edges.
        if !find_attr!(self.provider.attrs.get(hir::ItemLocalId::ZERO), AutomaticallyDerived) {
            return;
        }
        ...
        let adt_attrs = tcx.hir_attrs(adt_hir_id);
        // Check that there is an `#[expect]` to inherit at all before the derive check
        // below, which consults the `expn_that_defined` query and clones hygiene data —
        // the most expensive step here and almost always unnecessary.
        if !adt_attrs.iter().any(|attr| Level::from_opt_symbol(attr.name()) == Some(Level::Expect))
        {
            return;
        }
        // Only impls generated by a builtin derive inherit expectations, checked through
        // the def collector's record of which expansion created the impl. Span hygiene
        // must not be consulted here: proc macros can assign arbitrary spans to the
        // tokens they produce, so spans can neither prove nor disprove that an item came
        // from a derive. Builtin derives are compiler-controlled and always emit impls
        // for exactly the item they were applied to; proc-macro derives are not covered
        // (their output is not linted anyway).
        if !tcx.is_builtin_derived(owner.to_def_id()) {
            return;
        }
        ...
        for (attr_index, attr) in adt_attrs.iter().enumerate() {
    ...
    LevelSpec<P::LintExpectationId>: Into<LevelSpec>,
{
    ...
    fn resolve_lint_name(
        ...
        let lint_result = store.check_lint_name(&name, tool_name, self.registered_lint_tools);
        ...
        let (ids, name) = match lint_result {
            ...
            CheckLintNameResult::Renamed(ref replace) => {
                ...
                let CheckLintNameResult::Ok(ids) =
                    store.check_lint_name(replace, None, self.registered_lint_tools)
                else {
