            ...
            let level = match Level::from_attr(attr) {
                ...
                Some(Level::Expect(unstable_id)) if let Some(hir_id) = source_hir_id => {
                    ...
                    let stable_id = LintExpectationId::Stable {
                        ...
                        // when building a map to go from unstable to stable
                        // id.
            ...
            for (lint_index, li) in metas.iter_mut().enumerate() {
                ...
                match &lint_result {
                    CheckLintNameResult::Ok(ids) => {
                        ...
                        // in that case we want to avoid overriding the lint level but instead add
                        // an expectation that can't be fulfilled. The lint
                        // message will include an explanation, that the
                        // `unfulfilled_lint_expectations` lint can't be expected.
                        if let Level::Expect(expect_id) = level {
                            // The `unfulfilled_lint_expectations` lint is not part of any lint
                            // groups. Therefore. we only need to check
                            // the slice if it contains a single lint.
