    ...
    fn check_item(&mut self, cx: &LateContext<'_>, it: &hir::Item<'_>) {
        match it.kind {
            hir::ItemKind::Const(_, _, body_id) => {
                ...
                // trigger the query once for all constants since that will already report the
                // errors FIXME(generic_const_items): Does this work properly with
                // generic const items?
        ...
        fn ty_find_init_error<'tcx>(
            ...
            match ty.kind() {
                ...
                Adt(adt_def, args) if !adt_def.is_union() => {
                    ...
                    // (Leaving it zeroed could be okay, depending on which variant is encoded as
                    // zero tag.)
                ...
                for statement in statements {
                    ...
                    'label_loop: for (idx, _) in statement.match_indices(':') {
                        ...
                        for c in chars {
                            ...
                            // puproses of label detection because we
                            // assume that it can be replaced with some other valid label string
                            // later. `options(raw)` asm blocks cannot
                            // have format args, so they are excluded from this special case.
                            if !raw && in_bracket {
                                if c == '{' {
