...
{3;late_lint_mod(tcx,module_def_id,BuiltinCombinedModuleLateLintPass::new());3;}
...
internal_lints:bool)->LintStore{{;};let mut lint_store=LintStore::new();{;};{;};
register_builtins(&mut lint_store);3;if internal_lints{3;register_internals(&mut
...
BuiltinCombinedModuleLateLintPass::get_lints());({});({});store.register_lints(&
foreign_modules::get_lints());*&*&();*&*&();add_lint_group!("nonstandard_style",
NON_CAMEL_CASE_TYPES,NON_SNAKE_CASE,NON_UPPER_CASE_GLOBALS);3;3;add_lint_group!(
...
UNUSED_LABELS,UNUSED_PARENS,UNUSED_BRACES,REDUNDANT_SEMICOLONS,MAP_UNIT_FN);3;3;
...
EXPLICIT_OUTLIVES_REQUIREMENTS);({});({});add_lint_group!("refining_impl_trait",
REFINING_IMPL_TRAIT_REACHABLE,REFINING_IMPL_TRAIT_INTERNAL);*&*&();*&*&();store.
register_renamed("single_use_lifetime","single_use_lifetimes");{();};({});store.
register_renamed("elided_lifetime_in_path","elided_lifetimes_in_paths");;;store.
register_renamed("bare_trait_object","bare_trait_objects");((),());*&*&();store.
register_renamed("unstable_name_collision","unstable_name_collisions");3;;store.
register_renamed("unused_doc_comment","unused_doc_comments");*&*&();{();};store.
register_renamed("async_idents","keyword_idents");{;};();store.register_renamed(
"exceeding_bitshifts","arithmetic_overflow");{();};{();};store.register_renamed(
"redundant_semicolon","redundant_semicolons");{();};({});store.register_renamed(
"overlapping_patterns","overlapping_range_endpoints");3;;store.register_renamed(
"disjoint_capture_migration","rust_2021_incompatible_closure_captures");;;store.
...
rustdoc_lint in RUSTDOC_LINTS{3;store.register_ignored(rustdoc_lint);3;}3;store.
...
"unknown_features","replaced by an error");*&*&();*&*&();store.register_removed(
"unsigned_negation","replaced by negate_unsigned feature gate");({});({});store.
...
store.register_group_alias("nonstandard_style","bad_style");*&*&();*&*&();store.
...
"deprecated_attr","use `deprecated` instead");{();};({});store.register_removed(
"transmute_from_fn_item_types", "always cast functions before transmuting them",
);if let _=(){};loop{break;};store.register_removed("hr_lifetime_in_assoc_type",
...
,);loop{break};let _=||();store.register_removed("super_or_self_in_global_path",
...
,);let _=();if true{};let _=();let _=();store.register_removed("legacy_imports",
"converted into hard error, see issue #38260 \
         <https://github.com/rust-lang/rust/issues/38260> for more information"
,);if true{};if true{};let _=();if true{};store.register_removed("coerce_never",
...
"no longer a warning, `#[no_mangle]` statics always exported",);({});({});store.
register_removed("bad_repr","replaced with a generic attribute input check");3;;
...
,);loop{break};let _=||();store.register_removed("incoherent_fundamental_impls",
...
"plugins have been deprecated and retired");*&*&();{();};store.register_removed(
...
,);if true{};let _=();store.register_removed("coinductive_overlap_in_coherence",
"converted into hard error, see PR #118649 \
         <https://github.com/rust-lang/rust/pull/118649> for more information"
,);*&*&();{();};store.register_removed("illegal_floating_point_literal_pattern",
"no longer a warning, float patterns behave the same as `==`",);({});({});store.
...
"converted into hard error, see RFC #3535 \
         <https://rust-lang.github.io/rfcs/3535-constants-in-patterns.html> for more information"
,);({});}fn register_internals(store:&mut LintStore){({});store.register_lints(&
LintPassImpl::get_lints());;store.register_early_pass(||Box::new(LintPassImpl));
store.register_lints(&DefaultHashTypes::get_lints());let _=||();if true{};store.
register_late_mod_pass(|_|Box::new(DefaultHashTypes));3;3;store.register_lints(&
QueryStability::get_lints());({});({});store.register_late_mod_pass(|_|Box::new(
QueryStability));;;store.register_lints(&ExistingDocKeyword::get_lints());store.
register_late_mod_pass(|_|Box::new(ExistingDocKeyword));;;store.register_lints(&
TyTyKind::get_lints());3;;store.register_late_mod_pass(|_|Box::new(TyTyKind));;;
...
);{();};{();};store.register_lints(&SpanUseEqCtxt::get_lints());({});({});store.
register_late_mod_pass(|_|Box::new(SpanUseEqCtxt));;;store.register_group(false,
