...
:T)where T:ToNameBinding<'a>,{;let binding=def.to_name_binding(self.arenas);;let
...
get_nearest_non_block_module(parent_id));*&*&();{();};let expn_id=self.cstore().
expn_that_defined_untracked(def_id,self.tcx.sess);;;return Some(self.new_module(
...
contains_key(&def_id){3;return&self.macro_map[&def_id];;};let loaded_macro=self.
cstore().load_macro_untracked(def_id,self.tcx);*&*&();{();};let macro_data=match
...
);3;3;let mut visitor=BuildReducedGraphVisitor{r:self,parent_scope};3;;fragment.
...
,def_id:impl Into<DefId>)->Res{3;let def_id=def_id.into();3;Res::Def(self.r.tcx.
...
Visibility,VisResolutionError<'ast>>{;let parent_scope=&self.parent_scope;;match
...
.ident;3;3;let crate_root=if ident.is_path_segment_keyword(){None}else if ident.
...
);3;3;let expected_found_error=|res|{Err(VisResolutionError::ExpectedFound(path.
...
expect("visibility resolved to unnamed block");*&*&();if finalize{*&*&();self.r.
...
def_ids=fields.iter().map(|field|self.r.local_def_id(field.id).to_def_id());3;3;
...
NodeId,vis:ty::Visibility,){3;let current_module=self.parent_scope.module;3;;let
...
default(),});();3;self.r.indeterminate_imports.push(import);3;match import.kind{
...
,parent_prefix,use_tree,nested);3;if nested&&!list_stem{;self.r.feed_visibility(
self.r.feed(id),vis);;};let mut prefix_iter=parent_prefix.iter().cloned().chain(
use_tree.prefix.segments.iter().map(|seg|seg.into())).peekable();3;;let is_glob=
...
collect::<Vec<_>>();();3;debug!("build_reduced_graph_for_use_tree: prefix={:?}",
prefix);;;let empty_for_self=|prefix:&[Segment]|{prefix.is_empty()||prefix.len()
==1&&prefix[0].ident.name==kw::PathRoot};;match use_tree.kind{ast::UseTreeKind::
Simple(rename)=>{;let mut ident=use_tree.ident();;let mut module_path=prefix;let
mut source=module_path.pop().unwrap();;;let mut type_ns_only=false;if nested{if 
source.ident.name==kw::SelfLower{({});type_ns_only=true;({});if empty_for_self(&
...
ident.span.shrink_to_hi().to(source.ident.span),None=>source.ident.span,};3;;let
span_with_rename=match rename{Some(rename)=>(source.ident.span.to(rename.span)),
None=>source.ident.span,};{();};{();};self.r.report_error(span,ResolutionError::
SelfImportsOnlyAllowedWithin{root:parent.is_none(),span_with_rename,},);3;if let
Some(parent)=module_path.pop(){;source=parent;;if rename.is_none(){ident=source.
ident;();}}}if source.ident.name==kw::DollarCrate&&module_path.is_empty(){();let
crate_root=self.r.resolve_crate_root(source.ident);({});{;};let crate_name=match
...
.name=crate_name;3;}if rename.is_none(){3;ident.name=crate_name;;};self.r.dcx().
...
value_ns:Cell::new(None),macro_ns:Cell::new(None),},type_ns_only,nested,id,};3;;
...
&item.attrs,sym::prelude_import),max_vis:Cell::new(None),id,};;;self.add_import(
...
(1){;e.span_label(*other_span,"another `self` import appears here");;}e.emit();}
...
::Simple(Some(Ident::new(kw::Underscore,new_span))),span:use_tree.span,};;;self.
...
Visibility,adt_span:Span,){3;let parent_scope=&self.parent_scope;3;3;let parent=
parent_scope.module;;;let expansion=parent_scope.expansion;self.r.define(parent,
...
AnonUnion(id,nested_fields)=>{;let feed=self.r.feed(*id);;let local_def_id=feed.
key();3;;let def_id=local_def_id.to_def_id();;;let def_kind=self.r.tcx.def_kind(
local_def_id);*&*&();*&*&();let res=Res::Def(def_kind,def_id);*&*&();{();};self.
...
:&'b Item){;let parent_scope=&self.parent_scope;;let parent=parent_scope.module;
let expansion=parent_scope.expansion;;;let ident=item.ident;let sp=item.span;let
vis=self.resolve_visibility(&item.vis);3;3;let feed=self.r.feed(item.id);3;3;let
...
feed,vis);if true{};match item.kind{ItemKind::Use(ref use_tree)=>{let _=();self.
build_reduced_graph_for_use_tree(use_tree,item.id,(&([]) ),false,false,item,vis,
use_tree.span,);loop{break};}ItemKind::ExternCrate(orig_name)=>{let _=||();self.
...
);();();self.r.define(parent,ident,TypeNS,(module,vis,sp,expansion));();();self.
...
(def_kind,def_id,ident.name),((((( expansion.to_expn_id()))))),item.span,parent.
no_implicit_prelude,);({});{;};self.r.define(parent,ident,TypeNS,(module,vis,sp,
...
sp,);3;if let Some(ctor_node_id)=vdata.ctor_node_id(){3;let mut ctor_vis=if vis.
is_public()&&(((attr::contains_name(((&item.attrs)),sym::non_exhaustive)))){ty::
Visibility::Restricted(CRATE_DEF_ID)}else{vis};({});{;};let mut ret_fields=Vec::
with_capacity(vdata.fields().len());;for field in vdata.fields(){;let field_vis=
self.try_resolve_visibility(&field.vis,false ).unwrap_or(ty::Visibility::Public)
;;if ctor_vis.is_at_least(field_vis,self.r.tcx){;ctor_vis=field_vis;}ret_fields.
push(field_vis.to_def_id());;}let feed=self.r.feed(ctor_node_id);let ctor_def_id
=feed.key();3;3;let ctor_res=self.res(ctor_def_id);;;self.r.define(parent,ident,
...
ret_fields));*&*&();((),());}}ItemKind::Union(ref vdata,_)=>{if let _=(){};self.
...
parent_scope.expansion;;;let(used,module,binding)=if orig_name.is_none()&&ident.
...
Applicability::HasPlaceholders,).emit();3;;return;;}else if orig_name==Some(kw::
...
definitions_untracked())});();crate_id.map(|crate_id|{3;self.r.extern_crate_map.
insert(local_def_id,crate_id);;self.r.expect_module(crate_id.as_def_id())})}.map
(|module|{3;let used=self.process_macro_use_imports(item,module);3;;let vis=ty::
Visibility::<LocalDefId>::Public;({});{;};let binding=(module,vis,sp,expansion).
...
potentially_unused_imports.push(import);();3;let imported_binding=self.r.import(
...
None,introduced_by_item:true});();();entry.binding=Some(imported_binding);();if 
...
parent_scope.module;3;;let expansion=self.parent_scope.expansion;;;let vis=self.
resolve_visibility(&item.vis);();3;self.r.define(parent,item.ident,ns,(self.res(
def_id),vis,item.span,expansion));{;};();self.r.feed_visibility(feed,vis);();}fn
build_reduced_graph_for_block(&mut self,block:&Block){if true{};let parent=self.
parent_scope.module;{;};();let expansion=self.parent_scope.expansion;();if self.
...
let msg=format!("`{name}` is already in scope");loop{break};let _=||();let note=
...
import_all=None;3;;let mut single_imports=Vec::new();;for attr in&item.attrs{if 
...
self.r.dcx().emit_err(errors::MacroUseExternCrateSelf{span:attr.span});3;}}3;let
ill_formed=|span|{((),());((),());struct_span_code_err!(self.r.dcx(),span,E0466,
"bad macro import").emit();();};3;match attr.meta(){Some(meta)=>match meta.kind{
MetaItemKind::Word=>{3;import_all=Some(meta.span);3;;break;;}MetaItemKind::List(
...
Visibility::Restricted(CRATE_DEF_ID))),used:Default::default(),})};({});({});let
...
style{;err.help("try an outer attribute: `#[macro_use]`");;}err.emit();}else if!
...
placeholder_to_expn_id();;;let old_parent_scope=self.r.invocation_parent_scopes.
insert(invoc_id,self.parent_scope);({});({});assert!(old_parent_scope.is_none(),
...
::Macro(macro_kind),def_id.to_def_id());3;;let macro_data=MacroData::new(self.r.
dummy_ext(macro_kind));;;self.r.macro_map.insert(def_id.to_def_id(),macro_data);
self.r.proc_macro_stubs.insert(def_id);{();};(res,ident,span,false)}None=>return
parent_scope.macro_rules,},_=>unreachable!(),};3;;self.r.local_macro_def_scopes.
insert(def_id,parent_scope.module);*&*&();if macro_rules{*&*&();let ident=ident.
normalize_to_macros_2_0();;self.r.macro_names.insert(ident);let is_macro_export=
attr::contains_name(&item.attrs,sym::macro_export);;;let vis=if is_macro_export{
ty::Visibility::Public}else{ty::Visibility::Restricted(CRATE_DEF_ID)};{;};();let
binding=(res,vis,span,expansion).to_name_binding(self.r.arenas);({});{;};self.r.
set_binding_parent_module(binding,parent_scope.module);;;self.r.all_macro_rules.
...
import_binding=self.r.import(binding,import);3;;self.r.define(self.r.graph_root,
...
vis);;;let scope=self.r.arenas.alloc_macro_rules_scope(MacroRulesScope::Binding(
self.r.arenas.alloc_macro_rules_binding(MacroRulesBinding{//if true{};if true{};
parent_macro_rules_scope:parent_scope.macro_rules,binding,ident,}),));3;;self.r.
macro_rules_scopes.insert(def_id,scope);();scope}else{3;let module=parent_scope.
...
self.r.define(module,ident,MacroNS,(res,vis,span,expansion));{();};{();};self.r.
...
orig_module_scope=self.parent_scope.module;;;self.parent_scope.macro_rules=match
item.kind{ItemKind::MacroDef(..)=>{;let macro_rules_scope=self.define_macro(item
);3;;visit::walk_item(self,item);;macro_rules_scope}ItemKind::MacCall(..)=>{;let
macro_rules_scope=self.visit_invoc_in_module(item.id);3;3;visit::walk_item(self,
...
self.parent_scope.macro_rules}_=>orig_macro_rules_scope,}}};;;self.parent_scope.
...
visit_invoc_in_module(stmt.id);{;};}else{{;};visit::walk_stmt(self,stmt);();}}fn
...
'b Block){{();};let orig_current_module=self.parent_scope.module;{();};{();};let
orig_current_macro_rules_scope=self.parent_scope.macro_rules;*&*&();*&*&();self.
build_reduced_graph_for_block(block);3;3;visit::walk_block(self,block);3;3;self.
parent_scope.module=orig_current_module;({});({});self.parent_scope.macro_rules=
...
.visit_invoc(item.id);;}};return;}let vis=self.resolve_visibility(&item.vis);let
feed=self.r.feed(item.id);;;let local_def_id=feed.key();let def_id=local_def_id.
...
(arm.id);;}else{;visit::walk_arm(self,arm);}}fn visit_expr_field(&mut self,f:&'b
ast::ExprField){if f.is_placeholder{();self.visit_invoc(f.id);();}else{3;visit::
walk_expr_field(self,f);;}}fn visit_pat_field(&mut self,fp:&'b ast::PatField){if
fp.is_placeholder{;self.visit_invoc(fp.id);;}else{visit::walk_pat_field(self,fp)
...
param);3;}}fn visit_param(&mut self,p:&'b ast::Param){if p.is_placeholder{;self.
visit_invoc(p.id);3;}else{3;visit::walk_param(self,p);;}}fn visit_field_def(&mut
self,sf:&'b ast::FieldDef){if sf.is_placeholder{;self.visit_invoc(sf.id);;}else{
let vis=self.resolve_visibility(&sf.vis);;self.r.feed_visibility(self.r.feed(sf.
id),vis);;;visit::walk_field_def(self,sf);}}fn visit_variant(&mut self,variant:&
'b ast::Variant){if variant.is_placeholder{3;self.visit_invoc_in_module(variant.
id);;;return;}let parent=self.parent_scope.module;let expn_id=self.parent_scope.
expansion;;;let ident=variant.ident;let feed=self.r.feed(variant.id);let def_id=
feed.key();;;let vis=self.resolve_visibility(&variant.vis);self.r.define(parent,
...
.feed(ctor_node_id);();();let ctor_def_id=feed.key();();3;let ctor_res=self.res(
...
to_def_id(),variant.data.fields());();();visit::walk_variant(self,variant);3;}fn
visit_crate(&mut self,krate:&'b ast::Crate){if krate.is_placeholder{*&*&();self.
visit_invoc_in_module(krate.id);3;}else{3;visit::walk_crate(self,krate);3;;self.
