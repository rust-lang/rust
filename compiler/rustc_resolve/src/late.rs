...
impl<'a, 'ast, 'ra, 'tcx> LateResolutionVisitor<'a, 'ast, 'ra, 'tcx> {
    ...
    fn resolve_pattern_top(&mut self, pat: &'ast Pat, pat_src: PatternSource) {
            ...
            && let Some(items) = self.r.lookahead_items_in_block.get(&id)
        {
            ...
            for ((block_id, macro_def_id, ident), res) in extend_bindings {
                let Some(LookaheadItemInBlock::MacroDef { bindings: macro_bindings, .. }) = self
                    .r
                    .lookahead_items_in_block
                    .get_mut(&block_id)
                    .and_then(|block| block.get_mut(&macro_def_id))
                else {
                    unreachable!()
                };
                macro_bindings.insert(ident, (self.parent_scope.module, res));
            }
        }
