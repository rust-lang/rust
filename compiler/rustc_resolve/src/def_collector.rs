...
use crate::{
    ConstOwner, ImplTraitContext, InvocationParent, ParentScope, Resolver, with_owner,
    with_owner_tables,
};
...
impl<'a, 'ra, 'tcx> DefCollector<'a, 'ra, 'tcx> {
    ...
    fn create_closure(&mut self, expr: &'a Expr, constness: Const) {
        let feed = self.create_def(expr.id, None, DefKind::Closure, expr.span);

        let constness = match (constness, self.invocation_parent.const_owner) {
            (Const::No, _) => Const::No,
            ...
            (Const::Yes(span), None | Some((_, Const::No))) => {
                self.r
                    .tcx
                    .dcx()
                    .span_err(span, "cannot use `const` closures outside of const contexts");
                Const::No
            }
            (Const::Yes(_), Some((_, Const::Yes(_) | Const::Always(_)))) => constness,
        };
        let constness = self.lower_constness(constness);
        feed.constness(constness);
        self.with_parent(feed.def_id(), |this| visit::walk_expr(this, expr));
    }
}
...
impl<'a, 'ra, 'tcx> visit::Visitor<'a> for DefCollector<'a, 'ra, 'tcx> {
                ...
                let constness =
                    match (sig.header.constness, self.invocation_parent.const_owner.unwrap(), ctxt)
                    {
                        ...
                        (
                            ...
                            self.r.tcx.dcx().emit_err(TraitFnConst {
                                span,
                        ...
                        (
                            ...
                            self.r.tcx.dcx().emit_err(ImplFnConst { span, parent_constness });
    ...
    fn visit_anon_const(&mut self, constant: &'a AnonConst) {
        ...
        self.with_const_owner(Some((ConstOwner::Const, Const::Yes(constant.value.span))), |this| {
    ...
    fn visit_expr(&mut self, expr: &'a Expr) {
        ...
        match &expr.kind {
            ...
            ExprKind::Closure(closure) => self.create_closure(expr, closure.constness),
            ExprKind::Gen(..) => self.create_closure(expr, Const::No),
