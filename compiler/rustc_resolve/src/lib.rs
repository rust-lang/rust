...
use rustc_hir::def::{
    self, CtorOf, DefKind, DocLinkResMap, MacroKinds, NonMacroAttrKind, PartialRes, PerNS, ViaCrate,
};
...
#[derive(Debug)]
enum PathResult<'ra> {
    Module(ModuleOrUniformRoot<'ra>, Option<ViaCrate>),
...
struct DeclData<'ra> {
    kind: DeclKind<'ra>,
    /// The direct dependency edge through which this declaration was imported.
...
pub struct Resolver<'ra, 'tcx> {
    ...
    /// The first-hop dependency selected by each resolved macro invocation. `$crate` paths
    /// produced by an expansion inherit this value.
    macro_invocation_provenance: FxHashMap<LocalExpnId, ViaCrate> = default::fx_hash_map(),
...
impl<'ra> ResolverArenas<'ra> {
    fn new_def_decl(
        ...
        self.new_def_decl_with_via(res, vis, span, expansion, parent_module, None)
    }

    fn new_def_decl_with_via(
        &'ra self,
        res: Res,
        vis: Visibility<ModId>,
        span: Span,
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    ...
    fn direct_dependency_provenance(&self, krate: CrateNum, extern_name: Symbol) -> ViaCrate {
        ViaCrate {
            krate,
            extern_name,
            is_private: self
                ...
                .opts
                .externs
                .get(extern_name.as_str())
                .is_some_and(|entry| entry.is_private_dep),
        }
    }

    fn dollar_crate_provenance(&self, ident: Ident) -> Option<ViaCrate> {
        ...
        ident
            ...
            .outer_expn()
            .as_local()
            .and_then(|expn_id| self.macro_invocation_provenance.get(&expn_id).copied())
    }
                ...
                PendingDecl::Pending => {
                    ...
                    if is_open {
                        ...
                        crate_id.map(|crate_id| {
                            ...
                            let via_crate = self.direct_dependency_provenance(crate_id, ident.name);
                            self.arenas.new_def_decl_with_via(
                                res,
                                Visibility::Public,
                                DUMMY_SP,
                                LocalExpnId::ROOT,
