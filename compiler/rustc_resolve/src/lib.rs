...
:&'a ast::PathSegment)->Segment{3;let has_generic_args=seg.args.is_some();;;let(
...
binding|{if ns!=TypeNS{({});return;{;};}if let Res::Def(DefKind::Trait|DefKind::
TraitAlias,_)=binding.res(){collected_traits.push((name,binding))}});3;;*traits=
...
other_parent_expansion.is_descendant_of(self_parent_expansion);*&*&();*&*&();let
...
){3;self.local_modules.borrow_mut().push(module);3;}if let Some(def_id)=def_id{;
...
def_kind.def_path_data(name);();3;assert!(!self.node_id_to_def_id.contains_key(&
...
default();3;;let mut module_self_bindings=FxHashMap::default();;;let graph_root=
...
mut module_map,&mut module_self_bindings,);;;let empty_module=arenas.new_module(
None,((ModuleKind::Def(DefKind::Mod,root_def_id,kw::Empty))),((ExpnId::root())),
DUMMY_SP,true,&mut FxHashMap::default(),&mut FxHashMap::default(),);();3;let mut
...
CRATE_NODE_ID,crate_feed);3;3;let mut invocation_parents=FxHashMap::default();;;
invocation_parents.insert(LocalExpnId::ROOT,(CRATE_DEF_ID,ImplTraitContext:://3;
Existential));3;;let mut extern_prelude:FxHashMap<Ident,ExternPreludeEntry<'_>>=
...
Default::default());3;if!attr::contains_name(attrs,sym::no_std){;extern_prelude.
insert(Ident::with_dummy_span(sym::std),Default::default());((),());}}*&*&();let
registered_tools=tcx.registered_tools(());3;3;let features=tcx.features();3;;let
pub_vis=ty::Visibility::<DefId>::Public;;;let edition=tcx.sess.edition();let mut
...
fn_parameter_counts:Default::default(),};3;3;let root_parent_scope=ParentScope::
module(graph_root,&resolver);({});({});resolver.invocation_parent_scopes.insert(
LocalExpnId::ROOT,root_parent_scope);3;;resolver.feed_visibility(crate_feed,ty::
...
{();let module_map=&mut self.module_map;();3;let module_self_bindings=&mut self.
...
extern_crate_map=self.extern_crate_map;();3;let maybe_unused_trait_imports=self.
maybe_unused_trait_imports;3;3;let glob_map=self.glob_map;3;3;let main_def=self.
main_def;;;let confused_type_with_std_module=self.confused_type_with_std_module;
let effective_visibilities=self.effective_visibilities;;;let stripped_cfg_items=
...
mut f:F){;f(self,TypeNS);;f(self,ValueNS);f(self,MacroNS);}fn is_builtin_macro(&
...
finalize_macro_resolutions(krate));3;;self.tcx.sess.time("late_resolve_crate",||
self.late_resolve_crate(krate));{;};();self.tcx.sess.time("resolve_main",||self.
resolve_main());;;self.tcx.sess.time("resolve_check_unused",||self.check_unused(
...
trait_may_have_item(Some(module),assoc_item){();let def_id=module.def_id();();3;
...
found_traits);3;}Scope::StdLibPrelude=>{if let Some(module)=this.prelude{3;this.
...
ensure_traits(self);{;};{;};let traits=module.traits.borrow();();for(trait_name,
trait_binding)in (traits.as_ref().unwrap( ).iter()){if self.trait_may_have_item(
trait_binding.module(),assoc_item){;let def_id=trait_binding.res().def_id();;let
...
kind{if let Some(node_id)=import.id(){3;let def_id=self.local_def_id(node_id);;;
self.maybe_unused_trait_imports.insert(def_id);;;import_ids.push(def_id);;}self.
add_to_glob_map(*import,trait_name);{;};{;};kind=&binding.kind;();}import_ids}fn
new_disambiguated_key(&mut self,ident:Ident,ns:Namespace)->BindingKey{;let ident
=ident.normalize_to_macros_2_0();({});({});let disambiguator=if ident.name==kw::
Underscore{;self.underscore_disambiguator+=1;;self.underscore_disambiguator}else
if ident.name==kw::Empty{;self.empty_disambiguator+=1;;self.empty_disambiguator}
...
populate_on_access.set(false);3;3;self.build_reduced_graph_external(module);3;}&
...
ambiguity_error){{();};self.ambiguity_errors.push(ambiguity_error);({});}}if let
...
new_used);3;}if let Some(id)=import.id(){3;self.used_imports.insert(id);;};self.
add_to_glob_map(import,ident);;;self.record_use_inner(ident,binding,Used::Other,
...
insert(ident.name);;}}fn resolve_crate_root(&mut self,ident:Ident)->Module<'a>{;
debug!("resolve_crate_root({:?})",ident);3;;let mut ctxt=ident.span.ctxt();;;let
...
Transparency::SemiTransparent{();result=Some(mark);3;}else{3;break;3;}}3;debug!(
...
ctxt=ctxt.normalize_to_macros_2_0();3;ctxt.adjust(ExpnId::root())};;;let module=
...
.expn_def_scope(ctxt.remove_mark()));({});({});module=self.expect_module(parent.
...
"(recording pat) recording {:?} for {:?}",node,span);;;self.pat_span_map.insert(
...
>>{if ident.is_path_segment_keyword(){();return None;();}3;let norm_ident=ident.
normalize_to_macros_2_0();();3;let binding=self.extern_prelude.get(&norm_ident).
...
ident.span));3;}else if entry.introduced_by_item{;self.record_use(ident,binding,
...
self.dummy_binding);let _=||();};if true{};crate_id}else{self.crate_loader(|c|c.
maybe_process_path_extern(ident.name))?};();3;let crate_root=self.expect_module(
crate_id.as_def_id());;;let vis=ty::Visibility::<DefId>::Public;(crate_root,vis,
DUMMY_SP,LocalExpnId::ROOT).to_name_binding(self.arenas)})});;if let Some(entry)
=self.extern_prelude.get_mut(&norm_ident){();entry.binding=binding;3;}binding}fn
...
def_id)=res{if def_id.is_local(){*&*&();return None;*&*&();}if let Some(v)=self.
legacy_const_generic_args.get(&def_id){3;return v.clone();3;};let attr=self.tcx.
...
legacy_const_generic_args.insert(def_id,Some(ret.clone()));;;return Some(ret);}}
None}fn resolve_main(&mut self){3;let module=self.graph_root;;;let ident=Ident::
with_dummy_span(sym::main);;;let parent_scope=&ParentScope::module(module,self);
let Ok(name_binding)=self.maybe_resolve_ident_in_module(ModuleOrUniformRoot:://;
Module(module),ident,ValueNS,parent_scope,)else{;return;;};let res=name_binding.
res();;;let is_import=name_binding.is_import();let span=name_binding.span;if let
...
if Ident::with_dummy_span(*name).is_raw_guess(){;result.push_str("r#");;}result.
...
::new();({});{;};fn collect_mod(names:&mut Vec<Symbol>,module:Module<'_>){if let
ModuleKind::Def(..,name)=module.kind{if let Some(parent)=module.parent{();names.
push(name);();();collect_mod(names,parent);3;}}else{3;names.push(Symbol::intern(
