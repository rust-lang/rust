...
pub trait Visitor<'v>: Sized {
    ...
    fn visit_lit(&mut self, _hir_id: HirId, _lit: &'v Lit) -> Self::Result {
        Self::Result::output()
    }
...
pub fn walk_pat_expr<'v, V: Visitor<'v>>(visitor: &mut V, expr: &'v PatExpr<'v>) -> V::Result {
    ...
    match &expr.kind {
        PatExprKind::Lit { lit } => visitor.visit_lit(expr.hir_id, lit),
...
pub fn walk_expr<'v, V: Visitor<'v>>(visitor: &mut V, expression: &'v Expr<'v>) -> V::Result {
    ...
    match expression.kind {
        ...
        ExprKind::Lit(lit) => try_visit!(visitor.visit_lit(expression.hir_id, lit)),
        ExprKind::Err(_) => {}
    }
    V::Result::output()
}
