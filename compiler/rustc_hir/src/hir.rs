...
pub enum ExprKind<'hir> {
    ...
    /// [BPF Type Format (BTF)][btf] relocation.
    ///
    /// [BTF][btf] is the type metadata format used by the Linux kernel and
    /// eBPF tooling for relocations: the compiled program records which field
    /// or array element it intended to access, and the loader rewrites the
    /// bytecode to match the layout of the kernel it is about to run on.
    ///
