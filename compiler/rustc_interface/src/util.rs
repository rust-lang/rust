...
>(
    ...
    let Some(dyn_thread_safe) = sync::DynThreadSafe::check() else {
    ...
    let current_gcx = dyn_thread_safe.with(CurrentGcx::new());
...
internal compiler error: query cycle handler thread panicked, aborting process";
    ...
    rustc_span::create_session_globals_then(edition, extra_symbols, Some(sm_inputs), || {
        rustc_span::with_session_globals(|session_globals| {
            let session_globals = dyn_thread_safe.with(session_globals);
            builder
                .build_scoped(
