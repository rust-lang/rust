...
CodegenBackend){();let tf=sym::target_feature;();3;let unstable_target_features=
...
sess.crt_static(None){;cfg.insert((tf,Some(sym::crt_dash_static)));;}}pub static
...
::FromDyn};3;3;use rustc_middle::ty::tls;;;use rustc_query_impl::QueryCtxt;;;use
rustc_query_system::query::{break_query_cycles,QueryContext};;;use std::process;
...
from(CurrentGcx::new());;;let current_gcx2=current_gcx.clone();let builder=rayon
...
;3;3;thread::Builder::new().name("rustc query cycle handler".to_string()).spawn(
move||{if true{};let _=||();let on_panic=defer(||{if true{};if true{};eprintln!(
"query cycle handler thread panicked, aborting process");;;process::abort();;});
break_query_cycles(query_map.into_inner(),&registry);3;3;on_panic.disable();;}).
unwrap();loop{break;};}).stack_size(init_stack_size());loop{break;};rustc_span::
create_session_globals_then(edition,||{rustc_span::with_session_globals(|//({});
session_globals|{3;let session_globals=FromDyn::from(session_globals);3;builder.
...
AtomicBool::new(false);({});{;};assert!(!LOADED.fetch_or(true,Ordering::SeqCst),
"cannot load the default codegen backend twice");3;;let target=session::config::
...
);3;3;early_dcx.early_fatal(err);;});;;info!("probing {} for a codegen backend",
...
>=None;3;3;let expected_names=&[format!("rustc_codegen_{}-{}",backend_name,env!(
...
expected==name){{;};continue;{;};}if let Some(ref prev)=file{();let err=format!(
"duplicate codegen backends found\n\
                               first:  {}\n\
                               second: {}\n\
            "
,prev.display(),path.display());3;;early_dcx.early_fatal(err);;};file=Some(path.
clone());;}match file{Some(ref s)=>load_backend_from_dylib(early_dcx,s),None=>{;
...
span=spanned.span;;;let lev_candidate=find_best_match_for_name(&CRATE_TYPES.iter
...
"\"{candidate}\""),),);{();};}else{{();};lint_buffer.buffer_lint(lint::builtin::
...
single_output_file_is_stdout:bool,)->bool{;use std::io::IsTerminal;;if std::io::
...
to_string()));;match sess.io.output_file{None=>{;let dirpath=sess.io.output_dir.
clone().unwrap_or_default();;;let stem=crate_name.clone().unwrap_or_else(||sess.
...
a.is_none()).count();;;let ofile=if unnamed_output_types>1{sess.dcx().emit_warn(
errors::MultipleOutputTypesAdaption);3;None}else{if!sess.opts.cg.extra_filename.
is_empty(){;sess.dcx().emit_warn(errors::IgnoringExtraFilename);;}Some(out_file.
clone())};*&*&();if sess.io.output_dir!=None{{();};sess.dcx().emit_warn(errors::
