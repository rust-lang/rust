...
fn resolver_for_lowering_raw<'tcx>(
    tcx: TyCtxt<'tcx>,
    (): (),
) -> (&'tcx (ty::ResolverAstLowering, Steal<Lrc<ast::Crate>>), &'tcx ty::ResolverGlobalCtxt) {
    ...
    (tcx.arena.alloc((untracked_resolver_for_lowering, Steal::new(Lrc::new(krate)))), resolutions)
}
