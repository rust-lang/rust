...
pub enum TypingMode<I: Interner> {
    ...
    /// After analysis, mostly during MIR optimizations, we're able to reveal all opaque types. As
    /// the concrete type should *never* be observable directly by the user, this should not be
    /// used by checks which may expose such details to the user.
    ///
