...
bitflags::bitflags! {
    ...
    pub struct TypeFlags: u32 {
        ...
        const HAS_ERROR                   = TypeFlags::HAS_TY_OR_CT_ERROR.bits()
...
impl<I: Interner> FlagComputation<I> {
    ...
    fn add_kind(&mut self, kind: &ty::TyKind<I>) {
        match *kind {
            ...
            ty::Error(_) => self.add_flags(TypeFlags::HAS_TY_OR_CT_ERROR),
    ...
    fn add_const_kind(&mut self, c: &ty::ConstKind<I>) {
        match *c {
            ...
            ty::ConstKind::Error(_) => self.add_flags(TypeFlags::HAS_TY_OR_CT_ERROR),
        }
    }
