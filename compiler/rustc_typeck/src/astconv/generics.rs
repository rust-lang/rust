...
pub fn create_substs_for_generic_args<'a, 'tcx>(
    tcx: TyCtxt<'tcx>,
    def_id: DefId,
            ...
            match (args.peek(), params.peek()) {
                                    ...
                                    .map(|param| {
                                        (
                                            match param.kind {
                                                ...
                                                GenericParamDefKind::Const => ParamKindOrd::Const {
                ...
                (Some(&arg), None) => {
                        ...
                        && arg_count.explicit_late_bound == ExplicitLateBound::No
                    {
                        ...
                        generic_arg_mismatch_err(tcx.sess, provided, kind, false, None);
                    }
...
pub(crate) fn check_generic_arg_count(
    ...
    if position != GenericArgPosition::Type && !args.bindings.is_empty() {
        prohibit_assoc_ty_binding(tcx, args.bindings[0].span);
    }

    let explicit_late_bound = prohibit_explicit_late_bound_lifetimes(tcx, def, args, position);

    let check_kind_count =
        |kind, required, permitted, provided, offset, unexpected_spans: &mut Vec<Span>, silent| {
            debug!(
    ...
    let arg_count_correct = check_kind_count(
        kind_str,
        if infer_args { 0 } else { param_counts.consts + named_type_param_count - defaults.types },
...
pub(crate) fn prohibit_explicit_late_bound_lifetimes(
    ...
    if infer_lifetimes {
        ...
        if position == GenericArgPosition::Value && arg_counts.lifetimes != param_counts.lifetimes {
