...
pub fn ast_region_to_region<'tcx>(
    ...
    let r = match tcx.named_region(lifetime.hir_id) {
        ...
        Some(rl::Region::Free(scope, id)) => {
            let name = lifetime_name(id.expect_local());
            tcx.mk_region(ty::ReFree(ty::FreeRegion { scope, bound_region: ty::BrNamed(id, name) }))

            // (*) -- not late-bound, won't change
        }

        None => {
            astconv.re_infer(def, lifetime.span).unwrap_or_else(|| {
...
pub fn ast_path_substs_for_ty<'tcx>(
    ...
    let (substs, assoc_bindings, _) = create_substs_for_ast_path(
        astconv,
        span,
        def_id,
        &[],
    ...
    if let Some(b) = assoc_bindings.first() {
        generics::prohibit_assoc_ty_binding(astconv.tcx(), b.span);
    }
...
/// lists: `[Vec<u8>, u8, 'a]`.
fn create_substs_for_ast_path<'a, 'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    def_id: DefId,
    ...
    let arg_count = generics::check_generic_arg_count(
        tcx,
        span,
        &generic_params,
        &generic_args,
    ...
    struct SubstsForAstPathCtxt<'a, A> {
        astconv: &'a A,
        def_id: DefId,
    ...
    impl<'tcx, 'a, A: AstConv<'tcx>> SubstsForAstPathCtxt<'a, A> {
    ...
    impl<'a, 'tcx, A> CreateSubstsForGenericArgsCtxt<'a, 'tcx> for SubstsForAstPathCtxt<'a, A>
    where
        A: AstConv<'tcx>,
    {
        ...
        fn provided_kind(
            ...
            match (&param.kind, arg) {
                (GenericParamDefKind::Lifetime, GenericArg::Lifetime(lt)) => {
                    ast_region_to_region(self.astconv, &lt, Some(param)).into()
                }
                (&GenericParamDefKind::Type { has_default, .. }, GenericArg::Type(ty)) => {
                    ...
                    if let (hir::TyKind::Infer, false) = (&ty.kind, self.astconv.allow_ty_infer()) {
                        ...
                        ast_ty_to_ty(self.astconv, &ty).into()
                    }
                }
            ...
            match param.kind {
                ...
                GenericParamDefKind::Type { has_default, .. } => {
                    if !infer_args && has_default {
                        ...
                        let param =
                            if !self.default_needs_object_self(param) { Some(param) } else { None };
    ...
    let substs = generics::create_substs_for_generic_args(
        tcx,
        def_id,
        parent_substs,
        self_ty.is_some(),
    ...
    errors::complain_about_missing_type_params(
        astconv,
        ...
        .map(|binding| {
            let kind = match binding.kind {
                hir::TypeBindingKind::Equality { ref ty } => {
                    ConvertedBindingKind::Equality(ast_ty_to_ty(astconv, ty))
                }
...
crate fn create_substs_for_associated_item<'tcx>(
    astconv: &impl AstConv<'tcx>,
    tcx: TyCtxt<'tcx>,
    span: Span,
    item_def_id: DefId,
    ...
    if tcx.generics_of(item_def_id).params.is_empty() {
        prohibit_generics(astconv, slice::from_ref(item_segment));
...
pub fn instantiate_mono_trait_ref<'tcx>(
    ...
    prohibit_generics(astconv, trait_ref.path.segments.split_last().unwrap().1);
...
pub(super) fn instantiate_poly_trait_ref_inner<'tcx>(
    astconv: &impl AstConv<'tcx>,
    trait_ref: &hir::TraitRef<'_>,
    span: Span,
    constness: Constness,
    ...
    let (substs, assoc_bindings, arg_count) = create_substs_for_ast_trait_ref(
        astconv,
        trait_ref.path.span,
        trait_def_id,
        self_ty,
    ...
    for binding in &assoc_bindings {
        ...
        let _: Result<_, ErrorReported> = add_predicates_for_ast_type_binding(
            astconv,
...
pub fn instantiate_lang_item_trait_ref<'tcx>(
    astconv: &impl AstConv<'tcx>,
    ...
    let trait_def_id = astconv.tcx().require_lang_item(lang_item, Some(span));

    let (substs, assoc_bindings, _) =
        create_substs_for_ast_path(astconv, span, trait_def_id, &[], args, false, Some(self_ty));
    let poly_trait_ref = ty::Binder::bind(ty::TraitRef::new(trait_def_id, substs));
...
fn ast_path_to_mono_trait_ref<'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    trait_def_id: DefId,
    self_ty: Ty<'tcx>,
    ...
    let (substs, assoc_bindings, _) =
        create_substs_for_ast_trait_ref(astconv, span, trait_def_id, self_ty, trait_segment);
    if let Some(b) = assoc_bindings.first() {
        generics::prohibit_assoc_ty_binding(astconv.tcx(), b.span);
    }
...
fn create_substs_for_ast_trait_ref<'a, 'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    trait_def_id: DefId,
    self_ty: Ty<'tcx>,
    ...
    errors::complain_about_internal_fn_trait(astconv, span, trait_def_id, trait_segment);
...
fn trait_defines_associated_type_named<'tcx>(
    astconv: &impl AstConv<'tcx>,
    trait_def_id: DefId,
    assoc_name: Ident,
) -> bool {
...
fn add_bounds<'tcx>(
    ...
    let constness = astconv.default_constness_for_trait_bounds();
    for ast_bound in ast_bounds {
        match *ast_bound {
            hir::GenericBound::Trait(ref b, hir::TraitBoundModifier::None) => {
                instantiate_poly_trait_ref(astconv, b, constness, param_ty, bounds);
            }
            hir::GenericBound::Trait(ref b, hir::TraitBoundModifier::MaybeConst) => {
                instantiate_poly_trait_ref(astconv, b, Constness::NotConst, param_ty, bounds);
            }
            hir::GenericBound::Trait(_, hir::TraitBoundModifier::Maybe) => {}
            hir::GenericBound::LangItemTrait(lang_item, span, hir_id, args) => {
                instantiate_lang_item_trait_ref(
                    astconv, lang_item, span, hir_id, args, param_ty, bounds,
                )
            }
            hir::GenericBound::Outlives(ref l) => bounds
                .region_bounds
                .push((ty::Binder::bind(ast_region_to_region(astconv, l, None)), l.span)),
        }
    }
}
...
pub fn compute_bounds<'tcx>(
    ...
    add_bounds(astconv, param_ty, ast_bounds, &mut bounds);
    bounds.trait_bounds.sort_by_key(|(t, _, _)| t.def_id());

    bounds.implicitly_sized = if let SizedByDefault::Yes = sized_by_default {
        if !is_unsized(astconv, ast_bounds, span) { Some(span) } else { None }
    } else {
        None
    };
...
fn add_predicates_for_ast_type_binding<'tcx>(
    astconv: &impl AstConv<'tcx>,
    ...
    if !speculative {
        ...
        if let ConvertedBindingKind::Equality(ty) = binding.kind {
            ...
            let late_bound_in_ty = tcx.collect_referenced_late_bound_regions(&ty::Binder::bind(ty));
            ...
            //                         ----  ----     ^^^^^^^
            validate_late_bound_regions(late_bound_in_trait_ref, late_bound_in_ty, |br_name| {
                struct_span_err!(
                    tcx.sess,
                    binding.span,
                    E0582,
    ...
    let candidate =
        if trait_defines_associated_type_named(astconv, trait_ref.def_id(), binding.item_name) {
    ...
    let assoc_ty = tcx
        ...
        .find(|i| i.kind == ty::AssocKind::Type && i.ident.normalize_to_macros_2_0() == assoc_ident)
    ...
    match binding.kind {
        ...
        ConvertedBindingKind::Constraint(ast_bounds) => {
            ...
            add_bounds(astconv, param_ty, ast_bounds, bounds);
        }
    }
    Ok(())
}
...
fn ast_path_to_ty<'tcx>(
    ...
    let tcx = astconv.tcx();
    let substs = ast_path_substs_for_ty(astconv, span, did, item_segment);
    astconv.normalize_ty(span, tcx.at(span).type_of(did).subst(tcx, substs))
}

fn conv_object_ty_poly_trait_ref<'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    ...
    for trait_bound in trait_bounds.iter().rev() {
        if let GenericArgCountResult {
            correct: Err(GenericArgCountMismatch { invalid_args: cur_potential_assoc_types, .. }),
            ..
    ...
    errors::complain_about_missing_associated_types(
        astconv,
        associated_types,
    ...
    let trait_ref_to_existential = |trait_ref: ty::TraitRef<'tcx>| {
        if trait_ref.self_ty() != dummy_self {
            ...
            tcx.sess.delay_span_bug(
                DUMMY_SP,
                &format!("trait_ref_to_existential called on {:?} with non-dummy Self", trait_ref,),
            );
        }
    ...
    let region_bound = if !lifetime.is_elided() {
        ast_region_to_region(astconv, lifetime, None)
    } else {
        compute_object_lifetime_bound(astconv, span, existential_predicates).unwrap_or_else(|| {
            if tcx.named_region(lifetime.hir_id).is_some() {
                ast_region_to_region(astconv, lifetime, None)
            } else {
                astconv.re_infer(None, span).unwrap_or_else(|| {
                    let mut err = struct_span_err!(
                        tcx.sess,
                        span,
                        E0228,
...
fn report_ambiguous_associated_type<'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    type_str: &str,
    trait_str: &str,
    ...
    let tcx = astconv.tcx();
    let mut err = struct_span_err!(tcx.sess, span, E0223, "ambiguous associated type");
...
// any ambiguity.
fn find_bound_for_assoc_item<'tcx>(
    astconv: &impl AstConv<'tcx>,
    ...
    let predicates =
        &astconv.get_type_parameter_bounds(span, ty_param_def_id.to_def_id()).predicates;
...
// errors otherwise.
fn one_bound_for_assoc_type<'tcx, I>(
    astconv: &impl AstConv<'tcx>,
    ...
    I: Iterator<Item = ty::PolyTraitRef<'tcx>>,
{
    let mut matching_candidates = all_candidates()
        .filter(|r| trait_defines_associated_type_named(astconv, r.def_id(), assoc_name));

    let bound = match matching_candidates.next() {
        Some(bound) => bound,
        None => {
            errors::complain_about_assoc_type_not_found(
                astconv,
                all_candidates,
                &ty_param_name(),
                assoc_name,
    ...
    if let Some(bound2) = matching_candidates.next() {
        ...
        for bound in bounds {
            ...
            let bound_span = tcx
                ...
                .find_by_name_and_kind(tcx, assoc_name, ty::AssocKind::Type, bound_id)
                .and_then(|item| tcx.hir().span_if_local(item.def_id));

            if let Some(bound_span) = bound_span {
                err.span_label(
                    bound_span,
                    format!("ambiguous `{}` from `{}`", assoc_name, bound.print_only_trait_path(),),
                );
...
// parameter or `Self`.
pub fn associated_path_to_ty<'tcx>(
    astconv: &impl AstConv<'tcx>,
    hir_ref_id: hir::HirId,
    span: Span,
    qself_ty: Ty<'tcx>,
        ...
        if adt_def.is_enum() {
            ...
            if let Some(variant_def) = variant_def {
                if permit_variants {
                    ...
                    prohibit_generics(astconv, slice::from_ref(assoc_segment));
    ...
    let bound = match (&qself_ty.kind(), qself_res) {
        ...
        ) => find_bound_for_assoc_item(astconv, param_did.expect_local(), assoc_ident, span)?,
        _ => {
    ...
    let ty = astconv.projected_ty_from_poly_trait_ref(span, item.def_id, assoc_segment, bound);
    let ty = astconv.normalize_ty(span, ty);

    let kind = DefKind::AssocTy;
    if !item.vis.is_accessible_from(def_scope, tcx) {
        let kind = kind.descr(item.def_id);
        let msg = format!("{} `{}` is private", kind, assoc_ident);
        tcx.sess.struct_span_err(span, &msg).span_label(span, &format!("private {}", kind)).emit();
    }
...
fn qpath_to_ty<'tcx>(
    ...
    let trait_ref = ast_path_to_mono_trait_ref(astconv, span, trait_def_id, self_ty, trait_segment);

    let item_substs = create_substs_for_associated_item(
        astconv,
        tcx,
        span,
        item_def_id,
    ...
    astconv.normalize_ty(span, tcx.mk_projection(item_def_id, item_substs))
}

pub fn prohibit_generics<'a, 'tcx, T: IntoIterator<Item = &'a hir::PathSegment<'a>>>(
    astconv: &impl AstConv<'tcx>,
    segments: T,
) -> bool {
    ...
    for segment in segments {
        ...
        if let [binding, ..] = segment.generic_args().bindings {
            has_err = true;
            generics::prohibit_assoc_ty_binding(astconv.tcx(), binding.span);
        }
    }
    has_err
}
...
pub fn def_ids_for_value_path_segments<'tcx>(
    astconv: &impl AstConv<'tcx>,
...
pub fn res_to_ty<'tcx>(
    ...
    match path.res {
        Res::Def(DefKind::OpaqueTy, did) => {
            ...
            prohibit_generics(astconv, item_segment.1);
            let substs = ast_path_substs_for_ty(astconv, span, did, item_segment.0);
            astconv.normalize_ty(span, tcx.mk_opaque(did, substs))
        }
        Res::Def(
            DefKind::Enum
            | DefKind::TyAlias
            ...
            prohibit_generics(astconv, path.segments.split_last().unwrap().1);
            ast_path_to_ty(astconv, span, did, path.segments.last().unwrap())
        }
        Res::Def(kind @ DefKind::Variant, def_id) if permit_variants => {
            ...
            let path_segs =
                def_ids_for_value_path_segments(astconv, &path.segments, None, kind, def_id);
            ...
            ast_path_to_ty(astconv, span, *def_id, &path.segments[*index])
        }
        Res::Def(DefKind::TyParam, def_id) => {
            assert_eq!(opt_self_ty, None);
            prohibit_generics(astconv, path.segments);
        ...
        Res::SelfTy(_, Some((def_id, forbid_generic))) => {
            ...
            let normalized_ty = astconv.normalize_ty(span, tcx.at(span).type_of(def_id));
        ...
        Res::Def(DefKind::AssocTy, def_id) => {
            ...
            prohibit_generics(astconv, &path.segments[..path.segments.len() - 2]);
            qpath_to_ty(
                astconv,
                span,
                opt_self_ty,
...
pub fn ast_ty_to_ty<'tcx>(astconv: &impl AstConv<'tcx>, ast_ty: &hir::Ty<'_>) -> Ty<'tcx> {
    ast_ty_to_ty_inner(astconv, ast_ty, false)
}

/// Turns a `hir::Ty` into a `Ty`. For diagnostics' purposes we keep track of whether trait
/// objects are borrowed like `&dyn Trait` to avoid emitting redundant errors.
fn ast_ty_to_ty_inner<'tcx>(
    astconv: &impl AstConv<'tcx>,
    ast_ty: &hir::Ty<'_>,
    borrowed: bool,
) -> Ty<'tcx> {
    ...
    let result_ty = match ast_ty.kind {
        hir::TyKind::Slice(ref ty) => tcx.mk_slice(ast_ty_to_ty(astconv, &ty)),
        hir::TyKind::Ptr(ref mt) => {
            tcx.mk_ptr(ty::TypeAndMut { ty: ast_ty_to_ty(astconv, &mt.ty), mutbl: mt.mutbl })
        }
        hir::TyKind::Rptr(ref region, ref mt) => {
            let r = ast_region_to_region(astconv, region, None);
            debug!("ast_ty_to_ty: r={:?}", r);
            let t = ast_ty_to_ty_inner(astconv, &mt.ty, true);
            tcx.mk_ref(r, ty::TypeAndMut { ty: t, mutbl: mt.mutbl })
        }
        hir::TyKind::Never => tcx.types.never,
        hir::TyKind::Tup(ref fields) => {
            tcx.mk_tup(fields.iter().map(|t| ast_ty_to_ty(astconv, &t)))
        }
        hir::TyKind::BareFn(ref bf) => {
            require_c_abi_if_c_variadic(tcx, &bf.decl, bf.abi, ast_ty.span);
            tcx.mk_fn_ptr(ty_of_fn(
                astconv,
                bf.unsafety,
                bf.abi,
                &bf.decl,
        ...
        hir::TyKind::TraitObject(ref bounds, ref lifetime) => {
            conv_object_ty_poly_trait_ref(astconv, ast_ty.span, bounds, lifetime, borrowed)
        }
        hir::TyKind::Path(hir::QPath::Resolved(ref maybe_qself, ref path)) => {
            debug!("ast_ty_to_ty: maybe_qself={:?} path={:?}", maybe_qself, path);
            let opt_self_ty = maybe_qself.as_ref().map(|qself| ast_ty_to_ty(astconv, qself));
            res_to_ty(astconv, opt_self_ty, path, false)
        }
        hir::TyKind::OpaqueDef(item_id, ref lifetimes) => {
            ...
            match opaque_ty.kind {
                hir::ItemKind::OpaqueTy(hir::OpaqueTy { impl_trait_fn, .. }) => {
                    impl_trait_ty_to_ty(astconv, def_id, lifetimes, impl_trait_fn.is_some())
                }
        ...
        hir::TyKind::Path(hir::QPath::TypeRelative(ref qself, ref segment)) => {
            ...
            let ty = ast_ty_to_ty(astconv, qself);
            ...
            associated_path_to_ty(astconv, ast_ty.hir_id, ast_ty.span, ty, res, segment, false)
                .map(|(ty, _, _)| ty)
                .unwrap_or_else(|_| tcx.ty_error())
        }
        hir::TyKind::Path(hir::QPath::LangItem(lang_item, span)) => {
            let def_id = tcx.require_lang_item(lang_item, Some(span));
            let (substs, _, _) = create_substs_for_ast_path(
                astconv,
                span,
                def_id,
                &[],
            ...
            astconv.normalize_ty(span, tcx.at(span).type_of(def_id).subst(tcx, substs))
        }
        hir::TyKind::Array(ref ty, ref length) => {
            let length_def_id = tcx.hir().local_def_id(length.hir_id);
            let length = ty::Const::from_anon_const(tcx, length_def_id);
            let array_ty = tcx.mk_ty(ty::Array(ast_ty_to_ty(astconv, &ty), length));
            astconv.normalize_ty(ast_ty.span, array_ty)
        }
        ...
        hir::TyKind::Infer => {
            ...
            astconv.ty_infer(None, ast_ty.span)
        }
    ...
    astconv.record_ty(ast_ty.hir_id, result_ty, ast_ty.span);
    result_ty
}

pub fn impl_trait_ty_to_ty<'tcx>(
    astconv: &impl AstConv<'tcx>,
    def_id: DefId,
            ...
            match param.kind {
                GenericParamDefKind::Lifetime => {
                    if let hir::GenericArg::Lifetime(lifetime) = &lifetimes[i] {
                        ast_region_to_region(astconv, lifetime, None).into()
                    } else {
                        bug!()
                    }
                }
...
pub fn ty_of_arg<'tcx>(
    ...
    match ty.kind {
        hir::TyKind::Infer if expected_ty.is_some() => {
            astconv.record_ty(ty.hir_id, expected_ty.unwrap(), ty.span);
            expected_ty.unwrap()
        }
        _ => ast_ty_to_ty(astconv, ty),
    }
}
...
pub fn ty_of_fn<'tcx>(
    ...
    let input_tys = decl.inputs.iter().map(|a| ty_of_arg(astconv, a, None));
    let output_ty = match decl.output {
        hir::FnRetTy::Return(ref output) => {
            visitor.visit_ty(output);
            ast_ty_to_ty(astconv, output)
        }
    ...
    validate_late_bound_regions(late_bound_in_args, late_bound_in_ret, |br_name| {
        struct_span_err!(
            tcx.sess,
            decl.output.span(),
            E0581,
...
fn validate_late_bound_regions<'tcx>(
