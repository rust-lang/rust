...
pub(crate) fn complain_about_missing_type_params<'tcx>(
    ...
    if !suggested {
        err.span_label(
            span,
            format!("missing reference{} to {}", pluralize!(missing_type_params.len()), display,),
        );
    }
    err.note(
...
pub(crate) fn complain_about_internal_fn_trait<'a, 'tcx>(
    astconv: &impl AstConv<'tcx>,
    span: Span,
    trait_def_id: DefId,
...
pub(crate) fn complain_about_assoc_type_not_found<'tcx, I>(
    astconv: &impl AstConv<'tcx>,
    ...
    I: Iterator<Item = ty::PolyTraitRef<'tcx>>,
{
    ...
    let all_candidate_names: Vec<_> = all_candidates()
        .map(|r| tcx.associated_items(r.def_id()).in_definition_order())
        .flatten()
        .filter_map(
...
pub(crate) fn complain_about_missing_associated_types<'tcx>(
    astconv: &impl AstConv<'tcx>,
    ...
    for (span, assoc_items) in &associated_types {
        ...
        } else if let (Ok(snippet), false) = (tcx.sess.source_map().span_to_snippet(*span), dupes) {
            let types: Vec<_> =
