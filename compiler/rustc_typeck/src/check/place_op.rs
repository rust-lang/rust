...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(super) fn lookup_derefing(
        ...
        if let ty::Ref(region, _, hir::Mutability::Not) = method.sig.inputs()[0].data() {
    ...
    fn try_index_step(
        ...
        for &unsize in &[false, true] {
            ...
            if unsize {
                ...
                if let ty::Array(element_ty, _) = adjusted_ty.data() {
    ...
    fn convert_place_op_to_mutable(
        ...
        let region = if let ty::Ref(r, _, hir::Mutability::Mut) = method.sig.inputs()[0].data() {
            r
        } else {
