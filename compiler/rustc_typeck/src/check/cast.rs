...
impl<'a, 'tcx> CastCheck<'tcx> {
    ...
    fn report_cast_error(&self, fcx: &FnCtxt<'a, 'tcx>, e: CastError) {
        match e {
            ...
            CastError::NonScalar => {
                ...
                let mut sugg = None;
                if let ty::Ref(reg, _, mutbl) = *self.cast_ty.data() {
                    if fcx
                        .try_coerce(
                            self.expr,
            ...
            (None, Some(t_cast)) => {
                match *self.expr_ty.data() {
                    ...
                    ty::Ref(_, inner_ty, mutbl) => {
                        return match t_cast {
                            Int(_) | Float => match *inner_ty.data() {
                                ty::Int(_)
                                | ty::Uint(_)
                                | ty::Float(_)
    ...
    fn check_ref_cast(
        ...
        if m_expr.mutbl == hir::Mutability::Not && m_cast.mutbl == hir::Mutability::Not {
            if let ty::Array(ety, _) = m_expr.ty.data() {
    ...
    fn cenum_impl_drop_lint(&self, fcx: &FnCtxt<'a, 'tcx>) {
        if let ty::Adt(d, _) = self.expr_ty.data() {
