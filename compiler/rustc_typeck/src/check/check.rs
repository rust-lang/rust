...
pub(super) fn check_opaque_for_inheriting_lifetimes(tcx: TyCtxt<'tcx>, def_id: LocalDefId) {
...
fn check_opaque_for_cycles<'tcx>(
    ...
    if tcx.try_expand_impl_trait_type(def_id.to_def_id(), substs).is_err() {
        match origin {
            hir::OpaqueTyOrigin::AsyncFn => async_opaque_type_cycle_error(tcx, def_id),
            _ => opaque_type_cycle_error(tcx, def_id),
        }
        Err(ErrorReported)
    } else {
        Ok(())
...
fn check_representable(tcx: TyCtxt<'_>, item_def_id: LocalDefId) -> bool {
    ...
    // caught by case 1.
    match representability::ty_is_representable(tcx, rty, tcx.def_span(item_def_id)) {
...
pub(super) fn check_transparent<'tcx>(tcx: TyCtxt<'tcx>, def_id: LocalDefId, adt: &'tcx ty::AdtDef) {
...
fn async_opaque_type_cycle_error(tcx: TyCtxt<'tcx>, def_id: LocalDefId) {
    let span = tcx.def_span(def_id);
...
fn opaque_type_cycle_error(tcx: TyCtxt<'tcx>, def_id: LocalDefId) {
    let span = tcx.def_span(def_id);
