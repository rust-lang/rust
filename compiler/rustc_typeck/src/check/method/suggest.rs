...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub fn report_method_error<'b>(
        ...
        match error {
            MethodError::NoMatch(NoMatchData {
                ...
                let mut err = if !actual.references_error() {
                    ...
                    if let (true, false, SelfSource::MethodCall(expr), true) = (
                        ...
                        if let ty::RawPtr(_) = &actual.data() {
                            err.note(
                ...
                if let SelfSource::MethodCall(expr) = source {
                    let field_receiver =
                        self.autoderef(span, rcvr_ty).find_map(|(ty, _)| match ty.data() {
                ...
                if static_sources.len() == 1 {
                        ...
                        static_sources.get(0)
                    {
                        ...
                        match (&ty.peel_refs().data(), &actual.peel_refs().data()) {
                    ...
                    let mut collect_type_param_suggestions =
                        |self_ty: Ty<'tcx>, parent_pred: &ty::Predicate<'tcx>, obligation: &str| {
                            ...
                            if let (ty::Param(_), ty::PredicateAtom::Trait(p, _)) =
                                (self_ty.data(), parent_pred.skip_binders())
                            {
                                if let ty::Adt(def, _) = p.trait_ref.self_ty().data() {
            ...
            MethodError::IllegalSizedBound(candidates, needs_mut, bound_span) => {
                ...
                if let ty::Ref(region, t_type, mutability) = rcvr_ty.data() {
                    if needs_mut {
    ...
    fn suggest_traits_to_import<'b>(
        ...
        if !candidates.is_empty() {
            ...
            let param_type = match rcvr_ty.data() {
                ty::Param(param) => Some(param),
                ty::Ref(_, ty, _) => match ty.data() {
                    ty::Param(param) => Some(param),
                    _ => None,
                },
                _ => None,
