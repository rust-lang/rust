...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    fn check_expr_eq_type(&self, expr: &'tcx hir::Expr<'tcx>, expected: Ty<'tcx>) {
        ...
        self.demand_eqtype(self.tcx.hir().span(expr.hir_id), expected, ty);
    }
    ...
    fn check_expr_meets_expectation_or_error(
        ...
        if let Some(mut err) =
            self.demand_suptype_diag(self.tcx.hir().span(expr.hir_id), expected_ty, ty)
        {
    ...
    pub(super) fn check_expr_with_expectation(
        ...
        let is_try_block_generated_unit_expr = match expr.kind {
            ExprKind::Call(_, args) if expr_span.is_desugaring(DesugaringKind::TryBlock) => {
                args.len() == 1
                    && self.tcx.hir().span(args[0].hir_id).is_desugaring(DesugaringKind::TryBlock)
            }
        ...
        if !is_try_block_generated_unit_expr {
            self.warn_if_unreachable(expr.hir_id, expr_span, "expression");
        }
        ...
        match expr.kind {
            ...
            ExprKind::Call(..) if expr_span.is_desugaring(DesugaringKind::TryBlock) => {}
            ExprKind::Call(callee, _) => {
                self.warn_if_unreachable(expr.hir_id, self.tcx.hir().span(callee.hir_id), "call")
            }
            ExprKind::MethodCall(_, ref span, _, _) => {
                self.warn_if_unreachable(expr.hir_id, *span, "call")
            }
            _ => self.warn_if_unreachable(expr.hir_id, expr_span, "expression"),
        }
    ...
    fn check_expr_kind(
        ...
        match expr.kind {
            ...
            ExprKind::If(cond, then_expr, opt_else_expr) => self.check_then_else(
                cond,
                then_expr,
                opt_else_expr,
    ...
    fn check_expr_unary(
        ...
        if !oprnd_t.references_error() {
            let expr_span = self.tcx.hir().span(expr.hir_id);
            oprnd_t = self.structurally_resolved_type(expr_span, oprnd_t);
            match unop {
                hir::UnOp::Deref => {
                    if let Some(ty) = self.lookup_derefing(expr, oprnd, oprnd_t) {
                        ...
                        let sp = tcx.sess.source_map().start_point(expr_span);
                        if let Some(sp) =
    ...
    fn check_expr_addr_of(
        ...
        match kind {
            ...
            hir::BorrowKind::Ref => {
                ...
                // as it needs to live.
                let expr_span = self.tcx.hir().span(expr.hir_id);
                let region = self.next_region_var(infer::AddrOfRegion(expr_span));
    ...
    fn check_named_place_expr(&self, oprnd: &'tcx hir::Expr<'tcx>) {
        ...
        if !is_named {
            let oprnd_span = self.tcx.hir().span(oprnd.hir_id);
            self.tcx.sess.emit_err(AddressOfTemporaryTaken { span: oprnd_span })
        }
    }
    ...
    fn check_lang_item_path(
        ...
        self.resolve_lang_item_path(lang_item, self.tcx.hir().span(expr.hir_id), expr.hir_id).1
    }

    fn check_expr_path(&self, qpath: &hir::QPath<'_>, expr: &'tcx hir::Expr<'tcx>) -> Ty<'tcx> {
        let tcx = self.tcx;
        let expr_span = self.tcx.hir().span(expr.hir_id);
        let (res, opt_ty, segs) = self.resolve_ty_and_res_ufcs(qpath, expr.hir_id, expr_span);
        let ty = match res {
            Res::Err => {
            ...
            Res::Def(DefKind::Ctor(_, CtorKind::Fictive), _) => {
                report_unexpected_variant_res(tcx, res, expr_span);
                tcx.ty_error()
            }
            _ => self.instantiate_value_path(segs, opt_ty, res, expr_span, expr.hir_id).0,
        };
        ...
        if let ty::FnDef(..) = ty.kind() {
            ...
            let output = self
                ...
                .0;
            self.require_type_is_sized_deferred(output, expr_span, traits::SizedReturnType);
        }
    ...
    fn check_expr_break(
        ...
        if let Ok(target_id) = destination.target_id {
            ...
            if let Some(e) = expr_opt {
                ...
                cause = self.misc(self.tcx.hir().span(e.hir_id));
            } else {
                // Otherwise, this is a break *without* a value. That's
                // always legal, and is equivalent to `break ()`.
                e_ty = tcx.mk_unit();
                cause = self.misc(expr_span);
            }
    ...
    fn check_expr_return(
        ...
        if self.ret_coercion.is_none() {
            self.tcx.sess.emit_err(ReturnStmtOutsideOfFnBody { span: expr_span });
        } else if let Some(e) = expr_opt {
            if self.ret_coercion_span.get().is_none() {
                self.ret_coercion_span.set(Some(self.tcx.hir().span(e.hir_id)));
            }
            self.check_return_expr(e);
        } else {
            let mut coercion = self.ret_coercion.as_ref().unwrap().borrow_mut();
            if self.ret_coercion_span.get().is_none() {
                self.ret_coercion_span.set(Some(expr_span));
            }
            let cause = self.cause(expr_span, ObligationCauseCode::ReturnNoExpression);
    ...
    pub(super) fn check_return_expr(&self, return_expr: &'tcx hir::Expr<'tcx>) {
        let return_expr_span = self.tcx.hir().span(return_expr.hir_id);
        let ret_coercion = self.ret_coercion.as_ref().unwrap_or_else(|| {
            span_bug!(return_expr_span, "check_return_expr called outside fn body")
        });
        ...
        ret_coercion.borrow_mut().coerce(
            self,
            &self.cause(return_expr_span, ObligationCauseCode::ReturnValue(return_expr.hir_id)),
            return_expr,
            return_expr_ty,
        );
    }
    ...
    pub(crate) fn check_lhs_assignable(
        ...
        err.span_label(self.tcx.hir().span(lhs.hir_id), "cannot assign to this expression");
        err.emit();
    }
    ...
    fn check_then_else(
        ...
        if let Some(else_expr) = opt_else_expr {
            ...
            let opt_suggest_box_span = self.opt_suggest_box_span(
    ...
    fn check_expr_assign(
        ...
        let expr_span = self.tcx.hir().span(expr.hir_id);
        let expected_ty = expected.coercion_target_type(self, expr_span);
        if expected_ty == self.tcx.types.bool {
            // The expected type is `bool` but this will result in `()` so we can reasonably
            // say that the user intended to write `lhs == rhs` instead of `lhs = rhs`.
            // The likely cause of this is `if foo = bar { .. }`.
            let actual_ty = self.tcx.mk_unit();
            let mut err = self.demand_suptype_diag(expr_span, expected_ty, actual_ty).unwrap();
            ...
            if !lhs.is_syntactic_place_expr() {
                ...
                let mut span_err = || {
                    ...
                    err.span_suggestion_verbose(
                        self.tcx.hir().span(expr.hir_id).shrink_to_lo(),
            ...
            if self
                ...
                .if_let_suggestions
                .borrow()
                .get(&self.tcx.hir().span(expr.hir_id))
                .is_some()
            {
    ...
    fn check_method_call(
        ...
        let rcvr_t = self.structurally_resolved_type(self.tcx.hir().span(args[0].hir_id), rcvr_t);
    ...
    fn check_expr_cast(
        ...
        if t_expr.references_error() || t_cast.references_error() {
            ...
            let expr_span = self.tcx.hir().span(expr.hir_id);
            match cast::CastCheck::new(self, e, t_expr, t_cast, t_span, expr_span) {
                Ok(cast_check) => {
    ...
    fn check_expr_array(
        ...
        let element_ty = if !args.is_empty() {
            ...
            for e in args {
                ...
                let cause = self.misc(self.tcx.hir().span(e.hir_id));
                coerce.coerce(self, &cause, e, e_ty);
            }
        ...
        let (element_ty, t) = match uty {
            ...
            None => {
                let ty = self.next_ty_var(TypeVariableOrigin {
                    ...
                    span: self.tcx.hir().span(element.hir_id),
                });
    ...
    fn check_expr_tuple(
        ...
        if tuple.references_error() {
            ...
            let expr_span = self.tcx.hir().span(expr.hir_id);
            self.require_type_is_sized(tuple, expr_span, traits::TupleInitializerSized);
            tuple
        }
    }
    ...
    fn check_expr_struct(
        ...
        if !adt.did.is_local() && variant.is_field_list_non_exhaustive() {
            self.tcx
                .sess
                .emit_err(StructExprNonExhaustive { span: expr_span, what: adt.variant_descr() });
        }
        ...
        if let Some(base_expr) = base_expr {
            ...
            if !error_happened {
                ...
                match adt_ty.kind() {
                    ...
                    _ => {
                        let base_expr_span = self.tcx.hir().span(base_expr.hir_id);
                        self.tcx
                            .sess
                            .emit_err(FunctionalRecordUpdateOnNonStruct { span: base_expr_span });
                    }
                }
            }
        }
        self.require_type_is_sized(adt_ty, expr_span, traits::StructInitializerSized);
        adt_ty
    }
    ...
    fn check_field(
        ...
        let expr_t = self.structurally_resolved_type(self.tcx.hir().span(base.hir_id), expr_t);
        let expr_span = self.tcx.hir().span(expr.hir_id);
        let mut private_candidate = None;
        let mut autoderef = self.autoderef(expr_span, expr_t);
            ...
            match base_t.kind() {
                ty::Adt(base_def, substs) if !base_def.is_enum() => {
                        ...
                        fields.iter().position(|f| f.ident.normalize_to_macros_2_0() == ident)
                    {
                        ...
                        let field_ty = self.field_ty(expr_span, field, substs);
                        ...
                        if field.vis.is_accessible_from(def_scope, self.tcx) {
                            ...
                            self.tcx.check_stability(field.did, Some(expr.hir_id), expr_span);
                            return field_ty;
                        }
        ...
        if let ty::Adt(def, _) = output_ty.kind() {
            ...
            if !def.is_enum() {
                if def.non_enum_variant().fields.iter().any(|field| field.ident == field_ident) {
                    ...
                    err.span_suggestion_verbose(
                        self.tcx.hir().span(base.hir_id).shrink_to_hi(),
    ...
    fn maybe_suggest_array_indexing(
        ...
        {
            if let Ok(base) =
                self.tcx.sess.source_map().span_to_snippet(self.tcx.hir().span(base.hir_id))
            {
    ...
    fn check_expr_index(
        ...
        if base_t.references_error() {
            ...
            let base_t = self.structurally_resolved_type(base_span, base_t);
                    ...
                    if let ty::Tuple(..) = base_t.kind() {
                        ...
                        if let ExprKind::Lit(ref lit) = idx.kind {
                            if let ast::LitKind::Int(i, ast::LitIntType::Unsuffixed) = lit.node {
                                let snip = self.tcx.sess.source_map().span_to_snippet(base_span);
                                if let Ok(snip) = snip {
                                    err.span_suggestion(
                                        expr_span,
    ...
    fn check_expr_yield(
        ...
        match self.resume_yield_tys {
            ...
            _ => {
                let expr_span = self.tcx.hir().span(expr.hir_id);
                self.tcx.sess.emit_err(YieldExprOutsideOfGenerator { span: expr_span });
    ...
    fn check_expr_asm_operand(&self, expr: &'tcx hir::Expr<'tcx>, is_input: bool) {
        ...
        let expr_span = self.tcx.hir().span(expr.hir_id);
        self.require_type_is_sized(ty, expr_span, traits::InlineAsmSized);

        if !is_input && !expr.is_syntactic_place_expr() {
            let mut err = self.tcx.sess.struct_span_err(expr_span, "invalid asm output");
            err.span_label(expr_span, "cannot assign to this expression");
            err.emit();
        }
        ...
        // function.
        if is_input {
            let ty = self.structurally_resolved_type(expr_span, &ty);
            match *ty.kind() {
                ty::FnDef(..) => {
