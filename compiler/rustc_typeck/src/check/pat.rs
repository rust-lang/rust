...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn calc_adjust_mode(&self, pat: &'tcx Pat<'tcx>, opt_path_res: Option<Res>) -> AdjustMode {
        match &pat.kind {
            ...
            PatKind::Lit(lt) => match self.check_expr(lt).data() {
    ...
    fn peel_off_references(
        ...
        while let ty::Ref(_, inner_ty, inner_mutability) = *expected.data() {
    ...
    fn check_pat_lit(
        ...
        if let hir::ExprKind::Lit(Spanned { node: ast::LitKind::ByteStr(_), .. }) = lt.kind {
            ...
            if let ty::Ref(_, inner_ty, _) = expected.data() {
        ...
        if let Some(span) = self.tcx.hir().res_span(pat_res) {
            ...
            if let [hir::PathSegment { ident, .. }] = &*segments {
                ...
                match parent_pat {
                    ...
                    _ => {
                        let (type_def_id, item_def_id) = match pat_ty.data() {
                            Adt(def, _) => match res {
    ...
    fn e0023(
        ...
        let missing_parenthesis = match (&expected.data(), fields, had_err) {
    ...
    fn check_pat_tuple(
        ...
        if ddpos.is_some() {
            ...
            if let ty::Tuple(ref tys) = self.structurally_resolved_type(span, expected).data() {
    ...
    fn check_struct_pat_fields(
        ...
        let (substs, adt) = match adt_ty.data() {
    ...
    fn check_pat_slice(
        ...
        let expected = self.structurally_resolved_type(span, expected);
        let (element_ty, opt_slice_ty, inferred) = match *expected.data() {
    ...
    fn error_expected_array_or_slice(&self, span: Span, expected_ty: Ty<'tcx>) {
        ...
        if let ty::Ref(_, ty, _) = expected_ty.data() {
            if let ty::Array(..) | ty::Slice(..) = ty.data() {
