...
impl<'gcc, 'tcx> ConstMethods<'tcx> for CodegenCx<'gcc, 'tcx> {
    ...
    fn const_struct(&self, values: &[RValue<'gcc>], packed: bool) -> RValue<'gcc> {
        ...
        self.context
            .new_struct_constructor(None, struct_type.as_type(), None, values)
    }
        ...
        match cv {
            Scalar::Int(int) => {
                ...
                if ty == self.float_type {
                    ...
                    return self
                        .context
                        .new_rvalue_from_double(ty, f64::from_bits(data as u64));
                }
            ...
            Scalar::Ptr(ptr, _size) => {
                ...
                let offset = self
                    .context
                    .new_rvalue_from_long(self.usize_type, offset.bytes() as i64);
