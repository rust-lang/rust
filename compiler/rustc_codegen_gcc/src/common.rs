...
impl<'gcc, 'tcx> ConstCodegenMethods<'tcx> for CodegenCx<'gcc, 'tcx> {
    ...
    fn alloc_to_backend(
        ...
        if let Some(name) = name_hint {
            // For consts in inline `asm!()`, we might need to codegen multiple copies of it when
            // it has been inlined through MIR inlining. So we need to cache value to avoid
            // generating duplicated symbols.
            if let Some(global) = self.instances.borrow().get(&name) {
            ...
            let global = self.declare_global_with_linkage(sym.name, typ, GlobalKind::Internal);

            self.instances.borrow_mut().insert(name, global);
