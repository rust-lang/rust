...
impl<'a, 'gcc, 'tcx> AsmBuilderMethods<'tcx> for Builder<'a, 'gcc, 'tcx> {
    fn codegen_inline_asm(
                ...
                InlineAsmOperandRef::Out { reg, late, place } => {
                    ...
                    let (constraint, ty) = match (reg_to_gcc(reg), place) {
                        (Constraint(constraint), Some(place)) => {
                            (constraint, place.layout.gcc_type(self.cx, false))
                        }
            ...
            match *op {
                ...
                InlineAsmOperandRef::SymFn { instance } => {
                    inputs.push(AsmInOperand {
                        ...
                        val: self
                            .cx
                            .rvalue_as_function(get_fn(self.cx, instance))
                            .get_address(None),
                    });
                }
        ...
        let mut template_str = String::with_capacity(estimate_template_length(
            template,
            constants_len,
            att_dialect,
        ));
            ...
            match *piece {
                InlineAsmTemplatePiece::String(ref string) => {
                    for line in string.lines() {
                        ...
                        let line = if let Some(index) = line.rfind("//") {
                            &line[..index]
                        } else {
                            line
                        };
