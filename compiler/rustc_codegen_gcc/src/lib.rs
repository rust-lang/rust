...
impl CodegenBackend for GccCodegenBackend {
    fn init(&self, sess: &Session) {
        ...
        *self.supports_128bit_integers.lock().expect("lock") =
            check_context.get_last_error() == Ok(None);
    }
...
impl ExtraBackendMethods for GccCodegenBackend {
    fn codegen_allocator<'tcx>(
        ...
        unsafe {
            allocator::codegen(tcx, &mut mods, module_name, kind, has_alloc_error_handler);
        }
        mods
    }
...
impl WriteBackendMethods for GccCodegenBackend {
    ...
    fn run_fat_lto(
        ...
        let module = match modules.remove(0) {
    ...
    unsafe fn optimize(
        ...
        module
            .module_llvm
            .context
            .set_optimization_level(to_gcc_opt_level(config.opt_level));
        Ok(())
    }
