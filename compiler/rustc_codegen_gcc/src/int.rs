...
use gccjit::{BinaryOp, ComparisonOp, FunctionType, RValue, ToRValue, Type, UnaryOp};
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    pub fn gcc_lshr(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        ...
        if a_native && b_native {
            ...
            self.llbb()
                .end_with_conditional(None, condition, then_block, else_block);

            // TODO(antoyo): take endianness into account.
            let shift_value = self.gcc_sub(b, sixty_four);
            let high = self.high(a);
            let sign = if a_type.is_signed(self) {
                high >> sixty_three
            } else {
                zero
            };
            let values = [high >> shift_value, sign];
            ...
            let values = [(high << shift_value) | shifted_low, high >> b];
            let array_value = self.context.new_array_constructor(None, a_type, &values);
    ...
    pub fn gcc_icmp(
        ...
        if self.is_non_native_int_type(a_type) || self.is_non_native_int_type(b_type) {
            ...
            if left_type != right_type {
                ...
                if left_type.dyncast_function_ptr_type().is_some()
                    && right_type.dyncast_function_ptr_type().is_some()
                {
                    lhs = self
                        .context
                        .new_cast(None, lhs, self.usize_type.make_pointer());
                    rhs = self
                        .context
                        .new_cast(None, rhs, self.usize_type.make_pointer());
                }
    ...
    pub fn gcc_xor(&self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        ...
        if self.is_native_int_type_or_bool(a_type) && self.is_native_int_type_or_bool(b_type) {
            ...
            let values = [self.low(a) ^ self.low(b), self.high(a) ^ self.high(b)];
    ...
    pub fn gcc_shl(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        ...
        if a_native && b_native {
            ...
            // TODO(antoyo): take endianness into account.
            let values = [zero, self.low(a) << (b - sixty_four)];
            let array_value = self.context.new_array_constructor(None, a_type, &values);
            then_block.add_assignment(None, result, array_value);
            then_block.end_with_jump(None, after_block);
            ...
            let high_low = self
                .context
                .new_cast(None, casted_low >> shift_value, native_int_type);
            let values = [self.low(a) << b, (self.high(a) << b) | high_low];
        ...
        if !self.is_native_int_type(arg_type) {
            ...
            let msb = self
                ...
                .new_array_access(
                    None,
                    arg,
                    self.context.new_rvalue_from_int(self.int_type, 1),
                )
                .to_rvalue();
            ...
            return self
                .context
                .new_array_constructor(None, arg_type, &[swapped_msb, swapped_lsb]);
        }
...
impl<'gcc, 'tcx> CodegenCx<'gcc, 'tcx> {
    pub fn gcc_int(&self, typ: Type<'gcc>, int: i64) -> RValue<'gcc> {
        if self.is_native_int_type_or_bool(typ) {
            self.context
                .new_rvalue_from_long(typ, i64::try_from(int).expect("i64::try_from"))
        } else {
    ...
    pub fn gcc_uint_big(&self, typ: Type<'gcc>, num: u128) -> RValue<'gcc> {
        ...
        if num >> 64 != 0 {
            ...
            let num = self
                .context
                .new_rvalue_from_long(self.u64_type, num as u64 as i64);
    ...
    pub fn gcc_int_cast(&self, value: RValue<'gcc>, dest_typ: Type<'gcc>) -> RValue<'gcc> {
        ...
        if self.is_native_int_type_or_bool(dest_typ) && self.is_native_int_type_or_bool(value_type)
        {
            ...
            let is_negative =
                self.context
                    .new_comparison(None, ComparisonOp::LessThan, value, zero);
            let is_negative = self.gcc_int_cast(is_negative, dest_element_type);
            let values = [
                self.context.new_cast(None, value, dest_element_type),
                self.context
                    .new_unary_op(None, UnaryOp::Minus, dest_element_type, is_negative),
            ];
