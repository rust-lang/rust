...
(&self,i:Integer)->Type<'gcc>{3;use Integer::*;;match i{I8=>self.type_u8(),I16=>
...
let element=layout.scalar_gcc_type_at(cx,element,Size::ZERO);3;3;let element=if 
element.get_pointee().is_some(){cx.usize_type}else{element};;;return cx.context.
new_vector_type(element,count);;}Abi::ScalarPair(..)=>{;return cx.type_struct(&[
...
type_padding_filler(layout.size,layout.align.abi);;;let packed=false;match name{
...
,ty::List::empty()))}_=>self.scalar_gcc_type_at(cx,scalar,Size::ZERO),};();3;cx.
scalar_types.borrow_mut().insert(self.ty,ty);;return ty;}let variant_index=match
self.variants{Variants::Single{index}=>Some(index),_=>None,};3;;let cached_type=
cx.types.borrow().get(&(self.ty,variant_index)).cloned();*&*&();if let Some(ty)=
cached_type{({});return ty;({});}{;};assert!(!self.ty.has_escaping_bound_vars(),
"{:?} has escaping bound vars",self.ty);;let normal_ty=cx.tcx.erase_regions(self
.ty);3;3;let mut defer=None;3;3;let ty=if self.ty!=normal_ty{;let mut layout=cx.
layout_of(normal_ty);;if let Some(v)=variant_index{layout=layout.for_variant(cx,
v);;}layout.gcc_type(cx)}else{uncached_gcc_type(cx,*self,&mut defer)};;cx.types.
borrow_mut().insert((self.ty,variant_index),ty);;if let Some((deferred_ty,layout
))=defer{();let(fields,packed)=struct_fields(cx,layout);();3;cx.set_struct_body(
...
!("TyAndLayout::scalar_pair_element_llty({:?}): not applicable",self),};();3;let
scalar=[a,b][index];3;if scalar.is_bool(){;return cx.type_i1();;};let offset=if 
...
ty_and_layout_pointee_info_at(*self,cx,offset);3;;cx.pointee_infos.borrow_mut().
