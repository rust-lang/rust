...
impl<'gcc, 'tcx> CodegenCx<'gcc, 'tcx> {
    pub fn get_or_insert_global(
        ...
        if self.globals.borrow().contains_key(name) {
            ...
            let global = self
                .context
                .new_global(None, GlobalKind::Imported, typ, name);
            if is_tls {
    ...
    pub fn declare_unnamed_global(&self, ty: Type<'gcc>) -> LValue<'gcc> {
        ...
        self.context
            .new_global(None, GlobalKind::Internal, ty, &name)
    }
    ...
    pub fn declare_global_with_linkage(
        ...
        self.globals
            .borrow_mut()
            .insert(name.to_string(), global_address);
        global
    }
    ...
    pub fn declare_fn(&self, name: &str, fn_abi: &FnAbi<'tcx, Ty<'tcx>>) -> RValue<'gcc> {
        ...
        self.on_stack_function_params
            .borrow_mut()
            .insert(func, on_stack_param_indices);
        // FIXME(antoyo): this is a wrong cast. That requires changing the compiler API.
        unsafe { std::mem::transmute(func) }
    }
...
fn declare_raw_fn<'gcc>(
    ...
    let func = if cx.functions.borrow().contains_key(name) {
        ...
        let params: Vec<_> = param_types
            ...
            .map(|(index, param)| {
                cx.context
                    .new_parameter(None, *param, &format!("param{}", index))
