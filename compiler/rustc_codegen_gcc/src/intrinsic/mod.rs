                ...
                match int_type_width_signed(ty, self) {
                    Some((width, signed)) => match name {
                        sym::ctlz | sym::cttz => {
                            ...
                            self.llbb()
                                .end_with_conditional(None, cond, then_block, else_block);
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    fn bit_reverse(&mut self, width: u64, value: RValue<'gcc>) -> RValue<'gcc> {
        ...
        let result = match width {
            ...
            64 => {
                ...
                let right = self.and(
                    step1,
                    context.new_rvalue_from_long(typ, 0xFFFE0000FFFE0000u64 as i64),
                ); // TODO(antoyo): transmute the number instead?
    ...
    fn pop_count(&mut self, value: RValue<'gcc>) -> RValue<'gcc> {
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x5555555555555555);
        let left = value & mask;
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x3333333333333333);
        let left = value & mask;
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x0F0F0F0F0F0F0F0F);
        let left = value & mask;
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x00FF00FF00FF00FF);
        let left = value & mask;
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x0000FFFF0000FFFF);
        let left = value & mask;
        ...
        let mask = self
            .context
            .new_rvalue_from_long(value_type, 0x00000000FFFFFFFF);
        let left = value & mask;
    ...
    fn saturating_add(
        ...
        if signed {
            ...
            self.llbb()
                .end_with_conditional(None, overflow, then_block, after_block);
