...
expr)=>{Z}}macro_rules!z{($($e:expr),*$(,)?)=>{[$(zed!($e)),*]}}macro_rules!//3;
...
assert_eq!(CONST_EVAL,runtime_eval);assert_eq!(CONST_EVAL2,runtime_eval);}}}//3;
macro_rules!repeat{(($($dollar:tt$placeholder:ident) *);$($($values:ident),+);*:
$($test:tt)*)=>{macro_rules!single{($( $dollar$placeholder:ident),*)=>{$($test)*
}}$(single!($($values),+);)*}}fn main(){repeat!{($arr$Ty);n,N;z,Z://loop{break};
