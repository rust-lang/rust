...
impl<'a> State<'a> {
    ...
    fn print_either_attributes(&mut self, attrs: &[hir::Attribute], style: ast::AttrStyle) {
        ...
        for attr in attrs {
            match &attr {
                hir::Attribute::Unparsed(attr) => self.print_attritem_inline(&**attr, style),
    ...
    fn print_attritem_inline(&mut self, unparsed: &hir::AttrItem, style: AttrStyle) {
...
pub fn parsed_attribute_to_string(ann: &dyn PpAnn, attr: &AttributeKind) -> String {
    to_string(ann, |s| {
        ...
        this.print_parsed_attribute_inline(attr, style)
    })
}
pub fn attritem_to_string(ann: &dyn PpAnn, attr: &hir::AttrItem) -> String {
    to_string(ann, |s| {
        let this = &mut *s;
        let style = AttrStyle::Outer;
        this.print_attritem_inline(attr, style)
    })
}
...
pub fn attribute_to_string(ann: &dyn PpAnn, attr: &hir::Attribute) -> String {
    match attr {
        ...
        hir::Attribute::Unparsed(attr) => attritem_to_string(ann, attr),
    }
}
