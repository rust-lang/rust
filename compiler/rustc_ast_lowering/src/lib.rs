...
impl<'hir> LoweringContext<'hir> {
    fn new(tcx: TyCtxt<'hir>, resolver: &'hir ResolverAstLowering<'hir>, owner: NodeId) -> Self {
...
fn index_ast<'tcx>(tcx: TyCtxt<'tcx>, (): ()) -> &'tcx IndexVec<LocalDefId, AstOwner<'tcx>> {
    let (resolver, krate) = tcx.resolver_for_lowering();

    let mut indexer = Indexer { owners: &resolver.owners, index: IndexVec::new() };
