                ...
                FnRetTy::Ty(ref ty) => {
                    let context = match in_band_ty_params {
                        Some((node_id, _)) if kind.impl_trait_return_allowed() => {
                            let fn_def_id = self.local_def_id(node_id);
                            ImplTraitContext::ReturnPositionOpaqueTy {
            ...
            LifetimeRes::Anonymous { binder, elided } => {
                ...
                if let Some(mut captured_lifetimes) = self.captured_lifetimes.take() {
                    if !captured_lifetimes.binders_to_ignore.contains(&binder) {
                        ...
                        captured_lifetimes.captures.insert(p_def_id, (span, p_id, p_name, res));
                        l_name = hir::LifetimeName::Param(p_name)
                    }
