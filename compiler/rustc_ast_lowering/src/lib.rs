...
struct LoweringContext<'a, 'hir> {
    ...
    lowering_arena: &'a LoweringArena,
...
impl ResolverAstLoweringExt for ResolverAstLowering {
    ...
    fn take_extra_lifetime_params(&mut self, id: NodeId) -> Vec<(Ident, NodeId, LifetimeRes)> {
        self.extra_lifetime_params_map.get(&id).map_or_else(|| Vec::new(), |map| map.clone())
    }
...
impl<'itctx> ImplTraitContext<'_, 'itctx> {
    fn reborrow<'this>(&'this mut self) -> ImplTraitContext<'this, 'itctx> {
...
pub fn lower_to_hir<'hir>(tcx: TyCtxt<'hir>, (): ()) -> hir::Crate<'hir> {
    ...
    let lowering_arena = LoweringArena {
...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    ...
    fn lower_lifetime_binder<R>(
        ...
        if !self.resolver.generics_def_id_map.is_empty() {
            for old_node_id in generic_params.iter().map(|param| param.id) {
                ...
                // Add a definition for the generic param def.
                let new_def_id =
                    self.create_def(self.current_hir_id_owner, node_id, DefPathData::ImplTrait);

                new_remapping.insert(old_def_id, new_def_id);
                debug!("old_def_id={:?} new_def_id={:?}", old_def_id, new_def_id);
            }
        }
        ...
        let result = self.with_remapping(new_remapping, &[], |lctx| {
            ...
            let generic_params = lctx.arena.alloc_from_iter(generic_params);
    ...
    #[instrument(level = "debug", skip(self))]
    fn lower_assoc_ty_constraint<'itctx>(
        &mut self,
        constraint: &'itctx AssocConstraint,
            ...
            AssocConstraintKind::Bound { ref bounds } => {
                ...
                if desugar_to_impl_trait {
                    ...
                    self.with_dyn_type_scope(false, |this| {
                        ...
                        let ty = this.lowering_arena.tys.alloc(Ty {
                            id: node_id,
    ...
    #[instrument(level = "debug", skip(self))]
    fn lower_generic_arg<'itctx>(
        &mut self,
        arg: &'itctx ast::GenericArg,
        ...
        'a: 'itctx,
    {
            ...
            && let Res::Def(DefKind::Trait | DefKind::TraitAlias, _) = partial_res.base_res()
        {
            let (bounds, lifetime_bound) = self.with_dyn_type_scope(true, |this| {
                let poly_trait_ref = this.lowering_arena.ptr.alloc(PolyTraitRef {
            ...
            TyKind::BareFn(ref f) => {
                self.lower_lifetime_binder(t.id, &f.generic_params, |lctx, generic_params| {
                    hir::TyKind::BareFn(lctx.arena.alloc(hir::BareFnTy {
                        ...
                        decl: lctx.lower_fn_decl(
                            &f.decl,
                            None,
                            FnDeclKind::Pointer,
                            &Generics {
                                params: f.generic_params.clone(),
    ...
    fn lower_opaque_impl_trait_v2<'itctx>(
        ...
        origin: hir::OpaqueTyOrigin,
        opaque_ty_node_id: NodeId,
        ast_generics: &'itctx Generics,
        impl_trait_inputs: &Vec<&Ty>,
        bounds: &'itctx GenericBounds,
        mut itctx: ImplTraitContext<'_, 'itctx>,
    ) -> hir::TyKind<'hir>
    where
        'a: 'itctx,
    {
        ...
        self.with_hir_id_owner(opaque_ty_node_id, |lctx| {
            ...
            for ty in impl_trait_inputs {
                if let TyKind::ImplTrait(node_id, _) = ty.kind {
                    ...
                    // Add a definition for the generic param def.
                    let new_def_id =
                        lctx.create_def(opaque_ty_def_id, node_id, DefPathData::ImplTrait);
            ...
            let in_scope_generics: Vec<_> = ast_generics
                .params
                .iter()
                .map(|param| param.id)
                .chain(impl_trait_inputs.iter().map(|ty| {
                    if let TyKind::ImplTrait(node_id, _) = ty.kind {
                        node_id
                    } else {
                        unreachable!(
                ...
                .map(|node_id| lctx.local_def_id(node_id))
                .collect();
            ...
            lctx.with_remapping(new_remapping, &in_scope_generics, |lctx| {
                ...
                let type_const_defs: Vec<_> = ast_generics
                    .params
                    .iter()
                    .filter(|param| {
                        matches!(
                ...
                let generic_defs = lifetime_defs
                    .into_iter()
                    .chain(type_const_defs.into_iter())
        ...
        let type_const_args = self.collect_type_const_args(&ast_generics.params);
    ...
    #[instrument(level = "trace", skip(self))]
    fn lower_param_bound<'itctx>(
        &mut self,
        tpb: &'itctx GenericBound,
        itctx: ImplTraitContext<'_, 'itctx>,
    ) -> hir::GenericBound<'hir>
    where
        'a: 'itctx,
    ...
    #[instrument(level = "debug", skip(self))]
    fn lower_poly_trait_ref<'itctx>(
        &mut self,
        p: &'itctx PolyTraitRef,
        itctx: ImplTraitContext<'_, 'itctx>,
    ...
    fn lower_param_bounds_mut<'s, 'itctx>(
        &'s mut self,
        bounds: &'itctx [GenericBound],
        mut itctx: ImplTraitContext<'s, 'itctx>,
    ) -> impl Iterator<Item = hir::GenericBound<'hir>> + Captures<'s> + Captures<'a> + Captures<'itctx>
    where
        'a: 'itctx,
    {
    ...
    fn lower_generic_and_bounds(
        ...
        let preds = self.lower_generic_bound_predicate(
            ...
            bounds,
            ImplTraitContext::Universal { apit_nodes: &mut Vec::new() },
