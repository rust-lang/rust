...
impl<'hir> LoweringContext<'_, 'hir> {
        ...
        ensure_sufficient_stack(|| {
            ...
            let kind = match &e.kind {
                ...
                ExprKind::IncludedBytes(byte_sym) => {
                    let lit = respan(e.span, LitKind::ByteStr(*byte_sym, StrStyle::Cooked));
            ...
            hir::Expr { hir_id: expr_hir_id, kind, span: e.span }
        })
    }
    ...
    fn lower_assign_op(&mut self, a: AssignOp) -> AssignOp {
        Spanned { node: a.node, span: a.span }
    }
            ...
            && !is_never_pattern
        {
            ...
            if !is_never_pattern {
                if self.tcx.features().never_patterns() {
                    ...
                    let suggestion = arm.span.shrink_to_hi();
                    self.dcx().emit_err(MatchArmWithNoBody { span: arm.span, suggestion });
                }
            ...
            self.arena.alloc(hir::Expr {
                hir_id: self.next_id(),
                kind: hir::ExprKind::Loop(block, None, hir::LoopSource::Loop, arm.span),
                span: arm.span,
            })
        };
        hir::Arm { hir_id, pat, guard, body, span: arm.span }
    }
    ...
    fn lower_loop_destination(&mut self, destination: Option<(NodeId, Label)>) -> hir::Destination {
        ...
        let label = destination.map(|(_, label)| label).map(|label| Label { ident: label.ident });
