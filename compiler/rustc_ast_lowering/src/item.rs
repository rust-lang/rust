...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    fn lower_item_kind(
        ...
        match i {
            ...
            ItemKind::Const(box ast::ConstItem { ident, generics, ty, expr, .. }) => {
                let ident = self.lower_ident(*ident);
    ...
    fn lower_foreign_item(&mut self, i: &ForeignItem) -> &'hir hir::ForeignItem<'hir> {
        ...
        let (ident, kind) = match &i.kind {
            ForeignItemKind::Fn(box Fn { sig, ident, generics, .. }) => {
                let fdec = &sig.decl;
                let itctx = ImplTraitContext::Universal;
            ...
            ForeignItemKind::Static(box StaticItem { ident, ty, mutability, expr: _, safety }) => {
                let ty =
    ...
    fn lower_trait_item(&mut self, i: &AssocItem) -> &'hir hir::TraitItem<'hir> {
        ...
        let (ident, generics, kind, has_default) = match &i.kind {
            AssocItemKind::Const(box ConstItem { ident, generics, ty, expr, .. }) => {
