...
impl<'a> Visitor<'a> for ItemLowerer<'a, '_, '_> {
    fn visit_item(&mut self, item: &'a Item) {
        ...
        self.lctx.without_in_scope_lifetime_defs(|lctx| {
            lctx.with_hir_id_owner(item.id, |lctx| {
                let node = lctx.lower_item(item);
                if let hir::OwnerNode::Item(hir_item) = node {
                    item_hir_id = Some(hir_item.item_id());
                }
                node
            })
        });
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    pub(super) fn lower_item_ref(&mut self, i: &Item) -> SmallVec<[hir::ItemId; 1]> {
        ...
        node_ids
            .into_iter()
            .map(|node_id| hir::ItemId { def_id: self.resolver.local_def_id(node_id) })
            .collect()
    }
    ...
    fn lower_item(&mut self, i: &Item) -> hir::OwnerNode<'hir> {
        ...
        if let ItemKind::MacroDef(MacroDef { ref body, macro_rules }) = i.kind {
            ...
            if !macro_rules || self.sess.contains_name(&i.attrs, sym::macro_export) {
                ...
                return hir::OwnerNode::MacroDef(self.arena.alloc(def));
            } else {
                return hir::OwnerNode::NonExportedMacro(i.span, hir_id.expect_owner());
            }
        }
        ...
        let item = hir::Item { def_id: hir_id.expect_owner(), ident, kind, vis, span: i.span };
        hir::OwnerNode::Item(self.arena.alloc(item))
    }
                ...
                *ident = tree.ident();
                ...
                for (res, &new_node_id) in iter::zip(resolutions, &[id1, id2]) {
                    ...
                    self.with_hir_id_owner(new_node_id, |this| {
                        ...
                        let item = hir::Item { def_id: new_id, ident, kind, vis, span };
            ...
            UseTreeKind::Nested(ref trees) => {
                ...
                for &(ref use_tree, id) in trees {
                    ...
                    self.with_hir_id_owner(id, |this| {
                        ...
                        let item =
                            hir::Item { def_id: new_hir_id, ident, kind, vis, span: use_tree.span };
                        hir::OwnerNode::Item(this.arena.alloc(item))
                    });
                }
