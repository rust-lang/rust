        ...
        match kind {
            ...
            DefKind::Ctor(CtorOf::Variant, ..) | DefKind::Variant => {
                let (generics_def_id, index) = if let Some(self_ty) = self_ty {
                    ...
                    // reject args on `DefKind::Ctor` segments (see `is_alias_variant_ctor`). Also,
                    // `DefKind::Variant` segments never actually reach this branch as they're
                    // interpreted as `TypeRelative` paths whose lowering routines manually reject
                    // args on such segments.
                    ...
                    && let Res::Def(DefKind::Enum, _) = second_to_last.res
                {
                    ...
                    // certainly don't want to allow generic args on said segment!
