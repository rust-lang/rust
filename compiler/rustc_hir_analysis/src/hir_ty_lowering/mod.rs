...
use rustc_middle::ty::{
    ...
    TypeSuperFoldable, TypeVisitableExt, TypingMode, Unnormalized, Upcast, const_lit_matches_ty,
    fold_regions,
};
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    fn lower_path_segment(
            ...
            && tcx.type_alias_is_lazy(did)
        {
            ...
            tcx.at(span).type_of(did).instantiate(tcx, args).skip_normalization()
        }
    }
                ...
                tcx.associated_items(*trait_def_id)
                    ...
                    && tcx.all_impls(*trait_def_id)
                        .any(|impl_def_id| {
                            ...
                            let trait_ref = header.trait_ref.instantiate(tcx, infcx.fresh_args_for_item(DUMMY_SP, impl_def_id)).skip_normalization();
        ...
        let fields = variant_def
            ...
            .map(|(field_def, arg)| {
                self.lower_const_arg(
                    arg,
                    tcx.type_of(field_def.did).instantiate(tcx, adt_args).skip_normalization(),
                )
            })
            .collect::<Vec<_>>();
    ...
    fn lower_const_arg_anon(&self, anon: &AnonConst) -> Const<'tcx> {
        ...
        let ty = tcx.type_of(anon.def_id).instantiate_identity().skip_normalization();
    ...
    fn lower_delegation_ty(&self, infer: hir::InferDelegation<'tcx>) -> Ty<'tcx> {
        match infer {
            hir::InferDelegation::DefId(def_id) => {
                self.tcx().type_of(def_id).instantiate_identity().skip_normalization()
            }
