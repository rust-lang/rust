...
use rustc_hir::{AmbigArg, HirId, LangItem, PolyTraitRef};
...
fn remove_lang_item_bound<'tcx>(bounds: &mut Vec<(ty::Clause<'tcx>, Span)>, did: DefId) {
    bounds.retain(|(clause, _)| {
        clause.as_trait_clause().map_or(true, |clause| clause.skip_binder().def_id() != did)
    });
}

fn trait_generics_and_bounds_from_trait_item<'tcx>(
    tcx: TyCtxt<'tcx>,
    trait_item: &hir::TraitItem<'tcx>,
) -> (LocalDefId, &'tcx [hir::GenericBound<'tcx>], &'tcx hir::Generics<'tcx>) {
    ...
    match parent_trait.kind {
        hir::ItemKind::Trait(_, _, _, generics, supertraits, _) => (parent, supertraits, generics),
        hir::ItemKind::TraitAlias(_, generics, supertraits) => (parent, supertraits, generics),
        _ => unreachable!(),
    }
}
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    /// bounds if they are not added to trait associated items. See `requires_default_supertraits`
    /// and `add_sizedness_trait_item_bounds` for more information.
    ///
    /// Also removes `PointeeSized` params - see doc comment on `adjust_sizedness_predicates`.
    pub(crate) fn adjust_sizedness_supertraits(
        ...
        if self.should_add_metasized_supertrait(hir_bounds, trait_did)
            && self.requires_default_supertraits(
                hir_bounds,
                hir_generics,
                Some((meta_sized_did, trait_did)),
            )
        {
        ...
        let collected = collect_sizedness_bounds(tcx, hir_bounds, None, span);
        if let Some(bound_span) = collected.sized.positive
    ...
    /// Implicitly add `Self: MetaSized` clauses on trait associated items if they are not added as
    /// super trait bounds to the trait itself. See `requires_default_supertraits` and
    /// `adjust_sizedness_supertraits` for more information.
    pub(crate) fn add_sizedness_trait_item_bounds(
        &self,
        trait_item: &hir::TraitItem<'tcx>,
        bounds: &mut Vec<(ty::Clause<'tcx>, Span)>,
    ) {
        ...
        let (parent, trait_bounds, trait_generics) =
            trait_generics_and_bounds_from_trait_item(tcx, trait_item);

        let span = tcx.def_span(parent);
        let meta_sized_did = tcx.require_lang_item(LangItem::MetaSized, Some(span));

        if self.should_add_metasized_supertrait(trait_bounds, parent)
            && !self.requires_default_supertraits(
                trait_bounds,
                trait_generics,
    ...
    fn requires_default_supertraits(
        ...
        hir_bounds: &'tcx [hir::GenericBound<'tcx>],
        hir_generics: &'tcx hir::Generics<'tcx>,
        def_ids: Option<(DefId, LocalDefId)>, // just quickly
    ) -> bool {
        ...
        for bound in hir_bounds {
            ...
            if let hir::GenericBound::Trait(ptr) = bound
                && let Some((target_did, current_did)) = def_ids
                && let Some(did) = ptr.trait_ref.trait_def_id()
                && did != current_did.into()
            {
                let preds = self.tcx().explicit_predicates_of(did);
                found |= preds.predicates.iter().filter_map(|(c, _)| c.as_trait_clause()).any(|p| {
                    p.self_ty().skip_binder() == self.tcx().types.self_param
                        && p.def_id() == target_did
                })
            }
        }
    ...
    pub(crate) fn add_default_trait_item_bounds(
        ...
        let (parent, trait_bounds, trait_generics) =
            trait_generics_and_bounds_from_trait_item(tcx, trait_item);
        if !self.requires_default_supertraits(trait_bounds, trait_generics, None) {
    ...
    pub(crate) fn add_default_super_traits(
        ...
        if self.requires_default_supertraits(hir_bounds, hir_generics, None) {
