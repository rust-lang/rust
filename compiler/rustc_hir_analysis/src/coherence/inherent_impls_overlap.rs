...
AssocItems,)->bool{();let mut impl_items1=&impl_items1;3;3;let mut impl_items2=&
impl_items2;({});if impl_items1.len()>impl_items2.len(){({});std::mem::swap(&mut
impl_items1,&mut impl_items2);;}for&item1 in impl_items1.in_definition_order(){;
let collision=((impl_items2.filter_by_name_unhygienic(item1.name))).any(|&item2|
self.compare_hygienically(item1,item2));3;if collision{3;return true;;}}false}fn
...
){;let span=self.tcx.def_span(impl_item.def_id);;let ident=impl_item.ident(self.
tcx);3;3;let norm_ident=ident.normalize_to_macros_2_0();;match seen_items.entry(
norm_ident){IndexEntry::Occupied(entry)=>{();let former=entry.get();3;3;res=Err(
...
"other definition for `{ident}`")).emit());;}IndexEntry::Vacant(entry)=>{;entry.
...
impl_items1=self.tcx.associated_items(impl1);({});({});let impl_items2=self.tcx.
associated_items(impl2);{;};{;};let mut res=Ok(());{;};for&item1 in impl_items1.
...
"duplicate definitions with name `{}`",name);;;err.span_label(self.tcx.def_span(
item1.def_id),format!("duplicate definitions for `{name}`"),);3;;err.span_label(
...
::add_placeholder_note(&mut err);{();};}{();};res=Err(err.emit());{();};}}res}fn
...
owner_id.to_def_id());{();};({});let impls_items=impls.iter().map(|impl_def_id|(
...
default();{;};();let mut connected_region_ids=FxIndexMap::default();();for(i,&(&
_impl_def_id,impl_items))in (impls_items.iter().enumerate()){if impl_items.len()
==0{;continue;;}let mut idents_to_add=SmallVec::<[Symbol;8]>::new();let mut ids=
...
Some(*e.get())}else{;idents_to_add.push(item.name);;None}}).collect::<SmallVec<[
...
unwrap();();3;region.impl_blocks.insert(i);3;3;region.idents.extend_from_slice(&
idents_to_add);3;for ident in&idents_to_add{;connected_region_ids.insert(*ident,
id_to_set);({});}}&[id_to_set,..]=>{{;};let mut region=connected_regions.remove(
id_to_set).unwrap();({});{;};region.impl_blocks.insert(i);{;};{;};region.idents.
extend_from_slice(&idents_to_add);if true{};for ident in&idents_to_add{let _=();
...
idents.iter(){3;connected_region_ids.insert(*ident,id_to_set);3;};region.idents.
extend_from_slice(&r.idents);();3;region.impl_blocks.extend(r.impl_blocks);3;}3;
connected_regions.insert(id_to_set,region);if let _=(){};}}}loop{break;};debug!(
...
collect::<SmallVec<[usize;8]>>();{;};{;};impl_blocks.sort_unstable();{;};for(i,&
...
impl2_items_idx];;if self.impls_have_common_items(impl_items1,impl_items2){;res=
