...
fn visit_implementation_of_drop(checker: &Checker<'_>) -> Result<(), ErrorGuaranteed> {
    ...
    match checker.impl_header.trait_ref.instantiate_identity().skip_normalization().self_ty().kind()
    {
...
fn visit_implementation_of_copy(checker: &Checker<'_>) -> Result<(), ErrorGuaranteed> {
    ...
    let self_type = impl_header.trait_ref.instantiate_identity().skip_normalization().self_ty();
...
fn visit_implementation_of_const_param_ty(checker: &Checker<'_>) -> Result<(), ErrorGuaranteed> {
    ...
    let impl_did = checker.impl_def_id;
    let self_type = header.trait_ref.instantiate_identity().skip_normalization().self_ty();
    assert!(!self_type.has_escaping_bound_vars());
...
fn visit_implementation_of_dispatch_from_dyn(checker: &Checker<'_>) -> Result<(), ErrorGuaranteed> {
    let tcx = checker.tcx;
    let impl_did = checker.impl_def_id;
    let trait_ref = checker.impl_header.trait_ref.instantiate_identity().skip_normalization();
...
pub(crate) fn coerce_unsized_info<'tcx>(
    ...
    let source = tcx.type_of(impl_did).instantiate_identity().skip_normalization();
            ...
            if def_a.is_struct() && def_b.is_struct() =>
        {
            ...
            let diff_fields = fields
                ...
                .filter_map(|(i, f)| {
                    ...
                    // Ignore PhantomData fields
                    let unnormalized_ty =
                        tcx.type_of(f.did).instantiate_identity().skip_normalization();
                    if tcx
                        .try_normalize_erasing_regions(
                            ty::TypingEnv::non_body_analysis(tcx, def_a.did()),
                            unnormalized_ty,
...
fn visit_implementation_of_coerce_pointee_validity(
    ...
    let tcx = checker.tcx;
    let self_ty = tcx
        .impl_trait_ref(checker.impl_def_id)
