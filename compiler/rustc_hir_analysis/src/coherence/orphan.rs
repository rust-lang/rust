            ...
            Ok(()) => match err {
                OrphanCheckErr::UncoveredTy(UncoveredTy { uncovered, local_ty, in_self_ty }) => {
                    ...
                    let span = match in_self_ty {
                        ...
                        InSelfTy::No => hir_trait_ref.trait_ref.path.span, // point at `C<B>`.
                    };

                    for ty in uncovered {
                        match ty {
                            UncoveredTyKind::TyParam(ident) => tcx.emit_node_span_lint(
                                UNCOVERED_PARAM_IN_PROJECTION,
                                item.hir_id(),
                                ident.span,
                                rustc_errors::DiagDecorator(|diag| {
                                    decorate_uncovered_ty_diag(diag, ident.span, ty, local_ty)
                                }),
                            ),
