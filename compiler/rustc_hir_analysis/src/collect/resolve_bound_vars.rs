...
impl<'a, 'tcx> BoundVarContext<'a, 'tcx> {
    ...
    fn visit_segment_args(
        ...
        // lifetime defaults* for trait object types contained in any of the type arguments passed
        // to it (any inner generic containers will of course end up shadowing that default).
        //
        // FIXME(mgca, #151649): Assoc (and free?) consts should also qualify.
        // FIXME(trait_alias, #140710): Trait aliases should also qualify.
        ...
        for constraint in generic_args.constraints {
            // FIXME(fmease): This ambient object lifetime default should only be used on the RHS
            //                of assoc type constraints, not however for the args of the constraints.
            let scope = Scope::ObjectLifetimeDefault {
                ...
                //       that they occur in the item bounds of the assoc type, so we conservatively
                //       reject such cases to allow us to implement the correct behavior in the
                //       future (here we assume that the number of arguments equals the number of
                //       parameters which is fine since a mismatch would get rejected later anyway).
                //
    ...
    /// Compute a list of ambient object lifetime defaults, one for each type parameter,
    ...
    ///
    /// Therefore, we would compute a list like `['x, 'static]`.
    /// Note that the list only includes regions for type parameters
    ///
