...
/// Whether to fetch the hidden types of opaques as computed
/// by HIR typeck or MIR borrowck.
///
/// The types computed during HIR typeck are used to compute the
/// types used by borrowck. Do not use the types from HIR typeck
/// outside of borrowck as they have erased regions. We need
/// borrowck to infer the regions of the hidden type.
#[derive(Debug)]
pub(super) enum OpaqueTypesFrom {
    HirTypeck,
    Borrowck,
}
...
pub(super) fn find_opaque_ty_constraints_for_impl_trait_in_assoc_type(
    tcx: TyCtxt<'_>,
    def_id: LocalDefId,
    opaque_types_from: OpaqueTypesFrom,
) -> Ty<'_> {
        ...
        OpaqueTypesFrom::Borrowck => {
            ...
            if let Some(guar) = borrowck_result.tainted_by_errors {
                ...
                if let Err(guar) = hir_ty.error_reported() {
                    Ty::new_error(tcx, guar)
                } else if hir_ty.has_erased_regions() {
                    let guar = tcx.dcx().emit_err(UnconstrainedOpaqueType {
                        span: tcx.def_span(def_id),
                        name: Ident::from_str("impl Trait"),
                        what: "function",
                    });
                    Ty::new_error(tcx, guar)
                } else {
