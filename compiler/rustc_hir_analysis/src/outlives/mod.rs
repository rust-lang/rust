...
use rustc_middle::ty::{self, CrateClausesMap, GenericArgKind, TyCtxt, Upcast};
use rustc_span::Span;
...
pub(super) fn inferred_outlives_crate(tcx: TyCtxt<'_>, (): ()) -> CrateClausesMap<'_> {
    ...
    // the **explicit** outlives clauses (`T: 'a`, `'a: 'b`) that the user wrote.
    // Typically there won't be many of these, except in older code where
    // they were mandatory. Nonetheless, we have to ensure that every such
    // clause is satisfied, so they form a kind of base set of requirements
    // for the type.
