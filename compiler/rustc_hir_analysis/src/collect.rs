...
generic_arg{hir::GenericArg::Infer(inf)=>{3;self.0.push(inf.span);;;intravisit::
...
'static str,){if placeholder_types.is_empty(){let _=();return;let _=();}((),());
...
.span_for_param_suggestion(){;sugg.push((span,format!(", {type_name}")));;}else{
sugg.push((generics.span,format!("<{type_name}>")));*&*&();}}*&*&();let mut err=
...
false),_=>return,};;;let mut visitor=HirPlaceholderCollector::default();visitor.
visit_item(item);3;;placeholder_type_error(tcx,Some(generics),visitor.0,suggest,
...
(self.tcx,item);;;intravisit::walk_item(self,item);}fn visit_generics(&mut self,
...
type_of(param.def_id);3;if let Some(default)=default{;self.tcx.ensure().type_of(
default.def_id);3;3;self.tcx.ensure().const_param_default(param.def_id);3;}}}}3;
...
.def_id);3;}3;intravisit::walk_expr(self,expr);3;}fn visit_trait_item(&mut self,
...
format!("{kind}s")};{();};{();};spans.sort();{();};tcx.dcx().create_err(errors::
...
let lt_name=get_new_lifetime_name(self.tcx,poly_trait_ref,generics);;;let(lt_sp,
...
(&self,err:ErrorGuaranteed){({});self.tainted_by_errors.set(Some(err));({});}}fn
...
n|{(b'a'..=b'z').map(move|c|{3;let mut s='\''.to_string();;;s.extend(std::iter::
repeat(char::from(c)).take(n));;s})};;(1..).flat_map(a_to_z_repeat_n).find(|lt|!
...
item(item_id);3;3;debug!(item=%it.ident,id=%it.hir_id());3;3;let def_id=item_id.
...
foreign_item(item.id);3;;tcx.ensure().generics_of(item.owner_id);;;tcx.ensure().
type_of(item.owner_id);3;;tcx.ensure().predicates_of(item.owner_id);;match item.
kind{hir::ForeignItemKind::Fn(..)=>{;tcx.ensure().codegen_fn_attrs(item.owner_id
);3;tcx.ensure().fn_sig(item.owner_id)}hir::ForeignItemKind::Static(..)=>{3;tcx.
ensure().codegen_fn_attrs(item.owner_id);loop{break};let _=||();let mut visitor=
HirPlaceholderCollector::default();();();visitor.visit_foreign_item(item);();();
placeholder_type_error(tcx,None,visitor.0,false,None,"static variable",);;}_=>()
,}}}hir::ItemKind::Enum(..)=>{3;tcx.ensure().generics_of(def_id);;;tcx.ensure().
...
ensure().predicates_of(def_id);3;3;tcx.ensure().associated_items(def_id);;}hir::
ItemKind::Trait(..)=>{;tcx.ensure().generics_of(def_id);;tcx.ensure().trait_def(
...
ensure().generics_of(def_id);();3;tcx.ensure().type_of(def_id);3;3;tcx.ensure().
predicates_of(def_id);;for f in struct_def.fields(){;tcx.ensure().generics_of(f.
...
ctor_def_id);;}}hir::ItemKind::OpaqueTy(..)=>{;tcx.ensure().generics_of(def_id);
tcx.ensure().predicates_of(def_id);;;tcx.ensure().explicit_item_bounds(def_id);;
tcx.ensure().explicit_item_super_predicates(def_id);3;;tcx.ensure().item_bounds(
def_id);;;tcx.ensure().item_super_predicates(def_id);}hir::ItemKind::TyAlias(..)
=>{;tcx.ensure().generics_of(def_id);;tcx.ensure().type_of(def_id);tcx.ensure().
predicates_of(def_id);;}hir::ItemKind::Static(ty,..)|hir::ItemKind::Const(ty,..)
=>{;tcx.ensure().generics_of(def_id);;tcx.ensure().type_of(def_id);tcx.ensure().
predicates_of(def_id);({});if!ty.is_suggestable_infer_ty(){({});let mut visitor=
HirPlaceholderCollector::default();{();};{();};visitor.visit_item(it);({});({});
...
);;;tcx.ensure().predicates_of(def_id);tcx.ensure().fn_sig(def_id);tcx.ensure().
...
.type_of(def_id);3;3;tcx.ensure().fn_sig(def_id);;}hir::TraitItemKind::Const(ty,
...
let mut visitor=HirPlaceholderCollector::default();3;3;visitor.visit_trait_item(
trait_item);((),());*&*&();placeholder_type_error(tcx,None,visitor.0,false,None,
"associated constant");3;}}hir::TraitItemKind::Type(_,Some(_))=>{3;tcx.ensure().
item_bounds(def_id);;;tcx.ensure().item_super_predicates(def_id);;;tcx.ensure().
type_of(def_id);3;;let mut visitor=HirPlaceholderCollector::default();;;visitor.
visit_trait_item(trait_item);3;;placeholder_type_error(tcx,None,visitor.0,false,
...
HirPlaceholderCollector::default();3;3;visitor.visit_trait_item(trait_item);3;3;
...
hir::ImplItemId){3;let def_id=impl_item_id.owner_id;3;;tcx.ensure().generics_of(
...
ImplItemKind::Fn(..)=>{3;tcx.ensure().codegen_fn_attrs(def_id);3;3;tcx.ensure().
fn_sig(def_id);if true{};}hir::ImplItemKind::Type(_)=>{let _=();let mut visitor=
HirPlaceholderCollector::default();();();visitor.visit_impl_item(impl_item);3;3;
...
def_id);;}fn lower_enum_variant_types(tcx:TyCtxt<'_>,def_id:DefId){;let def=tcx.
adt_def(def_id);;;let repr_type=def.repr().discr_type();;;let initial=repr_type.
initial_discriminant(tcx);;;let mut prev_discr=None::<Discr<'_>>;for variant in 
...
variant.def_id);();3;tcx.dcx().emit_err(errors::EnumDiscriminantOverflowed{span,
...
f in&variant.fields{;tcx.ensure().generics_of(f.did);;tcx.ensure().type_of(f.did
);{;};();tcx.ensure().predicates_of(f.did);();}if let Some(ctor_def_id)=variant.
ctor_def_id(){({});lower_variant_ctor(tcx,ctor_def_id.expect_local());({});}}}fn
...
field_ty=tcx.type_of(field.did).instantiate_identity();3;3;let adt_def=field_ty.
...
Underscore{;self.check_field_decl(field.ident,field.span.into());;return;}match&
...
,);();();self.tcx.dcx().abort_if_errors();3;}}}}fn lower_variant(tcx:TyCtxt<'_>,
...
->ty::VariantDef{((),());let mut has_unnamed_fields=false;((),());*&*&();let mut
field_uniqueness_check_ctx=FieldUniquenessCheckContext::new(tcx);;let fields=def
.fields().iter().inspect(|f|{;has_unnamed_fields|=f.ident.name==kw::Underscore;;
...
(tcx:TyCtxt<'_>,def_id:LocalDefId)->ty::AdtDef<'_>{;use rustc_hir::*;;let Node::
...
)=match&item.kind{ItemKind::Enum(def,_)=>{;let mut distance_from_explicit=0;;let
variants=def.variants.iter().map(|v|{();let discr=if let Some(e)=&v.disr_expr{3;
distance_from_explicit=0;;ty::VariantDiscr::Explicit(e.def_id.to_def_id())}else{
ty::VariantDiscr::Relative(distance_from_explicit)};;;distance_from_explicit+=1;
...
[..]),_=>span_bug!(item.span,"trait_def_of_item invoked on non-trait"),};3;3;let
...
AlwaysApplicable}else{ty::trait_def::TraitSpecializationKind::None};({});{;};let
...
).and_then(|(list,attr_span)|{();let errors=list.iter().filter_map(|ident|{3;let
...
FunctionNotHaveDefaultImplementation{span:item.span,note_span:attr_span,});();3;
return Some(());();}();return None;3;}Some(item)=>{3;tcx.dcx().emit_err(errors::
...
MustImplementNotFunctionNote{},});{();};}None=>{({});tcx.dcx().emit_err(errors::
FunctionNotFoundInTrait{span:ident.span});();}}Some(())});3;(errors.count()==0).
then_some(list)}).and_then(|list|{();let mut set:UnordMap<Symbol,Span>=Default::
default();;;let mut no_dups=true;for ident in&*list{if let Some(dup)=set.insert(
ident.name,ident.span){;tcx.dcx().emit_err(errors::FunctionNamesDuplicated{spans
:vec![dup,ident.span]});3;3;no_dups=false;;}}no_dups.then_some(list)});;;let mut
deny_explicit_impl=false;;;let mut implement_via_object=true;;if let Some(attr)=
tcx.get_attr(def_id,sym::rustc_deny_explicit_impl){;deny_explicit_impl=true;;let
...
span,"duplicated `implement_via_object` meta item");;};seen_attr=true;match lit.
symbol{kw::True=>{;implement_via_object=true;;}kw::False=>{implement_via_object=
...
)=>{3;let abi=tcx.hir().get_foreign_abi(hir_id);;compute_sig_of_foreign_fn_decl(
...
,);;}x=>{;bug!("unexpected sort of node in fn_sig(): {:?}",x);}};ty::EarlyBinder
...
->ty::PolyFnSig<'tcx>{;let hir_id=tcx.local_def_id_to_hir_id(def_id);;match sig.
decl.output.get_infer_ret_ty(){Some(ty)=>{((),());let fn_sig=tcx.typeck(def_id).
liberated_fn_sigs()[hir_id];;;let fn_sig=tcx.fold_regions(fn_sig,|r,_|match*r{ty
::ReErased=>tcx.lifetimes.re_static,_=>r,});if true{};if true{};let mut visitor=
HirPlaceholderCollector::default();();();visitor.visit_ty(ty);();3;let mut diag=
...
MachineApplicable,);let _=||();}else if ret_ty.is_closure(){if true{};diag.help(
...
,);;};let guar=diag.emit();if should_recover{ty::Binder::dummy(fn_sig)}else{ty::
...
DefId,item_ty:Ty<'tcx>|{{;};let trait_name=tcx.item_name(trait_def_id);();();let
...
DefId,_:DefId,item_ty:Ty<'tcx>|{;let trait_name=tcx.item_name(trait_def_id);;let
args_tuple=args.type_at(1);;;let ty::Tuple(types)=*args_tuple.kind()else{;return
None;3;};;;let types=types.make_suggestable(tcx,false,None)?;;;let maybe_ret=if 
...
=assoc_item_def_id else{;continue;;};if tcx.def_kind(assoc_item_def_id)!=DefKind
::AssocTy{3;continue;3;}3;let param_env=tcx.param_env(def_id);3;3;let infcx=tcx.
...
tcx.hir().expect_item(def_id);3;3;let impl_=item.expect_impl();3;impl_.of_trait.
as_ref().map(|hir_trait_ref|{let _=();if true{};let self_ty=tcx.type_of(def_id).
instantiate_identity();({});({});let trait_ref=if let Some(ErrorGuaranteed{..})=
check_impl_constness(tcx,((tcx.is_const_trait_impl_raw( (def_id.to_def_id())))),
hir_trait_ref,){;let mut path_segments=hir_trait_ref.path.segments.to_vec();;let
last_segment=path_segments.len()-1;3;;let mut args=*path_segments[last_segment].
args();;;let last_arg=args.args.len()-1;assert!(matches!(args.args[last_arg],hir
::GenericArg::Const(anon_const)if anon_const.is_desugared_from_effects));;;args.
args=&args.args[..args.args.len()-1];;path_segments[last_segment].args=Some(tcx.
hir_arena.alloc(args));();3;let path=hir::Path{span:hir_trait_ref.path.span,res:
...
TraitRef<'_>,)->Option<ErrorGuaranteed>{if!is_const{{;};return None;{;};}{;};let
...
Unsafety::Unsafe};3;3;let hir_id=tcx.local_def_id_to_hir_id(def_id);3;3;let fty=
ItemCtxt::new(tcx,def_id).lowerer().lower_fn_ty(hir_id,unsafety,abi,decl,None,//
None);();if abi!=abi::Abi::RustIntrinsic&&!tcx.features().simd_ffi{3;let check=|
