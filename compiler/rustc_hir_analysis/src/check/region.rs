...
fn resolve_block<'tcx>(
...

    {
        ...
        if let Some(tail_expr) = blk.expr {
            ...
            // Check for temporary lifetime extension if we haven't already done so due to a parent
            // `let` statement initializer or block. This, e.g., allows `temp()` in `{ &temp() }`
            ...
            if !visitor.extended_blocks.contains(&blk.hir_id.local_id) {
                let blk_result_scope = prev_cx
                    .parent
                    .map(|blk_parent| visitor.scope_tree.default_temporary_scope(blk_parent).0);
...
fn resolve_expr<'tcx>(
    ...
    match expr.kind {
        ...
        hir::ExprKind::If(cond, then, Some(otherwise)) => {
            ...
            // Check for temporary lifetime extension if we haven't already done so due to a parent
            // `let` statement initializer or block. This, e.g., allows `format!("temp")` in
            ...
            if !visitor.extended_blocks.contains(&expr.hir_id.local_id) {
                let blk_result_scope = expr_cx
                    .parent
                    .map(|if_parent| visitor.scope_tree.default_temporary_scope(if_parent).0);
