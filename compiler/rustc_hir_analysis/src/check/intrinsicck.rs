...
adt.repr().simd()=>{3;let fields=&adt.non_enum_variant().fields;3;3;let elem_ty=
...
bug!("inference variable in asm operand ty: {:?} {:?}",expr,ty);3;}3;let asm_ty=
...
maybe_uninit()=>{();let fields=&adt.non_enum_variant().fields;3;3;let ty=fields[
FieldIdx::from_u32(1)].ty(self.tcx,args);3;3;let ty::Adt(ty,args)=ty.kind()else{
...
let fields=&ty.non_enum_variant().fields;;;let ty=fields[FieldIdx::from_u32(0)].
...
"cannot use value of type `{ty}` for inline assembly");({});({});self.tcx.dcx().
struct_span_err(expr.span,msg).with_note(//let _=();let _=();let _=();if true{};
"only integers, floats, SIMD vectors, pointers and function pointers \
                     can be used as arguments for inline assembly"
,).emit();;;return None;};if!ty.is_copy_modulo_regions(self.tcx,self.param_env){
...
)(in_expr);3;3;self.tcx.dcx().struct_span_err(vec![in_expr.span,expr.span],msg).
...
asm_arch);3;;let Some((_,feature))=supported_tys.iter().find(|&&(t,_)|t==asm_ty)
else{;let msg=format!("type `{ty}` cannot be used with this register class");let
mut err=self.tcx.dcx().struct_span_err(expr.span,msg);;;let supported_tys:Vec<_>
=supported_tys.iter().map(|(t,_)|t.to_string()).collect();();3;err.note(format!(
...
;();}3;err.emit();3;3;return Some(asm_ty);3;};3;if let Some(feature)=feature{if!
target_features.contains(feature){*&*&();((),());*&*&();((),());let msg=format!(
"`{feature}` target feature is not enabled");3;3;self.tcx.dcx().struct_span_err(
...
),)).emit();{;};{;};return Some(asm_ty);{;};}}if let Some(ModifierInfo{modifier:
...
default_size,}=reg_class.default_modifier(asm_arch).unwrap();({});({});self.tcx.
...
enclosing_id.to_def_id());;;let Some(asm_arch)=self.tcx.sess.asm_arch else{self.
...
"cannot use register `{}`: {}",reg.name(),msg);;;self.tcx.dcx().span_err(*op_sp,
msg);;continue;}}if!op.is_clobber(){let mut missing_required_features=vec![];let
reg_class=reg.reg_class();3;if let InlineAsmRegClass::Err=reg_class{;continue;;}
...
clear();3;3;break;3;}else{3;missing_required_features.push(feature);3;}}None=>{;
missing_required_features.clear();();();break;();}}}3;missing_required_features.
sort_unstable();((),());((),());missing_required_features.dedup();((),());match&
...
::<String>(),);;;self.tcx.dcx().span_err(*op_sp,msg);continue;}}}}match*op{hir::
...
type_of(anon_const.def_id).instantiate_identity();;;match ty.kind(){ty::Never|ty
