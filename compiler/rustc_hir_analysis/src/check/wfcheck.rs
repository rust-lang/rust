...
fn remap_opaque_captures(tcx: TyCtxt<'_>) {
    ...
    for def_id in items
        ...
        .chain(items.free_items().map(|item| item.owner_id))
        .chain(items.impl_items().map(|item| item.owner_id))
        .chain(items.trait_items().map(|item| item.owner_id))
        .chain(items.foreign_items().map(|item| item.owner_id))
    {
        tcx.ensure_ok().resolve_bound_vars(def_id);
    }
}
