...
pub fn maybe_stmt_static_mut(tcx: TyCtxt<'_>, stmt: &hir::Stmt<'_>) {
    ...
    } else if let rustc_hir::StmtKind::Semi(expr) = stmt.kind
        && let hir::ExprKind::AssignOp(_, expr, _) = expr.kind
        && let Some(err_span) = path_is_static_mut(expr, stmt.span)
    {
        handle_static_mut_ref(
            tcx,
            err_span,
            err_span.shrink_to_lo(),
            err_span.shrink_to_hi(),
            expr.span.edition().at_least_rust_2024(),
            Some(Mutability::Mut),
            stmt.hir_id,
            false,
        );
