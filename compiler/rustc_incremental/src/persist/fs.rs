...
pub(crate) fn prepare_session_directory(
    ...
    let new_session_directory = lock_directory(sess, &new_session_dir, true, true)
    ...
    return IncrCompSession { old_session_directory, new_session_directory };
}
...
fn find_source_directory(sess: &Session, crate_dir: &Path) -> Option<flock::LockedDir> {
    ...
    find_source_directory_in_iter(iter)
        .and_then(|session_dir| lock_directory(sess, &session_dir, false, false))
}
