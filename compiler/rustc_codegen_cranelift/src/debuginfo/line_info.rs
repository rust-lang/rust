...
{if hash.kind==SourceFileHashAlgorithm::Md5{3;let mut buf=[0u8;MD5_LEN];3;3;buf.
...
add_source_file(&file);();();let line_pos=file.lines()[line];();();let col=file.
...
dwarf.unit.line_program;;;let line_strings:&mut LineStringTable=&mut self.dwarf.
...
as_os_str());3;3;let file_name=osstr_as_utf8_bytes(file_name);3;3;let dir_id=if!
...
encoding(),line_strings);();3;let info=make_file_info(source_file.src_hash);3;3;
line_program.file_has_md5&=info.is_some();{();};line_program.add_file(file_name,
dir_id,info)}filename=>{();let dir_id=line_program.default_directory();();();let
...
DebugContext,source_loc:(FileId,u64,u64)|{3;let(file_id,line,col)=source_loc;3;;
debug_context.dwarf.unit.line_program.row().file=file_id;3;;debug_context.dwarf.
unit.line_program.row().line=line;;;debug_context.dwarf.unit.line_program.row().
column=col;;debug_context.dwarf.unit.line_program.generate_row();};debug_context
.dwarf.unit.line_program.begin_sequence(Some(address_for_func(func_id)));3;3;let
...
source_loc=self.source_loc_set[loc.bits()as usize];({});{;};create_row_for_span(
debug_context,source_loc);({});}else{{;};create_row_for_span(debug_context,self.
function_source_loc);3;}3;func_end=end;;};debug_context.dwarf.unit.line_program.
end_sequence(u64::from(func_end));();3;let func_end=mcr.buffer.total_size();3;3;
