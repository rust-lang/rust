...
impl BucketIndex {
    ...
    /// Adjustment factor from `ilog2` of a flat index, to its corresponding bucket number.
    ///
    /// For example, the first flat-index in bucket 2 is 8192. Computing
...
impl<T> Index<BucketIndex> for [T; BUCKETS] {
    ...
    fn index(&self, index: BucketIndex) -> &Self::Output {
        // The compiler should be able to see that `index.to_usize() < self.len()`,
        // and omit the bounds check.
        &self[index.to_usize()]
    }
}
