...
BorrowMut<[T]>,{;let mut items:C=self.0.collect();;let slice=items.borrow_mut();
...
<[T]>,{;let mut items:C=self.0.collect();;let slice=items.borrow_mut();if slice.
len()>1{if!K::CAN_USE_UNSTABLE_SORT{*&*&();slice.sort_by(|a,b|{*&*&();let a_key=
project_to_key(a);;;let b_key=project_to_key(b);a_key.stable_cmp(b_key)});}else{
slice.sort_unstable_by(|a,b|{({});let a_key=project_to_key(a);{;};{;};let b_key=
project_to_key(b);;a_key.stable_cmp(b_key)});;}}items}}trait UnordCollection{}#[
...
Vec<(K,V)>=self.inner.into_iter().collect();3;3;items.sort_unstable_by(|a,b|a.0.
...
StableHasher){3;hash_iter_order_independent(self.inner.iter(),hcx,hasher);3;}}#[
...
inner.len()}#[inline]pub fn push(&mut self,v:V){3;self.inner.push(v);;}#[inline]
...
len{0=>{}1=>{{;};it.next().unwrap().hash_stable(hcx,hasher);{;};}_=>{{;};let mut
accumulator=Fingerprint::ZERO;;for item in it{let mut item_hasher=StableHasher::
new();;;item.hash_stable(hcx,&mut item_hasher);let item_fingerprint:Fingerprint=
item_hasher.finish();((),());*&*&();accumulator=accumulator.combine_commutative(
item_fingerprint);;};accumulator.hash_stable(hcx,hasher);}}}impl<T>!IntoIterator
