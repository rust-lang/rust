...
/// A covariant [`Copy`] tagged pointer.
///
/// This is essentially `{ pointer: P, tag: T }` packed in a single pointer.
///
/// You should use this instead of the [`super::CopyTaggedPtr`] type in cases where
/// subtyping on the pointer type is necessary.
///
...
/// if you want that, wrap the [`super::CopyTaggedPtr`].
///
/// [`TaggedPtr`]: crate::tagged_ptr::TaggedPtr
pub struct CovariantCopyTaggedPtr<P, T, const COMPARE_PACKED: bool>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
...
impl<P, T, const CP: bool> CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
    ...
    pub fn new(pointer: P, tag: T) -> Self {
        Self {
            packed: Self::pack(P::into_ptr(pointer), tag).cast::<()>(),
    ...
    pub fn set_tag(&mut self, tag: T) {
        self.packed = Self::pack(self.pointer_raw(), tag).cast::<()>();
    }
    ...
    pub(super) fn pointer_raw(&self) -> NonNull<P::Target> {
        self.packed
            .map_addr(|addr| unsafe { NonZero::new_unchecked(addr.get() << T::BITS) })
            .cast::<P::Target>()
    }
...
impl<P, T, const CP: bool> Copy for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized> + Copy,
    T: Tag,
{
}

impl<P, T, const CP: bool> Clone for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized> + Copy,
    T: Tag,
{
...
impl<P, T, const CP: bool> Deref for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
...
impl<P, T, const CP: bool> DerefMut for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized> + DerefMut,
    T: Tag,
{
    #[inline]
...
impl<P, T, const CP: bool> fmt::Debug for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized> + fmt::Debug,
    T: Tag + fmt::Debug,
{
...
impl<P, T> PartialEq for CovariantCopyTaggedPtr<P, T, true>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
...
impl<P, T> Eq for CovariantCopyTaggedPtr<P, T, true>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
...
impl<P, T> Hash for CovariantCopyTaggedPtr<P, T, true>
where
    P: Pointer<Target: Sized>,
    T: Tag,
{
...
impl<P, T, HCX, const CP: bool> HashStable<HCX> for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Pointer<Target: Sized> + HashStable<HCX>,
...
unsafe impl<P, T, const CP: bool> Sync for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Sync + Pointer<Target: Sized>,
    T: Sync + Tag,
{
}
...
unsafe impl<P, T, const CP: bool> Send for CovariantCopyTaggedPtr<P, T, CP>
where
    P: Send + Pointer<Target: Sized>,
    T: Send + Tag,
{
}
