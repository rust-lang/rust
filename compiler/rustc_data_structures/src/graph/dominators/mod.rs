...
pub fn dominators<G: ControlFlowGraph>(graph: G) -> Dominators<G::Node> {
    ...
    //  * the idom is an ancestor of the sdom (i.e., idom may equal sdom)
    //  * the sdom is a proper ancestor of the vertex w (i.e., sdom != w)
    //
    // A semidominator (sdom) for a given vertex w is the vertex with minimum
    // preorder number such that there exists a path to w in which all elements have
    ...
    for w in (PreorderIndex::new(1)..PreorderIndex::new(reachable_vertices)).rev() {
        ...
        for &v in bucket[z].iter() {
            // This uses the result of Lemma 2.5 from the original 1979 paper,
            // to compute either the immediate or relative dominator for a given
            // vertex v.
            //
        ...
        for v in graph.predecessors(pre_order_to_real[w]) {
            ...
            // vertices semi[v] +> x *> v.
            //
            // From Lemma 2.4, we know that the semidominator of a vertex w is
            // the minimum (by preorder number) vertex of the following:
            //
