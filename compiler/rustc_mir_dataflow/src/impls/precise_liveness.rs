...
//! that writes to it without a `Deref` projection. It ends at `StorageLive` or
//! `StorageDead`, or at the early point of a whole-local move operand or the
//! last use of that local on a control-flow path (only for locals whose address
//! is never observed). This happens at `StorageLive` because it resets a local
...
use crate::{Analysis, GenKill, ResultsVisitor, visit_results};
...
// remain allocated until `StorageLive`, `StorageDead`, or a whole-local move.
//
...
impl<'a> KillPoints<'a> {
    fn compute<'tcx>(
        ...
        visit_results(
            body,
            mir::traversal::reachable(body).map(|(block, _)| block),
            &maybe_live_locals,
            &mut visitor,
        );
        trace!(?kill_points);
...
impl<'tcx> Analysis<'tcx> for PreciseLiveness<'_> {
    ...
    fn apply_primary_terminator_effect(
        ...
        // Gen destination places.
        VisitPlacesWith(|place: Place<'tcx>, ctxt| {
            // The resume place of a Yield is only evaluated after the coroutine
            // is resumed. This is in contrast to call destinations and inline
            // asm outputs which are evaluated before the call and therefore
            // remain allocated on both normal and unwind edges.
            if ctxt == PlaceContext::MutatingUse(MutatingUseContext::Yield) {
                return;
            }
    ...
    fn apply_call_return_effect(
        ...
        if let mir::CallReturnPlaces::Yield(place) = return_places
            && !place.is_indirect()
        {
...
pub fn liveness_matrix<'tcx>(
    ...
    for (block, block_data) in body.basic_blocks.iter_enumerated() {
        ...
        for (statement_index, statement) in block_data.statements.iter().enumerate() {
            ...
            // StorageLive and StorageDead free the old allocation, even if it
            // has been borrowed.
            if let mir::StatementKind::StorageLive(local) | mir::StatementKind::StorageDead(local) =
                statement.kind
            {
