    ...
    F: FnMut(MovePathIndex),
{
    match lookup_result {
        ...
        LookupResult::Exact(e) => on_all_children_bits(tcx, move_data, e, each_child),
    }
}
    ...
    F: FnMut(MovePathIndex),
{
    ...
    fn is_terminal_path<'tcx>(
        ...
        let ty = move_data.move_paths[path].ty;
        match ty.kind() {
        ...
        F: FnMut(MovePathIndex),
    {
        ...
        if is_terminal_path(tcx, move_data, move_path_index) {
            return;
        }

        let mut next_child_index = move_data.move_paths[move_path_index].first_child;
        while let Some(child_index) = next_child_index {
            on_all_children_bits(tcx, move_data, child_index, each_child);
            next_child_index = move_data.move_paths[child_index].next_sibling;
        }
    }
    on_all_children_bits(tcx, move_data, move_path_index, &mut each_child);
}
    ...
    F: FnMut(MovePathIndex, DropFlagState),
{
    ...
    for arg in body.args_iter() {
        ...
        on_lookup_result_bits(tcx, move_data, lookup_result, |mpi| {
    ...
    F: FnMut(MovePathIndex, DropFlagState),
{
    ...
    for mi in &move_data.loc_map[loc] {
        ...
        on_all_children_bits(tcx, move_data, path, |mpi| callback(mpi, DropFlagState::Absent))
    }
        ...
        body.stmt_at(loc).right()
    {
        if let LookupResult::Exact(mpi) = move_data.rev_lookup.find(place.as_ref()) {
            on_all_children_bits(tcx, move_data, mpi, |mpi| callback(mpi, DropFlagState::Absent))
        }
    }

    debug!("drop_flag_effects: assignment for location({:?})", loc);

    for_location_inits(tcx, move_data, loc, |mpi| callback(mpi, DropFlagState::Present));
}
    ...
    for ii in &move_data.init_loc_map[loc] {
        ...
        match init.kind {
            InitKind::Deep => {
                ...
                on_all_children_bits(tcx, move_data, path, &mut callback)
            }
            InitKind::Shallow => {
                let mpi = init.path;
                callback(mpi);
...
pub(crate) fn on_all_inactive_variants<'tcx>(
    ...
    for (variant_mpi, variant_path) in enum_path.children(&move_data.move_paths) {
        ...
        if variant_idx != active_variant {
            on_all_children_bits(tcx, move_data, variant_mpi, |mpi| handle_inactive_variant(mpi));
        }
    }
}
