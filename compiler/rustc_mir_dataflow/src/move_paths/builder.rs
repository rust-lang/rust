        ...
        for (i, elem) in place.projection.iter().enumerate() {
                ...
                ty::Ref(..) | ty::RawPtr(..) => {
                    ...
                    return Err(MoveError::cannot_move_out_of(
                        ...
                        BorrowedContent {
                            target_place: Place {
                                local: place.local,
                                projection: tcx.mk().place_elems(proj),
                            },
                        },
                    ));
                }
            ...
            if union_path.is_none() {
                base = self.add_move_path(base, elem, |tcx| Place {
                    local: place.local,
                    projection: tcx.mk().place_elems(&place.projection[..i + 1]),
                });
            }
        }
...
impl<'b, 'a, 'tcx> Gatherer<'b, 'a, 'tcx> {
        ...
        match &stmt.kind {
            ...
            StatementKind::Assign(box (place, rval)) => {
                ...
                if let RvalueInitializationState::Shallow = rval.initialization_state() {
                    ...
                    // the exterior.
                    self.create_move_path(self.builder.tcx.mk().place_deref(*place));
    ...
    fn gather_move(&mut self, place: Place<'tcx>) {
            ...
            **place.projection
        {
            ...
            let base_place =
                Place { local: place.local, projection: self.builder.tcx.mk().place_elems(base) };
            ...
            for offset in from..to {
                ...
                let path = self
                    .add_move_path(base_path, elem, |tcx| tcx.mk().place_elem(base_place, elem));
                self.record_move(place, path);
            }
        } else {
