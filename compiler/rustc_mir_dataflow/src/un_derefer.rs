...
PlaceRef<'tcx>)->Self{;let last=if place.as_local().is_none(){Some(place)}else{;
...
Option<(PlaceRef<'tcx>,PlaceElem<'tcx>)>{3;let place=self.places.read()?;3;3;let
partial_place=PlaceRef{local:place.local,projection:&place.projection[..self.//;
proj_idx]};3;;let elem=place.projection[self.proj_idx];;if self.proj_idx==place.
...
self.slice{[_,ref remainder@..]=>{;self.slice=remainder;;}[]=>self.last=None,}}}
