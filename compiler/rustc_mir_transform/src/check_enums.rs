...
impl<'a, 'tcx> Visitor<'tcx> for EnumFinder<'a, 'tcx> {
    fn visit_rvalue(&mut self, rvalue: &Rvalue<'tcx>, location: Location) {
        if let Rvalue::Cast(CastKind::Transmute, op, ty) = rvalue {
            ...
            let Ok(discriminant_layout) =
                self.tcx.layout_of(self.typing_env.as_query_input(ty.discriminant_ty(self.tcx)))
            else {
                return;
            };
            ...
            match enum_layout.variants {
                ...
                Variants::Single { .. } => {}
                // Construction of an enum with multiple variants and no niche optimizations.
                Variants::Multiple { tag_encoding: TagEncoding::Direct, .. } => {
                    ...
                    self.enums.push(EnumCheckType::Direct {
                        ...
                        discriminant_size: discriminant_layout.size,
                        op_size: op_layout.size,
                        discriminant_ty: ty.discriminant_ty(self.tcx),
                        valid_discrs,
                    });
                }
                ...
                Variants::Multiple {
                    ...
                    tag: Scalar::Initialized { valid_range, .. },
                    ..
                } => {
...
fn insert_discr_cast_to_u128<'tcx>(
    ...
    let (cast_kind, discr_ty_bits) = if discriminant_size.bytes() < op_size.bytes() {
        ...
        let offset = discriminant_size.bytes();
        mu_array.project_deeper(
            &[ProjectionElem::ConstantIndex { offset, min_length: array_len, from_end: false }],
            tcx,
        );
        (CastKind::Transmute, Operand::Copy(mu_array))
    } else {
    ...
    let rvalue = Rvalue::Cast(cast_kind, discr_ty_bits, discriminant_ty);
    let discr_in_discr_ty =
        local_decls.push(LocalDecl::with_source_info(discriminant_ty, source_info)).into();
    block_data.statements.push(Statement {
        source_info,
