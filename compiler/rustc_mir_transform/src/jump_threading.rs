...
impl<'a, 'tcx> OpportunitySet<'a, 'tcx> {
    ...
    fn apply(mut self) {
        ...
        while let Some(bb) = worklist.pop() {
            ...
            // We are modifying the `bb` in-place. Once a `ConditionTarget` has been applied, it
            // does not need to be applied again.
            let mut targets: Vec<_> = state
                .fulfilled
                .iter()
            ...
            while let Some(target) = targets.pop() {
                ...
                match target {
                    ConditionTarget::Goto(target) => {
                        ...
                        // We only have `target` as single successor. Drop all other target blocks.
                        targets.retain(|t| t.block() == target);
                        // Do this on targets that may be applied by a duplicate of `bb`.
                        for ts in self.entry_states[bb].targets.iter_mut() {
                            ts.retain(|t| t.block() == target);
                        }
                    }
                    ConditionTarget::Chain(target, condition) => {
                        let new_target = self.apply_chain(bb, target, condition);
                        ...
                        if let Some(new_target) = new_target {
                            ...
                            // Do this on targets that may be applied by a duplicate of `bb`.
                            for t in self.entry_states[bb]
                                .targets
                                .iter_mut()
                                .flat_map(|ts| ts.iter_mut())
                            {
                                t.replace_block(target, new_target)
                            }
                        }
