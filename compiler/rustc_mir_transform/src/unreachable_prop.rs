    ...
    fn run_pass<'tcx>(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        for (bb, bb_data) in traversal::postorder(body) {
            ...
            let is_unreachable = match &terminator.kind {
                ...
                // This will unconditionally run into an unreachable and is therefore unreachable as
                // well.
...
fn remove_successors_from_switch<'tcx>(
    ...
    // LLVM to turn it into just `x` later. Without the unreachable, such a transformation would
    // be illegal.
    //
