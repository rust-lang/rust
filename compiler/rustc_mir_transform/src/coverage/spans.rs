...
pub(super) fn extract_refined_covspans<'tcx>(
    ...
    if let Some(span) = invocation_span_for_macro_generated_body(tcx, hir_info) {
        mappings.push(Mapping { span, kind: MappingKind::Code { bcb: START_BCB } });
        return;
    }
...
fn invocation_span_for_macro_generated_body(
    ...
    // Walk up the expansion chain to find the outermost invocation site, i.e.
    // the one that appears literally in the source text.
    let mut invocation_span = body_span;
    let mut macro_kind = None;
    while !invocation_span.ctxt().is_root() {
        let expn_data = invocation_span.ctxt().outer_expn_data();
        ...
        let ExpnKind::Macro(kind, _) = expn_data.kind else { return None };
        if expn_data.call_site.is_dummy() {
            return None;
        }
        macro_kind = Some(kind);
        invocation_span = expn_data.call_site;
    }
    ...
    if macro_kind == Some(MacroKind::Bang) {
