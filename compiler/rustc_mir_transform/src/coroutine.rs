...
struct TransformVisitor<'tcx> {
    ...
    suspension_points: IndexVec<SuspensionPointIdx, SuspensionPoint<'tcx>>,
...
fn locals_live_across_suspend_points<'tcx>(
    ...
    let storage_conflicts = if nr_upvars > 0 {
        let nr_true_saved_locals = saved_locals.count();
        let nr_total_saved_locals = nr_true_saved_locals + nr_upvars;
        let mut enlarged_storage_conflicts =
            BitMatrix::new(nr_total_saved_locals, nr_total_saved_locals);
        let mut upvars = DenseBitSet::new_empty(nr_total_saved_locals);
        ...
        for upvar in nr_true_saved_locals..nr_total_saved_locals {
            let upvar = CoroutineSavedLocal::from_usize(upvar);
...
pub(crate) fn mir_coroutine_witnesses<'tcx>(
    ...
    let liveness_info =
        locals_live_across_suspend_points(tcx, body, always_live_locals, upvar_tys.len(), movable);
