    ...
    for (saved_local, local) in saved_locals.iter_enumerated() {
        ...
        if local < upvar_start {
            ...
            let decl = CoroutineSavedTy {
                ty,
                source_info: body.local_decls[ty::CAPTURE_STRUCT_LOCAL].source_info,
...
pub(crate) fn mir_coroutine_witnesses<'tcx>(
    ...
    let (upvar_tys, movable) = match *coroutine_ty.kind() {
    ...
    let (_, coroutine_layout, _) = compute_layout(liveness_info, body, upvars);
...
impl<'tcx> MirPass<'tcx> for StateTransform {
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let (discr_ty, upvar_tys, movable) = match *coroutine_ty.kind() {
            ty::Coroutine(_, args) => {
