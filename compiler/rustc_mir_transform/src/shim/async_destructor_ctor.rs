...
fn build_async_destructor_ctor_shim_for_coroutine<'tcx>(
    ...
    let impl_cor_ty = ty.find_async_drop_impl_coroutine(tcx);
...
pub(super) fn build_future_drop_poll_shim<'tcx>(
    ...
    if tcx.is_async_drop_in_place_poll_fn(*coroutine_def_id) {
        build_adrop_for_adrop_shim(tcx, proxy_ref, impl_ty, span, proxy_ref_place, instance)
    } else {
        build_adrop_for_coroutine_shim(tcx, proxy_ref, impl_ty, span, proxy_ref_place, instance)
    }
}
...
fn build_adrop_for_coroutine_shim<'tcx>(
    ...
    let cor_ptr = Ty::new_mut_ptr(tcx, impl_ty);
    ...
    let cor_ptr_local = body.local_decls.push(LocalDecl::new(cor_ptr, span));
    let cor_ref_local = body.local_decls.push(LocalDecl::new(cor_ref, span));
    let impl_ptr_place = Place::from(proxy_ref_local).project_deeper(
        &[
            PlaceElem::Deref,
            PlaceElem::Downcast(None, VariantIdx::ZERO),
            PlaceElem::Field(FieldIdx::ZERO, cor_ptr),
        ],
        tcx,
    );
...

    {
        ...
        // 0: _proxy = _1.0 : Pin<&ProxyLayout> ==> &ProxyLayout
...
fn build_adrop_for_adrop_shim<'tcx>(
    ...
    let cor_ptr_local = locals.push(LocalDecl::new(cor_ptr, span));
    let cor_ptr_place = Place::from(cor_ptr_local);
    ...
    let reborrow = Rvalue::Ref(
        tcx.lifetimes.re_erased,
        BorrowKind::Mut { kind: MutBorrowKind::Default },
        tcx.mk_place_deref(cor_ptr_place),
    );
    let cor_ref_place = Place::from(locals.push(LocalDecl::new(cor_ref, span)));
    let assign3 =
        Statement { source_info, kind: StatementKind::Assign(Box::new((cor_ref_place, reborrow))) };
    ...
    blocks.push(BasicBlockData {
        statements: vec![assign1, assign2, assign3],
