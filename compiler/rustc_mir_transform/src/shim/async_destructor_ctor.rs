...
pub(super) fn build_future_drop_poll_shim<'tcx>(
    ...
    let impl_ptr_place = |proxy_ref_local: Local| {
    ...
    if tcx.is_async_drop_in_place_poll_fn(*coroutine_def_id) {
        ...
        let assign2 = Statement {
            ...
            kind: StatementKind::Assign(Box::new((
                cor_ptr_place,
                Rvalue::CopyForDeref(impl_ptr_place(proxy_ref_local)),
            ))),
        };
    ...
    assert!(!tcx.is_async_drop_in_place_poll_fn(*coroutine_def_id));
