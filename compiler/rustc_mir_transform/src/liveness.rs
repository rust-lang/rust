...
pub fn check_liveness<'tcx>(tcx: TyCtxt<'tcx>, def_id: LocalDefId) -> BitSet<FieldIdx> {
    ...
    let (capture_kind, num_captures) = if tcx.is_closure(def_id.to_def_id()) {
        ...
        let (capture_kind, substs) = match self_ty.kind() {
            ty::Closure(_, substs) => {
                (CaptureKind::Closure(substs.as_closure().kind()), ty::UpvarSubsts::Closure(substs))
            }
            &ty::Generator(_, substs, _) => {
                (CaptureKind::Generator, ty::UpvarSubsts::Generator(substs))
            }
            _ => bug!("expected closure or generator, found {:?}", self_ty),
        };

        let captures = tcx.closure_captures(def_id);
        checked_places.insert_captures(tcx, self_is_ref, captures, substs.upvar_tys());
        ...
        (capture_kind, substs.upvar_tys().count())
    } else {
        (CaptureKind::None, 0)
    };
    debug!(?capture_kind);
        ...
        if statements.is_empty() {
            tcx.emit_spanned_lint(
                ...
                errors::UnusedVariable {
                    ...
                    sugg: errors::UnusedVariableSugg::TryPrefix {
                        spans: vec![def_span],
                        name: name.clone(),
                    },
                },
    ...
    for (index, statements) in assignments.into_iter_enumerated() {
        ...
        for (source_info, (live, kind)) in statements.into_iter().rev() {
            ...
            match kind {
                AccessKind::Assign => tcx.emit_spanned_lint(
                    lint::builtin::UNUSED_ASSIGNMENTS,
                    hir_id,
                    source_info.span,
                    errors::UnusedAssign { name: name.clone() },
                ),
                AccessKind::Param => tcx.emit_spanned_lint(
                    lint::builtin::UNUSED_ASSIGNMENTS,
                    hir_id,
                    source_info.span,
                    errors::UnusedAssignPassed { name: name.clone() },
                ),
                AccessKind::Capture => tcx.emit_spanned_lint(
                    lint::builtin::UNUSED_ASSIGNMENTS,
                    hir_id,
                    decl_span,
...
fn maybe_suggest_literal_matching_name(
    ...
    impl<'tcx> Visitor<'tcx> for LiteralFinder<'_, 'tcx> {
        fn visit_constant(&mut self, constant: &Constant<'tcx>, loc: Location) {
            if let ty::Ref(_, ref_ty,_ ) = constant.ty().kind() && ref_ty.kind() == &ty::Str {
                let rendered_constant = constant.literal.to_string();
            ...
            match rvalue {
                ...
                Rvalue::BinaryOp(op, box (Operand::Copy(lhs), _)) => {
                    ...
                    if let BinOp::Div | BinOp::Rem = op
                        ...
                        && let TerminatorKind::Assert { msg, .. } = &body.basic_blocks[pred].terminator().kind
                ...
                Rvalue::CheckedBinaryOp(_, box (Operand::Copy(lhs), _)) => {
                    ...
                    let TerminatorKind::Assert { cond, target, msg: box AssertKind::Overflow(..), .. }
                        = &bb_data.terminator().kind
                    else { continue };
                    let Some(assign) = body.basic_blocks[*target].statements.first() else { continue };
                    let StatementKind::Assign(box (dest, Rvalue::Use(Operand::Move(temp)))) = assign.kind else { continue };
                    ...
                    let [PlaceElem::Field(FIELD_0, _)] = &temp.projection.as_slice() else { continue };
                    let [PlaceElem::Field(FIELD_1, _)] = &cond.projection.as_slice() else { continue };
...
impl<'tcx> PlaceSet<'tcx> {
    ...
    fn insert_captures(
        ...
        upvars: impl Iterator<Item = Ty<'tcx>>,
    ) {
    ...
    fn get(&self, place: PlaceRef<'tcx>) -> Option<(PlaceIndex, &'tcx [PlaceElem<'tcx>])> {
        ...
        if place.local == ty::CAPTURE_STRUCT_LOCAL && !self.captures.is_empty()
    ...
    T: GenKill<PlaceIndex>,
{
    ...
    fn visit_rvalue(&mut self, rvalue: &Rvalue<'tcx>, location: Location) {
        match rvalue {
            ...
            Rvalue::Aggregate(
                box AggregateKind::Closure(def_id, _) | box AggregateKind::Generator(def_id, _, _),
                ref operands,
            ) => {
