    ...
    fn visit_place(&mut self, place: &mut Place<'tcx>, context: PlaceContext, location: Location) {
        if place.local == SELF_ARG {
            replace_base(
                ...
                Place {
                    local: SELF_ARG,
                    projection: self.tcx().mk().place_elems(&[ProjectionElem::Deref]),
                },
                self.tcx,
            );
        } else {
            ...
            replace_base(
                ...
                Place {
                    ...
                    projection: self
                        .tcx()
                        .mk()
                        .place_elems(&[ProjectionElem::Field(Field::new(0), self.ref_gen_ty)]),
                },
                self.tcx,
            );
        } else {
...
fn replace_base<'tcx>(place: &mut Place<'tcx>, new_base: Place<'tcx>, tcx: TyCtxt<'tcx>) {
    ...
    place.projection = tcx.mk().place_elems(&new_projection);
}
...
impl<'tcx> TransformVisitor<'tcx> {
    ...
    fn make_field(&self, variant_index: VariantIdx, idx: usize, ty: Ty<'tcx>) -> Place<'tcx> {
        ...
        Place { local: base.local, projection: self.tcx.mk().place_elems(&projection) }
    }
...
fn make_generator_state_argument_pinned<'tcx>(tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
    ...
    let substs = tcx.mk().substs(&[ref_gen_ty.into()]);
...
impl<'tcx> MirPass<'tcx> for StateTransform {
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let (state_adt_ref, state_substs) = if is_async_kind {
            ...
            let poll_substs = tcx.mk().substs(&[body.return_ty().into()]);
            (poll_adt_ref, poll_substs)
        } else {
            // Compute GeneratorState<yield_ty, return_ty>
            let state_did = tcx.require_lang_item(LangItem::GeneratorState, None);
            let state_adt_ref = tcx.adt_def(state_did);
            let state_substs = tcx.mk().substs(&[yield_ty.into(), body.return_ty().into()]);
