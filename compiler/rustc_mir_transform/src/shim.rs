...
fn make_shim<'tcx>(tcx: TyCtxt<'tcx>, instance: ty::InstanceKind<'tcx>) -> Body<'tcx> {
    ...
    let mut result = match instance {
        ...
        ty::InstanceKind::FnPtrAsPtrShim(def_id, ty) => build_fn_ptr_as_ptr_shim(tcx, def_id, ty),
        ty::InstanceKind::FnPtrFromPtrShim(def_id, ty) => {
            build_fn_ptr_from_ptr_shim(tcx, def_id, ty)
        }
...
fn build_fn_ptr_as_ptr_shim<'tcx>(
    ...
    let source = MirSource::from_instance(ty::InstanceKind::FnPtrAsPtrShim(def_id, self_ty));
...
fn build_fn_ptr_from_ptr_shim<'tcx>(
    ...
    let source = MirSource::from_instance(ty::InstanceKind::FnPtrFromPtrShim(def_id, self_ty));
    new_body(source, IndexVec::from_elem_n(start_block, 1), locals, sig.inputs().len(), span)
}
