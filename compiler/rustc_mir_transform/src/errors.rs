...
struct InlineWithTarget<'a, 'b, 'tcx> {
    tcx: TyCtxt<'tcx>,
    callee_def_id: DefId,
    caller_def_id: DefId,
    callee_only: &'a [&'b str],
...
impl<'a, 'b, 'c, 'tcx> rustc_errors::Diagnostic<'a, ()> for InlineWithTarget<'b, 'c, 'tcx> {
    fn into_diag(
        self,
        dcx: rustc_errors::DiagCtxtHandle<'a>,
        level: rustc_errors::Level,
    ) -> rustc_errors::Diag<'a, ()> {
        let Self { tcx, callee_def_id, caller_def_id, callee_only } = self;
        ...
        diag.note("function will not be inlined");

        diag.note(format!(
            "the following target features are on `{callee}` but missing from `{caller}`: {}",
            callee_only.join(", ")
        ));
        diag.span_note(callee_def_id.default_span(tcx), format!("`{callee}` is defined here"));
...
pub(crate) fn emit_inline_always_target_feature_diagnostic<'a, 'tcx>(
    ...
    tcx.node_span_lint(
        ...
        call_span,
        InlineWithTarget { tcx, callee_def_id, caller_def_id, callee_only },
    );
}
...
impl<'a, G: EmissionGuarantee, P: std::fmt::Debug> Diagnostic<'a, G> for AssertLint<P> {
...
impl<'a, G: EmissionGuarantee> Diagnostic<'a, G> for MustNotSupend<'_, '_> {
    fn into_diag(self, dcx: DiagCtxtHandle<'a>, level: Level) -> Diag<'a, G> {
        let mut diag = Diag::new(
            dcx,
            level,
        ...
        .with_span_label(self.yield_sp, msg!("the value is held across this suspend point"));
