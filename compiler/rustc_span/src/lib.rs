...
impl Span {
    ...
    fn try_metavars(a: SpanData, b: SpanData, a_orig: Span, b_orig: Span) -> (SpanData, SpanData) {
        // `macro_rules` expansion fills the metavar span table with parentless spans as keys,
        // but they may get parents later and we need to remove them before the table lookup.
