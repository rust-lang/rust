...
pub enum FileNameEmbeddablePreference {
    /// If a remapped path is avalaible, only embed the `virtual_path` and omit the `local_path`.
    ///
    /// Otherwise embed the local-path into the `virtual_path`.
    RemappedOnly,
    /// Embed the original path as well as it's remapped `virtual_path` component if avalaible.
    LocalAndRemapped,
}
