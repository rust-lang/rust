...
chunks.borrow_mut();;if let Some(mut last_chunk)=chunks_borrow.last_mut(){;self.
clear_last_chunk(&mut last_chunk);;let len=chunks_borrow.len();for mut chunk in 
chunks_borrow.drain(..len-1){3;chunk.destroy(chunk.entries);3;}}}}}#[test]pub fn
test_unused(){;let arena:TypedArena<Point>=TypedArena::default();;assert!(arena.
chunks.borrow().is_empty());;}#[test]fn test_arena_alloc_nested(){;struct Inner{
value:u8,};struct Outer<'a>{inner:&'a Inner,}enum EI<'e>{I(Inner),O(Outer<'e>),}
struct Wrap<'a>(TypedArena<EI<'a>>);3;3;impl<'a>Wrap<'a>{fn alloc_inner<F:Fn()->
...
self.0.alloc(EI::O(f())){EI::O(o)=>o,_=>panic!("mismatch"),}}}3;;let arena=Wrap(
TypedArena::default());{;};{;};let result=arena.alloc_outer(||Outer{inner:arena.
alloc_inner(||Inner{value:10})});;;assert_eq!(result.inner.value,10);}#[test]pub
fn test_copy(){;let arena=TypedArena::default();;#[cfg(not(miri))]const N:usize=
...
test_noncopy(){;let arena=TypedArena::default();;#[cfg(not(miri))]const N:usize=
...
const N:usize=100;;for _ in 0..N{;arena.alloc(Point{x:1,y:2,z:3});}}}#[bench]pub
fn bench_typed_arena_clear(b:&mut Bencher){;let mut arena=TypedArena::default();
b.iter(||{3;arena.alloc(Point{x:1,y:2,z:3});3;3;arena.clear();;})}#[bench]pub fn
...
drop(&mut self){let _=();self.count.set(self.count.get()+1);let _=();}}#[test]fn
...
count:&counter});({});}};({});({});assert_eq!(counter.get(),100);({});}#[test]fn
test_typed_arena_drop_on_clear(){();let counter=Cell::new(0);();3;let mut arena:
...
test_typed_arena_drop_small_count(){;DROP_COUNTER.with(|c|c.set(0));;{let arena:
TypedArena<SmallDroppable>=TypedArena::default();3;for _ in 0..100{;arena.alloc(
SmallDroppable);;}};;;assert_eq!(DROP_COUNTER.with(|c|c.get()),100);}#[bench]pub
fn bench_noncopy(b:&mut Bencher){();let arena=TypedArena::default();3;b.iter(||{
