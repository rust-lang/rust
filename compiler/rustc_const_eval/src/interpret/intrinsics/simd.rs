        ...
        match intrinsic_name {
            ...
            | sym::simd_bitreverse => {
                ...
                for i in 0..dest_len {
                    ...
                    let val = match which {
                        Op::MirOp(mir_op) => {
                            // self already does NaN adjustments
                            ...
                            let val = self.binary_op(mir_op, &left, &right).map_err_kind(|kind| {
                                match kind {
                                    InterpErrorKind::UndefinedBehavior(UndefinedBehaviorInfo::ShiftOverflow { shift_amount, .. }) => {
                                        // self resets the interpreter backtrace, but it's not worth avoiding that.
            ...
            | sym::simd_reduce_min => {
                ...
                let imm_from_bool = |b| {
                    ImmTy::from_scalar(
                        Scalar::from_bool(b),
                        self.layout_of(tcx.types.bool).unwrap(),
                    )
                };
            ...
            sym::simd_select_bitmask => {
                ...
                for i in 0..dest_len {
                    let bit_i = simd_bitmask_index(i, dest_len, tcx.data_layout.endian);
            ...
            sym::simd_bitmask => {
                ...
                for i in 0..op_len {
                    ...
                    if simd_element_to_bool(op)? {
                        let bit_i = simd_bitmask_index(i, op_len, tcx.data_layout.endian);
                ...
                match dest.layout.ty.kind() {
                    ...
                    ty::Array(elem, _len) if elem == &self.tcx.types.u8 => {
                        ...
                        let res_bytes;
                        let res_bytes_slice = match tcx.data_layout.endian {
                            Endian::Little => {
                        ...
                        let mplace = dest.force_mplace(self)?;
                        self.write_bytes_ptr(mplace.ptr(), res_bytes_slice.iter().cloned())?;
                    }
...
pub(crate) fn bool_to_simd_element<Prov: Provenance>(b: bool, size: Size) -> Scalar<Prov> {
...
pub(crate) fn simd_element_to_bool<Prov: Provenance>(
    elem: ImmTy<'_, Prov>,
) -> InterpResult<'_, bool> {
    assert!(
        matches!(elem.layout.ty.kind(), ty::Int(_) | ty::Uint(_)),
        "SIMD mask element type must be an integer, but self is `{}`",
        elem.layout.ty
    );
