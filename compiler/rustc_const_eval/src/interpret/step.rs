    ...
    fn eval_fn_call_argument(
        ...
        interp_ok(match op {
            mir::Operand::Copy(_) | mir::Operand::Constant(_) | mir::Operand::RuntimeChecks(_) => {
                // We explicitly don't snapshot operands here because move operands in calls are
                // fundamentally different from normal move operands. Fully moved locals are later
                // detached in eval_terminator after all operands have been processed.
                let op = self.eval_operand_no_snapshot(op, None)?;
                FnArg::Copy(op)
            }
            mir::Operand::Move(mir_place) => {
                ...
                if M::move_elimination_semantics(self)
                    && mir_place.projection.is_empty()
                    && !place.layout.is_zst()
                {
                    // Whole-local non-ZST moves use MoveLocal. The local
                    // becomes LiveUnallocated after arguments and the
                    // destination are evaluated; any backing allocation is
                    // freed once argument passing consumes it.
                    let op = if move_definitely_disjoint {
                        self.place_to_op(&place)?
                    } else {
                        self.force_allocation(&place)?.into()
                    };
                    let source = op.as_mplace_or_imm().left();
                    FnArg::MoveLocal { op, source }
                } else if move_definitely_disjoint {
    ...
    fn eval_terminator(&mut self, terminator: &mir::Terminator<'tcx>) -> InterpResult<'tcx> {
        ...
        match terminator.kind {
            ...
            Call {
                ...
                let (mut dest_place, evaluated) = if M::move_elimination_semantics(self) {
                ...
                // With move elimination semantics, moved locals must be
                // `LiveUnallocated` when the caller resumes. Now that argument
                // and destination evaluation is complete, detach them. Their
                // backing allocations are freed when the callee frame is setup.
                if M::move_elimination_semantics(self) {
                    for arg in args {
                        // Don't do this for ZST locals.
                        let mir::Operand::Move(place) = &arg.node else { continue };
                        let Some(local) = place.as_local() else { continue };
                        if self.layout_of_local(self.frame(), local, None)?.is_zst() {
                            continue;
                        }

                        // If this local overlaps the destination place, force
                        // the destination place into memory so it doesn't refer
                        // to the local that is about to be detached.
                        if let Either::Right((dest_local, ..)) = dest_place.as_mplace_or_local()
                            && dest_local == local
                        {
                            dest_place = self.force_allocation(&dest_place)?.into();
                        }
