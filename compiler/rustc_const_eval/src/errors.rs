    ...
    fn into_diagnostic(self, handler: &'_ Handler) -> DiagnosticBuilder<'_, ErrorGuaranteed> {
        ...
        let mut builder = match self.0.kind {
            PtrToUninhabited { ptr_kind: PointerKind::Box, ty } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_box_to_uninhabited);
            ...
            PtrToUninhabited { ptr_kind: PointerKind::Ref, ty } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_ref_to_uninhabited);
            ...
            MutableRefInConst => handler.struct_diagnostic(const_eval_validation_mutable_ref_in_const),
            ...
            NullablePtrOutOfRange { range, max_value } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_nullable_ptr_out_of_range);
            ...
            InvalidVTablePtr { value } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_invalid_vtable_ptr);
            ...
            DanglingPtrNoProvenance { ptr_kind: PointerKind::Box, pointer } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_dangling_box_no_provenance);
            ...
            DanglingPtrNoProvenance { ptr_kind: PointerKind::Ref, pointer } => {
                let mut builder = handler.struct_diagnostic(const_eval_validation_dangling_ref_no_provenance);
