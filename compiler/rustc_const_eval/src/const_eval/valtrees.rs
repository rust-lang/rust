...
fn branches<'tcx>(
    ...
    for i in 0..field_count {
        ...
        let valtree = _const_to_valtree_inner(ecx, &field, num_nodes, visited, settled)?;
...
fn slice_branches<'tcx>(
    ...
    for i in 0..n {
        ...
        let valtree = _const_to_valtree_inner(ecx, &place_elem, num_nodes, visited, settled)?;
...
fn const_to_valtree_inner<'tcx>(
    ...
    _const_to_valtree_inner(ecx, place, num_nodes, &mut visited, &mut settled)
}

#[instrument(skip(ecx, visited, settled), level = "debug")]
fn _const_to_valtree_inner<'tcx>(
    ecx: &CompileTimeInterpCx<'tcx>,
    place: &MPlaceTy<'tcx>,
    num_nodes: &mut usize,
    visited: &mut FxHashSet<MPlaceTy<'tcx>>,
    ...
    let result = match ty.kind() {
        ...
        ty::Pat(base, ..) => {
            ...
            ensure_sufficient_stack(|| {
                _const_to_valtree_inner(ecx, &place, num_nodes, visited, settled)
            })
        }
        ...
        ty::Ref(_, _, _) => {
            ...
            _const_to_valtree_inner(ecx, &derefd_place, num_nodes, visited, settled)
        }
