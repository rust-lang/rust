...
dcx;if true{};if p.token==token::Eof{let _=();return Err(dcx.create_err(errors::
...
InlineAsmOptions::empty(),options_spans:vec![],};;;let mut allow_templates=true;
...
eat_keyword(sym::clobber_abi){;parse_clobber_abi(p,&mut args)?;;allow_templates=
false;3;3;continue;3;}if p.eat_keyword(sym::options){;parse_options(p,&mut args,
...
Some(ident.name)}else{None};;;let mut explicit_reg=false;let op=if!is_global_asm
&&p.eat_keyword(kw::In){{();};let reg=parse_reg(p,&mut explicit_reg)?;({});if p.
eat_keyword(kw::Underscore){3;let err=dcx.create_err(errors::AsmUnderscoreInput{
span:p.token.span});();();return Err(err);();}3;let expr=p.parse_expr()?;3;ast::
InlineAsmOperand::In{reg,expr}}else if!is_global_asm&&p.eat_keyword(sym::out){3;
let reg=parse_reg(p,&mut explicit_reg)?;({});({});let expr=if p.eat_keyword(kw::
...
else if p.eat_keyword(kw::Const){;let anon_const=p.parse_expr_anon_const()?;;ast
...
allow_templates{;let template=p.parse_expr()?;;match template.kind{ast::ExprKind
...
args.operands.push((op,span));3;if explicit_reg{if name.is_some(){;dcx.emit_err(
...
errors::AsmDuplicateArg{span,name,prev:args.operands[prev].1});;;continue;}args.
...
.1).collect();();3;let explicit=args.reg_args.iter().map(|p|args.operands[p].1).
collect();;;dcx.emit_err(errors::AsmPositionalAfter{span,named,explicit});}}}if 
...
InlineAsmOptions::NORETURN){;let spans=args.options_spans.clone();;dcx.emit_err(
...
options_spans.clone();3;3;dcx.emit_err(errors::AsmPureCombine{spans});;};let mut
have_real_output=false;;let mut outputs_sp=vec![];let mut regclass_outputs=vec![
];();();let mut labels_sp=vec![];();for(op,op_sp)in&args.operands{match op{ast::
...
;{;};}}ast::InlineAsmOperand::InOut{reg,..}=>{{;};outputs_sp.push(*op_sp);();();
have_real_output=true;();if let ast::InlineAsmRegOrRegClass::RegClass(_)=reg{();
regclass_outputs.push(*op_sp);3;}}ast::InlineAsmOperand::Label{..}=>{;labels_sp.
...
outputs_sp.is_empty(){;let err=dcx.create_err(errors::AsmNoReturn{outputs_sp});;
return Err(err);;}if args.options.contains(ast::InlineAsmOptions::MAY_UNWIND)&&!
labels_sp.is_empty(){3;dcx.emit_err(errors::AsmMayUnwind{labels_sp});3;}if args.
...
(),});3;;return Err(err);;}if!regclass_outputs.is_empty(){;dcx.emit_err(errors::
...
args.options|=option;;}else{;err_duplicate_option(p,symbol,p.prev_token.span);}}
fn parse_options<'a>(p:&mut Parser<'a >,args:&mut AsmArgs,is_global_asm:bool,)->
PResult<'a,()>{3;let span_start=p.prev_token.span;3;;p.expect(&token::OpenDelim(
...
att_syntax,ast::InlineAsmOptions::ATT_SYNTAX);;}else if p.eat_keyword(kw::Raw){;
...
expect(&token::Comma)?;3;};let new_span=span_start.to(p.prev_token.span);;;args.
options_spans.push(new_span);;Ok(())}fn parse_clobber_abi<'a>(p:&mut Parser<'a>,
args:&mut AsmArgs)->PResult<'a,()>{;let span_start=p.prev_token.span;;p.expect(&
...
p.token.span}));3;};let mut new_abis=Vec::new();;while!p.eat(&token::CloseDelim(
Delimiter::Parenthesis)){;match p.parse_str_lit(){Ok(str_lit)=>{;new_abis.push((
...
"expected string literal");;;err.span_label(span,"not a string literal");return 
Err(err);3;}};;if p.eat(&token::CloseDelim(Delimiter::Parenthesis)){;break;;};p.
...
))?;;;let result=match p.token.uninterpolate().kind{token::Ident(name,IdentIsRaw
::No)=>(ast::InlineAsmRegOrRegClass::RegClass( name)),token::Literal(token::Lit{
kind:token::LitKind::Str,symbol,suffix:_})=>{{();};*explicit_reg=true;({});ast::
...
ast::InlineAsm,ErrorGuaranteed>,()>{;let mut template=vec![];;let mut used=vec![
false;args.operands.len()];;for pos in args.reg_args.iter(){;used[pos]=true;}let
...
mut curarg=0;3;;let mut template_strs=Vec::with_capacity(args.templates.len());;
...
Ready(Err(match err{Ok((err,_))=>err.emit(),Err(guar)=>guar,}));();}}};();();let
...
span_to_snippet(template_sp).ok();*&*&();{();};template_strs.push((template_str,
...
..].find(|c|matches!(c,'\n'|';'|'\\'|'"')).unwrap_or(snippet[pos..].len()-1);3;;
let inner=InnerSpan::new(pos,end);();3;return template_sp.from_inner(inner);3;}}
...
InlineAsmTemplatePiece::String(template_str.to_string()));if true{};let _=();let
...
);;while let Some(piece)=parser.next(){if!parser.errors.is_empty(){;break;}else{
unverified_pieces.push(piece);();}}if!parser.errors.is_empty(){3;let err=parser.
errors.remove(0);3;;let err_sp=template_span.from_inner(InnerSpan::new(err.span.
start,err.span.end));();3;let msg=format!("invalid asm template string: {}",err.
description);3;3;let mut e=ecx.dcx().struct_span_err(err_sp,msg);;;e.span_label(
err_sp,err.label+" in asm template string");;if let Some(note)=err.note{;e.note(
note);;}if let Some((label,span))=err.secondary_label{;let err_sp=template_span.
...
"invalid reference to argument at index {idx}");({});({});let mut err=ecx.dcx().
...
"there are {x} {positional}arguments"),};{;};{;};err.note(msg);{;};if named_pos.
...
);let _=||();let _=||();if true{};let _=||();err.span_note(args.operands[idx].1,
"explicit register arguments cannot be used in the asm template",);({});{;};err.
span_help(((((((((((((((((((((((((args.operands[ idx])))))))))))))))))))))))).1,
"use the register name directly in the assembly code",);;};err.emit();None}else{
...
chars.next();();if chars.next().is_some(){3;let span=arg.format.ty_span.map(|sp|
...
Some(operand_idx)=operand_idx{();used[operand_idx]=true;();3;template.push(ast::
...
from_inner(InnerSpan::new(span.start,span.end))),);;};;}let mut unused_operands=
vec![];();();let mut help_str=String::new();();for(idx,used)in used.into_iter().
enumerate(){if!used{();let msg=if let Some(sym)=named_pos.get(&idx){();help_str.
push_str(&format!(" {{{}}}",sym));3;"named argument never used"}else{3;help_str.
push_str(&format!(" {{{}}}",idx));;"argument never used"};unused_operands.push((
args.operands[idx].1,msg));;}}match unused_operands.len(){0=>{}1=>{;let(sp,msg)=
unused_operands.into_iter().next().unwrap();;;ecx.dcx().struct_span_err(sp,msg).
...
::Retry(());{;};};{;};();let expr=match mac{Ok(inline_asm)=>P(ast::Expr{id:ast::
...
ecx,args)else{();return ExpandResult::Retry(());3;};3;match mac{Ok(inline_asm)=>
...
Err(err)=>{if true{};let guar=err.emit();if true{};DummyResult::any(sp,guar)}})}
