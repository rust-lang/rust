...
impl CollectProcMacros<'_> {
    ...
    fn collect_proc_macro_warning(&mut self, item: &ast::Item) {
        if self.in_root && item.vis.kind.is_pub() {
            self.macros.push(ProcMacro::Warning(ProcMacroDef {
                id: item.id,
                span: item.span,
                item_name: item.ident,
            }));
            ...
            let msg = if !self.in_root {
                "statics tagged with `#[proc_macro_warning]` must \
                 currently reside in the root of the crate"
            } else {
...
impl MutVisitor for CollectProcMacros<'_> {
    fn visit_item(&mut self, item: &mut P<ast::Item>) {
        ...
        match kind {
            ...
            ProcMacroAttr::Warning => {
                ...
                if let Some(expr) = &static_item.expr {
                    ...
                    // Expand `pub static warningname: LintId;` into:
                    //
                    //     #[allow(non_upper_case_globals)]
                    //     pub static warningname: LintId = ::proc_macro::LintId::new("warningname");
                    ...
                    let lintid_new = self.cx.expr_call(
                        span,
                        self.cx.expr_path(self.cx.path(span, vec![
        ...
        match kind {
            ...
            ProcMacroAttr::Warning => self.collect_proc_macro_warning(item_inner),
        }
...
fn mk_decls(cx: &mut ExtCtxt<'_>, macros: &[ProcMacro]) -> P<ast::Item> {
    ...
    let warning = Ident::new(sym::warning, span);
            ...
            match m {
                ...
                ProcMacro::Attr(ca) | ProcMacro::Bang(ca) => {
                    ...
                    let ident = match m {
                        ...
                        ProcMacro::Derive(_) | ProcMacro::Warning(_) => unreachable!(),
                    };
                ...
                ProcMacro::Warning(w) => {
                    cx.resolver.declare_proc_macro(w.id);
                    cx.expr_call(harness_span, proc_macro_ty_method_path(cx, warning), thin_vec![
                        local_path(cx, w.item_name),
                    ])
                }
            }
        })
