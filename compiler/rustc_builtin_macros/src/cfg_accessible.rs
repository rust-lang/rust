...
MultiplePaths(l.span()));;}Some([nmi])=>match nmi.meta_item(){None=>{;ecx.dcx().
emit_err(LiteralPath(nmi.span()));{;};}Some(mi)=>{if!mi.is_word(){{;};ecx.dcx().
emit_err(HasArguments(mi.span));{;};}{;};return Some(&mi.path);{;};}},}None}impl
...
,..Default::default()};;;validate_attr::check_builtin_meta_item(&ecx.sess.psess,
meta_item,ast::AttrStyle::Outer,sym::cfg_accessible,template,);;;let Some(path)=
validate_input(ecx,meta_item)else{3;return ExpandResult::Ready(Vec::new());3;};;
