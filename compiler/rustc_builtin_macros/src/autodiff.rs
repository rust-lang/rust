...
mod llvm_enzyme {
    ...
    use rustc_ast::{
        ...
        FnRetTy, FnSig, GenericArg, Generics, ItemKind, MetaItemInner, PatKind, Path, QSelf,
        TyKind,
    };
    ...
    pub(crate) fn from_ast(
        ...
        let width = if meta_item.len() >= 3 && let Some(x) = width(&meta_item[2]) {
            first_activity = 3;
            match x.try_into() {
                Ok(x) => x,
                Err(_) => {
    ...
    pub(crate) fn expand(
        ...
        let symbol;
        if meta_item_vec.len() >= 3 && let Some(width) = width(&meta_item_vec[2]) {
            start_position = 3;
    ...
    fn gen_enzyme_body(
        ...
        let d_has_ret = has_ret(&d_sig.decl.output);
