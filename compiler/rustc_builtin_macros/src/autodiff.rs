...
// If we don't have a return type (or explicitley return -> ()),
...
pub(crate) fn from_ast(
    ...
    for x in &meta_item[2..] {
        ...
        match res {
            ...
            Err(_) => {
                dcx.emit_err(errors::AutoDiffUnknownActivity { span: x.span(), act: activity_str });
                errors = true;
            }
        };
    }
    ...
    let (ret_activity, input_activity) = if has_ret {
        match activities.split_last() {
            Some((last, rest)) => (last, rest),
            None => {
                // This should never be reached, since we count the number of activities before.
                dcx.emit_err(errors::AutoDiffInvalidApplication { span: meta_item[0].span() });
                return AutoDiffAttrs::error();
            }
        }
    } else {
...
pub(crate) fn expand(
    ...
    let d_annotatable = if is_impl {
        ...
        let mut d_fn =
            ecx.item(span, d_ident, thin_vec![d_attr.clone(), inline_never], ItemKind::Fn(asdf));
        d_fn.vis = vis;
...
fn gen_enzyme_body(
    ...
    match &exprs[..] {
        ...
        [arg] => {
            ret = ecx.expr_call(new_decl_span, blackbox_call_expr.clone(), thin_vec![arg.clone()]);
        }
        args => {
