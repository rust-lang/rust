...
pub(crate) fn expand_deriving_reborrow(
    ...
    // The simple form is `fn reborrow(self) -> Self { self }`, possibly with
    // some additional `AssertParamIsReborrow` assertions.
    //
    ...
    //   with Reborrow + Copy, and then there'd be no Reborrow impl at all if the
    //   user fills in something that is Reborrow but not Copy. After
    ...
    //   Reborrow because they require Copy for deriving, Reborrow alone is not
    //   enough. Whether Reborrow is implemented for fields is irrelevant so we
    //   don't assert it.)
    let bounds;
    let is_simple;
    match item {
        Annotatable::Item(annitem) => match &annitem.kind {
            ...
            | ItemKind::Enum(_, Generics { params, .. }, _) => {
                        ...
                        .any(|param| matches!(param.kind, ast::GenericParamKind::Type { .. }))
                {
                    ...
                    //     cs_reborrow_simple("Reborrow", c, s, sub, false)
                    // }));
                } else {
                    bounds = vec![];
                    is_simple = false;
                    // substructure =
                    //     combine_substructure(Box::new(|c, s, sub| cs_reborrow("Reborrow", c, s, sub)));
                }
            }
            ItemKind::Union(..) => {
                ...
                is_simple = true;
                // substructure = combine_substructure(Box::new(|c, s, sub| {
                //     cs_reborrow_simple("Reborrow", c, s, sub, true)
                // }));
            }
            _ => cx.dcx().span_bug(span, "`#[derive(Reborrow)]` on wrong item kind"),
        },
        _ => cx.dcx().span_bug(span, "`#[derive(Reborrow)]` on trait item or impl item"),
    }

    let trait_def = TraitDef {
        span,
        path: path_std!(marker::Reborrow),
...
fn _cs_reborrow_simple(
    ...
    let mut process_variant = |variant: &VariantData| {
        for field in variant.fields() {
            ...
            // assertions like `AssertParamIsReborrow<Foo>` where the type is a
                ...
                && !seen_type_names.insert(name)
            {
                ...
                // let _: AssertParamIsReborrow<FieldTy>;
                super::assert_ty_bounds(
                    cx,
                    &mut stmts,
                    field.ty.clone(),
                    field.span,
                    &[sym::reborrow, sym::AssertParamIsCopy],
                );
            }
        }
    };
...
fn _cs_reborrow(
    ...
    let ctor_path;
    let all_fields;
    let fn_path = cx.std_path(&[sym::reborrow, sym::Reborrow, sym::reborrow]);
