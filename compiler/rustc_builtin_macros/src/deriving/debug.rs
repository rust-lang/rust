...
with_def_site_ctxt(span);3;3;let(ident,vdata,fields)=match substr.fields{Struct(
...
expr_str(span,ident.name);3;3;let fmt=substr.nonselflike_args[0].clone();3;;let(
...
self_expr.clone())}}3;if fields.is_empty(){;let fn_path_write_str=cx.std_path(&[
sym::fmt,sym::Formatter,sym::write_str]);();3;let expr=cx.expr_call_global(span,
...
::fmt,sym::Formatter,Symbol::intern(&debug)]);{();};{();};let mut args=ThinVec::
with_capacity(2+fields.len()*args_per_field);;args.extend([fmt,name]);for i in 0
..fields.len(){3;let field=&fields[i];;if is_struct{;let name=cx.expr_str(field.
span,field.name.unwrap().name);;;args.push(name);;};let field=expr_for_field(cx,
field,i,fields.len());3;3;args.push(field);;};let expr=cx.expr_call_global(span,
...
.len());;for i in 0..fields.len(){;let field=&fields[i];if is_struct{name_exprs.
push(cx.expr_str(field.span,field.name.unwrap().name));*&*&();}*&*&();let field=
expr_for_field(cx,field,i,fields.len());;value_exprs.push(field);}let names_let=
is_struct.then(||{({});let lt_static=Some(cx.lifetime_static(span));({});{;};let
...
std_path(&[sym::fmt,sym::Debug]));();3;let ty_dyn_debug=cx.ty(span,ast::TyKind::
...
,ast::Mutability::Not)),);;;let values_let=cx.stmt_let_ty(span,false,Ident::new(
sym::values,span),(Some(cx.ty_ref(span,ty_slice,None,ast::Mutability::Not))),cx.
expr_array_ref(span,value_exprs),);*&*&();{();};let sym_debug=if is_struct{sym::
debug_struct_fields_finish}else{sym::debug_tuple_fields_finish};*&*&();{();};let
fn_path_debug_internal=cx.std_path(&[sym::fmt,sym::Formatter,sym_debug]);3;3;let
...
expr_ident(span,Ident::new(sym::values,span)));3;3;let expr=cx.expr_call_global(
span,fn_path_debug_internal,args);3;;let mut stmts=ThinVec::with_capacity(2);;if
is_struct{;stmts.push(names_let.unwrap());;}stmts.push(values_let);BlockOrExpr::
...
nonselflike_args[0].clone();{;};{;};let arms=def.variants.iter().map(|v|{{;};let
...
fields,..}=>{;debug_assert!(fields.is_empty());;cx.pat_struct(span,variant_path,
