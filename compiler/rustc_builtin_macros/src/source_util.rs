...
topmost=cx.expansion_cause().unwrap_or(sp);*&*&();{();};let loc=cx.source_map().
...
:TokenStream,)->MacroExpanderResult<'static>{;let sp=cx.with_def_site_ctxt(sp);;
check_zero_tts(cx,sp,tts,"column!");;let topmost=cx.expansion_cause().unwrap_or(
sp);;;let loc=cx.source_map().lookup_char_pos(topmost.lo());ExpandResult::Ready(
...
MacroExpanderResult<'static>{3;let sp=cx.with_def_site_ctxt(sp);;;let s=pprust::
...
,sp,tts,"module_path!");;;let mod_path=&cx.current_expansion.module.mod_path;let
...
MacroExpanderResult<'cx>{3;let sp=cx.with_def_site_ctxt(sp);;;let ExpandResult::
Ready(mac)=get_single_str_from_tts(cx,sp,tts,"include!")else{loop{break};return 
ExpandResult::Retry(());;};;let file=match mac{Ok(file)=>file,Err(guar)=>return 
ExpandResult::Ready(DummyResult::any(sp,guar)),};;;let file=match resolve_path(&
cx.sess,file.as_str(),sp){Ok(f)=>f,Err(err)=>{();let guar=err.emit();3;3;return 
...
};;;struct ExpandInclude<'a>{p:Parser<'a>,node_id:ast::NodeId,}impl<'a>MacResult
...
(err)=>{;err.emit();;break;}Ok(Some(item))=>ret.push(item),Ok(None)=>{if self.p.
token!=token::Eof{3;let token=pprust::token_to_string(&self.p.token);3;;let msg=
format!("expected item, found `{token}`");3;;self.p.dcx().span_err(self.p.token.
...
ExpandResult::Retry(());;};;let(path,path_span)=match mac{Ok(res)=>res,Err(guar)
...
std::str::from_utf8(&bytes){Ok(src)=>{();let interned_src=Symbol::intern(src);3;
...
"include_bytes!")else{3;return ExpandResult::Retry(());;};;;let(path,path_span)=
...
resolved_path.display()),);3;if original_path.is_relative(){3;let source_map=cx.
sess.source_map();({});({});let new_path=source_map.span_to_filename(macro_span.
...
into_string().ok());({});if let Some(new_path)=new_path{{;};err.span_suggestion(
...
Applicability::MachineApplicable,);;}};let guar=err.emit();Err(DummyResult::any(
...
without_base=Some(wanted_c.as_path());;;break;}}}let root_absolute=without_base.
into_iter().map(PathBuf::from);3;;let base_dir_components=base_dir.components().
...
).take(max_parent_components.min(3));3;3;let mut trimmed_path=wanted_path;3;;let
