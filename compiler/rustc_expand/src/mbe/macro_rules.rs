...
pub struct MacroRulesMacroExpander {
    ...
    name: Ident,
    span: Span,
    on_incomplete_macro_args: Option<Directive>,
        ...
        match try_match_macro_derive(psess, name, body, rules, &mut NoopTracker) {
            ...
            Err(CanRetry::Yes) => {
                let (_, guar) = failed_to_match_macro(
                    ...
                    body,
                    rules,
                    self.on_incomplete_macro_args.as_ref(),
                );
...
#[instrument(skip(cx, transparency, arg, rules, on_incomplete_macro_args))]
...
#[instrument(skip(cx, transparency, args, body, rules, on_incomplete_macro_args))]
...
pub fn compile_declarative_macro(
    ...
    let on_incomplete_macro_args = find_attr!(
        attrs,
        OnIncompleteMacroArgs { directive, .. } => directive.clone()
    )
    .flatten()
