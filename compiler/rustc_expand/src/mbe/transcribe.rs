                ...
                if let Some(cur_matched) = lookup_cur_matched(ident, interp, &repeats) {
                    let tt = match cur_matched {
                        MatchedSingle(ParseNtResult::Tt(tt)) => {
                            ...
                            maybe_use_metavar_location(sess, &stack, sp, tt, &mut marker)
                        }
                        ...
                        MatchedSeq(..) => {
                            ...
                            return Err(sess.dcx.create_err(VarStillRepeating { span: sp, ident }));
                        }
                    };
                    result.push(tt)
                } else {
...
fn maybe_use_metavar_location(
    ...
    if no_collision || sess.source_map().is_imported(metavar_span) {
        return orig_tt.clone();
    }
...
fn count_repetitions<'a>(
    ...
    fn count<'a>(depth_curr: usize, depth_max: usize, matched: &NamedMatch) -> PResult<'a, usize> {        match matched {
...
fn out_of_bounds_err<'a>(dcx: &'a DiagCtxt, max: usize, span: Span, ty: &str) -> Diag<'a> {    let msg = if max == 0 {
        format!(
...
fn transcribe_metavar_expr<'a>(
    ...
    match *expr {
        MetaVarExpr::Count(original_ident, depth) => {
            let matched = matched_from_ident(original_ident, interp, dcx)?;
            let count = count_repetitions(depth, matched, repeats, sp, dcx)?;
        ...
        MetaVarExpr::Ignore(original_ident) => {
            ...
            let _ = matched_from_ident(original_ident, interp, dcx)?;
        }
