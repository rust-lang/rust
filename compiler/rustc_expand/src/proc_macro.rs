...
impl base::BangProcMacro for WasmBangProcMacro {
    fn expand(
        ...
        (self.client)(ecx, ecx.current_expansion.id, input).map_err(|e| {
            ecx.dcx().emit_err(diagnostics::ProcMacroPanicked {
                span,
                message: e.map(|message| diagnostics::ProcMacroPanickedHelp { message }),
            })
        })
    }
}
...
impl base::AttrProcMacro for WasmAttrProcMacro {
    fn expand(
        ...
        (self.client)(ecx, ecx.current_expansion.id, annotation, annotated).map_err(|e| {
            ecx.dcx().emit_err(diagnostics::CustomAttributePanicked {
                span,
                message: e.map(|message| diagnostics::CustomAttributePanickedHelp { message }),
            })
        })
    }
}
...
fn expand_wasm_derive_macro(
    ...
    client(ecx, invoc_id, input).map_err(|e| {
        ...
        ecx.dcx().emit_err({
            diagnostics::ProcMacroDerivePanicked {
                span,
                message: e.map(|message| diagnostics::ProcMacroDerivePanickedHelp { message }),
            }
        });
    })
}
