...
mod generated {
    wasmtime::component::bindgen!({
        ...
        with: {
            ...
            "rust-lang:rust/host/span": rustc_span::Span,
...
impl host::DiagnosticInner {
    fn to_internal(
        ...
        rustc_proc_macro::bridge::Diagnostic {
            ...
            spans: self.spans.into_iter().map(|s| *table.get(&s).unwrap()).collect(),
            children: self
                .children
                .into_iter()
                .map(|c| {
    ...
    fn literal_from_str(&mut self, s: String) -> Result<host::Literal, String> {
        DeriveExpandCtx::with(|(ecx, _)| {
            ...
            Ok(host::Literal {
                ...
                symbol: host::Symbol { s: literal.symbol.to_string() },
                suffix: literal.suffix.map(|s| host::Symbol { s: s.to_string() }),
                span: host::HostSpan::call_site(self),
            })
        })
    }

    fn symbol_normalize_and_validate_ident(&mut self, string: String) -> Result<host::Symbol, ()> {
        DeriveExpandCtx::with(|(ecx, _)| {
            let sym = ecx.symbol_normalize_and_validate_ident(&string)?;
            Ok(host::Symbol { s: sym.to_string() })
        })
    }
...
type SpanResource = wasmtime::component::Resource<Span>;
...
// FIXME: Span may become a type alias for NonZeroU32 (i.e., Handle like on non-wasm).
impl generated::rust_lang::rust::host::HostSpan for Ctx {
    fn is_same(&mut self, a: SpanResource, b: SpanResource) -> bool {
        self.table.get(&a).unwrap() == self.table.get(&b).unwrap()
    }
    ...
    fn subspan(
        ...
        let span = *self.table.get(&a).unwrap();
        ...
        Some(self.table.push(ret).unwrap())
    }

    fn def_site(&mut self) -> SpanResource {
        DeriveExpandCtx::with(|(ecx, _)| self.table.push(ecx.def_site).unwrap())
    }

    fn call_site(&mut self) -> SpanResource {
        DeriveExpandCtx::with(|(ecx, _)| self.table.push(ecx.call_site).unwrap())
    }

    fn mixed_site(&mut self) -> SpanResource {
        DeriveExpandCtx::with(|(ecx, _)| self.table.push(ecx.mixed_site).unwrap())
    }

    fn line(&mut self, sp: SpanResource) -> u64 {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_line(span)).try_into().unwrap()
    }

    fn column(&mut self, sp: SpanResource) -> u64 {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_column(span).try_into().unwrap())
    }

    fn file(&mut self, sp: SpanResource) -> String {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_file(span))
    }

    fn local_file(&mut self, sp: SpanResource) -> Option<String> {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_local_file(span))
    }

    fn start(&mut self, sp: SpanResource) -> SpanResource {
        let span = *self.table.get(&sp).unwrap();
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_start(span));
        self.table.push(ret).unwrap()
    }

    fn end(&mut self, sp: SpanResource) -> SpanResource {
        let span = *self.table.get(&sp).unwrap();
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_end(span));
        self.table.push(ret).unwrap()
    }

    fn join(&mut self, this: SpanResource, other: SpanResource) -> Option<SpanResource> {
        let this = *self.table.get(&this).unwrap();
        let other = *self.table.get(&other).unwrap();

        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_join(this, other))?;
        Some(self.table.push(ret).unwrap())
    }

    fn byte_range_start(&mut self, sp: SpanResource) -> u64 {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_byte_range(span).start as u64)
    }

    fn byte_range_end(&mut self, sp: SpanResource) -> u64 {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_byte_range(span).end as u64)
    }

    fn parent(&mut self, sp: SpanResource) -> Option<SpanResource> {
        let span = *self.table.get(&sp).unwrap();
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_parent(span))?;
        Some(self.table.push(ret).unwrap())
    }

    fn source(&mut self, sp: SpanResource) -> SpanResource {
        let span = *self.table.get(&sp).unwrap();
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_source(span));
        self.table.push(ret).unwrap()
    }

    fn debug(&mut self, sp: SpanResource) -> String {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_debug(span))
    }

    fn source_text(&mut self, sp: SpanResource) -> Option<String> {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_source_text(span))
    }

    fn resolved_at(&mut self, this: SpanResource, at: SpanResource) -> SpanResource {
        let this = *self.table.get(&this).unwrap();
        let at = *self.table.get(&at).unwrap();
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.span_resolved_at(this, at));
        self.table.push(ret).unwrap()
    }

    fn save(&mut self, sp: SpanResource) -> u64 {
        let span = *self.table.get(&sp).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.span_save_span(span)).try_into().unwrap()
    }

    fn recover(&mut self, id: u64) -> SpanResource {
        let span = DeriveExpandCtx::with(|(ecx, _)| ecx.span_recover_proc_macro_span(id as usize));
        self.table.push(span).unwrap()
    }

    fn clone(&mut self, sp: SpanResource) -> SpanResource {
        let span = *self.table.get(&sp).unwrap();
        self.table.push(span).unwrap()
    }

    fn drop(&mut self, r: SpanResource) -> wasmtime::Result<()> {
        self.table.delete(r)?;
        Ok(())
    }
}
...
impl host::DelimSpan {
    fn to_internal(
        self,
        table: &mut wasmtime_wasi::ResourceTable,
    ) -> rustc_proc_macro::bridge::DelimSpan<Span> {
        rustc_proc_macro::bridge::DelimSpan {
            open: table.delete(self.open).unwrap(),
            close: table.delete(self.close).unwrap(),
            entire: table.delete(self.entire).unwrap(),
        }
    }
}
    ...
    fn to_internal(
        ...
        match self {
            host::TokenTree::Group(group) => bridge::TokenTree::Group(bridge::Group {
                ...
                span: group.span.to_internal(table),
            }),
            host::TokenTree::Punct(punct) => bridge::TokenTree::Punct(bridge::Punct {
                ch: punct.ch,
                joint: punct.joint,
                span: table.delete(punct.span).unwrap(),
            }),
            host::TokenTree::Ident(ident) => bridge::TokenTree::Ident(bridge::Ident {
                sym: Symbol::intern(&ident.sym.s),
                is_raw: ident.is_raw,
                span: table.delete(ident.span).unwrap(),
            }),
            host::TokenTree::Literal(literal) => bridge::TokenTree::Literal(bridge::Literal {
                kind: literal.kind.to_internal(),
                symbol: Symbol::intern(&literal.symbol.s),
                suffix: literal.suffix.map(|s| Symbol::intern(&s.s)),
                span: table.delete(literal.span).unwrap(),
            }),
        }
    }
}
...
impl generated::rust_lang::rust::host::HostTokenStream for Ctx {
    ...
    fn from_token_tree(&mut self, tree: host::TokenTree) -> TokenStreamResource {
        let ret = DeriveExpandCtx::with(|(ecx, _)| {
    ...
    fn clone(&mut self, input: TokenStreamResource) -> TokenStreamResource {
        let input = self.table.get(&input).unwrap();
        self.table.push(input.clone()).unwrap()
    }

    fn is_empty(&mut self, input: TokenStreamResource) -> bool {
        let input = self.table.get(&input).unwrap();
        input.is_empty()
    }

    fn expand_expr(&mut self, stream: TokenStreamResource) -> Result<TokenStreamResource, ()> {
    ...
    fn from_str(&mut self, src: String) -> Result<TokenStreamResource, String> {
        let ret = DeriveExpandCtx::with(|(ecx, _)| ecx.ts_from_str(&src))?;
        Ok(self.table.push(ret).unwrap())
    }

    fn to_string(&mut self, input: TokenStreamResource) -> String {
        let input = self.table.get(&input).unwrap();
        DeriveExpandCtx::with(|(ecx, _)| ecx.ts_to_string(&input))
    }

    fn into_trees(&mut self, stream: TokenStreamResource) -> Vec<host::TokenTree> {
        ...
        ret.into_iter()
            .map(|tt| match tt {
                bridge::TokenTree::Group(group) => host::TokenTree::Group(host::Group {
                    ...
                    span: host::DelimSpan {
                        open: self.table.push(group.span.open).unwrap(),
                        close: self.table.push(group.span.close).unwrap(),
                        entire: self.table.push(group.span.entire).unwrap(),
                    },
                }),
                bridge::TokenTree::Punct(punct) => host::TokenTree::Punct(host::Punct {
                    ch: punct.ch,
                    joint: punct.joint,
                    span: self.table.push(punct.span).unwrap(),
                }),
                bridge::TokenTree::Ident(ident) => host::TokenTree::Ident(host::Ident {
                    sym: host::Symbol { s: ident.sym.to_string() },
                    is_raw: ident.is_raw,
                    span: self.table.push(ident.span).unwrap(),
                }),
                bridge::TokenTree::Literal(literal) => host::TokenTree::Literal(host::Literal {
                    kind: literal.kind.to_internal(),
                    symbol: host::Symbol { s: literal.symbol.to_string() },
                    suffix: literal.suffix.map(|s| host::Symbol { s: s.to_string() }),
                    span: self.table.push(literal.span).unwrap(),
                }),
            })
            .collect()
    }
...
pub fn load_wasm_macro(
    ...
    let mut linker = wasmtime::component::Linker::new(&engine);
    wasmtime_wasi::p2::add_to_linker_sync(&mut linker).map_err(WasmLoadError::RuntimeFailed)?;
    generated::ProcMacro::add_to_linker::<_, wasmtime::component::HasSelf<_>>(
        &mut linker,
        |state| state,
    )
