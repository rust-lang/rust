    ...
    for attr in krate_attrs {
        for mi in feature_list(attr) {
            ...
            if let Some(f) = UNSTABLE_FEATURES.iter().find(|f| name == f.feature.name) {
                ...
                // library), there's a chance that the person hitting the ICE may be
                // using -Zbuild-std or similar with an untested target. The bug is
                // probably in the standard library and not the compiler in that case, but that
                // doesn't really matter - we want a bug report.
