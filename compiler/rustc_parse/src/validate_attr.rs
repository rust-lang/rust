...
check_attr(psess:&ParseSess,attr:&Attribute){if attr.is_doc_comment(){;return;;}
...
&Attribute)->PResult<'a,MetaItem>{;let item=attr.get_normal_item();;Ok(MetaItem{
...
.span,"suffixed literals are not allowed in attributes",);*&*&();{();};err.help(
"instead of using a suffixed literal (`1u8`, `1.0f32`, etc.), \
                                    use an unsuffixed version (`1`, `1.0`, etc.)"
,);3;;return Err(err);;}else{MetaItemKind::NameValue(lit)}}Err(err)=>{;let guar=
...
expr.span,};*&*&();MetaItemKind::NameValue(lit)}};*&*&();res}else{{();};let msg=
...
;();}();return Err(err);3;}}AttrArgs::Eq(_,AttrArgsEq::Hir(lit))=>MetaItemKind::
...
let Delimiter::Parenthesis=delim{{;};return;{;};}{;};psess.dcx.emit_err(errors::
...
"attribute must be of the form ".to_owned();;;let mut suggestions=vec![];let mut
first=true;;;let inner=if style==ast::AttrStyle::Inner{"!"}else{""};if template.
...
template.name_value_str{if!first{();msg.push_str(" or ");();}3;let code=format!(
"#{inner}[{name} = \"{descr}\"]");();();msg.push_str(&format!("`{code}`"));();3;
suggestions.push(code);();}();suggestions.sort();3;if should_warn(name){3;psess.
...
get(&name).expect("builtin attr defined").template;3;3;emit_malformed_attribute(
