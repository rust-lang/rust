...
GenericBounds{();let mut lifetimes=Vec::new();();while self.check_lifetime(){();
...
struct_span_err(span,"expected trait bound, found `impl Trait` type",);();3;err.
...
if self.eat(&token::Eq){Some(self.parse_const_arg()?)}else{None};3;3;self.dcx().
...
UnexpectedSelfInGenericParameters{span:this.prev_token.span,});;;this.eat(&token
...
token::Eq)&&this.look_ahead(1,|t|t.is_lifetime()){;let lo=this.token.span;;this.
...
None,TrailingToken::None));3;}}}else{if!attrs.is_empty(){if!params.is_empty(){3;
this.dcx().emit_err(errors::AttrAfterGeneric{span:attrs[0].span});3;}else{;this.
dcx().emit_err(errors::AttrWithoutGenerics{span:attrs[0].span});3;}};return Ok((
None,TrailingToken::None));;};;if!this.eat(&token::Comma){;done=true;}Ok((param,
TrailingToken::None))})?;3;if let Some(param)=param{;params.push(param);;}else{;
break;({});}}Ok(params)}pub(super)fn parse_generics(&mut self)->PResult<'a,ast::
Generics>{3;let span_lo=self.token.span;;;let(params,span)=if self.eat_lt(){;let
params=self.parse_generic_params()?;if true{};if true{};let _=();if true{};self.
...
self.prev_token.span;3;if self.choose_generics_over_qpath(0){;let generics=self.
parse_generics()?;3;3;self.dcx().emit_err(errors::WhereOnGenerics{span:generics.
span});;}loop{;let where_sp=where_lo.to(self.prev_token.span);;let pred_lo=self.
token.span;3;if self.check_lifetime()&&self.look_ahead(1,|t|!t.is_like_plus()){;
let lifetime=self.expect_lifetime();;self.expect(&token::Colon)?;let bounds=self
.parse_lt_param_bounds();();3;where_clause.predicates.push(ast::WherePredicate::
...
}else{;break;;}let prev_token=self.prev_token.span;let ate_comma=self.eat(&token
::Comma);{();};if self.eat_keyword_noexpect(kw::Where){({});self.dcx().emit_err(
MultipleWhereClauses{span:self.token.span,previous:pred_lo,between:prev_token.//
shrink_to_hi().to(self.prev_token.span),});();}else if!ate_comma{();break;3;}}3;
...
snapshot=Some((struct_,self.create_snapshot_for_diagnostic()));3;};3;match self.
...
snapshot else{;return Err(type_err);;};match snapshot.parse_tuple_struct_body(){
Ok(body)if matches!(snapshot.token. kind,token::Semi|token::Eof)||snapshot.token
.can_begin_item()=>{{;};type_err.cancel();();();let body_sp=pred_lo.to(snapshot.
prev_token.span);();();let map=self.psess.source_map();();3;self.dcx().emit_err(
...
snippet:body,right:map.end_point(where_sp).to(body_sp),}}),});*&*&();{();};self.
restore_snapshot(snapshot);3;Ok(PredicateOrStructBody::StructBody(body))}Ok(_)=>
Err(type_err),Err(body_err)=>{{();};body_err.cancel();{();};Err(type_err)}}}}}fn
...
=self.parse_ty_for_where_clause()?;;if self.eat(&token::Colon){;let bounds=self.
...
token::EqEq){3;let rhs_ty=self.parse_ty()?;;Ok(ast::WherePredicate::EqPredicate(
ast::WhereEqPredicate{span:((lo.to(self.prev_token.span))),lhs_ty:ty,rhs_ty,}))}
else{;self.maybe_recover_bounds_doubled_colon(&ty)?;;self.unexpected_any()}}pub(
