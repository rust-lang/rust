...
impl<'a> Parser<'a> {
    ...
    fn parse_expr_bottom(&mut self) -> PResult<'a, P<Expr>> {
        ...
        if let Some(expr) = self.eat_metavar_seq_with_matcher(
            ...
            return Ok(expr);
        } else if let Some(lit) = self.eat_metavar_seq(MetaVarKind::Literal, |this| {
            this.collect_tokens_no_attrs(|this| this.parse_literal_maybe_minus())
        }) {
            return Ok(lit);
        } else if let Some(block) = self.eat_metavar_seq(MetaVarKind::Block, |this| {
    ...
    pub fn eat_token_lit(&mut self) -> Option<token::Lit> {
        match self.token.uninterpolate().kind {
            ...
            token::OpenDelim(Delimiter::Invisible(InvisibleOrigin::MetaVar(
                ...
                let lit = self
                    .eat_metavar_seq(MetaVarKind::Literal, |this| {
                        // No need to collect tokens when we are only extracting `token_lit`.
                        this.parse_literal_maybe_minus()
                    })
            ...
            token::OpenDelim(Delimiter::Invisible(InvisibleOrigin::MetaVar(
                ...
                let expr = self
                    .eat_metavar_seq(mv_kind, |this| {
                        // No need to collect tokens because we only consult `expr.kind`.
