    ...
    pub(super) fn check_assoc_op(&self) -> Option<Spanned<AssocOp>> {
        let (op, span) = match (AssocOp::from_token(&self.token), self.token.ident()) {
            ...
            (None, Some((Ident { name: sym::and, span }, IdentIsRaw::No)))
                if self.may_recover() =>
            {
                self.dcx().emit_err(crate::diagnostics::InvalidLogicalOperator {
                    span: self.token.span,
                    incorrect: "and".into(),
                    sub: crate::diagnostics::InvalidLogicalOperatorSub::Conjunction(
                        self.token.span,
                    ),
                });
            ...
            (None, Some((Ident { name: sym::or, span }, IdentIsRaw::No))) if self.may_recover() => {
                self.dcx().emit_err(crate::diagnostics::InvalidLogicalOperator {
                    span: self.token.span,
                    incorrect: "or".into(),
                    sub: crate::diagnostics::InvalidLogicalOperatorSub::Disjunction(
                        self.token.span,
                    ),
                });
