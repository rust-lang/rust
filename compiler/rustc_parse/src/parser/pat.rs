...
use crate::errors::{
    ...
    ExpectedCommaAfterPatternField, ExtraEqInInclusiveRangePattern,
    InclusiveRangePatternWithoutEnd, InvalidMutInPattern, PatternOnWrongSideOfAt,
    RefMutOrderIncorrect, RemoveLet, RepeatedMutInPattern, TopLevelOrPatternNotAllowed,
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn inclusive_range_with_incorrect_end(&mut self, span: Span) {
        ...
        if matches!(tok.kind, token::Eq) && tok.span.lo() == span.hi() {
            ...
            self.sess.emit_err(ExtraEqInInclusiveRangePattern { span: span_with_eq });
        } else {
            self.sess.emit_err(InclusiveRangePatternWithoutEnd { span });
        }
    }
