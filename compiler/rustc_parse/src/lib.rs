    ...
    let tokens = match *nt {
        ...
        Nonterminal::NtStmt(ref stmt) => {
            if let ast::StmtKind::Empty = stmt.kind {
                let tokens = PreexpTokenStream::new(vec![(
                    tokenstream::PreexpTokenTree::Token(Token::new(TokenKind::Semi, stmt.span)),
                    Spacing::Alone,
                )]);
                prepend_attrs(&stmt.attrs(), Some(&LazyTokenStream::new(tokens)))
            } else {
                prepend_attrs(&stmt.attrs(), stmt.kind.tokens())
            }
        }
...
fn prepend_attrs(attrs: &[Attribute], tokens: Option<&LazyTokenStream>) -> Option<TokenStream> {
    ...
    let wrapped =
        PreexpTokenStream::new(vec![(PreexpTokenTree::Attributes(attr_data), Spacing::Alone)]);
