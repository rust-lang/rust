...
use super::{DelimitedData, LexTokenTree, Lexer, TokenArena, UnmatchedDelim};
...
impl<'psess, 'src> Lexer<'psess, 'src> {
    ...
    pub(super) fn lex_token_trees2(
        ...
        loop {
            if let Some(delim) = self.token.kind.open_delim() {
                ...
                let value = match self.lex_token_tree_open_delim2(arena, delim) {
                    Ok(value) => value,
                    Err(errs) => return Err(errs),
                };
                ...
                // Get the next normal token.
                let (this_tok, this_spacing) = self.bump();
                arena.push(LexTokenTree::Token(this_tok, this_spacing));
            }
        }
    }
    ...
    fn lex_token_tree_open_delim2(
        ...
        let open_spacing = self.lex_token_trees2(arena, /* is_delimited */ true)?;
        ...
        let close_spacing = if let Some(close_delim) = self.token.kind.close_delim() {
            if close_delim == open_delim {
                ...
                if index == arena.length() && close_delim == Delimiter::Brace {
                    let empty_block_span = pre_span.to(close_delimiter_span);
                    if !sm.is_multiline(empty_block_span) {
                        // Only track if the block is in the form of `{}`, otherwise it is
                        // likely that it was written on purpose.
