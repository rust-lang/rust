...
use rustc_middle::error::lint_lints::*;
...
trait Mapping {
    const MAPPING: (&'static [Symbol], AttributeTemplate, AcceptFn<LintParser>);
}
impl<T: Lint> Mapping for T {
    const MAPPING: (&'static [Symbol], AttributeTemplate, AcceptFn<LintParser>) = (
        &[T::KIND.symbol()],
        template!(
        ...
        |this, cx, args| {
            if let Some(lint_attr) = validate_lint_attr::<T>(cx, args) {
...
fn check_lint<'a>(
    ...
    let check_lint_name = lint_store.check_lint_name(original_name.as_str(), tool_name, tools);
    match check_lint_name {
            ...
            if !early {
                cx.emit_lint(
                    ...
                    RenamedLint {
                        name: *full_name,
                        replace,
                        suggestion: RenamedLintSuggestion::WithSpan { suggestion: span, replace },
                    },
                    span,
                );
            }
        ...
        CheckLintNameResult::Removed(reason) => {
            if !early {
                cx.emit_lint(
                    RENAMED_AND_REMOVED_LINTS,
                    RemovedLint { name: *full_name, reason },
                    span,
                );
            }
            None
        ...
        CheckLintNameResult::NoLint(suggestion) => {
            if !early {
                cx.emit_lint(
                    ...
                    UnknownLint {
                        name: *full_name,
                        suggestion: suggestion.map(|(symbol, is_rustc)| {
