            ...
            match meta.args() {
                ArgParser::List(list) => {
                    match meta.path().word_sym() {
                        ...
                        Some(sym::version) => {
                            try_gate_cfg(sym::version, meta.span(), cx.sess(), Some(cx.features()));
                            let Some(version) = list.single() else {
                                cx.emit_err(session_diagnostics::ExpectedSingleVersionLiteral {
                                    span: list.span,
                                });
                                return None;
                            };
                            let Some(version_lit) = version.lit() else {
                                cx.emit_err(session_diagnostics::ExpectedVersionLiteral {
                                    span: version.span(),
                                });
                                return None;
                            };
                            ...
                            CfgEntry::Version(min_version, list.span)
                        }
                        _ => {
