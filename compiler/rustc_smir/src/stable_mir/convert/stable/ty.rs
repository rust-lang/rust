...
impl<'tcx> Stable<'tcx> for ty::ExistentialProjection<'tcx> {
    ...
    fn stable(
        ...
        stable_mir::ty::ExistentialProjection {
            def_id: tables.trait_def(*def_id),
            generic_args: args.stable(tables, cx),
            term: term.unpack().stable(tables, cx),
        }
    }
}
...
impl<'tcx> Stable<'tcx> for ty::GenericArgs<'tcx> {
    ...
    fn stable(
        ...
        GenericArgs(self.iter().map(|arg| arg.unpack().stable(tables, cx)).collect())
    }
}
        ...
        match self {
            ...
            PredicateKind::AliasRelate(a, b, alias_relation_direction) => {
                stable_mir::ty::PredicateKind::AliasRelate(
                    a.unpack().stable(tables, cx),
                    b.unpack().stable(tables, cx),
    ...
    fn stable(
        ...
        match *self {
            ...
            ClauseKind::WellFormed(term) => {
                stable_mir::ty::ClauseKind::WellFormed(term.unpack().stable(tables, cx))
            }
