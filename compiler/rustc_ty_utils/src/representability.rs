...
use rustc_errors::{Applicability, E0072, MultiSpan, pluralize, struct_span_code_err};
...
pub(crate) fn provide(providers: &mut Providers) {
    ...
    providers.query_cycle_handlers.representability = Some(representability_cycle_decor);
    providers.query_cycle_handlers.representability_adt_ty = Some(representability_cycle_decor);
}

fn representability(tcx: TyCtxt<'_>, def_id: LocalDefId) {
...
fn representability_ty<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) {
    match *ty.kind() {
...
*not* in the cycle.
*/
fn representability_adt_ty<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) {
...
fn representability_cycle_decor(
    tcx: TyCtxt<'_>,
    ...
    Some(recursive_type_error(tcx, item_and_field_ids, &representable_ids))
}
