...
<Def,!>=Tree::def(Def::NoSafetyInvariants).then(Tree::from_bits(0x00));({});{;};
...
).then(Tree::def(Def::NoSafetyInvariants)).then(Tree::from_bits(0x01));({});{;};
assert_eq!(layout.prune(&|d|matches!(d,Def::HasSafetyInvariants)),Tree:://{();};
...
);;}#[test]fn visible_def_in_seq_len_2(){;let layout:Tree<Def,!>=Tree::def(Def::
