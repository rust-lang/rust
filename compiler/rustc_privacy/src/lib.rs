...
impl<'tcx, 'a> Visitor<'tcx> for TestReachabilityVisitor<'tcx, 'a> {
    fn visit_item(&mut self, item: &'tcx hir::Item<'tcx>) {
        self.access_level_diagnostic(item.owner_id.def_id);
...
impl<'a, 'tcx> Visitor<'tcx> for ObsoleteVisiblePrivateTypesVisitor<'a, 'tcx> {
        ...
        match item.kind {
            ...
            hir::ItemKind::Impl(ref impl_) => {
                    ...
                    || impl_.items.iter().any(|impl_item_ref| {
                        ...
                        match impl_item.kind {
                            hir::ImplItemKind::Const(..) | hir::ImplItemKind::Fn(..) => {
                                self.access_levels.is_reachable(impl_item_ref.id.owner_id.def_id)
                            }
                ...
                if !self_contains_private && not_private_trait && trait_or_some_public_method {
                    ...
                    for impl_item_ref in impl_.items {
                        if self.access_levels.is_reachable(impl_item_ref.id.owner_id.def_id)
                            || self.tcx.visibility(impl_item_ref.id.owner_id).is_public()
                        {
                            let impl_item = self.tcx.hir().impl_item(impl_item_ref.id);
    ...
    fn visit_foreign_item(&mut self, item: &'tcx hir::ForeignItem<'tcx>) {
        if self.access_levels.is_reachable(item.owner_id.def_id) {
