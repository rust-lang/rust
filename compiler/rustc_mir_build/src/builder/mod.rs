...
fn construct_fn<'tcx>(
    ...
    let coroutine = match tcx.type_of(fn_def).instantiate_identity().skip_normalization().kind() {
...
fn construct_error(tcx: TyCtxt<'_>, def_id: LocalDefId, guar: ErrorGuaranteed) -> Body<'_> {
    ...
    let (inputs, output, coroutine) = match tcx.def_kind(def_id) {
        ...
        | DefKind::Static { .. }
        | DefKind::GlobalAsm => {
            (vec![], tcx.type_of(def_id).instantiate_identity().skip_normalization(), None)
        }
        ...
        for bbdata in self.cfg.basic_blocks.iter_mut() {
            ...
            if !ty_is_inhabited {
                ...
                // https://github.com/rust-lang/rust/issues/149571
                    && self
                        .tcx
                        .fn_sig(self.def_id).instantiate_identity().skip_normalization()
                        .skip_binder()
                        .output()
                        .is_inhabited_from(
                            self.tcx,
