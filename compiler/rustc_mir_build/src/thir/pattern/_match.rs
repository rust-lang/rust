...
impl<'tcx> PatternFolder<'tcx> for LiteralExpander<'tcx> {
    fn fold_pattern(&mut self, pat: &Pat<'tcx>) -> Pat<'tcx> {
        debug!("fold_pattern {:?} {:?} {:?}", pat, pat.ty.data(), pat.kind);
        match (pat.ty.data(), &*pat.kind) {
            (&ty::Ref(_, rty, _), &PatKind::Constant { value: Const { val, ty: const_ty } })
                if const_ty.is_ref() =>
            {
                let crty =
                    if let ty::Ref(_, crty, _) = const_ty.data() { crty } else { unreachable!() };
...
impl<'tcx> Constructor<'tcx> {
    ...
    fn apply<'p>(
        ...
        let pat = match self {
            Single | Variant(_) => match ty.data() {
                ty::Adt(..) | ty::Tuple(..) => {
...
fn pat_constructor<'tcx>(
    ...
    match *pat.kind {
        ...
        | PatKind::Slice { ref prefix, ref slice, ref suffix } => {
            let array_len = match pat.ty.data() {
...
fn slice_pat_covered_by_const<'tcx>(
    ...
    let data: &[u8] = match (const_val_val, &const_val.ty.data()) {
...
fn specialize_one_pattern<'p, 'tcx>(
    ...
    let result = match *pat.kind {
        ...
        PatKind::Constant { value } if constructor.is_slice() => {
            ...
            let (alloc, offset, n, ty) = match value.ty.data() {
                ty::Array(t, n) => {
