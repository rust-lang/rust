...
impl<'p, 'tcx> PatStack<'p, 'tcx> {
    ...
    fn from_vec(
        ...
        PatStack { pats: vec, parent_row, is_under_guard, reachable: false }
    }
    ...
    // Expand the first pattern into its subpatterns. Only useful if the pattern is an or-pattern.
...
impl<'p, 'tcx> Matrix<'p, 'tcx> {
    /// Make an empty matrix. Internal method, use [`Matrix::new`].
    fn empty() -> Self {
        Matrix { rows: vec![] }
    }
    /// Pushes a new row to the matrix. If the row starts with an or-pattern, this recursively
    /// expands it. Internal method, use [`Matrix::new`].
...
/// each row in the matrix (in `row.reachable`). We track reachability of each subpattern using
/// interior mutability in `DeconstructedPat`.
///
...
/// The entrypoint for the usefulness algorithm. Computes whether a match is exhaustive and which of
/// its arms are reachable.
