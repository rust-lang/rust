...
fn make_mirror_unadjusted<'a, 'tcx>(
    ...
    let kind = match expr.kind {
        ...
        hir::ExprKind::Struct(ref qpath, ref fields, ref base) => match expr_ty.data() {
        ...
        hir::ExprKind::Closure(..) => {
            ...
            let (def_id, substs, movability) = match *closure_ty.data() {
...
fn convert_var<'tcx>(
    ...
    match upvar_index {
        ...
        Some(upvar_index) => {
            ...
            let self_expr = if let ty::Closure(_, closure_substs) = closure_ty.data() {
...
fn overloaded_place<'a, 'tcx>(
    ...
    let (region, mutbl) = match *recv_ty.data() {
