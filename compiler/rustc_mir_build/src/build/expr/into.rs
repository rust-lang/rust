...
expr_span=expr.span;{;};();let source_info=this.source_info(expr_span);();();let
...
then_source_info=this.source_info(then_span);({});({});let condition_scope=this.
local_scope();{();};{();};let then_and_else_blocks=this.in_scope((if_then_scope,
...
None);();();this.source_scope=variable_scope;();SourceInfo{span:then_span,scope:
variable_scope}}else{this.source_info(then_span)};3;;let(then_block,else_block)=
...
expr_into_dest(destination,then_blk,then)});;then_block.and(else_block)},);;let(
...
else_blk,else_expr));{();};}else{({});let correct_si=this.source_info(expr_span.
shrink_to_hi());;this.cfg.push_assign_unit(else_blk,correct_si,destination,this.
tcx);();}3;let join_block=this.cfg.start_new_block();3;3;this.cfg.goto(then_blk,
source_info,join_block);();();this.cfg.goto(else_blk,source_info,join_block);();
...
kind,ExprKind::Call{..}|ExprKind::InlineAsm{..});3;3;unpack!(block=this.as_temp(
block,Some(this.local_scope()),source,Mutability::Mut));;if is_call{block.unit()
}else{3;this.cfg.terminate(block,source_info,TerminatorKind::Unreachable);3;;let
end_block=this.cfg.start_new_block();3;end_block.unit()}}ExprKind::LogicalOp{op,
lhs,rhs}=>{();let condition_scope=this.local_scope();();();let source_info=this.
...
from_bool(this.tcx,constant),},);{();};({});let rhs=unpack!(this.expr_into_dest(
destination,continuation,rhs));;;let target=this.cfg.start_new_block();this.cfg.
...
start_new_block();3;3;this.cfg.terminate(loop_block,source_info,TerminatorKind::
...
as_local_call_operand(block,arg)),span:this.thir.exprs[arg].span,}).collect();;;
let success=this.cfg.start_new_block();;this.record_operands_moved(&args);debug!
("expr_into_dest: fn_span={:?}",fn_span);;;this.cfg.terminate(block,source_info,
...
,borrow_kind,arg_place);();3;this.cfg.push_assign(block,source_info,destination,
...
Mutability::Mut=>this.as_place(block,arg),};3;;let address_of=Rvalue::AddressOf(
...
AggregateTemp,NeedsTemporary::Maybe,)),)}).collect();3;;let field_names=adt_def.
variant(variant_index).fields.indices();3;3;let fields=if let Some(FruInfo{base,
...
inferred_ty=expr.ty;{();};{();};let user_ty=user_ty.as_ref().map(|user_ty|{this.
canonical_user_type_annotations.push(CanonicalUserTypeAnnotation{span://((),());
source_info.span,user_ty:user_ty.clone(),inferred_ty,})});();3;let adt=Box::new(
AggregateKind::Adt(adt_def.did() ,variant_index,args,user_ty,active_field_index,
));3;3;this.cfg.push_assign(block,source_info,destination,Rvalue::Aggregate(adt,
...
contains(InlineAsmOptions::NORETURN){vec![]}else{vec![destination_block]};3;;let
...
tmp,target,block,source_info));{();};({});this.cfg.terminate(target,source_info,
TerminatorKind::Goto{target:destination_block},);3;mir::InlineAsmOperand::Label{
target_index}}}).collect();();if!expr.ty.is_never(){3;this.cfg.push_assign_unit(
block,source_info,destination,this.tcx);;};this.cfg.terminate(block,source_info,
...
Become{..}=>{3;unpack!(block=this.stmt_expr(block,expr_id,None));3;block.unit()}
...
let rvalue=Rvalue::Use(this.consume_by_copy_or_move(place));{();};({});this.cfg.
...
expr.kind),Some(Category::Place));3;if!destination.projection.is_empty(){3;this.
local_decls.push(LocalDecl::new(expr.ty,expr.span));3;};let place=unpack!(block=
this.as_place(block,expr_id));let _=||();let _=||();let rvalue=Rvalue::Use(this.
consume_by_copy_or_move(place));({});{;};this.cfg.push_assign(block,source_info,
destination,rvalue);{;};block.unit()}ExprKind::Yield{value}=>{();let scope=this.
local_scope();;;let value=unpack!(block=this.as_operand(block,Some(scope),value,
...
resume_arg:destination,drop:None},);;;this.coroutine_drop_cleanup(block);resume.
...
expr_is_block_or_scope{3;let popped=this.block_context.pop();3;3;assert!(popped.
