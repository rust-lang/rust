    ...
    pub(crate) fn expr_into_dest(
        ...
        let block_and = match expr.kind {
            ...
            ExprKind::LogicalOp { op, lhs, rhs } => {
                ...
                // - If the operator is `&&`, passing `lhs` leads to continuation of evaluation on
                //   `rhs`; failing it leads to the short-circuting path which assigns `false` to
                //   the place.
                // - If the operator is `||`, failing `lhs` leads to continuation of evaluation on
                //   `rhs`; passing it leads to the short-circuting path which assigns `true` to the
                //   place.
                this.cfg.push_assign_constant(
                    short_circuit,
                    source_info,
