...
fn mir_build(tcx: TyCtxt<'_>, def: ty::WithOptConstParam<LocalDefId>) -> Body<'_> {
    ...
    tcx.infer_ctxt().enter(|infcx| {
        ...
        let body = if let Some(ErrorReported) = cx.typeck_results().tainted_by_errors {
            ...
            let implicit_argument = match ty.data() {
                ty::Closure(..) => {
...
fn liberated_closure_env_ty(
    ...
    let (closure_def_id, closure_substs) = match *closure_ty.data() {
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    fn args_and_body(
        ...
        if let Some(upvars) = hir_typeck_results.closure_captures.get(&fn_def_id) {
            ...
            if let ty::Ref(_, ty, _) = closure_ty.data() {
                closure_env_projs.push(ProjectionElem::Deref);
                closure_ty = ty;
            }
            let upvar_substs = match closure_ty.data() {
