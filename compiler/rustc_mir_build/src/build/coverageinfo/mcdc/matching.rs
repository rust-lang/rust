...
impl CandidateCtx {
    ...
    fn into_matching_decision(
        ...
        let conditions: Vec<_> = self
            ...
            .collect();
        let decision = MCDCDecisionSpan::new(Span::default());
        MCDCTargetInfo { decision, conditions, nested_decisions_id: vec![] }
    }
}

#[derive(Debug)]
pub struct MatchingDecisionCtx {
...
impl MatchingDecisionCtx {
    pub fn new(candidates: &[DecisionId]) -> Self {
        Self {
            candidates: candidates.into_iter().map(|&id| (id, CandidateCtx::new())).collect(),
...
impl LateMatchingCtx {
    fn finish_arm(
        ...
        decision_span_hint: Span,
        arm_block: BlockMarkerId,
        ...
        decision.span = decision_span_hint;
...
impl LateMatchingState {
    ...
    pub fn is_guard_decision(&self, boolean_decision_id: DecisionId) -> bool {
        self.guard_decisions_info.contains_key(&boolean_decision_id)
    }

    pub fn add_guard_decision(&mut self, boolean_decision_id: DecisionId, info: MCDCTargetInfo) {
    ...
    fn finish_arm(
        ...
        let guard = self
            ...
            .flatten();
        ctx.finish_arm(id, candidate_span, arm_block, guard);
        if ctx.all_arms_finished() { self.matching_ctx.pop() } else { None }
    }
}

impl MCDCInfoBuilder {
    fn create_pattern_decision(&mut self, num_candidates: usize) -> Vec<DecisionId> {
        self.ensure_active_state();
        let state = self.state_stack.last_mut().expect("ensured just now");
        let decision_ids: Vec<_> =
            (0..num_candidates).map(|_| self.decision_id_gen.next_decision_id()).collect();
        assert!(state.is_empty(), "active state for new pattern decision must be empty");
        state.current_ctx = Some(DecisionCtx::new_matching(&decision_ids));
        decision_ids
    }
    ...
    fn visit_evaluated_matching_candidate(
        ...
        let Some(ctx) = self.late_matching_state.finish_arm(decision_id, candidate_span, arm_block)
        else {
            return;
        };
...
impl Builder<'_, '_> {
    ...
    pub(crate) fn mcdc_create_matching_decisions(
        ...
        {
            let ids = mcdc_info.create_pattern_decision(candidates.len());
    ...
    pub(crate) fn mcdc_finish_matching_tree(
        ...
        let has_decision = candidates.iter().any(|candidate| candidate.coverage_id().is_valid());
        if has_decision
            && let Some(coverage_info) = self.coverage_info.as_mut()
            && let Some(mcdc_info) = coverage_info.mcdc_info.as_mut()
        {
    ...
    pub(crate) fn mcdc_visit_matching_decision_end(
        ...
        if coverage_id.is_valid()
            ...
            && let Some(mcdc_info) = coverage_info.mcdc_info.as_mut()
        {
            let arm_marker = coverage_info.markers.inject_block_marker(
                &mut self.cfg,
                SourceInfo { span: candidate_span, scope: self.source_scope },
                arm_block,
            );
