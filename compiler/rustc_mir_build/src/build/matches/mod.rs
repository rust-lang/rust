                ...
                matched_candidates.iter().flat_map(|candidate| &candidate.bindings)
            {
                    ...
                    source.projection.iter().rposition(|elem| elem == ProjectionElem::Deref)
                {
                    ...
                    fake_borrows.insert(Place {
                        local: source.local,
                        projection: self.tcx.mk().place_elems(proj_base),
                    });
                }
            }
        }
        ...
        all_fake_borrows
            ...
            .map(|matched_place_ref| {
                let matched_place = Place {
                    ...
                    projection: tcx.mk().place_elems(matched_place_ref.projection),
                };
                let fake_borrow_deref_ty = matched_place.ty(&self.local_decls, tcx).ty;
                let fake_borrow_ty = tcx.mk().imm_ref(tcx.lifetimes.re_erased, fake_borrow_deref_ty);
    ...
    fn declare_binding(
        ...
        let locals = if has_guard.0 {
            let ref_for_guard = self.local_decls.push(LocalDecl::<'tcx> {
                ...
                ty: tcx.mk().imm_ref(tcx.lifetimes.re_erased, var_ty),
                user_ty: None,
                source_info,
                internal: false,
                is_block_tail: None,
