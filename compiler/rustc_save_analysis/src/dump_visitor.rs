...
impl<'tcx> DumpVisitor<'tcx> {
    ...
    fn process_method(
        ...
        self.nest_typeck_results(def_id.def_id, |v| {
            ...
            if let Some(mut method_data) = v.save_ctxt.get_method_data(hir_id, ident, span) {
                ...
                v.dumper.dump_def(&access_from_vis!(v.save_ctxt, vis, def_id.def_id), method_data);
            }
    ...
    fn process_fn(
        ...
        self.nest_typeck_results(item.def_id.def_id, |v| {
            ...
            if let Some(fn_data) = v.save_ctxt.get_item_data(item) {
                ...
                v.dumper.dump_def(&access_from!(v.save_ctxt, item, item.def_id.def_id), fn_data);
            }
    ...
    fn process_static_or_const_item(
        ...
        self.nest_typeck_results(item.def_id.def_id, |v| {
            if let Some(var_data) = v.save_ctxt.get_item_data(item) {
                ...
                v.dumper.dump_def(&access_from!(v.save_ctxt, item, item.def_id.def_id), var_data);
            }
            v.visit_ty(&typ);
            v.visit_expr(expr);
        });
    ...
    fn process_assoc_const(
        ...
        let qualname = format!("::{}", self.tcx.def_path_str(def_id.def_id.to_def_id()));
        ...
        if !self.span.filter_generated(ident.span) {
            ...
            self.dumper.dump_def(
                &access_from_vis!(self.save_ctxt, vis, def_id.def_id),
                Def {
                    kind: DefKind::Const,
                    id: id_from_def_id(def_id.def_id.to_def_id()),
                    span,
    ...
    fn process_struct(
        ...
        let qualname = format!("::{}", self.tcx.def_path_str(item.def_id.def_id.to_def_id()));
        ...
        if !self.span.filter_generated(item.ident.span) {
            ...
            self.dumper.dump_def(
                &access_from!(self.save_ctxt, item, item.def_id.def_id),
                Def {
                    kind,
                    id: id_from_def_id(item.def_id.def_id.to_def_id()),
                    span,
                    name,
    ...
    fn process_enum(
        ...
        let access = access_from!(self.save_ctxt, item, item.def_id.def_id);
            ...
            match variant.data {
                hir::VariantData::Struct(ref fields, ..) => {
                    ...
                    if !self.span.filter_generated(name_span) {
                        ...
                        let parent = Some(id_from_def_id(item.def_id.def_id.to_def_id()));
    ...
    fn process_impl(&mut self, item: &'tcx hir::Item<'tcx>, impl_: &'tcx hir::Impl<'tcx>) {
        ...
        self.nest_typeck_results(item.def_id.def_id, |v| {
            ...
            for impl_item in impl_.items {
                v.process_impl_item(map.impl_item(impl_item.id), item.def_id.def_id.to_def_id());
            }
        });
    }
    ...
    fn process_trait(
        ...
        if !self.span.filter_generated(item.ident.span) {
            let id = id_from_def_id(item.def_id.def_id.to_def_id());
            let span = self.span_from_span(item.ident.span);
            let children =
                methods.iter().map(|i| id_from_def_id(i.id.def_id.def_id.to_def_id())).collect();
            let attrs = self.tcx.hir().attrs(item.hir_id());
            self.dumper.dump_def(
                &access_from!(self.save_ctxt, item, item.def_id.def_id),
                Def {
            ...
            if let Some(id) = def_id {
                if !self.span.filter_generated(sub_span) {
                    ...
                    self.dumper.dump_relation(Relation {
                        ...
                        to: id_from_def_id(item.def_id.def_id.to_def_id()),
                    });
                }
            }
        }
        ...
        for method in methods {
            let map = &self.tcx.hir();
            self.process_trait_item(map.trait_item(method.id), item.def_id.def_id.to_def_id())
        }
    }
    ...
    fn process_mod(&mut self, item: &'tcx hir::Item<'tcx>) {
        if let Some(mod_data) = self.save_ctxt.get_item_data(item) {
            ...
            self.dumper.dump_def(&access_from!(self.save_ctxt, item, item.def_id.def_id), mod_data);
        }
    }
    ...
    fn process_trait_item(&mut self, trait_item: &'tcx hir::TraitItem<'tcx>, trait_id: DefId) {
        ...
        match trait_item.kind {
            ...
            hir::TraitItemKind::Type(ref bounds, ref default_ty) => {
                ...
                let qualname =
                    format!("::{}", self.tcx.def_path_str(trait_item.def_id.def_id.to_def_id()));

                if !self.span.filter_generated(trait_item.ident.span) {
                    let span = self.span_from_span(trait_item.ident.span);
                    let id = id_from_def_id(trait_item.def_id.def_id.to_def_id());
                    let attrs = self.tcx.hir().attrs(trait_item.hir_id());
    ...
    fn visit_item(&mut self, item: &'tcx hir::Item<'tcx>) {
        ...
        match item.kind {
            hir::ItemKind::Use(path, hir::UseKind::Single) => {
                ...
                if !self.span.filter_generated(sub_span) {
                    ...
                    let parent = self
                        .save_ctxt
                        .tcx
                        .parent(item.def_id.def_id.to_def_id())
                        .map(id_from_def_id);
                    self.dumper.import(
                        &access,
                        Import {
            ...
            hir::ItemKind::TyAlias(ty, ref generics) => {
                let qualname =
                    format!("::{}", self.tcx.def_path_str(item.def_id.def_id.to_def_id()));
