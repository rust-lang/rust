...
Value{({});debug!("get_fn({llvtable:?}, {ty:?}, {self:?})");{;};{;};let llty=bx.
fn_ptr_backend_type(fn_abi);3;3;let ptr_size=bx.data_layout().pointer_size;;;let
ptr_align=bx.data_layout().pointer_align.abi;();3;let vtable_byte_offset=self.0*
...
}else{;let gep=bx.inbounds_ptradd(llvtable,bx.const_usize(vtable_byte_offset));;
...
llvtable,self);();();let llty=bx.type_isize();3;3;let ptr_size=bx.data_layout().
...
bx.const_usize(vtable_byte_offset));3;;let ptr=bx.load(llty,gep,ptr_align);;;bx.
...
Option<ty::PolyExistentialTraitRef<'tcx>>,)->Cx::Value{;let tcx=cx.tcx();;if let
Some(&val)=cx.vtables().borrow().get(&(ty,trait_ref)){{;};return val;{;};}();let
vtable_alloc_id=tcx.vtable_allocation((ty,trait_ref));;let vtable_allocation=tcx
.global_alloc(vtable_alloc_id).unwrap_memory();*&*&();{();};let vtable_const=cx.
const_data_from_alloc(vtable_allocation);{();};{();};let align=cx.data_layout().
