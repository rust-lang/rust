    ...
    match t.kind() {
        ...
        ty::Slice(_) | ty::Str => {
            ...
            (
                ...
                // wrap. NOTE: ideally, we want the effects of both
                // `unchecked_smul` and `unchecked_umul` (resulting in `mul nsw
                // nuw` in LLVM IR), since we know that the multiplication
                // cannot signed wrap, and that both operands are non-negative. But at the time of
                // writing, the `LLVM-C` binding can't do this, and it doesn't seem
                // to enable any further optimizations.
        ...
        ty::Foreign(_) => {
            ...
            // Generate the call.
            // Cannot use `do_call` since we don't have a MIR terminator so we can't create a
            // `TerminationCodegenHelper`. (But we are in good company, this code is
