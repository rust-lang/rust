...
pub fn allocator_shim_contents(tcx: TyCtxt<'_>, kind: AllocatorKind) -> Vec<AllocatorMethod> {
    ...
    // AllocToken and heap partitioning support. For allocators registered with the
    // `#[global_allocator]` attribute, the token-enabled versions of these allocation functions
    // forward to the non-token-enabled allocation functions of the registered allocator, ignoring
    // the token identifier, for backward compatibility with existing allocators.
...
/// functions by prepending the prefix to the already-mangled name of the callee.
pub fn allocator_shim_method_name<'tcx>(tcx: TyCtxt<'tcx>, method: &AllocatorMethod) -> String {
    ...
    if method.with_token { format!("{ALLOC_TOKEN_FN_PREFIX}{name}") } else { name }
}
