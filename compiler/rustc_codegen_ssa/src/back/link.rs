...
fn linker_with_args(
    ...
    if crate_type == CrateType::Cdylib {
        ...
        for lib in codegen_results.crate_info.used_libraries.iter().filter(|lib| {
            matches!(lib.kind, NativeLibKind::Static { export_symbols: Some(true), .. })
        }) {
            if !seen.insert((lib.name, lib.verbatim)) {
                continue;
            }

            if let Err(err) = add_c_staticlib_symbols(&sess, lib, &mut export_symbols) {
                sess.dcx()
                    .fatal(format!("failed to process C static library `{}`: {}", lib.name, err));
            }
        }
    }
