...
impl ImportLibraryItem {
    fn into_coff_short_export(self, sess: &Session) -> COFFShortExport {
        let import_name = (sess.target.arch == Architecture::Arm64EC).then(|| self.name.clone());
        COFFShortExport {
            name: self.name,
            ext_name: None,
    ...
    fn create_dll_import_lib(
        ...
        if common::is_mingw_gnu_toolchain(&sess.target) {
            ...
            let machine = match sess.target.arch {
                Architecture::X86_64 => MachineTypes::AMD64,
                Architecture::X86 => MachineTypes::I386,
                Architecture::AArch64 => MachineTypes::ARM64,
                Architecture::Arm64EC => MachineTypes::ARM64EC,
                Architecture::Arm => MachineTypes::ARMNT,
...
fn create_mingw_dll_import_lib(
    ...
    let (dlltool_target_arch, dlltool_target_bitness) = match sess.target.arch {
        Architecture::X86_64 => ("i386:x86-64", "--64"),
        Architecture::X86 => ("i386", "--32"),
        Architecture::AArch64 => ("arm64", "--64"),
...
impl<'a> ArArchiveBuilder<'a> {
    fn build_inner(self, output: &Path) -> io::Result<bool> {
        ...
        write_archive_to_stream(
            ...
            archive_kind,
            false,
            /* is_ec = */ Some(self.sess.target.arch == Architecture::Arm64EC),
        )?;
