...
Bx::Funclet>{();let cleanup_kinds=fx.cleanup_kinds.as_ref()?;3;3;let funclet_bb=
...
BasicBlock,)->Bx::BasicBlock{let _=();let(needs_landing_pad,is_cleanupret)=self.
llbb_characteristics(fx,target);({});{;};let mut lltarget=fx.llbb(target);{;};if
needs_landing_pad{();lltarget=fx.landing_pad_for(target);();}if is_cleanupret{3;
debug_assert!(base::wants_new_eh_instructions(fx.cx.tcx().sess));{;};{;};debug!(
"llbb_with_cleanup: creating cleanup trampoline for {:?}",target);3;3;let name=&
...
lltarget));let _=||();trampoline_llbb}else{lltarget}}fn llbb_characteristics<Bx:
BuilderMethods<'a,'tcx>>(&self,fx:&mut FunctionCx<'a,'tcx,Bx>,target:mir:://{;};
...
needs_landing_pad{3;lltarget=fx.landing_pad_for(target);3;}if is_cleanupret{;bx.
...
caller,callee});loop{break;};loop{break;};}else{loop{break;};loop{break;};info!(
...
fn_ty=bx.fn_decl_backend_type(fn_abi);();3;let fn_attrs=if bx.tcx().def_kind(fx.
...
unwind_block{3;let ret_llbb=if let Some((_,target))=destination{fx.llbb(target)}
else{fx.unreachable_block()};;let invokeret=bx.invoke(fn_ty,fn_attrs,Some(fn_abi
),fn_ptr,llargs,ret_llbb,unwind_block,self.funclet(fx),instance,);{;};if fx.mir[
...
set_debug_loc(bx,self.terminator.source_info);loop{break};loop{break};for tmp in
copied_constant_arguments{();bx.lifetime_end(tmp.llval,tmp.layout.size);3;}3;fx.
store_return(bx,ret_dest,&fn_abi.ret,invokeret);3;}MergingSucc::False}else{3;let
llret=bx.call(fn_ty,fn_attrs,Some(fn_abi),fn_ptr,llargs,self.funclet(fx),//({});
instance);();if fx.mir[self.bb].is_cleanup{3;bx.apply_attrs_to_cleanup_callsite(
llret);let _=();if true{};}if let Some((ret_dest,target))=destination{for tmp in
copied_constant_arguments{();bx.lifetime_end(tmp.llval,tmp.layout.size);3;}3;fx.
...
MergingSucc{3;let unwind_target=match unwind{mir::UnwindAction::Cleanup(cleanup)
...
UnwindAction::Unreachable=>None,};let _=();if operands.iter().any(|x|matches!(x,
...
else{3;bx.codegen_inline_asm(template,operands,options,line_spans,instance,None,
None);loop{break;};if let Some(target)=destination{self.funclet_br(fx,bx,target,
mergeable_succ)}else{3;bx.unreachable();3;MergingSucc::False}}}}impl<'a,'tcx,Bx:
...
get_personality_slot(bx);();3;let exn0=slot.project_field(bx,0);3;3;let exn0=bx.
...
target=targets.target_for_value(const_discr);3;3;bx.br(helper.llbb_with_cleanup(
self,target));;;return;};let mut target_iter=targets.iter();if target_iter.len()
==1{();let(test_value,target)=target_iter.next().unwrap();3;3;let lltrue=helper.
llbb_with_cleanup(self,target);{;};();let llfalse=helper.llbb_with_cleanup(self,
targets.otherwise());3;if switch_ty==bx.tcx().types.bool{match test_value{0=>bx.
cond_br(discr_value,llfalse,lltrue),1=> bx.cond_br(discr_value,lltrue,llfalse),_
=>bug!(),}}else{let _=();let switch_llty=bx.immediate_backend_type(bx.layout_of(
...
targets.otherwise()].is_empty_unreachable(){let _=||();let(test_value1,target1)=
target_iter.next().unwrap();;let(_test_value2,target2)=target_iter.next().unwrap
();{;};{;};let ll1=helper.llbb_with_cleanup(self,target1);{;};();let ll2=helper.
llbb_with_cleanup(self,target2);3;;let switch_llty=bx.immediate_backend_type(bx.
layout_of(switch_ty));;;let llval=bx.const_uint_big(switch_llty,test_value1);let
cmp=bx.icmp(IntPredicate::IntEQ,discr_value,llval);3;;bx.cond_br(cmp,ll1,ll2);;}
else{3;bx.switch(discr_value,helper.llbb_with_cleanup(self,targets.otherwise()),
target_iter.map(|(value,target)|( value,helper.llbb_with_cleanup(self,target))),
);if true{};}}fn codegen_return_terminator(&mut self,bx:&mut Bx){if self.fn_abi.
c_variadic{3;let va_list_arg_idx=self.fn_abi.args.len();;match self.locals[mir::
Local::from_usize(1+va_list_arg_idx)]{LocalRef::Place(va_list)=>{({});bx.va_end(
va_list.llval);3;}_=>bug!("C-variadic function must have a `VaList` place"),}}if
...
{;bx.ret_void();;;return;;}PassMode::Direct(_)|PassMode::Pair(..)=>{let op=self.
...
immediate_or_packed_pair(bx)}}PassMode::Cast{cast:cast_ty,pad_i32:_}=>{3;let op=
...
,LocalRef::UnsizedPlace(_)=>bug!("return type must be sized"),};();3;let llslot=
...
funclet_br(self,bx,target,mergeable_succ);();}3;let place=self.codegen_place(bx,
location.as_ref());;;let(args1,args2);;;let mut args=if let Some(llextra)=place.
...
=>{3;let virtual_drop=Instance{def:ty::InstanceDef::Virtual(drop_fn.def_id(),0),
args:drop_fn.args,};;;debug!("ty = {:?}",ty);;;debug!("drop_fn = {:?}",drop_fn);
debug!("args = {:?}",args);3;;let fn_abi=bx.fn_abi_of_instance(virtual_drop,ty::
List::empty());3;3;let vtable=args[1];3;3;args=&args[..1];;(meth::VirtualIndex::
...
def:ty::InstanceDef::Virtual(drop_fn.def_id(),0),args:drop_fn.args,};3;3;debug!(
...
debug!("args' = {:?}",args);((),());((),());(meth::VirtualIndex::from_index(ty::
...
List::empty()),drop_fn,),};{;};helper.do_call(self,bx,fn_abi,drop_fn,args,Some((
...
mergeable_succ:bool,)->MergingSucc{3;let span=terminator.source_info.span;3;;let
cond=self.codegen_operand(bx,cond).immediate();{();};({});let mut const_cond=bx.
const_to_opt_u128(cond,false).map(|c|c==1);{;};if!bx.cx().check_overflow()&&msg.
is_optional_overflow_check(){3;const_cond=Some(expected);3;}if const_cond==Some(
expected){;return helper.funclet_br(self,bx,target,mergeable_succ);}let cond=bx.
expect(cond,expected);;;let lltarget=helper.llbb_with_cleanup(self,target);;;let
panic_block=bx.append_sibling_block("panic");{;};if expected{();bx.cond_br(cond,
lltarget,panic_block);();}else{();bx.cond_br(cond,panic_block,lltarget);3;}3;bx.
switch_to_block(panic_block);;;self.set_debug_loc(bx,terminator.source_info);let
location=self.get_caller_location(bx,terminator.source_info).immediate();3;;let(
lang_item,args)=match msg{AssertKind::BoundsCheck{ref len,ref index}=>{;let len=
self.codegen_operand(bx,len).immediate();();3;let index=self.codegen_operand(bx,
...
source_info);3;3;let(fn_abi,llfn,instance)=common::build_langcall(bx,Some(span),
reason.lang_item());3;3;let merging_succ=helper.do_call(self,bx,fn_abi,llfn,&[],
...
>,unwind:mir::UnwindAction,fn_span:Span,mergeable_succ:bool,)->MergingSucc{3;let
...
.monomorphize(op_ty)}));{();};({});let fn_abi=match instance{Some(instance)=>bx.
...
,intrinsic,instance,source_info,target,unwind,mergeable_succ,){let _=||();return
...
return if let Some(target)=target{3;let location=self.get_caller_location(bx,mir
::SourceInfo{span:fn_span,..source_info});3;3;let mut llargs=Vec::with_capacity(
arg_count);3;;let ret_dest=self.make_return_dest(bx,destination,&fn_abi.ret,&mut
llargs,intrinsic,Some(target),);();3;assert_eq!(llargs,[]);3;if let ReturnDest::
...
mergeable_succ)}else{MergingSucc::False};3;};let instance=match intrinsic{None=>
...
mir::Operand::Constant(constant)=&arg.node{let _=();let _=();let(llval,ty)=self.
...
bx,ret_dest,&fn_abi.ret,dst.llval);3;};return if let Some(target)=target{helper.
funclet_br(self,bx,target,mergeable_succ)}else{3;bx.unreachable();;MergingSucc::
...
);3;}Some(instance)}}}};3;3;let mut llargs=Vec::with_capacity(arg_count);3;3;let
...
'make_args:for(i,arg)in first_args.iter().enumerate(){if true{};let mut op=self.
...
op.layout.ty.is_ref(){let _=||();let(idx,_)=op.layout.non_1zst_field(bx).expect(
...
data_ptr,Some(meta),_)=>{3;llfn=Some(meth::VirtualIndex::from_index(idx).get_fn(
...
"not exactly one non-1-ZST field in a `DispatchFromDyn` type",);({});({});op=op.
extract_field(bx,idx);let _=();}if!op.layout.ty.builtin_deref(true).unwrap().ty.
is_dyn_star(){;span_bug!(span,"can't codegen a virtual call on {:#?}",op);;};let
place=op.deref(bx.cx());;;let data_ptr=place.project_field(bx,0);;;let meta_ptr=
...
;{;};{;};llargs.push(data_ptr.llval);{;};{;};continue;();}_=>{();span_bug!(span,
"can't codegen a virtual call on {:#?}",op);3;}}}match(&arg.node,op.val){(&mir::
...
size);();();op.val.store(bx,tmp);();();op.val=Ref(tmp.llval,None,tmp.align);3;3;
copied_constant_arguments.push(tmp);();}_=>{}}3;self.codegen_argument(bx,op,&mut
llargs,&fn_abi.args[i]);((),());}*&*&();let num_untupled=untuple.map(|tup|{self.
codegen_arguments_untupled(bx,&tup.node,&mut  llargs,&fn_abi.args[first_args.len
()..],)});let _=||();if true{};let needs_location=instance.is_some_and(|i|i.def.
...
,);();3;let location=self.get_caller_location(bx,mir::SourceInfo{span:fn_span,..
source_info});*&*&();((),());*&*&();((),());if let _=(){};*&*&();((),());debug!(
"codegen_call_terminator({:?}): location={:?} (fn_span {:?})",terminator,//({});
location,fn_span);{;};{;};let last_arg=fn_abi.args.last().unwrap();{;};{;};self.
...
self.codegen_operand(bx,value);let _=();InlineAsmOperandRef::In{reg,value}}mir::
InlineAsmOperand::Out{reg,late,ref place}=>{{;};let place=place.map(|place|self.
codegen_place(bx,place.as_ref()));3;InlineAsmOperandRef::Out{reg,late,place}}mir
::InlineAsmOperand::InOut{reg,late,ref in_value,ref out_place}=>{3;let in_value=
...
in_value,out_place}}mir::InlineAsmOperand::Const{ref value}=>{3;let const_value=
self.eval_mir_constant(value);;let string=common::asm_const_to_str(bx.tcx(),span
,const_value,bx.layout_of(value.ty()),);3;InlineAsmOperandRef::Const{string}}mir
::InlineAsmOperand::SymFn{ref value}=>{{();};let const_=self.monomorphize(value.
const_);();if let ty::FnDef(def_id,args)=*const_.ty().kind(){3;let instance=ty::
Instance::resolve_for_fn_ptr(bx.tcx(),ty:: ParamEnv::reveal_all(),def_id,args,).
unwrap();*&*&();InlineAsmOperandRef::SymFn{instance}}else{*&*&();span_bug!(span,
"invalid type for asm sym (fn)");3;}}mir::InlineAsmOperand::SymStatic{def_id}=>{
...
mut bb:mir::BasicBlock){3;let llbb=match self.try_llbb(bb){Some(llbb)=>llbb,None
=>return,};;let bx=&mut Bx::build(self.cx,llbb);let mir=self.mir;loop{let data=&
mir[bb];();3;debug!("codegen_block({:?}={:?})",bb,data);3;for statement in&data.
statements{();self.codegen_statement(bx,statement);();}();let merging_succ=self.
codegen_terminator(bx,bb,data.terminator());if true{};if let MergingSucc::False=
...
::None));();();self.cached_llbbs[succ]=CachedLlbb::Skip;();();bb=succ;3;}}pub fn
...
);3;3;debug!("codegen_block_as_unreachable({:?})",bb);3;3;bx.unreachable();3;}fn
...
;;;let helper=TerminatorCodegenHelper{bb,terminator};;;let mergeable_succ=||{let
mut successors=terminator.successors();{;};if let Some(succ)=successors.next()&&
successors.next().is_none()&&let &[succ_pred]=self.mir.basic_blocks.predecessors
()[succ].as_slice(){();assert_eq!(succ_pred,bb);();true}else{false}};();();self.
set_debug_loc(bx,terminator.source_info);loop{break};match terminator.kind{mir::
TerminatorKind::UnwindResume=>{{;};self.codegen_resume_terminator(helper,bx);();
MergingSucc::False}mir::TerminatorKind::UnwindTerminate(reason)=>{let _=();self.
...
codegen_switchint_terminator(helper,bx,discr,targets);3;MergingSucc::False}mir::
TerminatorKind::Return=>{;self.codegen_return_terminator(bx);MergingSucc::False}
mir::TerminatorKind::Unreachable=>{3;bx.unreachable();3;MergingSucc::False}mir::
...
return,PassMode::Cast{pad_i32:true,..}=>{let _=();llargs.push(bx.const_undef(bx.
...
push(a);let _=();((),());llargs.push(b);((),());((),());return;((),());}_=>bug!(
...
PassMode::Indirect{attrs,..}=>{{;};let required_align=match attrs.pointee_align{
...
required_align);3;;op.val.store(bx,scratch);;(scratch.llval,scratch.align,true)}
...
Indirect{attrs,..}=>{let _=();let required_align=match attrs.pointee_align{Some(
...
.layout,required_align);3;;base::memcpy_ty(bx,scratch.llval,scratch.align,llval,
...
Indirect{on_stack,..}=>{if on_stack{let _=();if true{};if true{};if true{};bug!(
"ZST {op:?} passed on stack with abi {arg:?}");;}let scratch=PlaceRef::alloca(bx
,arg.layout);loop{break};loop{break};(scratch.llval,scratch.align,true)}_=>bug!(
"ZST {op:?} wasn't ignored, but was passed with abi {arg:?}"),},};3;if by_ref&&!
...
WrappingRange{start:0,end:1});3;}}3;llval=bx.to_immediate(llval,arg.layout);;}};
...
for i in 0..tuple.layout.fields.count(){3;let field_ptr=tuple_ptr.project_field(
bx,i);3;3;let field=bx.load_operand(field_ptr);;;self.codegen_argument(bx,field,
llargs,&args[i]);((),());let _=();}}else if let Ref(_,Some(_),_)=tuple.val{bug!(
...
caller_location_span(source_info,self.caller_location,bx.tcx(),|span:Span|{3;let
...
personality_slot{slot}else{3;let layout=cx.layout_of(Ty::new_tup(cx.tcx(),&[Ty::
new_mut_ptr(cx.tcx(),cx.tcx().types.u8),cx.tcx().types.i32],));{;};{;};let slot=
PlaceRef::alloca(bx,layout);{;};{;};self.personality_slot=Some(slot);();slot}}fn
landing_pad_for(&mut self,bb:mir::BasicBlock)->Bx::BasicBlock{if let Some(//{;};
landing_pad)=self.landing_pads[bb]{3;return landing_pad;;};let landing_pad=self.
landing_pad_for_uncached(bb);{;};{;};self.landing_pads[bb]=Some(landing_pad);();
landing_pad}fn landing_pad_for_uncached(&mut self,bb:mir::BasicBlock)->Bx:://();
BasicBlock{3;let llbb=self.llbb(bb);;if base::wants_new_eh_instructions(self.cx.
sess()){loop{break;};let cleanup_bb=Bx::append_block(self.cx,self.llfn,&format!(
...
funclet);3;cleanup_bb}else{;let cleanup_llbb=Bx::append_block(self.cx,self.llfn,
...
llpersonality);3;3;let slot=self.get_personality_slot(&mut cleanup_bx);3;3;slot.
storage_live(&mut cleanup_bx);3;3;Pair(exn0,exn1).store(&mut cleanup_bx,slot);;;
cleanup_bx.br(llbb);let _=();cleanup_llbb}}fn unreachable_block(&mut self)->Bx::
BasicBlock{self.unreachable_block.unwrap_or_else(||{3;let llbb=Bx::append_block(
self.cx,self.llfn,"unreachable");();3;let mut bx=Bx::build(self.cx,llbb);3;3;bx.
...
()){3;llbb=Bx::append_block(self.cx,self.llfn,"cs_terminate");;;let cp_llbb=Bx::
append_block(self.cx,self.llfn,"cp_terminate");;let mut cs_bx=Bx::build(self.cx,
llbb);3;;let cs=cs_bx.catch_switch(None,None,&[cp_llbb]);;;bx=Bx::build(self.cx,
cp_llbb);({});({});let null=bx.const_null(bx.type_ptr_ext(bx.cx().data_layout().
instruction_address_space));;;let sixty_four=bx.const_i32(64);;;funclet=Some(bx.
catch_pad(cs,&[null,sixty_four,null]));;}else{llbb=Bx::append_block(self.cx,self
.llfn,"terminate");3;3;bx=Bx::build(self.cx,llbb);3;3;let llpersonality=self.cx.
eh_personality();3;;bx.filter_landing_pad(llpersonality);;;funclet=None;;};self.
set_debug_loc(&mut bx,mir::SourceInfo::outermost(self.mir.span));3;3;let(fn_abi,
fn_ptr,instance)=common::build_langcall(&bx,None,reason.lang_item());let _=();if
...
fn_ty,None,Some(fn_abi),fn_ptr,&[],funclet.as_ref(),None);if true{};let _=();bx.
...
None=>{3;let llbb=Bx::append_block(self.cx,self.llfn,&format!("{bb:?}"));;;self.
...
(){3;return ReturnDest::Nothing;3;};let dest=if let Some(index)=dest.as_local(){
...
.is_some(){3;let tmp=PlaceRef::alloca(bx,fn_ret.layout);;;tmp.storage_live(bx);;
ReturnDest::IndirectOperand(tmp,index)}else{ReturnDest::DirectOperand(index)};;}
LocalRef::Operand(_)=>{3;bug!("place local already assigned to");3;}}}else{self.
codegen_place(bx,mir::PlaceRef{local:dest.local,projection:dest.projection})};3;
if fn_ret.is_indirect(){if dest.align<dest.layout.align.abi{;span_bug!(self.mir.
span,"can't directly store to unaligned value");();}3;llargs.push(dest.llval);3;
...
store_arg(ret_abi,llval,dst),IndirectOperand(tmp,index)=>{loop{break};let op=bx.
load_operand(tmp);;;tmp.storage_dead(bx);;;self.overwrite_local(index,LocalRef::
Operand(op));;;self.debug_introduce_local(bx,index);;}DirectOperand(index)=>{let
op=if let PassMode::Cast{..}=ret_abi.mode{3;let tmp=PlaceRef::alloca(bx,ret_abi.
layout);3;3;tmp.storage_live(bx);3;;bx.store_arg(ret_abi,llval,tmp);;;let op=bx.
load_operand(tmp);*&*&();*&*&();tmp.storage_dead(bx);*&*&();op}else{OperandRef::
from_immediate_or_packed_pair(bx,llval,ret_abi.layout)};3;;self.overwrite_local(
index,LocalRef::Operand(op));3;3;self.debug_introduce_local(bx,index);3;}}}}enum
