...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    fn try_codegen_const_aggregate_as_immediate(
        ...
        if matches!(kind, mir::AggregateKind::Array(_) | mir::AggregateKind::RawPtr(..)) {
            return false;
        }
        if let mir::AggregateKind::Adt(_, _, _, _, Some(_)) = kind {
            return false;
        }
        ...
        if operands.len() != dest.layout.fields.count() {
            return false;
        }
