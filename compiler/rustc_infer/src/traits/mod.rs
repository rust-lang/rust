...
pub enum FulfillmentErrorCode<'tcx> {
    /// Inherently impossible to fulfill; this trait is implemented if and only if it is already
    /// implemented.
    Cycle(Vec<PredicateObligation<'tcx>>),
    SelectionError(SelectionError<'tcx>),
    ProjectionError(MismatchedProjectionTypes<'tcx>),
    SubtypeError(ExpectedFound<Ty<'tcx>>, TypeError<'tcx>), /* always comes from a
                                                             * SubtypePredicate */
