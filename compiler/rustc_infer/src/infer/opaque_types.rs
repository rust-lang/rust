...
impl<'tcx> InferCtxt<'tcx> {
    ...
    pub fn replace_opaque_types_with_inference_vars<T: TypeFoldable<TyCtxt<'tcx>>>(
        ...
        let replace_opaque_type = |def_id: DefId| {
            def_id.as_local().map_or(false, |def_id| {
                self.opaque_type_origin(def_id, defining_use_anchor).is_some()
            })
        };
    ...
    pub fn handle_opaque_type(
        ...
        let process = |a: Ty<'tcx>, b: Ty<'tcx>, a_is_expected| match *a.kind() {
            ty::Alias(ty::Opaque, ty::AliasTy { def_id, substs, .. }) if def_id.is_local() => {
                ...
                let origin = match defining_use_anchor {
                    DefiningAnchor::Bind(_) => {
                        ...
                        // }
                        // ```
                        self.opaque_type_origin(def_id, defining_use_anchor)?
                    }
                ...
                if let ty::Alias(ty::Opaque, ty::AliasTy { def_id: b_def_id, .. }) = *b.kind() {
                    ...
                    if let Some(OpaqueTyOrigin::TyAlias) = b_def_id
                        .as_local()
                        .and_then(|b_def_id| self.opaque_type_origin(b_def_id, defining_use_anchor))
                    {
    ...
    pub fn opaque_type_origin(
        ...
        let parent_def_id = match defining_use_anchor {
