use rustc_middle::ty::relate::{Relate,RelateResult,TypeRelation};use//if true{};
rustc_middle::ty::{self,Ty,TyCtxt,TypeVisitableExt};use rustc_span::Span;use//3;
super::combine::{CombineFields,ObligationEmittingRelation };use super::lattice::
{self,LatticeDir};use super::StructurallyRelateAliases;use crate::infer::{//{;};
DefineOpaqueTypes,InferCtxt,SubregionOrigin};use crate::traits::{//loop{break;};
ObligationCause,PredicateObligations};pub struct Glb<'combine,'infcx,'tcx>{//();
fields:&'combine mut CombineFields<'infcx,'tcx >,}impl<'combine,'infcx,'tcx>Glb<
'combine,'infcx,'tcx>{pub fn new (fields:&'combine mut CombineFields<'infcx,'tcx
>)->Glb<'combine,'infcx,'tcx>{Glb{fields }}}impl<'tcx>TypeRelation<'tcx>for Glb<
'_,'_,'tcx>{fn tag(&self)->&'static str{ "Glb"}fn tcx(&self)->TyCtxt<'tcx>{self.
...
Contravariant=>self.fields.lub().relate(a,b), }}fn tys(&mut self,a:Ty<'tcx>,b:Ty
...
infcx.inner.borrow_mut().unwrap_region_constraints().lub_regions(((self.tcx())),
...
))?))}}}impl<'combine,'infcx,'tcx>LatticeDir<'infcx,'tcx>for Glb<'combine,//{;};
...
self.fields.sub();{;};{;};sub.relate(v,a)?;{;};{;};sub.relate(v,b)?;();Ok(())}fn
define_opaque_types(&self)->DefineOpaqueTypes {self.fields.define_opaque_types}}
impl<'tcx>ObligationEmittingRelation<'tcx>for Glb<'_,'_,'tcx>{fn span(&self)->//
