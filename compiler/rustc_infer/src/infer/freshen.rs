        ...
        F: FnOnce(u32) -> ty::InferConst<'tcx>,
    {
        ...
        match self.const_freshen_map.entry(key) {
            ...
            Entry::Vacant(entry) => {
                ...
                let ct = self.infcx.tcx.mk().const_(freshener(index), ty);
                entry.insert(ct);
                ct
            }
        }
    ...
    fn fold_infer_ty(&mut self, v: ty::InferTy) -> Option<Ty<'tcx>> {
        match v {
            ty::TyVar(v) => {
                ...
                Some(self.freshen_ty(opt_ty, ty::TyVar(v), |n| self.infcx.tcx.mk().fresh_ty(n)))
            }
            ...
            ty::IntVar(v) => Some(
                self.freshen_ty(
                    ...
                    ty::IntVar(v),
                    |n| self.infcx.tcx.mk().fresh_int_ty(n),
                ),
            ),
            ...
            ty::FloatVar(v) => Some(
                self.freshen_ty(
                    self.infcx
                        ...
                        .map(|v| v.to_type(self.infcx.tcx)),
                    ty::FloatVar(v),
                    |n| self.infcx.tcx.mk().fresh_float_ty(n),
                ),
            ),
