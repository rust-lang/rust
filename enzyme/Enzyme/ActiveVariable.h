...
// DataType isIntASecretFloat(const std::map<llvm::Argument*, DataType>
// typeInfo, llvm::Value* val, IntType defaultType=IntType::Unknown, bool
// errIfNotFound=false);

//! return the secret float type of val if found, otherwise nullptr
//!   if onlyFirst is set, consider only the first element of the pointer val
//!   (e.g. if we have {int, double, double}, consider only the int)
//    onlyFirst should typically be used for store operands, whereas not used
//    for memcpy
// DataType isIntPointerASecretFloat(const std::map<llvm::Argument*, DataType>
// typeInfo, llvm::Value* val, bool onlyFirst, bool errIfNotFound=true);
