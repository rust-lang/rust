...
// propagateFromOperand should consider the value that would make this call active and return true if it does make it active and thus
//   we don't need to consider any further operands
void propagateArgumentInformation(CallInst& CI, std::function<bool(Value*)> propagateFromOperand) {
    ...
    for(auto& a: CI.arg_operands()) {
        if (propagateFromOperand(a)) break;
    }
}
...
bool isconstantM(TypeResults &TR, Instruction* inst, SmallPtrSetImpl<Value*> &constants, SmallPtrSetImpl<Value*> &nonconstant, SmallPtrSetImpl<Value*> &constantvals, SmallPtrSetImpl<Value*> &retvals, AAResults &AA, uint8_t directions) {
	...
	if (auto op = dyn_cast<CallInst>(inst)) {
		if(auto called = op->getCalledFunction()) {
			if (called->getName() == "printf" || called->getName() == "puts" || called->getName() == "__assert_fail" || called->getName() == "free" || called->getName() == "_ZdlPv" || called->getName() == "_ZdlPvm"
