...
; CHECK: define internal {} @diffemat_mult(i64 %rows, i64 %cols, double* noalias nocapture readonly %lhs_data, double* %"lhs_data'", double* noalias nocapture readonly %rhs_data, double* %"rhs_data'", double* noalias nocapture %out_data, double* %"out_data'")
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %rhs_ij = getelementptr inbounds double, double* %rhs_data, i64 %1
...
; CHECK-NEXT:   %3 = phi double [ %add, %for.body23 ], [ %mul, %for.body5 ]
; CHECK-NEXT:   %iv.next4 = add nuw i64 %iv3, 1
; CHECK-NEXT:   %4 = mul nsw i64 %iv.next4, %rows
; CHECK-NEXT:   %5 = add nsw i64 %4, %iv
; CHECK-NEXT:   %lhs_ki = getelementptr inbounds double, double* %lhs_data, i64 %5
; CHECK-NEXT:   %L_lhs_ki = load double, double* %lhs_ki, align 8, !tbaa !2, !invariant.group !8
; CHECK-NEXT:   %6 = add nsw i64 %iv.next4, %1
; CHECK-NEXT:   %rhs_kk = getelementptr inbounds double, double* %rhs_data, i64 %6
; CHECK-NEXT:   %L_rhs_kk = load double, double* %rhs_kk, align 8, !tbaa !2, !invariant.group !9
; CHECK-NEXT:   %mul2 = fmul fast double %L_rhs_kk, %L_lhs_ki
; CHECK-NEXT:   %add = fadd fast double %3, %mul2
...
; CHECK-NEXT:   %8 = load double, double* %"out_iji'ipg_unwrap8", align 8
; CHECK-NEXT:   store double 0.000000e+00, double* %"out_iji'ipg_unwrap8", align 8
; CHECK-NEXT:   %9 = fadd fast double %16, %8
...
; CHECK-NEXT:   %m0diffeL_rhs_ij = fmul fast double %9, %L_lhs_i_unwrap
...
; CHECK-NEXT:   %m1diffeL_lhs_i = fmul fast double %9, %L_rhs_ij_unwrap
; CHECK-NEXT:   %"rhs_ij'ipg_unwrap" = getelementptr inbounds double, double* %"rhs_data'", i64 %_unwrap6
; CHECK-NEXT:   %10 = load double, double* %"rhs_ij'ipg_unwrap", align 8
; CHECK-NEXT:   %11 = fadd fast double %10, %m0diffeL_rhs_ij
; CHECK-NEXT:   store double %11, double* %"rhs_ij'ipg_unwrap", align 8
; CHECK-NEXT:   %"lhs_i'ipg_unwrap" = getelementptr inbounds double, double* %"lhs_data'", i64 %"iv'ac.0"
; CHECK-NEXT:   %12 = load double, double* %"lhs_i'ipg_unwrap", align 8
; CHECK-NEXT:   %13 = fadd fast double %12, %m1diffeL_lhs_i
; CHECK-NEXT:   store double %13, double* %"lhs_i'ipg_unwrap", align 8
; CHECK-NEXT:   %14 = icmp eq i64 %"iv1'ac.0", 0
; CHECK-NEXT:   br i1 %14, label %invertfor.cond2.preheader, label %incinvertfor.body5
...
; CHECK-NEXT:   %15 = load double, double* %"out_iji'ipg_unwrap8", align 8
; CHECK-NEXT:   store double 0.000000e+00, double* %"out_iji'ipg_unwrap8", align 8
; CHECK-NEXT:   %16 = fadd fast double %"add'de.0", %15
...
; CHECK-NEXT:   %m0diffeL_rhs_kk = fmul fast double %16, %L_lhs_ki_unwrap
...
; CHECK-NEXT:   %m1diffeL_lhs_ki = fmul fast double %16, %L_rhs_kk_unwrap
; CHECK-NEXT:   %"rhs_kk'ipg_unwrap" = getelementptr inbounds double, double* %"rhs_data'", i64 %_unwrap11
; CHECK-NEXT:   %17 = load double, double* %"rhs_kk'ipg_unwrap", align 8
; CHECK-NEXT:   %18 = fadd fast double %17, %m0diffeL_rhs_kk
; CHECK-NEXT:   store double %18, double* %"rhs_kk'ipg_unwrap", align 8
; CHECK-NEXT:   %"lhs_ki'ipg_unwrap" = getelementptr inbounds double, double* %"lhs_data'", i64 %_unwrap10
; CHECK-NEXT:   %19 = load double, double* %"lhs_ki'ipg_unwrap", align 8
; CHECK-NEXT:   %20 = fadd fast double %19, %m1diffeL_lhs_ki
; CHECK-NEXT:   store double %20, double* %"lhs_ki'ipg_unwrap", align 8
...
; CHECK-NEXT:   %22 = add nsw i64 %"iv3'ac.0", -1
