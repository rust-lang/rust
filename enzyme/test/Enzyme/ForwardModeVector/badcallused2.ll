...
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call i1 @fwddiffe3subf(double* %x, [3 x double*] %"x'")
...
; CHECK: define internal i1 @fwddiffe3subf(double* nocapture %x, [3 x double*] %"x'")
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   call void @fwddiffe3omegasubf(double* %x, [3 x double*] %"x'")
; CHECK-NEXT:   %13 = call i1 @fwddiffe3metasubf(double* %x, [3 x double*] %"x'")
; CHECK-NEXT:   ret i1 %13
; CHECK-NEXT: }

; CHECK: define internal void @fwddiffe3omegasubf(double* nocapture %x, [3 x double*] %"x'")
; CHECK-NEXT: entry:
...
; CHECK: define internal i1 @fwddiffe3metasubf(double* nocapture %x, [3 x double*] %"x'")
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = extractvalue [3 x double*] %"x'", 0
; CHECK-NEXT:   %"arrayidx'ipg" = getelementptr inbounds double, double* %0, i64 1
; CHECK-NEXT:   %1 = extractvalue [3 x double*] %"x'", 1
