...
; CHECK-NEXT:   %[[augret:.+]] = extractvalue { i1 } %[[augsubf]], 0
...
; CHECK-NEXT:   %1 = call {} @diffeomegasubf(double* %x, double* %"x'")
...
; CHECK: define internal {{(dso_local )?}}{} @diffeomegasubf(double* nocapture %x, double* nocapture %"x'")
; CHECK-NEXT: entry:
; CHECK-NEXT:   %[[tostore:.+]] = getelementptr inbounds double, double* %"x'", i64 1
; CHECK-NEXT:   store double 0.000000e+00, double* %[[tostore]], align 8
; CHECK-NEXT:   ret {} undef
