; RUN: %opt < %s %loadEnzyme -enzyme -enzyme_preopt=false -inline -mem2reg -O3 -S | FileCheck %s
...
; CHECK-NEXT:   %[[ret:.+]] = tail call fastcc double @diffemeta(i32 %n, i8* nonnull %[[aug]])
...
; CHECK: define internal {{(dso_local )?}}fastcc nonnull i8* @augmented_meta(double** nocapture %arrayp, double** nocapture %"arrayp'", double %x, i32 %n)
; CHECK-NEXT: entry:
...
; CHECK: define internal {{(dso_local )?}}fastcc double @diffeallocateAndSet(i32 %n, i8* nocapture %[[callp:.+]])
; CHECK-NEXT: entry:
