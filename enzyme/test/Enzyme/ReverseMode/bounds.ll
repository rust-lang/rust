...
; CHECK-NEXT:   %"add'de.0" = phi float [ %1, %mergeinvertfor.body_for.cond.cleanup.loopexit ], [ %[[esel:.+]], %incinvertfor.body ]
...
; CHECK-NEXT:   %[[esel]] = select{{( fast)?}} i1 %[[ecmp]], float 0.000000e+00, float %"add'de.0"
