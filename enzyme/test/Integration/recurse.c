...
// RUN: %clang -std=c11 -O1 -disable-llvm-optzns %s -S -emit-llvm -o - | %opt - %loadEnzyme -enzyme -S | %lli - 
...
//   note not doing O0 below as to ensure we get tbaa
// RUN: %clang -std=c11 -O1 -disable-llvm-optzns %s -S -emit-llvm -o - | %opt - %loadEnzyme -enzyme -enzyme_inline=1 -S | %lli - 
