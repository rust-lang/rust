...
//!
//! * `log!(level, ...)` - the generic logging macro, takes a level as a u32
//!                        and any related `format!` arguments
...
//!
//! If you want to check at runtime if a given logging level is enabled (e.g.
//! if the information you would want to log is expensive to produce), you
//! can use the following macro:
//!
//! * `log_enabled!(level)` - returns true if logging of the given level
//!                           is enabled
//!
//! ## Enabling logging
//!
//! Log levels are controlled on a per-module basis, and by default all
//! logging is disabled except for `error!` (a log level of 1). Logging is
//! controlled via the `RUST_LOG` environment variable. The value of this
//! environment variable is a comma-separated list of logging directives.
...
//!
//! The actual `log_level` is optional to specify. If omitted, all logging
//! will be enabled. If specified, the it must be either a numeric in the range
//! of 1-255, or it must be one of the strings `debug`, `error`, `info`, or
//! `warn`. If a numeric is specified, then all logging less than or equal to
//! that numeral is enabled. For example, if logging level 3 is active, error,
//! warn, and info logs will be printed, but debug will be omitted.
//!
...
//! they're turned off (just a load and an integer comparison). This also
//! means that if logging is disabled, none of the components of the log will
//! be executed.
