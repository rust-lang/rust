...
fn test_words_unicode() {
    ...
    let words_rev: Vec<&str> = data.words_unicode().rev().collect();
    assert_eq!(words_rev, ["em", "of", "100,000", "häd", "Märy", "reality", "In"])
}
...
fn test_split_words_uax29() {
    ...
    for &(s, w) in &test_word[..] {
        // test forward iterator
        assert!(order::equals(s.split_words_uax29(), w.iter().cloned()));

        // test reverse iterator
        assert!(order::equals(s.split_words_uax29().rev(), w.iter().rev().cloned()));
        ...
        assert!(s.split_words_uax29_indices()
                 .zip(indices.iter())
                 .all(|((l,_),m)| l == *m));

        // test backward indices iterator
        assert!(s.split_words_uax29_indices().rev()
