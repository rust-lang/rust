...
pub fn main() {
    ...
    assert_eq!(map.get(&"abc".to_owned().into_cow()), Some(&a));
    assert_eq!(map.get(&"bcd".to_owned().into_cow()), Some(&b));
    assert_eq!(map.get(&"cde".to_owned().into_cow()), Some(&c));
    assert_eq!(map.get(&"def".to_owned().into_cow()), Some(&d));
