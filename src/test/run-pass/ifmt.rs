...
impl Foo for A {
    fn fmt(_: &A, f: &mut Formatter) { f.buf.push_str("aloha"); }
}
impl Foo for B {
    fn fmt(_: &B, f: &mut Formatter) { f.buf.push_str("adios"); }
}

#[fmt="bar"]
trait Bar {
    #[allow(unused_variable)] // FIXME(#8148)
    fn fmt(b: &Self, f: &mut Formatter) { f.buf.push_str("default"); }
}
impl Bar for A {}
impl Bar for B {}
...
impl Snowman for A {
    fn fmt(_: &A, f: &mut Formatter) { f.buf.push_str("☃"); }
}
...
pub fn main() {
    ...
    t(ifmt!("{:foo}", B), "adios");
    t(ifmt!("{:π}", A), "☃");
    t(ifmt!("{:bar}", A), "default");
    t(ifmt!("{:bar}", B), "default");
