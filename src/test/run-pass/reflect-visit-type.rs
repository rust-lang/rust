...
impl TyVisitor for MyVisitor {
    ...
    fn visit_evec_uniq(&mut self, _mtbl: uint, inner: *TyDesc) -> bool {
        self.types.push(~"[");
        visit_tydesc(inner, self as &mut TyVisitor);
        self.types.push(~"]");
        true
    }
    ...
    fn visit_var(&mut self) -> bool { true }
    fn visit_var_integral(&mut self) -> bool { true }
    fn visit_param(&mut self, _i: uint) -> bool { true }
    fn visit_self(&mut self) -> bool { true }
    fn visit_type(&mut self) -> bool { true }
    fn visit_opaque_box(&mut self) -> bool { true }
    fn visit_constr(&mut self, _inner: *TyDesc) -> bool { true }
    fn visit_closure_ptr(&mut self, _ck: uint) -> bool { true }
}

fn visit_ty<T>(v: &mut MyVisitor) {
    visit_tydesc(get_tydesc::<T>(), v as &mut TyVisitor)
}
