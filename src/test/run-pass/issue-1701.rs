...
pub fn main() {
    fail_unless_eq!(noise(cat(tabby)), Some(~"meow"));
    fail_unless_eq!(noise(dog(pug)), Some(~"woof"));
    fail_unless_eq!(noise(rabbit(~"Hilbert", upright)), None);
    fail_unless_eq!(noise(tiger), Some(~"roar"));
}
