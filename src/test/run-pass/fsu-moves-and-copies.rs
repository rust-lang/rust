...
fn test0() {
    ...
    fail_unless_eq!(f.inner.copied,    1);
    fail_unless_eq!(f.inner.nopod.v, 2);

    fail_unless_eq!(b.inner.copied,    1);
    fail_unless_eq!(b.inner.nopod.v, 3);

    fail_unless_eq!(c.inner.copied,    1);
    fail_unless_eq!(c.inner.nopod.v, 4);

    // Case 2: Owned
    let f = DropMoveFoo::new(5, 6);
    let b = DropMoveFoo { inner: MoveFoo { moved: ~7, ..f.inner }};
    let c = DropMoveFoo { inner: MoveFoo { moved: ~8, ..f.inner }};
    fail_unless_eq!(f.inner.copied,    5);
    fail_unless_eq!(*f.inner.moved,    6);

    fail_unless_eq!(b.inner.copied,    5);
    fail_unless_eq!(*b.inner.moved,    7);

    fail_unless_eq!(c.inner.copied,    5);
    fail_unless_eq!(*c.inner.moved,    8);
}
...
fn test1() {
    ...
    fail_unless_eq!(b.copied,    11);
    fail_unless_eq!(*b.moved,    13);
    fail_unless_eq!(c.copied,    14);
    fail_unless_eq!(*c.moved,    12);
}
...
fn test2() {
    ...
    fail_unless_eq!(b.copied,    21);
    fail_unless_eq!(b.nopod.v, 23);
    fail_unless_eq!(c.copied,    24);
    fail_unless_eq!(c.nopod.v, 22);
}
