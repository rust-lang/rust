...
impl to_str for () {
    fn to_string_(&self) -> String { "()".to_owned() }
}
...
pub fn main() {
    assert_eq!(foo(vec!(1)), ["hi".to_owned()]);
    assert_eq!(bar::<isize, Vec<isize> >(vec!(4, 5)), ["4".to_owned(), "5".to_owned()]);
    assert_eq!(bar::<String, Vec<String> >(vec!("x".to_owned(), "y".to_owned())),
               ["x".to_owned(), "y".to_owned()]);
    assert_eq!(bar::<(), Vec<()>>(vec!(())), ["()".to_owned()]);
}
