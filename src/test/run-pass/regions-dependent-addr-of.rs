...
pub fn main() {
    ...
    let p = get_v1(&a);
    fail_unless_eq!(*p, a.value.v1);

    let p = get_v2(&a, 1);
    fail_unless_eq!(*p, a.value.v2[1]);

    let p = get_v3(&a, 1);
    fail_unless_eq!(*p, a.value.v3[1]);

    let p = get_v4(&a, 1);
    fail_unless_eq!(*p, a.value.v4.f);

    let p = get_v5(&a, 1);
    fail_unless_eq!(*p, a.value.v5.f);

    let p = get_v6_a(&a, 1);
    fail_unless_eq!(*p, a.value.v6.unwrap().f);
