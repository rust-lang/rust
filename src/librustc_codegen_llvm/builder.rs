...
// a zero-length c-string
const UNNAMED: *const c_char = &0;
...
impl BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn udiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildUDiv(self.llbuilder, lhs, rhs, UNNAMED)
        }
    }

    fn exactudiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildExactUDiv(self.llbuilder, lhs, rhs, UNNAMED)
        }
    }
    ...
    fn exactsdiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildExactSDiv(self.llbuilder, lhs, rhs, UNNAMED)
        }
    }
    ...
    fn urem(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildURem(self.llbuilder, lhs, rhs, UNNAMED)
        }
    }
    ...
    fn ashr(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildAShr(self.llbuilder, lhs, rhs, UNNAMED)
        }
    }
    ...
    fn neg(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildNeg(self.llbuilder, v, UNNAMED)
        }
    }

    fn fneg(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFNeg(self.llbuilder, v, UNNAMED)
        }
    }

    fn not(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildNot(self.llbuilder, v, UNNAMED)
        }
    }
