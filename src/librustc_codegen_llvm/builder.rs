...
const ZERO_LENGTH_CSTR: *const c_char = &0;
...
impl BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn add(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildAdd(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fadd(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFAdd(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fadd_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildFAdd(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR);
    ...
    fn sub(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildSub(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fsub(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFSub(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fsub_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildFSub(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR);
            llvm::LLVMRustSetHasUnsafeAlgebra(instr);
            instr
        }
    }
    ...
    fn mul(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildMul(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fmul(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFMul(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fmul_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildFMul(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR);
            llvm::LLVMRustSetHasUnsafeAlgebra(instr);
            instr
        }
    }
    ...
    fn udiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildUDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn exactudiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildExactUDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn sdiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildSDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn exactsdiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildExactSDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fdiv(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fdiv_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildFDiv(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR);
            llvm::LLVMRustSetHasUnsafeAlgebra(instr);
            instr
        }
    }
    ...
    fn urem(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildURem(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn srem(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildSRem(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn frem(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFRem(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn frem_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMBuildFRem(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR);
            llvm::LLVMRustSetHasUnsafeAlgebra(instr);
            instr
        }
    }
    ...
    fn shl(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildShl(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn lshr(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildLShr(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn ashr(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildAShr(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn and(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildAnd(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn or(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildOr(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn xor(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildXor(self.llbuilder, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn neg(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildNeg(self.llbuilder, v, ZERO_LENGTH_CSTR)
        }
    }

    fn fneg(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFNeg(self.llbuilder, v, ZERO_LENGTH_CSTR)
        }
    }

    fn not(&mut self, v: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildNot(self.llbuilder, v, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn dynamic_alloca(&mut self, ty: &'ll Type, name: &str, align: Align) -> &'ll Value {
        unsafe {
            let alloca = if name.is_empty() {
                llvm::LLVMBuildAlloca(self.llbuilder, ty, ZERO_LENGTH_CSTR)
            } else {
                let name = SmallCStr::new(name);
    ...
    fn array_alloca(&mut self,
        ...
        unsafe {
            let alloca = if name.is_empty() {
                llvm::LLVMBuildArrayAlloca(self.llbuilder, ty, len, ZERO_LENGTH_CSTR)
            } else {
                let name = SmallCStr::new(name);
    ...
    fn load(&mut self, ptr: &'ll Value, align: Align) -> &'ll Value {
        unsafe {
            let load = llvm::LLVMBuildLoad(self.llbuilder, ptr, ZERO_LENGTH_CSTR);
   ...
   fn atomic_store(&mut self, val: &'ll Value, ptr: &'ll Value,
    ...
    fn gep(&mut self, ptr: &'ll Value, indices: &[&'ll Value]) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildGEP(self.llbuilder, ptr, indices.as_ptr(),
                               indices.len() as c_uint, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn inbounds_gep(&mut self, ptr: &'ll Value, indices: &[&'ll Value]) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildInBoundsGEP(
                self.llbuilder, ptr, indices.as_ptr(), indices.len() as c_uint, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn struct_gep(&mut self, ptr: &'ll Value, idx: u64) -> &'ll Value {
        ...
        unsafe {
            llvm::LLVMBuildStructGEP(self.llbuilder, ptr, idx as c_uint, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn trunc(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildTrunc(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn sext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildSExt(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn fptoui(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFPToUI(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn fptosi(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFPToSI(self.llbuilder, val, dest_ty,ZERO_LENGTH_CSTR)
        }
    }

    fn uitofp(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildUIToFP(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn sitofp(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildSIToFP(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn fptrunc(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFPTrunc(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn fpext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFPExt(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn ptrtoint(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildPtrToInt(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn inttoptr(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildIntToPtr(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }

    fn bitcast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildBitCast(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn pointercast(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildPointerCast(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn icmp(&mut self, op: IntPredicate, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        ...
        unsafe {
            llvm::LLVMBuildICmp(self.llbuilder, op as c_uint, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }

    fn fcmp(&mut self, op: RealPredicate, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildFCmp(self.llbuilder, op as c_uint, lhs, rhs, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn select(
        ...
        unsafe {
            llvm::LLVMBuildSelect(self.llbuilder, cond, then_val, else_val, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn va_arg(&mut self, list: &'ll Value, ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildVAArg(self.llbuilder, list, ty, ZERO_LENGTH_CSTR)
        }
    }

    fn extract_element(&mut self, vec: &'ll Value, idx: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildExtractElement(self.llbuilder, vec, idx, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn extract_value(&mut self, agg_val: &'ll Value, idx: u64) -> &'ll Value {
        assert_eq!(idx as c_uint as u64, idx);
        unsafe {
            llvm::LLVMBuildExtractValue(self.llbuilder, agg_val, idx as c_uint, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn landing_pad(&mut self, ty: &'ll Type, pers_fn: &'ll Value,
        ...
        unsafe {
            llvm::LLVMBuildLandingPad(self.llbuilder, ty, pers_fn,
                                      num_clauses as c_uint, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn zext(&mut self, val: &'ll Value, dest_ty: &'ll Type) -> &'ll Value {
        unsafe {
            llvm::LLVMBuildZExt(self.llbuilder, val, dest_ty, ZERO_LENGTH_CSTR)
        }
    }
...
impl Builder<'a, 'll, 'tcx> {
    ...
    pub fn insert_element(
        ...
        unsafe {
            llvm::LLVMBuildInsertElement(self.llbuilder, vec, elt, idx, ZERO_LENGTH_CSTR)
        }
    }
    ...
    pub fn shuffle_vector(
        ...
        unsafe {
            llvm::LLVMBuildShuffleVector(self.llbuilder, v1, v2, mask, ZERO_LENGTH_CSTR)
        }
    }
    ...
    fn phi(&mut self, ty: &'ll Type, vals: &[&'ll Value], bbs: &[&'ll BasicBlock]) -> &'ll Value {
        ...
        let phi = unsafe {
            llvm::LLVMBuildPhi(self.llbuilder, ty, ZERO_LENGTH_CSTR)
        };
        unsafe {
