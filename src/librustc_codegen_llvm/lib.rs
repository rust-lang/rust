...
impl WriteBackendMethods for LlvmCodegenBackend {
    ...
    fn new_diagnostic_handlers<'a>(
        ...
        back::write::new_diagnostic_handlers(cgcx, handler, llcx)
    }
    fn drop_diagnostic_handlers<'a>(diag : &mut DiagnosticHandlers<'a, Self>) {
        back::write::drop_diagnostic_handlers(diag)
    }
    unsafe fn optimize(
        cgcx: &CodegenContext<Self>,
...
impl<'a> CodegenBackend for LlvmCodegenBackend {
    ...
    fn join_codegen_and_link(
        ...
        if sess.opts.debugging_opts.incremental_info {
            rustc_codegen_ssa::back::write::dump_incremental_data(&ongoing_codegen);
        }
