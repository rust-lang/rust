...
pub fn new_diagnostic_handlers<'a>(cgcx: &'a CodegenContext<LlvmCodegenBackend>,
           handler: &'a Handler,
           llcx: &'a llvm::Context) -> DiagnosticHandlers<'a, LlvmCodegenBackend> {
    let data = Box::into_raw(Box::new((cgcx, handler)));
    unsafe {
        llvm::LLVMRustSetInlineAsmDiagnosticHandler(llcx, inline_asm_handler, data as *mut _);
        llvm::LLVMContextSetDiagnosticHandler(llcx, diagnostic_handler, data as *mut _);
    }
    DiagnosticHandlers { data, llcx: llcx }
}

pub fn drop_diagnostic_handlers(diag: &mut DiagnosticHandlers<'a, LlvmCodegenBackend>) {
    use std::ptr::null_mut;
    unsafe {
        llvm::LLVMRustSetInlineAsmDiagnosticHandler(diag.llcx, inline_asm_handler, null_mut());
        llvm::LLVMContextSetDiagnosticHandler(diag.llcx, diagnostic_handler, null_mut());
        drop(Box::from_raw(diag.data));
    }
}
    ...
    -> Result<(), FatalError>
{
    ...
    let _handlers = new_diagnostic_handlers(cgcx, diag_handler, llcx);
    ...
    -> Result<CompiledModule, FatalError>
{
    timeline.record("codegen");
    {
        ...
        let module_name = module.name.clone();
        let module_name = Some(&module_name[..]);
        let handlers = new_diagnostic_handlers(cgcx, diag_handler, llcx);
