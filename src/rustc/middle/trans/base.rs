...
fn trans_tuple_struct(ccx: @crate_ctxt,
    ...
    for fields.eachi |i, field| {
        ...
        let llarg;
        match fcx.llargs.get(field.node.id) {
            local_mem(x) => llarg = x,
            _ => {
...
fn trans_struct_def(ccx: @crate_ctxt, struct_def: @ast::struct_def,
    ...
    if tps.len() == 0u {
        ...
        if struct_def.fields.len() == 0 ||
            ...
            let llfndecl = get_item_val(ccx, struct_def.ctor_id);
            trans_tuple_struct(ccx, struct_def.fields, struct_def.ctor_id,
                               None, llfndecl);
        }
    }
    ...
    match ccx.item_vals.find(id) {
      ...
      None => {
        ...
        let val = match ccx.tcx.items.get(id) {
          ...
          ast_map::node_ctor(struct_def, struct_item, struct_path) => {
            // Only register the constructor if this is a tuple-like struct.
            if struct_def.fields.len() == 0 ||
                    struct_def.fields[0].node.kind == ast::unnamed_field {
                let llfn = register_fn(ccx, struct_item.span, *struct_path,
                                       struct_def.ctor_id);
                set_inline_hint(llfn);
                llfn
            } else {
                ccx.tcx.sess.bug(~"attempt to register a constructor of a \
                                   non-tuple-like struct")
            }
          }
