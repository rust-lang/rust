...
impl<T> Arc<T> {
    ...
    pub fn new(data: T) -> Arc<T> {
        ...
        Arc { _ptr: unsafe { NonZero::new_unchecked(mem::transmute(x)) } }
    }
...
impl<T: Sync + Send> Drop for Arc<T> {
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            // once)
            let ptr: &*mut ArcInner<T> = mem::transmute(&self._ptr);
            ...
            // threads unless we are going to delete the object. This same logic applies to the
            // below `fetch_sub` to the `weak` count.
            if self.inner().strong.fetch_sub(1, Release) != 1 { return }

            // This fence is needed to prevent reordering of use of the data and deletion of the
            // data. Because it is marked `Release`, the decreasing of the reference count
            // synchronizes with this `Acquire` fence. This means that use of the data happens
            // before decreasing the reference count, which happens before this fence, which
