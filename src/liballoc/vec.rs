...
impl<T> Vec<T> {
    ...
    pub fn clear(&mut self) {
        ...
        //
        // * Setting `self.len = 0` before calling `drop_in_place` means that, if an element's
        //   `Drop` impl panics, the vector's `Drop` impl will do nothing (leaking the rest of
        //   the elements) instead of dropping some twice.
        unsafe {
            self.len = 0;
