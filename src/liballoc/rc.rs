...
impl<T, A: Alloc> Rc<T, A> {
    /// Constructs a new `Rc<T, A>` using the given allocator.
    ...
    pub fn new_in(value: T, alloc: A) -> Result<Rc<T, A>, A::Err> {
        ...
        let ptr = unsafe {
            *unique.as_mut().as_mut_ptr() = RcBox {
                strong: Cell::new(1),
                weak: Cell::new(1),
                alloc: alloc_after,
                value,
        ...
        Ok(Rc { ptr: ptr, phantom: PhantomData })
    }
...
impl<T, A: Alloc> Weak<T, A> {
    ...
    pub fn new_in() -> Weak<T, A> {
        Weak {
            ptr: NonNull::new(usize::MAX as *mut RcBox<T, A>).expect("MAX is not 0"),
        }
    }
    ...
    pub fn as_raw(this: &Self) -> *const T {
        match this.inner() {
            ...
            Some(inner) => {
                ...
                let ptr = inner as *const RcBox<T, A>;
