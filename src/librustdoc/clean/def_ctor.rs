...
pub fn get_def_from_def_id<F>(cx: &DocContext, def_id: DefId, callback: &F) -> Vec<Item>
where
    F: Fn(&dyn Fn(DefId) -> Def) -> Vec<Item>,
{
    ...
    match ty.sty {
        ...
        ty::Int(_) | ty::Uint(_) | ty::Float(_) | ty::Str | ty::Bool | ty::Char => {
            callback(&move |_: DefId| match ty.sty {
...
where
    F: Fn(&dyn Fn(DefId) -> Def, String) -> Vec<Item>,
{
