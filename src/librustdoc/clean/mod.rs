                ...
                let param_idx = (|| {
                    match p.skip_binders() {
                        ty::PredicateAtom::Trait(pred, _constness) => {
                            if let ty::Param(param) = pred.self_ty().data() {
                                return Some(param.index);
                            }
                        }
                        ty::PredicateAtom::TypeOutlives(ty::OutlivesPredicate(ty, _reg)) => {
                            if let ty::Param(param) = ty.data() {
                                return Some(param.index);
                            }
                        }
                        ty::PredicateAtom::Projection(p) => {
                            if let ty::Param(param) = p.projection_ty.self_ty().data() {
        ...
        let inner = match self.kind {
            ...
            ty::AssocKind::Fn => {
                ...
                if self.fn_has_self_parameter {
                    ...
                    } else if let ty::Ref(_, ty, _) = *self_arg_ty.data() {
                        if ty == self_ty {
    ...
    fn clean(&self, cx: &DocContext<'_>) -> Type {
        ...
        match self.kind {
            ...
            TyKind::Path(hir::QPath::TypeRelative(ref qself, ref segment)) => {
                ...
                if let ty::Projection(proj) = ty.data() {
