...
use rustc::ty::{self, AdtKind, Region, RegionVid, Ty, TyCtxt};
...
impl ItemEnum {
    ...
    pub fn is_associated(&self) -> bool {
        match *self {
            ItemEnum::TypedefItem(_, _) | ItemEnum::AssociatedTypeItem(_, _) => true,
            _ => false,
        }
    }
}
    ...
    fn clean(&self, cx: &DocContext) -> WherePredicate {
        ...
        WherePredicate::RegionPredicate {
            ...
            bounds: vec![GenericBound::Outlives(
                b.clean(cx).expect("failed to clean bounds"),
            )],
        }
    }
}
...
impl<'tcx> Clean<WherePredicate> for ty::OutlivesPredicate<Ty<'tcx>, ty::Region<'tcx>> {
    fn clean(&self, cx: &DocContext) -> WherePredicate {
        ...
        WherePredicate::BoundPredicate {
            ty: ty.clean(cx),
            bounds: vec![GenericBound::Outlives(
                lt.clean(cx).expect("failed to clean lifetimes"),
            )],
        }
    }
}
...
impl<'a, 'tcx> Clean<Generics> for (&'a ty::Generics, &'a Lrc<ty::GenericPredicates<'tcx>>) {
    fn clean(&self, cx: &DocContext) -> Generics {
        ...
        let mut where_predicates = preds
            .predicates
            .iter()
            .map(|(p, _)| p.clean(cx))
            .collect::<Vec<_>>();
    ...
    pub fn to_self(&self) -> Option<SelfTy> {
        ...
        match self.type_ {
            BorrowedRef {
                ...
                mutability,
                ref type_,
            } if type_.is_self_type() => Some(SelfBorrowed(lifetime.clone(), mutability)),
...
impl Clean<Item> for hir::ImplItem {
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.node {
            ...
            hir::ImplItemKind::Existential(ref bounds) => ExistentialItem(
                Existential {
                            ...
                            match param.kind {
                                hir::GenericParamKind::Lifetime { .. } => {
                                    ...
                                    if let Some(lt) = lifetime.cloned() {
                                        if !lt.is_elided() {
                                            let lt_def_id = cx.tcx.hir().local_def_id(param.id);
    ...
    fn clean(&self, cx: &DocContext) -> Type {
        match self.sty {
            ...
            ty::Dynamic(ref obj, ref reg) => {
                ...
                reg.clean(cx)
                    .map(|b| typarams.push(GenericBound::Outlives(b)));
                ...
                for pb in obj.projection_bounds() {
                    bindings.push(TypeBinding {
                        name: cx
                            .tcx
                            .associated_item(pb.item_def_id())
                            .ident
                            .name
                            .clean(cx),
...
pub fn register_def(cx: &DocContext, def: Def) -> DefId {
    ...
    let (did, kind) = match def {
        ...
        Def::Variant(i) => (
            cx.tcx.parent_def_id(i).expect("cannot get parent def id"),
            TypeKind::Enum,
        ),
