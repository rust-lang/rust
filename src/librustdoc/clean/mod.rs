...
fn clean_proc_macro<'tcx>(
    ...
    let Some((trait_name, helper_attrs)) = find_attr!(attrs, AttributeKind::ProcMacroDerive(hir::attrs::ProcMacroDerive { trait_name, helper_attrs, ..}) => (*trait_name, helper_attrs))
    else {
        return ProcMacroItem(ProcMacro { kind, helpers: vec![] });
    };
    *name = trait_name;
