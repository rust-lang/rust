...
use doctree;
use fold::DocFolder;
use ascii::format::{ConstnessSpace};
use ascii::format::{TyParamBounds, WhereClause, href, AbiSpace};
use ascii::format::{VisSpace, Method, UnsafetySpace, MutableSpace, AsciiDisplay};
use ascii::item_type::ItemType;
use html::markdown::{self, Markdown};
use ascii::{highlight, layout};
    ...
    for (&did, imps) in &cache.implementors {
        ...
        for imp in imps {
            ...
            try!(write!(&mut f, r#""impl{} {}{} for {}","#,
                        imp.generics.get_display(),
                        if imp.polarity == Some(clean::ImplPolarity::Negative) { "!" } else { "" },
                        imp.trait_.get_display(), imp.for_.get_display()));
        }
        try!(writeln!(&mut f, r"];"));
        try!(writeln!(&mut f, "{}", r"
        ...
        match item.inner {
            ...
            clean::ModuleItem(..) => {
                ...
                self.recurse(name, |this| {
                    ...
                    let dst = this.dst.join("index.ascii");
...
impl<'a> Item<'a> {
    ...
    fn href(&self, cx: &Context) -> Option<String> {
        ...
        if let Some(krate) = imported_macro_from {
            ...
            Some(format!("{root}/{krate}/macro.{name}.ascii?gotomacrosrc=1",
                         root = root,
                         krate = krate,
        ...
        // has anchors for the line numbers that we're linking to.
            ...
            Some(format!("{root}src/{krate}/{path}.ascii#{href}",
...
impl<'a> fmt::Display for Item<'a> {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !is_primitive {
            ...
            for (_, component) in cur.iter().enumerate().take(amt) {
                try!(write!(fmt, "{}{}{}::",
                            RED,
                            component,
                            NORMAL));
...
fn item_path(item: &clean::Item) -> String {
    match item.inner {
        clean::ModuleItem(..) => {
            format!("{}/index.ascii", item.name.as_ref().unwrap())
        }
        _ => {
            format!("{}.{}.ascii",
    ...
    for &idx in &indices {
        ...
        match myitem.inner {
                ...
                match *src {
                    Some(ref src) => {
                        /*try!(write!(w, "<tr><td><code>{}extern crate {} as {};",
                                    ...
                                    src,
                                    name))*/
                        try!(write!(w, "{vis}extern crate {brown}{src}{normal} as {brown}{name}{normal};",
                            vis = VisSpace(myitem.visibility).get_display(),
                            src = src,
                            name = name,
                            brown = BROWN,
                            normal = NORMAL))
                    ...
                    None => {
                        ...
                        try!(write!(w, "{}extern crate {}{}{};",
                            VisSpace(myitem.visibility).get_display(),
                            BROWN,
                            name,
                            NORMAL));
                    }
            ...
            clean::ImportItem(ref import) => {
                ...
                try!(write!(w, "{}{}",
                            VisSpace(myitem.visibility).get_display(), *import));
            }
            ...
            _ => {
                ...
                try!(write!(w, "{brown}{name}{normal}{stab_docs} {docs}",
                brown = BROWN,
                normal = NORMAL,
                name = *myitem.name.as_ref().unwrap(),
                stab_docs = stab_docs,
    ...
    item.stability.as_ref().and_then(|stab| {
        ...
        let text = if !stab.deprecated_since.is_empty() {
            let since = if show_reason {
                format!(" since {}", &stab.deprecated_since)
            } else {
                String::new()
            };
        ...
        Some(format!("{}|{}| {}{}",
                     BLUE, item.stability_class(), text, NORMAL))
    })
}
...
fn item_constant(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    try!(write!(w, "{vis}const \
                    {name}: {typ}{init}",
           vis = VisSpace(it.visibility).get_display(),
           name = it.name.as_ref().unwrap(),
           typ = c.type_.get_display(),
           init = Initializer(&c.expr)));
    document(w, it)
}
...
fn item_static(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    try!(write!(w, "{vis}static {mutability}\
                    {name}: {typ}{init}",
           vis = VisSpace(it.visibility).get_display(),
           mutability = MutableSpace(s.mutability).get_display(),
           name = it.name.as_ref().unwrap(),
           typ = s.type_.get_display(),
           init = Initializer(&s.expr)));
    document(w, it)
}
...
fn item_function(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    try!(write!(w, "\n\n{red}{vis}{unsafety}{abi}{constness}fn \
                    {name}{generics}{decl}{where_clause}{normal}",
           red = RED,
           vis = VisSpace(it.visibility).get_display(),
           unsafety = UnsafetySpace(f.unsafety).get_display(),
           abi = AbiSpace(f.abi).get_display(),
           constness = ConstnessSpace(f.constness).get_display(),
           name = it.name.as_ref().unwrap(),
           generics = f.generics.get_display(),
           where_clause = WhereClause(&f.generics).get_display(),
           decl = f.decl,
           normal = NORMAL));
    document(w, it)
}
...
fn item_trait(w: &mut fmt::Formatter, cx: &Context, it: &clean::Item,
    ...
    try!(write!(w, "{}{}{}trait {}{}{}{}{} ",
                  ...
                  UnsafetySpace(t.unsafety).get_display(),
                  it.name.as_ref().unwrap(),
                  t.generics.get_display(),
                  bounds,
                  WhereClause(&t.generics).get_display(),
                  NORMAL));
        ...
        Some(implementors) => {
            for i in implementors {
                ...
                try!(writeln!(w, "{}impl{} {} for {}{}{}",
                              BLUE, i.generics.get_display(), i.trait_.get_display(),
                              i.for_.get_display(),
                              WhereClause(&i.generics).get_display(), NORMAL));
            }
        }
        None => {}
    }
...
fn assoc_type(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    if let Some(ref default) = *default {
        try!(write!(w, " = {}", default.get_display()));
    }
    Ok(())
}
...
fn render_assoc_item(w: &mut fmt::Formatter, meth: &clean::Item,
    ...
    fn method(w: &mut fmt::Formatter,
        ...
        write!(w, "{}{}{}fn {name}\
               ...
               UnsafetySpace(unsafety).get_display(),
               ConstnessSpace(constness).get_display(),
               match abi {
                   Abi::Rust => String::new(),
                   a => format!("extern {} ", a.to_string())
               },
               name = name,
               generics = (*g).get_display(),
               decl = Method(selfty, d).get_display(),
               where_clause = WhereClause(g).get_display())
    }
    match meth.inner {
    ...
    if let doctree::Plain = s.struct_type {
        if fields.peek().is_some() {
            ...
            for field in fields {
                ...
                try!(write!(w, "{}{}{}\n", GREEN, field.name.as_ref().unwrap(), NORMAL));
...
fn item_enum(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    if !e.variants.is_empty() {
        ...
        for variant in &e.variants {
            ...
            try!(write!(w, "{}{}{}\n", BLUE, variant.name.as_ref().unwrap(), NORMAL));
                    ...
                    match var.kind {
                        clean::StructVariant(ref s) => {
                            ...
                            for field in fields {
                                ...
                                try!(write!(w, "{}{}{}\n", RED, field.name.as_ref().unwrap(), NORMAL));
                                try!(document(w, field));
                                //try!(write!(w, "</td></tr>"));
                            }
                            //try!(write!(w, "</table>"));
...
fn render_struct(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    try!(write!(w, "{}{}{}",
                  ...
                  it.name.as_ref().unwrap()));
    match g {
        Some(g) => try!(write!(w, "{}{}", (*g).get_display(), WhereClause(g).get_display())),
        None => {}
    }
    match ty {
        doctree::Plain => {
            ...
            for field in fields {
                match field.inner {
                    ...
                    clean::StructFieldItem(clean::TypedStructField(ref ty)) => {
                        try!(write!(w, "    {}{}: {},\n{}",
                                      VisSpace(field.visibility).get_display(),
        ...
        doctree::Tuple | doctree::Newtype => {
            ...
            for (i, field) in fields.iter().enumerate() {
                ...
                match field.inner {
                    ...
                    clean::StructFieldItem(clean::TypedStructField(ref ty)) => {
                        try!(write!(w, "{}{}", VisSpace(field.visibility).get_display(),
                            (*ty).get_display()))
                    }
                    _ => unreachable!()
                }
            ...
            clean::TypedefItem(ref tydef, _) => {
                ...
                /*try!(write!(w, "<h4 id='assoc_type.{}' class='{}'><code>",
                            *name,
                            shortty(item)));*/
                try!(write!(w, "{}", RED));
                try!(write!(w, "type {} = {}", name, tydef.type_.get_display()));
                //try!(write!(w, "</code></h4>\n"));
                try!(write!(w, "{}\n", NORMAL));
            }
            clean::AssociatedConstItem(ref ty, ref default) => {
...
fn item_typedef(w: &mut fmt::Formatter, it: &clean::Item,
    ...
    /*try!(write!(w, "<pre class='rust typedef'>type {}{}{where_clause} = {type_};</pre>",
                  ...
                  type_ = t.type_));*/
    try!(write!(w, "{blue}type{normal} {green}{name}{normal}{brown}{generics}{normal}\
                    {magenta}{where_clause}{normal} = {cyan}{type_}{normal};",
                  blue = BLUE,
                  normal = NORMAL,
                  green = GREEN,
                  brown = BROWN,
                  ...
                  cyan = CYAN,
                  name = it.name.as_ref().unwrap(),
                  generics = t.generics.get_display(),
                  where_clause = WhereClause(&t.generics).get_display(),
                  type_ = t.type_.get_display()));
    document(w, it)
}
