...
impl<'a, 'tcx, 'rcx> RustdocVisitor<'a, 'tcx, 'rcx> {
    ...
    pub fn visit(&mut self, krate: &hir::Crate) {
        ...
        let macro_exports: Vec<_> = krate
            .exported_macros
            .iter()
            .map(|def| self.visit_local_macro(def, None))
            .collect();
    ...
    fn maybe_inline_local(
        ...
        let ret = match tcx.hir().get(def_node_id) {
            ...
            Node::MacroDef(def) if !glob => {
                om.macros
                    .push(self.visit_local_macro(def, renamed.map(|i| i.name)));
                true
            }
            _ => false,
        };
    ...
    pub fn visit_item(&mut self, item: &hir::Item, renamed: Option<ast::Ident>, om: &mut Module) {
        ...
        match item.node {
            ...
            hir::ItemKind::Use(ref path, kind) => {
                ...
                if item.vis.node.is_pub() && self.inside_public_path {
                    ...
                    if self.maybe_inline_local(item.id, path.def, ident, is_glob, om, please_inline)
                    {
                        return;
                    }
                }
            ...
            hir::ItemKind::Enum(ref ed, ref gen) => om
                .enums
                .push(self.visit_enum_def(item, ident.name, ed, gen)),
            hir::ItemKind::Struct(ref sd, ref gen) => om
                .structs
                .push(self.visit_variant_data(item, ident.name, sd, gen)),
            hir::ItemKind::Union(ref sd, ref gen) => om
                .unions
                .push(self.visit_union_data(item, ident.name, sd, gen)),
    ...
    fn visit_local_macro(&self, def: &hir::MacroDef, renamed: Option<ast::Name>) -> Macro {
