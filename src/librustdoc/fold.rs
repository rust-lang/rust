...
pub trait DocFolder {
    ...
    fn fold_item_recur(&mut self, item: Item) -> Option<Item> {
        ...
        let inner = match inner {
            StructItem(mut i) => {
                ...
                i.fields.extend(foo.iter_owned().filter_map(|x| self.fold_item(x)));
            ...
            EnumItem(mut i) => {
                ...
                i.variants.extend(foo.iter_owned().filter_map(|x| self.fold_item(x)));
            ...
            TraitItem(mut i) => {
                ...
                i.methods.extend(foo.iter_owned().filter_map(|x| vtrm(self, x)));
                TraitItem(i)
            },
            ImplItem(mut i) => {
                let mut foo = Vec::new(); swap(&mut foo, &mut i.methods);
                i.methods.extend(foo.iter_owned().filter_map(|x| self.fold_item(x)));
                ImplItem(i)
            },
            VariantItem(i) => {
                ...
                match i.kind {
                    StructVariant(mut j) => {
                        ...
                        j.fields.extend(foo.iter_owned().filter_map(c));
    ...
    fn fold_mod(&mut self, m: Module) -> Module {
        Module {
            is_crate: m.is_crate,
            items: m.items.iter_owned().filter_map(|i| self.fold_item(i)).collect()
        }
    }
