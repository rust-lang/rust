    ...
    fn variant_field<'path>(
        ...
        match ty_res {
            Res::Def(DefKind::Enum | DefKind::TyAlias, did) => {
                match tcx.type_of(did).instantiate_identity().skip_normalization().kind() {
...
fn resolve_self_ty<'tcx>(
    ...
    match tcx.def_kind(self_id) {
        DefKind::Impl { .. } => {
            ty_to_res(tcx, tcx.type_of(self_id).instantiate_identity().skip_normalization())
        }
        DefKind::Use => None,
    ...
    match root_res {
        Res::Def(DefKind::TyAlias, alias_did) => {
            ...
            let Some(aliased_res) =
                ty_to_res(tcx, tcx.type_of(alias_did).instantiate_identity().skip_normalization())
            else {
                return vec![];
            };
            let aliased_items =
...
fn resolve_assoc_on_adt<'tcx>(
    ...
    let adt_ty = tcx.type_of(adt_def_id).instantiate_identity().skip_normalization();
...
fn resolve_assoc_on_simple_type<'tcx>(
    ...
    let ty = tcx.type_of(ty_def_id).instantiate_identity().skip_normalization();
            ...
            for mut failure in kinds {
                ...
                }) = &mut failure
                {
                    ...
                    let is_struct_variant = |did| {
                        if let ty::Adt(def, _) =
                            tcx.type_of(did).instantiate_identity().skip_normalization().kind()
                            && def.is_enum()
                            && let Some(variant) =
