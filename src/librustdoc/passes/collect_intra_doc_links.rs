...
pub const COLLECT_INTRA_DOC_LINKS: Pass = Pass::early(
...
impl<'a, 'tcx, 'rcx> DocFolder for LinkCollector<'a, 'tcx, 'rcx> {
    fn fold_item(&mut self, mut item: Item) -> Option<Item> {
        ...
        let current_item = match item.inner {
            ...
            ImplItem(Impl { ref for_, .. }) => for_
                .def_id()
                .map(|did| self.cx.tcx.item_name(did).to_string()),
...
fn macro_resolve(cx: &DocContext, path_str: &str) -> Option<Def> {
    ...
    if let Ok(def) =
        resolver.resolve_macro_to_def_inner(&path, MacroKind::Bang, &parent_scope, false, false)
    {
...
pub fn span_of_attrs(attrs: &Attributes) -> syntax_pos::Span {
    ...
    let end = attrs
        .doc_strings
        .last()
        .expect("No doc strings provided")
        .span();
    start.to(end)
}
...
fn resolution_failure(
    ...
    diag.help(
        "to escape `[` and `]` characters, just add '\\' before them like \
         `\\[` or `\\]`",
    );
    diag.emit();
}
...
const PRIMITIVES: &[(&str, Def)] = &[
    (
        "u8",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::U8)),
    ),
    (
        "u16",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::U16)),
    ),
    (
        "u32",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::U32)),
    ),
    (
        "u64",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::U64)),
    ),
    (
        "u128",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::U128)),
    ),
    (
        "usize",
        Def::PrimTy(hir::PrimTy::Uint(syntax::ast::UintTy::Usize)),
    ),
    ("i8", Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::I8))),
    (
        "i16",
        Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::I16)),
    ),
    (
        "i32",
        Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::I32)),
    ),
    (
        "i64",
        Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::I64)),
    ),
    (
        "i128",
        Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::I128)),
    ),
    (
        "isize",
        Def::PrimTy(hir::PrimTy::Int(syntax::ast::IntTy::Isize)),
    ),
    (
        "f32",
        Def::PrimTy(hir::PrimTy::Float(syntax::ast::FloatTy::F32)),
    ),
    (
        "f64",
        Def::PrimTy(hir::PrimTy::Float(syntax::ast::FloatTy::F64)),
    ),
