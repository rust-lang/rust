...
pub(crate) fn id_from_item(item: &clean::Item, tcx: TyCtxt<'_>) -> Id {
    match *item.kind {
        clean::ItemKind::UseItem(ref import) => {
            let extra =
...
fn from_clean_item(item: clean::Item, tcx: TyCtxt<'_>) -> ItemEnum {
    ...
    match *item.kind {
        ...
        UseItem(i) => ItemEnum::Use(i.into_tcx(tcx)),
        ...
        ExternTypeItem => ItemEnum::ExternType,
...
fn convert_abi(a: RustcAbi) -> Abi {
    match a {
        ...
        RustcAbi::C { unwind } => Abi::C { can_unwind: unwind },
        RustcAbi::Cdecl { unwind } => Abi::Cdecl { can_unwind: unwind },
        RustcAbi::Stdcall { unwind } => Abi::Stdcall { can_unwind: unwind },
        RustcAbi::Fastcall { unwind } => Abi::Fastcall { can_unwind: unwind },
        RustcAbi::Aapcs { unwind } => Abi::Aapcs { can_unwind: unwind },
        RustcAbi::Win64 { unwind } => Abi::Win64 { can_unwind: unwind },
        RustcAbi::SysV64 { unwind } => Abi::SysV64 { can_unwind: unwind },
        RustcAbi::System { unwind } => Abi::System { can_unwind: unwind },
...
impl FromWithTcx<clean::GenericParamDefKind> for GenericParamDefKind {
    fn from_tcx(kind: clean::GenericParamDefKind, tcx: TyCtxt<'_>) -> Self {
        ...
        match kind {
            ...
            Type { bounds, default, is_synthetic } => GenericParamDefKind::Type {
                bounds: bounds.into_tcx(tcx),
                default: default.map(|x| (*x).into_tcx(tcx)),
                is_synthetic,
            },
            Const { ty, default, is_synthetic: _ } => GenericParamDefKind::Const {
                ...
                generic_params: bound_params
                    ...
                    .map(|x| {
                        ...
                        let kind = match x.kind {
                            ...
                            clean::GenericParamDefKind::Type { bounds, default, is_synthetic } => {
                            ...
                            clean::GenericParamDefKind::Const { ty, default, is_synthetic: _ } => {
...
impl FromWithTcx<clean::FunctionSignature> for FunctionSignature {
    fn from_tcx(decl: clean::FunctionSignature, tcx: TyCtxt<'_>) -> Self {
        let clean::FunctionSignature { inputs, output, is_c_variadic } = decl;
        FunctionSignature {
            inputs: inputs
                .values
                .into_iter()
...
impl FromWithTcx<clean::Use> for Use {
    fn from_tcx(import: clean::Use, tcx: TyCtxt<'_>) -> Self {
        use clean::UseKind::*;
