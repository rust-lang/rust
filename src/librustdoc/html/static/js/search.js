...
class DocSearch {
    ...
    async getTypeData(id) {
        ...
        let invertedFunctionSignatureIndex = RoaringBitmap.empty();

        if (typeof raw[0] === "string") {
            if (raw[1]) {
                searchUnbox = true;
            }
            invertedFunctionSignatureIndex = raw[0] === undefined ?
                RoaringBitmap.empty() :
                new RoaringBitmap(makeUint8ArrayFromBase64(raw[0]));
        } else if (raw[0]) {
            searchUnbox = true;
        }
    ...
    async execQuery(parsedQuery, filterCrates, currentCrate) {
         ...
         */
        function containsTypeFromQuery(elems, returned, list, where_clause) {
            if (!list) return false;
            for (const ty of returned) {
            ...
            async function*(currentCrate) {
                ...
                if (results) {
                    for await (const result of results.prefixMatches()) {
                        ...
                        if (prefixResults.length > MAX_RESULTS && (yield* flush(prefixResults))) {
                            return;
                        }
                    }
                    if (yield* flush(prefixResults)) {
        ...
        const innerRunTypeQuery =
            ...
            async function*(inputs, output, typeInfo, currentCrate) {
                ...
                /** @type {[stringdex.RoaringBitmap, rustdoc.QueryElement][]} */
                 ...
                 * @returns {Promise<[stringdex.RoaringBitmap, rustdoc.QueryElement][]>}
                 */
                const unpackPostingsList = async elem => {
                    ...
                    const types = (await Promise.all(typePromises))
                        .filter(([_id, name, ty, path]) =>
                            ...
                            ty && !ty.invertedFunctionSignatureIndex.isEmpty() &&
                    ...
                    if (types.length === 0) {
                            ...
                            for await (const searchResults of levResults) {
                                for (const id of searchResults.matches().entries()) {
                                    ...
                                    if (name !== null && ty !== null && path !== null &&
                                        !ty.invertedFunctionSignatureIndex.isEmpty() &&
                        ...
                        if (areGenericsAllowed) {
                            ...
                            return [[
                                await this.getGenericInvertedIndex(genericId),
                                {
                                    name: elem.name,
                                    id: (-genericId) - 1,
                    ...
                    for (const [id, _name, typeData] of types) {
                        if (!typeData || typeData.invertedFunctionSignatureIndex.isEmpty()) {
                            continue;
                        }
                        const upla = await unpackPostingsListAll(elem.generics);
                        const uplb = await unpackPostingsListBindings(elem.bindings);
                        for (const [genericsPostingList, generics] of upla) {
                            for (const [bindingsPostingList, bindings] of uplb) {
                                results.push([
                                    typeData.invertedFunctionSignatureIndex
                                        .intersection(bindingsPostingList)
                                        .intersection(genericsPostingList),
                                    {
                                        name: elem.name,
                                        id,
                                        typeFilter,
                ...
                /** @type {[stringdex.RoaringBitmap, rustdoc.QueryElement[]][]} */
                const empty_nested_postings_list = [[RoaringBitmap.everything(), []]];
                /**
                ...
                const unpackPostingsListAll = async elems => {
                    if (!elems || elems.length === 0) {
                        return empty_nested_postings_list;
                    }
                    ...
                    /** @type {[stringdex.RoaringBitmap, rustdoc.QueryElement[]][]} */
                    const results = [];
                    for (const [firstBitmap, firstElem] of firstPostingsList) {
                        for (const [remainingBitmap, remainingElems] of remainingAll) {
                            results.push([
                                firstBitmap.intersection(remainingBitmap),
                                [firstElem, ...remainingElems],
                            ]);
                            if ((results.length & 0x7F) === 0) {
                                await yieldToBrowser();
                            }
                ...
                const unpackPostingsListBindings = async elems => {
                    ...
                    if (!firstKeyIds) {
                        ...
                        return [[RoaringBitmap.empty(), new Map()]];
                    }
                    ...
                    /** @type {[stringdex.RoaringBitmap, Map<number, rustdoc.QueryElement[]>][]} */
                    const results = [];
                ...
                for (const [inputsPostingList, inputs] of allInputs) {
                    for (const [outputPostingList, output] of allOutput) {
                        ...
                        for (const id of postingList.entries()) {
                            checkCounter += 1;
