...
function initSearch(rawSearchIndex) {
    ...
    function execQuery(parsedQuery, searchWords, filterCrates, currentCrate) {
        ...
        function checkType(row, elem, literalSearch, maxLevDistance) {
            ...
            if (literalSearch) {
                ...
                if (elem.generics.length === 0) {
                    ...
                    // the generics!
                    lev = checkIfInGenerics(row, elem, maxLevDistance);
         ...
         * @param {integer} lev
         */
        function addIntoResults(results, fullId, id, index, lev, maxLevDistance) {
            if (lev === 0 || (!parsedQuery.literalSearch && lev <= maxLevDistance)) {
        ...
        function handleSingleArg(
            ...
            addIntoResults(results_in_args, fullId, pos, index, in_args, maxLevDistance);
            addIntoResults(results_returned, fullId, pos, index, returned, maxLevDistance);
            ...
            if (parsedQuery.literalSearch) {
                if (searchWord === elem.name) {
                    addIntoResults(results_others, fullId, pos, -1, 0, maxLevDistance);
                }
                return;
            }
            ...
            if (elem.name.length === 0) {
                if (row.type !== null) {
                    ...
                    addIntoResults(results_others, fullId, pos, index, lev, maxLevDistance);
                }
                return;
            }

            if (elem.fullPath.length > 1) {
                lev = checkPath(elem.pathWithoutLast, row, maxLevDistance);
                if (lev > maxLevDistance || (parsedQuery.literalSearch && lev !== 0)) {
                    return;
                } else if (lev > 0) {
                    lev_add = lev / 10;
                }
        ...
        function handleArgs(row, pos, results, maxLevDistance) {
            ...
            addIntoResults(results, row.id, pos, 0, lev, maxLevDistance);
        }
        ...
        function innerRunQuery() {
            ...
            const maxLevDistance = Math.ceil(queryLen / 3);
