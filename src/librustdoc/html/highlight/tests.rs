...
fn test_dos_backline() {
    ...
    println!(\"foo\");\r\n\
        ...
        expect_file!["fixtures/dos_line.html"]
            .assert_eq(&write_code(src, None, None, None).to_string());
    });
}
...
let y = Self::whatever;";
        ...
        expect_file!["fixtures/highlight.html"]
            .assert_eq(&write_code(src, None, None, None).to_string());
    });
}
...
fn test_union_highlighting() {
    create_default_session_globals_then(|| {
        ...
        expect_file!["fixtures/union.html"]
            .assert_eq(&write_code(src, None, None, None).to_string());
    });
}
...
let a = 4;";
        ...
        expect_file!["fixtures/decorations.html"].assert_eq(
            &write_code(src, None, Some(&DecorationInfo(decorations)), None).to_string(),
        );
    });
}
