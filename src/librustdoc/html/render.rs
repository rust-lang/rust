...
use clean::{self, AttributesExt, Deprecation, GetDefId, Mutability, SelfTy};
use config::RenderOptions;
use doctree;
use fold::DocFolder;
use html::escape::Escape;
use html::format::fmt_impl_for_trait_page;
use html::format::{href, AbiSpace, GenericBounds, WhereClause};
use html::format::{AsyncSpace, ConstnessSpace};
use html::format::{Method, MutableSpace, UnsafetySpace, VisSpace};
use html::item_type::ItemType;
use html::markdown::{self, ErrorCodes, IdMap, Markdown, MarkdownHtml, MarkdownSummaryLine};
...
fn build_index(krate: &clean::Crate, cache: &mut Cache) -> String {
    ...
    let crate_doc = krate
        .module
        .as_ref()
        .map(|module| plain_summary_line_short(module.doc_value()))
...
fn write_shared(
    ...
    write_minify(
        cx.dst
            .join(&format!("noscript{}.css", cx.shared.resource_suffix)),
    ...
    write(
        cx.dst
            .join(&format!("down-arrow{}.svg", cx.shared.resource_suffix)),
...
[{}].forEach(function(item) {{
    ...
    if cx.shared.include_sources {
        write_minify(
            cx.dst
                .join(&format!("source-script{}.js", cx.shared.resource_suffix)),
    ...
    impl Hierarchy {
        ...
        fn to_json_string(&self) -> String {
            ...
            format!(
                "{{\"name\":\"{name}\",\"dirs\":[{subs}],\"files\":[{files}]}}",
            ...
            loop {
                ...
                if elems.peek().is_none() {
                    ...
                    h.children
                        ...
                        .or_insert_with(|| Hierarchy::new(e));
    ...
    let mut w = try_err!(File::create(&dst), &dst);
    try_err!(
        writeln!(&mut w, "var N = null;var searchIndex = {{}};"),
        &dst
    );
    ...
    if options.enable_index_page {
        if let Some(index_page) = options.index_page.clone() {
            ...
            let content = format!(
                ...
                krates
                    .iter()
                    .map(|s| format!("<li><a href=\"{}/index.html\">{}</li>", s, s))
                    .collect::<String>()
            );
            try_err!(
                layout::render(
    ...
    fn fold_item(&mut self, item: clean::Item) -> Option<clean::Item> {
        ...
        let ret = self.fold_item_recur(item).and_then(|item| {
            ...
            } = item
            {
                ...
                if impl_item
                    .trait_did()
                    .map_or(true, |d| self.traits.contains_key(&d))
                {
                    for did in dids {
                        self.impls
                            .entry(did)
...
fn render_markdown(
    ...
    write!(
        ...
        prefix,
        Markdown(md_text, &links, RefCell::new(&mut ids), cx.codes)
    )
}
...
fn document_non_exhaustive(w: &mut fmt::Formatter, item: &clean::Item) -> fmt::Result {
    if item.is_non_exhaustive() {
        write!(
            w,
            "<div class='docblock non-exhaustive non-exhaustive-{}'>",
            {
                if item.is_struct() {
                    "struct"
...
fn name_key(name: &str) -> (&str, u64, usize) {
    let end = name
        ...
        .map_or(name.len(), |i| i + 1);

    // find number at end
    let split = name[0..end]
        .bytes()
        .rposition(|b| !b.is_ascii_digit())
        .map_or(0, |i| i + 1);

    // count leading zeroes
    let after_zeroes = name[split..end]
        .bytes()
        .position(|b| b != b'0')
...
fn item_trait(
    ...
    fn trait_item(
        ...
        write!(
            w,
            "{extra}<h3 id='{id}' class='method'><code id='{ns_id}'>",
    ...
    if !e.variants.is_empty() {
        ...
        for variant in &e.variants {
            ...
            }) = variant.inner
            {
                ...
                write!(
                    w,
                    "<span class='autohide sub-variant' id='{id}'>",
                    id = variant_id
                )?;
                write!(
                    w,
        ...
        match item.inner {
            ...
            | clean::TyMethodItem(clean::TyMethod { ref decl, .. }) => {
                ...
                if render_method_item {
                    ...
                    write!(
                        w,
                        "<table id='{}' class='table-display'><tbody><tr><td><code>",
                        ns_id
                    )?;
...
fn item_existential(
    ...
    write!(
        w,
        "existential type {}{}{where_clause}: {bounds};</pre>",
        it.name.as_ref().unwrap(),
        t.generics,
        where_clause = WhereClause {
            gens: &t.generics,
    ...
    if let Some(v) = c.impls.get(&it.def_id) {
...

        {
            ...
            let ret = v
                ...
                .flat_map(move |i| {
                    get_methods(i.inner_impl(), false, &mut used_links_bor.borrow_mut())
                })
                .collect::<String>();
            if !ret.is_empty() {
                out.push_str(&format!(
                    ...
                    .next()
                {
                    ...
                    if let Some(impls) = inner_impl {
                        ...
                        let ret = impls
                            .iter()
                            .filter(|i| i.inner_impl().trait_.is_none())
                            .flat_map(|i| get_methods(i.inner_impl(), true, &mut used_links))
                            .collect::<String>();
