...
fn item_module(w: &mut Writer, cx: &Context,
    ...
    local_data::get(cache_key, |cache| {
        ...
        // try to resolve the use view paths, and returns a copy of
        // any unresolved paths. resolved paths go to the indices.
        let resolve_view_paths = |indices: &mut Vec<(ItemType, Indexed, uint)>,
                                  vps: &Vec<clean::ViewPath>,
                                  pos: uint| -> Vec<clean::ViewPath> {
                ...
                match vp {
                    ...
                    &clean::ImportList(ref src, ref idents) => {
                        ...
                        if !unresolved_idents.is_empty() {
                            unresolved.push(clean::ImportList(src.clone(),
                                                              unresolved_idents));
                        }
                    }
                }
                ...
                clean::ViewItemItem(ref v) => {
                    match v.inner {
                        clean::Import(ref vps) => {
                            let unresolved = resolve_view_paths(&mut indices, vps, i);
                            if !unresolved.is_empty() {
                                // reconstructs the item out of remaining paths
                                let item = clean::Item {
                                    source: item.source.clone(),
                                    name: item.name.clone(),
                                    attrs: item.attrs.clone(),
                                    inner: clean::ViewItemItem(clean::ViewItem {
                                        inner: clean::Import(unresolved)
                                    }),
