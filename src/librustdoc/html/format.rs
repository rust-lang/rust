...
fn print_where_clause_with_opts(
    ...
    fn where_preds(
        ...
        fmt::from_fn(move |f| {
            predicates
                .iter()
                .map(|predicate| print_where_predicate_with_opts(predicate, cx, opts))
                .joined(&sep, f)
        })
    }
    ...
    Some(fmt::from_fn(move |f| {
        if f.alternate() {
            write!(f, " where {:#}", where_preds(&gens.where_predicates, cx, ", ", opts))?;
        ...
        let where_preds = where_preds(&gens.where_predicates, cx, sep, opts);
