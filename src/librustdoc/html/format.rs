    ...
    match *t {
        ...
        clean::QPath(box clean::QPathData {
            ...
            if !f.alternate() {
                ...
                // type (in respect to source order) that bears the given name (`assoc.name`) and
                // that is affiliated with the computed `DefId`. This is obviously
                // incorrect when we have multiple impl blocks. Ideally, we would
                // thread the `DefId` of the assoc ty itself through here and map it
                // to the corresponding HTML ID that was generated by
                // `render::Context::derive_id` when the impl blocks were rendered.
                // There is no such mapping unfortunately.
                // As a hack, we could badly imitate `derive_id` here by keeping *count* when
                // looking for the assoc ty `DefId` in
                // `tcx.associated_items(self_ty_did).in_definition_order()`
...
impl clean::FnDecl {
    ...
    ///   characters in the function declaration up to but not including the parentheses. This is
    ///   expected to go into a `<pre>`/`code-header` block, so indentation and newlines are
    ///   preserved.
    ...
    pub(crate) fn full_print<'a, 'tcx: 'a>(
        ...
        display_fn(move |f| {
            // First, generate the text form of the declaration, with no line wrapping, and count
            // the bytes.
