...
use rustc::hir::def_id::{CrateNum, DefId, DefIndex, DefIndexAddressSpace, LOCAL_CRATE};
...
use clean;
use clean::{get_path_for_type, AttributesExt, Clean, MAX_DEF_ID};
        ...
        hir::Ty {
            ...
            node: hir::TyKind::Path(hir::QPath::Resolved(
                ...
                P(hir::Path {
                    ...
                    segments: HirVec::from_vec(vec![hir::PathSegment::from_ident(
                        Ident::from_interned_str(param.name),
                    )]),
                }),
            )),
            span: DUMMY_SP,
...
pub fn new_handler(
    ...
    let emitter: Box<dyn Emitter + sync::Send> = match error_format {
        ...
        ErrorOutputType::Json(pretty) => {
            ...
            Box::new(JsonEmitter::stderr(None, source_map, pretty).ui_testing(ui_testing))
        }
...
pub fn run_core(options: RustdocOptions) -> (clean::Crate, RenderInfo, RenderOptions, Vec<String>) {
    ...
    let lints = lint::builtin::HardwiredLints
        .get_lints()
        .into_iter()
    ...
    let sessopts = config::Options {
        ...
        lint_opts: if !display_warnings { lints } else { vec![] },
    ...
    driver::spawn_thread_pool(sessopts, move |sessopts| {
        ...
        let diagnostic_handler = new_handler(
            error_format,
        ...
        let mut sess = session::build_session_(sessopts, cpath, diagnostic_handler, source_map);
        ...
        lint::builtin::HardwiredLints
            ...
            .for_each(|l| {
                sess.driver_lint_caps
                    .insert(lint::LintId::of(l), lint::Allow);
            });
        ...
        abort_on_err(
            driver::phase_3_run_analysis_passes(
                ...
                |tcx, analysis, _, result| {
                    ...
                    let mut passes: Vec<String> = passes::defaults(default_passes)
                        .iter()
                        .map(|p| p.to_string())
                        .collect();
