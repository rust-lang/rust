...
pub(crate) fn create_config(
    ...
    interface::Config {
        ...
        override_queries: Some(|_sess, providers| {
            ...
            providers.lint_mod.override_provider(|tcx, module_def_id| {
                late_lint_mod(tcx, module_def_id, MissingDoc)
            });
            // hack so that `used_trait_imports` won't try to call typeck
            providers.used_trait_imports.override_provider(|_, _| {
                static EMPTY_SET: LazyLock<UnordSet<LocalDefId>> = LazyLock::new(UnordSet::default);
                &EMPTY_SET
            });
            // In case typeck does end up being called, don't ICE in case there were name resolution errors
            providers.typeck.override_provider(move |tcx, def_id| {
                ...
                (rustc_interface::DEFAULT_QUERY_PROVIDERS.typeck.get())(tcx, def_id)
            });
        }),
