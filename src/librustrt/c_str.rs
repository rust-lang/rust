...
//! null-terminated strings for use with FFI calls (back to C). Most C APIs
//! require that the string being passed to them is null-terminated, and by
//! default rust's string types are *not* null terminated.
//!
...
//! "owns" the internal buffer of characters and will automatically deallocate
//! the buffer when the string is dropped. The `ToCStr` trait is implemented for
//! `&str` and `&[u8]`, but the conversions can fail due to some of the
//! limitations explained above.
//!
//! This also means that currently whenever a C string is created, an allocation
//! must be performed to place the data elsewhere (the lifetime of the C string
//! is not tied to the lifetime of the original string/data buffer). If C
//! strings are heavily used in applications, then caching may be advisable to
//! prevent unnecessary amounts of allocations.
//!
...
//!     // The `c_buffer` pointer will be deallocated when `my_c_string` goes
//!     // out of scope.
