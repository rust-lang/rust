...
impl<'tcx> Progress<'tcx> {
    fn error(tcx: TyCtxt<'tcx>) -> Self {
        Progress { ty: tcx.err(), obligations: vec![] }
    }
...
fn confirm_param_env_candidate<'cx, 'tcx>(
    ...
    match infcx.at(cause, param_env).eq(cache_trait_ref, obligation_trait_ref) {
        ...
        Err(e) => {
            ...
            Progress { ty: infcx.tcx.err(), obligations: vec![] }
        }
    }
}
...
fn confirm_impl_candidate<'cx, 'tcx>(
    ...
    let assoc_ty = match assoc_ty_def(selcx, impl_def_id, assoc_item_id) {
        ...
        Err(ErrorReported) => return Progress { ty: tcx.err(), obligations: nested },
    };
    ...
    if !assoc_ty.item.defaultness.has_value() {
        ...
        return Progress { ty: tcx.err(), obligations: nested };
    }
    ...
    if substs.len() != tcx.generics_of(assoc_ty.item.def_id).count() {
        ...
        Progress { ty: tcx.err(), obligations: nested }
    } else {
