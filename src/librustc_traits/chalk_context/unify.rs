...
crate fn unify<'me, 'gcx, 'tcx, T: Relate<'tcx>>(
    ...
    debug!(
        "unify: goals = {:?}, constraints = {:?}",
        delegate.goals, delegate.constraints
    );
...
impl ChalkTypeRelatingDelegate<'me, 'gcx, 'tcx> {
    fn new(infcx: &'me InferCtxt<'me, 'gcx, 'tcx>, environment: Environment<'tcx>) -> Self {
        Self {
            infcx,
            environment,
            goals: Vec::new(),
...
impl TypeRelatingDelegate<'tcx> for &mut ChalkTypeRelatingDelegate<'_, '_, 'tcx> {
    ...
    fn push_outlives(&mut self, sup: ty::Region<'tcx>, sub: ty::Region<'tcx>) {
        self.constraints
            .push(ty::OutlivesPredicate(sup.into(), sub));
    }

    fn push_domain_goal(&mut self, domain_goal: DomainGoal<'tcx>) {
        let goal = self
            .environment
            .with(self.infcx.tcx.mk_goal(domain_goal.into_goal()));
