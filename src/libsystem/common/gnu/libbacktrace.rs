...
use error::prelude::*;
use backtrace::prelude::*;
use os_str::prelude::*;
use libc;

pub fn print<O: BacktraceOutput>(w: &mut O, idx: isize, addr: *mut (),
             symaddr: *mut ()) -> Result<()> {
    use env::prelude::*;
    use c_str::CStr;
    use core::ptr;
    ...
    if ret == 0 || data.is_null() {
        try!(w.output(idx, addr, None));
    } else {
        try!(w.output(idx, addr, Some(unsafe { CStr::from_ptr(data).to_bytes() })));
    }
    ...
    if ret == 0 {
        for (i, &(file, line)) in fileline_buf[..fileline_count].iter().enumerate() {
            ...
            try!(w.output_fileline(file, line, i == FILELINE_SIZE - 1));
        }
    }
