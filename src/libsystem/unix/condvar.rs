...
impl sys::Condvar for Condvar {
    ...
    unsafe fn wait_timeout(&self, mutex: &Mutex, dur: Duration) -> bool {
        // First, figure out what time it currently is, in both system and
        // stable time.  pthread_cond_timedwait uses system time, but we want to
        // report timeout based on stable time.
        let mut sys_now = libc::timeval { tv_sec: 0, tv_usec: 0 };
        let stable_now = <time::imp::SteadyTime as time::SteadyTime>::now().unwrap();
        ...
        let stable_done = <time::imp::SteadyTime as time::SteadyTime>::now().unwrap();
