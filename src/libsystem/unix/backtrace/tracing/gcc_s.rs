...
use unix::backtrace::printing::print;

#[inline(never)] // if we know this is a function call, we can skip it when
                 // tracing
pub fn write<O: BacktraceOutput>(w: &mut O) -> Result<()> {
    struct Context<'a, O: 'a> {
        idx: isize,
        writer: &'a mut O,
    ...
    return match unsafe {
        uw::_Unwind_Backtrace(trace_fn::<O>,
    ...
    extern fn trace_fn<O: BacktraceOutput>(ctx: *mut uw::_Unwind_Context,
                       arg: *mut libc::c_void) -> uw::_Unwind_Reason_Code {
        let cx: &mut Context<O> = unsafe { mem::transmute(arg) };
        ...
        if cx.idx > 100 {
            match write!(cx.writer, " ... <frames omitted>\n") {
                Ok(()) => {}
                Err(e) => { cx.last_error = Some(e.into()); }
            }
