    ...
    fn sleep(dur: Duration) -> Result<()> {
        ...
        unsafe {
            while libc::nanosleep(&ts, &mut ts) == -1 {
                match Error::expect_last_error() {
                    ref e if e.code() == libc::EINTR => (),
                    e => return Err(e),
                }
            }
        }
...
fn min_stack_size(attr: *const libc::pthread_attr_t) -> usize {
    use dynamic_lib::{self as dl, DynamicLibrary};
    use sync::prelude::*;
    ...
    INIT.call_once(|| {
        let lib = match dl::imp::DynamicLibrary::open(None) {
            Ok(l) => l,
            Err(..) => return,
        };
        unsafe {
