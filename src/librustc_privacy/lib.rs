        ...
        Some(hir_id) => {
            let vis = match tcx.hir().get_by_hir_id(hir_id) {
                ...
                Node::ImplItem(impl_item) => {
                    match tcx.hir().get_by_hir_id(tcx.hir().get_parent(hir_id)) {
                ...
                Node::StructCtor(vdata) => {
                    let struct_hir_id = tcx.hir().get_parent(hir_id);
                ...
                Node::Expr(expr) => {
                    return (ty::Visibility::Restricted(
                        tcx.hir().get_module_parent(expr.hir_id)), expr.span, "private")
                }
                node => bug!("unexpected node kind: {:?}", node)
            };
...
impl<'a, 'tcx> TypePrivacyVisitor<'a, 'tcx> {
    ...
    fn check_expr_pat_type(&mut self, id: hir::HirId, span: Span) -> bool {
        self.span = span;
        if self.visit(self.tables.hir_id_to_type(id)) || self.visit(self.tables.node_substs(id)) {
            return true;
        }
...
impl<'a, 'tcx> Visitor<'tcx> for TypePrivacyVisitor<'a, 'tcx> {
    ...
    fn visit_ty(&mut self, hir_ty: &'tcx hir::Ty) {
        ...
        if self.in_body {
            // Types in bodies.
            if self.visit(self.tables.hir_id_to_type(hir_ty.hir_id)) {
                return;
            }
        } else {
            // Types in signatures.
    ...
    fn visit_item(&mut self, item: &'tcx hir::Item) {
        let orig_current_item = mem::replace(
            &mut self.current_item, self.tcx.hir().local_def_id_from_hir_id(item.hir_id));
    ...
    fn check_def_id(&mut self, def_id: DefId, kind: &str, descr: &dyn fmt::Display) -> bool {
        ...
        if !vis.is_at_least(self.required_visibility, self.tcx) {
            ...
            if self.has_pub_restricted || self.has_old_errors || self.in_assoc_ty {
                ...
                self.tcx.lint_node(lint::builtin::PRIVATE_IN_PUBLIC, hir_id, self.span,
    ...
    fn visit_item(&mut self, item: &'tcx hir::Item) {
        ...
        match item.node {
            ...
            hir::ItemKind::Trait(.., ref trait_item_refs) => {
                ...
                for trait_item_ref in trait_item_refs {
                    let hir_id = self.tcx.hir().node_to_hir_id(trait_item_ref.id.node_id);
                    let mut check = self.check(hir_id, item_visibility);
            ...
            hir::ItemKind::Impl(.., ref trait_ref, _, ref impl_item_refs) => {
                ...
                for impl_item_ref in impl_item_refs {
                    ...
                    let mut check = self.check(impl_item.hir_id, impl_item_vis);
