...
fn test_split_char_first() {
    ...
    fail_unless_eq!(u, ~"hello");
    fail_unless_eq!(v, ~" sweet world");

    let (u,v) = split_char_first("hello sweet world", ',');
    fail_unless_eq!(u, ~"hello sweet world");
    fail_unless_eq!(v, ~"");
}
...
fn test_get_authority() {
    ...
    fail_unless_eq!(u, Some(UserInfo::new(~"user", Some(~"pass"))));
    fail_unless_eq!(h, ~"rust-lang.org");
    ...
    fail_unless_eq!(h, ~"rust-lang.org");
    fail_unless!(p.is_none());
    fail_unless_eq!(r, ~"#blah");

    // ipv6 tests
    let (_, h, _, _) = get_authority(
        "//2001:0db8:85a3:0042:0000:8a2e:0370:7334#blah").unwrap();
    fail_unless_eq!(h, ~"2001:0db8:85a3:0042:0000:8a2e:0370:7334");
    ...
    fail_unless_eq!(u, Some(UserInfo::new(~"us", Some(~"p"))));
    fail_unless_eq!(h, ~"2001:0db8:85a3:0042:0000:8a2e:0370:7334");
    fail_unless_eq!(p, Some(~"8000"));
...
fn test_get_path() {
    ...
    fail_unless_eq!(p, ~"/something+ orother");
    fail_unless_eq!(r, ~"");
    let (p, r) = get_path("test@email.com#fragment", false).unwrap();
    fail_unless_eq!(p, ~"test@email.com");
    fail_unless_eq!(r, ~"#fragment");
    let (p, r) = get_path("/gen/:addr=?q=v", false).unwrap();
    fail_unless_eq!(p, ~"/gen/:addr=");
    fail_unless_eq!(r, ~"?q=v");
...
mod tests {
    ...
    fn test_url_parse() {
        ...
        let up = from_str(url);
        let u = up.unwrap();
        fail_unless_eq!(&u.scheme, &~"http");
        fail_unless_eq!(&u.user, &Some(UserInfo::new(~"user", Some(~"pass"))));
        fail_unless_eq!(&u.host, &~"rust-lang.org");
        fail_unless_eq!(&u.port, &Some(~"8080"));
        fail_unless_eq!(&u.path, &~"/doc/~u");
        fail_unless_eq!(&u.query, &~[(~"s", ~"v")]);
        fail_unless_eq!(&u.fragment, &Some(~"something"));
    }
    ...
    fn test_url_host_with_port() {
        ...
        let url = from_str(urlstr).unwrap();
        fail_unless_eq!(&url.scheme, &~"scheme");
        fail_unless_eq!(&url.host, &~"host");
        fail_unless_eq!(&url.port, &Some(~"1234"));
        fail_unless_eq!(&url.path, &~""); // is empty path really correct? Other tests think so
        let urlstr = ~"scheme://host:1234/";
        let url = from_str(urlstr).unwrap();
        fail_unless_eq!(&url.scheme, &~"scheme");
        fail_unless_eq!(&url.host, &~"host");
        fail_unless_eq!(&url.port, &Some(~"1234"));
        fail_unless_eq!(&url.path, &~"/");
    }
    ...
    fn test_full_url_parse_and_format() {
        ...
        fail_unless_eq!(from_str(url).unwrap().to_str(), url);
    }
    ...
    fn test_empty_query_url_parse_and_format() {
        ...
        fail_unless_eq!(from_str(url).unwrap().to_str(), should_be);
    }
    ...
    #[test]
    fn test_encode() {
        fail_unless_eq!(encode(""), ~"");
        fail_unless_eq!(encode("http://example.com"), ~"http://example.com");
        fail_unless_eq!(encode("foo bar% baz"), ~"foo%20bar%25%20baz");
        fail_unless_eq!(encode(" "), ~"%20");
        fail_unless_eq!(encode("!"), ~"!");
        fail_unless_eq!(encode("\""), ~"\"");
        fail_unless_eq!(encode("#"), ~"#");
        fail_unless_eq!(encode("$"), ~"$");
        fail_unless_eq!(encode("%"), ~"%25");
        fail_unless_eq!(encode("&"), ~"&");
        fail_unless_eq!(encode("'"), ~"%27");
        fail_unless_eq!(encode("("), ~"(");
        fail_unless_eq!(encode(")"), ~")");
        fail_unless_eq!(encode("*"), ~"*");
        fail_unless_eq!(encode("+"), ~"+");
    ...
    fn test_encode_component() {
        fail_unless_eq!(encode_component(""), ~"");
        fail_unless!(encode_component("http://example.com") ==
            ~"http%3A%2F%2Fexample.com");
        fail_unless!(encode_component("foo bar% baz") ==
            ~"foo%20bar%25%20baz");
        fail_unless_eq!(encode_component(" "), ~"%20");
        fail_unless_eq!(encode_component("!"), ~"%21");
        fail_unless_eq!(encode_component("#"), ~"%23");
        fail_unless_eq!(encode_component("$"), ~"%24");
        fail_unless_eq!(encode_component("%"), ~"%25");
        fail_unless_eq!(encode_component("&"), ~"%26");
        fail_unless_eq!(encode_component("'"), ~"%27");
        fail_unless_eq!(encode_component("("), ~"%28");
        fail_unless_eq!(encode_component(")"), ~"%29");
        fail_unless_eq!(encode_component("*"), ~"%2A");
        fail_unless_eq!(encode_component("+"), ~"%2B");
        fail_unless_eq!(encode_component(","), ~"%2C");
        fail_unless_eq!(encode_component("/"), ~"%2F");
        fail_unless_eq!(encode_component(":"), ~"%3A");
        fail_unless_eq!(encode_component(";"), ~"%3B");
        fail_unless_eq!(encode_component("="), ~"%3D");
        fail_unless_eq!(encode_component("?"), ~"%3F");
        fail_unless_eq!(encode_component("@"), ~"%40");
        fail_unless_eq!(encode_component("["), ~"%5B");
        fail_unless_eq!(encode_component("]"), ~"%5D");
    }

    #[test]
    fn test_decode() {
        fail_unless_eq!(decode(""), ~"");
        fail_unless_eq!(decode("abc/def 123"), ~"abc/def 123");
        fail_unless_eq!(decode("abc%2Fdef%20123"), ~"abc%2Fdef 123");
        fail_unless_eq!(decode("%20"), ~" ");
        fail_unless_eq!(decode("%21"), ~"%21");
        fail_unless_eq!(decode("%22"), ~"%22");
        fail_unless_eq!(decode("%23"), ~"%23");
        fail_unless_eq!(decode("%24"), ~"%24");
        fail_unless_eq!(decode("%25"), ~"%");
        fail_unless_eq!(decode("%26"), ~"%26");
        fail_unless_eq!(decode("%27"), ~"'");
        fail_unless_eq!(decode("%28"), ~"%28");
        fail_unless_eq!(decode("%29"), ~"%29");
        fail_unless_eq!(decode("%2A"), ~"%2A");
        fail_unless_eq!(decode("%2B"), ~"%2B");
        fail_unless_eq!(decode("%2C"), ~"%2C");
        fail_unless_eq!(decode("%2F"), ~"%2F");
        fail_unless_eq!(decode("%3A"), ~"%3A");
        fail_unless_eq!(decode("%3B"), ~"%3B");
        fail_unless_eq!(decode("%3D"), ~"%3D");
        fail_unless_eq!(decode("%3F"), ~"%3F");
        fail_unless_eq!(decode("%40"), ~"%40");
        fail_unless_eq!(decode("%5B"), ~"%5B");
        fail_unless_eq!(decode("%5D"), ~"%5D");
    }

    #[test]
    fn test_decode_component() {
        fail_unless_eq!(decode_component(""), ~"");
        fail_unless_eq!(decode_component("abc/def 123"), ~"abc/def 123");
        fail_unless_eq!(decode_component("abc%2Fdef%20123"), ~"abc/def 123");
        fail_unless_eq!(decode_component("%20"), ~" ");
        fail_unless_eq!(decode_component("%21"), ~"!");
        fail_unless_eq!(decode_component("%22"), ~"\"");
        fail_unless_eq!(decode_component("%23"), ~"#");
        fail_unless_eq!(decode_component("%24"), ~"$");
        fail_unless_eq!(decode_component("%25"), ~"%");
        fail_unless_eq!(decode_component("%26"), ~"&");
        fail_unless_eq!(decode_component("%27"), ~"'");
        fail_unless_eq!(decode_component("%28"), ~"(");
        fail_unless_eq!(decode_component("%29"), ~")");
        fail_unless_eq!(decode_component("%2A"), ~"*");
        fail_unless_eq!(decode_component("%2B"), ~"+");
        fail_unless_eq!(decode_component("%2C"), ~",");
        fail_unless_eq!(decode_component("%2F"), ~"/");
        fail_unless_eq!(decode_component("%3A"), ~":");
        fail_unless_eq!(decode_component("%3B"), ~";");
        fail_unless_eq!(decode_component("%3D"), ~"=");
        fail_unless_eq!(decode_component("%3F"), ~"?");
        fail_unless_eq!(decode_component("%40"), ~"@");
        fail_unless_eq!(decode_component("%5B"), ~"[");
        fail_unless_eq!(decode_component("%5D"), ~"]");
    }

    #[test]
    fn test_encode_form_urlencoded() {
        let mut m = HashMap::new();
        fail_unless_eq!(encode_form_urlencoded(&m), ~"");
        ...
        fail_unless_eq!(encode_form_urlencoded(&m), ~"foo=bar&foo=123");
    ...
    fn test_decode_form_urlencoded() {
        fail_unless_eq!(decode_form_urlencoded([]).len(), 0);

        let s = "a=1&foo+bar=abc&foo+bar=12+%3D+34".as_bytes();
        let form = decode_form_urlencoded(s);
        fail_unless_eq!(form.len(), 2);
        fail_unless_eq!(form.get(&~"a"), &~[~"1"]);
        fail_unless_eq!(form.get(&~"foo bar"), &~[~"abc", ~"12 = 34"]);
    }
}
