...
/// Like Unlock but provides stronger guarantees about the enviroment
/// of the closure argument.
pub trait ScopedUnlock<T> {
    /// See Unlock::unlock.
    fn unlock_with<U>(&mut self, T, extern fn(T) -> U) -> U;
}

impl <T: Send, U: Unlock> ScopedUnlock<T> for U {
    #[inline]
    fn unlock_with<V>(&mut self, msg: T, blk: extern fn(T) -> V) -> V {
