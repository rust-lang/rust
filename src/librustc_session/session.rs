    ...
    pub fn link_dead_code(&self) -> bool {
        match self.opts.cg.link_dead_code {
            ...
            None => {
                ...
                // Issue #64685: (Linker error because `-wL as-needed` conflicts with
                // `-C link-dead-code`). As described by this issue, the "link dead code" option
                // produces incorrect binaries when compiled and linked under MSVC. The resulting
                // Rust programs typically crash with a segmentation fault, or produce an empty
                // "*.profraw" file (profiling counter results normally generated during program
                // exit).
                //
