...
// FIXME: ideally we'd put this on `SnippetWorkspaceEdit.change_annotations`, but that doesn't work,
// most likely because the lint fires in the impls generated by the derives as well.
#![expect(
    clippy::disallowed_types,
    reason = "`SnippetWorkspaceEdit.change_annotations` needs to match `lsp_types::WorkspaceEdit.change_annotations` for the `From` impl"
)]
