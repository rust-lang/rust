...
use rustc_hir::{BindingMode, Expr, ExprField, ExprKind, Node, PatKind, Path, QPath, UnOp, Rest};
...
impl LateLintPass<'_> for UnnecessaryStruct {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        ...
        let sugg = match (field_path, base) {
            (Some(&path), Rest::None | Rest::DefaultFields(_)) => {
                // all fields match, no base given
                path.span
            },
            (Some(path), Rest::Base(base)) if base_is_suitable(cx, expr, base) && path_matches_base(path, base) => {
                // all fields match, has base: ensure that the path of the base matches
                base.span
            },
            (None, Rest::Base(base)) if fields.is_empty() && base_is_suitable(cx, expr, base) => {
