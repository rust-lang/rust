...
use rustc_hir::{
    self as hir, BlockCheckMode, BodyId, Constness, Expr, ExprKind, FnDecl, HirId, Impl, Item,
    ItemKind, UnsafeSource, Unsafety,
};
use rustc_lint::{LateContext, LateLintPass};
use rustc_middle::hir::nested_filter;
use rustc_middle::traits::Reveal;
use rustc_middle::ty::{
    self, Binder, BoundConstness, Clause, GenericArgKind, GenericParamDefKind, ImplPolarity,
    ParamEnv, PredicateKind, TraitPredicate, Ty, TyCtxt,
};
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'_>) {
        if let ItemKind::Impl(Impl { of_trait: Some(ref trait_ref), .. }) = item.kind {
            ...
            let is_automatically_derived =
                cx.tcx.has_attr(item.owner_id.to_def_id(), sym::automatically_derived);
...
fn check_copy_clone<'tcx>(
    ...
    if !is_copy(cx, ty) {
        if ty_subs.non_erasable_generics().next().is_some() {
            let has_copy_impl = cx.tcx.impls_in_crate(LOCAL_CRATE).get(&Some(copy_id)).map_or(false, |impls| {
                impls
                    .iter()
                    .any(|(id, _)| matches!(cx.tcx.type_of(id).kind(), ty::Adt(adt, _) if ty_adt.did() == adt.did()))
            });
            if !has_copy_impl {
                return;
            }
    ...
    ParamEnv::new(
        tcx.mk_predicates(ty_predicates.iter().map(|&(p, _)| p).chain(
            params.iter().filter(|&&(_, needs_eq)| needs_eq).map(|&(param, _)| {
                tcx.mk_predicate(Binder::dummy(PredicateKind::Clause(Clause::Trait(
                    TraitPredicate {
