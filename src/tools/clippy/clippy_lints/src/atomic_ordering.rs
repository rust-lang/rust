...
fn check_atomic_load_store(cx: &LateContext<'_, '_>, expr: &Expr<'_>) {
    if_chain! {
        ...
        if let Some(ordering_def_id) = cx.typeck_results().qpath_res(ordering_qpath, ordering_arg.hir_id).opt_def_id();
        then {
            if method == "load" &&
...
fn check_memory_fence(cx: &LateContext<'_, '_>, expr: &Expr<'_>) {
    if_chain! {
        ...
        if let Some(ordering_def_id) = cx.typeck_results().qpath_res(ordering_qpath, args[0].hir_id).opt_def_id();
