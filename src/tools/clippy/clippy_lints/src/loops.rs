...
fn is_end_eq_array_len<'tcx>(
    ...
    if_chain! {
        ...
        if let ty::Array(_, arr_len_const) = indexed_ty.data();
...
fn check_for_loop_over_map_kv<'tcx>(
    ...
    if let PatKind::Tuple(ref pat, _) = pat.kind {
        if pat.len() == 2 {
            ...
            let (new_pat_span, kind, ty, mutbl) = match *cx.typeck_results().expr_ty(arg).data() {
        ...
        match expr.kind {
            ...
            ExprKind::Call(ref f, args) => {
                ...
                for expr in args {
                    ...
                    self.prefer_mutable = false;
                    if let ty::Ref(_, _, mutbl) = *ty.data() {
                        if mutbl == Mutability::Mut {
                            self.prefer_mutable = true;
                        }
                    }
