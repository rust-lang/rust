...
fn needless_borrow_count<'tcx>(
    ...
    let fn_sig = cx.tcx.fn_sig(fn_id).instantiate_identity().skip_normalization().skip_binder();
    ...
    let mut check_reference_and_referent = |reference: &Expr<'tcx>, referent: &Expr<'tcx>| {
        ...
        predicates.iter().all(|predicate| {
            ...
            let predicate = EarlyBinder::bind(predicate).instantiate(cx.tcx, &args_with_referent_ty[..]).skip_normalization();
    ...
    cx.tcx
        ...
        .any(|assoc_item| {
            if assoc_item.is_method() {
                let self_ty = cx
                    .tcx
                    .fn_sig(assoc_item.def_id).instantiate_identity().skip_normalization()
                    .skip_binder()
                    .inputs()[0];
