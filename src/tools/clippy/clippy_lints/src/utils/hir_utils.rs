...
use rustc_hir::{
    BinOpKind, Block, BlockCheckMode, BodyId, BorrowKind, CaptureBy, Expr, ExprKind, Field,
...
impl<'a, 'tcx> SpanlessEq<'a, 'tcx> {
    pub fn new(cx: &'a LateContext<'a, 'tcx>) -> Self {
        Self { cx, tables: cx.typeck_results(), ignore_fn: false }
    }

    pub fn ignore_fn(self) -> Self {
        Self { cx: self.cx, tables: self.cx.typeck_results(), ignore_fn: true }
    }
    ...
    pub fn eq_expr(&mut self, left: &Expr<'_>, right: &Expr<'_>) -> bool {
        ...
        match (&left.kind, &right.kind) {
            ...
            (
                &ExprKind::AssignOp(ref lo, ref ll, ref lr),
                &ExprKind::AssignOp(ref ro, ref rl, ref rr),
            ...
            (
                &ExprKind::Field(ref l_f_exp, ref l_f_ident),
                &ExprKind::Field(ref r_f_exp, ref r_f_ident),
            ...
            (
                &ExprKind::Loop(ref lb, ref ll, ref lls),
                &ExprKind::Loop(ref rb, ref rl, ref rls),
            ) => {
                lls == rls
                    && self.eq_block(lb, rb)
            ...
            (
                &ExprKind::Match(ref le, ref la, ref ls),
                &ExprKind::Match(ref re, ref ra, ref rs),
            ) => {
                ls == rs
                    && self.eq_expr(le, re)
                    && over(la, ra, |l, r| {
            ...
            (
                &ExprKind::MethodCall(l_path, _, l_args, _),
                &ExprKind::MethodCall(r_path, _, r_args, _),
            ) => {
                !self.ignore_fn
            ...
            (
                &ExprKind::Struct(ref l_path, ref lf, ref lo),
                &ExprKind::Struct(ref r_path, ref rf, ref ro),
            ) => {
            ...
            (&ExprKind::Unary(l_op, ref le), &ExprKind::Unary(r_op, ref re)) => {
                l_op == r_op && self.eq_expr(le, re)
            }
    ...
    fn eq_generic_arg(&mut self, left: &GenericArg<'_>, right: &GenericArg<'_>) -> bool {
        match (left, right) {
            (GenericArg::Lifetime(l_lt), GenericArg::Lifetime(r_lt)) => {
                Self::eq_lifetime(l_lt, r_lt)
            }
    ...
    pub fn eq_pat(&mut self, left: &Pat<'_>, right: &Pat<'_>) -> bool {
        match (&left.kind, &right.kind) {
            ...
            (
                &PatKind::TupleStruct(ref lp, ref la, ls),
                &PatKind::TupleStruct(ref rp, ref ra, rs),
            ) => self.eq_qpath(lp, rp) && over(la, ra, |l, r| self.eq_pat(l, r)) && ls == rs,
            (
                &PatKind::Binding(ref lb, .., ref li, ref lp),
                &PatKind::Binding(ref rb, .., ref ri, ref rp),
            ) => {
                lb == rb
                    && li.name.as_str() == ri.name.as_str()
            ...
            (&PatKind::Range(ref ls, ref le, li), &PatKind::Range(ref rs, ref re, ri)) => {
                both(ls, rs, |a, b| self.eq_expr(a, b))
                    && both(le, re, |a, b| self.eq_expr(a, b))
                    && (li == ri)
            }
    ...
    pub fn eq_ty_kind(&mut self, left: &TyKind<'_>, right: &TyKind<'_>) -> bool {
        match (left, right) {
            ...
            (&TyKind::Array(ref lt, ref ll_id), &TyKind::Array(ref rt, ref rl_id)) => {
                ...
                let mut celcx = constant_context(self.cx, self.cx.tcx.typeck_body(ll_id.body));
                self.tables = self.cx.tcx.typeck_body(ll_id.body);
                let ll = celcx.expr(&self.cx.tcx.hir().body(ll_id.body).value);

                let mut celcx = constant_context(self.cx, self.cx.tcx.typeck_body(rl_id.body));
                self.tables = self.cx.tcx.typeck_body(rl_id.body);
                let rl = celcx.expr(&self.cx.tcx.hir().body(rl_id.body).value);
...
impl<'a, 'tcx> SpanlessHash<'a, 'tcx> {
    pub fn new(cx: &'a LateContext<'a, 'tcx>, tables: &'a TypeckResults<'tcx>) -> Self {
        Self { cx, tables, s: StableHasher::new() }
    }
            ...
            ExprKind::InlineAsm(ref asm) => {
                ...
                for op in asm.operands {
                    match op {
                        ...
                        InlineAsmOperand::Const { expr } | InlineAsmOperand::Sym { expr } => {
                            self.hash_expr(expr)
                        }
                    }
                }
    ...
    pub fn hash_body(&mut self, body_id: BodyId) {
        ...
        self.tables = self.cx.tcx.typeck_body(body_id);
