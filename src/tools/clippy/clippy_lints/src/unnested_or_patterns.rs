...
fn insert_necessary_parens(pat: &mut Pat) {
    ...
    impl MutVisitor for Visitor {
        fn visit_pat(&mut self, pat: &mut Pat) {
            ...
            target.kind = Paren(take_pat(target));
        }
    }
...
fn take_pat(from: &mut Box<Pat>) -> Box<Pat> {
    let dummy = Box::new(Pat {
        id: DUMMY_NODE_ID,
        kind: Wild,
        span: DUMMY_SP,
        tokens: None,
...
fn extend_with_tail_or(target: &mut Box<Pat>, tail_or: ThinVec<Box<Pat>>) -> bool {
    fn extend(target: &mut Box<Pat>, mut tail_or: ThinVec<Box<Pat>>) {
