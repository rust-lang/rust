...
fn check_single_match(cx: &LateContext<'_>, ex: &Expr<'_>, arms: &[Arm<'_>], expr: &Expr<'_>) {
    if arms.len() == 2 && arms[0].guard.is_none() && arms[1].guard.is_none() {
        ...
        if *ty.data() != ty::Bool || is_allowed(cx, MATCH_BOOL, ex.hir_id) {
...
fn check_match_bool(cx: &LateContext<'_>, ex: &Expr<'_>, arms: &[Arm<'_>], expr: &Expr<'_>) {
    ...
    if *cx.typeck_results().expr_ty(ex).data() == ty::Bool {
        span_lint_and_then(
            cx,
            MATCH_BOOL,
            expr.span,
...
fn check_match_as_ref(cx: &LateContext<'_>, ex: &Expr<'_>, arms: &[Arm<'_>], expr: &Expr<'_>) {
    if arms.len() == 2 && arms[0].guard.is_none() && arms[1].guard.is_none() {
        ...
        if let Some(rb) = arm_ref {
            ...
            let cast = if_chain! {
                if let ty::Adt(_, substs) = input_ty.data();
                let input_ty = substs.type_at(0);
                if let ty::Adt(_, substs) = output_ty.data();
                let output_ty = substs.type_at(0);
                if let ty::Ref(_, output_ty, _) = *output_ty.data();
