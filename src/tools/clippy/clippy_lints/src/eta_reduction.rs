...
use rustc_hir::{BindingMode, Expr, ExprKind, FnRetTy, Safety, Param, PatKind, QPath, TyKind};
                ...
                ) =>
            {
                ...
                let sig = match callee_ty_adjusted.kind() {
                    ...
                    ty::Closure(_, subs) => cx
                        .tcx
                        .signature_unclosure(subs.as_closure().sig(), Safety::Default)
                        .skip_binder(),
                    _ => {
                        ...
                        {
                            cx.tcx.mk_fn_sig(tys, output, false, Safety::Default, Abi::Rust)
                        } else {
                            return;
                        }
                    },
...
fn check_sig<'tcx>(cx: &LateContext<'tcx>, closure: ClosureArgs<'tcx>, call_sig: FnSig<'_>) -> bool {
    call_sig.safety == Safety::Default
        && !has_late_bound_to_non_late_bound_regions(
            cx.tcx
                .signature_unclosure(closure.sig(), Safety::Default)
                .skip_binder(),
            call_sig,
        )
}
