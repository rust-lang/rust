                ...
                match (&from_ty.data(), &to_ty.data()) {
                    ...
                    (ty::Int(ast::IntTy::I32) | ty::Uint(ast::UintTy::U32), &ty::Char) => {
                        span_lint_and_then(
                            ...
                            |diag| {
                                let arg = sugg::Sugg::hir(cx, &args[0], "..");
                                let arg = if let ty::Int(_) = from_ty.data() {
                    ...
                    (ty::Ref(_, ty_from, from_mutbl), ty::Ref(_, ty_to, to_mutbl)) => {
                        if_chain! {
                            if let (&ty::Slice(slice_ty), &ty::Str) = (&ty_from.data(), &ty_to.data());
                            if let ty::Uint(ast::UintTy::U8) = slice_ty.data();
                    ...
                    (ty::Int(_) | ty::Uint(_), ty::Float(_)) if !const_context => span_lint_and_then(
                        ...
                        |diag| {
                            let arg = sugg::Sugg::hir(cx, &args[0], "..");
                            let arg = if let ty::Int(int_ty) = from_ty.data() {
                                arg.as_ty(format!(
                                    "u{}",
                    ...
                    (ty::Float(float_ty), ty::Int(_) | ty::Uint(_)) if !const_context => span_lint_and_then(
                        ...
                        |diag| {
                            ...
                            arg = if let ty::Int(int_ty) = to_ty.data() {
