...
impl<'tcx> LateLintPass<'tcx> for LenWithoutIsEmpty {
    ...
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx ImplItem<'_>) {
        if item.ident.name == sym::len
            ...
            && let Some(output) = LenOutput::new(cx, cx.tcx.fn_sig(item.owner_id).instantiate_identity().skip_normalization().skip_binder())
        {
    ...
    let (msg, is_empty_span, is_empty_expected_sig) = match is_empty {
        ...
        Some(is_empty)
            if !(is_empty.is_method()
                && check_is_empty_sig(
                    cx,
                    cx.tcx.fn_sig(is_empty.def_id).instantiate_identity().skip_normalization().skip_binder(),
                    len_self_kind,
                    len_output,
                )) =>
        {
