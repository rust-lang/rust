...
impl<'tcx> LateLintPass<'tcx> for Methods {
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx hir::Expr<'_>) {
        ...
        match expr.kind {
            hir::ExprKind::MethodCall(ref method_call, ref method_span, ref args, _) => {
                ...
                match self_ty.data() {
                    ty::Ref(_, ty, _) if *ty.data() == ty::Str => {
    ...
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, impl_item: &'tcx hir::ImplItem<'_>) {
        ...
        if let hir::ImplItemKind::Fn(_, _) = impl_item.kind {
            ...
            if let ty::Opaque(def_id, _) = *ret_ty.data() {
...
fn lint_expect_fun_call(
                ...
                hir::ExprKind::MethodCall(method_name, _, call_args, _) => {
                    if call_args.len() == 1
                        ...
                        && {
                            ...
                            *base_type.data() == ty::Str || is_type_diagnostic_item(cx, base_type, sym!(string_type))
                        }
                    {
                        &call_args[0]
                    } else {
    ...
    fn requires_to_string(cx: &LateContext<'_>, arg: &hir::Expr<'_>) -> bool {
        ...
        if let ty::Ref(_, ty, ..) = arg_ty.data() {
            if *ty.data() == ty::Str && can_be_static_str(cx, arg) {
                return false;
            }
        };
        true
                ...
                if let hir::ExprKind::Path(ref p) = fun.kind {
                    match cx.qpath_res(p, fun.hir_id) {
                        hir::def::Res::Def(hir::def::DefKind::Fn | hir::def::DefKind::AssocFn, def_id) => matches!(
                            cx.tcx.fn_sig(def_id).output().skip_binder().data(),
                            ty::Ref(ty::ReStatic, ..)
                        ),
                        _ => false,
                    }
            ...
            hir::ExprKind::MethodCall(..) => {
                cx.typeck_results()
                    ...
                    .map_or(false, |method_id| {
                        matches!(
                            cx.tcx.fn_sig(method_id).output().skip_binder().data(),
                            ty::Ref(ty::ReStatic, ..)
                        )
                    })
            },
...
fn lint_clone_on_copy(cx: &LateContext<'_>, expr: &hir::Expr<'_>, arg: &hir::Expr<'_>, arg_ty: Ty<'_>) {
    ...
    if let ty::Ref(_, inner, _) = arg_ty.data() {
        if let ty::Ref(_, innermost, _) = inner.data() {
            span_lint_and_then(
                cx,
                CLONE_DOUBLE_REF,
                expr.span,
                ...
                |diag| {
                    if let Some(snip) = sugg::Sugg::hir_opt(cx, arg) {
                        ...
                        let mut n = 0;
                        while let ty::Ref(_, inner, _) = ty.data() {
                            ty = inner;
                            n += 1;
                        }
...
fn lint_clone_on_ref_ptr(cx: &LateContext<'_>, expr: &hir::Expr<'_>, arg: &hir::Expr<'_>) {
    ...
    if let ty::Adt(_, subst) = obj_ty.data() {
...
fn lint_string_extend(cx: &LateContext<'_>, expr: &hir::Expr<'_>, args: &[hir::Expr<'_>]) {
    ...
    if let Some(arglists) = method_chain_args(arg, &["chars"]) {
        ...
        let ref_str = if *self_ty.data() == ty::Str {
            ""
...
fn lint_cstring_as_ptr(cx: &LateContext<'_>, expr: &hir::Expr<'_>, source: &hir::Expr<'_>, unwrap: &hir::Expr<'_>) {
    if_chain! {
        ...
        if let ty::Adt(def, substs) = source_type.data();
...
fn lint_iter_next<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx hir::Expr<'_>, iter_args: &'tcx [hir::Expr<'_>]) {
    ...
    if derefs_to_slice(cx, caller_expr, cx.typeck_results().expr_ty(caller_expr)).is_some() {
        ...
        || matches!(
            &cx.typeck_results().expr_ty(caller_expr).peel_refs().data(),
            ty::Array(_, _)
        )
    {
...
fn lint_map_flatten<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx hir::Expr<'_>, map_args: &'tcx [hir::Expr<'_>]) {
    ...
    if match_trait_method(cx, expr, &paths::ITERATOR) {
        ...
        let is_map_to_option = match map_closure_ty.data() {
            ty::Closure(_, _) | ty::FnDef(_, _) | ty::FnPtr(_) => {
                let map_closure_sig = match map_closure_ty.data() {
...
fn ty_has_iter_method(cx: &LateContext<'_>, self_ref_ty: Ty<'_>) -> Option<(&'static str, &'static str)> {
    has_iter_method(cx, self_ref_ty).map(|ty_name| {
        let mutbl = match self_ref_ty.data() {
    ...
    fn matches<'a>(self, cx: &LateContext<'a>, parent_ty: Ty<'a>, ty: Ty<'a>) -> bool {
        fn matches_value<'a>(cx: &LateContext<'a>, parent_ty: Ty<'_>, ty: Ty<'_>) -> bool {
            if ty == parent_ty {
                ...
                if let ty::Adt(_, substs) = ty.data() {
...
fn check_pointer_offset(cx: &LateContext<'_>, expr: &hir::Expr<'_>, args: &[hir::Expr<'_>]) {
    if_chain! {
        if args.len() == 2;
        if let ty::RawPtr(ty::TypeAndMut { ref ty, .. }) = cx.typeck_results().expr_ty(&args[0]).data();
