...
impl<'tcx> LateLintPass<'tcx> for RedundantPubCrate {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        if_chain! {
            ...
            if !cx.access_levels.is_exported(item.owner_id.def_id) && self.is_exported.last() == Some(&false);
        ...
        if let ItemKind::Mod { .. } = item.kind {
            self.is_exported.push(cx.access_levels.is_exported(item.owner_id.def_id));
        }
    }
