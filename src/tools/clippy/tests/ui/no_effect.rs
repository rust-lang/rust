...
fn issue17430(value: i32) {
    // Should not lint: the `let` binding is generated by an external/proc macro even though the
    // bound identifier keeps its original span (as happens with `#[no_panic]`).
    external! {
        let $_produced = 1;
    }
    with_span! {
    ...
    // Should not lint: here the initializer keeps its original span, so only checking the whole
    // `let` statement's span (rather than just the pattern's) catches the generated binding.
    external! {
        let $_produced = $value;
    }
}
