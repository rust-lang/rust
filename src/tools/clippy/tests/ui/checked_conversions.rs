...
fn i64_to_u16(value: i64) -> Option<u16> {
    if value <= i64::from(u16::MAX) && value >= 0 {
        Some(value as u16)
    } else {
        None
    }
...
fn isize_to_u8(value: isize) -> Option<u8> {
    if value <= (u8::MAX as isize) && value >= 0 {
        Some(value as u8)
    } else {
        None
    }
...
fn usize_to_isize(value: usize) -> isize {
    if value <= isize::MAX as usize && value as i32 == 5 {
        5
    } else {
        1
    }
...
fn u32_to_u16(value: u32) -> isize {
    if value <= u16::MAX as u32 && value as i32 == 5 {
        5
    } else {
        1
    }
...
fn no_i64_to_i32(value: i64) -> Option<i32> {
    if value <= (i32::MAX as i64) && value >= 0 {
        Some(value as i32)
    } else {
        None
    }
