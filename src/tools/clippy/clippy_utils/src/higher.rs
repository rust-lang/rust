...
) -> Option<(&'tcx hir::Pat<'tcx>, &'tcx hir::Expr<'tcx>, &'tcx hir::Expr<'tcx>, Span)> {
    if_chain! {
        ...
        then {
            return Some((&*local.pat, &iterargs[0], expr, cx.tcx.hir().span(arms[0].hir_id)));
        }
    }
    None
}
...
pub fn vec_macro<'e>(cx: &LateContext<'_>, expr: &'e hir::Expr<'_>) -> Option<VecArgs<'e>> {
    if_chain! {
        ...
        if is_expn_of(cx.tcx.hir().span(fun.hir_id), "vec").is_some();
