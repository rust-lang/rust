...
pub fn eq_expr(l: &Expr, r: &Expr) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (
            ForLoop(box ast::ForLoop { pat: lp, iter: li, body: lb, label: ll }),
            ForLoop(box ast::ForLoop { pat: rp, iter: ri, body: rb, label: rl })
        ) => eq_label(ll, rl) && eq_pat(lp, rp) && eq_expr(li, ri) && eq_block(lb, rb),
