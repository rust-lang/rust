...
impl From<char> for Category {
    fn from(value: char) -> Self {
        match value {
            _ if value.is_numeric() => Category::Numeric,
            _ => Category::Other,
        }
    }
}
...
fn version_sort(a: &str, b: &str) -> Ordering {
    ...
    while let (Some(x), Some(y)) = (it1.peek(), it2.peek()) {
        match (Category::from(*x), Category::from(*y)) {
            ...
            (Category::Numeric, Category::Other) | (Category::Other, Category::Numeric) => {
                return x.cmp(y);
            }
            (Category::Other, Category::Other) => match x.cmp(y) {
                Ordering::Equal => {
                    it1.next();
                    it2.next();
                    continue;
