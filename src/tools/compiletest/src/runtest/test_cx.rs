...
pub(super) struct TestCx<'test> {
    ...
    pub props: &'test TestProps,
    pub testpaths: &'test TestPaths,
    pub revision: Option<&'test str>,
}

impl<'test> TestCx<'test> {
    /// Code executed for each revision in turn (or, if there are no
    /// revisions, exactly once, with revision == None).
    pub(super) fn run_revision(&self) {
    ...
    pub(super) fn pass_mode(&self) -> Option<PassMode> {
        self.props.pass_mode(self.config)
    }

    pub(super) fn should_run(&self, pm: Option<PassMode>) -> WillExecute {
    ...
    pub(super) fn run_if_enabled(&self) -> WillExecute {
        if self.config.run_enabled() { WillExecute::Yes } else { WillExecute::Disabled }
    }

    pub(super) fn should_run_successfully(&self, pm: Option<PassMode>) -> bool {
        match self.config.mode {
    ...
    pub(super) fn check_if_test_should_compile(&self, proc_res: &ProcRes, pm: Option<PassMode>) {
    ...
    pub(super) fn get_output(&self, proc_res: &ProcRes) -> String {
    ...
    pub(super) fn check_correct_failure_status(&self, proc_res: &ProcRes) {
    ...
    pub(super) fn print_source(&self, read_from: ReadFrom, pretty_type: &str) -> ProcRes {
    ...
    pub(super) fn compare_source(&self, expected: &str, actual: &str) {
        if expected != actual {
            self.fatal(&format!(
    ...
    pub(super) fn typecheck_source(&self, src: String) -> ProcRes {
        let mut rustc = Command::new(&self.config.rustc_path);
    ...
    pub(super) fn run_debuginfo_cdb_test(&self) {
    ...
    pub(super) fn run_debuginfo_gdb_test(&self) {
        assert!(self.revision.is_none(), "revisions not relevant here");
    ...
    pub(super) fn run_debuginfo_lldb_test(&self) {
        assert!(self.revision.is_none(), "revisions not relevant here");
    ...
    pub(super) fn cmd2procres(&self, cmd: &mut Command) -> ProcRes {
    ...
    pub(super) fn check_error_patterns(
        &self,
        output_to_check: &str,
        proc_res: &ProcRes,
        pm: Option<PassMode>,
    ...
    pub(super) fn check_no_compiler_crash(&self, proc_res: &ProcRes, should_ice: bool) {
    ...
    pub(super) fn check_forbid_output(&self, output_to_check: &str, proc_res: &ProcRes) {
    ...
    pub(super) fn check_expected_errors(
        &self,
    ...
    pub(super) fn should_emit_metadata(&self, pm: Option<PassMode>) -> EmitMetadata {
    ...
    pub(super) fn compile_test_general(
        &self,
        will_execute: WillExecute,
        emit_metadata: EmitMetadata,
    ...
    pub(super) fn document(&self, out_dir: &Path) -> ProcRes {
    ...
    pub(super) fn exec_compiled_test(&self) -> ProcRes {
    ...
    pub(super) fn compose_and_run_compiler(
        &self,
        mut rustc: Command,
        input: Option<String>,
    ) -> ProcRes {
    ...
    pub(super) fn make_compile_args(
        &self,
        input_file: &Path,
    ...
    pub(super) fn make_exe_name(&self) -> PathBuf {
    ...
    pub(super) fn aux_output_dir_name(&self) -> PathBuf {
    ...
    pub(super) fn output_base_dir(&self) -> PathBuf {
    ...
    pub(super) fn output_base_name(&self) -> PathBuf {
    ...
    pub(super) fn fatal(&self, err: &str) -> ! {
        self.error(err);
        error!("fatal error, panic: {:?}", err);
        panic!("fatal error");
    }

    pub(super) fn fatal_proc_rec(&self, err: &str, proc_res: &ProcRes) -> ! {
        self.error(err);
        proc_res.fatal(None, || ());
    }

    pub(super) fn fatal_proc_rec_with_ctx(
        &self,
        err: &str,
        proc_res: &ProcRes,
    ...
    pub(super) fn compile_test_and_save_ir(&self) -> ProcRes {
        let aux_dir = self.aux_output_dir_name();
    ...
    pub(super) fn compile_test_and_save_assembly(&self) -> (ProcRes, PathBuf) {
    ...
    pub(super) fn verify_with_filecheck(&self, output: &Path) -> ProcRes {
    ...
    pub(super) fn compare_to_default_rustdoc(&mut self, out_dir: &Path) {
    ...
    pub(super) fn check_rustdoc_test_option(&self, res: ProcRes) {
    ...
    pub(super) fn init_incremental_test(&self) {
    ...
    pub(super) fn aggressive_rm_rf(&self, path: &Path) -> io::Result<()> {
    ...
    pub(super) fn load_compare_outputs(
        &self,
        proc_res: &ProcRes,
    ...
    /// with LF.
    pub(super) fn normalize_platform_differences(output: &str) -> String {
        lazy_static! {
    ...
    pub(super) fn load_expected_output(&self, kind: &str) -> String {
    ...
    pub(super) fn load_expected_output_from_path(&self, path: &Path) -> Result<String, String> {
    ...
    pub(super) fn compare_output(&self, kind: &str, actual: &str, expected: &str) -> usize {
        if actual == expected {
            return 0;
        }
    ...
    pub(super) fn prune_duplicate_outputs(&self, modes: &[CompareMode]) {
        if self.config.bless {
    ...
    pub(super) fn create_stamp(&self) {
