...
fn make_directive_handlers_map() -> HashMap<&'static str, Handler> {
    ...
    let handlers: Vec<NamedHandler> = vec![
        ...
        handler(ADD_CORE_STUBS, |config, ln, props| {
            props.update_add_core_stubs(ln, config);
        }),
        handler(CORE_STUBS_COMPILE_FLAGS, |config, ln, props| {
            if let Some(flags) = config.parse_name_value_directive(ln, CORE_STUBS_COMPILE_FLAGS) {
                let flags = split_flags(&flags);
                for flag in &flags {
                    if flag == "--edition" || flag.starts_with("--edition=") {
                        panic!("you must use `//@ edition` to configure the edition");
