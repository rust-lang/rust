    ...
    pub fn parse_name_value_directive(
        ...
        if line.starts_with(directive) && line.as_bytes().get(colon) == Some(&b':') {
            ...
            if value.is_empty() {
                ...
                help!("expected syntax is: `//@ {}: value`", directive);
...
pub(crate) fn make_test_description<R: Read>(
    ...
    iter_directives(
        ...
        &mut |directive @ DirectiveLine { line_number, raw_directive: ln, .. }| {
            ...
            decision!(ignore_llvm(config, path, ln, path, line_number));
...
fn ignore_llvm(
    ...
    if let Some(needed_components) =
        config.parse_name_value_directive(line, "needs-llvm-components", testfile, line_number)
    {
    ...
    if let Some(actual_version) = &config.llvm_version {
        ...
        if let Some(version_string) =
            config.parse_name_value_directive(line, "min-llvm-version", testfile, line_number)
        {
            ...
            config.parse_name_value_directive(line, "max-llvm-major-version", testfile, line_number)
        {
        ...
        } else if let Some(version_string) = config.parse_name_value_directive(
            line,
            "min-system-llvm-version",
            testfile,
            line_number,
            ...
            config.parse_name_value_directive(line, "ignore-llvm-version", testfile, line_number)
        {
