...
pub(crate) fn check_directive<'a>(directive_ln: &'a str, mode: Mode) -> CheckDirectiveResult<'a> {
    ...
    let is_known_directive = KNOWN_DIRECTIVE_NAMES.contains(&directive_name)
        || match mode {
            Mode::Rustdoc => KNOWN_HTMLDOCCK_DIRECTIVE_NAMES.contains(&directive_name),
            Mode::RustdocJson => KNOWN_JSONDOCCK_DIRECTIVE_NAMES.contains(&directive_name),
            _ => false,
        };
