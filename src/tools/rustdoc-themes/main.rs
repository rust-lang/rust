...
fn get_themes<P: AsRef<Path>>(style_path: P) -> Vec<String> {
    ...
    const BEGIN_THEME_MARKER: &str = "/* Begin theme: ";
    const END_THEME_MARKER: &str = "/* End theme: ";
    ...
    for line in BufReader::new(File::open(style_path).expect("read rustdoc.css failed")).lines() {
        ...
        let line = line.trim();
        if let Some(line) = line.strip_prefix(BEGIN_THEME_MARKER) {
            let theme_name = line.trim().trim_end_matches("*/").trim();
