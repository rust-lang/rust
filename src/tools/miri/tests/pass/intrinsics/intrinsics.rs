...
fn main() {
    ...
    for _ in 0..50 {
        // Use `black_box` to make sure that the non-determinism of
        // `is_val_statically_known` isn't lost to optimization (and to
        // test that intrinsic as well...).
        if intrinsics::is_val_statically_known(intrinsics::black_box(0)) {
            saw_true = true;
        } else {
            saw_false = true;
        }
