...
impl<'a, 'tcx> LateLintPass<'a, 'tcx> for UnusedResults {
    fn check_stmt(&mut self, cx: &LateContext, s: &hir::Stmt) {
        ...
        let type_permits_lack_of_use = if t.is_unit()
            || cx
                .tcx
                .is_ty_uninhabited_from(cx.tcx.hir().get_module_parent(expr.id), t)
        {
            true
        } else {
            match t.sty {
...
impl UnusedParens {
    fn check_unused_parens_expr(
        ...
        if let ast::ExprKind::Paren(ref inner) = value.node {
            let necessary = followed_by_block
                && match inner.node {
    ...
    fn check_unused_parens_pat(&self, cx: &EarlyContext, value: &ast::Pat, msg: &str) {
