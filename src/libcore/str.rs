...
fn iter_matches<'a,'b>(s: &'a str, sep: &'b str, f: &fn(uint, uint)) {
...
fn iter_between_matches<'a,'b>(s: &'a str, sep: &'b str, f: &fn(uint, uint)) {
    let mut last_end = 0u;
 ...
 * ~~~
 */
pub fn split_str<'a,'b>(s: &'a str, sep: &'b str) -> ~[~str] {
    let mut result = ~[];
...
pub fn split_str_nonempty<'a,'b>(s: &'a str, sep: &'b str) -> ~[~str] {
    let mut result = ~[];
    do iter_between_matches(s, sep) |from, to| {
        if to > from {
            unsafe { result.push(raw::slice_bytes_unique(s, from, to)); }
 ...
 */
pub fn find_str_from<'a,'b>(haystack: &'a str, needle: &'b str, start: uint)
                         -> Option<uint> {
...
pub trait StrSlice {
    ...
    fn split_str<'a>(&self, sep: &'a str) -> ~[~str];
...
impl<'self> StrSlice for &'self str {
     ...
     */
    #[inline]
    fn split_str<'a>(&self, sep: &'a str) -> ~[~str] { split_str(*self, sep) }
    /// Returns true if one string starts with another
    #[inline]
    fn starts_with<'a>(&self, needle: &'a str) -> bool {
...
mod tests {
    ...
    #[test]
    fn test_split_str() {
        fn t<'a>(s: &str, sep: &'a str, i: int, k: &str) {
