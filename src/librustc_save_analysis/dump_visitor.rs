...
impl<'l, 'tcx> DumpVisitor<'l, 'tcx> {
        ...
        F: FnOnce(&mut Self),
    {
        let typeck_results = if self.tcx.has_typeck_results(item_def_id) {
            self.tcx.typeck(item_def_id)
        } else {
            self.save_ctxt.empty_typeck_results
        };

        let old_typeck_results = self.save_ctxt.typeck_results;
        self.save_ctxt.typeck_results = typeck_results;
        f(self);
        self.save_ctxt.typeck_results = old_typeck_results;
    }
