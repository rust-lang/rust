        ...
        match state {
                ...
                match cur {
                    ...
                    'c' => if stack.len() > 0 {
                        match stack.pop().unwrap() {
                            ...
                            _       => return Err("a non-char was used with %c".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    'p' => state = PushParam,
                    'P' => state = SetVar,
                    'g' => state = GetVar,
                    '\'' => state = CharConstant,
                    ...
                    'l' => if stack.len() > 0 {
                        match stack.pop().unwrap() {
                            ...
                            _        => return Err("a non-str was used with %l".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    '+' => if stack.len() > 1 {
                        match (stack.pop().unwrap(), stack.pop().unwrap()) {
                            (Number(y), Number(x)) => stack.push(Number(x + y)),
                            _ => return Err("non-numbers on stack with +".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    '-' => if stack.len() > 1 {
                        match (stack.pop().unwrap(), stack.pop().unwrap()) {
                    ...
                    'A' => if stack.len() > 1 {
                        match (stack.pop().unwrap(), stack.pop().unwrap()) {
                            ...
                            _ => return Err("non-numbers on stack with logical and".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    'O' => if stack.len() > 1 {
                        match (stack.pop().unwrap(), stack.pop().unwrap()) {
                            ...
                            (Number(_), Number(_)) => stack.push(Number(1)),
                            _ => return Err("non-numbers on stack with logical or".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    '!' => if stack.len() > 0 {
                        match stack.pop().unwrap() {
                            ...
                            _ => return Err("non-number on stack with logical not".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    '~' => if stack.len() > 0 {
                        match stack.pop().unwrap() {
                            Number(x) => stack.push(Number(!x)),
                            _         => return Err("non-number on stack with %~".into_string())
                        }
                    } else { return Err("stack is empty".into_string()) },
                    'i' => match (mparams[0].clone(), mparams[1].clone()) {
                        ...
                        (_, _) => return Err("first two params not numbers with %i".into_string())
                    },
            ...
            PushParam => {
                ...
                stack.push(mparams[match cur.to_digit(10) {
                    Some(d) => d - 1,
                    None => return Err("bad param number".into_string())
                }].clone());
            },
            SetVar => {
                if cur >= 'A' && cur <= 'Z' {
                    ...
                    return Err("bad variable name in %P".into_string());
                }
            },
            GetVar => {
                if cur >= 'A' && cur <= 'Z' {
                    ...
                    return Err("bad variable name in %g".into_string());
                }
            },
            CharConstant => {
            ...
            CharClose => {
                if cur != '\'' {
                    return Err("malformed character constant".into_string());
                }
            },
            IntConstant(i) => {
                match cur {
                    ...
                    _ => return Err("bad int constant".into_string())
                }
            }
            FormatPattern(ref mut flags, ref mut fstate) => {
                ...
                match (*fstate, cur) {
                    ...
                    (FormatStateWidth,'0'...'9') => {
                        ...
                        if flags.width < old { return Err("format width overflow".into_string()) }
                    }
                    (FormatStateWidth,'.') => {
                        *fstate = FormatStatePrecision;
                    }
                    (FormatStatePrecision,'0'...'9') => {
                        ...
                        if flags.precision < old {
                            return Err("format precision overflow".into_string())
                        }
                    }
                    _ => return Err("invalid format specifier".into_string())
                }
            }
            SeekIfElse(level) => {
                if cur == '%' {
    ...
    let mut s = match val {
        Number(d) => {
            let s = match (op, flags.sign) {
                ...
                (FormatHEX, _)       => format!("{:X}", d).into_bytes(),
                (FormatString, _)    => {
                    return Err("non-number on stack with %s".into_string())
                }
            };
    ...
    fn test_format() {
        ...
        assert_eq!(expand(b"%p1%s%p2%2s%p3%2s%p4%.2s",
                          &[Words("foo".into_string()),
                            Words("foo".into_string()),
                            Words("f".into_string()),
                            Words("foo".into_string())], vars),
                   Ok("foofoo ffo".bytes().collect::<Vec<_>>()));
        assert_eq!(expand(b"%p1%:-4.2s", &[Words("foo".into_string())], vars),
