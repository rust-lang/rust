...
fn subst_ident(&ext_ctxt cx, &vec[invk_binding] ibs, &ident i, ast_fold fld)
    -> ident {
...
fn subst_path(&ext_ctxt cx, &vec[invk_binding] ibs, &path_ p, ast_fold fld)
    -> path_ {
    // Don't substitute into qualified names.
    if (len(p.types) > 0u || len(p.idents) != 1u) { ret p; }
    ret alt (lookup(ibs, p.idents.(0))) {
        case (some(ident_binding(_, ?id))) { rec(idents=[id.node], types=[]) }
...
fn subst_expr(&ext_ctxt cx, &vec[invk_binding] ibs, &ast::expr_ e, 
        ...
        case (expr_path(?p)){
            ...
            alt (lookup(ibs, p.node.idents.(0))) {
                case (some(ident_binding(_, ?id))) { 
                    expr_path(respan(id.span, rec(idents=[id.node],types=[])))
                }
...
fn path_to_ident(&path pth) -> option::t[ident] {
    if (vec::len(pth.node.idents) == 1u 
        && vec::len(pth.node.types) == 0u) {
