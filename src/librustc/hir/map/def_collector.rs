...
impl<'a> DefCollector<'a> {
    ...
    fn visit_async_fn(
        ...
        return self.with_parent(fn_def, |this| {
            ...
            let closure_def =
                this.create_def(closure_id, DefPathData::ClosureExpr, REGULAR_SPACE, span);
...
impl<'a> visit::Visitor<'a> for DefCollector<'a> {
    fn visit_item(&mut self, i: &'a Item) {
        ...
        let def_data = match i.node {
            ...
            ItemKind::Fn(
                ref decl,
                ref header @ FnHeader {
                    asyncness: IsAsync::Async { .. },
                    ..
                },
                ref generics,
                ref body,
            ) => {
                return self.visit_async_fn(i.id, i.ident.name, i.span, header, generics, decl, body)
            }
    ...
    fn visit_trait_item(&mut self, ti: &'a TraitItem) {
        let def_data = match ti.node {
            ...
            TraitItemKind::Type(..) => DefPathData::AssocTypeInTrait(ti.ident.as_interned_str()),
    ...
    fn visit_expr(&mut self, expr: &'a Expr) {
        ...
        match expr.node {
            ...
            ExprKind::Async(_, async_id, _) => {
                let async_def =
                    self.create_def(async_id, DefPathData::ClosureExpr, REGULAR_SPACE, expr.span);
                self.parent_def = Some(async_def);
            }
            _ => {}
        };
