...
pub trait Visitor<'v> : Sized {
    ...
    fn visit_mod(&mut self, m: &'v Mod, _s: Span, h: HirId) {
        walk_mod(self, m, h)
    }
...
pub fn walk_stmt<'v, V: Visitor<'v>>(visitor: &mut V, statement: &'v Stmt) {
    match statement.node {
        StmtKind::Decl(ref declaration, hir_id) => {
            visitor.visit_id(hir_id);
            visitor.visit_decl(declaration)
        }
        StmtKind::Expr(ref expression, hir_id) |
        StmtKind::Semi(ref expression, hir_id) => {
            visitor.visit_id(hir_id);
...
impl IdRange {
    pub fn max() -> IdRange {
        IdRange {
            min: HirId { owner: CRATE_DEF_INDEX, local_id: ItemLocalId::MAX },
            max: HirId { owner: CRATE_DEF_INDEX, local_id: ItemLocalId::from_u32_const(0) },
        }
    }

    pub fn empty(&self) -> bool {
        self.min.local_id >= self.max.local_id
    }

    pub fn contains(&self, id: HirId) -> bool {
        id.local_id >= self.min.local_id && id.local_id < self.max.local_id
    }

    pub fn add(&mut self, id: HirId) {
        self.min.local_id = cmp::min(self.min.local_id, id.local_id);
        self.max.local_id = cmp::max(self.max.local_id,
                                     ItemLocalId::from_u32_const(id.local_id.as_u32() + 1));
    }
}
