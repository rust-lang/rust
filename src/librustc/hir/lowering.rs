...
pub struct LoweringContext<'r, 'a> {
    ...
    items: BTreeMap<hir::HirId, hir::Item<'a>>,
...
impl<'a> LoweringContext<'_, 'a> {
    fn lower_crate(mut self, c: &Crate) -> hir::Crate<'a> {
        ...
        struct MiscCollector<'l, 'tcx, 'interner> {
            lctx: &'l mut LoweringContext<'tcx, 'interner>,
        ...
        impl<'a, 'l, 'tcx, 'interner> Visitor<'a> for MiscCollector<'l, 'tcx, 'interner> {
        ...
        struct ItemLowerer<'l, 'tcx, 'interner: 'tcx> {
            lctx: &'l mut LoweringContext<'tcx, 'interner>,
        }

        impl<'l, 'tcx, 'interner> ItemLowerer<'l, 'tcx, 'interner> {
        ...
        impl<'a, 'l, 'tcx, 'interner> Visitor<'a> for ItemLowerer<'l, 'tcx, 'interner> {
    ...
    fn insert_item(&mut self, item: hir::Item<'a>) {
        let id = item.hir_id;
        ...
        F: FnOnce(&mut LoweringContext<'_, 'a>, &mut Vec<hir::GenericParam<'a>>) -> T,
    {
        ...
        let mut params: SmallVec<[_; 8]> = lowered_generics
            .params
            .iter()
            .cloned()
            .chain(in_band_defs)
        ...
        lowered_generics.params = params.into_iter().collect_hir_vec(self.arena);
    ...
    fn make_async_expr(
        ...
        let gen_future = self.expr_std_path(
            unstable_span, &[sym::future, sym::from_generator], None, thin_hir_vec![]);
        hir::ExprKind::Call(p!(self, gen_future), hir_vec![self; generator])
    }

    fn lower_body(
        &mut self,
        f: impl FnOnce(&mut LoweringContext<'_, 'a>) -> (HirVec<'a, hir::Arg<'a>>, hir::Expr<'a>),
    ) -> hir::BodyId {
    ...
    fn lower_fn_body(
        ...
        self.lower_body(|this| (
            decl.inputs.iter().map(|x| this.lower_arg(x)).collect_hir_vec(arena),
            body(this),
        ))
    }
    ...
    fn copy_attrs<I: IntoIterator<Item=Attribute>>(
        ...
        let vec = P::from_existing(self.arena.alloc_from_iter(attrs));
        if vec.is_empty() {
            P::empty_thin()
        } else {
        ...
        let kind = match t.node {
            ...
            TyKind::ImplicitSelf => {
                ...
                hir::TyKind::Path(hir::QPath::Resolved(
                    ...
                    p!(self, hir::Path {
                        res,
                        segments: hir_vec![self; hir::PathSegment::from_ident(
                            ...
                            hir::TypeBinding {
                                ...
                                kind: hir::TypeBindingKind::Equality {
                                    ty: output
                                        ...
                                        .unwrap_or_else(||
                                            p!(this, mk_tup(this, hir::HirVec::new(), span))
                                        ),
                                },
    ...
    fn lower_local(&mut self, l: &Local) -> (hir::Local<'a>, SmallVec<[NodeId; 1]>) {
    ...
    fn lower_async_fn_ret_ty(
        ...
        let (exist_ty_id, lifetime_params) = self.with_hir_id_owner(exist_ty_node_id, |this| {
            ...
            let exist_ty_item = hir::ExistTy {
                ...
                bounds: hir_vec![this; future_bound],
        ...
        hir::FunctionRetTy::Return(p!(self, hir::Ty {
            node: exist_ty_ref,
            span,
            hir_id: self.next_id(),
        }))
    ...
    fn lower_async_fn_output_type_to_future_bound(
        ...
        // "<Output = T>"
        let future_params = p!(self, hir::GenericArgs {
            args: hir_vec![],
            bindings: hir_vec![self; hir::TypeBinding {
                ident: Ident::with_empty_ctxt(FN_OUTPUT_NAME),
                kind: hir::TypeBindingKind::Equality {
                    ty: output_ty,
                },
    ...
    fn lower_maybe_async_body(
        ...
        self.lower_body(|this| {
            ...
            for (index, argument) in decl.inputs.iter().enumerate() {
                ...
                if is_simple_argument {
                    ...
                    let pattern_stmt = this.stmt_let_pat(
                        desugared_span, Some(p!(this, pattern_expr)), argument.pat,
                        hir::LocalSource::AsyncFn);
            ...
            (HirVec::from_slice(this.arena, &arguments),
        ...
        match tree.kind {
            UseTreeKind::Simple(rename, id1, id2) => {
                *ident = tree.ident();
                ...
                let path =
                    p!(self, self.lower_path_extra(ret_res, &path, ParamMode::Explicit, None));
            ...
            UseTreeKind::Nested(ref trees) => {
                ...
                let res = self.lower_res(res);
                let path = p!(self, self.lower_path_extra(res, &prefix, ParamMode::Explicit, None));
        ...
        if let ItemKind::MacroDef(ref def) = i.node {
            if !def.legacy || attr::contains_name(&i.attrs, sym::macro_export) ||
                ...
                self.exported_macros.push(hir::MacroDef {
                    ...
                    hir_id,
                    span: i.span,
                    body: P::from_existing(self.arena.alloc(body)),
                    legacy: def.legacy,
                });
            }
            return None;
    ...
    fn lower_expr(&mut self, e: &Expr) -> hir::Expr<'a> {
        ...
        let kind = match e.node {
            ...
            ExprKind::MethodCall(ref seg, ref args) => {
                let hir_seg = p!(self, self.lower_path_segment(
                    e.span,
                    seg,
                    ParamMode::Optional,
                    0,
            ...
            ExprKind::Lit(ref l) => {
                let lit = respan(l.span, l.node.clone());
                let lit = P::from_existing(self.arena.alloc(lit));
            ...
            ExprKind::If(ref cond, ref then, ref else_opt) => {
                ...
                let then_arm = self.arm(hir_vec![self; then_pat], p!(self, then_expr));
                ...
                let else_expr = match else_opt {
                    ...
                    Some(els) => match els.node {
                        ExprKind::IfLet(..) => {
                            ...
                            let blk = self.block_all(els.span, hir_vec![], Some(p!(self, els)));
                            self.expr_block(p!(self, blk), thin_hir_vec![])
                        }
                        _ => self.lower_expr(els),
                    }
                };
                let else_arm = self.arm(hir_vec![self; else_pat], p!(self, else_expr));
                ...
                let cond = self.expr_drop_temps(span_block, p!(self, cond), thin_hir_vec![]);

                hir::ExprKind::Match(
                    p!(self, cond),
                    hir_vec![self; then_arm, else_arm],
            ...
            ExprKind::Range(ref e1, ref e2, lims) => {
                ...
                if is_unit {
                    ...
                    hir::ExprKind::Struct(p!(self, struct_path), fields, None)
                }
            }
            ...
            ExprKind::InlineAsm(ref asm) => {
                ...
                let inputs = asm.inputs
                    ...
                    .collect_hir_vec(arena);
                let hir_asm = P::from_existing(self.arena.alloc(hir_asm));
            ...
            ExprKind::IfLet(ref pats, ref sub_expr, ref body, ref else_opt) => {
                ...
                // _ => [<else_opt>|{}]
                {
                    ...
                    arms.push(self.arm(hir_vec![self; wildcard_pattern], p!(self, body)));
                }
            ...
            ExprKind::ForLoop(ref pat, ref head, ref body, opt_label) => {
                ...
                let pat_arm = {
                    ...
                    let val_expr = p!(self, self.expr_ident(pat.span, val_ident, val_pat_hid));
                    let next_expr = p!(self, self.expr_ident(pat.span, next_ident, next_pat_hid));
                ...
                // `match ::std::iter::Iterator::next(&mut iter) { ... }`
                let match_expr = {
                    let iter = p!(self, self.expr_ident(head_sp, iter, iter_pat_nid));
                    let ref_mut_iter = self.expr_mut_addr_of(head_sp, iter);
                    let next_path = &[sym::iter, sym::Iterator, sym::next];
                    let next_expr = p!(self, self.expr_call_std_path(
                        head_sp,
                        next_path,
                        hir_vec![self; ref_mut_iter],
                    ));
                    let arms = hir_vec![self; pat_arm, break_arm];
                ...
                let next_expr = p!(self, self.expr_ident(head_sp, next_ident, next_pat_hid));
                ...
                let into_iter_expr = {
                    ...
                    p!(self, self.expr_call_std_path(
                        head_sp,
                        into_iter_path,
                        hir_vec![self; head],
                    ))
                ...
                return self.expr_drop_temps(head_sp, match_expr, attrs)
            }
            ...
            ExprKind::Try(ref sub_expr) => {
                ...
                let err_arm = {
                    ...
                    let from_expr = {
                        ...
                        self.expr_call_std_path(try_span, from_path, hir_vec![self; err_expr])
                    };
                    let from_err_expr =
                    ...
                    let ret_expr = if let Some(catch_node) = catch_scope {
                        ...
                        p!(
                            self,
                            self.expr(try_span, hir::ExprKind::Ret(Some(from_err_expr)), attrs)
                        )
                    };
    ...
    fn lower_stmt(&mut self, s: &Stmt) -> SmallVec<[hir::Stmt<'a>; 1]> {
        smallvec![match s.node {
            StmtKind::Local(ref l) => {
                ...
                ids.push({
                    hir::Stmt {
                        ...
                        node: hir::StmtKind::Local(p!(self, l)),
                        span: s.span,
                    }
                });
                return ids;
            ...
            StmtKind::Expr(ref e) => {
                hir::Stmt {
                    hir_id: self.lower_node_id(s.id),
                    node: hir::StmtKind::Expr(p!(self, self.lower_expr(e))),
                    span: s.span,
                }
            },
            StmtKind::Semi(ref e) => {
                hir::Stmt {
                    hir_id: self.lower_node_id(s.id),
                    node: hir::StmtKind::Semi(p!(self, self.lower_expr(e))),
                    span: s.span,
                }
            },
    ...
    fn expr_call_std_path(
        ...
        let path = p!(self, self.expr_std_path(span, path_components, None, thin_hir_vec![]));
    ...
    fn expr_call_std_assoc_fn(
        ...
        let ty_path = p!(self, self.std_path(span, ty_path_components, None, false));
        let ty = p!(self, self.ty_path(ty_path_id, span, hir::QPath::Resolved(None, ty_path)));
        let fn_seg = p!(self, hir::PathSegment::from_ident(Ident::from_str(assoc_fn_name)));
        let fn_path = hir::QPath::TypeRelative(ty, fn_seg);
        let fn_expr = p!(self, self.expr(span, hir::ExprKind::Path(fn_path), thin_hir_vec![]));
        hir::ExprKind::Call(fn_expr, args)
    }

    fn expr_ident(&mut self, span: Span, ident: Ident, binding: hir::HirId) -> hir::Expr<'a> {
    ...
    fn expr_unit(&mut self, sp: Span) -> hir::Expr<'a> {
        self.expr_tuple(sp, hir_vec![])
    }

    fn expr_tuple(&mut self, sp: Span, exprs: hir::HirVec<'a, hir::Expr<'a>>) -> hir::Expr<'a> {
        self.expr(sp, hir::ExprKind::Tup(exprs), thin_hir_vec![])
    }
    ...
    fn stmt(&mut self, span: Span, node: hir::StmtKind<'a>) -> hir::Stmt<'a> {
    ...
    fn stmt_let_pat(
        ...
        self.stmt(span, hir::StmtKind::Local(p!(self, local)))
    }

    fn expr_block_empty(&mut self, span: Span) -> hir::Expr<'a> {
    ...
    fn expr_unsafe(&mut self, expr: P<'a, hir::Expr<'a>>) -> hir::Expr<'a> {
        let hir_id = self.next_id();
        let span = expr.span;
        self.expr(
            span,
            hir::ExprKind::Block(p!(self, hir::Block {
                stmts: hir_vec![],
                expr: Some(expr),
                hir_id,
    ...
    fn pat_bool(&mut self, span: Span, val: bool) -> P<'a, hir::Pat<'a>> {
        let lit = Spanned { span, node: LitKind::Bool(val) };
        let lit = P::from_existing(self.arena.alloc(lit));
        let expr = self.expr(span, hir::ExprKind::Lit(lit), thin_hir_vec![]);
        self.pat(span, hir::PatKind::Lit(p!(self, expr)))
    }

    fn pat_ok(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &[sym::result, sym::Result, sym::Ok], hir_vec![self; pat])
    }

    fn pat_err(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &[sym::result, sym::Result, sym::Err], hir_vec![self; pat])
    }

    fn pat_some(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &[sym::option, sym::Option, sym::Some], hir_vec![self; pat])
    }
    ...
    fn pat_ident(&mut self, span: Span, ident: Ident) -> (P<'a, hir::Pat<'a>>, hir::HirId) {
    ...
    fn lower_await(
        ...
        // })`
        let poll_expr = {
            let pinned = p!(self, self.expr_ident(span, pinned_ident, pinned_pat_hid));
            ...
            let new_unchecked_expr_kind = self.expr_call_std_assoc_fn(
                ...
                &[sym::pin, sym::Pin],
                "new_unchecked",
                hir_vec![self; ref_mut_pinned],
            );
            let new_unchecked = p!(self, self.expr(span, new_unchecked_expr_kind, thin_hir_vec![]));
            let unsafe_expr = self.expr_unsafe(new_unchecked);
            p!(self, self.expr_call_std_path(
                gen_future_span,
                &[sym::future, sym::poll_with_tls_context],
                hir_vec![self; unsafe_expr],
            ))
        };
        ...
        let ready_arm = {
            ...
            let x_expr = p!(self, self.expr_ident(span, x_ident, x_pat_hid));
            ...
            let break_x = self.with_loop_scope(loop_node_id, |this| {
                ...
                p!(this, this.expr(await_span, expr_break, thin_hir_vec![]))
            });
        ...
        let pending_arm = {
            ...
            let empty_block = p!(self, self.expr_block_empty(span));
            self.arm(hir_vec![self; pending_pat], empty_block)
        };
        ...
        let match_stmt = {
            let match_expr = p!(self, self.expr_match(
                span,
                poll_expr,
                hir_vec![self; ready_arm, pending_arm],
        ...
        let yield_stmt = {
            ...
            let yield_expr = p!(self, self.expr(
                span,
                hir::ExprKind::Yield(p!(self, unit)),
                thin_hir_vec![],
            ));
        ...
        hir::ExprKind::Block(
            p!(self, self.block_all(span, hir_vec![self; pinned_let], Some(loop_expr))),
            None,
        )
    }
}
