...
use lint::builtin::{self, ELIDED_LIFETIMES_IN_PATHS, PARENTHESIZED_PARAMS_IN_TYPES_AND_MODULES};
...
pub trait Resolver {
    ...
    fn resolve_hir_path(&mut self, path: &ast::Path, is_value: bool) -> hir::Path;
...
impl<'a> LoweringContext<'a> {
            ...
            fn visit_item(&mut self, item: &'lcx Item) {
                ...
                if item_lowered {
                    let item_generics = match self.lctx.items.get(&item.id).unwrap().node {
                        ...
                        | hir::ItemKind::Trait(_, _, ref generics, ..) => generics.params.clone(),
                        _ => HirVec::new(),
                    };

                    self.lctx
                        .with_parent_impl_lifetime_defs(&item_generics, |this| {
            ...
            .map(|(span, hir_name)| {
                ...
                let (str_name, kind) = match hir_name {
                    ParamName::Plain(ident) => {
                        (ident.as_interned_str(), hir::LifetimeParamKind::InBand)
                    }
                    ParamName::Fresh(_) => (
    ...
    fn lower_token_tree(&mut self, tree: TokenTree) -> TokenStream {
        match tree {
            ...
            TokenTree::Delimited(span, delim, tts) => {
                TokenTree::Delimited(span, delim, self.lower_token_stream(tts.into()).into()).into()
            }
        }
    }
    ...
    fn lower_existential_impl_trait(
        ...
        let exist_ty_span =
            self.allow_internal_unstable(CompilerDesugaringKind::ExistentialReturnType, span);
        ...
        impl<'r, 'a: 'r, 'v> hir::intravisit::Visitor<'v> for ImplTraitLifetimeCollector<'r, 'a> {
            ...
            fn visit_lifetime(&mut self, lifetime: &'v hir::Lifetime) {
                    ...
                    && !self.already_defined_lifetimes.contains(&name)
                {
                    ...
                    self.output_lifetimes
                        .push(hir::GenericArg::Lifetime(hir::Lifetime {
                    ...
                    let _ = self
                        .context
                        .lower_node_id_with_owner(def_node_id, self.exist_ty_id);
    ...
    fn lower_qpath(
        ...
        let qself = qself
            .as_ref()
            .map(|q| self.lower_ty(&q.ty, itctx.reborrow()));
        ...
        if !generic_args.parenthesized && !has_lifetimes {
            ...
            if expected_lifetimes > 0 && param_mode == ParamMode::Explicit {
                ...
                let (incl_angl_brckt, insertion_span, suggestion) = if no_ty_args && no_bindings {
                    ...
                    (
                        false,
                        first_generic_span.shrink_to_lo(),
                        format!("{}, ", anon_lt_suggestion),
                    )
                };
    ...
    fn lower_angle_bracketed_parameter_data(
        ...
        (
            hir::GenericArgs {
                args: args
                    .iter()
                    .map(|a| self.lower_generic_arg(a, itctx.reborrow()))
                    .collect(),
                bindings: bindings
                    .iter()
            ...
            implicit_self: decl
                ...
                .map_or(hir::ImplicitSelfKind::None, |arg| {
                    let is_mutable_pat = match arg.pat.node {
                        ...
                        | PatKind::Ident(BindingMode::ByRef(mt), _, _) => mt == Mutability::Mutable,
                        _ => false,
                    };
                    ...
                    match arg.ty.node {
                        ...
                        TyKind::Rptr(_, ref mt)
                            if mt.ty.node.is_implicit_self()
    ...
    fn lower_async_fn_ret_ty(
                ...
                if !self.currently_bound_lifetimes.contains(&name) {
                    if let Some((current_lt_name, current_lt_span)) = self.output_lifetime {
                        ...
                        if current_lt_name != name {
                            ...
                            .help(
                                "`async fn` can only accept borrowed values \
        ...
        let impl_trait_ty = self.lower_existential_impl_trait(
            ...
            |this| {
                ...
                let LoweredNodeId { node_id, hir_id } = this.next_id();
                let mut bounds = vec![hir::GenericBound::Trait(
                    hir::PolyTraitRef {
                ...
                if let Some((name, span)) = bound_lifetime {
                    bounds.push(hir::GenericBound::Outlives(hir::Lifetime {
                        id: this.next_id().node_id,
                        name,
                        span,
                    }));
        ...
        match param.kind {
            ...
            GenericParamKind::Type { ref default, .. } => {
                ...
                if !add_bounds.is_empty() {
                    let params = self
                        .lower_param_bounds(add_bounds, itctx.reborrow())
                        .into_iter();
    ...
    fn lower_poly_trait_ref(
        ...
        let trait_ref = self.with_parent_impl_lifetime_defs(&bound_generic_params, |this| {
    ...
    fn lower_param_bounds(
        ...
        bounds
            .iter()
            .map(|bound| self.lower_param_bound(bound, itctx.reborrow()))
            .collect()
    }
    ...
    fn lower_async_body(&mut self, decl: &FnDecl, asyncness: IsAsync, body: &Block) -> hir::BodyId {
        self.lower_body(Some(decl), |this| {
            if let IsAsync::Async { closure_id, .. } = asyncness {
                let async_expr = this.make_async_expr(CaptureBy::Value, closure_id, None, |this| {
        ...
        match *i {
            ...
            ItemKind::Fn(ref decl, header, ref generics, ref body) => {
                ...
                self.with_new_scopes(|this| {
                    ...
                    hir::ItemKind::Fn(fn_decl, this.lower_fn_header(header), generics, body_id)
                })
            }
            ...
            ItemKind::Existential(ref b, ref generics) => {
                ...
                *ident = tree.ident();
                ...
                for (def, &new_node_id) in defs.zip([id1, id2].iter()) {
                    ...
                    self.with_hir_id_owner(new_node_id, |this| {
                        ...
                        let path = this.lower_path_extra(def, &path, ParamMode::Explicit, None);
                ...
                let path = P(self.lower_path_extra(ret_def, &path, ParamMode::Explicit, None));
            ...
            UseTreeKind::Nested(ref trees) => {
                ...
                for &(ref use_tree, id) in trees {
                    ...
                    self.with_hir_id_owner(new_id, |this| {
                        let item = this
                            .lower_use_tree(use_tree, &prefix, new_id, &mut vis, &mut ident, attrs);
    ...
    fn lower_trait_item(&mut self, i: &TraitItem) -> hir::TraitItem {
        ...
        let (generics, node) = match i.node {
            ...
            TraitItemKind::Method(ref sig, None) => {
                ...
                (
                    generics,
                    hir::TraitItemKind::Method(sig, hir::TraitMethod::Required(names)),
                )
            }
            TraitItemKind::Method(ref sig, Some(ref body)) => {
                ...
                (
                    generics,
                    hir::TraitItemKind::Method(sig, hir::TraitMethod::Provided(body_id)),
                )
            }
    ...
    fn lower_item_id(&mut self, i: &Item) -> SmallVec<[hir::ItemId; 1]> {
        match i.node {
            ...
            ItemKind::Fn(..) | ItemKind::Impl(.., None, _, _) => {
                smallvec![hir::ItemId { id: i.id }]
            }
    ...
    fn lower_item_id_use_tree(
        ...
        match tree.kind {
            ...
            UseTreeKind::Simple(_, id1, id2) => {
                for (_, &id) in self
                    .expect_full_def_from_use(base_id)
                    .skip(1)
                    .zip([id1, id2].iter())
                {
                    vec.push(hir::ItemId { id });
    ...
    pub fn lower_item(&mut self, i: &Item) -> Option<hir::Item> {
        ...
        if let ItemKind::MacroDef(ref def) = i.node {
            if !def.legacy
                || attr::contains_name(&i.attrs, "macro_export")
    ...
    fn lower_foreign_item(&mut self, i: &ForeignItem) -> hir::ForeignItem {
        ...
        hir::ForeignItem {
            ...
            node: match i.node {
                ...
                ForeignItemKind::Static(ref t, m) => hir::ForeignItemKind::Static(
                    self.lower_ty(t, ImplTraitContext::disallowed()),
                    m,
                ),
    ...
    fn lower_expr(&mut self, e: &Expr) -> hir::Expr {
        let kind = match e.node {
            ...
            ExprKind::TryBlock(ref body) => self.with_catch_scope(body.id, |this| {
                let unstable_span =
            ...
            ExprKind::ForLoop(ref pat, ref head, ref body, opt_label) => {
                ...
                let desugared_span =
                    self.allow_internal_unstable(CompilerDesugaringKind::ForLoop, head_sp);
                ...
                let into_iter_expr = {
                    ...
                    let into_iter =
                        P(self.expr_std_path(head_sp, into_iter_path, None, ThinVec::new()));
            ...
            ExprKind::Try(ref sub_expr) => {
                ...
                let discr = {
                    ...
                    let path = P(self.expr_std_path(unstable_span, path, None, ThinVec::new()));
                ...
                let err_arm = {
                    ...
                    let from_expr = {
                        ...
                        let from = P(self.expr_std_path(e.span, path, None, ThinVec::new()));
    ...
    fn lower_visibility(
        ...
        let node = match v.node {
            ...
            VisibilityKind::Restricted { ref path, id } => {
                ...
                let def = self.expect_full_def(id);
                hir::VisibilityKind::Restricted {
                    path: P(self.lower_path_extra(def, path, ParamMode::Explicit, explicit_owner)),
    ...
    fn expr_mut_addr_of(&mut self, span: Span, e: P<hir::Expr>) -> hir::Expr {
        self.expr(
            span,
            hir::ExprKind::AddrOf(hir::MutMutable, e),
            ThinVec::new(),
        )
    }
    ...
    fn expr_match(
        ...
        self.expr(
            span,
            hir::ExprKind::Match(arg, arms, source),
            ThinVec::new(),
        )
    }
