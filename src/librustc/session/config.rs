...
options! {CodegenOptions, CodegenSetter, basic_codegen_options,
    ...
    target_feature: String = ("".to_owned(), parse_string,
    ...
    extra_filename: String = ("".to_owned(), parse_string,
...
pub fn parse_cfgspecs(cfgspecs: Vec<String> ) -> ast::CrateConfig {
    cfgspecs.into_iter().map(|s| {
        parse::parse_meta_from_source_str("cfgspec".to_owned(),
                                          s.to_string(),
                                          Vec::new(),
                                          &parse::ParseSess::new())
...
mod tests {
    ...
    fn test_switch_implies_cfg_test() {
        let matches =
            &match getopts(&["--test".to_owned()], &optgroups()) {
              Ok(m) => m,
