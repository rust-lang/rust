...
use super::{PredicateObligation, PredicateObligations, TraitObligation, ObligationCause};
...
///
/// - `Ok(Some(Selection))`: success occurred with result `r`
/// - `Ok(None)`: could not definitely determine anything, usually due to
///               inconclusive type inference.
/// - `Err(e)`: error `e` occurred
pub type SelectionResult<'tcx> = Result<Option<Selection<'tcx>>, SelectionError<'tcx>>;

type SelectionCandidateResult<'tcx> =
    Result<Option<SelectionCandidate<'tcx>>, SelectionError<'tcx>>;
...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
    ...
    fn maybe_extend_obligations<I>(&mut self, obligations: I)
        where I: IntoIterator<Item=PredicateObligation<'tcx>>
    {
        if let Some(ref mut inferred_obligations) = self.inferred_obligations {
            inferred_obligations.extend(obligations);
        }
    }

    /// Uses the specified inferred obligations storage for the operation `f`. Restores the old
    /// inferred obligations upon finishing `f`.
    fn with_inferred_obligations_storage<F, R>(&mut self,
                                               store: Option<PredicateObligations<'tcx>>,
                                               f: F)
        -> (R, Option<PredicateObligations<'tcx>>)
        where F: FnOnce(&mut Self) -> R
    {
        let old_obligations = mem::replace(&mut self.inferred_obligations, store);
        let result = f(self);
        (result, mem::replace(&mut self.inferred_obligations, old_obligations))
    }
    ...
    {
        self.with_inferred_obligations_storage(None, |this| {
            this.infcx.probe(|snapshot| f(this, snapshot))
        }).0
    }
    ...
    {
        if self.inferred_obligations.is_some() {
            let (result, inferred) =
                self.with_inferred_obligations_storage(
                    Some(PredicateObligations::new()), |this| {
                        this.infcx.commit_if_ok(|snapshot| f(this, snapshot))
                    });
            match result {
                Ok(ok) => {
                    self.inferred_obligations.as_mut().unwrap().extend(inferred.unwrap());
                    Ok(ok)
                },
                Err(err) => Err(err),
            }
    ...
    pub fn select(&mut self, obligation: &TraitObligation<'tcx>)
        ...
        match self.with_inferred_obligations_storage(
            None, |this| this.candidate_from_obligation(&stack))
        {
            (Ok(None), _) => Ok(None),
            (Ok(Some(candidate)), _) => {
                let (candidate_result, inferred_obligations) =
                    self.with_inferred_obligations_storage(
                        Some(PredicateObligations::new()),
                        |this| this.confirm_candidate(obligation, candidate));
                candidate_result.map(|mut candidate| {
                    candidate.nested_obligations_mut().extend(inferred_obligations.unwrap());
                    Some(candidate)
                })
            },
            (Err(err), _) => Err(err),
        ...
        match obligation.predicate {
            ...
            ty::Predicate::Equate(ref p) => {
                ...
                match self.infcx.equality_predicate(obligation.cause.span, p) {
                    Ok(InferOk { .. }) => {
                        // We don't collect new obligations during evaluation, as we're supposed to
                        // be in a probe.
                        EvaluatedToOk
                    },
                        ...
                        -> bool
    {
        ...
        match self.infcx.sub_poly_trait_refs(false,
            ...
            Ok(InferOk { obligations, .. }) => {
                self.maybe_extend_obligations(obligations);
            }
            Err(_) => { return false; }
        }
    ...
    /// Given a selection candidate, performs mutation of the inference context and returns the
    /// selection result with all inferred obligations. Clears `inferred_obligations`.
                         ...
                         -> Result<Selection<'tcx>,SelectionError<'tcx>>
    {
        ...
        match candidate {
            BuiltinCandidate(builtin_bound) => {
                let vtable_builtin = self.confirm_builtin_candidate(obligation, builtin_bound)?;
                Ok(VtableBuiltin(vtable_builtin))
            }
            ...
            DefaultImplCandidate(trait_def_id) => {
                let vtable_default = self.confirm_default_impl_candidate(obligation, trait_def_id);
                Ok(VtableDefaultImpl(vtable_default))
            }

            DefaultImplObjectCandidate(trait_def_id) => {
                let vtable_impl_object =
                    self.confirm_default_impl_object_candidate(obligation, trait_def_id);
                Ok(VtableDefaultImpl(vtable_impl_object))
            }

            ImplCandidate(impl_def_id) => {
                let vtable_impl = self.confirm_impl_candidate(obligation, impl_def_id)?;
            ...
            ObjectCandidate => {
                let vtable_object = self.confirm_object_candidate(obligation);
                Ok(VtableObject(vtable_object))
            }

            FnPointerCandidate => {
                let fn_type = self.confirm_fn_pointer_candidate(obligation)?;
                Ok(VtableFnPointer(fn_type, vec![]))
            }
            ...
            BuiltinUnsizeCandidate => {
                let vtable_builtin_unsize = self.confirm_builtin_unsize_candidate(obligation)?;
                Ok(VtableBuiltin(vtable_builtin_unsize))
            }
        }
    }
                               ...
                               -> Result<(), SelectionError<'tcx>>
    {
        ...
        self.infcx.sub_poly_trait_refs(false,
            ...
            .map(|InferOk { obligations, .. }| self.maybe_extend_obligations(obligations))
