...
pub fn ty_to_string<'tcx>(cx: &ctxt<'tcx>, typ: Ty<'tcx>) -> String {
    ...
    fn push_sig_to_string<'tcx>(cx: &ctxt<'tcx>,
                                ...
                                bounds: &str) {
        s.push_char(bra);
        let strs: Vec<String> = sig.inputs.iter().map(|a| ty_to_string(cx, *a)).collect();
...
impl<'tcx, T:Repr<'tcx>> Repr<'tcx> for infer::Bounds<T> {
    fn repr(&self, tcx: &ctxt<'tcx>) -> String {
        format!("({} <= {})",
                self.lb.repr(tcx),
                self.ub.repr(tcx))
