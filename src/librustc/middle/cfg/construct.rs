...
pub fn construct(tcx: &ty::ctxt,
    ...
    let CFGBuilder {graph, has_unreachable, ..} = cfg_builder;
...
impl<'a, 'tcx> CFGBuilder<'a, 'tcx> {
    ...
    fn match_(&mut self, id: ast::NodeId, discr: &ast::Expr,
        ...
        // The CFG for match expression is quite complex, so no ASCII art for it.
        //
        // The CFG generated below matches roughly what trans puts out. Each pattern and guard is
        // visited in parallel, with arms containing multiple patterns generating multiple nodes for
        // the same guard expression. The guard expressions chain into each other from top to
        // bottom, with a specific exception to allow some additional valid programs (explained
        // below). Trans differs slightly in that the pattern matching may continue after a guard
        // but the visible behaviour should be the same.
        //
        ...
        for arm in arms {
            // Add an exit node for when we've visited all the patterns and the
            // guard (if there is one) in the arm.
            ...
            for pat in &arm.pats {
                ...
                if let Some(ref guard) = arm.guard {
                    // Add a dummy node for the previous guard expression to target
                    ...
                    // If both this pattern and the previous pattern were free of guards, they must
                    // consist only of "constant" patterns. Since we can't match an all-constant
                    // pattern, fail the guard, then match *another* all-constant pattern. This is
                    // because if the previous pattern matches, then we *can't* match this one,
                    // unless all the constants are the same (which is rejected by `check_match`).
                    //
                    // We can use this to be smarter about the flow along guards. If the previous
                    // pattern matched, then we know we won't visit the guard in this one (whether
                    // or not the guard succeeded), if the previous pattern failed, then we know the
                    // guard for that pattern won't have been visited. Thus, it isn't possible to
                    // visit both the previous guard and the current one when both patterns consist
                    // only of constant sub-patterns.
                    //
                    // However, if the above does not hold, then all previous guards need to be
                    // wired to visit the current guard pattern.
    ...
    fn add_ast_node(&mut self, id: ast::NodeId, preds: &[CFGIndex]) -> CFGIndex {
        if id == ast::DUMMY_NODE_ID {
            self.add_dummy_node(preds)
        } else {
            let data = CFGNodeData::AST(id);
            self.add_node(data, preds)
        }
    }
    ...
    fn add_node(&mut self, data: CFGNodeData, preds: &[CFGIndex]) -> CFGIndex {
        ...
        for &pred in preds {
            // Don't add edges from unreachable nodes
            if *self.graph.node_data(pred) != CFGNodeData::Unreachable {
...
fn remove_unreachable(cfg: CFG) -> CFG {
    ...
    let mut new_entry = graph::InvalidNodeIndex;
    let mut new_exit = graph::InvalidNodeIndex;

    let mut idx_map = FnvHashMap();

    cfg.graph.each_node(|idx, node| {
        if node.data != CFGNodeData::Unreachable {
            let new_idx = new_graph.add_node(node.data);
    ...
    for e in cfg.graph.all_edges() {
        let new_source = idx_map.get(&e.source());
        let new_target = idx_map.get(&e.target());

        match (new_source, new_target) {
            (Some(&src), Some(&tgt)) => {
                new_graph.add_edge(src, tgt, e.data.clone());
            }
            _ => ()
        }
    }
