...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
                                                   ...
                                                   -> bool
    {
        ...
        match (candidate_i, candidate_j) {
            ...
            (&BuiltinUnsizeCandidate(..), &ParamCandidate(_)) |
            ...
            self.infcx.try(|snapshot| {
                ...
                let skol_predicate =
                    util::predicate_for_trait_def(
                        ...
                        skol_ty,
                        vec![]);
                Ok(self.infcx().plug_leaks(skol_map, snapshot, &skol_predicate))
            })
    ...
    fn confirm_builtin_unsize_candidate(&mut self,
        ...
        match (&source.sty, &target.sty) {
            ...
            (_, &ty::ty_trait(ref data)) => {
                let object = data.principal_trait_ref_with_self_ty(tcx, tcx.types.err);
                if !object_safety::is_object_safe(tcx, object.clone()) {
                    return Err(TraitNotObjectSafe(object));
                }
        ...
        Ok(VtableBuiltin(VtableBuiltinData {
