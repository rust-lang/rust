                    ...
                    if let hir::GenericArg::Lifetime(lifetime) = lifetime {
                        ...
                        if let Some(Region::LateBound(_, def_id, _)) = def {
                            if let Some(hir_id) = self.tcx.hir().as_local_hir_id(def_id) {
                                ...
                                let parent_impl_id = hir::ImplItemId { node_id: parent_node_id };
                                let parent_trait_id = hir::TraitItemId { node_id: parent_node_id };
        ...
        match trait_item.node {
            Method(ref sig, _) => {
                ...
                self.visit_early_late(
                    Some(tcx.hir().get_parent(trait_item.hir_id)),
                    &sig.decl,
                    &trait_item.generics,
    ...
    fn visit_impl_item(&mut self, impl_item: &'tcx hir::ImplItem) {
        ...
        match impl_item.node {
            Method(ref sig, _) => {
                let tcx = self.tcx;
                self.visit_early_late(
                    Some(tcx.hir().get_parent(impl_item.hir_id)),
                    &sig.decl,
                    &impl_item.generics,
    ...
    fn suggest_eliding_single_use_lifetime(
        ...
        if let Node::Lifetime(hir_lifetime) = self.tcx.hir().get_by_hir_id(lifetime.hir_id) {
            if let Some(parent) = self.tcx.hir().find_by_hir_id(
                self.tcx.hir().get_parent(hir_lifetime.hir_id))
            {
                match parent {
                    Node::Item(item) => {
            ...
            match lifetimeuseset {
                Some(LifetimeUseSet::One(lifetime)) => {
                    ...
                    if let Some((id, span, name)) = match self.tcx.hir().get_by_hir_id(hir_id) {
                        ...
                        debug!("id = {:?} span = {:?} name = {:?}", hir_id, span, name);
        ...
        let body = match self.tcx.hir().get_by_hir_id(parent) {
            ...
            Node::TraitItem(&hir::TraitItem {
                ...
                if let hir::ItemKind::Trait(.., ref trait_items) = self.tcx
                    .hir()
                    .expect_item_by_hir_id(self.tcx.hir().get_parent(parent))
                    .node
                {
                    assoc_item_kind = trait_items
                        .iter()
                        .find(|ti| ti.id.node_id == self.tcx.hir().hir_to_node_id(parent))
                        .map(|ti| ti.kind);
                }
                match *m {
            ...
            Node::ImplItem(&hir::ImplItem {
                    ...
                    .node
                {
                    ...
                    assoc_item_kind = impl_items
                        .iter()
                        .find(|ii| ii.id.node_id == self.tcx.hir().hir_to_node_id(parent))
                        .map(|ii| ii.kind);
                }
                Some(body)
            }
    ...
    fn report_elision_failure(
        ...
        for (i, info) in elided_params.into_iter().enumerate() {
            ...
            let help_name = if let Some(body) = parent {
                ...
                format!("`{}`", self.tcx.hir().node_to_pretty_string_by_hir_id(arg.pat.hir_id))
            } else {
