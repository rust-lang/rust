...
impl<'a, 'tcx> CheckLoanCtxt<'a, 'tcx> {
    ...
    pub fn report_error_if_loan_conflicts_with_restriction(&self,
        ...
        for restr_path in loan1.restricted_paths.iter() {
                ...
                (_, ty::UniqueImmBorrow) => {
                    self.bccx.span_err(
                        ...
                        format!("cannot borrow `{}` as {} because \
                                ...
                                self.bccx.loan_path_to_string(&*new_loan.loan_path),
                                new_loan.kind.to_user_str()).as_str());
                }
                ...
                (_, _) => {
                    self.bccx.span_err(
                        ...
                        format!("cannot borrow `{}` as {} because \
                                ...
                                old_pronoun,
                                old_loan.kind.to_user_str()).as_str());
                }
            }
            ...
            self.bccx.span_note(
                old_loan.span,
                format!("{}; {}", borrow_summary, rule_summary).as_str());
            ...
            UseWhileBorrowed(loan_path, loan_span) => {
                self.bccx.span_err(
                    ...
                    format!("cannot use `{}` because it was mutably borrowed",
                            self.bccx.loan_path_to_string(copy_path).as_str())
                    .as_str());
                self.bccx.span_note(
                    loan_span,
                    format!("borrow of `{}` occurs here",
                            self.bccx.loan_path_to_string(&*loan_path).as_str())
                    .as_str());
            }
        }
    }
    ...
    fn check_for_move_of_borrowed_path(&self,
        ...
        match self.analyze_restrictions_on_use(id, move_path, ty::MutBorrow) {
            ...
            UseWhileBorrowed(loan_path, loan_span) => {
                let err_message = match move_kind {
                    move_data::Captured =>
                        format!("cannot move `{}` into closure because it is borrowed",
                                self.bccx.loan_path_to_string(move_path).as_str()),
                    move_data::Declared |
                    move_data::MoveExpr |
                    move_data::MovePat =>
                ...
                self.bccx.span_err(span, err_message.as_str());
                self.bccx.span_note(
                    loan_span,
                    format!("borrow of `{}` occurs here",
                            self.bccx.loan_path_to_string(&*loan_path).as_str())
            ...
            Some(lp) => {
                self.bccx.span_err(
                    ...
                    format!("cannot assign to {} {} `{}`",
                            ...
                            self.bccx.loan_path_to_string(&*lp)).as_str());
            }
            None => {
                self.bccx.span_err(
                    assignment_span,
                    format!("cannot assign to {} {}",
                            assignee_cmt.mutbl.to_user_str(),
                            self.bccx.cmt_to_string(&*assignee_cmt)).as_str());
            }
        }
        return;
