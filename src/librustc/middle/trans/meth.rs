...
pub fn trans_impl(ccx: &CrateContext,
    ...
    for method in methods.iter() {
        if method.generics.ty_params.len() == 0u {
            let llfn = match get_item_val(ccx, method.id) {
                LvalueDatum(_) => {
...
pub fn trans_method_callee(bcx: &Block, method_call: MethodCall) -> Callee {
    ...
    match origin {
        typeck::MethodStatic(did) => {
            Fn(callee::trans_fn_ref(bcx, did, MethodCall(method_call)))
        }
            ...
            match bcx.fcx.param_substs {
                Some(substs) => {
                    ...
                    trans_monomorphized_callee(bcx, method_call,
                                               trait_id, off,
                                               find_vtable(bcx.tcx(), substs, p, b))
                }
        ...
        typeck::MethodObject(ref mt) => {
            TraitMethod(monomorphize_type(bcx, method_ty), mt.real_index)
        }
    }
}
...
pub fn trans_static_method_callee(bcx: &Block,
    ...
    match vtbls.get(bound_index).get(0) {
        &typeck::vtable_static(impl_did, ref rcvr_substs, rcvr_origins) => {
            ...
            let callee = trans_fn_ref_with_vtables(bcx, mth_id, ExprId(expr_id),
                                                   callee_substs.as_slice(),
                                                   Some(callee_origins));

            let callee_ty = node_id_type(bcx, expr_id);
            let llty = type_of_fn_from_ty(ccx, callee_ty).ptr_to();
            pod_value(bcx.tcx(), PointerCast(bcx, callee.val, llty), callee_ty)
        }
        _ => {
...
fn trans_monomorphized_callee(bcx: &Block,
    ...
    match vtbl {
      typeck::vtable_static(impl_did, ref rcvr_substs, rcvr_origins) => {
          ...
          Fn(trans_fn_ref_with_vtables(bcx,
                                       mth_id,
...
pub fn trans_trait_callee<'a>(bcx: &'a Block<'a>,
                              method_ty: ty::t,
                              n_method: uint,
                              self_datum: Datum<Expr>,
    ...
    let llpair = if ty::type_needs_drop(bcx.tcx(), self_datum.ty) {
        // Take ownership of self by converting to an rvalue.
        let self_datum = unpack_datum!(
            bcx, self_datum.to_rvalue_datum(bcx, "trait_callee"));

        // Convert to by-ref to extract the fields.
        let self_datum = unpack_datum!(
    ...
    let llcallee_ty = match ty::get(method_ty).sty {
        ty::ty_bare_fn(ref f) if f.abis.is_rust() => {
            type_of_rust_fn(ccx, true, f.sig.inputs.slice_from(1), f.sig.output)
        }
        _ => ccx.sess().bug("meth::trans_trait_callee given non-bare-rust-fn")
    };
    ...
    trait_method_def_ids.map(|method_def_id| {
        ...
        if m.generics.has_type_params() ||
           ty::type_has_self(ty::mk_bare_fn(tcx, m.fty.clone())) {
            ...
            trans_fn_ref_with_vtables(bcx, m_id, ExprId(0), substs, Some(vtables)).val
        }
    })
}
