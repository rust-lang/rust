...
//! features for generating the debug information. The general principle
//! is this:
//!
...
//! correct metadata into the LLVM IR when called with the right parameters. The
//! module is thus driven from an outside client with functions like
//! `debuginfo::create_local_var_metadata(bcx: block, local: &ast::local)`.
//!
//! Internally the module will try to reuse already created metadata by
//! utilizing a cache. The way to get a shared metadata node when needed is thus
//! to just call the corresponding function in this module:
//!
...
//! other type which in turn (transitively) refers to X. This introduces cycles
//! into the type referral graph. A naive algorithm doing an on-demand,
//! depth-first traversal of this graph when describing types, can get trapped
...
//! stub (as type members are not described and added to it yet) but it allows
//! the algorithm to already refer to the type. After the stub is inserted into
//! the cache, the algorithm continues as before. If it now encounters a
//! recursive reference, it will hit the cache and does not try to describe
//! the type anew.
//!
...
//! traversal at the type members after the type has been registered with the
//! cache. (This implementation approach might be a tad over-engineered and may
//! change in the future)
//!
//!
//! ## Source Locations and Line Information In addition to data type
//! descriptions the debugging information must also allow to map machine code
//! locations back to source code locations in order to be useful. This
//! functionality is also handled in this module. The following functions allow
...
//! `clear_source_location()`. In the later case, subsequent IR instruction will
//! not be linked to any source location. As you can see, this is a stateful API
//! (mimicking the one in LLVM), so be careful with source locations set by
//! previous calls. It's probably best to not rely on any specific state being
//! present at a given point in code.
//!
//! One topic that deserves some extra attention is *function prologues*. At the
//! beginning of a function's machine code there are typically a few
...
//! locations even while source location emission is still disabled, so there is
//! no need to do anything special with source location handling here.
//!
//! ## Unique Type Identification In order for link-time optimization to work
//! properly, LLVM needs a unique type identifier that tells it across
//! compilation units which types are the same as others. This type identifier
//! is created by TypeMap::get_unique_type_id_of_type() using the following
//! algorithm:
//!
...
//! in debuginfo, more than one `ty::t` instance may map to the same debuginfo
//! type metadata, that is, some struct `Struct<'a>` may have N instantiations
//! with different concrete substitutions for `'a`, and thus there will be N
//! `ty::t` instances for the type `Struct<'a>` even though it is not generic
//! otherwise. Unfortunately this means that we cannot use `ty::type_id()` as
//! cheap identifier for type metadata---we have done this in the past, but it
//! led to unnecessary metadata duplication in the best case and LLVM assertions
//! in the worst. However, the unique type ID as described above *can* be used
//! as identifier. Since it is comparatively expensive to construct, though,
//! `ty::type_id()` is still used additionally as an optimization for cases
//! where the exact same type has been seen before (which is most of the time).
