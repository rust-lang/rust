...
fn trans_rvalue_dps_unadjusted(bcx: block, expr: @ast::expr,
    ...
    match expr.node {
        ...
        ast::expr_fn(proto, decl, ref body, cap_clause) => {
            ...
            // astconv instead.
            return closure::trans_expr_fn(bcx, expr.span, proto,
            ...
            match ty::get(expr_ty).sty {
                ty::ty_fn(ref fn_ty) => {
                    ...
                    return closure::trans_expr_fn(
                        bcx, expr.span, fn_ty.meta.proto, decl, (*body),
                ...
                ty::ty_fn(ref fn_ty) => {
                    match blk.node {
                        ast::expr_fn_block(decl, ref body, cap) => {
                            return closure::trans_expr_fn(
                                bcx, expr.span, fn_ty.meta.proto, decl,
                                (*body), blk.id, cap, Some(None), dest);
                        }
                        _ => {
                            bcx.sess().impossible_case(
                                expr.span,
