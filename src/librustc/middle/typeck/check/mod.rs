...
pub fn check_no_duplicate_fields(tcx: ty::ctxt,
    ...
    for p in fields.iter() {
        ...
        match orig_sp {
            Some(orig_sp) => {
                span_err!(tcx.sess, sp, A0161,
                          "duplicate field name {} in record type declaration",
                                              tcx.sess.str_of(id));
        ...
        if m.abis.is_intrinsic() {
            ...
            for item in m.items.iter() {
                ...
                if tpt.generics.has_type_params() {
                    span_err!(ccx.tcx.sess, item.span, A0162,
                ...
                match item.node {
                    ast::ForeignItemFn(ref fn_decl, _) => {
                        if fn_decl.variadic && !m.abis.is_c() {
                            span_err!(ccx.tcx.sess, item.span, A0163,
...
fn compare_impl_method(tcx: ty::ctxt,
    ...
    if num_impl_m_type_params != num_trait_m_type_params {
        span_err!(tcx.sess,
            impl_m_span, A0168,
            "method `{}` has {} type parameter(s), but its trait \
    ...
    match infer::mk_subty(&infcx, false, infer::MethodCompatCheck(impl_m_span),
        ...
        result::Err(ref terr) => {
            span_err!(tcx.sess,
                impl_m_span, A0172,
                "method `{}` has an incompatible type: {}",
                        tcx.sess.str_of(trait_m.ident),
...
fn check_type_parameter_positions_in_path(function_context: @FnCtxt,
    ...
    for i in range(0, path.segments.len() - 2) {
        for lifetime in path.segments[i].lifetimes.iter() {
            span_err!(function_context.tcx().sess,
                      lifetime.span, A0173,
    ...
    match def {
        ...
        ast::DefStaticMethod(_, provenance, _) => {
            ...
            if supplied_ty_param_count < required_ty_param_count {
                ...
                span_err!(function_context.tcx().sess, path.span, A0176,
                    "the {} referenced by this path {} {} type \
                ...
                span_err!(function_context.tcx().sess, path.span, A0177,
                    "the {} referenced by this path {} {} type \
                            parameter{}, but {} type parameter{} were supplied",
                            name, needs,
...
pub fn check_expr_with_unifier(fcx: @FnCtxt,
    ...
    match expr.node {
      ...
      ast::ExprBox(place, subexpr) => {
                          ...
                          Some(item_def_id) if def_id == item_def_id => {
                              ...
                              let gc_struct_id =
                                  match tcx.lang_items
                                      ...
                                      Ok(id) => id,
                                      Err(msg) => {
                                          span_err!(tcx.sess, expr.span, A0211,
                                                    "{}", msg);
                                          ast::DefId {
          ...
          if !checked {
              span_err!(tcx.sess, expr.span, A0184,
            ...
            match ty::get(t_1).sty {
                ...
                _ => {
                    ...
                    if type_is_c_like_enum(fcx, expr.span, t_e) && t_1_is_trivial {
                        ...
                        span_err!(fcx.tcx().sess, expr.span, A0189,
...
pub fn check_simd(tcx: ty::ctxt, sp: Span, id: ast::NodeId) {
    ...
    if ty::type_needs_subst(t) {
        span_err!(tcx.sess, sp, A0192, "SIMD vector cannot be generic");
        return;
    }
    match ty::get(t).sty {
        ty::ty_struct(did, ref substs) => {
            ...
            if fields.is_empty() {
                span_err!(tcx.sess, sp, A0193, "SIMD vector cannot be empty");
                return;
            }
            let e = ty::lookup_field_type(tcx, did, fields[0].id, substs);
            if !fields.iter().all(
                         |f| ty::lookup_field_type(tcx, did, f.id, substs) == e) {
                span_err!(tcx.sess, sp, A0194, "SIMD vector should be homogeneous");
                return;
            }
            if !ty::type_is_machine(e) {
                span_err!(tcx.sess, sp, A0212, "SIMD vector element type should be \
                                       machine type");
                return;
            }
        }
...
pub fn check_enum_variants(ccx: @CrateCtxt,
    ...
    fn do_check(ccx: @CrateCtxt,
        ...
        for &v in vs.iter() {
            ...
            match v.node.disr_expr {
                Some(e) => {
                    ...
                    match const_eval::eval_const_expr_partial(&ccx.tcx, e) {
                        ...
                        Ok(_) => {
                            span_err!(ccx.tcx.sess, e.span, A0195,
                                      "expected signed integer constant");
                        }
                        Err(ref err) => {
                            span_err!(ccx.tcx.sess, e.span, A0196,
            ...
            if disr_vals.contains(&current_disr_val) {
                span_err!(ccx.tcx.sess, v.span, A0197,
            ...
            match hint {
                ...
                attr::ReprInt(sp, ity) => {
                    if !disr_in_range(ccx, ity, current_disr_val) {
                        span_err!(ccx.tcx.sess, v.span, A0198,
    ...
    if hint != attr::ReprAny && vs.len() <= 1 {
        span_err!(ccx.tcx.sess, sp, A0199,
                  "unsupported representation for {}variant enum",
...
pub fn instantiate_path(fcx: @FnCtxt,
    ...
    let (tps, regions) = if ty_substs_len == 0 {
        ...
        span_err!(fcx.ccx.tcx.sess, span, A0201,
        ...
        span_err!(fcx.ccx.tcx.sess, span, A0202,
        ...
        span_err!(fcx.ccx.tcx.sess, span, A0203,
...
pub fn check_bounds_are_used(ccx: @CrateCtxt,
    ...
    for (i, b) in tps_used.iter().enumerate() {
        if !*b {
            span_err!(ccx.tcx.sess, span, A0213,
...
pub fn check_intrinsic_type(ccx: @CrateCtxt, it: &ast::ForeignItem) {
    ...
    let (n_tps, inputs, output) = if name.starts_with("atomic_") {
        ...
        match split[1] {
            ...
            op => {
                span_err!(tcx.sess, it.span, A0204,
        ...
        match name {
            ...
            "get_tydesc" => {
              let tydesc_ty = match ty::get_tydesc_ty(ccx.tcx) {
                  Ok(t) => t,
                  Err(s) => {
                      span_fatal!(tcx.sess, it.span, A0034,
                                  "error type-checking get_tydesc intrinsic: {}", s);
                  }
              };
            ...
            "type_id" => {
                ...
                match langid {
                    ...
                    Err(msg) => {
                        span_fatal!(tcx.sess, it.span, A0214,
                                    "error type-checking type_id intrinsic: {}", msg);
                    }
                }
            },
            "visit_tydesc" => {
              let tydesc_ty = match ty::get_tydesc_ty(ccx.tcx) {
                  Ok(t) => t,
                  Err(s) => {
                        span_fatal!(tcx.sess, it.span, A0035,
                                    "error type-checking visit_tydesc intrinsic: {}", s);
                    }
              };
              ...
              let visitor_object_ty = match ty::visitor_object_ty(tcx, region) {
                  Ok((_, vot)) => vot,
                  Err(s) => {
                        span_fatal!(tcx.sess, it.span, A0036,
                                    "error type-checking visit_tydesc intrinsic: {}", s);
                  }
              };
            ...
            ref other => {
                span_err!(tcx.sess, it.span, A0205,
    ...
    if i_n_tps != n_tps {
        span_err!(tcx.sess, it.span, A0206,
