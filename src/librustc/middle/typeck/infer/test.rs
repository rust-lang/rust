...
use driver::driver::{optgroups, build_session_options, build_session, str_input,
                     build_configuration};
...
impl Env {
    ...
    fn create_simple_region_hierarchy(&self) {
        // creates a region hierarchy where 1 is root, 10 and 11 are children of 1, etc
    ...
    fn t_fn(&self, input_tys: &[ty::t], output_ty: ty::t) -> ty::t {
        let inputs = input_tys.map(|t| {mode: ast::expl(ast::by_copy), ty: *t});
    ...
    fn t_rptr_free(&self, nid: ast::node_id, id: uint) -> ty::t {
        ty::mk_imm_rptr(self.tcx, ty::re_free(nid, ty::br_anon(id)), self.t_int())
    }
    ...
    fn check_no_lub(&self, t1: ty::t, t2: ty::t) {
        match self.lub().tys(t1, t2) {
            Err(_) => {}
            Ok(t) => {
                fail fmt!("Unexpected success computing LUB: %?", self.ty_to_str(t))
            }
        }
    }
}
