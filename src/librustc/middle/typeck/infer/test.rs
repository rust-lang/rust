...
fn subst_ty_renumber_bound() {
    ...
    test_env(EMPTY_SOURCE_STR, errors([]), |env| {
        ...
        let t_expected = {
            ...
            env.t_fn([t_ptr_bound2], ty::mk_nil())
        };
...
fn subst_ty_renumber_some_bounds() {
    ...
    test_env(EMPTY_SOURCE_STR, errors([]), |env| {
        ...
        let t_source = {
            let t_param = env.t_param(subst::TypeSpace, 0);
            env.t_pair(t_param, env.t_fn([t_param], ty::mk_nil()))
        };
        ...
        let t_expected = {
            ...
            env.t_pair(t_rptr_bound1, env.t_fn([t_rptr_bound2], ty::mk_nil()))
        };
...
fn escaping() {
    ...
    test_env(EMPTY_SOURCE_STR, errors([]), |env| {
        ...
        assert!(!ty::type_has_escaping_regions(ty::mk_nil()));
        ...
        let t_fn = env.t_fn([t_param], ty::mk_nil());
        assert!(!ty::type_has_escaping_regions(t_fn));

        // t_fn = |&int|+'a
        let t_fn = env.t_closure([t_rptr_bound1], ty::mk_nil(), env.re_free(0, 1));
        assert!(!ty::type_has_escaping_regions(t_fn));

        // t_fn = |&int|+'a (where &int has depth 2)
        let t_fn = env.t_closure([t_rptr_bound2], ty::mk_nil(), env.re_free(0, 1));
        assert!(ty::type_has_escaping_regions(t_fn));
...
fn subst_region_renumber_region() {
    ...
    test_env(EMPTY_SOURCE_STR, errors([]), |env| {
        ...
        let t_source = {
            ...
            env.t_fn([env.t_rptr(re_early)], ty::mk_nil())
        };
        ...
        let t_expected = {
            let t_rptr_bound2 = env.t_rptr_late_bound_with_debruijn(1, ty::DebruijnIndex::new(2));
            env.t_fn([t_rptr_bound2], ty::mk_nil())
        };
