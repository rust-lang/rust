...
use middle::ty::{ReLateBound, ReSemeRegion, ReVar, ReSkolemized, BrFresh};
...
impl<'a> RegionVarBindings<'a> {
    ...
    fn add_constraint(&self,
        ...
        if self.constraints.borrow_mut().insert(constraint.clone(), origin) {
            if self.in_snapshot() {
    ...
    pub fn make_subregion(&self,
        ...
        match (sub, sup) {
          (&ReEarlyBound(..), &ReEarlyBound(..)) => {
            ...
            self.add_verify(VerifyRegSubReg(origin,
                                            (*sub).clone(),
                                            (*sup).clone()));
          }
          (&ReEarlyBound(..), _) |
          ...
          (r, &ReInfer(ReVar(sup_id))) => {
            self.add_constraint(ConstrainRegSubVar((*r).clone(), sup_id),
                                origin);
          }
          (&ReInfer(ReVar(sub_id)), r) => {
            self.add_constraint(ConstrainVarSubReg(sub_id, (*r).clone()),
                                origin);
          }
          _ => {
            self.add_verify(VerifyRegSubReg(origin,
    ...
    pub fn verify_param_bound(&self,
                              ...
                              sub: &Region,
                              sups: Vec<Region>) {
        self.add_verify(VerifyParamBound(param_ty,
                                         origin,
                                         (*sub).clone(),
                                         sups));
    }
    ...
    pub fn max_regions(&self,
                       ...
                       b: &Region)
                       -> Option<Region> {
        match self.glb_concrete_regions(loop_analysis, a, b) {
            Ok(r) => Some(r),
            Err(_) => None
        }
    }
    ...
    pub fn combine_vars(&self,
        ...
        self.combine_map(t).borrow_mut().insert(vars.clone(), c);
        if self.in_snapshot() {
    ...
    pub fn tainted(&self, mark: RegionMark, r0: &Region) -> Vec<Region> {
        ...
        let mut result_set = vec!((*r0).clone());
        let mut result_index = 0;
        while result_index < result_set.len() {
            // nb: can't use uint::range() here because result_set grows
            let r = (*result_set.get(result_index)).clone();
                ...
                self.undo_log.borrow().slice_from(mark.length).iter()
            {
                match undo_entry {
                    ...
                    &AddConstraint(ConstrainRegSubVar(ref a, b)) => {
                        consider_adding_bidirectional_edges(
                            &mut result_set,
                            &r,
                            a,
                    ...
                    &AddConstraint(ConstrainVarSubReg(a, ref b)) => {
                        consider_adding_bidirectional_edges(
                            &mut result_set,
                            &r,
                            &ReInfer(ReVar(a)),
                    ...
                    &AddVerify(i) => {
                        match self.verifys.borrow().get(i) {
                            ...
                            &VerifyParamBound(_, _, ref a, ref bs) => {
                                for b in bs.iter() {
                                    consider_adding_bidirectional_edges(
                                        &mut result_set,
                                        &r,
    ...
    errors are reported.
    */
    pub fn resolve_regions(&self, loop_analysis: Option<&LoopAnalysis>)
                           -> Vec<RegionResolutionError> {
        debug!("RegionVarBindings: resolve_regions()");
        let mut errors = vec!();
        let v = self.infer_variable_values(loop_analysis, &mut errors);
...
impl<'a> RegionVarBindings<'a> {
    fn is_subregion_of(&self, sub: &Region, sup: &Region) -> bool {
        self.tcx.region_maps.is_subregion_of(self.tcx, sub, sup)
    }

    fn lub_concrete_regions<'a>(
                            &self,
                            loop_analysis: Option<&LoopAnalysis>,
                            a: &'a Region,
                            b: &'a Region)
        ...
        match (a, b) {
          ...
          (&ReEmpty, r) | (r, &ReEmpty) => {
            (*r).clone() // everything lives longer than empty
          }
          ...
          (&ReFree(ref fr), &ReSemeRegion(ref s_seme_region)) |
          (&ReSemeRegion(ref s_seme_region), &ReFree(ref fr)) => {
            let s_id = s_seme_region.lub_scope(self.tcx);
            let f = ReFree(*fr);
            ...
            match self.tcx
                      .region_maps
                      .nearest_common_ancestor(fr.scope_id, s_id) {
          ...
          (&ReSemeRegion(ref a_seme_region),
           &ReSemeRegion(ref b_seme_region)) => {
            match loop_analysis {
                None => {
                    self.tcx
                        .sess
                        .bug("RegionVarBindings::lub_concrete_regions(): \
                              cannot relate SEME regions if no loop \
                              analysis present")
                }
                Some(loop_analysis) => {
                    match a_seme_region.lub(b_seme_region,
                                            self.tcx,
                                            loop_analysis) {
                        Some(seme_region) => ReSemeRegion(seme_region),
                        None => ReStatic,
                    }
                }
            }
    ...
    fn lub_free_regions(&self, a: &FreeRegion, b: &FreeRegion) -> ty::Region {
        /*!
    ...
    fn glb_concrete_regions<'a>(
                            &self,
                            loop_analysis: Option<&LoopAnalysis>,
                            a: &'a Region,
                            b: &'a Region)
                            -> cres<Region> {
        debug!("glb_concrete_regions({}, {})",
               a.repr(self.tcx),
               b.repr(self.tcx));
        match (a, b) {
            (&ReLateBound(..), _) |
            ...
            (&ReFree(ref fr), &ReSemeRegion(ref seme_region)) |
            (&ReSemeRegion(ref seme_region), &ReFree(ref fr)) => {
                let s = ReSemeRegion((*seme_region).clone());
                let s_id = seme_region.lub_scope(self.tcx);
                // Free region is something "at least as big as
                // `fr.scope_id`."  If we find that the scope
                // `fr.scope_id` is bigger than the scope `s_id`, then
                // we can say that the GLB is the scope `s_id`.
                // Otherwise, as we do not know big the free region is
                // precisely, the GLB is undefined.
                match self.tcx
                          .region_maps
                          .nearest_common_ancestor(fr.scope_id,
                                                   s_id) {
                    Some(r_id) if r_id == fr.scope_id => Ok(s),
                    _ => {
                        Err(ty::terr_regions_no_overlap((*b).clone(),
                                                        (*a).clone()))
                    }
                }
            }
             ...
             &ReSemeRegion(ref b_seme_region)) => {
                match loop_analysis {
                    None => {
                        self.tcx
                            .sess
                            .bug("RegionVarBindings::glb_concrete_regions(): \
                                  cannot relate SEME regions if no loop \
                                  analysis present")
                    }
                    Some(loop_analysis) => {
                        match a_seme_region.glb(b_seme_region,
                                                self.tcx,
                                                loop_analysis) {
                            Some(seme_region) => {
                                Ok(ReSemeRegion(seme_region))
                            }
                            None => {
                                Err(ty::terr_regions_no_overlap((*a).clone(),
                                                                (*b).clone()))
                            }
                        }
                    }
            ...
            (&ReFree(ref a_fr), &ReFree(ref b_fr)) => {
                self.glb_free_regions(loop_analysis, a_fr, b_fr)
            }
    ...
    fn glb_free_regions(&self,
        ...
        return match a.cmp(b) {
            Less => helper(self, loop_analysis, a, b),
            Greater => helper(self, loop_analysis, b, a),
        ...
        fn helper(this: &RegionVarBindings,
            ...
            if this.tcx.region_maps.sub_free_region(*a, *b) {
                ...
                match loop_analysis {
                    None => Ok(ReSemeRegion(SemeRegion::dummy())),
                    Some(loop_analysis) => {
                        this.intersect_scopes(loop_analysis,
                                              &ty::ReFree(*a),
                                              &ty::ReFree(*b),
                                              a.scope_id,
                                              b.scope_id)
    ...
    fn intersect_scopes(&self,
        ...
        match self.tcx.region_maps.nearest_common_ancestor(scope_a, scope_b) {
            Some(r_id) if scope_a == r_id => {
                Ok(ReSemeRegion(SemeRegion::from_scope(self.tcx,
                                                       loop_analysis,
                                                       scope_b)))
            }
            Some(r_id) if scope_b == r_id => {
                Ok(ReSemeRegion(SemeRegion::from_scope(self.tcx,
                                                        loop_analysis,
                                                        scope_a)))
            }
            _ => {
                Err(ty::terr_regions_no_overlap((*region_a).clone(),
                                                (*region_b).clone()))
            }
        }
    }
...
impl<'a> RegionVarBindings<'a> {
    fn infer_variable_values(&self,
        ...
        self.expansion(loop_analysis, var_data.as_mut_slice());
        self.contraction(loop_analysis, var_data.as_mut_slice());
        let values =
            self.extract_values_and_collect_conflicts(loop_analysis,
                                                      var_data.as_slice(),
                                                      errors);
        ...
        self.iterate_until_fixed_point("Expansion", |constraint| {
            debug!("expansion: constraint={} origin={}",
                   ...
                   self.constraints.borrow()
                                   ...
                                   .repr(self.tcx));
            match *constraint {
              ConstrainRegSubVar(ref a_region, b_vid) => {
                let b_data = &mut var_data[b_vid.index];
                self.expand_node(loop_analysis, a_region, b_vid, b_data)
              }
              ConstrainVarSubVar(a_vid, b_vid) => {
                match var_data[a_vid.index].value.clone() {
                  NoValue | ErrorValue => false,
                  Value(a_region) => {
                    let b_node = &mut var_data[b_vid.index];
                    self.expand_node(loop_analysis, &a_region, b_vid, b_node)
                  }
                }
              }
              ConstrainVarSubReg(..) => {
    ...
    fn expand_node(&self,
        ...
        match b_data.value {
          NoValue => {
            ...
            b_data.value = Value((*a_region).clone());
            return true;
          }

          Value(ref cur_region) => {
            lub = self.lub_concrete_regions(loop_analysis,
                                            a_region,
                                            cur_region);
            if lub == *cur_region {
                return false;
    ...
    fn contraction(&self,
        ...
        self.iterate_until_fixed_point("Contraction", |constraint| {
            ...
            match *constraint {
              ...
              ConstrainVarSubVar(a_vid, b_vid) => {
                match var_data[b_vid.index].value.clone() {
                  NoValue | ErrorValue => false,
                  Value(b_region) => {
                    let a_data = &mut var_data[a_vid.index];
                    self.contract_node(loop_analysis,
                                       a_vid,
                                       a_data,
                                       &b_region)
                  }
              ...
              ConstrainVarSubReg(a_vid, ref b_region) => {
                let a_data = &mut var_data[a_vid.index];
                self.contract_node(loop_analysis, a_vid, a_data, b_region)
              }
            }
        })
    }
    ...
    fn contract_node(&self,
        ...
        debug!("contract_node({} == {}/{}, {})",
               a_vid,
               a_data.value.repr(self.tcx),
               a_data.classification,
               b_region.repr(self.tcx));

        return match a_data.value.clone() {
            NoValue => {
                assert_eq!(a_data.classification, Contracting);
                a_data.value = Value((*b_region).clone());
                true // changed
            }
        ...
        fn adjust_node(this: &RegionVarBindings,
                       ...
                       b_region: &Region)
                    -> bool {
            match this.glb_concrete_regions(loop_analysis,
                                            a_region,
                                            b_region) {
                Ok(glb) => {
                    if glb == *a_region {
    ...
    fn collect_concrete_region_errors(
        ...
        for verify in self.verifys.borrow().iter() {
            match *verify {
                VerifyRegSubReg(ref origin, ref sub, ref sup) => {
                    ...
                    debug!("ConcreteFailure: !(sub <= sup): sub={}, sup={}",
                           sub.repr(self.tcx),
                           sup.repr(self.tcx));
                    errors.push(ConcreteFailure((*origin).clone(),
                                                (*sub).clone(),
                                                (*sup).clone()));
                }

                VerifyParamBound(ref param_ty,
                                 ref origin,
                                 ref sub,
                                 ref sups) => {
    ...
    fn extract_values_and_collect_conflicts(
        ...
        Vec::from_fn(self.num_vars(), |idx| var_data[idx].value.clone())
    }
            ...
            for upper_bound in upper_bounds.iter() {
                if !self.is_subregion_of(&lower_bound.region,
                    ...
                    errors.push(SubSupConflict(
                        ...
                        lower_bound.region.clone(),
                        upper_bound.origin.clone(),
                        upper_bound.region.clone()));
                    return;
                }
            }
        }
    ...
    fn collect_error_for_contracting_node(
        ...
        for upper_bound_1 in upper_bounds.iter() {
            for upper_bound_2 in upper_bounds.iter() {
                match self.glb_concrete_regions(loop_analysis,
                  ...
                  Err(_) => {
                    errors.push(SupSupConflict(
                        ...
                        upper_bound_1.region.clone(),
                        upper_bound_2.origin.clone(),
                        upper_bound_2.region.clone()));
                    return;
                  }
                }
            }
        ...
        self.tcx.sess.span_bug(
            self.var_origins.borrow().get(node_idx.index).span(),
            format!("collect_error_for_contracting_node() could not find \
                     error for var {}, upper_bounds={}",
                    node_idx,
                    upper_bounds.iter()
                                .map(|x| x.region.clone())
                                .collect::<Vec<ty::Region>>()
        ...
        fn process_edges(this: &RegionVarBindings,
            ...
            graph.each_adjacent_edge(source_node_index, dir, |_, edge| {
                match edge.data {
                    ...
                    ConstrainRegSubVar(ref region, _) |
                    ConstrainVarSubReg(_, ref region) => {
                        state.result.push(RegionAndOrigin {
                            region: (*region).clone(),
                            origin: this.constraints
                                        .borrow()
                                        .get_copy(&edge.data),
                        });
                    }
    ...
    fn iterate_until_fixed_point(&self,
        ...
        while changed {
            ...
            if iteration > 1000 {
                self.tcx.sess.bug("infinite loop in regionck iteration")
            }
...
impl Repr for Constraint {
    fn repr(&self, tcx: &ty::ctxt) -> String {
        match *self {
            ...
            ConstrainRegSubVar(ref a, b) => {
                format!("ConstrainRegSubVar({}, {})", a.repr(tcx), b.repr(tcx))
            }
            ConstrainVarSubReg(a, ref b) => {
...
impl Repr for VarValue {
    fn repr(&self, tcx: &ty::ctxt) -> String {
        match *self {
            ...
            Value(ref r) => format!("Value({})", r.repr(tcx)),
