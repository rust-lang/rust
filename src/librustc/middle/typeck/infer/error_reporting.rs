    ...
    fn report_type_error(&self, trace: TypeTrace, terr: &ty::type_err) {
        ...
        self.tcx.sess.span_err(
            ...
            format!("{}: {} ({})",
                 message_root_str,
                 expected_found_str,
                 ty::type_err_to_str(self.tcx, terr)).as_str());
            ...
            ty::ReFree(ty::FreeRegion {bound_region: ty::BrNamed(..), ..}) => {
                ...
                self.tcx.sess.span_err(
                    ...
                    format!(
                        ...
                        param_ty.user_string(self.tcx),
                        sub.user_string(self.tcx)).as_str());
            }
            ...
            ty::ReStatic => {
                ...
                self.tcx.sess.span_err(
                    ...
                    format!(
                        ...
                        param_ty.user_string(self.tcx),
                        param_ty.user_string(self.tcx)).as_str());
            }
    ...
    fn report_concrete_failure(&self,
        ...
        match origin {
            ...
            infer::Managed(span) => {
                self.tcx.sess.span_err(
                    span,
                    format!("cannot put borrowed references into managed memory").as_str());
            }
        }
    }
...
impl<'a, 'tcx> Rebuilder<'a, 'tcx> {
    ...
    fn pick_lifetime(&self,
        ...
        if region_names.len() > 0 {
            ...
            names.sort();
            let name = token::str_to_ident(names.get(0).as_str()).name;
                    ...
                    let a_def = match self.tcx.def_map.borrow().find(&id) {
                        None => {
                            self.tcx
                                ...
                                .fatal(format!(
                                        "unbound path {}",
                                        pprust::path_to_string(path)).as_str())
                        }
                        Some(&d) => d
                    };
                    match a_def {
...
impl LifeGiver {
    ...
    fn give_lifetime(&self) -> ast::Lifetime {
        ...
        loop {
            ...
            s.push_str(num_to_string(self.counter.get()).as_str());
            if !self.taken.contains(&s) {
                lifetime = name_to_dummy_lifetime(
                                    token::str_to_ident(s.as_str()).name);
