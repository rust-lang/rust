...
impl<'tcx> ty::ctxt<'tcx> {
    pub fn note_and_explain_region(&self,
        ...
        fn explain_span(tcx: &ty::ctxt, heading: &str, span: Span) -> (String, Option<Span>) {
        ...
        let (description, span) = match region {
            ...
            ty::ReFree(ref fr) => {
                let prefix = match fr.bound_region {
                    ty::BrAnon(idx) => format!("the anonymous lifetime #{} defined on", idx + 1),
...
pub trait ErrorReporting<'tcx> {
    ...
    fn report_and_explain_type_error(&self, trace: TypeTrace<'tcx>, terr: &TypeError<'tcx>);
...
impl<'a, 'tcx> ErrorReporting<'tcx> for InferCtxt<'a, 'tcx> {
    fn report_region_errors(&self, errors: &Vec<RegionResolutionError<'tcx>>) {
        ...
        let errors = if p_errors.is_empty() {
            errors
        } else {
            &p_errors
        };
        for error in errors {
            match error.clone() {
                ...
                SubSupConflict(var_origin, sub_origin, sub_r, sup_origin, sup_r) => {
                    self.report_sub_sup_conflict(var_origin, sub_origin, sub_r, sup_origin, sup_r);
                }

                ProcessedErrors(ref var_origins, ref trace_origins, ref same_regions) => {
        ...
        for error in errors {
            match error.clone() {
                ...
                SubSupConflict(var_origin, _, sub_r, _, sup_r) => {
                    debug!("processing SubSupConflict sub: {:?} sup: {:?}",
                           sub_r,
                           sup_r);
    ...
    fn report_type_error(&self,
        ...
        match trace.origin {
            TypeOrigin::MatchExpressionArm(_, arm_span, source) => {
                match source {
    ...
    fn check_and_note_conflicting_crates(&self,
        ...
        let report_path_match = |err: &mut DiagnosticBuilder, did1: DefId, did2: DefId| {
            ...
            if !(did1.is_local() || did2.is_local()) && did1.krate != did2.krate {
                let exp_path = self.tcx.with_path(did1, |p| {
                    p.map(|x| x.to_string())
                     .collect::<Vec<_>>()
                });
                let found_path = self.tcx.with_path(did2, |p| {
                    p.map(|x| x.to_string())
                     .collect::<Vec<_>>()
                });
                ...
                if exp_path == found_path {
                    ...
                    err.span_note(sp,
                                  &format!("Perhaps two different versions of crate `{}` are \
                                            being used?",
                                           crate_name));
                }
            }
        ...
        match *terr {
            TypeError::Sorts(ref exp_found) => {
                ...
                match (&exp_found.expected.sty, &exp_found.found.sty) {
                    ...
                    (&ty::TyStruct(ref exp_adt, _),
                     &ty::TyStruct(ref found_adt, _)) |
    ...
    fn expected_found_str<T: fmt::Display + Resolvable<'tcx> + TypeFoldable<'tcx>>
        ...
        println!("expected before resolve = {:?}, after resolve = {:?}",
                 exp_found.expected,
                 expected);
        ...
        println!("found before resolve = {:?}, after resolve = {:?}",
                 exp_found.found,
                 found);
        ...
        match origin {
            ...
            infer::Reborrow(span) => {
                let mut err = struct_span_err!(self.tcx.sess,
                                               span,
                                               E0312,
                                               "lifetime of reference outlines lifetime of \
                                                borrowed content...");
                self.tcx.note_and_explain_region(&mut err,
    ...
    fn give_suggestion(&self, err: &mut DiagnosticBuilder, same_regions: &[SameRegions]) {
        ...
        let (fn_decl,
             ...
             name,
             expl_self,
             span) = node_inner.expect("expect item fn");
        let rebuilder = Rebuilder::new(self.tcx,
                                       fn_decl,
                                       expl_self,
                                       generics,
                                       same_regions,
        ...
        self.give_expl_lifetime_param(err,
                                      &fn_decl,
                                      unsafety,
                                      constness,
                                      name,
...
impl<'a, 'tcx> Rebuilder<'a, 'tcx> {
    ...
    fn rebuild(&self) -> (hir::FnDecl, Option<hir::ExplicitSelf_>, hir::Generics) {
        ...
        for sr in self.same_regions {
            ...
            let (anon_nums, region_names) = self.extract_anon_nums_and_names(sr);
            ...
            expl_self_opt = self.rebuild_expl_self(expl_self_opt,
                                                   lifetime,
                                                   &anon_nums,
                                                   &region_names);
            inputs = self.rebuild_args_ty(&inputs[..], lifetime, &anon_nums, &region_names);
            output = self.rebuild_output(&output, lifetime, &anon_nums, &region_names);
            ty_params = self.rebuild_ty_params(ty_params, lifetime, &region_names);
        }
    ...
    fn pick_lifetime(&self, region_names: &HashSet<ast::Name>) -> (hir::Lifetime, FreshOrKept) {
    ...
    fn rebuild_generics(&self,
        ...
        for lt in &generics.lifetimes {
            if keep.contains(&lt.lifetime.name) || !remove.contains(&lt.lifetime.name) {
    ...
    fn rebuild_args_ty(&self,
        ...
        for arg in inputs {
            let new_ty = self.rebuild_arg_ty_or_output(&*arg.ty, lifetime, anon_nums, region_names);
    ...
    fn rebuild_output(&self,
                      ty: &hir::FunctionRetTy,
                      lifetime: hir::Lifetime,
                      anon_nums: &HashSet<u32>,
                      region_names: &HashSet<ast::Name>)
                      -> hir::FunctionRetTy {
        match *ty {
            hir::Return(ref ret_ty) => {
                hir::Return(self.rebuild_arg_ty_or_output(&**ret_ty,
                                                          lifetime,
                                                          anon_nums,
                                                          region_names))
            }
                ...
                hir::TyPath(ref maybe_qself, ref path) => {
                    let a_def = match self.tcx.def_map.borrow().get(&cur_ty.id) {
                        None => {
                            self.tcx
                                .sess
                                .fatal(&format!("unbound path {}", pprust::path_to_string(path)))
                        }
                        Some(d) => d.full_def(),
                    };
                    match a_def {
                        def::DefTy(did, _) | def::DefStruct(did) => {
                            ...
                            let expected = generics.regions.len(subst::TypeSpace) as u32;
                            let lifetimes = path.segments.last().unwrap().parameters.lifetimes();
                            let mut insert = Vec::new();
                            ...
                            let qself = maybe_qself.as_ref().map(|qself| {
                                hir::QSelf {
                                    ty: self.rebuild_arg_ty_or_output(&qself.ty,
                                                                      lifetime,
                                                                      anon_nums,
                                                                      region_names),
    ...
    fn rebuild_ty(&self, from: P<hir::Ty>, to: P<hir::Ty>) -> P<hir::Ty> {

        fn build_to(from: P<hir::Ty>, to: &mut Option<P<hir::Ty>>) -> P<hir::Ty> {
            ...
            from.map(|hir::Ty { id, node, span }| {
                let new_node = match node {
                    ...
                    hir::TyFixedLengthVec(ty, e) => hir::TyFixedLengthVec(build_to(ty, to), e),
                    hir::TyTup(tys) => {
    ...
    fn rebuild_path(&self, rebuild_info: RebuildPathInfo, lifetime: hir::Lifetime) -> hir::Path {
        let RebuildPathInfo {
            path,
            indexes,
            expected,
        ...
        let new_parameters = match last_seg.parameters {
            hir::ParenthesizedParameters(..) => last_seg.parameters.clone(),

            hir::AngleBracketedParameters(ref data) => {
                let mut new_lts = Vec::new();
                if data.lifetimes.is_empty() {
                    // traverse once to see if there's a need to insert lifetime
                    let need_insert = (0..expected).any(|i| indexes.contains(&i));
                    if need_insert {
                        for i in 0..expected {
                ...
                let new_types = data.types
                                    .iter()
                                    .map(|t| {
                                        self.rebuild_arg_ty_or_output(&**t,
                                                                      lifetime,
                                                                      anon_nums,
                                                                      region_names)
                                    })
                                    .collect();
                let new_bindings = data.bindings
                                       .iter()
                                       .map(|b| {
                                           hir::TypeBinding {
                                               id: b.id,
...
impl<'a, 'tcx> ErrorReportingHelpers<'tcx> for InferCtxt<'a, 'tcx> {
    fn give_expl_lifetime_param(&self,
                                ...
                                generics: &hir::Generics,
                                span: Span) {
        let suggested_fn = pprust::fun_to_string(decl,
                                                 unsafety,
                                                 constness,
                                                 name,
                                                 opt_explicit_self,
                                                 generics);
        let msg = format!("consider using an explicit lifetime parameter as shown: {}",
                          suggested_fn);
        ...
        match *origin {
            infer::Subtype(ref trace) => {
                ...
                match self.values_str(&trace.values) {
                    Some(values_str) => {
                        err.span_note(trace.origin.span(),
                                      &format!("...so that {} ({})", desc, values_str));
                    }
                    None => {
                        ...
                        err.span_note(trace.origin.span(), &format!("...so that {}", desc));
                    }
                }
            }
            infer::Reborrow(span) => {
...
impl<'tcx> Resolvable<'tcx> for ty::TraitRef<'tcx> {
    fn resolve<'a>(&self, infcx: &InferCtxt<'a, 'tcx>) -> ty::TraitRef<'tcx> {
        infcx.resolve_type_vars_if_possible(self)
    }
}

impl<'tcx> Resolvable<'tcx> for ty::PolyTraitRef<'tcx> {
    fn resolve<'a>(&self, infcx: &InferCtxt<'a, 'tcx>) -> ty::PolyTraitRef<'tcx> {
        infcx.resolve_type_vars_if_possible(self)
    }
}

fn lifetimes_in_scope(tcx: &ty::ctxt, scope_id: ast::NodeId) -> Vec<hir::LifetimeDef> {
