...
fn check_expr(cx: &mut MatchCheckCtxt, ex: &ast::Expr) {
    ...
    match ex.node {
        ast::ExprMatch(ref scrut, ref arms) => {
...
fn check_arms(cx: &MatchCheckCtxt, arms: &[(Vec<P<Pat>>, Option<&ast::Expr>)]) {
    ...
    let pat = match ty::get(left_ty).sty {
        ...
        ty::ty_enum(cid, _) | ty::ty_struct(cid, _)  => {
            ...
            if is_structure {
                ...
                let field_pats: Vec<ast::FieldPat> = fields.into_iter()
                    .zip(pats)
                    .filter(|&(_, ref pat)| pat.node != ast::PatWild(ast::PatWildSingle))
                    .map(|(field, pat)| ast::FieldPat {
