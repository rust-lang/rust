...
impl<N,E> Graph<N,E> {
    ...
    pub fn each_node(&self, f: &fn(NodeIndex, &Node<N>) -> bool) {
        //! Iterates over all edges defined in the graph.

        for uint::range(0, self.nodes.len()) |i| {
            if !f(NodeIndex(i), &self.nodes[i]) {
                return;
            }
        }
    }

    pub fn each_edge(&self, f: &fn(EdgeIndex, &Edge<E>) -> bool) {
        //! Iterates over all edges defined in the graph.

        for uint::range(0, self.edges.len()) |i| {
            if !f(EdgeIndex(i), &self.edges[i]) {
                return;
            }
        }
    }
    ...
    // computation. It holds the edges constant and iterates over
    // them, while provides mutable access to the array of nodes.
    // The callback `op` should return true if changes were made.

    pub fn iterate_until_fixed_point(&mut self,
        ...
        while changed {
            ...
            for self.edges.each |edge| {
                changed |= op(iteration, self.nodes, edge);
            }
        }
    }
}
