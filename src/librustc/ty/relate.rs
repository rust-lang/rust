    ...
    'tcx: 'a,
{
    ...
    match (a_sty, b_sty) {
        ...
        (&ty::Param(ref a_p), &ty::Param(ref b_p)) if a_p.idx == b_p.idx => Ok(a),
            ...
            TraitObjectMode::SquashAutoTraitsIssue33140 => {
                ...
                let normalize = |d: &[ty::ExistentialPredicate<'tcx>]| {
                    let mut result: Vec<_> = d
                        .iter()
                        .map(|pi| match pi {
                            Trait(ref a) if tcx.trait_is_auto(a.def_id) => AutoTrait(a.def_id),
                            other => *other,
                        })
                        .collect();
...
impl<'tcx> Relate<'tcx> for Kind<'tcx> {
        ...
        'tcx: 'a,
    {
        match (a.unpack(), b.unpack()) {
            ...
            (UnpackedKind::Lifetime(unpacked), x) => bug!(
                "impossible case reached: can't relate: {:?} with {:?}",
                unpacked,
                x
            ),
            (UnpackedKind::Type(unpacked), x) => bug!(
                "impossible case reached: can't relate: {:?} with {:?}",
                unpacked,
                x
            ),
