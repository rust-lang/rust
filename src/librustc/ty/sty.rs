...
pub enum TyKind<'tcx> {
    ...
    Dynamic(
        Binder<&'tcx List<ExistentialPredicate<'tcx>>>,
        ty::Region<'tcx>,
    ),
...
#[derive(
    Debug, Copy, Clone, PartialEq, PartialOrd, Ord, Eq, Hash, RustcEncodable, RustcDecodable,
)]
...
#[derive(
    Copy, Clone, PartialEq, Eq, Hash, RustcEncodable, RustcDecodable, Debug, PartialOrd, Ord,
)]
...
impl<'a, 'gcx, 'tcx> TyS<'tcx> {
    ...
    pub fn conservative_is_privately_uninhabited(&self, tcx: TyCtxt<'a, 'gcx, 'tcx>) -> bool {
        ...
        match self.sty {
            ...
            ty::Tuple(tys) => tys
                .iter()
                .any(|ty| ty.conservative_is_privately_uninhabited(tcx)),
            ty::Array(ty, len) => {
    ...
    pub fn builtin_deref(&self, explicit: bool) -> Option<TypeAndMut<'tcx>> {
        match self.sty {
            Adt(def, _) if def.is_box() => Some(TypeAndMut {
                ty: self.boxed_ty(),
    ...
    pub fn push_regions(&self, out: &mut SmallVec<[ty::Region<'tcx>; 4]>) {
        match self.sty {
            ...
            | Generator(_, GeneratorSubsts { ref substs }, _) => out.extend(substs.regions()),
...
#[derive(
    Copy, Clone, Debug, Hash, RustcEncodable, RustcDecodable, Eq, PartialEq, Ord, PartialOrd,
)]
...
impl<'tcx> Const<'tcx> {
    ...
    pub fn unwrap_bits(&self, tcx: TyCtxt<'_, '_, '_>, ty: ParamEnvAnd<'tcx, Ty<'tcx>>) -> u128 {
        self.assert_bits(tcx, ty)
            .unwrap_or_else(|| bug!("expected bits of {}, got {:#?}", ty.value, self))
    }

    #[inline]
    pub fn unwrap_usize(&self, tcx: TyCtxt<'_, '_, '_>) -> u64 {
        self.assert_usize(tcx)
            .unwrap_or_else(|| bug!("expected constant usize, got {:#?}", self))
    }
}
