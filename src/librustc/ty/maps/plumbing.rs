...
pub(super) trait GetCacheInternal<'tcx>: QueryDescription<'tcx> + Sized {
    type I : CacheEntryIterable<Self::Key, Self::Value>;
    fn get_cache_internal<'a>(tcx: TyCtxt<'a, 'tcx, 'tcx>) -> Ref<'a, Self::I>;
}

pub(super) trait CacheEntryIterable<K, V> {
    fn cache_entry_iter<'b>(&'b self) -> Box<'b + Iterator<Item=(&'b K, &'b QueryValue<V>)>>;
}

impl<'tcx, Q: QueryDescription<'tcx>> CacheEntryIterable<Q::Key, Q::Value>
                                      for QueryMap<'tcx, Q> {
    fn cache_entry_iter<'b>(&'b self)
                            -> Box<'b + Iterator<Item=(&'b Q::Key, &'b QueryValue<Q::Value>)>> {
        ::std::boxed::Box::new(self.map.iter())
    }
}

impl<V> CacheEntryIterable<CrateNum, V> for Option<QueryValue<V>> {
    fn cache_entry_iter<'b>(&'b self)
                            -> Box<'b + Iterator<Item=(&'b CrateNum, &'b QueryValue<V>)>> {
        ::std::boxed::Box::new(self.iter().map(|v| (&LOCAL_CRATE, v)))
    }
}
...
macro_rules! macro_if {
    ([$if:ty] [$then:tt] [$else:tt]) => ($then);
    ([] [$then:tt] [$else:tt]) => ($else);
    ([$if:ty] [$then:pat] [$else:pat]) => ($then);
    ([] [$then:pat] [$else:pat]) => ($else);
}

macro_rules! default_key_value {
    ($val:ident, $K:ty) => ($val);
    ($val:ident,) => (LOCAL_CRATE);
}

macro_rules! default_key_type {
    ($K:ty) => ($K);
    () => (CrateNum);
}
       ...
       [$($modifiers:tt)*] fn $name:ident: $node:ident($($K:ty)*) -> $V:ty,)*) => {
        ...
        impl<$tcx> Maps<$tcx> {
            pub fn new(providers: IndexVec<CrateNum, Providers<$tcx>>)
                ...
                Maps {
                    ...
                    $($name: RefCell::new(macro_if!([$($K)*] [(QueryMap::new())] [None]))),*
                }
            }
        }
        ...
        impl<$tcx> Query<$tcx> {
            pub fn describe(&self, tcx: TyCtxt) -> String {
                let (r, name) = match *self {
                    $(macro_if!([$($K)*] [Query::$name(key)] [Query::$name()]) => {
                        (queries::$name::describe(tcx, default_key_value!(key, $($K)*)),
                            stringify!($name))
                    })*
                };
                if tcx.sess.verbose() {
        ...
        impl<'a, $tcx, 'lcx> queries::$name<$tcx> {
                            ...
                            -> Result<$V, CycleError<'a, $tcx>>
            {
                debug!("ty::queries::{}::try_get_with(key={:?}, span={:?})",
                       stringify!($name),
                       default_key_value!(key, $($K)*),
                       span);

                profq_msg!(tcx,
                    ProfileQueriesMsg::QueryBegin(
                        span.data(),
                        QueryMsg::$name(profq_key!(tcx, default_key_value!(key, $($K)*)))
                    )
                );

                if let Some(value) = macro_if!([$($K)*]
                        [{tcx.maps.$name.borrow().map.get(&key)}]
                        [{tcx.maps.$name.borrow().as_ref()}]) {
                ...
                if span == DUMMY_SP && stringify!($name) != "def_span" {
                    span = default_key_value!(key, $($K)*).default_span(tcx)
                }
                ...
                if !tcx.dep_graph.is_fully_enabled() {
                    ...
                    return Self::force(tcx, default_key_value!(key, $($K)*), span, null_dep_node)
                                .map(|(v, _)| v);
                }

                let dep_node = Self::to_dep_node(tcx$(, key as $K)*);

                if dep_node.kind.is_anon() {
                    profq_msg!(tcx, ProfileQueriesMsg::ProviderBegin);

                    let res = tcx.cycle_check(span, Query::$name($(key as $K)*), || {
                        tcx.sess.diagnostic().track_diagnostics(|| {
                            tcx.dep_graph.with_anon_task(dep_node.kind, || {
                                Self::compute_result(tcx.global_tcx(),
                                    default_key_value!(key, $($K)*))
                            })
                        })
                    })?;
                ...
                match Self::force(tcx, default_key_value!(key, $($K)*), span, dep_node) {
            ...
            pub fn ensure(tcx: TyCtxt<'a, $tcx, 'lcx> $(, key: $K)*) -> () {
                ...
                if tcx.try_mark_green_and_read(&dep_node).is_none() {
                    ...
                    let _ = tcx.$name($(key as $K)*);
                }
            }

            fn compute_result(tcx: TyCtxt<'a, $tcx, 'lcx>, key: default_key_type!($($K)*)) -> $V {
                let provider = tcx.maps.providers[key.map_crate()].$name;
                provider(tcx.global_tcx()$(, key as $K)*)
            }
                    ...
                    let (result, _ ) = tcx.cycle_check(span, Query::$name($(key as $K)*), || {
                        ...
                        tcx.sess.diagnostic().track_diagnostics(|| {
                            ...
                            tcx.dep_graph.with_ignore(|| {
                                Self::compute_result(tcx, default_key_value!(key, $($K)*))
                            })
                        })
                    })?;
                    result
            ...
            pub fn try_get(tcx: TyCtxt<'a, $tcx, 'lcx>, span: Span $(, key: $K)*)
                ...
                match Self::try_get_with(tcx, span $(, key as $K)*) {
                    Ok(e) => Ok(e),
        ...
        impl<'a, $tcx, 'lcx> TyCtxt<'a, $tcx, 'lcx> {
            ...
            pub fn $name(self $(, key: $K)*) -> $V {
                self.at(DUMMY_SP).$name($(key as $K)*)
            })*
        }
        ...
        impl<'a, $tcx, 'lcx> TyCtxtAt<'a, $tcx, 'lcx> {
            $($(#[$attr])*
            pub fn $name(self $(, key: $K)*) -> $V {
                queries::$name::try_get(self.tcx, self.span $(, key as $K)*)
...
macro_rules! define_map_struct {
    (tcx: $tcx:tt,
     input: ($(([$(modifiers:tt)*] [$($attr:tt)*] [$name:ident] [$($K:ty)*]))*)) => {
        pub struct Maps<$tcx> {
            providers: IndexVec<CrateNum, Providers<$tcx>>,
            query_stack: RefCell<Vec<(Span, Query<$tcx>)>>,
            $($(#[$attr])*  $name: macro_if!([$($K)*]
                [(RefCell<QueryMap<$tcx, queries::$name<$tcx>>>)]
                [(RefCell<Option<QueryValue<<queries::$name<$tcx> as QueryConfig>::Value>>>)]),)*
            ...
            fn default() -> Self {
                $(fn $name<'a, $tcx>(_: TyCtxt<'a, $tcx, $tcx>$(, key: $K)*) -> $R {
                    bug!("tcx.maps.{}({:?}) unsupported by its crate",
                         stringify!($name), default_key_value!(key, $($K)*));
                })*
                Providers { $($name),* }
            }
        }
