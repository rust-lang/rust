...
use crate::session::config::{BorrowckMode, InputsAndOutputs};
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn def_path(self, id: DefId) -> hir_map::DefPath {
        if id.is_local() {
            ...
            self.lowered_hir().defs.def_path(id.index)
        } else {
    ...
    pub fn def_path_hash(self, def_id: DefId) -> hir_map::DefPathHash {
        if def_id.is_local() {
            ...
            self.lowered_hir().defs.def_path_hash(def_id.index)
        } else {
...
pub fn provide(providers: &mut ty::query::Providers<'_>) {
    providers.in_scope_traits_map = |tcx, id| {
        tcx.lowered_hir().trait_map.get(&id)
    };
    providers.module_exports = |tcx, id| {
        tcx.lowered_hir().export_map.get(&id).map(|v| &v[..])
    };
    ...
    providers.maybe_unused_trait_import = |tcx, id| {
        tcx.lowered_hir().maybe_unused_trait_imports.contains(&id)
    };
    providers.maybe_unused_extern_crates = |tcx, cnum| {
        assert_eq!(cnum, LOCAL_CRATE);
        &tcx.lowered_hir().maybe_unused_extern_crates[..]
    };
    providers.names_imported_by_glob_use = |tcx, id| {
        assert_eq!(id.krate, LOCAL_CRATE);
        Lrc::new(tcx.lowered_hir().glob_map.get(&id).cloned().unwrap_or_default())
    };
