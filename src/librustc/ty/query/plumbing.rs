...
pub struct QueryCache<'tcx, D: QueryConfig<'tcx> + ?Sized> {
    pub(super) results: FxHashMap<D::Key, PackedQueryValue<'tcx, D::Value>>,
}
...
/// to avoid increasing the alignment of the PackedQueryValue type
#[repr(packed(4))]
...
union PackedQueryState<'tcx, T> {
    value: T,
    job: UnalignedQueryJob<'tcx>,
}

pub(super) struct PackedQueryValue<'tcx, T> {
    state: PackedQueryState<'tcx, T>,
    index: u32,
}

static_assert!(
    MEM_SIZE_OF_PACKED_QUERY_VALUE:
    mem::size_of::<PackedQueryValue<'_, DefId>>() <= mem::size_of::<(DefId, DepNodeIndex)>()
);

unsafe impl<#[may_dangle] 'tcx, #[may_dangle] T> Drop for PackedQueryValue<'tcx, T> {
    fn drop(&mut self) {
        unsafe {
            if self.active() {
                ptr::drop_in_place(&mut self.state.job)
            } else {
                ptr::drop_in_place(&mut self.state.value)
            }
        }
    }
}

impl<'tcx, T> PackedQueryValue<'tcx, T> {
    #[inline]
    fn new_active(job: Lrc<QueryJob<'tcx>>) -> Self {
        PackedQueryValue {
            state: PackedQueryState {
                job: UnalignedQueryJob(job),
            },
            index: -1i8 as u32,
        }
    }
    ...
    #[inline]
    fn new_complete(value: T, index: DepNodeIndex) -> Self {
        debug_assert!(index.as_u32() != -1i8 as u32);
    ...
    #[inline]
    fn active(&self) -> bool {
        self.index == -1i8 as u32
    }

    #[inline]
    fn decode(&self) -> QueryValue<'_, 'tcx, T> {
        unsafe {
            if self.active() {
                QueryValue::Active(&self.state.job)
            } else {
                QueryValue::Complete {
                    value: &self.state.value,
                    index: DepNodeIndex::from_u32(self.index)
                }
            }
        }
    }
    ...
    pub fn unwrap(&self) -> (&T, DepNodeIndex) {
        unsafe {
            if !self.active() {
                (&self.state.value, DepNodeIndex::from_u32(self.index))
            } else {
                panic!("incomplete query value")
            }
        }
    }
}

enum QueryValue<'a, 'tcx, T> {
    ...
    Active(&'a UnalignedQueryJob<'tcx>)
}
...
impl<'a, 'tcx, Q: QueryDescription<'tcx>> JobOwner<'a, 'tcx, Q> {
    ...
    pub(super) fn try_get(
        ...
        loop {
            ...
            let job = match lock.results.entry((*key).clone()) {
                Entry::Occupied(ref entry) => match entry.get().decode() {
                    QueryValue::Complete { value, index } => {
                    ...
                    QueryValue::Active(job) => job.clone(),
                }
                Entry::Vacant(entry) => {
                    ...
                    return tls::with_related_context(tcx, |icx| {
                        ...
                        entry.insert(PackedQueryValue::new_active(job));
    ...
    pub(super) fn complete(self, result: &Q::Value, dep_node_index: DepNodeIndex) {
        ...
        cache.borrow_mut().results.insert(key, PackedQueryValue::new_complete(
            result.clone(),
            dep_node_index,
        ));
