...
def variable_from_gdb(var: gdb.Value) -> Variable:
    ...
    # The closest equivalent to LLDB's `SBTypeFormat` seems to be the dict `gdb.print_options()`
    # but it's not worth storing that on an individual variable level since it's a global setting.
    # Some other formatting happens based on the args of `gdb.Value.format_string()`, but since
    # that's on a call-by-call basis, we can't really store that either.
    format = None
