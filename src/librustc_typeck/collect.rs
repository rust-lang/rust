...
fn generics_of<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> &'tcx ty::Generics {
    ...
    let parent_def_id = match node {
        ...
        | Node::StructCtor(_) | Node::Field(_) => {
            let parent_id = tcx.hir().get_parent(hir_id);
...
fn type_of<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> Ty<'tcx> {
    ...
    match tcx.hir().get_by_hir_id(hir_id) {
        ...
        Node::Expr(&hir::Expr {
            ...
            if gen.is_some() {
                return tcx.typeck_tables_of(def_id).hir_id_to_type(hir_id);
            }
        ...
        Node::AnonConst(_) => match tcx.hir().get_by_hir_id(
            tcx.hir().get_parent_node(hir_id))
        {
            Node::Ty(&hir::Ty {
...
fn find_existential_constraints<'a, 'tcx>(
    ...
    let hir_id = tcx.hir().as_local_hir_id(def_id).unwrap();
    let parent = tcx.hir().get_parent(hir_id);
...
fn explicit_predicates_of<'a, 'tcx>(
    ...
    if let Some((self_trait_ref, trait_items)) = is_trait {
        predicates.extend(trait_items.iter().flat_map(|trait_item_ref| {
            ...
            let def_id = tcx.hir().local_def_id_from_hir_id(trait_item.hir_id);
            let assoc_ty = tcx.mk_projection(def_id, self_trait_ref.substs);
                ...
                tcx.sess
                   .struct_span_err(
                       ...
                       &format!(
                           ...
                           tcx.hir().node_to_pretty_string_by_hir_id(ast_ty.hir_id)
                       ),
                   )
