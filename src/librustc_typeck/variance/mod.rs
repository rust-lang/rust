...
fn item_variances<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, item_def_id: DefId)
                            -> Rc<Vec<ty::Variance>> {
    ...
    match item.node {
        ...
        hir::ItemUnion(..) => {
            ...
            let crate_map = tcx.dep_graph.with_ignore(|| {
                ty::queries::crate_variances::get(tcx, DUMMY_SP, LOCAL_CRATE)
            });
            tcx.dep_graph.read(DepNode::Hir(item_def_id));
            for &dep_def_id in crate_map.dependencies.less_than(&item_def_id) {
                if dep_def_id.is_local() {
                    tcx.dep_graph.read(DepNode::Hir(dep_def_id));
                } else {
