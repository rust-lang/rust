...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn try_overloaded_call_traits(&self,
        ...
        for &(opt_trait_def_id, method_name) in &[(self.tcx.lang_items.fn_trait(),
                                                   token::intern("call")),
                                                  (self.tcx.lang_items.fn_mut_trait(),
                                                   token::intern("call_mut")),
                                                  (self.tcx.lang_items.fn_once_trait(),
                                                   token::intern("call_once"))] {
    ...
    fn resolve<'a>(&mut self, fcx: &FnCtxt<'a, 'gcx, 'tcx>) {
        ...
        match fcx.try_overloaded_call_traits(self.call_expr,
            ...
            Some(method_callee) => {
                ...
                for (&method_arg_ty, &self_arg_ty) in method_sig.inputs[1..]
                                                          .iter()
                                                          .zip(&self.fn_sig.inputs) {
