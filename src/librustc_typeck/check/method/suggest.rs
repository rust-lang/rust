...
use rustc::ty::{self, Adt, ToPolyTraitRef, ToPredicate, Ty, TyCtxt, TypeFoldable};
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
            ...
            MethodError::NoMatch(NoMatchData {
                                ...
                                if let &QPath::Resolved(_, ref path) = &qpath {
                                    if let hir::def::Def::Local(node_id) = path.def {
                                        ...
                                        let parent_node = self
                                            .tcx
                                            .hir()
                                            .get(self.tcx.hir().get_parent_node(node_id));
                                        let msg = format!(
                ...
                if self.is_fn_ty(&rcvr_ty, span) {
                    ...
                    if let SelfSource::MethodCall(expr) = source {
                        ...
                        } else if let ExprKind::Path(QPath::Resolved(_, ref path)) = expr.node {
    ...
    fn type_derefs_to_local(&self, span: Span, rcvr_ty: Ty<'tcx>, source: SelfSource) -> bool {
