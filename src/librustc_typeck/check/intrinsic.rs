...
fn equate_intrinsic_type<'a, 'tcx>(ccx: &CrateCtxt<'a, 'tcx>,
    ...
    if i_n_tps != n_tps {
        ...
        require_same_types(ccx, it.span, i_ty.ty, fty, "intrinsic has wrong type");
    }
}
...
pub fn check_intrinsic_type(ccx: &CrateCtxt, it: &hir::ForeignItem) {
    ...
    let (n_tps, inputs, output) = if name.starts_with("atomic_") {
        ...
        let (n_tps, inputs, output) = match split[1] {
            "cxchg" | "cxchgweak" => {
                (1,
                 vec![tcx.mk_mut_ptr(param(ccx, 0)), param(ccx, 0), param(ccx, 0)],
                 tcx.mk_tup(vec![param(ccx, 0), tcx.types.bool]))
            }
            "load" => (1, vec![tcx.mk_imm_ptr(param(ccx, 0))], param(ccx, 0)),
            "store" => (1, vec![tcx.mk_mut_ptr(param(ccx, 0)), param(ccx, 0)], tcx.mk_nil()),
            ...
            "min" |
            "umax" |
            "umin" => (1, vec![tcx.mk_mut_ptr(param(ccx, 0)), param(ccx, 0)], param(ccx, 0)),
        ...
        let (n_tps, inputs, output) = match &name[..] {
            ...
            "size_of" |
            "pref_align_of" |
            "min_align_of" => (1, Vec::new(), ccx.tcx.types.usize),
            "size_of_val" | "min_align_of_val" => {
                (1,
                 vec![tcx.mk_imm_ref(tcx.mk_region(ty::ReLateBound(ty::DebruijnIndex::new(1),
                                                                   ty::BrAnon(0))),
                                     param(ccx, 0))],
                 ccx.tcx.types.usize)
            }
            ...
            "drop_in_place" => (1, vec![tcx.mk_mut_ptr(param(ccx, 0))], tcx.mk_nil()),
            ...
            "volatile_load" => (1, vec![tcx.mk_imm_ptr(param(ccx, 0))], param(ccx, 0)),
            "volatile_store" => {
                (1, vec![tcx.mk_mut_ptr(param(ccx, 0)), param(ccx, 0)], tcx.mk_nil())
            }
...
pub fn check_platform_intrinsic_type(ccx: &CrateCtxt, it: &hir::ForeignItem) {
    ...
    let (n_tps, inputs, output) = match &*name {
        ...
        "simd_and" |
        "simd_or" |
        "simd_xor" => (1, vec![param(0), param(0)], param(0)),
        ...
        name if name.starts_with("simd_shuffle") => {
            match name["simd_shuffle".len()..].parse() {
                Ok(n) => {
                    let params = vec![param(0), param(0), tcx.mk_ty(ty::TyArray(tcx.types.u32, n))];
                    (2, params, param(1))
                }
                Err(_) => {
                    span_err!(tcx.sess,
...
fn match_intrinsic_type_to_type<'tcx, 'a>(ccx: &CrateCtxt<'a, 'tcx>,
                                          position: &str,
                                          span: Span,
    ...
    match *expected {
        ...
        Integer(signed, bits, _llvm_width) => {
            match (signed, bits, &t.sty) {
        ...
        Aggregate(_flatten, ref expected_contents) => {
            match t.sty {
                ...
                _ => simple_error(&format!("`{}`", t), &format!("tuple")),
            }
        }
    }
}
