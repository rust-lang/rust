        ...
        let (n_tps, inputs, output) = match &name[..] {
            ...
            "va_copy" => match tcx.lang_items().va_list() {
                Some(did) => {
                    ...
                    (
                        0,
                        vec![tcx.mk_imm_ref(tcx.mk_region(env_region), va_list_ty)],
                        ret_ty,
                    )
                }
                None => bug!("va_list lang_item must be defined to use va_list intrinsics"),
...
fn match_intrinsic_type_to_type<'a, 'tcx>(
    ...
    match *expected {
        ...
        Float(bits) => match (bits, &t.sty) {
            (32, &ty::Float(ast::FloatTy::F32)) | (64, &ty::Float(ast::FloatTy::F64)) => {}
