...
use super::{Expectation, FnCtxt, check_fn};
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
    ...
    fn check_closure(&self,
        ...
        let fn_sig = self.tcx.liberate_late_bound_regions(self.tcx
                                                              .region_maps
                                                              .call_site_extent(expr.id, body.id),
                                                          &fn_ty.sig);
        ...
        let expected_kind =
            fulfillment_cx.pending_obligations()
                          ...
                          .filter_map(|obligation| {
                              ...
                              opt_trait_ref.and_then(|tr| {
                                               self.self_type_matches_expected_vid(tr, expected_vid)
                                           })
                                           .and_then(|tr| {
                                               self.tcx.lang_items.fn_trait_kind(tr.def_id())
                                           })
                          })
                          .fold(None,
