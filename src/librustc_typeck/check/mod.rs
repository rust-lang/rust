...
use rustc::ty::{
    self, AdtKind, CanonicalUserTypeAnnotation, GenericParamDefKind, RegionKind, ToPolyTraitRef,
    ToPredicate, Ty, TyCtxt, UserTypeAnnotation, Visibility,
};
...
fn primary_body_of<'a, 'tcx>(
    ...
    match tcx.hir().get(id) {
        Node::Item(item) => match item.node {
            hir::ItemKind::Const(_, body) | hir::ItemKind::Static(_, _, body) => Some((body, None)),
            hir::ItemKind::Fn(ref decl, .., body) => Some((body, Some(decl))),
            _ => None,
        },
...
fn typeck_tables_of<'a, 'tcx>(
    ...
    let tables = Inherited::build(tcx, def_id).enter(|inh| {
        ...
        let fcx = if let Some(decl) = fn_decl {
            ...
            let revealed_ty = if tcx.features().impl_trait_in_bindings {
                fcx.instantiate_opaque_types_from_value(id, &expected_type)
            } else {
                expected_type
            };
    ...
    assert_eq!(
        ...
        Some(DefId::local(
            tcx.hir().definitions().node_to_hir_id(id).owner
        ))
    );
    tables
}
    ...
    fn assign(&mut self, span: Span, nid: ast::NodeId, ty_opt: Option<LocalTy<'tcx>>) -> Ty<'tcx> {
        match ty_opt {
            None => {
                ...
                self.fcx.locals.borrow_mut().insert(
                    nid,
                    LocalTy {
                        decl_ty: var_ty,
                        revealed_ty: var_ty,
        ...
        let local_ty = match local.ty {
            Some(ref ty) => {
                ...
                let revealed_ty = if self.fcx.tcx.features().impl_trait_in_bindings {
                    self.fcx
                        .instantiate_opaque_types_from_value(self.parent_id, &o_ty)
                } else {
                    o_ty
                };
                ...
                let c_ty = self
                    ...
                    .inh
                    .infcx
                    .canonicalize_user_type_annotation(&UserTypeAnnotation::Ty(revealed_ty));
                debug!(
    ...
    fn visit_pat(&mut self, p: &'gcx hir::Pat) {
        if let PatKind::Binding(_, _, ident, _) = p.node {
            ...
            debug!(
                ...
                ident,
                self.fcx
                    .ty_to_string(self.fcx.locals.borrow().get(&p.id).unwrap().clone().decl_ty),
                var_ty
            );
        }
...
fn check_fn<'a, 'gcx, 'tcx>(
    ...
    *fcx.ps.borrow_mut() = UnsafetyState::function(fn_sig.unsafety, fn_id);
    ...
    let outer_def_id = fcx
        .tcx
        .closure_base_def_id(fcx.tcx.hir().local_def_id(fn_id));
    let outer_node_id = fcx.tcx.hir().as_local_node_id(outer_def_id).unwrap();
    GatherLocalsVisitor {
        fcx: &fcx,
        parent_id: outer_node_id,
    }
    .visit_body(body);
                    ...
                    if let Node::Item(item) = fcx.tcx.hir().get(fn_id) {
                        if let ItemKind::Fn(_, _, ref generics, _) = item.node {
                            if !generics.params.is_empty() {
                                fcx.tcx
                                    .sess
                                    .span_err(span, "should have no type parameters");
                            }
                        }
                    }
                } else {
            ...
            if let Some(alloc_layout_did) = fcx.tcx.lang_items().alloc_layout() {
                ...
                if inputs.len() == 1 {
                    ...
                    if !arg_is_alloc_layout {
                        fcx.tcx
                            .sess
                            .span_err(decl.inputs[0].span, "argument should be `Layout`");
                    }
...
fn check_specialization_validity<'a, 'tcx>(
    ...
    let parent = ancestors
        .defs(tcx, trait_item.ident, kind, trait_def.def_id)
        .nth(1)
    ...
    if !missing_items.is_empty() {
        ...
        for trait_item in missing_items {
            if let Some(span) = tcx.hir().span_if_local(trait_item.def_id) {
                ...
                err.note_trait_signature(trait_item.ident.to_string(), trait_item.signature(&tcx));
            }
        }
        err.emit();
    }
...
fn check_transparent<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, sp: Span, def_id: DefId) {
    ...
    let field_infos = adt.non_enum_variant().fields.iter().map(|field| {
        ...
        let align1 = layout
            .map(|layout| layout.align.abi.bytes() == 1)
            .unwrap_or(false);
        (span, zst, align1)
    });
...
fn report_unexpected_variant_def<'a, 'gcx, 'tcx>(
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
        ...
        if !method.substs.is_noop() {
            ...
            if !method_generics.params.is_empty() {
                let user_type_annotation = self.infcx.probe(|_| {
                    ...
                    self.infcx
                        .canonicalize_user_type_annotation(&UserTypeAnnotation::TypeOf(
                            method.def_id,
                            user_substs,
                        ))
                });
                ...
                debug!(
                    "write_method_call: user_type_annotation={:?}",
    ...
    pub fn write_user_type_annotation_from_substs(
        ...
        if !substs.is_noop() {
            ...
            debug!(
                "write_user_type_annotation_from_substs: canonicalized={:?}",
                canonicalized
            );
    ...
    fn instantiate_opaque_types_from_value<T: TypeFoldable<'tcx>>(
        ...
        let (value, opaque_type_map) = self.register_infer_ok_obligations(
            self.instantiate_opaque_types(parent_def_id, self.body_id, self.param_env, value),
        );
    ...
    pub fn require_type_is_sized_deferred(
        ...
        self.deferred_sized_obligations
            .borrow_mut()
            .push((ty, span, code));
    }
    ...
    pub fn to_ty_saving_user_provided_ty(&self, ast_ty: &hir::Ty) -> Ty<'tcx> {
        ...
        if ty.has_free_regions() || ty.has_projections() {
            let c_ty = self
                .infcx
                .canonicalize_response(&UserTypeAnnotation::Ty(ty));
            debug!("to_ty_saving_user_provided_ty: c_ty={:?}", c_ty);
            self.tables
                .borrow_mut()
                .user_provided_types_mut()
                .insert(ast_ty.hir_id, c_ty);
        }
    ...
    fn check_expr_meets_expectation_or_error(
        ...
        if let Some(mut err) = self.demand_suptype_diag(expr.span, expected_ty, ty) {
            ...
            if let (ExpectIfCondition, &ExprKind::Assign(ref lhs, ref rhs)) = (expected, &expr.node)
            {
    ...
    fn expected_inputs_for_expected_output(
        ...
        let expect_args = self
            .fudge_regions_if_ok(&RegionVariableOrigin::Coercion(call_span), || {
                ...
                match expr_t.sty {
                    ...
                    ty::Array(_, len) => {
                        if let (Some(len), Ok(user_index)) =
                            (len.assert_usize(self.tcx), field.as_str().parse::<u64>())
                        {
                            let base = self
                                .tcx
                                .sess
    ...
    pub fn check_struct_path(
        ...
        let variant = match def {
            ...
            Def::Variant(..) => match ty.sty {
                ty::Adt(adt, substs) => Some((adt.variant_of_def(def), adt.did, substs)),
    ...
    fn check_expr_struct(
        ...
        let (variant, adt_ty) = if let Some(variant_ty) = self.check_struct_path(qpath, expr.id) {
            variant_ty
        } else {
    ...
    pub fn resolve_ty_and_def_ufcs<'b>(
        ...
        debug!(
            "resolve_ty_and_def_ufcs: qpath={:?} node_id={:?} span={:?}",
            qpath, node_id, span
        );
    ...
    pub fn instantiate_value_path(
        ...
        match def {
            ...
            Def::Method(def_id) | Def::AssociatedConst(def_id) => {
                ...
                debug!(
                    "instantiate_value_path: def={:?} container={:?}",
                    def, container
                );
                match container {
