...
// typeck_results every time a new type is inferred would be unreasonably slow,
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    pub fn resolve_type_vars_in_body(
        ...
        debug!("writeback: typeck_results for {:?} are {:#?}", item_def_id, wbcx.typeck_results);
...
// fn-specific typeck_results to find references to types or regions. It
// resolves those regions to remove inference variables and writes the
// final result back into the master typeck_results in the tcx. Here and
...
impl<'cx, 'tcx> WritebackCx<'cx, 'tcx> {
    fn new(
        ...
        WritebackCx {
            fcx,
            typeck_results: ty::TypeckResults::empty(Some(owner)),
            body,
...
// traffic in node-ids or update typeck_results in the type context etc.
...
impl<'cx, 'tcx> WritebackCx<'cx, 'tcx> {
        ...
        T: TypeFoldable<'tcx>,
    {
        ...
        // users of the typeck_results don't produce extra errors, or worse, ICEs.
