...
use rustc::hir::Node;
use rustc::hir::{print, Item, ItemKind};
...
impl<'a, 'gcx, 'tcx> FnCtxt<'a, 'gcx, 'tcx> {
        ...
        methods.retain(|m| {
            self.has_no_input_arg(m)
                && self
                    ...
                    // `as_mut`?
                    .find(|a| a.check_name("rustc_conversion_suggestion"))
                    .is_some()
        });
                    ...
                    && sp.ctxt().outer().expn_info().is_none()
                {
                    match expr.node {
                        ...
                        _ => {
                            if self
                                .infcx
                                .type_is_copy_modulo_regions(self.param_env, checked, sp)
                            {
    ...
    fn is_range_literal(&self, expr: &hir::Expr) -> bool {
        use hir::{ExprKind, Path, QPath, TyKind};
    ...
    pub fn check_for_cast(
        ...
        if let Ok(src) = self.tcx.sess.source_map().span_to_snippet(expr.span) {
            ...
            let suggest_to_change_suffix_or_into =
                |err: &mut DiagnosticBuilder, note: Option<&str>| {
            ...
            match (&expected_ty.sty, &checked_ty.sty) {
                (&ty::Int(ref exp), &ty::Int(ref found)) => {
                    match (found.bit_width(), exp.bit_width()) {
                        ...
                        _ => {
                            suggest_to_change_suffix_or_into(err, Some(will_sign_extend));
                        }
                    }
                    true
                }
                (&ty::Uint(ref exp), &ty::Uint(ref found)) => {
                    match (found.bit_width(), exp.bit_width()) {
                        ...
                        _ => {
                            suggest_to_change_suffix_or_into(err, Some(will_zero_extend));
                        }
                    }
                    true
                }
                ...
                (&ty::Float(ref exp), &ty::Float(ref found)) => {
                    if found.bit_width() < exp.bit_width() {
                        suggest_to_change_suffix_or_into(err, None);
                    } else if can_cast {
                        err.span_suggestion_with_applicability(
                            expr.span,
                ...
                (&ty::Float(ref exp), &ty::Uint(ref found)) => {
                    ...
                    if exp.bit_width() > found.bit_width().unwrap_or(256) {
                        err.span_suggestion_with_applicability(
                            expr.span,
                            &format!(
                                "{}, producing the floating point representation of the \
                                 integer",
                                msg
                            ),
                            into_suggestion,
