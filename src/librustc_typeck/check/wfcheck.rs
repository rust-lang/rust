...
use check::FnCtxt;
use constrained_type_params::{Parameter, identify_constrained_type_params};
use CrateCtxt;
use hir::def_id::DefId;
use middle::region::CodeExtent;
use rustc::ty::subst::{self, FnSpace, ParamSpace, SelfSpace, TypeSpace};
use rustc::traits;
...
impl<'a, 'gcx, 'tcx> CheckWfFcxBuilder<'a, 'gcx, 'tcx> {
    fn with_fcx<F>(&'tcx mut self, f: F)
        where F: for<'b> FnOnce(&FnCtxt<'b, 'gcx, 'tcx>,
...
impl<'ccx, 'gcx> CheckTypeWellFormedVisitor<'ccx, 'gcx> {
    pub fn new(ccx: &'ccx CrateCtxt<'ccx, 'gcx>) -> CheckTypeWellFormedVisitor<'ccx, 'gcx> {
    ...
    fn check_item_well_formed(&mut self, item: &hir::Item) {
        ...
        match item.node {
            ...
            /// for `T`
            hir::ItemImpl(_, hir::ImplPolarity::Positive, _, ref trait_ref, ref self_ty, _) => {
            ...
            hir::ItemStruct(ref struct_def, ref ast_generics) => {
                self.check_type_defn(item, |fcx| vec![fcx.struct_variant(struct_def)]);

                self.check_variances_for_type_defn(item, ast_generics);
            }
            hir::ItemEnum(ref enum_def, ref ast_generics) => {
                self.check_type_defn(item, |fcx| fcx.enum_variants(enum_def));
        ...
        self.for_id(item_id, span).with_fcx(|fcx, this| {
            ...
            let (mut implied_bounds, self_ty) = match item.container() {
                ...
                ty::ImplContainer(def_id) => {
                    (fcx.impl_implied_bounds(def_id, span), fcx.tcx.lookup_item_type(def_id).ty)
                }
            };
            ...
            match item {
                ...
                ty::MethodTraitItem(method) => {
                    ...
                    this.check_fn_or_method(fcx,
                                            span,
                                            &method_ty,
                                            &predicates,
                                            free_id_outlive,
                                            &mut implied_bounds);
                    this.check_method_receiver(fcx, span, &method, free_id_outlive, self_ty);
                }
    ...
    fn for_item<'tcx>(&self, item: &hir::Item) -> CheckWfFcxBuilder<'ccx, 'gcx, 'tcx> {
        self.for_id(item.id, item.span)
    }

    fn for_id<'tcx>(&self, id: ast::NodeId, span: Span) -> CheckWfFcxBuilder<'ccx, 'gcx, 'tcx> {
    ...
    fn check_trait(&mut self, item: &hir::Item, items: &[hir::TraitItem]) {
    ...
    fn check_item_fn(&mut self, item: &hir::Item, body: &hir::Block) {
        self.for_item(item).with_fcx(|fcx, this| {
            let free_substs = &fcx.parameter_environment.free_substs;
    ...
    fn check_item_type(&mut self, item: &hir::Item) {
        ...
        self.for_item(item).with_fcx(|fcx, this| {
            ...
            match *ast_trait_ref {
                Some(ref ast_trait_ref) => {
                    ...
                    let trait_ref = fcx.instantiate_type_scheme(ast_trait_ref.path.span,
                                                                free_substs,
                                                                &trait_ref);
                    let obligations = ty::wf::trait_obligations(fcx,
                                                                fcx.body_id,
                                                                &trait_ref,
    ...
    fn check_where_clauses<'fcx, 'tcx>(&mut self,
        ...
        let obligations = predicates.predicates
                                    .iter()
                                    .flat_map(|p| {
                                        ty::wf::predicate_obligations(fcx, fcx.body_id, p, span)
                                    });
    ...
    fn check_method_receiver<'fcx, 'tcx>(&mut self,
        ...
        let rcvr_ty = match method.explicit_self {
            ...
            ty::ExplicitSelfCategory::ByReference(region, mutability) => {
                fcx.tcx.mk_ref(fcx.tcx.mk_region(region),
                               ty::TypeAndMut {
                                   ty: self_ty,
                                   mutbl: mutability,
                               })
        ...
        let rcvr_ty = fcx.tcx.liberate_late_bound_regions(free_id_outlive, &ty::Binder(rcvr_ty));

        debug!("check_method_receiver: receiver ty = {:?}", rcvr_ty);

        fcx.require_same_types(span, sig.inputs[0], rcvr_ty, "mismatched method receiver");
    }

    fn check_variances_for_type_defn(&self, item: &hir::Item, ast_generics: &hir::Generics) {
        let item_def_id = self.tcx().map.local_def_id(item.id);
        let ty_predicates = self.tcx().lookup_predicates(item_def_id);
        let variances = self.tcx().item_variances(item_def_id);

        let mut constrained_parameters: HashSet<_> = variances.types
                                                              .iter_enumerated()
                                                              .filter(|&(_, _, &variance)| {
                                                                  variance != ty::Bivariant
                                                              })
                                                              .map(|(space, index, _)| {
    ...
    fn report_bivariance(&self, span: Span, param_name: ast::Name) {
...
fn reject_shadowing_type_parameters(tcx: TyCtxt, span: Span, generics: &ty::Generics) {
    let impl_params = generics.types
    ...
    fn struct_variant(&self, struct_def: &hir::VariantData) -> AdtVariant<'tcx> {
        let fields = struct_def.fields()
                               ...
                               .map(|field| {
                                   ...
                                   let field_ty =
                                       self.instantiate_type_scheme(field.span,
...
fn error_392<'a, 'tcx>(ccx: &CrateCtxt<'a, 'tcx>,
                       span: Span,
                       param_name: ast::Name)
