...
use rustc::ty::{self, GenericParamDefKind, Lift, ToPredicate, Ty, TyCtxt, TyKind, TypeFoldable};
...
impl<'a, 'gcx, 'tcx> CheckWfFcxBuilder<'a, 'gcx, 'tcx> {
    ...
    where
        F: for<'b> FnOnce(&FnCtxt<'b, 'gcx, 'tcx>, TyCtxt<'b, 'gcx, 'gcx>) -> Vec<Ty<'tcx>>,
    {
        let id = self.id;
        let span = self.span;
...
pub fn check_item_well_formed<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) {
    ...
    match item.node {
        ...
        hir::ItemKind::Impl(_, polarity, defaultness, _, ref trait_ref, ref self_ty, _) => {
            let is_auto = tcx
                .impl_trait_ref(tcx.hir().local_def_id(item.id))
                .map_or(false, |trait_ref| tcx.trait_is_auto(trait_ref.def_id));
            if let (hir::Defaultness::Default { .. }, true) = (defaultness, is_auto) {
                tcx.sess
                    .span_err(item.span, "impls of auto traits cannot be default");
            }
        ...
        hir::ItemKind::Union(ref struct_def, ref ast_generics) => {
            check_type_defn(
                ...
                item,
                true,
                |fcx| vec![fcx.non_enum_variant(struct_def)],
            );
...
fn check_associated_item<'a, 'tcx>(
    ...
    for_id(tcx, item_id, span).with_fcx(|fcx, tcx| {
        ...
        let (mut implied_bounds, self_ty) = match item.container {
            ...
            ty::ImplContainer(def_id) => (
                fcx.impl_implied_bounds(def_id, span),
            ...
            let needs_drop_copy = || {
                packed && {
                    ...
                    let ty = fcx
                        ...
                        .erase_regions(&ty)
                        .lift_to_tcx(fcx_tcx)
                        .unwrap_or_else(|| span_bug!(item.span, "inference variables in {:?}", ty));
...
fn check_where_clauses<'a, 'gcx, 'fcx, 'tcx>(
    ...
    let default_obligations = predicates
        ...
        .map(|pred| {
            ...
            let cause =
                traits::ObligationCause::new(span, fcx.body_id, traits::ItemObligation(def_id));
    ...
    if let Some(return_ty) = return_ty {
        predicates
            .predicates
            .extend(check_existential_types(tcx, fcx, def_id, span, return_ty));
    }

    let predicates = fcx.normalize_associated_types_in(span, &predicates);

    debug!(
        "check_where_clauses: predicates={:?}",
    ...
    ty.fold_with(&mut ty::fold::BottomUpFolder {
        ...
        fldop: |ty| {
            if let ty::Opaque(def_id, substs) = ty.sty {
                trace!(
                    "check_existential_types: opaque_ty, {:?}, {:?}",
                    def_id,
                    substs
                );
                let generics = tcx.generics_of(def_id);
                ...
                if generics.parent.is_none() {
                    ...
                    if may_define_existential_type(tcx, fn_def_id, opaque_node_id) {
                        trace!(
                            "check_existential_types may define. Generics: {:#?}",
                            generics
                        );
...
fn check_method_receiver<'fcx, 'gcx, 'tcx>(
    fcx: &FnCtxt<'fcx, 'gcx, 'tcx>,
    ...
    let self_ty = fcx
        .tcx
        .liberate_late_bound_regions(method.def_id, &ty::Binder::bind(self_ty));
    ...
    let receiver_ty = fcx
        .tcx
        .liberate_late_bound_regions(method.def_id, &ty::Binder::bind(receiver_ty));
    ...
    if fcx.tcx.features().arbitrary_self_types {
        if !receiver_is_valid(fcx, span, receiver_ty, self_ty, true) {
            ...
            fcx.tcx
                ...
                .mut_span_err(
                    span,
                    &format!("invalid method receiver type: {:?}", receiver_ty),
                )
...
fn receiver_is_valid<'fcx, 'tcx, 'gcx>(
    ...
    loop {
        if let Some((potential_self_ty, _)) = autoderef.next() {
            ...
            debug!(
                "receiver_is_valid: type `{:?}` does not deref to `{:?}`",
                receiver_ty, self_ty
            );
            return false;
        }
    ...
    if !arbitrary_self_types_enabled {
        ...
        let obligation =
            traits::Obligation::new(cause.clone(), fcx.param_env, trait_ref.to_predicate());

        if !fcx.predicate_must_hold_modulo_regions(&obligation) {
            debug!(
                "receiver_is_valid: type `{:?}` does not implement `Receiver` trait",
                receiver_ty
            );
            return false;
        }
...
fn check_variances_for_type_defn<'a, 'tcx>(
    tcx: TyCtxt<'a, 'tcx, 'tcx>,
    item: &hir::Item,
    ...
    identify_constrained_type_params(tcx, &ty_predicates, None, &mut constrained_parameters);
...
fn report_bivariance<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, span: Span, param_name: ast::Name) {
...
fn reject_shadowing_parameters(tcx: TyCtxt, def_id: DefId) {
    ...
    let impl_params: FxHashMap<_, _> = parent
        .params
        .iter()
...
impl<'a, 'gcx> CheckTypeWellFormedVisitor<'a, 'gcx> {
    pub fn new(tcx: TyCtxt<'a, 'gcx, 'gcx>) -> CheckTypeWellFormedVisitor<'a, 'gcx> {
