...
impl Type {
    #[inline(always)]
    pub(crate) fn from_ref(r: TypeRef) -> Type {
        Type {
            rf: r
        }
    }
    ...
    pub(crate) fn to_ref_slice(slice: &[Type]) -> &[TypeRef] {
        unsafe { mem::transmute(slice) }
    }

    pub(crate) fn void(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMVoidTypeInContext(ccx.llcx()))
    }

    pub(crate) fn nil(ccx: &CrateContext) -> Type {
        Type::empty_struct(ccx)
    }

    pub(crate) fn metadata(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMRustMetadataTypeInContext(ccx.llcx()))
    }

    pub(crate) fn i1(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMInt1TypeInContext(ccx.llcx()))
    }

    pub(crate) fn i8(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMInt8TypeInContext(ccx.llcx()))
    }

    pub(crate) fn i8_llcx(llcx: ContextRef) -> Type {
        ty!(llvm::LLVMInt8TypeInContext(llcx))
    }

    pub(crate) fn i16(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMInt16TypeInContext(ccx.llcx()))
    }

    pub(crate) fn i32(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMInt32TypeInContext(ccx.llcx()))
    }

    pub(crate) fn i64(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMInt64TypeInContext(ccx.llcx()))
    }

    pub(crate) fn i128(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMIntTypeInContext(ccx.llcx(), 128))
    }

    // Creates an integer type with the given number of bits, e.g. i24
    pub(crate) fn ix(ccx: &CrateContext, num_bits: u64) -> Type {
        ty!(llvm::LLVMIntTypeInContext(ccx.llcx(), num_bits as c_uint))
    }

    pub(crate) fn f32(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMFloatTypeInContext(ccx.llcx()))
    }

    pub(crate) fn f64(ccx: &CrateContext) -> Type {
        ty!(llvm::LLVMDoubleTypeInContext(ccx.llcx()))
    }

    pub(crate) fn bool(ccx: &CrateContext) -> Type {
        Type::i8(ccx)
    }

    pub(crate) fn char(ccx: &CrateContext) -> Type {
        Type::i32(ccx)
    }

    pub(crate) fn i8p(ccx: &CrateContext) -> Type {
        Type::i8(ccx).ptr_to()
    }

    pub(crate) fn i8p_llcx(llcx: ContextRef) -> Type {
        Type::i8_llcx(llcx).ptr_to()
    }

    pub(crate) fn int(ccx: &CrateContext) -> Type {
    ...
    pub(crate) fn int_from_ty(ccx: &CrateContext, t: ast::IntTy) -> Type {
        match t {
    ...
    pub(crate) fn uint_from_ty(ccx: &CrateContext, t: ast::UintTy) -> Type {
        match t {
    ...
    pub(crate) fn float_from_ty(ccx: &CrateContext, t: ast::FloatTy) -> Type {
        match t {
    ...
    pub(crate) fn func(args: &[Type], ret: &Type) -> Type {
        let slice: &[TypeRef] = Type::to_ref_slice(args);
        ty!(llvm::LLVMFunctionType(ret.to_ref(), slice.as_ptr(),
                                   args.len() as c_uint, False))
    }

    pub(crate) fn variadic_func(args: &[Type], ret: &Type) -> Type {
        let slice: &[TypeRef] = Type::to_ref_slice(args);
        ty!(llvm::LLVMFunctionType(ret.to_ref(), slice.as_ptr(),
                                   args.len() as c_uint, True))
    }

    pub(crate) fn struct_(ccx: &CrateContext, els: &[Type], packed: bool) -> Type {
    ...
    pub(crate) fn named_struct(ccx: &CrateContext, name: &str) -> Type {
        let name = CString::new(name).unwrap();
        ty!(llvm::LLVMStructCreateNamed(ccx.llcx(), name.as_ptr()))
    }

    pub(crate) fn empty_struct(ccx: &CrateContext) -> Type {
        Type::struct_(ccx, &[], false)
    }

    pub(crate) fn array(ty: &Type, len: u64) -> Type {
        ty!(llvm::LLVMRustArrayType(ty.to_ref(), len))
    }

    pub(crate) fn vector(ty: &Type, len: u64) -> Type {
        ty!(llvm::LLVMVectorType(ty.to_ref(), len as c_uint))
    }

    pub(crate) fn vec(ccx: &CrateContext, ty: &Type) -> Type {
        Type::struct_(ccx,
            &[Type::array(ty, 0), Type::int(ccx)],
        false)
    }

    pub(crate) fn opaque_vec(ccx: &CrateContext) -> Type {
        Type::vec(ccx, &Type::i8(ccx))
    }

    pub(crate) fn vtable_ptr(ccx: &CrateContext) -> Type {
    ...
    pub(crate) fn set_struct_body(&mut self, els: &[Type], packed: bool) {
    ...
    pub(crate) fn element_type(&self) -> Type {
        unsafe {
    ...
    pub(crate) fn vector_length(&self) -> usize {
        unsafe {
    ...
    pub(crate) fn func_params(&self) -> Vec<Type> {
        unsafe {
    ...
    pub(crate) fn float_width(&self) -> usize {
        match self.kind() {
            Float => 32,
            Double => 64,
            X86_FP80 => 80,
    ...
    pub(crate) fn int_width(&self) -> u64 {
        unsafe {
            llvm::LLVMGetIntTypeWidth(self.to_ref()) as u64
        }
    }

    pub(crate) fn from_integer(cx: &CrateContext, i: layout::Integer) -> Type {
    ...
    pub(crate) fn from_primitive(ccx: &CrateContext, p: layout::Primitive) -> Type {
        match p {
