...
pub fn trans_into<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                  expr: &ast::Expr,
                                  dest: Dest)
                                  -> &'blk BlockS {
    ...
    if !qualif.intersects(
        ...
        if !qualif.intersects(check_const::ConstQualif::PREFER_IN_PLACE) {
            if let SaveIn(lldest) = dest {
                ...
                let expr_ty = expr_ty_adjusted(bcx, expr);
                memcpy_ty(bcx, lldest, global, expr_ty);
            }
                ...
                ast::ExprPath(..) => {
                    match bcx.def(expr.id) {
                        def::DefConst(did) => {
                            ...
                            let bl = bcx.bl;
                            bcx.bl = debuginfo::with_source_location_override(bcx.fcx,
                                                                              expr.debug_loc(),
                                                                              |fcx| {
    ...
    let bl = match kind {
        ty::LvalueExpr | ty::RvalueDatumExpr => {
            trans_unadjusted(bcx, expr).store_to_dest(bcx.fcx, dest, expr.id)
        }
        ty::RvalueDpsExpr => {
    ...
    bcx.fcx.pop_and_trans_ast_cleanup_scope(bl, expr.id)
}
...
/// stack.
pub fn trans<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                             expr: &ast::Expr)
    ...
    let global = if !qualif.intersects(
        ...
        if qualif.intersects(check_const::ConstQualif::HAS_STATIC_BORROWS) {
            ...
            return DatumBlock::new(bcx.bl, datum.to_expr_datum());
        }
    ...
    let bl = bcx.fcx.pop_and_trans_ast_cleanup_scope(bcx.bl, expr.id);
    return DatumBlock::new(bl, datum);
}

pub fn get_len(bcx: &mut Block, fat_ptr: ValueRef) -> ValueRef {
    GEPi(bcx, fat_ptr, &[0, abi::FAT_PTR_EXTRA])
}

pub fn get_dataptr(bcx: &mut Block, fat_ptr: ValueRef) -> ValueRef {
...
/// translation of `expr`.
fn apply_adjustments<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                     expr: &ast::Expr,
                                     ...
                                     -> DatumBlock<'blk, 'tcx, Expr>
{
    ...
    let adjustment = match bcx.tcx().adjustments.borrow().get(&expr.id).cloned() {
        None => {
            return DatumBlock::new(bcx.bl, datum);
        }
        Some(adj) => { adj }
    };
    ...
    match adjustment {
        ...
        AdjustDerefRef(ref adj) => {
            ...
            if skip_reborrows == 0 && adj.autoref.is_some() {
                let ad = apply_autoref(bcx, expr, datum);
                datum = unpack_datum!(bcx, ad);
            }

            if let Some(target) = adj.unsize {
                let m = bcx.monomorphize(&target);
                let up = unsize_pointer(bcx, datum, m);
                datum = unpack_datum!(bcx, up);
            }
        }
    }
    ...
    fn apply_autoref<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                     expr: &ast::Expr,
                                     datum: Datum<'tcx, Expr>)
                                     -> DatumBlock<'blk, 'tcx, Expr> {
        let mut bcx = &mut bl.with(fcx);
    ...
    fn unsize_pointer<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                      datum: Datum<'tcx, Expr>,
                                      target: Ty<'tcx>)
                                      -> DatumBlock<'blk, 'tcx, Expr> {
        let mut bcx = &mut bl.with(fcx);
        ...
        let dp = get_dataptr(bcx, scratch);
        Store(bcx, base, dp);
        let dl = get_len(bcx, scratch);
        Store(bcx, info, dl);

        DatumBlock::new(bcx.bl, Datum::new(scratch, target, RvalueExpr(Rvalue::new(ByRef))))
    }
}
...
///
///   { tmp = x(); tmp.f }
pub fn trans_to_lvalue<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                       expr: &ast::Expr,
                                       name: &str)
...
/// directly.
fn trans_unadjusted<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                    expr: &ast::Expr)
                                    -> DatumBlock<'blk, 'tcx, Expr> {
    let mut bcx = &mut bl.with(fcx);
    ...
    return match ty::expr_kind(bcx.tcx(), expr) {
        ...
        ty::RvalueStmtExpr => {
            let bl = trans_rvalue_stmt_unadjusted(bcx, expr);
            let expr_ty = expr_ty(bcx, expr);
            nil(&mut bl.with(bcx.fcx), expr_ty)
        }

        ty::RvalueDpsExpr => {
            let ty = expr_ty(bcx, expr);
            if type_is_zero_size(bcx.ccx(), ty) {
                let bl = trans_rvalue_dps_unadjusted(bcx, expr, Ignore);
                nil(&mut bl.with(bcx.fcx), ty)
            } else {
                let scratch = rvalue_scratch_datum(bcx, ty, "");
                bcx.bl = trans_rvalue_dps_unadjusted(
    ...
    fn nil<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>, ty: Ty<'tcx>)
                       -> DatumBlock<'blk, 'tcx, Expr> {
        let llval = C_undef(type_of::type_of(bcx.ccx(), ty));
        let datum = immediate_rvalue(llval, ty);
        DatumBlock::new(bcx.bl, datum.to_expr_datum())
    }
}
...
fn trans_datum_unadjusted<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
    ...
    match expr.node {
        ...
        ast::ExprPath(..) => {
            let def = bcx.def(expr.id);
            trans_def(bcx, expr, def)
        }
        ...
        ast::ExprAddrOf(_, ref x) => {
            match x.node {
                ast::ExprRepeat(..) | ast::ExprVec(..) => {
                    ...
                    let bl = bcx.fcx.pop_and_trans_ast_cleanup_scope(bcx.bl, x.id);
                    DatumBlock::new(bl, datum)
                }
                _ => {
...
fn trans_field<'r, 'blk, 'tcx, F>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                  base: &ast::Expr,
                                  get_idx: F)
    ...
    F: FnOnce(&'blk ty::ctxt<'tcx>, &[ty::field<'tcx>]) -> usize,
{
    ...
    with_field_tys(bcx.tcx(), bare_ty, None, move |discr, field_tys| {
        ...
        let d = base_datum.get_element(
            bcx,
            field_tys[ix].mt.ty,
            |bcx, srcval| adt::trans_field_ptr(bcx, &*repr, srcval, discr, ix));

        if type_is_sized(bcx.tcx(), d.ty) {
            DatumBlock { datum: d.to_expr_datum(), bcx: bcx.bl }
        } else {
            let scratch = rvalue_scratch_datum(bcx, d.ty, "");
            let dp = get_dataptr(bcx, scratch.val);
            Store(bcx, d.val, dp);
            let bl = get_len(bcx, base_datum.val);
            let info = Load(bcx, bl);
...
fn trans_rec_field<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                   base: &ast::Expr,
                                   field: ast::Name)
                                   -> DatumBlock<'blk, 'tcx, Expr> {
...
/// Translates `base.<idx>`.
fn trans_rec_tup_field<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                   base: &ast::Expr,
                                   idx: usize)
                                   -> DatumBlock<'blk, 'tcx, Expr> {
...
fn trans_index<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                               index_expr: &ast::Expr,
                               base: &ast::Expr,
                               idx: &ast::Expr,
    ...
    let elt_datum = match method_ty {
        ...
        None => {
            ...
            let expected = Call(bcx,
                                ...
                                None,
                                index_expr_debug_loc);
            bcx.bl = with_cond(bcx, expected, |bcx| {
    ...
    DatumBlock::new(bcx.bl, elt_datum)
}
...
fn trans_def<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
    ...
    match def {
        ...
        def::DefStatic(did, _) => {
            ...
            DatumBlock::new(bcx.bl, Datum::new(val, const_ty, LvalueExpr))
        }
        def::DefConst(_) => {
        ...
        _ => {
            DatumBlock::new(bcx.bl, trans_local_var(bcx, def).to_expr_datum())
        }
    }
}
...
fn trans_rvalue_dps_unadjusted<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
    ...
    match expr.node {
        ...
        ast::ExprPath(..) => {
            let def = bcx.def(expr.id);
            trans_def_dps_unadjusted(bcx, expr, def, dest)
        }
        ...
        ast::ExprStruct(_, ref fields, ref base) => {
            let ty = node_id_type(bcx, expr.id);
            trans_struct(bcx,
                         &fields[..],
                         base.as_ref().map(|e| &**e),
                         expr.span,
...
fn trans_def_dps_unadjusted<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                            ref_expr: &ast::Expr,
                                            def: def::Def,
                                            dest: Dest)
                                            -> &'blk BlockS {
...
pub fn trans_local_var<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                       def: def::Def)
                                       -> Datum<'tcx, Lvalue> {
...
fn trans_struct<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                fields: &[ast::Field],
                                base: Option<&ast::Expr>,
                                expr_span: codemap::Span,
                                expr_id: ast::NodeId,
                                ty: Ty<'tcx>,
                                dest: Dest) -> &'blk BlockS {
...
pub fn trans_adt<'a, 'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                     ty: Ty<'tcx>,
                                     discr: ty::Disr,
    ...
    if ty::type_is_simd(bcx.tcx(), ty) {
        ...
        match ty::expr_kind(bcx.tcx(), &*base.expr) {
            ty::RvalueDpsExpr | ty::RvalueDatumExpr if !bcx.fcx.type_needs_drop(ty) => {
                bcx.bl = trans_into(bcx, &*base.expr, SaveIn(addr));
            },
        ...
        for &(i, ref e) in fields {
            ...
            bcx.bl = trans_into(bcx, &**e, SaveIn(dest));
            let scope = cleanup::CustomScope(custom_cleanup_scope);
            bcx.fcx.schedule_lifetime_end(scope, dest);
            bcx.fcx.schedule_drop_mem(scope, dest, e_ty);
        }
    }
    ...
    match dest {
        SaveIn(_) => bcx.bl,
        Ignore => {
            bcx.bl = glue::drop_ty(bcx, addr, ty, debug_location);
...
fn trans_immediate_lit<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                       expr: &ast::Expr,
                                       lit: &ast::Lit)
                                       -> DatumBlock<'blk, 'tcx, Expr> {
...
fn trans_unary<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                               expr: &ast::Expr,
                               op: ast::UnOp,
                               sub_expr: &ast::Expr)
                               -> DatumBlock<'blk, 'tcx, Expr> {
    ...
    match op {
        ast::UnNot => {
            let t = trans(bcx, sub_expr);
            let datum = unpack_datum!(bcx, t);
            let lls = datum.to_llscalarish(bcx);
            let llresult = Not(bcx, lls, debug_loc);
        ...
        ast::UnNeg => {
            ...
            let (bl, llneg) = {
                if ty::type_is_fp(un_ty) {
                    ...
                    let bl = if bcx.ccx().check_overflow() && is_signed {
            ...
            immediate_rvalue_bcx(&mut bl.with(bcx.fcx), llneg, un_ty).to_expr_datumblock()
        }
        ast::UnUniq => {
            let expr_ty = expr_ty(bcx, sub_expr);
            trans_uniq_expr(bcx, expr, un_ty, sub_expr, expr_ty)
        }
        ast::UnDeref => {
            let datum = unpack_datum!(bcx, trans(bcx, sub_expr));
...
fn trans_uniq_expr<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                   box_expr: &ast::Expr,
                                   box_ty: Ty<'tcx>,
                                   contents: &ast::Expr,
                                   contents_ty: Ty<'tcx>)
    ...
    let Result { bcx: bl, val } = malloc_raw_dyn(bcx,
                                                 llty_ptr,
                                                 box_ty,
                                                 size,
                                                 align,
    ...
    // `Box<ZeroSizeType>`.
    let mut bcx = if llsize_of_alloc(bcx.ccx(), llty) == 0 {
        trans_into(&mut bl.with(bcx.fcx), contents, SaveIn(val))
    } else {
        let custom_cleanup_scope = bcx.fcx.push_custom_cleanup_scope();
        bcx.fcx.schedule_free_value(cleanup::CustomScope(custom_cleanup_scope),
                                    val, cleanup::HeapExchange, contents_ty);
        let bl = trans_into(&mut bl.with(bcx.fcx), contents, SaveIn(val));
        bcx.fcx.pop_custom_cleanup_scope(custom_cleanup_scope);
        bl
    }.with(bcx.fcx);
    immediate_rvalue_bcx(&mut bcx, val, box_ty).to_expr_datumblock()
}

fn ref_fat_ptr<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
...
fn trans_addr_of<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                 expr: &ast::Expr,
                                 subexpr: &ast::Expr)
                                 -> DatumBlock<'blk, 'tcx, Expr> {
...
// and the other not.
fn trans_eager_binop<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                     binop_expr: &ast::Expr,
                                     binop_ty: Ty<'tcx>,
                                     op: ast::BinOp,
                                     lhs_t: Ty<'tcx>,
    ...
    let val = match op.node {
      ...
      ast::BiDiv => {
        if is_float {
            ...
            bcx.bl = base::fail_if_zero_or_overflows(bcx,
...
fn trans_lazy_binop<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                    binop_expr: &ast::Expr,
                                    op: lazy_binop_ty,
                                    a: &ast::Expr,
                                    b: &ast::Expr)
    ...
    let lhs = lhs.to_llscalarish(&mut past_lhs.with(bcx.fcx));

    if past_lhs.unreachable.get() {
        return immediate_rvalue_bcx(&mut past_lhs.with(bcx.fcx), lhs, binop_ty)
            .to_expr_datumblock();
    }

    let join = bcx.fcx.new_id_block("join", binop_expr.id);
    let before_rhs = bcx.fcx.new_id_block("before_rhs", b.id);

    match op {
      lazy_and => CondBr(&mut past_lhs.with(bcx.fcx),
                         lhs, before_rhs.llbb, join.llbb, DebugLoc::None),
      lazy_or => CondBr(&mut past_lhs.with(bcx.fcx),
                        lhs, join.llbb, before_rhs.llbb, DebugLoc::None)
    }

    let DatumBlock {bcx: past_rhs, datum: rhs} = trans(&mut before_rhs.with(bcx.fcx), b);
    let rhs = rhs.to_llscalarish(&mut past_rhs.with(bcx.fcx));

    if past_rhs.unreachable.get() {
        return immediate_rvalue_bcx(&mut join.with(bcx.fcx), lhs, binop_ty).to_expr_datumblock();
    }

    Br(&mut past_rhs.with(bcx.fcx), join.llbb, DebugLoc::None);
    let ty = Type::i1(bcx.ccx());
    let phi = Phi(&mut join.with(bcx.fcx), ty, &[lhs, rhs],
                  &[past_lhs.llbb, past_rhs.llbb]);

    return immediate_rvalue_bcx(&mut join.with(bcx.fcx), phi, binop_ty).to_expr_datumblock();
}

fn trans_binary<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                expr: &ast::Expr,
                                op: ast::BinOp,
                                lhs: &ast::Expr,
                                rhs: &ast::Expr)
...
fn trans_overloaded_op<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                       expr: &ast::Expr,
                                       method_call: MethodCall,
                                       lhs: Datum<'tcx, Expr>,
...
fn trans_overloaded_call<'a, 'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
    ...
    let ty = monomorphize_type(bcx, method_type);
    unpack_result!(bcx,
                   callee::trans_call_inner(bcx,
                                            expr.debug_loc(),
                                            ty,
...
fn trans_assign_op<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                   expr: &ast::Expr,
                                   op: ast::BinOp,
                                   dst: &ast::Expr,
                                   src: &ast::Expr)
...
fn auto_ref<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                            datum: Datum<'tcx, Expr>,
                            expr: &ast::Expr)
                            -> DatumBlock<'blk, 'tcx, Expr> {
    let mut bcx = &mut bl.with(fcx);
    ...
    // of type `&referent_ty`.
    DatumBlock::new(bcx.bl, Datum::new(llref, ptr_ty, RvalueExpr(Rvalue::new(ByValue))))
}

fn deref_multiple<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                  expr: &ast::Expr,
                                  datum: Datum<'tcx, Expr>,
                                  times: usize)
                                  -> DatumBlock<'blk, 'tcx, Expr> {
...
fn deref_once<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                              expr: &ast::Expr,
                              datum: Datum<'tcx, Expr>,
                              method_call: MethodCall)
                              -> DatumBlock<'blk, 'tcx, Expr> {
    ...
    let r = match datum.ty.sty {
        ...
        ty::ty_rptr(_, ty::mt { ty: content_ty, .. }) => {
            if type_is_sized(bcx.tcx(), content_ty) {
                ...
                DatumBlock::new(bcx.bl, Datum::new(datum.val, content_ty, LvalueExpr))
            }
        }
    ...
    fn deref_owned_pointer<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                           expr: &ast::Expr,
                                           datum: Datum<'tcx, Expr>,
                                           content_ty: Ty<'tcx>)
                                           -> DatumBlock<'blk, 'tcx, Expr> {
...
impl OverflowOpViaIntrinsic {
    ...
    fn build_intrinsic_call<'r, 'blk, 'tcx>(&self, bcx: &mut Block<'r, 'blk, 'tcx>,
                                            info: NodeIdAndSpan,
        ...
        let ty = Type::i1(bcx.ccx());
        let cond = ICmp(bcx, llvm::IntEQ, overflow, C_integral(ty, 1, false),
                        binop_debug_loc);

        let expect = bcx.ccx().get_intrinsic(&"llvm.expect.i1");
        let ty = Type::i1(bcx.ccx());
        Call(bcx, expect, &[cond, C_integral(ty, 0, false)],
...
impl OverflowOpViaInputCheck {
    fn build_with_input_check<'r, 'blk, 'tcx>(&self,
...
fn build_unchecked_lshift<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                          lhs: ValueRef,
                                          rhs: ValueRef,
...
fn build_unchecked_rshift<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                          lhs_t: Ty<'tcx>,
                                          lhs: ValueRef,
                                          rhs: ValueRef,
                                          binop_debug_loc: DebugLoc) -> ValueRef {
...
fn shift_mask_rhs<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                  rhs: ValueRef,
