...
use trans::datum;
use trans::debuginfo;
use trans::debuginfo::SourceLocation::{mod, NoSourceLoc};
use trans::expr;
use trans::meth;
...
pub fn trans_if<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    let cond_source_loc = SourceLocation::from_expr(cond);
    ...
    match els {
        ...
        None => {
            ...
            Br(then_bcx_out, next_bcx.llbb, NoSourceLoc);
...
pub fn trans_while<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    Br(bcx, cond_bcx_in.llbb, SourceLocation::from_expr(loop_expr));
    ...
    let cond_source_loc = SourceLocation::from_expr(cond);
    CondBr(cond_bcx_out, cond_val, body_bcx_in.llbb, cleanup_llbb, cond_source_loc);

    // loop body:
    let body_bcx_out = trans_block(body_bcx_in, body, expr::Ignore);
    Br(body_bcx_out, cond_bcx_in.llbb, NoSourceLoc);
...
pub fn trans_for<'blk, 'tcx>(mut bcx: Block<'blk, 'tcx>,
    ...
    Br(bcx, loopback_bcx_in.llbb, NoSourceLoc);
    ...
    CondBr(loopback_bcx_out, llcondition, body_bcx_in.llbb, cleanup_llbb, NoSourceLoc);
    ...
    Br(body_bcx_out, loopback_bcx_in.llbb, NoSourceLoc);
...
pub fn trans_loop<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    Br(bcx, body_bcx_in.llbb, SourceLocation::from_expr(loop_expr));
    let body_bcx_out = trans_block(body_bcx_in, body, expr::Ignore);
    Br(body_bcx_out, body_bcx_in.llbb, NoSourceLoc);
...
pub fn trans_break_cont<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    Br(bcx, cleanup_llbb, SourceLocation::from_expr(expr));
...
pub fn trans_ret<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    Br(bcx, cleanup_llbb, SourceLocation::from_expr(return_expr));
    Unreachable(bcx);
    return bcx;
}
