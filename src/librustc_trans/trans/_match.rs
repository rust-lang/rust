...
pub enum OptResult<'blk> {
    SingleResult(Result<'blk>),
    RangeResult(Result<'blk>, Result<'blk>),
    LowerBound(Result<'blk>)
}
...
fn expand_nested_bindings<'a, 'p, 'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                                  m: &[Match<'a, 'p, 'blk, 'tcx>],
                                                  col: usize,
                                                  val: ValueRef)
...
fn enter_match<'a, 'b, 'p, 'r, 'blk, 'tcx, F>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                              dm: &DefMap,
                                              m: &[Match<'a, 'p, 'blk, 'tcx>],
                                              col: usize,
                                              val: ValueRef,
...
fn enter_default<'a, 'p, 'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                         dm: &DefMap,
                                         m: &[Match<'a, 'p, 'blk, 'tcx>],
                                         col: usize,
                                         val: ValueRef)
...
fn get_branches<'a, 'p, 'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                        m: &[Match<'a, 'p, 'blk, 'tcx>],
                                        col: usize)
                                        -> Vec<Opt<'p, 'tcx>> {
    let tcx = bcx.tcx();
...
struct ExtractedBlock<'blk> {
    vals: Vec<ValueRef>,
    bcx: &'blk BlockS
}

fn extract_variant_args<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                        repr: &adt::Repr<'tcx>,
                                        disr_val: ty::Disr,
                                        val: ValueRef)
                                        -> ExtractedBlock<'blk> {
    ...
    ExtractedBlock { vals: args, bcx: bcx.bl }
}
...
fn bind_subslice_pat(bcx: &mut Block,
                     pat_id: ast::NodeId,
                     val: ValueRef,
                     offset_left: usize,
    ...
    let ol = C_uint(bcx.ccx(), offset_left);
    let slice_begin = InBoundsGEP(bcx, base, &[ol]);
    ...
    let fpa = GEPi(bcx, scratch.val, &[0, abi::FAT_PTR_ADDR]);
    Store(bcx, slice_begin, fpa);
    let fpe = GEPi(bcx, scratch.val, &[0, abi::FAT_PTR_EXTRA]);
    Store(bcx, slice_len, fpe);
    scratch.val
}

fn extract_vec_elems<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                     left_ty: Ty<'tcx>,
                                     before: usize,
                                     after: usize,
                                     val: ValueRef)
    ...
    elems.extend((0..after).rev().map(|i| {
        let ty = C_uint(bcx.ccx(), i + 1);
        let sub = Sub(bcx, len, ty, DebugLoc::None);
        InBoundsGEP(bcx, base, &[sub])
    }));
    ExtractedBlock { vals: elems, bcx: bcx.bl }
}
...
impl FailureHandler {
    ...
    fn handle_fail(&self, bcx: &mut Block) {
        match *self {
            Infallible =>
...
fn compare_values<'r, 'blk, 'tcx>(cx: &mut Block<'r, 'blk, 'tcx>,
                                  lhs: ValueRef,
                                  rhs: ValueRef,
                                  rhs_t: Ty<'tcx>,
                                  debug_loc: DebugLoc)
                                  -> Result<'blk> {
    fn compare_str<'r, 'blk, 'tcx>(cx: &mut Block<'r, 'blk, 'tcx>,
                                   lhs: ValueRef,
                                   rhs: ValueRef,
                                   rhs_t: Ty<'tcx>,
                                   debug_loc: DebugLoc)
                                   -> Result<'blk> {
        let ty_to_string = cx.ty_to_string(rhs_t);
        let did = langcall(cx, None, &format!("comparison of `{}`", ty_to_string),
                           StrEqFnLangItem);
        ...
        call_lifetime_end(&mut res.bcx.with(cx.fcx), lhs_arg);
        call_lifetime_end(&mut res.bcx.with(cx.fcx), rhs_arg);
        ...
        ty::ty_rptr(_, mt) => match mt.ty.sty {
            ...
            ty::ty_vec(ty, _) => match ty.sty {
                ty::ty_uint(ast::TyU8) => {
                    ...
                    let rv = GEPi(cx, rhs, &[0, 0]);
                    let rp = expr::get_dataptr(cx, rhs_str);
                    Store(cx, rv, rp);
                    let ty = C_uint(cx.ccx(), pat_len);
                    let l = expr::get_len(cx, rhs_str);
                    Store(cx, ty, l);
                    ...
                    if val_ty(lhs) == val_ty(rhs) {
                        ...
                        let lv = GEPi(cx, lhs, &[0, 0]);
                        let lp = expr::get_dataptr(cx, lhs_str);
                        Store(cx, lv, lp);
                        let ty = C_uint(cx.ccx(), pat_len);
                        let l = expr::get_len(cx, lhs_str);
                        Store(cx, ty, l);
                    }
                    else {
...
fn insert_lllocals<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                   bindings_map: &BindingsMap<'tcx>,
    ...
    for (&ident, &binding_info) in bindings_map {
        let llval = match binding_info.trmode {
            ...
            TrByCopy(llbinding) => {
                ...
                bcx.bl = &mut datum.store_to(bcx, llbinding);
                if let Some(cs) = cs {
...
fn compile_guard<'a, 'p, 'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                         guard_expr: &ast::Expr,
    ...
    let mut bcx = &mut insert_lllocals(bcx, &data.bindings_map, None).with(bcx.fcx);
...
fn compile_submatch_continue<'a, 'p, 'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
    ...
    let adt_vals = if any_irrefutable_adt_pat(bcx.tcx(), m, col) {
        ...
        let (arg_count, struct_val) = if type_is_sized(bcx.tcx(), left_ty) {
            ...
            let dp = expr::get_dataptr(bcx, val);
            let ld = Load(bcx, dp);
            (arg_count - 1, ld)
        };
        ...
        match left_ty.sty {
            ty::ty_struct(def_id, substs) if !type_is_sized(bcx.tcx(), left_ty) => {
                ...
                let l = expr::get_len(bcx, val);
                let len = Load(bcx, l);
                let dp = expr::get_dataptr(bcx, scratch);
                let lp = expr::get_len(bcx, scratch);
                Store(bcx, data, dp);
                Store(bcx, len, lp);
    ...
    let defaults = enter_default(&mut else_cx.with(bcx.fcx), dm, m, col, val);
    ...
    for (i, opt) in opts.iter().enumerate() {
            ...
            match kind {
                ...
                Compare | CompareSliceLength => {
                    ...
                    let Result { bcx: after_cx, val: matches } = {
                        match opt.trans(bcx) {
                            SingleResult(Result { bcx: bl, val }) => {
                                compare_values(&mut bl.with(bcx.fcx), test_val, val, t, debug_loc)
                            }
                            RangeResult(Result { val: vbegin, .. },
                                        Result { bcx: bl, val: vend }) => {
                                let llge = compare_scalar_types(&mut bl.with(bcx.fcx),
                                                                test_val, vbegin,
                                                                t, ast::BiGe, debug_loc);
                                let llle = compare_scalar_types(&mut bl.with(bcx.fcx),
                                                                test_val, vend,
                                                                t, ast::BiLe, debug_loc);
                                Result::new(bcx.bl, And(bcx, llge, llle, DebugLoc::None))
                            }
                            LowerBound(Result { bcx: bl, val }) => {
                                Result::new(bcx.bl, compare_scalar_types(&mut bl.with(bcx.fcx),
                                                                         test_val,
                                                                         val, t, ast::BiGe,
                                                                         debug_loc))
                            }
                    ...
                    bcx.bl = bcx.fcx.new_temp_block("compare_next");
                    ...
                    if i + 1 < len && (guarded || multi_pats || kind == CompareSliceLength) {
                        branch_chk = Some(JumpToBasicBlock(bcx.bl.llbb));
                    }
                    CondBr(&mut after_cx.with(bcx.fcx), matches, opt_cx.llbb, bcx.bl.llbb,
                           debug_loc);
                }
                _ => ()
            }
        ...
        match *opt {
            Variant(disr_val, ref repr, _, _) => {
                let ExtractedBlock {vals: argvals, bcx: new_bcx} =
                    extract_variant_args(&mut opt_cx.with(bcx.fcx), &**repr, disr_val, val);
                size = argvals.len();
                unpacked = argvals;
                opt_cx = new_bcx;
            }
            SliceLengthEqual(len, _) => {
                let args = extract_vec_elems(&mut opt_cx.with(bcx.fcx), left_ty, len, 0, val);
                size = args.vals.len();
                unpacked = args.vals.clone();
                opt_cx = args.bcx;
            }
            SliceLengthGreaterOrEqual(before, after, _) => {
                let args = extract_vec_elems(&mut opt_cx.with(bcx.fcx),
                                             left_ty, before, after, val);
                size = args.vals.len();
                unpacked = args.vals.clone();
                opt_cx = args.bcx;
            }
        ...
        let opt_ms = enter_opt(&mut opt_cx.with(bcx.fcx), pat_id, dm, m, opt, col, size, val);
        let mut opt_vals = unpacked;
        opt_vals.push_all(&vals_left[..]);
        compile_submatch(&mut opt_cx.with(bcx.fcx),
                         &opt_ms[..],
                         &opt_vals[..],
    ...
    if !exhaustive && kind != Single {
        ...
        match chk {
            ...
            &JumpToBasicBlock(_) if defaults.len() == 1 && has_genuine_default => {
                chk.handle_fail(&mut else_cx.with(bcx.fcx));
            }
            _ => {
                compile_submatch(&mut else_cx.with(bcx.fcx),
                                 &defaults[..],
                                 &vals_left[..],
                                 chk,
                                 has_genuine_default);
...
pub fn trans_match<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                   match_expr: &ast::Expr,
                                   discr_expr: &ast::Expr,
                                   arms: &[ast::Arm],
                                   dest: Dest)
...
fn is_discr_reassigned(bcx: &mut Block, discr: &ast::Expr, body: &ast::Expr) -> bool {
...
fn create_bindings_map<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>, pat: &ast::Pat,
...
fn trans_match_inner<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                     match_id: ast::NodeId,
                                     discr_expr: &ast::Expr,
                                     arms: &[ast::Arm],
    ...
    let arm_datas: Vec<ArmData> = arms.iter().map(|arm| ArmData {
        bodycx: bcx.fcx.new_id_block("case_body", arm.body.id),
        arm: arm,
        bindings_map: create_bindings_map(bcx, &*arm.pats[0], discr_expr, &*arm.body)
    }).collect();

    let mut pat_renaming_map = if bcx.sess().opts.debuginfo != NoDebugInfo {
        Some(FnvHashMap())
    } else {
        None
    };
    ...
    let arm_pats: Vec<Vec<P<ast::Pat>>> = {
        let mut static_inliner = StaticInliner::new(bcx.tcx(),
    ...
    for arm_data in &arm_datas {
        let bcx = &mut arm_data.bodycx.with(bcx.fcx);

        // insert bindings into the lllocals map and add cleanups
        let cs = bcx.fcx.push_custom_cleanup_scope();
        bcx.bl = insert_lllocals(bcx, &arm_data.bindings_map, Some(cleanup::CustomScope(cs)));
        bcx.bl = expr::trans_into(bcx, &*arm_data.arm.body, dest);
        let bl = bcx.fcx.pop_and_trans_custom_cleanup_scope(bcx.bl, cs);
        arm_cxs.push(bl);
    }

    bcx.fcx.join_blocks(match_id, &arm_cxs[..])
}

/// Generates code for a local variable declaration like `let <pat>;` or `let <pat> =
/// <opt_init_expr>`.
pub fn store_local<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                   local: &ast::Local)
                                   -> &'blk BlockS {
    ...
    fn create_dummy_locals<'r, 'blk, 'tcx>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                           pat: &ast::Pat)
                                           -> &'blk BlockS {
        ...
        pat_bindings(&tcx.def_map, pat, |_, p_id, _, path1| {
            ...
            bl = mk_binding_alloca(
                &mut bl.with(bcx.fcx), p_id, path1.node.name, scope, (),
                |(), bcx, llval, ty| { drop_done_fill_mem(bcx, llval, ty); bcx.bl });
        });
        bl
    }
...
pub fn store_arg<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                 pat: &ast::Pat,
...
fn mk_binding_alloca<'r, 'blk, 'tcx, A, F>(bcx: &mut Block<'r, 'blk, 'tcx>,
                                           p_id: ast::NodeId,
                                           name: ast::Name,
                                           ...
                                           populate: F)
                                           -> &'blk BlockS where
    F: FnOnce(A, &mut Block<'r, 'blk, 'tcx>, ValueRef, Ty<'tcx>) -> &'blk BlockS,
{
    let var_ty = node_id_type(bcx, p_id);

    // Allocate memory on stack for the binding.
    let name = bcx.name(name);
    let llval = alloc_ty(bcx, var_ty, &name);

    // Subtle: be sure that we *populate* the memory *before*
    // we schedule the cleanup.
    let bcx = &mut populate(arg, bcx, llval, var_ty).with(bcx.fcx);
...
fn bind_irrefutable_pat<'r, 'blk, 'tcx>(&mut Block { bl, ref mut fcx }: &mut Block<'r, 'blk, 'tcx>,
                                        pat: &ast::Pat,
                                        val: ValueRef,
                                        cleanup_scope: cleanup::ScopeId)
                                        -> &'blk BlockS {
    ...
    if bcx.sess().asm_comments() {
        let r = pat.repr(bcx.tcx());
        add_comment(bcx, &format!("bind_irrefutable_pat(pat={})", r));
    }
    ...
    match pat.node {
        ast::PatIdent(pat_binding_mode, ref path1, ref inner) => {
            ...
            if let Some(ref inner_pat) = *inner {
                bcx.bl = bind_irrefutable_pat(bcx, &**inner_pat, val, cleanup_scope);
            }
        }
                ...
                Some(def::DefVariant(enum_id, var_id, _)) => {
                    ...
                    if let Some(ref sub_pat) = *sub_pats {
                        for (i, &argval) in args.vals.iter().enumerate() {
                            bcx.bl = bind_irrefutable_pat(bcx, &*sub_pat[i],
                        ...
                        Some(ref elems) => {
                            ...
                            for (i, elem) in elems.iter().enumerate() {
                                let fldptr = adt::trans_field_ptr(bcx, &*repr,
                                                                  val, 0, i);
                                bcx.bl = bind_irrefutable_pat(bcx, &**elem,
            ...
            expr::with_field_tys(tcx, pat_ty, Some(pat.id), |discr, field_tys| {
                for f in fields {
                    ...
                    let fldptr = adt::trans_field_ptr(bcx, &*pat_repr, val,
                                                      discr, ix);
                    bcx.bl = bind_irrefutable_pat(bcx, &*f.node.pat, fldptr, cleanup_scope);
                }
            })
        }
        ast::PatTup(ref elems) => {
            ...
            for (i, elem) in elems.iter().enumerate() {
                ...
                bcx.bl = bind_irrefutable_pat(bcx, &**elem, fldptr, cleanup_scope);
            }
        }
        ast::PatBox(ref inner) => {
            let llbox = Load(bcx, val);
            bcx.bl = bind_irrefutable_pat(bcx, &**inner, llbox, cleanup_scope);
        }
        ast::PatRegion(ref inner, _) => {
            let loaded_val = Load(bcx, val);
            bcx.bl = bind_irrefutable_pat(bcx, &**inner, loaded_val, cleanup_scope);
        }
        ast::PatVec(ref before, ref slice, ref after) => {
            ...
            bcx.bl = before
                ...
                .fold(bcx.bl, |bl, (inner, elem)|
                    bind_irrefutable_pat(&mut bl.with(bcx.fcx), &**inner, elem, cleanup_scope)
                );
        }
        ast::PatMac(..) => {
