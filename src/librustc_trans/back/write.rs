    ...
    let tm = unsafe {
        triple.with_c_str(|t| {
            let cpu = match sess.opts.cg.target_cpu {
                Some(ref s) => s.index(&FullRange),
                None => sess.target.target.options.cpu.index(&FullRange)
            };
            cpu.with_c_str(|cpu| {
...
unsafe fn optimize_and_codegen(cgcx: &CodegenContext,
    ...
    if config.emit_no_opt_bc {
        ...
        output_names.with_extension(ext.index(&FullRange)).with_c_str(|buf| {
            ...
            match cgcx.lto_ctxt {
                Some((sess, reachable)) if sess.lto() =>  {
                    ...
                    if config.emit_lto_bc {
                        ...
                        output_names.with_extension(name.index(&FullRange)).with_c_str(|buf| {
                            llvm::LLVMWriteBitcodeToFile(llmod, buf);
                        })
                    }
                },
    ...
    time(config.time_passes, "codegen passes", (), |()| {
        if config.emit_ir {
            ...
            output_names.with_extension(ext.index(&FullRange)).with_c_str(|output| {
