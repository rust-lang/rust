...
use macros::{InvocationData, LegacyScope, ParentScope};
use resolve_imports::ImportDirective;
use resolve_imports::ImportDirectiveSubclass::{self, GlobImport, SingleImport};
use Namespace::{self, MacroNS, TypeNS, ValueNS};
use {resolve_error, resolve_struct_error, ResolutionError};
use {ExternPreludeEntry, ModuleOrUniformRoot, PerNS, Resolver, ResolverArenas};
...
use syntax::feature_gate::{emit_feature_err, is_builtin_attr, GateIssue};
...
impl<'a> Resolver<'a> {
    ...
    fn build_reduced_graph_for_use_tree(
        ...
        let is_glob = if let ast::UseTreeKind::Glob = use_tree.kind {
            true
        } else {
            false
        };
        let crate_root = match prefix_iter.peek() {
            Some(seg)
                if !seg.ident.is_path_segment_keyword()
                    && seg.ident.span.rust_2015()
                    && self.session.rust_2015() =>
            {
            ...
            ast::UseTreeKind::Nested(ref items) => {
                ...
                if items.is_empty() && !empty_for_self(&prefix) {
                    ...
                    let tree = ast::UseTree {
                        prefix: ast::Path::from_ident(Ident::new(
                            keywords::SelfLower.name(),
                            new_span,
                        )),
    ...
    fn build_reduced_graph_for_foreign_item(&mut self, item: &ForeignItem, expansion: Mark) {
        let (def, ns) = match item.node {
            ...
            ForeignItemKind::Ty => (
                Def::ForeignTy(self.definitions.local_def_id(item.id)),
                TypeNS,
            ),
    ...
    pub fn get_macro(&mut self, def: Def) -> Lrc<SyntaxExtension> {
        let def_id = match def {
            ...
            Def::NonMacroAttr(attr_kind) => {
                return Lrc::new(SyntaxExtension::NonMacroAttr {
...
impl<'a, 'b> Visitor<'a> for BuildReducedGraphVisitor<'a, 'b> {
    ...
    fn visit_item(&mut self, item: &'a Item) {
        let macro_use = match item.node {
            ItemKind::MacroDef(..) => {
                self.resolver
                    .define_macro(item, self.expansion, &mut self.current_legacy_scope);
                return;
            }
            ItemKind::Mac(..) => {
        ...
        self.resolver
            .build_reduced_graph_for_item(item, parent_scope);
    ...
    fn visit_attribute(&mut self, attr: &'a ast::Attribute) {
        if !attr.is_sugared_doc && is_builtin_attr(attr) {
            ...
            parent_scope
                ...
                .builtin_attrs
                .borrow_mut()
                .push((attr.path.segments[0].ident, parent_scope));
        }
