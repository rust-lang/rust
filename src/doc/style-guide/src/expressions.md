...
- The body is a single expression or a block that contains only one expression.
  (The other conditions apply to that expression.)
- The expression is not a control-flow expression.
- The expression does not start with `{`. (A body that starts with `{` is always
  a block, never an expression.)
...
If the body is a block that contains only another block, combine the two blocks.
Write the two opening braces together on one line, as in `=> {{`. Block-indent
the contents of the inner block once. Write the two closing braces together on
one line, at the same indent as the arm. (The reason to keep the inner braces is
that the macro removes only the outer braces when it expands the arm.) For
example:
...
For the other rules in this section, `{{` is the opening brace and `}}` is the
closing brace. Never put the two opening braces on different lines. If the `=>`
and the two opening braces do not fit after the predicate, break the predicate,
as specified below.
