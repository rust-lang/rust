...
Freestanding/bare-metal x86 binaries in ELF format: firmware, kernels, etc.

## Target maintainers

[@alexhrao](https://github.com/alexhrao)
...
By default, code generated with this target should run on any `x86`
hardware; enabling additional target features may raise this baseline.

On `i686-unknown-none`, `extern "C"` uses the [standard System V i386 calling
convention](https://gitlab.com/x86-psABIs/i386-ABI), without red zones.
...
As `i686-unknown-none` supports a variety of different environments and does
...
dependencies), you will need an appropriate `i686` toolchain.

Rust *may* be able to use an `i686-linux-gnu-` toolchain with appropriate
...
`i686-unknown-none` (or `i686-elf-`) toolchain.

On some `x86` hosts that use ELF binaries, you *may* be able to use the host
C toolchain, if it does not introduce assumptions about the host environment
that don't match the expectations of a standalone environment. Otherwise, you
may need a separate toolchain for standalone/freestanding development, just as
when cross-compiling from a non-`x86` platform.
