...
```{rust}
let greater_than_forty_two = range(0i32, 100)
                             .find(|x| *x > 42);
...
```{rust,ignore}
range(1i32, 100).map(|x| println!("{}", x));
```
...
```{rust}
for i in std::iter::count(1i32, 5).take(5) {
    println!("{}", i);
}
```
...
```{rust}
for i in range(1i32, 100).filter(|&x| x % 2 == 0) {
    println!("{}", i);
}
```
