...
pub struct DiagnosticHandlers<'a, B : WriteBackendMethods> {
    pub data: *mut (&'a CodegenContext<B>, &'a Handler),
    pub llcx: &'a B::Context,
}

impl<'a, B : WriteBackendMethods> Drop for DiagnosticHandlers<'a, B> {
    fn drop(&mut self) {
        B::drop_diagnostic_handlers(self);
    }
}
...
pub fn start_async_codegen<B : ExtraBackendMethods>(
    ...
    let linker_info = LinkerInfo::new(tcx, backend.clone());
...
pub fn dump_incremental_data<B : ExtraBackendMethods>(
    _codegen_results: &CodegenResults<B>
) {
...
fn start_executing_work<B : ExtraBackendMethods>(
    ...
    let helper = jobserver.into_helper_thread(move |token| {
        let msg : Message<B> = Message::Token(token);
        drop(coordinator_send2.send(Box::new(msg)));
    ...
    let cgcx : CodegenContext<B> = CodegenContext {
...
impl<B : ExtraBackendMethods> Drop for Bomb<B> {
    fn drop(&mut self) {
        ...
        let msg : Message<B> = match self.result.take() {
...
impl<B : ExtraBackendMethods> OngoingCodegen<B> {
    pub fn join(
        self,
        sess: &Session
    ) -> (CodegenResults<B>, FxHashMap<WorkProductId, WorkProduct>) {
...
pub fn submit_codegened_module_to_llvm<B : ExtraBackendMethods>(
    ...
    let msg : Message<B> = Message::CodegenDone {
        llvm_work_item,
        cost,
    };
    drop(tcx.tx_to_llvm_workers.lock().send(Box::new(msg)));
}
...
pub fn submit_pre_lto_module_to_llvm<B : ExtraBackendMethods>(
    ...
    let msg : Message<B> = Message::AddImportOnlyModule {
