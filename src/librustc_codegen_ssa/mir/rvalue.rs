    ...
    pub fn codegen_rvalue(
        ...
        match *rvalue {
            ...
            mir::Rvalue::Repeat(ref elem, count) => {
                ...
                let align = dest
                    .align
                    .restrict_for_offset(dest.layout.field(bx.cx(), 0).size);
                cg_elem.val.store(
                    &mut body_bx,
            ...
            mir::Rvalue::Aggregate(ref kind, ref operands) => {
                let (dest, active_field_index) = match **kind {
                    mir::AggregateKind::Adt(adt_def, variant_index, _, _, active_field_index) => {
                        ...
                        if adt_def.is_enum() {
                            (
                                dest.project_downcast(&mut bx, variant_index),
                                active_field_index,
                            )
                        } else {
...
fn cast_int_to_float<'a, 'tcx: 'a, Bx: BuilderMethods<'a, 'tcx>>(
    ...
    let is_u128_to_f32 =
        !signed && bx.cx().int_width(int_ty) == 128 && bx.cx().float_width(float_ty) == 32;
    if is_u128_to_f32 {
