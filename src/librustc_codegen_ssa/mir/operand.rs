...
impl<'a, 'tcx: 'a, V: CodegenObject> OperandRef<'tcx, V> {
    ...
    pub fn from_const<Bx: BuilderMethods<'a, 'tcx, Value = V>>(
        ...
        let val = match val.val {
            ...
            ConstValue::Scalar(x) => {
                ...
                let llval =
                    bx.cx()
                        .scalar_to_backend(x, scalar, bx.cx().immediate_backend_type(layout));
    ...
    pub fn deref<Cx: CodegenMethods<'tcx, Value = V>>(self, cx: &Cx) -> PlaceRef<'tcx, V> {
        ...
        let mut val = match (self.val, &self.layout.abi) {
            ...
            (OperandValue::Pair(a_llval, b_llval), &layout::Abi::ScalarPair(ref a, ref b)) => {
                if offset.bytes() == 0 {
                    ...
                    assert_eq!(
                        offset,
                        a.value.size(bx.cx()).align_to(b.value.align(bx.cx()).abi)
                    );
...
impl<'a, 'tcx: 'a, V: CodegenObject> OperandValue<V> {
    ...
    fn store_with_flags<Bx: BuilderMethods<'a, 'tcx, Value = V>>(
        ...
        match self {
            OperandValue::Ref(r, None, source_align) => base::memcpy_ty(
                bx,
                dest.llval,
                dest.align,
                r,
            ...
            OperandValue::Pair(a, b) => {
                ...
                let b_offset = a_scalar
                    .value
                    .size(bx)
                    .align_to(b_scalar.value.align(bx).abi);
                ...
                self.eval_mir_constant(bx, constant)
                    ...
                    .unwrap_or_else(|err| {
                        ...
                        bx.load_operand(PlaceRef::new_sized(
                            bx.cx()
                                .const_undef(bx.cx().type_ptr_to(bx.cx().backend_type(layout))),
                            layout,
                            layout.align.abi,
                        ))
                    })
