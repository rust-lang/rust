...
impl<'a, 'tcx: 'a, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn codegen_terminator(
        ...
        ) -> impl for<'b> Fn(&'b FunctionCx<'a, 'tcx, Bx>) -> Option<&'b Bx::Funclet> {
            move |this| match funclet_bb {
        ...
        let funclet_br = |this: &mut Self, bx: &mut Bx, target: mir::BasicBlock| {
            let (lltarget, is_cleanupret) = lltarget(this, target);
            if is_cleanupret {
        ...
        match terminator.kind {
            ...
            mir::TerminatorKind::Return => {
                let llval = match self.fn_ty.ret.mode {
                    ...
                    PassMode::Cast(cast_ty) => {
                        ...
                        let addr =
                            bx.pointercast(llslot, bx.type_ptr_to(bx.cast_backend_type(&cast_ty)));
            ...
            mir::TerminatorKind::Drop {
                ...
                let (drop_fn, fn_ty) = match ty.sty {
                    ...
                    _ => (bx.get_fn(drop_fn), bx.fn_type_of_instance(&drop_fn)),
                };
                do_call(
                    self,
                    &mut bx,
            ...
            mir::TerminatorKind::Assert {
                ...
                let (lang_item, args) = match *msg {
                    EvalErrorKind::BoundsCheck { ref len, ref index } => {
                        ...
                        let file_line_col =
                            bx.static_addr_of(file_line_col, align, Some("panic_bounds_check_loc"));
                        (
                    ...
                    _ => {
                        ...
                        let msg_file_line_col =
                            bx.const_struct(&[msg_str, filename, line, col], false);
                        let msg_file_line_col =
                            bx.static_addr_of(msg_file_line_col, align, Some("panic_loc"));
            ...
            mir::TerminatorKind::Call {
                ...
                let fn_ty = match def {
                    Some(ty::InstanceDef::Virtual(..)) => bx.new_vtable(sig, &extra_args),
                    ...
                    && fn_ty.ret.layout.abi.is_uninhabited()
                {
                    ...
                    let msg_str = Symbol::intern(&str).as_str();
                    let msg_str = bx.const_str_slice(msg_str);
                    let msg_file_line_col = bx.const_struct(&[msg_str, filename, line, col], false);
                    let msg_file_line_col =
                        bx.static_addr_of(msg_file_line_col, align, Some("panic_loc"));
                    ...
                    do_call(
                        ...
                        destination
                            .as_ref()
                            .map(|(_, bb)| (ReturnDest::Nothing, *bb)),
                        cleanup,
                    );
                    return;
                }
                    ...
                    if let (0, Some(ty::InstanceDef::Virtual(_, idx))) = (i, def) {
                        if let Pair(..) = op.val {
                            ...
                            match op.val {
                                Pair(data_ptr, meta) => {
                                    llfn = Some(
                                        meth::VirtualIndex::from_index(idx)
                            ...
                            llfn = Some(
                                meth::VirtualIndex::from_index(idx).get_fn(&mut bx, meta, &fn_ty),
                            );
                            llargs.push(data_ptr);
                            continue;
                        } else {
    ...
    fn codegen_argument(
        ...
        if let PassMode::Pair(..) = arg.mode {
            ...
            match op.val {
                ...
                _ => bug!(
                    "codegen_argument: {:?} invalid for unsized indirect argument",
                    op
                ),
            }
        }
