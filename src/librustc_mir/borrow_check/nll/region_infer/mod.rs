...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    fn check_universal_region(
        ...
        debug!("check_universal_region(fr={:?})", fr,);

        // Find every region `o` such that `fr: o`
        // (because `fr` includes `end(o)`).
        for outlived_fr in inferred_values.universal_regions_outlived_by(fr) {
            // If it is known that `fr: o`, carry on.
            if self.universal_regions.outlives(fr, outlived_fr) {
                continue;
            }

            debug!(
                "check_universal_region: fr={:?} does not outlive outlived_fr={:?}",
                fr,
                outlived_fr,
            );
            ...
            if let Some(fr_minus) = self.universal_regions.non_local_lower_bound(fr) {
                debug!("check_universal_region: fr_minus={:?}", fr_minus);

                // Grow `outlived_fr` until we find a non-local
                // regon. (We always will.)  We'll call that
                // `outlived_fr+` -- it's ever so slightly larger than
                // `fr`.
                let outlived_fr_plus = self.universal_regions.non_local_upper_bound(outlived_fr);
                debug!(
                    "check_universal_region: outlived_fr_plus={:?}",
                    outlived_fr_plus
                );

                // Push the constraint `fr-: outlived_fr+`
                outlives_requirements.push(ClosureOutlivesRequirement {
                    free_region: fr_minus,
                    outlived_free_region: outlived_fr_plus,
            ...
            self.report_error(infcx, fr, outlived_fr, blame_span);
        }
    }
