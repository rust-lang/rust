...
use rustc::mir::interpret::{ErrorHandled, GlobalId};
    ...
    let kind = match expr.node {
        ...
        hir::ExprKind::Call(ref fun, ref args) => {
            if cx.tables().is_method_call(expr) {
                ...
                let adt_data =
                    if let hir::ExprKind::Path(hir::QPath::Resolved(_, ref path)) = fun.node {
        ...
        hir::ExprKind::Assign(ref lhs, ref rhs) => ExprKind::Assign {
            lhs: lhs.to_ref(),
            rhs: rhs.to_ref(),
        },
            ...
            if cx.tables().is_method_call(expr) {
                ...
                match (op.node, cx.constness) {
                    ...
                    (hir::BinOpKind::And, hir::Constness::Const) => {
                        cx.control_flow_destroyed
                            .push((op.span, "`&&` operator".into()));
                        ExprKind::Binary {
                            op: BinOp::BitAnd,
                            lhs: lhs.to_ref(),
                            rhs: rhs.to_ref(),
                    ...
                    (hir::BinOpKind::And, hir::Constness::NotConst) => ExprKind::LogicalOp {
                        op: LogicalOp::And,
                        lhs: lhs.to_ref(),
                        rhs: rhs.to_ref(),
                    },
                    (hir::BinOpKind::Or, hir::Constness::NotConst) => ExprKind::LogicalOp {
                        op: LogicalOp::Or,
                        lhs: lhs.to_ref(),
                        rhs: rhs.to_ref(),
                    },
        ...
        hir::ExprKind::Struct(ref qpath, ref fields, ref base) => match expr_ty.sty {
        ...
        hir::ExprKind::InlineAsm(ref asm, ref outputs, ref inputs) => ExprKind::InlineAsm {
            asm,
        ...
        hir::ExprKind::If(ref cond, ref then, ref otherwise) => ExprKind::If {
            condition: cond.to_ref(),
            then: then.to_ref(),
            otherwise: otherwise.to_ref(),
        },
        hir::ExprKind::While(ref cond, ref body, _) => ExprKind::Loop {
            condition: Some(cond.to_ref()),
            body: block::to_expr_ref(cx, body),
        },
        hir::ExprKind::Loop(ref body, _, _) => ExprKind::Loop {
            condition: None,
            body: block::to_expr_ref(cx, body),
        },
        ...
        hir::ExprKind::Cast(ref source, ref cast_ty) => {
            ...
            let cast = if let Some(&TyCastKind::CoercionCast) =
                cx.tables().cast_kinds().get(source.hir_id)
            {
                ...
                let source = if let Some((did, offset, var_ty)) = var {
                    ...
                    match did {
                        Some(did) => {
                            ...
                            let lhs =
                                mk_const(ty::Const::unevaluated(cx.tcx(), did, substs, var_ty));
        ...
        hir::ExprKind::Box(ref value) => ExprKind::Box {
            value: value.to_ref(),
        },
...
fn user_substs_applied_to_def(
    ...
    let user_provided_type = match def {
        ...
        // user.
        Def::StructCtor(_def_id, CtorKind::Const) | Def::VariantCtor(_def_id, CtorKind::Const) => {
            cx.user_substs_applied_to_ty_of_hir_id(hir_id)
        }

        // `Self` is used in expression as a tuple struct constructor or an unit struct constructor
        Def::SelfCtor(_) => cx.user_substs_applied_to_ty_of_hir_id(hir_id),

        _ => bug!(
            "user_substs_applied_to_def: unexpected def {:?} at {:?}",
            def,
            hir_id
        ),
    };
    debug!(
        "user_substs_applied_to_def: user_provided_type={:?}",
        user_provided_type
    );
    user_provided_type
}
...
fn convert_path_expr<'a, 'gcx, 'tcx>(
    ...
    match def {
        ...
        | Def::SelfCtor(..) => {
            ...
            ExprKind::Literal {
                literal: ty::Const::zero_sized(cx.tcx, cx.tables().node_id_to_type(expr.hir_id)),
                user_ty,
            }
        }
        ...
        Def::StructCtor(def_id, CtorKind::Const) | Def::VariantCtor(def_id, CtorKind::Const) => {
            ...
            debug!(
                "convert_path_expr: user_provided_type={:?}",
                user_provided_type
            );
