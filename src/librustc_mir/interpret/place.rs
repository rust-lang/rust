...
use rustc_target::abi::{Abi, Align, DiscriminantKind, FieldPlacement};
    ...
    pub(super) fn len(self, cx: &impl HasDataLayout) -> InterpResult<'tcx, u64> {
        if self.layout.is_unsized() {
            ...
            match self.layout.fields {
                FieldPlacement::Array { count, .. } => Ok(count),
    ...
    M::AllocExtra: AllocationExtra<Tag>,
{
    ...
    ) -> InterpResult<'tcx, impl Iterator<Item = InterpResult<'tcx, MPlaceTy<'tcx, Tag>>> + 'tcx>
    {
        ...
        let stride = match base.layout.fields {
            FieldPlacement::Array { stride, .. } => stride,
    ...
    fn mplace_subslice(
        ...
        let from_offset = match base.layout.fields {
            FieldPlacement::Array { stride, .. } => stride * from, // `Size` multiplication is checked
