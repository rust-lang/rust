...
use rustc::mir::interpret::{AllocKind, EvalErrorKind, EvalResult, Scalar};
use rustc::ty;
use rustc::ty::layout::{self, Align, LayoutOf, Size, TyLayout, VariantIdx};
use rustc_data_structures::fx::FxHashSet;
use syntax_pos::symbol::Symbol;

use super::{EvalContext, Machine, OpTy, ValueVisitor};
...
impl<'rt, 'a, 'mir, 'tcx, M: Machine<'a, 'mir, 'tcx>> ValueVisitor<'a, 'mir, 'tcx, M>
    for ValidityVisitor<'rt, 'a, 'mir, 'tcx, M>
{
    ...
    fn visit_variant(
        ...
        let name = old_op.layout.ty.ty_adt_def().unwrap().variants[variant_id]
            .ident
            .name;
        ...
        match self.walk_value(op) {
            ...
            Err(err) => match err.kind {
                EvalErrorKind::InvalidDiscriminant(val) => {
                    validation_failure!(val, self.path, "a valid enum discriminant")
                }
                EvalErrorKind::ReadPointerAsBytes => {
                    validation_failure!("a pointer", self.path, "plain (non-pointer) bytes")
                }
                _ => Err(err),
            },
        }
        ...
        match ty.sty {
            ...
            ty::RawPtr(..) => {
                if self.const_mode {
                    ...
                    // accept undef.
                    let _ptr = try_validation!(
                        value.to_scalar_ptr(),
                        "undefined address in raw pointer",
                        self.path
                    );
                    let _meta = try_validation!(
                        value.to_meta(),
                        "uninitialized data in raw fat pointer metadata",
                        self.path
                    );
                } else {
            ...
            _ if ty.is_box() || ty.is_region_ptr() => {
                ...
                let ptr = try_validation!(
                    value.to_scalar_ptr(),
                    "undefined address in pointer",
                    self.path
                );
                let meta = try_validation!(
                    value.to_meta(),
                    "uninitialized data in fat pointer metadata",
                    self.path
                );
                let layout = self
                    .ecx
                    .layout_of(value.layout.ty.builtin_deref(true).unwrap().ty)?;
                ...
                if let Some(ref mut ref_tracking) = self.ref_tracking {
                    assert!(
                        self.const_mode,
                        "We should only do recursie checking in const mode"
                    );
            ...
            ty::FnPtr(_sig) => {
                let value = value.to_scalar_or_undef();
                let ptr = try_validation!(value.to_ptr(), value, self.path, "a pointer");
            ...
            Scalar::Ptr(ptr) => {
                if lo == 1 && hi == max_hi {
                    ...
                    let non_null = self
                        .ecx
                        .memory
                        .check_align(Scalar::Ptr(ptr), Align::from_bytes(1).unwrap())
                        .is_ok()
                    ...
                    Err(err) => {
                        ...
                        match err.kind {
                            EvalErrorKind::ReadUndefBytes(offset) => {
                                ...
                                return validation_failure!("undefined bytes", self.path);
                            }
