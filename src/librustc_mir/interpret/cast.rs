...
use rustc::mir::interpret::{
    truncate, EvalErrorKind, EvalResult, Pointer, PointerArithmetic, Scalar,
};
use rustc::mir::CastKind;
use rustc_apfloat::ieee::{Double, Single};
use rustc_apfloat::Float;

use super::{EvalContext, Immediate, Machine, OpTy, PlaceTy};

impl<'a, 'mir, 'tcx, M: Machine<'a, 'mir, 'tcx>> EvalContext<'a, 'mir, 'tcx, M> {
    fn type_is_fat_ptr(&self, ty: Ty<'tcx>) -> bool {
        match ty.sty {
            ty::RawPtr(ty::TypeAndMut { ty, .. }) | ty::Ref(_, ty, _) => !self.type_is_sized(ty),
                    ...
                    match src.layout.variants {
                        layout::Variants::Single { index } => {
                            if let Some(def) = src.layout.ty.ty_adt_def() {
                                ...
                                let discr_val = def.discriminant_for_variant(*self.tcx, index).val;
        ...
        match val {
            ...
            Scalar::Bits { bits, size } => {
                ...
                debug_assert_eq!(
                    ...
                    bits,
                    "Unexpected value of size {} before casting",
                    size
                );
                ...
                match res {
                    ...
                    Scalar::Bits { bits, size } => {
                        ...
                        debug_assert_eq!(
                            truncate(bits, Size::from_bytes(size.into())),
                            bits,
                            "Unexpected value of size {} after casting",
                            size
                        );
                    }
                }
    ...
    fn cast_from_float(
        ...
        match dest_ty.sty {
            ...
            Uint(t) => {
                let width = t
                    .bit_width()
                    .unwrap_or_else(|| self.pointer_size().bits() as usize);
                let v = match fty {
            ...
            // f64 -> f32
            Float(FloatTy::F32) if fty == FloatTy::F64 => Ok(Scalar::from_uint(
                Single::to_bits(Double::from_bits(bits).convert(&mut false).value),
                Size::from_bits(32),
            )),
            // f32 -> f64
            Float(FloatTy::F64) if fty == FloatTy::F32 => Ok(Scalar::from_uint(
    ...
    fn cast_from_ptr(
        ...
        match ty.sty {
            ...
            RawPtr(_) | Int(IntTy::Isize) | Uint(UintTy::Usize) => Ok(ptr.into()),
    ...
    fn unsize_into_ptr(
        ...
        match (&src_pointee_ty.sty, &dest_pointee_ty.sty) {
            (&ty::Array(_, length), &ty::Slice(_)) => {
                ...
                let val = Immediate::new_slice(ptr, length.unwrap_usize(self.tcx.tcx), self);
                self.write_immediate(val, dest)
            }
