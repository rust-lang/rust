...
pub fn mir_build<'a, 'tcx>(tcx: TyCtxt<'a, 'tcx, 'tcx>, def_id: DefId) -> Mir<'tcx> {
    ...
    tcx.infer_ctxt().enter(|infcx| {
        ...
        let mut mir = if cx.tables().tainted_by_errors {
            ...
            let fn_def_id = tcx.hir().local_def_id_from_hir_id(fn_hir_id);
            ...
            let implicit_argument = match ty.sty {
                ...
                ty::Generator(..) => {
                    let gen_ty = tcx.body_tables(body_id).hir_id_to_type(fn_hir_id);
                                     ...
                                     -> Mir<'tcx>
{
    ...
    if let hir::VariantData::Tuple(ref fields, hir_id) = *v {
        tcx.infer_ctxt().enter(|infcx| {
            let mut mir = shim::build_adt_ctor(&infcx, hir_id, fields, span);
            ...
            mir_util::dump_mir(tcx, None, "mir_map", &0,
                               MirSource::item(tcx.hir().local_def_id_from_hir_id(hir_id)),
                               &mir, |_, _| Ok(()) );
...
fn liberated_closure_env_ty<'a, 'gcx, 'tcx>(tcx: TyCtxt<'a, 'gcx, 'tcx>,
                                            ...
                                            body_id: hir::BodyId)
                                            -> Ty<'tcx> {
    let closure_ty = tcx.body_tables(body_id).hir_id_to_type(closure_expr_hir_id);
    ...
    where A: Iterator<Item=ArgInfo<'gcx>>
{
    ...
    let closure_expr_id = tcx_hir.local_def_id_from_hir_id(fn_id);
