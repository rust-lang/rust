...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
    ...
    fn expr_as_rvalue(
        ...
        match expr.kind {
            ...
            ExprKind::Box { value } => {
                ...
                unpack!(block = this.into(&Place::local(result).deref(tcx), block, value));
    ...
    fn limit_capture_mutability(
        ...
        let mutability = match arg_place.split_projection(tcx) {
            (base_place, Some(projection)) => {
                match projection {
                    ProjectionElem::Deref => {
                        match base_place.split_projection(tcx) {
                            (base_place, None) => {
                                if let PlaceBase::Local(local) = base_place.base {
                                    debug_assert!(
                                        if let Some(ClearCrossCrate::Set(
                                    ...
                                    ProjectionElem::Field(upvar_index, _) => {
                                        ...
                                        debug_assert!(
                                            if base_place.has_no_projection() {
                                                ...
                                                if base_place.elems.len() == 1 &&
                                                    base_place.elems[0] == ProjectionElem::Deref {
                                                    base_place.base ==
                                                        PlaceBase::Local(Local::new(1))
                                                } else {
                                                    false
                                                }
                                            }
                    ...
                    ProjectionElem::Field(upvar_index, _) => {
                        debug_assert!(
                            if base_place.has_no_projection() {
                                ...
                                if base_place.elems.len() == 1 &&
                                    base_place.elems[0] == ProjectionElem::Deref {
                                        base_place.base == PlaceBase::Local(Local::new(1))
                                    } else {
                                        false
                                }
                            }
