...
pub trait NonConstOp: std::fmt::Debug {
    ...
    /// Returns the symbol for a feature gate that would allow this operation,
    /// or `None` if no such a feature gate exists.
    ...
    fn is_allowed_in_item(&self, item: &Item<'_, '_>) -> bool {
        Self::feature_gate()
            .map(|feature| item.tcx.features().on(feature))
            .unwrap_or(false)
    }
...
impl NonConstOp for UnionAccess {
    fn is_allowed_in_item(&self, item: &Item<'_, '_>) -> bool {
        ...
        || item.tcx.features().on(Self::feature_gate().unwrap())
    }
