...
pub(crate) mod indexes {
    ...
    macro_rules! new_index {
        ($Index:ident, $debug_name:expr) => {
            ...
            pub(crate) struct $Index(NonZero<usize>);
...
pub(crate) use self::indexes::MovePathIndex;
pub(crate) use self::indexes::MoveOutIndex;

impl MoveOutIndex {
    pub(crate) fn move_path_index(&self, move_data: &MoveData) -> MovePathIndex {
...
pub(crate) struct MoveData<'tcx> {
    pub(crate) move_paths: IndexVec<MovePathIndex, MovePath<'tcx>>,
    pub(crate) moves: IndexVec<MoveOutIndex, MoveOut>,
    ...
    pub(crate) loc_map: LocationMap<Vec<MoveOutIndex>>,
    pub(crate) path_map: IndexVec<MovePathIndex, Vec<MoveOutIndex>>,
    pub(crate) rev_lookup: MovePathLookup<'tcx>,
}

pub(crate) trait HasMoveData<'tcx> {
    fn move_data(&self) -> &MoveData<'tcx>;
}

#[derive(Debug)]
pub(crate) struct LocationMap<T> {
...
#[derive(Debug)]
pub(crate) struct MovePathLookup<'tcx> {
...
impl<'tcx> MovePathLookup<'tcx> {
    ...
    // parent.
    pub(crate) fn find(&self, lval: &Lvalue<'tcx>) -> LookupResult {
        match *lval {
