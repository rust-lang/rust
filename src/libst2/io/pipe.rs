...
impl PipeStream {
    ...
    /// }
    /// ```
    pub fn open(fd: libc::c_int) -> IoResult<PipeStream> { unimplemented!() }

    // FIXME: expose this some other way
    /// Wrap a FileDesc directly, taking ownership.
    #[doc(hidden)]
    pub fn from_filedesc(fd: FileDesc) -> PipeStream { unimplemented!() }
    ...
    pub fn pair() -> IoResult<PipePair> { unimplemented!() }
}

impl sys_common::AsFileDesc for PipeStream {
    fn as_fd(&self) -> &sys::fs::FileDesc { unimplemented!() }
}

impl Clone for PipeStream {
    fn clone(&self) -> PipeStream { unimplemented!() }
}
