...
impl<T> Key<T> {
    ...
    /// });
    /// ```
    pub fn set<R>(&'static self, t: &T, cb: || -> R) -> R { unimplemented!() }
    ...
    /// });
    /// ```
    pub fn with<R>(&'static self, cb: |&T| -> R) -> R { unimplemented!() }

    /// Test whether this TLS key has been `set` for the current thread.
    pub fn is_set(&'static self) -> bool { unimplemented!() }
}
...
mod imp {
    ...
    impl<T> KeyInner<T> {
        #[doc(hidden)]
        pub unsafe fn set(&self, ptr: *mut T) { unimplemented!() }
        #[doc(hidden)]
        pub unsafe fn get(&self) -> *mut T { unimplemented!() }
    }
}
