...
impl<T, E: ToStr> Result<T, E> {
    ...
    pub fn get(self) -> T {
        match self {
            Ok(t) => t,
            Err(e) => fail!("called `Result::get()` on `Err` value: %s", e.to_str()),
        }
    }
    ...
    pub fn get_err(self) -> E {
        self.expect_err("called `Result::get_err()` on `Ok` value")
    }
...
///     }
#[inline]
pub fn map_vec<T, U: ToStr, V>(ts: &[T], op: &fn(&T) -> Result<V,U>) -> Result<~[V],U> {
