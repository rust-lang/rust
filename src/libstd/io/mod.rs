...
//! errors. Errors should be painfully visible when they happen, and handling
//! them should be easy to work with. It should be convenient to handle specific
//! I/O errors, and it should also be convenient to not deal with I/O errors.
//!
...
//! required ultimately required for types to implement `write_line`) there is
//! no need to inspect or unwrap the `IoResult<File>` and we simply call
//! `write_line` on it. If `new` returned an `Err(..)` then the followup call to
//! `write_line` will also return an error.
//!
//! ## `try!`
//!
...
//! alleviated with extra methods implemented on `IoResult`, but others have
//! more complex interdependencies among each I/O operation.
//!
