...
enum StdSource {
    TTY(Box<RtioTTY + Send + 'static>),
    File(Box<RtioFileStream + Send + 'static>),
}
...
local_data_key!(local_stdout: Box<Writer + Send + 'static>)
...
pub fn set_stdout(stdout: Box<Writer + Send + 'static>)
                  -> Option<Box<Writer + Send + 'static>> {
...
pub fn set_stderr(stderr: Box<Writer + Send + 'static>)
                  -> Option<Box<Writer + Send + 'static>> {
...
fn with_task_stdout(f: |&mut Writer| -> IoResult<()>) {
    let result = if Local::exists(None::<Task>) {
        let mut my_stdout = local_stdout.replace(None).unwrap_or_else(|| {
            box stdout() as Box<Writer + Send + 'static>
        });
