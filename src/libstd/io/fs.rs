...
//! associated with various filesystem operations. They all operate on `Path`
//! objects.
//!
//! All operations in this module, including those as part of `File` et al
//! block the task during execution. In the event of failure, all
//! functions/methods will return an `IoResult` type with an `Err` value.
//!
