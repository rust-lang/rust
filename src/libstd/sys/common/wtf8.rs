...
impl<R: ops::RangeArgument<usize>> ops::Index<R> for Wtf8 {
    type Output = Wtf8;

    #[inline]
    fn index(&self, index: R) -> &Wtf8 {
        let start = *index.start().unwrap_or(&0);
        let end = *index.end().unwrap_or(&self.len());
        // is_code_point_boundary checks that the index is in [0, .len()]
        if start <= end &&
           is_code_point_boundary(self, start) &&
           is_code_point_boundary(self, end) {
            unsafe { slice_unchecked(self, start, end) }
        } else {
