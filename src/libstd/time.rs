...
native mod rustrt {
    ...
    fn rust_gmtime(&&sec: i64, &&usec: i32, &&result: tm);
    fn rust_localtime(&&sec: i64, &&usec: i32, &result: tm);
    fn rust_timezone(&&gmtoff: c_long) -> str;
...
fn at_utc(clock: timeval) -> tm {
    let mut {sec, usec} = clock;
    let mut tm = empty_tm();
    rustrt::rust_gmtime(sec, usec, tm);
    tm
}
...
fn at(clock: timeval) -> tm {
    let mut {sec, usec} = clock;
    let mut tm = empty_tm();
    rustrt::rust_localtime(sec, usec, tm);
    tm
}
...
fn timezone() -> (i32, str) {
    let mut gmtoff = 0 as c_long;
    let zone = rustrt::rust_timezone(gmtoff);
    (gmtoff as i32, zone)
}

fn timegm(tm: tm) -> timeval {
    let mut sec = 0i64;
    rustrt::rust_timegm(tm, sec);
    {
        sec: sec,
        usec: tm.tm_usec
    }
...
fn mktime(tm: tm) -> timeval {
    let mut sec = 0i64;
    rustrt::rust_mktime(tm, sec);
    {
        sec: sec,
        usec: tm.tm_usec
    }
...
fn parse(s: str, format: str) -> result<tm, str> {
    type tm_mut = {
       mut tm_sec: i32,
       mut tm_min: i32,
       mut tm_hour: i32,
    ...
    io::with_str_reader(format) { |rdr|
        ...
        let mut pos = 0u;
        let len = str::len(s);
        let mut result = err("Invalid format");
...
fn strftime(format: str, tm: tm) -> str {
    fn parse_type(ch: char, tm: tm) -> str {
        ...
        alt check ch {
          ...
          's' { #fmt("%d", tm.to_timeval().sec as int) }
          'T' | 'X' {
            #fmt("%s:%s:%s",
                parse_type('H', tm),
                parse_type('M', tm),
...
impl tm for tm {
    ...
    fn to_timeval() -> timeval {
        if self.tm_gmtoff == 0_i32 { timegm(self) } else { mktime(self) }
    }
    ...
    "]
    fn ctime() -> str { self.format("%c") }

    #[doc = "Formats the time according to the format string."]
    fn format(format: str) -> str { strftime(format, self) }
    ...
    fn rfc822() -> str {
        if self.tm_gmtoff == 0_i32 {
            self.format("%a, %d %b %Y %T GMT")
        } else {
            self.format("%a, %d %b %Y %T %Z")
        }
    }
    ...
    fn rfc3339() -> str {
        if self.tm_gmtoff == 0_i32 {
            ...
            let s = self.format("%Y-%m-%dT%H:%M:%S");
...
mod tests {
    ...
    fn test_at_utc() {
        ...
        let time = { sec: 1234567890_i64, usec: 54321_i32 };
        let utc = at_utc(time);
    ...
    fn test_at() {
        ...
        #error("test_at local.tm_zone: %?", local.tm_zone);
        //assert local.tm_zone == "PST";
        assert local.tm_usec == 54321_i32;
    }
    ...
    fn test_to_timeval() {
        ...
        assert utc.to_timeval() == time;
        assert utc.to_local().to_timeval() == time;
    }
    ...
    fn test_conversions() {
        ...
        #error("test_conversions %? %?",
                sys::size_of::<tm>(),
                sys::align_of::<tm>());

        #error("local.to_timeval %?", local.to_timeval());

        #error("local:                     %?", local);
        #error("local.to_local():          %?", local.to_local());
        #error("local.to_utc():            %?", local.to_utc());
        #error("local.to_utc().to_local(): %?", local.to_utc().to_local());
        #error("utc:                       %?", utc);
        #error("utc.to_utc():              %?", utc.to_utc());
        #error("utc.to_local():            %?", utc.to_local());
        #error("utc.to_local().to_utc():   %?", utc.to_local().to_utc());
    ...
    fn test_parse() {
        ...
        alt parse("", "") {
          ok(tm) {
            ...
            assert tm.tm_zone == "";
            assert tm.tm_usec as i32 == 0_i32;
          }
          err(_) {}
        }

        let format = "%a %b %e %T %Y";
        assert parse("", format) == err("Invalid format");
        assert parse("Fri Feb 13 15:31:30", format) == err("Invalid format");

        alt parse("Fri Feb 13 15:31:30 2009", format) {
          err(e) { fail e }
          ok(tm) {
            ...
            // FIXME: some platforms don't set all the values.
            //assert tm.tm_yday == 0_i32;

            assert tm.tm_isdst == 0_i32;
            assert tm.tm_gmtoff == 0_i32;
            assert tm.tm_zone == "";
            assert tm.tm_usec == 0_i32;
          }
        }

        fn test(s: str, format: str) -> bool {
            alt parse(s, format) {
              ok(tm) { tm.format(format) == s }
              err(e) { fail e }
            }
        }
        ...
        assert test("2009", "%Y");
        assert test("09", "%y");
        assert parse("UTC", "%Z").get().tm_zone == "UTC";
        #error("test_parse parse: %?", parse("PST", "%Z"));
        //assert parse("PST", "%Z").get().tm_zone == "";
        assert parse("-0000", "%z").get().tm_gmtoff == 0_i32;
        assert parse("-0800", "%z").get().tm_gmtoff == 0_i32;
        assert test("%", "%%");
    }
   ...
   fn test_format() {
        ...
        assert local.format("") == "";
        assert local.format("%A") == "Friday";
        assert local.format("%a") == "Fri";
        assert local.format("%B") == "February";
        assert local.format("%b") == "Feb";
        assert local.format("%C") == "20";
        assert local.format("%c") == "Fri Feb 13 15:31:30 2009";
        assert local.format("%D") == "02/13/09";
        assert local.format("%d") == "13";
        assert local.format("%e") == "13";
        assert local.format("%F") == "2009-02-13";
        // assert local.format("%G") == "2009";
        // assert local.format("%g") == "09";
        assert local.format("%H") == "15";
        assert local.format("%I") == "03";
        assert local.format("%j") == "044";
        assert local.format("%k") == "15";
        assert local.format("%l") == " 3";
        assert local.format("%M") == "31";
        assert local.format("%m") == "02";
        assert local.format("%n") == "\n";
        assert local.format("%P") == "pm";
        assert local.format("%p") == "PM";
        assert local.format("%R") == "15:31";
        assert local.format("%r") == "03:31:30 PM";
        assert local.format("%S") == "30";
        assert local.format("%s") == "1234567890";
        assert local.format("%T") == "15:31:30";
        assert local.format("%t") == "\t";
        // assert local.format("%U") == "06";
        assert local.format("%u") == "5";
        // assert local.format("%V") == "07";
        assert local.format("%v") == "13-Feb-2009";
        // assert local.format("%W") == "06";
        assert local.format("%w") == "5";
        // handle "%X"
        // handle "%x"
        assert local.format("%Y") == "2009";
        assert local.format("%y") == "09";
        #error("test_format local.format: %?", local.format("%Z"));
        //assert local.format("%Z") == "PST";
        assert local.format("%z") == "-0700";
        assert local.format("%%") == "%";

        assert local.ctime() == "Fri Feb 13 15:31:30 2009";
        #error("test_format local.rfc822: %?", local.rfc822());
