...
//! value As `&mut Any`, there is also the `as_mut` method, for getting a mutable reference to the
//! inner value.  `~Any` adds the `move` method, which will unwrap a `~T` from the object.  See the
