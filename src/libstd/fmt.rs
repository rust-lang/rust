...
//! ```
//!
//! This is invalid because the first argument is both referred to as an integer
//! as well as a string.
//!
//! Because formatting is done via traits, there is no requirement that the
//! `d` format actually takes an `int`, but rather it simply requires a type
//! which ascribes to the `Signed` formatting trait. There are various
//! parameters which do require a particular type, however. Namely if the syntax
//! `{:.*s}` is used, then the number of characters to print from the string
//! precedes the actual string and must have the type `uint`. Although a `uint`
//! can be printed with `{:u}`, it is illegal to reference an argument as such.
//! For example, this is another invalid format string:
//!
//! ```text
//! {:.*s} {0:u}
//! ```
...
//! This allows multiple actual types to be formatted via `{:d}` (like `i8` as
...
//! > and is an experimental implementation that should not be relied upon. In
//! > order to use the `?` modifier, the libdebug crate must be linked against.
//!
...
//!         // documentation for details, and the function `pad` can be used to
//!         // pad strings.
...
//! the '0' flag is specified for numerics, then the implicit fill character
//! is '0'.
//!
