...
macro_rules! impl_num_cast(
    ($T:ty, $conv:ident) => (
        impl NumCast for $T {
            ...
            #[inline(force)] fn to_u8(&self)    -> u8    { *self as u8    }
            #[inline(force)] fn to_u16(&self)   -> u16   { *self as u16   }
            #[inline(force)] fn to_u32(&self)   -> u32   { *self as u32   }
            #[inline(force)] fn to_u64(&self)   -> u64   { *self as u64   }
            #[inline(force)] fn to_uint(&self)  -> uint  { *self as uint  }

            #[inline(force)] fn to_i8(&self)    -> i8    { *self as i8    }
            #[inline(force)] fn to_i16(&self)   -> i16   { *self as i16   }
            #[inline(force)] fn to_i32(&self)   -> i32   { *self as i32   }
            #[inline(force)] fn to_i64(&self)   -> i64   { *self as i64   }
            #[inline(force)] fn to_int(&self)   -> int   { *self as int   }

            #[inline(force)] fn to_f32(&self)   -> f32   { *self as f32   }
            #[inline(force)] fn to_f64(&self)   -> f64   { *self as f64   }
            #[inline(force)] fn to_float(&self) -> float { *self as float }
        }
    )
)
