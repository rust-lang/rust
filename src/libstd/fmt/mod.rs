...
/// A type which represents the result of a formatting operation. This error is
/// propagated upwards from the underlying io::Writer instance.
...
#[allow(missing_doc)]
pub trait Default { fn fmt(&Self, &mut Formatter) -> Result; }
...
pub fn writeln(output: &mut io::Writer, args: &Arguments) -> Result {
    if_ok!(unsafe { write_unsafe(output, args.fmt, args.args) });
    output.write(['\n' as u8])
}
...
impl<'self> Formatter<'self> {
    ...
    fn runplural(&mut self, value: uint, pieces: &[rt::Piece]) {
        ::uint::to_str_bytes(value, 10, |buf| {
            ...
            for piece in pieces.iter() {
                ret = self.run(piece, Some(valuestr));
                if ret.is_err() {
                    break
                }
            }
    ...
    pub fn pad_integral(&mut self, s: &[u8], alternate_prefix: &str,
        ...
        let sign = |this: &mut Formatter| {
            if !signprinted {
                let r1 = if this.flags & 1 << (FlagSignPlus as uint) != 0 &&
                            positive
                {
...
impl<T> Poly for T {
    fn fmt(t: &T, f: &mut Formatter) -> Result {
        match (f.width, f.precision) {
            (None, None) => repr::write_repr(f.buf, t),

            // If we have a specified width for formatting, then we have to make
            // this allocation of a new string
            _ => f.pad(repr::repr_to_str(t)),
        }
    }
}
