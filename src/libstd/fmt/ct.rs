...
// FIXME(#8179): it would be awesome to raise errors through conditions, but
//               they don't work cross-crate yet
...
/// A count is used for the precision and width parameters of a in integer, and
...
pub struct Parser<'self> {
    ...
    // FIXME(#8179): use a condition instead (see above)
    priv err_fn: &'self fn(&str),
}
...
impl<'self> Parser<'self> {
    ...
    pub fn new<'a>(s: &'a str, err_fn: &'a fn(&str)) -> Parser<'a> {
        Parser {
            input: s,
...
mod tests {
    ...
    fn same(fmt: &'static str, p: ~[Piece<'static>]) {
        let mut parser = Parser::new(fmt, |m| fail!("%s", m));
