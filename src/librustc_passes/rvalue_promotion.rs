...
impl<'a, 'gcx> CheckCrateVisitor<'a, 'gcx> {
    ...
    fn handle_const_fn_call(&mut self, def_id: DefId) -> Promotability {
...
fn check_expr_kind<'a, 'tcx>(
    ...
    let ty_result = match node_ty.sty {
        ty::Adt(def, _) if def.has_dtor(v.tcx) => NotPromotable,
        _ => Promotable,
    };
    ...
    let node_result = match e.node {
        ...
        hir::ExprKind::Binary(op, ref lhs, ref rhs) => {
            ...
            match v.tables.node_id_to_type(lhs.hir_id).sty {
                ty::RawPtr(_) | ty::FnPtr(..) => {
                    assert!(
                        op.node == hir::BinOpKind::Eq
                            || op.node == hir::BinOpKind::Ne
                            || op.node == hir::BinOpKind::Le
                            || op.node == hir::BinOpKind::Lt
                            || op.node == hir::BinOpKind::Ge
                            || op.node == hir::BinOpKind::Gt
                    );
        ...
        hir::ExprKind::Cast(ref from, _) => {
            ...
            match v.tables.cast_kinds().get(from.hir_id) {
                ...
                Some(&CastKind::PtrAddrCast) | Some(&CastKind::FnPtrAddrCast) => NotPromotable,
                            ...
                            if attr.check_name("thread_local") {
                                debug!(
                                    "Reference to Static(id={:?}) is unpromotable \
                                     due to a #[thread_local] attribute",
                                    did
                                );
                                return NotPromotable;
                            }
        ...
        hir::ExprKind::Call(ref callee, ref hirvec) => {
            ...
            let def_result = match def {
                ...
                Def::Method(did) => match v.tcx.associated_item(did).container {
        ...
        hir::ExprKind::MethodCall(ref _pathsegment, ref _span, ref hirvec) => {
            ...
            if let Some(def) = v.tables.type_dependent_defs().get(e.hir_id) {
                ...
                v.tcx
                    .sess
                    .delay_span_bug(e.span, "no type-dependent def for method call");
                NotPromotable
            }
        }
        ...
        hir::ExprKind::Lit(_) | hir::ExprKind::Err => Promotable,

        hir::ExprKind::AddrOf(_, ref expr) | hir::ExprKind::Repeat(ref expr, _) => {
            v.check_expr(&expr)
        }
        ...
        hir::ExprKind::Block(ref box_block, ref _option_label) => v.check_block(box_block),
        ...
        hir::ExprKind::Type(ref expr, ref _ty) => v.check_expr(&expr),
...
impl<'a, 'gcx, 'tcx> euv::Delegate<'tcx> for CheckCrateVisitor<'a, 'gcx> {
    ...
    fn borrow(
        ...
        loop {
            match cur.cat {
                Categorization::ThreadLocal(..) | Categorization::Rvalue(..) => {
