...
impl FileMap {
    ...
    pub fn get_line(&self, line: int) -> String {
        ...
        let slice = self.src.as_str().slice_from(begin);
        match slice.find('\n') {
...
impl CodeMap {
    ...
    pub fn new_filemap(&self, filename: FileName, src: String) -> Rc<FileMap> {
        ...
        let mut src = if src.as_str().starts_with("\ufeff") {
            String::from_str(src.as_str().slice_from(3))
        } else {
            String::from_str(src.as_str())
        };
    ...
    pub fn span_to_snippet(&self, sp: Span) -> Option<String> {
        ...
        if begin.fm.start_pos != end.fm.start_pos {
            ...
            Some(begin.fm.src.as_str().slice(begin.pos.to_uint(),
