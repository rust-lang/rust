...
use errors::{Applicability, Diagnostic, DiagnosticBuilder, FatalError};
...
impl<'a> StringReader<'a> {
    ...
    pub fn new_without_err(
        sess: &'a ParseSess,
        source_file: Lrc<syntax_pos::SourceFile>,
        override_span: Option<Span>,
        prepend_error_text: &str,
    ) -> Result<Self, ()> {
        let mut sr = StringReader::new_raw(sess, source_file, override_span);
        if sr.advance_token().is_err() {
        ...
        match self.scan_whitespace_or_comment() {
            ...
            None => {
                if self.is_eof() {
                    ...
                    let (real, raw) =
                        self.mk_sp_and_raw(self.source_file.end_pos, self.source_file.end_pos);
                    self.peek_span = real;
    ...
    fn scan_number(&mut self, c: char) -> token::Lit {
        ...
        let num_digits = if c == '0' {
            match self.ch.unwrap_or('\0') {
                ...
                '0'..='9' | '_' | '.' | 'e' | 'E' => self.scan_digits(10, 10) + 1,
                _ => {
                    // just a 0
...
mod tests {
    ...
    fn character_a() {
        with_globals(|| {
            ...
            let sh = mk_sess(sm.clone());
            assert_eq!(
                setup(&sm, &sh, "'a'".to_string()).next_token().tok,
    ...
    fn character_space() {
        with_globals(|| {
            ...
            let sh = mk_sess(sm.clone());
            assert_eq!(
                setup(&sm, &sh, "' '".to_string()).next_token().tok,
    ...
    fn lifetime_name() {
        with_globals(|| {
            ...
            let sh = mk_sess(sm.clone());
            assert_eq!(
                setup(&sm, &sh, "'abc".to_string()).next_token().tok,
    ...
    fn raw_string() {
        with_globals(|| {
            ...
            let sh = mk_sess(sm.clone());
            assert_eq!(
                setup(&sm, &sh, "r###\"\"#a\\b\x00c\"\"###".to_string())
                    .next_token()
                    .tok,
        ...
        with_globals(|| {
            ...
            macro_rules! test {
                ($input: expr, $tok_type: ident, $tok_contents: expr) => {{
                    assert_eq!(
                        setup(&sm, &sh, format!("{}suffix", $input))
                            .next_token()
                            .tok,
                        token::Literal(
            ...
            assert_eq!(
                setup(&sm, &sh, "2us".to_string()).next_token().tok,
                token::Literal(
            ...
            assert_eq!(
                setup(&sm, &sh, "r###\"raw\"###suffix".to_string())
                    .next_token()
                    .tok,
                token::Literal(
            ...
            assert_eq!(
                setup(&sm, &sh, "br###\"raw\"###suffix".to_string())
                    .next_token()
                    .tok,
                token::Literal(
