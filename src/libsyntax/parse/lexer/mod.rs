...
use ast::{self, Ident};
use syntax_pos::{self, BytePos, CharPos, Pos, Span, DUMMY_SP};
...
impl<'a> StringReader<'a> {
    fn mk_sp(&self, lo: BytePos, hi: BytePos) -> Span {
        if !self.override_span.source_equal(&DUMMY_SP) {
            return self.override_span;
        }
        Span::new(lo, hi, self.override_span.ctxt())
    }
    fn mk_ident(&self, string: &str) -> Ident {
        let mut ident = Ident::from_str(string);
        ident.ctxt = self.override_span.ctxt();
        ident
    }

    /// Set the override for the spans generated by this `StringReader`:
    ///
    /// * If this span is a `DUMMY_SP` with no syntax context, the override will
    ///   have no effect.
    /// * If the span is a `DUMMY_SP` with an syntax context, the expansion
    ///   context of the generated spans will be set.
    /// * If the span is not DUMMY_SP, the span of all tokens will be overridden
    ///   with that span.
    pub fn override_span(&mut self, span: Span) {
        self.override_span = span;
        // re-generate our peek span with the correct span context and override
        // information.
        self.peek_span = self.mk_sp(self.peek_span.lo(), self.peek_span.hi());
        self.span = self.mk_sp(self.span.lo(), self.span.hi());
    }
...
impl<'a> StringReader<'a> {
    ...
    fn new_raw_internal(sess: &'a ParseSess, filemap: Lrc<syntax_pos::FileMap>) -> Self {
        ...
        StringReader {
            ...
            open_braces: Vec::new(),
            override_span: syntax_pos::DUMMY_SP,
        }
    }
