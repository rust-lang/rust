...
mod tests {
    ...
    fn test_ecfg() -> ExpansionConfig<'static> {
        ExpansionConfig::default(String::from("test"))
    }

    // make sure that macros can't escape fns
    #[should_panic]
    #[test] fn macros_cant_escape_fns_test () {
        let src = String::from("fn bogus() {macro_rules! z (() => (3+4));}\
                                fn inty() -> i32 { z!() }");
    ...
    #[test] fn macros_cant_escape_mods_test () {
        let src = String::from("mod foo {macro_rules! z (() => (3+4));}\
                                fn inty() -> i32 { z!() }");
        let sess = parse::ParseSess::new();
        let crate_ast = parse::parse_crate_from_source_str(
            String::from("<test>"),
    ...
    #[test] fn macros_can_escape_flattened_mods_test () {
        let src = String::from("#[macro_use] mod foo {macro_rules! z (() => (3+4));}\
                                fn inty() -> i32 { z!() }");
        let sess = parse::ParseSess::new();
        let crate_ast = parse::parse_crate_from_source_str(
            String::from("<test>"),
    ...
    #[test] fn macro_tokens_should_match(){
        expand_crate_str(
            String::from("macro_rules! m((a)=>(13)) ;fn main(){m!(a);}"));
    }

    // should be able to use a bound identifier as a literal in a macro definition:
    #[test] fn self_macro_parsing(){
        expand_crate_str(
            String::from("macro_rules! foo ((zz) => (287;));
                          fn f(zz: i32) {foo!(zz);}")
            );
    }
    ...
    fn macro_in_method_posn(){
        expand_crate_str(
            String::from("macro_rules! my_method (() => (fn thirteen(&self) -> i32 {13}));
                          struct A;
                          impl A{ my_method!(); }
                          fn f(){A.thirteen;}");
    }
    ...
    fn run_renaming_test(t: &RenamingTest, test_idx: usize) {
        ...
        let (teststr, bound_connections, bound_ident_check) = match *t {
            (ref str,ref conns, bic) => (String::from(str), conns.clone(), bic)
        };
        let cr = expand_crate_str(String::from(teststr));
    ...
    fn fmt_in_macro_used_inside_module_macro() {
        let crate_str = String::from("macro_rules! fmt_wrap(($b:expr)=>(String::from($b)));
...
");
    ...
    fn pat_idents(){
        let pat = string_to_pat(
            String::from("(a,Foo{x:c @ (b,9),y:Bar(4,d)})"));
    ...
    fn crate_bindings_test(){
        let the_crate = string_to_crate(String::from("fn main (a: i32) -> i32 {|b| {
        match 34 {None => 3, Some(i) | i => j, Foo{k:z,l:y} => \"banana\"}} }");
    ...
    fn ident_renamer_test () {
        let the_crate = string_to_crate(String::from("fn f(x: i32){let x = x; x}"));
