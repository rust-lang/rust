...
pub struct MacroExpander<'a, 'b:'a> {
    ...
    // The type of the impl currently being expanded. This only works because
    // impls cannot be nested.
...
impl<'a, 'b> Folder for MacroExpander<'a, 'b> {
    ...
    fn fold_item(&mut self, item: P<ast::Item>) -> SmallVector<P<ast::Item>> {
        if let ast::Item_::ItemImpl(_, _, ref ty, _) = item.node {
            debug!("nrc setting impl_ty to {}", ty);
