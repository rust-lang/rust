...
use std::slice;
use std::vec;
use syntax_pos::{self, BytePos, SourceFile, Span, DUMMY_SP, NO_EXPANSION};
...
use source_map::{self, dummy_spanned, respan, ExpnInfo, MacroAttribute, SourceMap};
                ...
                .map(
                    |ast::Item {
                        ...
                        let allow_dead_code_item =
                            attr::mk_list_item(DUMMY_SP, allow_ident, vec![dc_nested]);
                        let allow_dead_code =
...
fn mk_main(cx: &mut TestCtxt) -> P<ast::Item> {
    ...
    let mut test_runner = cx
        .test_runner
        .clone()
        .unwrap_or(ecx.path(sp, vec![test_id, ecx.ident_of("test_main_static")]));

    test_runner.span = sp;

    let test_main_path_expr = ecx.expr_path(test_runner);
    let call_test_main = ecx.expr_call(sp, test_main_path_expr, vec![mk_tests_slice(cx)]);
    ...
    let main = ast::ItemKind::Fn(
        ecx.fn_decl(vec![], ast::FunctionRetTy::Ty(main_ret_ty)),
    ...
    let main_id = Ident::new(
        cx.reexport_test_harness_main
            .unwrap_or(Symbol::gensym("main")),
        sp,
    );
...
fn get_test_runner(sd: &errors::Handler, krate: &ast::Crate) -> Option<ast::Path> {
    ...
    if let Some(meta_list) = test_attr.meta_item_list() {
        if meta_list.len() != 1 {
            sd.span_fatal(
                test_attr.span(),
                "#![test_runner(..)] accepts exactly 1 argument",
            )
            .raise()
        }
        ...
        sd.span_fatal(
            test_attr.span(),
            "test_runner must be of the form #[test_runner(..)]",
        )
        .raise()
    }
}
