...
use ast::{Attribute, GenericArg, MacDelimiter};
use ast::{GenericBound, SelfKind, TraitBoundModifier};
use attr;
...
pub trait PrintState<'a> {
    ...
    fn print_literal(&mut self, lit: &ast::Lit) -> io::Result<()> {
        ...
        match lit.node {
            ...
            ast::LitKind::Int(i, t) => match t {
                ...
                ast::LitIntType::Unsuffixed => self.writer().word(i.to_string()),
            },
    ...
    fn print_attribute_inline(&mut self, attr: &ast::Attribute, is_inline: bool) -> io::Result<()> {
        ...
        if attr.is_sugared_doc {
            self.writer()
                .word(attr.value_str().unwrap().as_str().get())?;
            self.writer().hardbreak()
        } else {
            match attr.style {
    ...
    fn print_tt(&mut self, tt: tokenstream::TokenTree) -> io::Result<()> {
        match tt {
            ...
            TokenTree::Delimited(_, delim, tts) => {
                self.writer()
                    .word(token_to_string(&token::OpenDelim(delim)))?;
                self.writer().space()?;
...
impl<'a> State<'a> {
    ...
    pub fn print_item(&mut self, item: &ast::Item) -> io::Result<()> {
        ...
        match item.node {
            ...
            ast::ItemKind::Fn(ref decl, header, ref typarams, ref body) => {
                self.head("")?;
                self.print_fn(decl, header, Some(item.ident), typarams, &item.vis)?;
                self.s.word(" ")?;
    ...
    pub fn print_generic_params(&mut self, generic_params: &[ast::GenericParam]) -> io::Result<()> {
        ...
        self.commasep(Inconsistent, &generic_params, |s, param| match param.kind {
