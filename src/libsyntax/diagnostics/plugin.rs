...
pub fn expand_register_diagnostic<'cx>(
    ...
    let (code, description) = match (
        ...
        (1, Some(&TokenTree::Token(_, token::Ident(ref code, _))), None, None) => (code, None),
        (
            3,
            Some(&TokenTree::Token(_, token::Ident(ref code, _))),
    ...
    MacEager::items(smallvec![ecx.item_mod(
        span,
        span,
        sym,
        Vec::new(),
...
pub fn expand_build_diagnostic_array<'cx>(
    ...
    if let Ok(target_triple) = env::var("CFG_COMPILER_HOST_TRIPLE") {
        ecx.parse_sess
            ...
            .with_lock(|diagnostics| {
                if let Err(e) =
                    output_metadata(ecx, &target_triple, &crate_name.as_str(), diagnostics)
                {
                    ecx.span_bug(
                        span,
                        &format!(
