    ...
    match map.find(id) {
        ...
        Some(NodeMethod(m)) => {
            (format!("method {} in {} (id={})",
                    ...
                    map.path_to_string(id), id)).to_string()
        }
        ...
        Some(NodeExpr(ref expr)) => {
            (format!("expr {} (id={})",
                    pprust::expr_to_string(&**expr), id)).to_string()
        }
        Some(NodeStmt(ref stmt)) => {
            (format!("stmt {} (id={})",
                    pprust::stmt_to_string(&**stmt), id)).to_string()
        }
        Some(NodeArg(ref pat)) => {
            (format!("arg {} (id={})",
                    pprust::pat_to_string(&**pat), id)).to_string()
        }
        ...
        Some(NodePat(ref pat)) => {
            (format!("pat {} (id={})", pprust::pat_to_string(&**pat), id)).to_string()
        }
        Some(NodeBlock(ref block)) => {
            (format!("block {} (id={})",
                    pprust::block_to_string(&**block), id)).to_string()
        }
        ...
        Some(NodeLifetime(ref l)) => {
            (format!("lifetime {} (id={})",
                    pprust::lifetime_to_string(&**l), id)).to_string()
        }
        None => {
