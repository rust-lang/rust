...
pub fn prepare_session_directory(tcx: TyCtxt) -> Result<bool, ()> {
    ...
    loop {
        ...
        if let Ok(allows_links) = copy_files(&session_dir,
                                             &source_directory,
...
pub fn finalize_session_directory(sess: &Session, svh: Svh) {
    ...
    debug!("finalize_session_directory() - new path: {}",
           new_path.display());
...
fn copy_files(target_dir: &Path,
    ...
    if print_stats_on_success {
        println!("incr. comp. session directory: {} files hard-linked",
                 files_linked);
        println!("incr. comp. session directory: {} files copied",
                 files_copied);
    }
...
fn generate_session_dir_path(crate_dir: &Path) -> PathBuf {
    ...
    debug!("generate_session_dir_path: random_number = {}",
           random_number);

    let directory_name = format!("s-{}-{}-working",
                                 timestamp,
                                 encode_base_36(random_number as u64));
    debug!("generate_session_dir_path: directory_name = {}",
           directory_name);
    let directory_path = crate_dir.join(directory_name);
    debug!("generate_session_dir_path: directory_path = {}",
    ...
    where I: Iterator<Item = PathBuf>
{
    ...
    for session_dir in iter {
        ...
        let timestamp = extract_timestamp_from_session_dir(&directory_name)
            .unwrap_or_else(|_| {
                bug!("unexpected incr-comp session dir: {}",
                     session_dir.display())
            });
...
pub fn find_metadata_hashes_for(tcx: TyCtxt, cnum: CrateNum) -> Option<PathBuf> {
    ...
    let sub_dir =
        find_metadata_hashes_iter(&target_svh,
...
fn crate_path(sess: &Session, crate_name: &str, crate_disambiguator: &str) -> PathBuf {
...
pub fn garbage_collect_session_directories(sess: &Session) -> io::Result<()> {
    ...
    let lock_file_to_session_dir: FnvHashMap<String, Option<String>> = lock_files.into_iter()
        .map(|lock_file_name| {
    ...
    let lock_file_to_session_dir: FnvHashMap<String, String> = lock_file_to_session_dir.into_iter()
...
fn test_all_except_most_recent() {
    assert_eq!(all_except_most_recent(vec![
...
fn test_find_source_directory_in_iter() {
    ...
    // Find newest
    assert_eq!(find_source_directory_in_iter(vec![PathBuf::from("crate-dir/s-3234-0000-svh"),
    ...
    // Filter out "-working"
    assert_eq!(find_source_directory_in_iter(vec![PathBuf::from("crate-dir/s-3234-0000-working"),
                                                  PathBuf::from("crate-dir/s-2234-0000-svh"),
                                                  PathBuf::from("crate-dir/s-1234-0000-svh")]
                                                 .into_iter(),
                                             &already_visited),
