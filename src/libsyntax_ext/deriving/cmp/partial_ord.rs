...
fn cs_op(
    ...
    let par_cmp = |cx: &mut ExtCtxt, span, self_f: P<Expr>, other_fs: &[P<Expr>], default| {
        ...
        let cmp_path =
            cx.expr_path(cx.path_global(span, cx.std_path(&["cmp", "PartialOrd", "partial_cmp"])));
        ...
        let unwrap_path =
            cx.expr_path(cx.path_global(span, cx.std_path(&["option", "Option", "unwrap_or"])));
    ...
    let fold = cs_fold1(
        ...
        |cx, span, subexpr, self_f, other_fs| {
            ...
            let then_with_path =
                cx.expr_path(cx.path_global(span, cx.std_path(&["cmp", "Ordering", "then_with"])));
            cx.expr_call(
                span,
                then_with_path,
                vec![par_cmp, cx.lambda0(span, subexpr)],
            )
        },
        |cx, args| match args {
