...
use syntax::codemap::{self, DUMMY_SP, respan};
        ...
        let methods = self.methods
                          ...
                          .map(|method_def| {
                              let (explicit_self, self_args, nonself_args, tys) =
                                  method_def.split_self_nonself_args(cx,
                                                                     self,
                                                                     type_ident,
                                                                     generics);
...
impl<'a> MethodDef<'a> {
    ...
    fn build_enum_match_tuple<'b>(&self,
        ...
        let catch_all_substructure = EnumNonMatchingCollapsed(self_arg_idents,
                                                              &variants[..],
                                                              &vi_idents[..]);
        ...
        let mut match_arms: Vec<ast::Arm> =
            variants.iter()
                    .enumerate()
                    .filter(|&(_, v)| {
                        !(self.unify_fieldless_variants && v.node.data.fields().is_empty())
                    })
                    .map(|(index, variant)| {
                        let mk_self_pat = |cx: &mut ExtCtxt, self_arg_name: &str| {
                            let (p, idents) =
                                trait_.create_enum_variant_pattern(cx,
                                                                   type_ident,
                                                                   variant,
                                                                   self_arg_name,
                                                                   ast::Mutability::Immutable);
        ...
        if variants.len() > 1 && self_args.len() > 1 {
            ...
            for (&ident, self_arg) in vi_idents.iter().zip(&self_args) {
                ...
                match first_ident {
                    Some(first) => {
                        ...
                        discriminant_test = cx.expr_binary(sp,
                                                           BinOpKind::And,
                                                           discriminant_test,
                                                           test)
                    }
            ...
            cx.expr_block(cx.block_all(sp, index_let_stmts, Some(arm_expr)))
