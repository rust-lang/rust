...
pub fn expand_test_or_bench(
    ...
    // creates test::$name
    let test_path = |name| cx.path(sp, vec![test_id, cx.ident_of(name)]);

    // creates test::$name
    let should_panic_path = |name| {
        cx.path(
            sp,
            vec![test_id, cx.ident_of("ShouldPanic"), cx.ident_of(name)],
        )
    };
        ...
        vec![
            ...
            cx.attribute(
                ...
                cx.meta_list(
                    attr_sp,
                    Symbol::intern("cfg"),
                    vec![cx.meta_list_item_word(attr_sp, Symbol::intern("test"))],
                ),
            ),
            // #[rustc_test_marker]
            cx.attribute(
                attr_sp,
                cx.meta_word(attr_sp, Symbol::intern("rustc_test_marker")),
            ),
        ],
