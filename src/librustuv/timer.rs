...
pub enum NextAction {
    WakeTask,
    CallOnce(Box<Callback + Send + 'static>),
    CallMany(Box<Callback + Send + 'static>, uint),
}
...
impl RtioTimer for TimerWatcher {
    ...
    fn oneshot(&mut self, msecs: u64, cb: Box<Callback + Send + 'static>) {
    ...
    fn period(&mut self, msecs: u64, cb: Box<Callback + Send + 'static>) {
        // similarly to the destructor, we must drop the previous action outside
        // of the homing missile
        let _prev_action = {
            let _m = self.fire_homing_missile();
