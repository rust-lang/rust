...
pub struct PipeListener {
    ...
    outgoing: Sender<IoResult<Box<rtio::RtioPipe + Send + 'static>>>,
    incoming: Receiver<IoResult<Box<rtio::RtioPipe + Send + 'static>>>,
}
...
impl rtio::RtioUnixListener for PipeListener {
    fn listen(self: Box<PipeListener>)
        ...
        match unsafe { uvll::uv_listen(acceptor.listener.pipe, 128, listen_cb) } {
            0 => Ok(acceptor as Box<rtio::RtioUnixAcceptor + Send + 'static>),
...
extern fn listen_cb(server: *mut uvll::uv_stream_t, status: libc::c_int) {
    ...
    let msg = match status {
        0 => {
            ...
            Ok(box client as Box<rtio::RtioPipe + Send + 'static>)
        }
        n => Err(uv_error_to_io_error(UvError(n)))
    };
