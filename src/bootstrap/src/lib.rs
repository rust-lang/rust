...
const LLVM_TOOLS: &[&str] = &[
    ...
    "llvm-readobj",  /* used to get information from ELFs/objects that the other tools don't
                      * provide */
...
impl Build {
    ...
    pub fn new(mut config: Config) -> Build {
        ...
        // run. This is ok because `setup` never runs any build commands, so it won't fail if
        ...
        if !matches!(build.config.cmd, Subcommand::Setup { .. }) {
            ...
            // Make sure we update these before gathering metadata so we don't get an error about
    ...
    pub(crate) fn update_submodule(&self, relative_path: &Path) {
        ...
        let update = |progress: bool| {
            // Git is buggy and will try to fetch submodules from the tracking branch for *this*
            // repository, even though that has no relation to the upstream for the
            // submodule.
            let current_branch = {
                let output = self
                    .config
            ...
            if let Some(branch) = current_branch {
                // If there is a tag named after the current branch, git will try to disambiguate by
                // prepending `heads/` to the branch name. This syntax isn't
                // accepted by `branch.{branch}`. Strip it.
        ...
        // Save any local changes, but avoid running `git stash pop` if there are none (since it
        // will exit with an error). diff-index reports the modifications through the exit
        // status
    ...
    fn is_rust_llvm(&self, target: TargetSelection) -> bool {
        match self.config.target_config.get(&target) {
            // We're using a user-controlled version of LLVM. The user has explicitly told us
            // whether the version has our patches. (They might be wrong, but that's not
            // a supported use-case.) In particular, this tries to support `submodules =
            // false` and `patches = false`, for using a newer version of LLVM that's not through
    ...
    /// Runs a command, printing out contextual info if it fails, and delaying errors until the
    /// build finishes.
    ...
    /// Return a `Group` guard for a [`Step`] that is only built once and isn't affected by
    /// `--stage`.
    ///
    /// [`Step`]: crate::core::builder::Step
    #[must_use = "Groups should not be dropped until the Step finishes running"]
