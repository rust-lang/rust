...
impl StepSelectors {
    ...
    fn intersection_removing_matches(
        ...
        match self {
            StepSelectors::Set(set) => {
                StepSelectors::Set(set.iter().filter(|&p| check(p)).cloned().collect())
            }
        ...
        self.paths.insert(StepSelectors::Set(
            paths
                ...
                .map(|p| {
                    ...
                    StepSelection { path: p.into(), kind: Some(self.kind) }
                })
                .collect(),
        ));
        self
