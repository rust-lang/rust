...
pub struct GccTargetPair {
    /// Target on which the libgccjit.so file will be executed.
    host: TargetSelection,
    /// Target for which the libgccjit.so will generate assembly.
...
impl GccTargetPair {
    ...
    pub fn for_cross_build(host: TargetSelection, target: TargetSelection) -> Self {
        Self { host, target }
    }
