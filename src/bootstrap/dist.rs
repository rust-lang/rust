...
use crate::util::{exe, is_dylib, libdir};
...
fn maybe_install_llvm_dylib(builder: &Builder, target: Interned<String>, image: &Path) {
    ...
    if llvm_dylib_path.exists() {
        let llvm_dylib_path = llvm_dylib_path.canonicalize().unwrap_or_else(|e| {
            panic!(
                "dist: Error calling canonicalize path `{}`: {}",
                llvm_dylib_path.display(),
                e
            );
        });
