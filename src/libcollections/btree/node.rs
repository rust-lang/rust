...
impl<'a, K, V> Handle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::KV> {
    ...
    pub fn bulk_steal_left(&mut self, n: usize) {
        unsafe {
            ...
            ptr::copy(right_k, right_k.offset(n as isize), right_len);
            ptr::copy(right_v, right_v.offset(n as isize), right_len);
            if let Some(edges) = right_e {
                ptr::copy(edges, edges.offset(n as isize), right_len + 1);
            }

            // Move elements from the left child to the right one.
            let left_ind = (left_len - n) as isize;
            ptr::copy_nonoverlapping(left_k.offset(left_ind + 1), right_k, n - 1);
            ptr::copy_nonoverlapping(left_v.offset(left_ind + 1), right_v, n - 1);
            match (left_e, right_e) {
                (Some(left), Some(right)) => {
                    ptr::copy_nonoverlapping(left.offset(left_ind + 1), right, n);
                }
            ...
            ptr::copy_nonoverlapping(parent_k, right_k.offset(n as isize - 1), 1);
            ptr::copy_nonoverlapping(parent_v, right_v.offset(n as isize - 1), 1);
            // Copy left-most stolen pair to parent.
            ptr::copy_nonoverlapping(left_k.offset(left_ind), parent_k, 1);
            ptr::copy_nonoverlapping(left_v.offset(left_ind), parent_v, 1);
