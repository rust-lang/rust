...
/// `Context`, notwithstanding whatever safety checks you have asked the compiler to [`Assume`] are
/// satisfied.
...
pub struct Assume {
    /// When `true`, the compiler assumes that *you* are ensuring (either dynamically or
    /// statically) that destination referents do not have stricter alignment requirements than
    /// source referents.
    pub alignment: bool,

    /// When `true`, the compiler assume that *you* are ensuring that lifetimes are not extended in
    /// a manner that violates Rust's memory model.
    pub lifetimes: bool,

    /// When `true`, the compiler assumes that *you* have ensured that it is safe for you to
    /// violate the type and field privacy of the destination type (and sometimes of the source
    /// type, too).
    pub safety: bool,

    /// When `true`, the compiler assumes that *you* are ensuring that the source type is actually
    /// a valid instance of the destination type.
    pub validity: bool,
}
