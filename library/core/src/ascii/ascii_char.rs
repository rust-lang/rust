...
impl AsciiChar {
    ...
    pub const fn eq_ignore_ascii_case(&self, other: &Self) -> bool {
        // FIXME(const-hack) `arg.to_u8()` -> `arg` once `PartialEq` is const for `Self`.
    ...
    ///
    /// **Note**: This method always returns `true` for an `ascii::Char`. This
    /// method is provided only for symmetry with the use of [`u8`] as an ASCII
    /// character.
    ///
    /// # Examples
    ///
    ...
    ///
    /// assert!(uppercase_a.is_ascii());
    /// assert!(uppercase_g.is_ascii());
    /// assert!(a.is_ascii());
    /// assert!(g.is_ascii());
    /// assert!(zero.is_ascii());
    /// assert!(percent.is_ascii());
    ...
    /// assert!(!eight.is_ascii_octdigit());
    /// assert!(!percent.is_ascii_octdigit());
    /// assert!(!lf.is_ascii_octdigit());
    /// assert!(!esc.is_ascii_octdigit());
    /// ```
    #[must_use]
