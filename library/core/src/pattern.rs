...
///
/// The trait is used in combination with the [`Pattern`] trait to express a pattern
/// that can be used to search for elements in a given haystack.
pub trait Haystack {
    /// Returns a cursor pointing at the beginning of the haystack.
    fn cursor_at_front(&self) -> usize;

    /// Returns a cursor pointing at the end of the haystack.
...
impl EmptyNeedleSearcher {
    ...
    pub fn new<H: Haystack + ?Sized>(haystack: &H) -> Self {
        Self {
...
#[unstable(feature = "pattern_internals", issue = "none")]
pub struct Split<'h, H: Haystack + ?Sized, S: Searcher<'h, H>> {
    ...
    finished: bool,
    ctx: crate::marker::PhantomData<&'h H>,
}
...
/// into.
#[unstable(feature = "pattern_internals", issue = "none")]
pub struct SplitN<'h, H: Haystack + ?Sized, S: Searcher<'h, H>> {
...
impl<'h, H: Haystack + ?Sized, S: Searcher<'h, H> + Clone> Clone for Split<'h, H, S> {
    fn clone(&self) -> Self {
        Self { searcher: self.searcher.clone(), ..*self }
    }
}

impl<'h, H: Haystack + ?Sized, S: Searcher<'h, H> + Clone> Clone for SplitN<'h, H, S> {
    fn clone(&self) -> Self {
        Self { inner: self.inner.clone(), ..*self }
    }
}

impl<'h, H, S> fmt::Debug for Split<'h, H, S>
where
    S: Searcher<'h, H> + fmt::Debug,
    H: Haystack + ?Sized,
{
...
impl<'h, H, S> fmt::Debug for SplitN<'h, H, S>
where
    S: Searcher<'h, H> + fmt::Debug,
    H: Haystack + ?Sized,
{
...
impl<'h, H: Haystack + ?Sized, S: Searcher<'h, H>> Split<'h, H, S> {
    ...
    pub fn with_limit(self, limit: usize) -> SplitN<'h, H, S> {
...
impl<'h, H: Haystack + ?Sized, S: Searcher<'h, H>> Split<'h, H, S> {
    ...
    /// pattern.
    #[inline]
    pub fn next_fwd<const INCLUSIVE: bool>(&mut self) -> Option<&'h H> {
        if self.finished {
            return None;
        }
    ...
    /// pattern.
    #[inline]
    pub fn next_bwd<const INCLUSIVE: bool>(&mut self) -> Option<&'h H>
    where
        S: ReverseSearcher<'h, H>,
    {
        if self.finished {
            return None;
        }
    ...
    #[inline]
    pub fn remainder(&self) -> Option<&'h H> {
        (!self.finished).then(|| {
            // SAFETY: self.start and self.end come from Haystack or Searcher
            // and thus are guaranteed to be valid split positions.
    ...
    /// return `None`.
    #[inline]
    fn get_end(&mut self) -> Option<&'h H> {
        if !self.finished {
            self.finished = true;
...
impl<'h, H: Haystack + ?Sized, S: Searcher<'h, H>> SplitN<'h, H, S> {
