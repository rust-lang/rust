...
unsafe impl<T> SliceIndex<[T]> for Clamp<range::RangeInclusive<usize>> {
    ...
    fn get(self, slice: &[T]) -> Option<&Self::Output> {
        (self.0.start.min(slice.len() - 1)..=self.0.last.min(slice.len() - 1)).get(slice)
    }

    fn get_mut(self, slice: &mut [T]) -> Option<&mut Self::Output> {
        (self.0.start.min(slice.len() - 1)..=self.0.last.min(slice.len() - 1)).get_mut(slice)
    }

    unsafe fn get_unchecked(self, slice: *const [T]) -> *const Self::Output {
        // SAFETY: the index before the length is always valid
        unsafe {
            (self.0.start.min(slice.len() - 1)..=self.0.last.min(slice.len() - 1))
                .get_unchecked(slice)
        }
    }
    ...
    fn index(self, slice: &[T]) -> &Self::Output {
        (self.0.start.min(slice.len() - 1)..=self.0.last.min(slice.len() - 1)).index(slice)
    }

    fn index_mut(self, slice: &mut [T]) -> &mut Self::Output {
        (self.0.start.min(slice.len() - 1)..=self.0.last.min(slice.len() - 1)).index_mut(slice)
    }
}
...
unsafe impl<T> SliceIndex<[T]> for Clamp<range::RangeToInclusive<usize>> {
    ...
    fn get(self, slice: &[T]) -> Option<&Self::Output> {
        (..=self.0.last.min(slice.len() - 1)).get(slice)
    }

    fn get_mut(self, slice: &mut [T]) -> Option<&mut Self::Output> {
        (..=self.0.last.min(slice.len() - 1)).get_mut(slice)
    }

    unsafe fn get_unchecked(self, slice: *const [T]) -> *const Self::Output {
        // SAFETY: the index before the length is always valid
        unsafe { (..=self.0.last.min(slice.len() - 1)).get_unchecked(slice) }
    }

    unsafe fn get_unchecked_mut(self, slice: *mut [T]) -> *mut Self::Output {
        // SAFETY: the index before the length is always valid
        unsafe { (..=self.0.last.min(slice.len() - 1)).get_unchecked_mut(slice) }
    }

    fn index(self, slice: &[T]) -> &Self::Output {
        (..=self.0.last.min(slice.len() - 1)).index(slice)
    }

    fn index_mut(self, slice: &mut [T]) -> &mut Self::Output {
        (..=self.0.last.min(slice.len() - 1)).index_mut(slice)
    }
}
