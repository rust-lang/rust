...
///
/// The iterator stores an optional item (`Option<T>`) and a successor closure (`impl FnMut(&T) -> Option<T>`).
/// The iterator's `next` method returns the stored optional item and
