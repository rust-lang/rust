...
#[repr(transparent)]
pub struct Bytes<F>(PhantomData<F>, [u8]);

impl<F: Flavour> crate::fmt::Debug for Bytes<F> {
    fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
        crate::fmt::Debug::fmt(self.as_bytes(), f)
    }
}

impl<F: Flavour> Bytes<F> {
...
impl Bytes<Unstructured> {
    #[inline]
    /// Wraps `&[u8]` into `Bytes`.
    pub fn from_bytes(val: &[u8]) -> &Self {
        // SAFETY: Bytes<Unstructured> has no encoding requirements
        unsafe { Bytes::new(val) }
    }
}
...
impl Bytes<Utf8> {
    ...
    pub fn from_str(val: &str) -> &Self {
        // SAFETY: `str` is guaranteed to be UTF-8
        unsafe { Bytes::new(val.as_bytes()) }
    }

    /// Returns the contents of this `Bytes` as a `&str`.
    #[inline]
    pub fn into_str(&self) -> &str {
        if cfg!(debug_assertions) {
            crate::str::from_utf8(self.as_bytes()).unwrap()
        } else {
            // SAFETY: Bytes with Utf8 flavor are guaranteed to be valid UTF-8
            unsafe { crate::str::from_utf8_unchecked(self.as_bytes()) }
        }
    }
}

/// A marker for bytes slice which is not necessarily well-formed.
#[derive(Clone, Copy, Debug)]
pub enum Unstructured {}
/// A marker for well-formed WTF-8 bytes slice.
#[derive(Clone, Copy, Debug)]
pub enum Wtf8 {}
/// A marker for well-formed UTF-8 bytes slice.
#[derive(Clone, Copy, Debug)]
pub enum Utf8 {}
...
pub trait Flavour: private::Flavour + 'static {}
...
mod private {
    ...
    impl Flavour for super::Unstructured {
        ...
        fn adjust_position_bwd(bytes: &[u8], range: Range) -> usize {
            let shift = bytes[range.start..range.end + 1]
                .iter()
                .rev()
    ...
    impl Flavour for Wtf8 {
        fn adjust_position_fwd(bytes: &[u8], range: Range) -> usize {
            ...
            for _ in 0..3 {
                ...
                // comparison.
                if bytes.get(pos).is_none_or(|b: &u8| b.is_utf8_char_boundary()) {
                    break;
                }
                pos += 1;
            }
        ...
        fn advance_range_start(bytes: &[u8]) -> usize {
            ...
            // sequence to skip from the first byte.
        ...
        fn get_last_code_point(bytes: &[u8]) -> Option<(char, usize)> {
            // SAFETY: We’re Wtf8 flavour.  Client promises that bytes are
            // well-formed WTF-8.
            let cp = unsafe { next_code_point_reverse(&mut bytes.iter())? };
            // WTF-8 might produce surrogate code points so we still need to
            // verify that we got a valid character.
            char::from_u32(cp).map(|chr| (chr, len_utf8(cp)))
        }
    ...
    impl Flavour for Utf8 {
        ...
        fn find_code_point_bwd(bytes: &[u8]) -> Option<(usize, char, usize)> {
            ...
            // sequence but also that produced code points are valid.
            let chr = unsafe {
                let code = next_code_point_reverse(&mut bytes.iter())?;
...
trait SearchResult: crate::pattern::SearchResult {
...
impl<F: Flavour> Haystack for Bytes<F> {
...
impl<F: Flavour> pattern::Pattern<Bytes<F>> for char {
    type Searcher<'h> = CharSearcher<'h, F>;
    fn into_searcher<'h>(self, haystack: &'h Bytes<F>) -> Self::Searcher<'h> {
        Self::Searcher::new(haystack, self)
    }

    fn is_contained_in(self, haystack: &Bytes<F>) -> bool {
        let mut buf = [0; 4];
        encode_utf8(self, &mut buf).is_contained_in(haystack)
    }

    fn is_prefix_of(self, haystack: &Bytes<F>) -> bool {
        let mut buf = [0; 4];
        encode_utf8(self, &mut buf).is_prefix_of(haystack)
    }
    fn strip_prefix_of<'h>(self, haystack: &'h Bytes<F>) -> Option<&'h Bytes<F>> {
        let mut buf = [0; 4];
        encode_utf8(self, &mut buf).strip_prefix_of(haystack)
    }

    fn is_suffix_of<'h>(self, haystack: &'h Bytes<F>) -> bool
    where
        Self::Searcher<'h>: pattern::ReverseSearcher<'h, Bytes<F>>,
    {
        let mut buf = [0; 4];
        encode_utf8(self, &mut buf).is_suffix_of(haystack)
    }
    fn strip_suffix_of<'h>(self, haystack: &'h Bytes<F>) -> Option<&'h Bytes<F>>
    where
        Self::Searcher<'h>: pattern::ReverseSearcher<'h, Bytes<F>>,
    {
        let mut buf = [0; 4];
...
impl CharSearcherInner {
    ...
    #[inline]
    fn next_fwd<R: SearchResult, F: Flavour>(&mut self, haystack: &Bytes<F>) -> R {
        match self {
    ...
    #[inline]
    fn next_bwd<R: SearchResult, F: Flavour>(&mut self, haystack: &Bytes<F>) -> R {
        match self {
...
impl<'h, F: Flavour> crate::fmt::Debug for CharSearcher<'h, F> {
    fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
        f.debug_struct("CharSearcher").finish_non_exhaustive()
    }
}
...
impl<'h, F: Flavour> CharSearcher<'h, F> {
    /// Creates a new searcher for the given character.
    #[inline]
    pub fn new(haystack: &'h Bytes<F>, chr: char) -> Self {
        Self { haystack, state: CharSearcherInner::new(haystack.len(), chr) }
    }
}

unsafe impl<'h, F: Flavour> pattern::Searcher<'h, Bytes<F>> for CharSearcher<'h, F> {
...
unsafe impl<'h, F: Flavour> pattern::ReverseSearcher<'h, Bytes<F>> for CharSearcher<'h, F> {
...
impl CharSearcherState {
    ...
    fn find_match_fwd<F: Flavour>(&mut self, haystack: &Bytes<F>) -> OptRange {
    ...
    fn next_reject_fwd<F: Flavour>(&mut self, haystack: &Bytes<F>) -> OptRange {
    ...
    fn find_match_bwd<F: Flavour>(&mut self, haystack: &Bytes<F>) -> OptRange {
    ...
    fn next_reject_bwd<F: Flavour>(&mut self, haystack: &Bytes<F>) -> OptRange {
...
#[diagnostic::do_not_recommend]
impl<F: Flavour, P: FnMut(char) -> bool> pattern::Pattern<Bytes<F>> for P {
    type Searcher<'h> = PredicateSearcher<'h, F, P>;
    fn into_searcher<'h>(self, haystack: &'h Bytes<F>) -> Self::Searcher<'h> {
        Self::Searcher::new(haystack, self)
    }

    fn is_prefix_of(mut self, haystack: &Bytes<F>) -> bool {
        haystack.get_first_code_point().is_some_and(|(chr, _)| self(chr))
    }
    fn strip_prefix_of<'h>(mut self, haystack: &'h Bytes<F>) -> Option<&'h Bytes<F>> {
    ...
    fn is_suffix_of<'h>(mut self, haystack: &'h Bytes<F>) -> bool
    where
        Self::Searcher<'h>: pattern::ReverseSearcher<'h, Bytes<F>>,
    {
        haystack.get_last_code_point().is_some_and(|(chr, _)| self(chr))
    }
    fn strip_suffix_of<'h>(mut self, haystack: &'h Bytes<F>) -> Option<&'h Bytes<F>>
    where
        Self::Searcher<'h>: pattern::ReverseSearcher<'h, Bytes<F>>,
    {
...
#[derive(Clone)]
pub struct PredicateSearcher<'h, F, P> {
    haystack: &'h Bytes<F>,
    pred: P,
    start: usize,
    end: usize,
...
impl<'h, F: Flavour, P> crate::fmt::Debug for PredicateSearcher<'h, F, P> {
    fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
        f.debug_struct("PredicateSearcher").finish_non_exhaustive()
    }
}
...
impl<'h, F: Flavour, P: FnMut(char) -> bool> PredicateSearcher<'h, F, P> {
...
unsafe impl<'h, F, P> pattern::Searcher<'h, Bytes<F>> for PredicateSearcher<'h, F, P>
where
    F: Flavour,
    P: FnMut(char) -> bool,
{
...
unsafe impl<'h, F, P> pattern::ReverseSearcher<'h, Bytes<F>> for PredicateSearcher<'h, F, P>
where
    F: Flavour,
    P: FnMut(char) -> bool,
{
...
impl<'h, F, P> pattern::DoubleEndedSearcher<'h, Bytes<F>> for PredicateSearcher<'h, F, P>
where
    F: Flavour,
    P: FnMut(char) -> bool,
{
...
#[diagnostic::do_not_recommend]
impl<'p, F: Flavour> pattern::Pattern<Bytes<F>> for &'p str {
    type Searcher<'h> = StrSearcher<'h, 'p, F>;
    fn into_searcher<'h>(self, haystack: &'h Bytes<F>) -> Self::Searcher<'h> {
        Self::Searcher::new(haystack, self)
    }
...
#[derive(Clone)]
pub struct StrSearcher<'h, 'p, F> {
    haystack: &'h Bytes<F>,
    inner: StrSearcherInner<'p>,
}

impl<'h, 'p, F: Flavour> crate::fmt::Debug for StrSearcher<'h, 'p, F> {
    fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
        f.debug_struct("StrSearcher").finish_non_exhaustive()
    }
}

impl<'h, 'p, F: Flavour> StrSearcher<'h, 'p, F> {
    /// Creates a new searcher for the given substring.
    pub fn new(haystack: &'h Bytes<F>, needle: &'p str) -> Self {
        let inner = StrSearcherInner::new(haystack, needle);
        Self { haystack, inner }
    }
}

unsafe impl<'h, 'p, F: Flavour> pattern::Searcher<'h, Bytes<F>> for StrSearcher<'h, 'p, F> {
    fn haystack(&self) -> &'h Bytes<F> {
        self.haystack
    }
    #[inline]
...
unsafe impl<'h, 'p, F: Flavour> pattern::ReverseSearcher<'h, Bytes<F>> for StrSearcher<'h, 'p, F> {
    fn next_back(&mut self) -> SearchStep {
...
impl<'p> StrSearcherInner<'p> {
    fn new<F: Flavour>(haystack: &Bytes<F>, needle: &'p str) -> Self {
...
impl EmptySearcherState {
    fn new<F: Flavour>(haystack: &Bytes<F>) -> Self {
        Self(pattern::EmptyNeedleSearcher::new(haystack))
    }

    fn next_fwd<R: pattern::SearchResult, F: Flavour>(&mut self, bytes: &Bytes<F>) -> R {
        self.0.next_fwd(|range| bytes.advance_range_start(range))
    }

    fn next_bwd<R: pattern::SearchResult, F: Flavour>(&mut self, bytes: &Bytes<F>) -> R {
...
impl<'p> StrSearcherState<'p> {
    ...
    fn next_fwd<R: SearchResult, F: Flavour>(&mut self, bytes: &Bytes<F>) -> R {
        if self.searcher.position >= bytes.len() {
            return R::DONE;
        }
        if self.searcher.memory == usize::MAX {
            self.searcher.next_fwd::<R>(&bytes.1, self.needle.as_bytes(), true)
        } else {
            self.searcher.next_fwd::<R>(&bytes.1, self.needle.as_bytes(), false)
        }
        .adjust_reject_end_fwd(bytes, bytes.len(), &mut self.searcher.position)
    }

    fn next_bwd<R: SearchResult, F: Flavour>(&mut self, bytes: &Bytes<F>) -> R {
        if self.searcher.end == 0 {
            return R::DONE;
        }
        if self.searcher.memory == usize::MAX {
            self.searcher.next_bwd::<R>(&bytes.1, self.needle.as_bytes(), true)
        } else {
            self.searcher.next_bwd::<R>(&bytes.1, self.needle.as_bytes(), false)
        }
