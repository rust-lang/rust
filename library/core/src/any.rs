...
pub const fn try_as_dyn<T: Any, U: ptr::Pointee<Metadata = ptr::DynMetadata<U>> + ?Sized>(
    t: &T,
) -> Option<&U> {
...
#[must_use]
#[unstable(feature = "try_as_dyn", issue = "144361")]
pub const fn try_as_dyn_mut<T: Any, U: ptr::Pointee<Metadata = ptr::DynMetadata<U>> + ?Sized>(
    t: &mut T,
) -> Option<&mut U> {
    let vtable: Option<ptr::DynMetadata<U>> = const { intrinsics::vtable_for::<T, U>() };
    match vtable {
