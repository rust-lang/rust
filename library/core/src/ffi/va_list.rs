...
crate::cfg_select! {
    all(
        ...
        #[derive(Debug)]
        #[cfg_attr(not(bootstrap), lang = "va_list_tag")]
        #[cfg_attr(bootstrap, lang = "va_list")]
        struct VaListTag<'a> {
            stack: *mut c_void,
            gr_top: *mut c_void,
            vr_top: *mut c_void,
...
crate::cfg_select! {
    all(
        ...
        #[repr(transparent)]
        #[derive(Debug)]
        #[cfg_attr(not(bootstrap), lang = "va_list")]
        pub struct VaList<'a> {
        ...
        impl<'a> VaList<'a> {
            ...
            pub fn copy(
                ...
                let mut tag = VaListTag {
                    ptr: crate::ptr::null_mut(),
                    _marker: PhantomInvariantLifetime::new(),
                };

                // SAFETY: `self` is a valid `va_list`, `tag` has sufficient space to store a `va_list`.
                unsafe { va_copy_intrinsic::va_copy(&mut tag, &self.inner) };
...
/// Trait which permits the allowed types to be used with [`VaListTag::arg`].
///
/// # Safety
///
