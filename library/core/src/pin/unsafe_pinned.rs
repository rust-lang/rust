...
impl<T: ?Sized> UnsafePinned<T> {
    ...
    /// `*const T` returned here are UB! Use [`UnsafeCell`]
    ...
    /// The difference from [`get_mut_pinned`] and [`get_mut_unchecked`] is that
    /// this function accepts a raw pointer, which is useful to avoid the
    /// creation of temporary references.
    ///
