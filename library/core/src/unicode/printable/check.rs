...
impl Singletons {
    ...
    pub(super) const fn new(upper: &'static [(u8, u8)], lower: &'static [u8]) -> Self {
        ...
        assert!(
            ...
            "Sum of upper singleton lengths does not match lower singletons length."
        );
    ...
    pub(super) fn check(&self, x: u16) -> bool {
        ...
        for (upper, lowers) in self.iter() {
            if upper == x_upper {
                // SAFETY: The invariant ensures that the sum of all lengths in `self.upper`
                // is equal to the length of `self.lower`, so `lower_end` is guaranteed to be
                // less than `self.lower.len()`.
                for &lower in lowers {
                    if lower == x_lower {
                        return false;
                    }
