...
impl<'a> Formatter<'a> {
    ...
    /// Writes an integer representation to the `Formatter`.
    ///
    /// The `digits` must contain the absolute value of the original number,
    /// without a sign. Whether a sign is written by `pad_integral` depends on
    /// the `is_nonnegative` flag and the formatting parameters. The optional
    /// `prefix` (e.g., `"0x"`) always gets written after any sign and before
    /// any digits.
    ///
    ...
    /// Writes a number representation to the `Formatter` using a closure.
    ///
    /// The `write_abs` closure must write the absolute value of the number,
    /// without a sign, and `write_len` must match the number of characters
    /// written by `write_abs`.
    ///
    /// The output of `write_abs` should honor any precision specified by the
    /// formatting parameters. Likewise, `sign` should already account for
    /// [`Self::sign_plus`].
    ///
    /// This function correctly applies padding an alignment to the output.
        ...
        F: Fn(&mut Self) -> Result,
    {
        ...
        if self.sign_aware_zero_pad() {
            // The sign goes before the padding.
            self.write_str(sign)?;
    ...
    /// Writes n ASCII digits (U+0030) to the Formatter.
