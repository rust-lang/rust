...
impl<'a> Formatter<'a> {
    ...
    pub fn pad(&mut self, s: &str) -> Result {
        ...
        let s = if let Some(max) = self.options.get_precision() {
        ...
        if self.options.width == 0 {
            ...
            if chars_count >= usize::from(self.options.width) {
                self.buf.write_str(s)
            }
            ...
            // up the minimum width with the specified string + some alignment.
                ...
                let post_padding = self.padding(self.options.width - chars_count as u16, align)?;
                self.buf.write_str(s)?;
                post_padding.write(self)
            }
        }
