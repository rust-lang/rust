...
impl char {
    ...
    ///
    /// assert_eq!(
    ///     decode_utf16(v).map(|r| r.map_err(|e| e.unpaired_surrogate())).collect::<Vec<_>>(),
    ///     vec![Ok('𝄞'), Ok('m'), Ok('u'), Ok('s'), Err(0xDD1E), Ok('i'), Ok('c'), Err(0xD834)]
    /// );
    /// ```
    ///
    ...
    ///
    /// assert_eq!(
    ///     decode_utf16(v).map(|r| r.unwrap_or(REPLACEMENT_CHARACTER)).collect::<String>(),
    ///     "𝄞mus�ic�"
    /// );
    /// ```
