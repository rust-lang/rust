...
- Volatile operations, however, may also be used to access memory that is _outside_ of any Rust
  ...
  created with [`without_provenance`][crate::ptr::without_provenance]. The access must not trap. It
  can cause side-effects, but those must not affect Rust-allocated memory in any way.
