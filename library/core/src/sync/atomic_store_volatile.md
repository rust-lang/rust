...
- When a volatile operation is used for memory inside an [allocation], it behaves exactly like
  [`store`][Self::store], except for the additional guarantee that it won't be elided or reordered
  ...
  usual rules for memory accesses apply (including provenance).
...
When invoked during const evaluation, this behaves like a regular atomic store. In
