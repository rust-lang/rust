...
mod c {
    ...
    pub fn compile(cfg: &Config) {
        ...
        // RUSTC_DEBUGINFO_MAP looks like:
        // "library/=/rustc/<hash>/library\t/home/foo=...\t..."
        // Extract the hash from the first mapping's destination.
        let debuginfo_map = env::var("RUSTC_DEBUGINFO_MAP").unwrap_or_default();
        ...
        if let Some(hash) = commit_hash {
            ...
            build.flag_if_supported(&format!("-fdebug-prefix-map={}=/rustc/{}", manifest_dir, hash));
            build.flag_if_supported(&format!("-fdebug-prefix-map={}=/rustc/{}", root.display(), hash));
        }
