...
pub fn write_lib_rs<T: TypeDefinition>(
    w: &mut impl std::io::Write,
    notice: &str,
    cfg: &str,
    definitions: &str,
...
fn generate_rust_test_loop<T: TypeDefinition>(
    w: &mut impl std::io::Write,
...
fn create_rust_test<T: TypeDefinition>(
    w: &mut impl std::io::Write,
    intrinsic: &Intrinsic<T>,
) -> std::io::Result<()> {
...
/// `w`.
pub fn write_bindings_rust<T: TypeDefinition>(
    w: &mut impl std::io::Write,
    i: usize,
    intrinsics: &[Intrinsic<T>],
) -> std::io::Result<()> {
