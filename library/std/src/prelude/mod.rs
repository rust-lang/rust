...
//!
//! * <code>[std::marker]::{[Copy], [Send], [Sized], [Sync], [Unpin]}</code>, marker traits that
//!   indicate fundamental properties of types.
//! * <code>[std::ops]::{[Drop], [Fn], [FnMut], [FnOnce]}</code>, various operations for both
//!   destructors and overloading `()`.
//! * <code>[std::mem]::[drop]</code>, a convenience function for explicitly dropping a value.
//! * <code>[std::boxed]::[Box]</code>, a way to allocate values on the heap.
//! * <code>[std::borrow]::[ToOwned]</code>, the conversion trait that defines [`to_owned`], the
//!   generic method for creating an owned type from a borrowed type.
//! * <code>[std::clone]::[Clone]</code>, the ubiquitous trait that defines [`clone`][Clone::clone],
//!   the method for producing a copy of a value.
//! * <code>[std::cmp]::{[PartialEq], [PartialOrd], [Eq], [Ord]}</code>, the comparison traits,
//!   which implement the comparison operators and are often seen in trait bounds.
//! * <code>[std::convert]::{[AsRef], [AsMut], [Into], [From]}</code>, generic conversions, used by
//!   savvy API authors to create overloaded methods.
...
//! * <code>[std::option]::[Option]::{[self][Option], [Some], [None]}</code>, a type which expresses
//!   the presence or absence of a value. This type is so commonly used, its variants are also
//!   exported.
//! * <code>[std::result]::[Result]::{[self][Result], [Ok], [Err]}</code>, a type for functions that
//!   may succeed or fail. Like [`Option`], its variants are exported as well.
