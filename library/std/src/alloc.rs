...
impl System {
    ...
    // SAFETY: Same as `Allocator::grow_with_token`
    #[inline]
    unsafe fn grow_impl_with_token(
        &self,
        ptr: NonNull<u8>,
        old_layout: Layout,
        new_layout: Layout,
        ...
        match old_layout.size() {
            0 => self.alloc_impl_with_token(new_layout, zeroed, token),
            ...
            old_size if old_layout.align() == new_layout.align() => unsafe {
                ...
                let raw_ptr = imp::realloc_with_token(ptr.as_ptr(), old_layout, new_size, token);
                let ptr = NonNull::new(raw_ptr).ok_or(AllocError)?;
                if zeroed {
                    raw_ptr.add(old_size).write_bytes(0, new_size - old_size);
                }
            ...
            // for `dealloc` must be upheld by the caller.
            old_size => unsafe {
                let new_ptr = self.alloc_impl_with_token(new_layout, zeroed, token)?;
                ptr::copy_nonoverlapping(ptr.as_ptr(), new_ptr.as_mut_ptr(), old_size);
                Allocator::deallocate(self, ptr, old_layout);
                Ok(new_ptr)
            },
...
unsafe impl Allocator for System {
    ...
    fn allocate_with_token(
        ...
        self.alloc_impl_with_token(layout, false, token)
    }
    ...
    fn allocate_zeroed_with_token(
        ...
        self.alloc_impl_with_token(layout, true, token)
    }
    ...
    unsafe fn grow_with_token(
        ...
        // SAFETY: all conditions must be upheld by the caller
        unsafe { self.grow_impl_with_token(ptr, old_layout, new_layout, false, token) }
    }
    ...
    unsafe fn shrink_with_token(
        ...
        match new_layout.size() {
            ...
            // for `dealloc` must be upheld by the caller.
            new_size => unsafe {
                let new_ptr = Allocator::allocate_with_token(self, new_layout, token)?;
                ptr::copy_nonoverlapping(ptr.as_ptr(), new_ptr.as_mut_ptr(), new_size);
                Allocator::deallocate(self, ptr, old_layout);
                Ok(new_ptr)
            },
