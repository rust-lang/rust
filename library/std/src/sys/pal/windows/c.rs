...
// Unused items in windows_sys are expected.
// Some targets use things that others don't and those targets in turn may use things that are unused by other targets.
// For the sake of contributors we also prefer to keep churn in the bindings to a minimum by only removing things once
// we're sure they will not be needed again.
#[allow(unused)]
mod windows_sys;
pub use windows_sys::*;
