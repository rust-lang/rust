...
pub fn env() -> Env {
    let keys = uefi_env::get_all_keys().expect("not supported on this platform");

    let env: Vec<(OsString, OsString)> = keys
        .into_iter()
        .map(|x| {
            let val = uefi_env::get(&x).expect("not supported on this platform");
            (x, val)
        })
        .collect();
...
mod uefi_env {
    ...
    pub(crate) fn get(key: &OsStr) -> Option<OsString> {
        ...
        let val = unsafe { ((*shell.as_ptr()).get_env)(key_ptr.as_mut_ptr()) };
    ...
    pub(crate) fn get_all_keys() -> io::Result<Vec<OsString>> {
        let shell = helpers::open_shell().ok_or(unsupported_err())?;
        ...
        for i in 0.. {
            if unsafe { val.add(i) }.is_null() {
                // Two NULL signal end of string
                if i == start {
                    break;
                }

                vars.push(OsString::from_wide(unsafe {
                    crate::slice::from_raw_parts(val.add(start).cast(), i - start)
                }));
                start = i + 1;
            }
        }
