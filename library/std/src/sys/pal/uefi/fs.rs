...
pub fn rename(old: &Path, new: &Path) -> io::Result<()> {
    ...
    let info = f.get_info()?;
    let mut new_info = uefi_fs::Info::with_fname(&info, new.as_os_str());
    f.set_info(&mut new_info)
}
...
mod uefi_fs {
    ...
    impl File {
        ...
        /// FIXME: Add support for different volumes
        ...
        pub fn from_device_handle(device_handle: NonNull<crate::ffi::c_void>) -> io::Result<Self> {
        ...
        pub fn open(&self, path: &Path, open_mode: u64, attr: u64) -> io::Result<Self> {
    ...
    pub fn rootfs() -> io::Result<File> {
        ...
        File::from_device_handle(device_handle)
    }
    ...
    impl Info {
        ...
        pub fn with_fname(&self, fname: &crate::ffi::OsStr) -> Box<Info> {
            let mut name = fname.encode_wide().collect::<Vec<u16>>();
            name.push(0);

            let other_size = crate::mem::size_of::<file::Info<0>>() + name.len() * 2;
            let mut other = Self::alloc(other_size);

            other.size = other_size as u64;
            other.file_size = self.file_size;
            other.physical_size = self.physical_size;

            // 0 indicates do not update
            other.create_time = Default::default();
            other.last_access_time = Default::default();
            other.modification_time = Default::default();

            other.attribute = self.attribute;
            other.file_name.copy_from_slice(&name);
