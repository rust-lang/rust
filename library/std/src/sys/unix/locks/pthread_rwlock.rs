...
pub struct RwLock {
    boxed: LazyBox<StaticRwLock>,
}

impl RwLock {
    #[inline]
    pub const fn new() -> RwLock {
        RwLock { boxed: LazyBox::new() }
    }

    #[inline]
    pub fn read(&self) {
        unsafe {
            let r = libc::pthread_rwlock_rdlock(self.boxed.inner.get());
            ...
            if r == libc::EAGAIN {
            ...
            } else if r == libc::EDEADLK || (r == 0 && *self.boxed.write_locked.get()) {
                ...
                self.boxed.num_readers.fetch_add(1, Ordering::Relaxed);
            }
        }
    }
    ...
    pub fn try_read(&self) -> bool {
        unsafe {
            let r = libc::pthread_rwlock_tryrdlock(self.boxed.inner.get());
            if r == 0 {
                if *self.boxed.write_locked.get() {
    ...
    pub fn write(&self) {
        unsafe {
            let r = libc::pthread_rwlock_wrlock(self.boxed.inner.get());
            // See comments above for why we check for EDEADLK and write_locked. For the same reason,
            // we also need to check that there are no readers (tracked in `num_readers`).
            if r == libc::EDEADLK
                || (r == 0 && *self.boxed.write_locked.get())
                || self.boxed.num_readers.load(Ordering::Relaxed) != 0
            {
                // Above, we make sure to only access `write_locked` when `r == 0` to avoid
                // data races.
                if r == 0 {
            ...
            *self.boxed.write_locked.get() = true;
        }
    }
    ...
    pub fn try_write(&self) -> bool {
        unsafe {
            let r = libc::pthread_rwlock_trywrlock(self.boxed.inner.get());
            if r == 0 {
                if *self.boxed.write_locked.get()
                    || self.boxed.num_readers.load(Ordering::Relaxed) != 0
                {
    ...
    unsafe fn raw_unlock(&self) {
        let r = libc::pthread_rwlock_unlock(self.boxed.inner.get());
        debug_assert_eq!(r, 0);
    }

    #[inline]
    pub unsafe fn read_unlock(&self) {
        debug_assert!(!*self.boxed.write_locked.get());
        self.boxed.num_readers.fetch_sub(1, Ordering::Relaxed);
        self.raw_unlock();
    }

    #[inline]
    pub unsafe fn write_unlock(&self) {
        debug_assert_eq!(self.boxed.num_readers.load(Ordering::Relaxed), 0);
        debug_assert!(*self.boxed.write_locked.get());
        *self.boxed.write_locked.get() = false;
        self.raw_unlock();
    }
}
