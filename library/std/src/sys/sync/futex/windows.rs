...
pub(crate) type SmallPrimitive = u8;

pub(crate) unsafe trait Futexable {}
pub(crate) unsafe trait Waitable {
    type Futex;
}
...
pub(crate) fn wait_on_address<W: Waitable>(
    address: &W::Futex,
    compare: W,
...
pub(crate) fn wake_by_address_single<T: Futexable>(address: &T) {
    unsafe {
        let addr = ptr::from_ref(address).cast::<c_void>();
...
pub(crate) fn wake_by_address_all<T: Futexable>(address: &T) {
    unsafe {
        let addr = ptr::from_ref(address).cast::<c_void>();
...
pub(crate) fn futex_wait<W: Waitable>(
    futex: &W::Futex,
    expected: W,
    timeout: Option<Duration>,
) -> bool {
...
pub(crate) fn futex_wake<T: Futexable>(futex: &T) -> bool {
    wake_by_address_single(futex);
    false
}

pub(crate) fn futex_wake_all<T: Futexable>(futex: &T) {
