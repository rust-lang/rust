...
//! for the output of `SysRng` so we simply wait until it is initialized. When
//! `HashMap` keys however, this represents a potential source of deadlocks, as
//! the additional entropy may only be generated once the program makes forward
//! progress. In that case, we just use the best random data the system has
