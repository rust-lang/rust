cfg_if::cfg_if! {
    if #[cfg(target_os = "hermit")] {
        ...
        // SGX should protect in-enclave data from outside attackers, so ther
        // must not be any data leakage to the OS, particularily no 1-1 mapping
        // between SGX thread names and OS thread names. Hence this method is
        ...
        mod wasi;
        pub use wasi::{DEFAULT_MIN_STACK_SIZE, sleep, yield_now};
        #[cfg(target_feature = "atomics")]
        pub use wasi::{Thread, available_parallelism};
        #[expect(dead_code)]
        mod unsupported;
        pub use unsupported::{current_os_id, set_name};
        #[cfg(not(target_feature = "atomics"))]
        pub use unsupported::{Thread, available_parallelism};
    } else if #[cfg(all(target_family = "wasm", target_feature = "atomics"))] {
        mod wasm;
        pub use wasm::sleep;
