...
pub fn hostname() -> io::Result<OsString> {
    ...
    loop {
        ...
        match (r != 0).then(errno) {
            ...
            // returned is if the system uses `EINVAL` to report that the name
            // does not fit in the provided buffer. In that case (or in the case
            // of `ENAMETOOLONG`), resize the buffer and try again.
