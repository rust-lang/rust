...
//!
//! But they probably should've been part of `libSystem.dylib`, both because they link to symbols
...
//! it requires a platform parameter to differentiate between macOS and Mac Catalyst), though it's
...
use super::{current_version, pack_os_version};
...
pub(super) extern "C" fn __isPlatformVersionAtLeast(
    ...
    let version = pack_os_version(major as u16, minor as u8, subminor as u8);
