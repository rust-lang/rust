...
/// # Semantics
///
/// The reported version on macOS might be 10.16 if SDK version of the binary is less than 11.0.
...
///
/// It _is_ possible to get the real version regardless of the SDK version of the binary, and this
/// is what Zig does:
/// <https://github.com/ziglang/zig/blob/0.13.0/lib/std/zig/system/darwin/macos.zig>
///
/// However, we do not do that, instead, we follow Apple's behaviour here, and return 10.16 when
...
fn current_version() -> OSVersion {
    ...
    //
    // 0.0.0 is never going to be a valid version, so we use that as our sentinel value.
...
fn version_from_plist() -> OSVersion {
    ...
    // utilities for parsing PLists (to avoid having to re-implement that in here).
    //
    // If this is found to be undesired, we _could_ possibly hack it by parsing the PList manually
    // (it seems to not use the binary encoding in any version), but that seems brittle.
...
fn parse_version_from_plist(root: &Path, plist_buffer: &[u8]) -> OSVersion {
    ...
    macro_rules! dlsym {
        (
            ...
            let ptr = unsafe { libc::dlsym(cf_handle, concat!(stringify!($name), '\0').as_bytes().as_ptr().cast()) };
            if ptr.is_null() {
                let err = unsafe { CStr::from_ptr(libc::dlerror()) };
                panic!("could not find function {}: {err:?}", stringify!($name));
            }
            unsafe { crate::mem::transmute::<*mut c_void, unsafe extern "C" fn($($param_ty),*) $(-> $ret)?>(ptr) }
        }};
    }
