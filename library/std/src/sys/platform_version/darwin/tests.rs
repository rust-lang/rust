...
/// if `rustc` decides to parse it without CoreFoundation in the future, that
/// it would continue to work, even on older platforms.
#[test]
fn parse_plist() {
    #[track_caller]
    fn check(
        ...
        let root = if cfg!(target_abi = "sim") {
            PathBuf::from(env::var_os("IPHONE_SIMULATOR_ROOT").unwrap())
        } else {
            PathBuf::from("/")
        };
        ...
        assert_eq!(expected, parse_version_from_plist(&root, plist.as_bytes()));
    }
...
fn invalid_plist() {
    ...
    let _ = parse_version_from_plist(&root, b"INVALID");
}
...
<dict>
    ...
    let _ = parse_version_from_plist(&root, plist.as_bytes());
}
