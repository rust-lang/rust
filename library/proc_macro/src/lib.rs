...
pub mod bridge {
    pub use super::backend::{
        DelimSpan, Diagnostic, ExpnGlobals, Group, Ident, LitKind, Literal, PanicMessage, Punct,
        TokenTree, client, server,
    };
}
...
impl Literal {
    ...
    pub fn str_value(&self) -> Result<String, ConversionErrorKind> {
        self.0.symbol.with(|symbol| match self.0.kind {
            ...
            backend::LitKind::StrRaw(_) => Ok(symbol.to_string()),
