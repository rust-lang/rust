...
/// A variant that takes the arguments from the command line, and exist the process.
///
...
pub fn test_main_env_args(tests: &[&TestDescAndFn]) -> ! {
    ...
    test_main_inner(&args, tests, None).exit_process()
}

/// A variant that takes the arguments from the command line, and exits the process.
///
...
pub fn test_main_env_args_abort(tests: &[&TestDescAndFn]) -> ! {
    ...
    // Tests are sorted by name at compile time by mk_tests_slice.
    let tests = TestList::new(tests, TestListOrder::Sorted);
    test_main_inner(&args, tests, Some(Options::new().panic_abort(true))).exit_process()
}
