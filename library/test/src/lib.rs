...
pub fn test_main(args: &[String], tests: Vec<TestDescAndFn>, options: Option<Options>) {
    ...
    if opts.list {
        ...
        if !opts.nocapture {
            // If we encounter a non-unwinding panic, flush any captured output from the current
            // test, and stop capturing output to ensure that the non-unwinding panic
            // message is visible. We also acquire the locks for both output streams to
            // prevent output from other threads from interleaving with the panic
            // message or appearing after it.
