...
pub fn test_main_with_exit_callback<F: FnOnce()>(
    ...
    let tests = TestList::new(&tests_bor, TestListOrder::Unsorted);
...
pub fn test_main_static_abort(tests: &[&TestDescAndFn]) {
    ...
    if let Ok(name) = env::var(SECONDARY_TEST_INVOKER_VAR) {
        ...
        let mut tests = tests.iter().copied().map(make_owned_test).collect::<Vec<_>>();
...
pub fn filter_tests(opts: &TestOpts, tests: TestList<'_>) -> Vec<TestDescAndFn> {
    ...
    let mut filtered = if opts.filters.is_empty() {
        tests.iter().copied().map(make_owned_test).collect::<Vec<_>>()
...
fn filter_exact_match<'a>(tests: &[&'a TestDescAndFn], filters: &[String]) -> Vec<TestDescAndFn> {
    ...
    for &idx in indexes.iter() {
        result.push(make_owned_test(tests[idx]));
    }
    result
}
