...
use core::num::imp::flt2dec::strategy::{dragon, grisu};
use core::num::imp::flt2dec::{
...
macro_rules! check_short {
    ($v:expr => $digits:expr, $pow10:expr) => {
        ...
        let mut bufd = [MaybeUninit::new(b'_'); MAX_SIG_DIGITS];
        let d = dragon::format_short(&dec, &mut bufd);
        assert_eq!(d, want, "dragon short format of {label}");

        let mut bufg = [MaybeUninit::new(b'_'); MAX_SIG_DIGITS];
        if let Some(g) = grisu::format_short(&dec, &mut bufg) {
            assert_eq!(g, want, "grisu short format of {label}");
        }
    };
}
...
macro_rules! check_fixed_resolution {
    ($v:expr, $resolution:expr => $digits:expr, $pow10:expr) => {
        ...
        let mut bufd = LARGE_TEST_BUF;
        let d = dragon::format_fixed(&dec, &mut bufd, resolution);
        assert_eq!(d, want, "dragon short format of {label}");

        let mut bufg = LARGE_TEST_BUF;
        if let Some(g) = grisu::format_fixed(&dec, &mut bufg, resolution) {
            assert_eq!(g, want, "dragon short format of {label}");
        }
    };
}
...
macro_rules! check_fixed_buf_match {
    ($v:expr => $digits:expr, $pow10:expr) => {
        ...
        let mut bufd = LARGE_TEST_BUF;
        let mut matchedd = &mut bufd[..want.0.len()];
        let d = dragon::format_fixed(&dec, &mut matchedd, UNLIMITED_RESOLUTION);
        assert_eq!(d, want, "dragon fixed format of {label}");

        let mut bufg = LARGE_TEST_BUF;
        let mut matchedg = &mut bufg[..want.0.len()];
        if let Some(g) = grisu::format_fixed(&dec, &mut matchedg, UNLIMITED_RESOLUTION) {
            assert_eq!(g, want, "grisu fixed format of {label}");
        }
    };
}
