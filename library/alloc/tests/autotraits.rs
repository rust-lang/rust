...
fn test_binary_heap() {
    ...
    require_send_sync(async {
        let _v = None::<alloc::collections::binary_heap::Drain<'_, &u32, Global, {CO_ALLOC_PREF_META_NO!()}>>;
        async {}.await;
    });

    require_send_sync(async {
        let _v = None::<alloc::collections::binary_heap::Drain<'_, &u32, Global, {CO_ALLOC_PREF_META_YES!()}>>;
        async {}.await;
    });
