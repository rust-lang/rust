use core::str::next_code_point_val;
...
fn check_decoded_string<I: Iterator<Item = u8>>(mut iter: &mut I, expected: &str) {
    for char in expected.chars() {
        assert_eq!(Some(char as u32), unsafe { next_code_point_val(&mut iter) });
    }
    assert_eq!(None, unsafe { next_code_point_val(&mut iter) });
}
...
pub fn dirt_simple_code_point() {
    unsafe {
        ...
        for char in "banana".chars() {
            assert_eq!(Some(char as u32), next_code_point_val(&mut iter));
        }

        let tmp = next_code_point_val(&mut iter);
        assert_eq!(None, tmp);
    }
    {
        let src = [
