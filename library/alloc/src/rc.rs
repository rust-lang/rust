...
use core::any::Any;
use core::cell::{Cell, CloneFromCell, UnsafeCell};
...
impl<T: RefUnwindSafe + ?Sized, A: Allocator + RefUnwindSafe> RefUnwindSafe for Rc<T, A> {}
...
impl<T: ?Sized> Rc<T> {
    ...
    pub unsafe fn strong_count_from_raw(ptr: *const T) -> usize {
        // SAFETY: Caller guarantees the validity of `ptr`.
        unsafe {
            *RawRc::<T, Global>::strong_count_from_raw(NonNull::new_unchecked(ptr.cast_mut())).get()
        }
    }
}
...
impl<T: ?Sized, A: Allocator> Rc<T, A> {
    ...
    fn into_raw_rc(this: Self) -> RawRc<T, A> {
        ...
        // SAFETY: It is safe to transfer ownership of the underlying `RawRc` to caller.
        unsafe { ptr::read(&this.raw_rc) }
    }

    fn raw_strong_count(&self) -> &Cell<usize> {
        // SAFETY: Reference counters used in `Rc` are logically `Cell<usize>`, but stored as
        // `UnsafeCell<usize>`, we can safely recover the logical type.
        unsafe { mem::transmute::<&UnsafeCell<usize>, &Cell<usize>>(self.raw_rc.strong_count()) }
    }

    fn raw_weak_count(&self) -> &Cell<usize> {
        // SAFETY: Reference counters used in `Rc` are logically `Cell<usize>`, but stored as
        // `UnsafeCell<usize>`, we can safely recover the logical type.
        unsafe { mem::transmute::<&UnsafeCell<usize>, &Cell<usize>>(self.raw_rc.weak_count()) }
    }
...
impl<A: Allocator> Rc<dyn Any, A> {
    ...
    pub unsafe fn downcast_unchecked<T: Any>(self) -> Rc<T, A> {
        ...
        // SAFETY: Caller ensures typecast is correct.
...
unsafe impl<#[may_dangle] T: ?Sized, A: Allocator> Drop for Rc<T, A> {
    ...
    fn drop(&mut self) {
        ...
        // implementation ensures no further access to the allocation through `self`.
...
impl<T: ?Sized, A: Allocator> Weak<T, A> {
    ...
    fn raw_strong_count(&self) -> Option<&Cell<usize>> {
        // SAFETY: Reference counters used in non-dangling `Weak`s are logically `Cell<usize>`, but
        // stored as `UnsafeCell<usize>`, we can safely recover the logical type.
        self.raw_weak.strong_count().map(|strong_count| unsafe {
            mem::transmute::<&UnsafeCell<usize>, &Cell<usize>>(strong_count)
        })
    }

    fn raw_weak_count(&self) -> Option<&Cell<usize>> {
        // SAFETY: Reference counters used in non-dangling `Weak`s are logically `Cell<usize>`, but
        // stored as `UnsafeCell<usize>`, we can safely recover the logical type.
        self.raw_weak.weak_count().map(|weak_count| unsafe {
            mem::transmute::<&UnsafeCell<usize>, &Cell<usize>>(weak_count)
        })
    }
    ...
    pub fn strong_count(&self) -> usize {
        self.raw_strong_count().map_or(0, Cell::get)
    }
    ...
    pub fn weak_count(&self) -> usize {
        self.raw_weak_count().map_or(0, |count| count.get() - 1)
    }
...
impl<T, A: Allocator> UniqueRc<T, A> {
    ...
    // #[unstable(feature = "allocator_api", issue = "163177")]
    pub fn try_new_in(value: T, alloc: A) -> Result<Self, AllocError> {
        RawUniqueRc::try_new_in(value, alloc).map(|raw_unique_rc| Self { raw_unique_rc })
    }
    ...
    #[cfg(not(no_global_oom_handling))]
    #[unstable(feature = "unique_rc_arc", issue = "112566")]
    pub fn map<U>(this: Self, f: impl FnOnce(T) -> U) -> UniqueRc<U, A> {
        let raw_unique_rc = Self::into_raw_unique_rc(this);
...
impl<T: ?Sized, A: Allocator> UniqueRc<T, A> {
    ...
    fn into_raw_unique_rc(this: Self) -> RawUniqueRc<T, A> {
        ...
        // SAFETY: It is safe to transfer the underlying `RawUniqueRc` to owner.
...
impl<T, A: Allocator> UniqueRc<mem::MaybeUninit<T>, A> {
    /// Writes the value and converts to `UniqueRc<T, A>`.
    ///
    /// This method converts similarly to [`assume_init`](Self::assume_init) but
    /// writes `value` into it before conversion, thus guaranteeing safety.
    #[unstable(feature = "unique_rc_arc", issue = "112566")]
    #[must_use]
    pub fn write(this: Self, value: T) -> UniqueRc<T, A> {
        let raw_unique_rc = Self::into_raw_unique_rc(this).write(value);

        UniqueRc { raw_unique_rc }
    }

    /// Converts to `UniqueRc<T, A>`.
    ///
    /// # Safety
    ///
    /// As with [`MaybeUninit::assume_init`],
    ...
    pub unsafe fn assume_init(self) -> UniqueRc<T, A> {
        ...
        UniqueRc {
            ...
            raw_unique_rc: unsafe { raw_unique_rc.assume_init() },
        }
    }
}
...
unsafe impl<#[may_dangle] T: ?Sized, A: Allocator> Drop for UniqueRc<T, A> {
    fn drop(&mut self) {
        // SAFETY: `self.raw_unique_rc` is only accessed with the same `RefCounter`, and the `Drop`
        // implementation prevents further access to the allocation through `self`.
