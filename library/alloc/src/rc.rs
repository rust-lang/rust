...
unsafe impl raw_rc::RcOps for RcOps {
    ...
    unsafe fn is_unique(strong_count: &UnsafeCell<usize>, weak_count: &UnsafeCell<usize>) -> bool {
        unsafe { *strong_count.get() == 1 && *weak_count.get() == 1 }
    }
...
impl<T, A: Allocator> Rc<T, A> {
    ...
    pub fn try_unwrap(this: Self) -> Result<T, Self> {
        unsafe { Rc::into_raw_rc(this).try_unwrap::<RcOps>().map_err(|raw_rc| Self { raw_rc }) }
    }
    ...
    pub fn into_inner(this: Self) -> Option<T> {
        unsafe { RawRc::into_inner::<RcOps>(Rc::into_raw_rc(this)) }
    }
}
...
impl<T, A: Allocator> Rc<mem::MaybeUninit<T>, A> {
    ...
    pub unsafe fn assume_init(self) -> Rc<T, A> {
        Rc { raw_rc: unsafe { Rc::into_raw_rc(self).assume_init() } }
    }
}
...
impl<T: ?Sized, A: Allocator> Rc<T, A> {
    ...
    pub fn into_raw(this: Self) -> *const T {
        Rc::into_raw_rc(this).into_raw().as_ptr()
    }
    ...
    pub fn into_raw_with_allocator(this: Self) -> (*const T, A) {
        let (ptr, alloc) = Rc::into_raw_rc(this).into_raw_parts();
...
impl<T: Clone, A: Allocator> Rc<T, A> {
    ...
    pub fn unwrap_or_clone(this: Self) -> T {
        unsafe { Rc::into_raw_rc(this).unwrap_or_clone::<RcOps>() }
    }
}
...
impl<A: Allocator> Rc<dyn Any, A> {
    ...
    pub fn downcast<T: Any>(self) -> Result<Rc<T, A>, Self> {
        match Rc::into_raw_rc(self).downcast::<T>() {
    ...
    pub unsafe fn downcast_unchecked<T: Any>(self) -> Rc<T, A> {
        Rc { raw_rc: unsafe { Rc::into_raw_rc(self).downcast_unchecked() } }
    }
}
...
impl<T: ?Sized, A: Allocator> UniqueRc<T, A> {
    ...
    pub fn into_rc(this: Self) -> Rc<T, A> {
        Rc {
            raw_rc: unsafe { ptr::read(&ManuallyDrop::new(this).raw_unique_rc).into_rc::<RcOps>() },
        }
    }
}
...
impl<T: ?Sized, A: Allocator + Clone> UniqueRc<T, A> {
    ...
    #[unstable(feature = "unique_rc_arc", issue = "112566")]
    pub fn downgrade(this: &Self) -> Weak<T, A> {
        Weak { raw_weak: unsafe { this.raw_unique_rc.downgrade::<RcOps>() } }
    }
}
