    ...
    R: RcOps,
{
    ...
    /// - After `WeakGuard` being dropped, the content pointed to by `weak.ptr` should not be
    ///   accessed anymore.
    ...
    T: ?Sized,
{
    ...
    /// Assume the value pointed to by `ptr` is initialized, drop the value along with the `RawWeak`
    /// object.
    #[inline]
