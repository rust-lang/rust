...
pub(crate) unsafe trait RcOps: Sized {
    ...
    /// # Safety
    ///
    /// - `strong_count` is zero.
    /// - The associated value has been initialized.
    /// - `strong_count` should only be handled by the same `RcOps` implementation.
    ...
    ///   to notify the caller moving is needed in order to make caller the only owner.
    /// - If strong count is greater than 1, `Make::by_clone` will be called to notify the caller
    ///   cloning is needed in order to make caller the only owner.
    ///
    /// # Safety
    ///
...
impl RcLayout {
    ...
    /// # Safety
    ///
    /// `RcLayout::from_value_layout(value_layout)` must return `Ok`.
    #[inline]
    ...
    /// Returns the byte offset of the value stored in an reference counted allocation that is
    /// described by `self`.
    #[inline]
    ...
    /// Creates an `RcLayout` for storing a value that is pointed by `value_ptr`, then call `f` on
    /// the `RcLayout`.
    ///
    /// # Safety
    ///
    ...
    #[cfg(not(no_global_oom_handling))]
    unsafe fn from_value_ptr<T, F, R>(value_ptr: NonNull<T>, f: F) -> R
    where
        T: ?Sized,
        F: FnOnce(NonNull<()>, Self) -> R,
    {
        fn with_value_layout<F, R>(value_ptr: NonNull<()>, value_layout: Layout, f: F) -> R
        where
            F: FnOnce(NonNull<()>, RcLayout) -> R,
        {
            f(value_ptr, RcLayout::from_value_layout(value_layout))
        }
        ...
        trait SpecFromValuePtr {
            unsafe fn spec_from_value_ptr<F, R>(value_ptr: NonNull<Self>, f: F) -> R
            where
                F: FnOnce(NonNull<()>, RcLayout) -> R;
        }

        impl<T> SpecFromValuePtr for T
        where
            T: ?Sized,
        {
            #[inline]
            default unsafe fn spec_from_value_ptr<F, R>(value_ptr: NonNull<Self>, f: F) -> R
            where
                F: FnOnce(NonNull<()>, RcLayout) -> R,
            {
                with_value_layout(
                    value_ptr.cast(),
                    unsafe { Layout::for_value_raw(value_ptr.as_ptr()) },
                    f,
                )
            }
        }
        ...
        impl<T> SpecFromValuePtr for T {
            ...
            where
                F: FnOnce(NonNull<()>, RcLayout) -> R,
            {
                f(value_ptr.cast(), Self::RC_LAYOUT)
            }
        }

        unsafe { T::spec_from_value_ptr(value_ptr, f) }
    }
    ...
    unsafe fn from_value_ptr_unchecked<T, F, R>(value_ptr: NonNull<T>, f: F) -> R
    where
        T: ?Sized,
        F: FnOnce(NonNull<()>, Self) -> R,
    {
        unsafe fn with_value_layout<F, R>(value_ptr: NonNull<()>, value_layout: Layout, f: F) -> R
        where
            F: FnOnce(NonNull<()>, RcLayout) -> R,
        {
            f(value_ptr, unsafe { RcLayout::from_value_layout_unchecked(value_layout) })
        }

        /// A helper trait for computing `RcLayout` to store a `Self` object. If `Self` is
        /// `Sized`, the `RcLayout` value is computed at compile time.
        trait SpecFromValuePtrUnchecked {
            unsafe fn spec_from_value_ptr_unchecked<F, R>(value_ptr: NonNull<Self>, f: F) -> R
            where
                F: FnOnce(NonNull<()>, RcLayout) -> R;
        }

        impl<T> SpecFromValuePtrUnchecked for T
        where
            T: ?Sized,
        {
            #[inline]
            default unsafe fn spec_from_value_ptr_unchecked<F, R>(
                value_ptr: NonNull<Self>,
                f: F,
            ) -> R
            where
                F: FnOnce(NonNull<()>, RcLayout) -> R,
        ...
        unsafe { T::spec_from_value_ptr_unchecked(value_ptr, f) }
    }
}
...
trait RcLayoutExt {
    /// Computes `RcLayout` at compile time of `Self` is `Sized`.
...
impl<T> RcLayoutExt for T {
    const RC_LAYOUT: RcLayout = match RcLayout::try_from_value_layout(T::LAYOUT) {
        ...
        Err(_) => panic!("value size is too big"),
    };
}
    ...
    A: Allocator,
{
    unsafe fn copy_value_bytes(
        ...
        move |dst_ptr| {
            let value_offset = rc_layout.value_offset();
            let value_size = unsafe { rc_layout.size().unchecked_sub(value_offset) };
    ...
    allocate_with_in::<A, _, STRONG_COUNT>(alloc, rc_layout, unsafe {
        copy_value_bytes(src_ptr, rc_layout)
    })
}

/// Returns a function that is used for allocating a chunk of reference counted memory with
/// specified bytes data.
///
/// The returned function takes two arguments:
///
/// - A `NonNull<()>` pointer that points to the desired bytes to by copied from.
/// - An `RcLayout` that describes the desired allocation memory layout.
///
/// The function returns a pointer to the newly allocated value object.
///
/// # Safety
///
/// The `NonNull<()>` argument passed to the returned function contains enough data for copying to
/// the value storage in the reference counted allocation that is described by the `RcLayout`
/// argument.
#[cfg(not(no_global_oom_handling))]
unsafe fn allocate_with_bytes_fn<A, const STRONG_COUNT: usize>(
    alloc: &A,
) -> impl FnOnce(NonNull<()>, RcLayout) -> NonNull<()> + use<'_, A, STRONG_COUNT>
where
    A: Allocator,
{
    move |value_ptr, rc_layout| unsafe {
        allocate_with_bytes_in::<A, STRONG_COUNT>(value_ptr, alloc, rc_layout)
    }
}
    ...
    A: Allocator,
{
    ...
    let new_ptr = unsafe {
        RcLayout::from_value_ptr(src_ptr, allocate_with_bytes_fn::<A, STRONG_COUNT>(alloc))
    };

    unsafe { non_null_with_metadata_of(new_ptr, src_ptr) }
}
...
/// Allocate a chunk of reference counted memory with value that is copied from `value`.
///
/// # Safety
///
/// - Object pointed by value can fit inside a reference counted allocation.
#[cfg(not(no_global_oom_handling))]
unsafe fn allocate_with_value_in_unchecked<T, A, const STRONG_COUNT: usize>(
    value: &T,
    alloc: &A,
) -> NonNull<T>
where
    ...
    A: Allocator,
{
    ...
    let new_ptr = unsafe {
        RcLayout::from_value_ptr_unchecked(
            src_ptr,
            allocate_with_bytes_fn::<A, STRONG_COUNT>(alloc),
        )
    };
...
/// A guard type that is used for setting the pointer inside an `RawRc` object on drop.
#[cfg(not(no_global_oom_handling))]
struct UpdatePtrOnDrop<'a, T, A>
where
    T: ?Sized,
{
    rc: &'a mut RawRc<T, A>,
...
#[cfg(not(no_global_oom_handling))]
impl<T, A> Drop for UpdatePtrOnDrop<'_, T, A>
where
    T: ?Sized,
{
    fn drop(&mut self) {
    ...
    T: ?Sized,
{
    ...
    {
        unsafe {
            // Used for dropping old `RawWeak` object if the allocation panics.
            let guard = WeakGuard::<T, A, R>::new(&mut self.rc.weak);

            let new_ptr = allocate_with_value_in_unchecked::<T, A, 1>(
                guard.weak.ptr.as_ref(),
                &guard.weak.alloc,
            );

            // Allocation succeeds, defuse the guard and drop it later.
            mem::forget(guard);

            // Used for updating the pointer inside `self.rc`, even if panic happens.
            let update_ptr_on_drop = UpdatePtrOnDrop { rc: self.rc, new_ptr };

            // Manually drop old `RawWeak`.
    ...
    {
        unsafe {
            RcLayout::from_value_ptr_unchecked(self.rc.weak.ptr, |_, rc_layout| {
                let new_ptr =
                    allocate_with_in::<A, _, 1>(&self.rc.weak.alloc, rc_layout, |dst_ptr| {
                        T::clone_to_uninit(self.rc.as_ref(), dst_ptr.as_ptr().cast())
                    });

                let new_ptr = non_null_with_metadata_of(new_ptr, self.rc.weak.ptr);
...
/// `RawWeak::drop` or `RawWeak::drop_unchecked` to clean up this object.
    ...
    T: ?Sized,
{
    ...
    {
        #[inline(never)]
        unsafe fn drop_impl<A, R>(value_ptr: NonNull<()>, alloc: &A, rc_layout: RcLayout)
        where
            A: Allocator,
            R: RcOps,
        {
            let deallocate = unsafe { decrement_weak_ref_count::<R>(value_ptr) };

            if deallocate {
                unsafe { deallocate_value_ptr::<A>(value_ptr, alloc, rc_layout) };
            }
        }

        unsafe fn drop_fn<A, R>(alloc: &A) -> impl FnOnce(NonNull<()>, RcLayout) + use<'_, A, R>
        where
            A: Allocator,
            R: RcOps,
        {
            move |value_ptr, rc_layout| unsafe { drop_impl::<A, R>(value_ptr, alloc, rc_layout) }
        }

        unsafe { RcLayout::from_value_ptr_unchecked(self.ptr, drop_fn::<A, R>(&self.alloc)) };
    }
    ...
    where
        ...
        R: RcOps,
    {
        unsafe fn try_upgrade<R>(value_ptr: NonNull<()>) -> bool
        where
            R: RcOps,
        {
...
/// `RawRc::drop` to clean up this object.
#[repr(transparent)]
    ...
    T: ?Sized,
{
    ...
    where
        ...
        R: RcOps,
    {
        unsafe fn downgrade<R>(value_ptr: NonNull<()>)
        where
            R: RcOps,
        {
        ...
        unsafe {
            downgrade::<R>(self.weak.ptr.cast());
    ...
    where
        ...
        R: RcOps,
    {
        let needs_drop = unsafe { decrement_strong_ref_count::<R>(self.weak.ptr.cast()) };
    ...
    where
        R: RcOps,
    {
        unsafe fn do_get_mut<R>(value_ptr: NonNull<()>) -> Option<NonNull<()>>
        where
            R: RcOps,
        {
        ...
        unsafe { do_get_mut::<R>(self.weak.ptr.cast()) }
...
impl<T, A> RawRc<T, A> {
    ...
    where
        ...
        F: FnOnce() -> T,
    {
        unsafe fn init_fn<F, T>(f: F) -> impl FnOnce(NonNull<()>)
        where
            F: FnOnce() -> T,
        {
            |ptr| unsafe { ptr.cast().write(f()) }
        }

        let (ptr, alloc) = allocate_with::<A, _, 1>(T::RC_LAYOUT, unsafe { init_fn(f) });
    ...
    where
        ...
        R: RcOps,
    {
        let success = unsafe { decrement_strong_ref_count::<R>(self.weak.ptr.cast()) };
    ...
    where
        ...
        R: RcOps,
    {
        unsafe fn lock_strong_count<R>(value_ptr: NonNull<()>) -> bool
        where
            R: RcOps,
        {
        ...
        let success = unsafe { lock_strong_count::<R>(self.weak.ptr.cast()) };
    ...
    T: ?Sized,
{
    ...
    where
        R: RcOps,
    {
        unsafe fn unlock_strong_count<R>(value_ptr: NonNull<()>)
        where
            R: RcOps,
        {
