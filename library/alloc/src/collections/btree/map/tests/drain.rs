...
fn range_small() {
    fn range_keys(size: i32, range: impl RangeBounds<i32>) -> Vec<i32> {
        let mut map = BTreeMap::from_iter((1..=size).map(|i| (i, ())));
...
fn random_1() {
    ...
    for _ in 0..if cfg!(miri) { 1 } else { 140 } {
        ...
        let mut drainage = map.drain(bounds.clone());
        let drained = drainage.len();
        assert!(drainage.all(|(k, _)| bounds.contains(&k)));
        assert_eq!(drained + map.len(), size);
        map.check();
