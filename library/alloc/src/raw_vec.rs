...
impl RawVec<Global> {
    ...
    #[must_use]
    #[inline]
    pub fn with_capacity_zeroed(capacity: usize, elem_layout: Layout) -> Self {
        Self::with_capacity_zeroed_in(capacity, Global, elem_layout)
    }
}
...
pub const fn min_non_zero_cap(size: usize) -> usize {
    if size == 1 {
        8
    } else if size <= 1024 {
        4
...
impl<A: Allocator> RawVec<A> {
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline]
    pub fn with_capacity_in(capacity: usize, alloc: A, elem_layout: Layout) -> Self {
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline]
    pub fn with_capacity_zeroed_in(capacity: usize, alloc: A, elem_layout: Layout) -> Self {
    ...
    pub unsafe fn into_box<T>(self, len: usize) -> Box<[MaybeUninit<T>], A> {
        // Sanity-check one half of the safety requirement (we cannot check the other half).
        debug_assert!(
            len <= self.capacity(core::mem::size_of::<T>()),
    ...
    /// guaranteed.
    #[inline]
    pub unsafe fn from_raw_parts_in<T>(ptr: *mut T, capacity: usize, alloc: A) -> Self {
    ...
    #[inline]
    pub unsafe fn from_nonnull_in<T>(ptr: NonNull<T>, capacity: usize, alloc: A) -> Self {
        let cap = if T::IS_ZST { Cap::ZERO } else { unsafe { Cap(capacity) } };
    ...
    #[inline]
    pub fn non_null<T>(&self) -> NonNull<T> {
        unsafe { core::mem::transmute(self.ptr) }
    }
    ...
    #[inline]
    pub fn capacity(&self, elem_size: usize) -> usize {
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline]
    pub fn reserve(&mut self, len: usize, additional: usize, elem_layout: Layout) {
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline(never)]
    pub fn grow_one(&mut self, elem_layout: Layout) {
    ...
    /// Aborts on OOM.
    #[cfg(not(no_global_oom_handling))]
    pub fn reserve_exact(&mut self, len: usize, additional: usize, elem_layout: Layout) {
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline]
    pub fn shrink_to_fit(&mut self, cap: usize, elem_layout: Layout) {
    ...
    pub fn drop(&mut self, elem_layout: Layout) {
