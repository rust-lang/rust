...
impl RcValuePointer {
    /// Creates a new `RcValuePointer` from a raw pointer to a referenced allocation.
    ///
    /// # Safety
    ///
    /// Caller must ensure that `ptr` is a pointer to some valid reference-counted allocation that
    ...
    pub(crate) unsafe fn from_allocation_ptr(
        ...
        // SAFETY: Caller guarantees that `allocation_ptr` point to some reference-counted
    ...
    /// # Safety
    ///
    /// Caller must ensure that `value_ptr` is a valid pointer to the value location inside some valid
    /// reference-counted allocation.
    #[inline]
