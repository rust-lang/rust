    ...
    A: Allocator,
{
    ...
    unsafe {
        let allocation_ptr = value_ptr.as_ptr().byte_sub(value_offset);
    ...
    A: Allocator,
{
    ...
    // SAFETY: `try_allocate_with_in` is guaranteed to pass a valid pointer to the closure returned
    // By `clone_to_uninit`.
    ...
    A: Allocator,
{
    ...
    // SAFETY: Caller guarantees the validify of `src_ptr`, `value_ptr` is also guaranteed to be
    // valid.
    unsafe {
    ...
    A: Allocator,
{
    ...
    // SAFETY: `rc_layout` is derived from `src`, so it must be valid.
...
where
    ...
    A: Allocator,
{
    // SAFETY: Caller guarantees we can derive a valid `RcLayout` from `src`.
    ...
    A: Allocator,
{
    ...
    // SAFETY: `allocate_with_in` is guaranteed to call the closure returned by `clone_to_uninit`
