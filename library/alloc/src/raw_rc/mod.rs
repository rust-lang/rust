...
pub(crate) unsafe trait RefCounter: Sized {
    ...
    /// Increments the reference counter. If `self` needs to be called with by both `downgrade` and
    /// `is_unique` concurrently, both operations will be performed atomically.
    fn downgrade(&self);
    ...
    /// called with by both `downgrade` and `is_unique` concurrently, both operations will be
