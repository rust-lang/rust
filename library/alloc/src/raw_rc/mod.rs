...
pub(crate) enum MakeMutStrategy {
    ...
    /// pointers.
    Move,
    /// There are more than one strong pointers that reference the value.
    Clone,
}
...
/// Initialize reference counters in a reference-counted allocation pointed to by `value_ptr`
