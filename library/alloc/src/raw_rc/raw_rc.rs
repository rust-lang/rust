    ...
    T: ?Sized,
{
        ...
        R: RcOps,
    {
        ...
        unsafe { inner::<R>(self.weak.as_ptr().cast()) }.map(|ptr| unsafe {
            super::non_null_with_metadata_of(ptr, self.weak.as_ptr()).as_mut()
        })
    }
            ...
            if let Some(strategy) = R::make_mut(self.ref_counts()) {
                ...
                match strategy {
                    MakeMutStrategy::Move => {
                        ...
                        let new_ptr = super::non_null_with_metadata_of(new_ptr, self.weak.as_ptr());
