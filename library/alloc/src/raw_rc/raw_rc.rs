    ...
    T: ?Sized,
{
        ...
        T: CloneToUninit,
    {
        ...
        // SAFETY: `ptr` is allocated from `alloc`.
    ...
    // # Safety
    //
    // `T` must be safe to be reinterpreted as `U`.
    pub(crate) unsafe fn cast<U>(self) -> RawRc<U, A> {
        // SAFETY: Caller guarantees the validity of the casting.
    ...
    //
    // - `T` must be safe to be reinterpreted as `U`.
    // - `f` must return an reinterpreted pointer to the contained value.
    #[inline]
    ...
    where
        ...
        R: RefCounter,
    {
        // SAFETY: Caller guarantees the validity of `ptr` and the consistency of `R`.
    ...
    where
        ...
        R: RefCounter,
    {
        // SAFETY: Caller guarantees the validity of `ptr` and `alloc`, and the consistency of `R`.
        ...
        R: RefCounter,
    {
        ...
        where
            R: RefCounter,
        {
            // SAFETY: Caller guarantees the validity of `RcValuePointer` and the consistency of `R`.
            unsafe {
        ...
        R: RefCounter,
    {
        ...
        if is_last_strong_ref {
            // SAFETY: we are the exclusive owner of the contained value, we can drop the value now.
        ...
        R: RefCounter,
    {
        ...
        // SAFETY: Caller guarantees the consistency of `R`.
        unsafe {
            inner::<R>(RcValuePointer::from_value_ptr(ptr))
                .map(|ptr| NonNull::from_raw_parts(ptr.as_ptr(), metadata).as_mut())
        }
    }
        ...
        R: RefCounter,
    {
        ...
        where
            T: ?Sized,
        {
            // SAFETY: Caller guaranttes the validity of `ptr`.
            DropGuard::new(rc, move |rc| unsafe { rc.weak.set_ptr(ptr) })
        }

        // SAFETY: See the comments inside.
        unsafe {
    ...
    pub(crate) fn ref_counts(&self) -> &crate::raw_rc::RefCounts {
        // SAFETY: `RawWeak` inside a `RawRc is never dangling.
    ...
    pub(crate) unsafe fn strong_count_from_raw<'a>(ptr: NonNull<T>) -> &'a UnsafeCell<usize> {
        // SAFETY: Caller guarantees the validity of `ptr`.
        unsafe { RawWeak::<T, A>::strong_count_from_raw_unchecked(ptr) }
    }
...
impl<T, A> RawRc<T, A> {
    ...
    where
        ...
        R: RefCounter,
    {
        // SAFETY: Caller guarantees the validify of `R`.
        let guard = unsafe { raw_weak::new_weak_guard::<T, A, R>(&mut weak) };
        let data = data_fn(&guard);

        DropGuard::dismiss(guard);

        // SAFETY: We are the only one that will access the value storage since the strong count is
        // 0.
        unsafe { RawUniqueRc::from_weak_with_value(weak, data).into_rc::<R>() }
    }

    #[cfg(not(no_global_oom_handling))]
    pub(crate) fn new_cyclic_in<F, R>(data_fn: F, alloc: A) -> Self
    where
        A: Allocator,
        F: FnOnce(&RawWeak<T, A>) -> T,
        R: RefCounter,
    ...
    #[cfg(not(no_global_oom_handling))]
    pub(crate) fn new_cyclic<F, R>(data_fn: F) -> Self
    where
        A: Allocator + Default,
        F: FnOnce(&RawWeak<T, A>) -> T,
        R: RefCounter,
...
impl<T, A> RawRc<MaybeUninit<T>, A> {
    ...
    where
        A: Allocator,
    {
        // SAFETY: The strong count is 1 in `weak`, we take ownership of the strong count.
    ...
    where
        A: Allocator,
    {
        let weak = RawWeak::new_zeroed_in::<1>(alloc);
...
impl<T, A> RawRc<[T], A> {
    ...
    pub(crate) fn into_array<const N: usize>(self) -> Result<RawRc<[T; N], A>, Self> {
        // SAFETY: The slice is checked to have length of `N`.
...
impl<T, A> RawRc<[MaybeUninit<T>], A> {
    ...
    where
        A: Allocator,
    {
        let weak = RawWeak::new_uninit_slice_in::<1>(length, alloc);
    ...
    where
        A: Allocator + Default,
    {
        let weak = RawWeak::new_uninit_slice::<1>(length);
    ...
    where
        A: Allocator,
    {
        let weak = RawWeak::new_zeroed_slice_in::<1>(length, alloc);
    ...
    where
        A: Allocator + Default,
    {
        let weak = RawWeak::new_zeroed_slice::<1>(length);
...
{
    fn as_ref(&self) -> &T {
        // SAFETY: The contained value in `RawRc` is always valid.
    ...
    A: Allocator,
{
    fn from(value: Box<T, A>) -> Self {
        ...
        // SAFETY: Dropping `ManuallyDrop<T>` frees the box memory without running the `T`
        // destructor. The result `RawRc` will take owner ship of the `T` value.
        unsafe {
    ...
    A: Allocator + Default,
{
    fn spec_from_slice(slice: &[T]) -> Self {
        ...
        // SAFETY: `ptr` is allocated with `alloc` and is properly initialized.
        unsafe { Self::from_raw_parts(ptr, alloc) }
    }
}
    ...
    A: Allocator + Default,
{
    ...
    fn from(value: &str) -> Self {
        ...
        // SAFETY: Bytes contained in `rc_of_bytes` is copied from a valid `str`, so the result is
        // also a valid `str`.
    ...
    A: Allocator,
{
    fn from(value: Vec<T, A>) -> Self {
        let src = &*value;
        let value_ptr = rc_alloc::allocate_with_value_in::<[T], A, 1>(src, value.allocator());
        let (vec_ptr, _length, capacity, alloc) = value.into_parts_with_allocator();

        // SAFETY: We are safe to free the original `Vec` with a reference to `alloc`. And
        // `value_ptr` is allocated with `alloc`, we can construct a new `RawRc` with them.
        unsafe {
            drop(Vec::from_raw_parts_in(vec_ptr.as_ptr(), 0, capacity, &alloc));
    ...
    R: RefCounter,
{
    // SAFETY: Caller guarantees the validify of `value_ptr`.
    unsafe {
        let ref_counts = value_ptr.ref_counts_ptr().as_ref();
