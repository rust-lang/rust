...
impl<T, A> RawWeak<T, A> {
    ...
    /// Consumes the `RawWeak` object and returns the contained value and the allocator, assuming
    ...
    pub(super) unsafe fn assume_init_into_inner_with_allocator<R>(mut self) -> (T, A)
    where
        A: Allocator,
        R: RefCounter,
    {
    ...
    /// - The value pointed to by `self` is initialized.
    /// - The strong reference count is zero.
    pub(super) unsafe fn assume_init_into_inner<R>(self) -> T
    where
        A: Allocator,
        R: RefCounter,
    {
        // SAFETY: Guaranteed by caller.
        unsafe { self.assume_init_into_inner_with_allocator::<R>() }.0
    }
}
