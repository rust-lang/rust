...
pub struct ThinBox<T: ?Sized> {
    ...
    ptr: with_header::WithOpaqueHeader,
...
impl<T> ThinBox<T> {
    ...
    pub fn new(value: T) -> Self {
        ...
        let ptr = with_header::WithOpaqueHeader::new(meta, value);
    ...
    pub fn try_new(value: T) -> Result<Self, core::alloc::AllocError> {
        let meta = ptr::metadata(&value);
        with_header::WithOpaqueHeader::try_new(meta, value)
            .map(|ptr| ThinBox { ptr, _marker: PhantomData })
    }
}
...
impl<Dyn: ?Sized> ThinBox<Dyn> {
    ...
    {
        if mem::size_of::<T>() == 0 {
            let ptr = with_header::WithOpaqueHeader::new_unsize_zst::<Dyn, T>(value);
            ThinBox { ptr, _marker: PhantomData }
        } else {
...
impl<T: ?Sized> ThinBox<T> {
    ...
    fn with_header(&self) -> &with_header::WithHeader<<T as Pointee>::Metadata> {
        // SAFETY: both types are transparent to `NonNull<u8>`
        unsafe { &*(core::ptr::addr_of!(self.ptr) as *const with_header::WithHeader<_>) }
    }
}
...
mod with_header {
    ...
    ///    pointed-to location.
    #[repr(transparent)]
    pub(super) struct WithHeader<H>(NonNull<u8>, PhantomData<H>);

    /// An opaque representation of `WithHeader<H>` to avoid the
    /// projection invariance of `<T as Pointee>::Metadata`.
    #[repr(transparent)]
    pub(super) struct WithOpaqueHeader(NonNull<u8>);

    impl WithOpaqueHeader {
        #[cfg(not(no_global_oom_handling))]
        pub(super) fn new<H, T>(header: H, value: T) -> Self {
            let ptr = WithHeader::new(header, value);
            Self(ptr.0)
        }

        #[cfg(not(no_global_oom_handling))]
        pub(super) fn new_unsize_zst<Dyn, T>(value: T) -> Self
        where
            Dyn: ?Sized,
            T: Unsize<Dyn>,
        {
        ...
        pub(super) fn try_new<H, T>(header: H, value: T) -> Result<Self, core::alloc::AllocError> {
    ...
    impl<H> WithHeader<H> {
        ...
        pub(super) unsafe fn drop<T: ?Sized>(&self, value: *mut T) {
            struct DropGuard<H> {
                ptr: NonNull<u8>,
                value_layout: Layout,
        ...
        pub(super) fn header(&self) -> *mut H {
            //  Safety:
        ...
        pub(super) fn value(&self) -> *mut u8 {
            self.0.as_ptr()
        }
