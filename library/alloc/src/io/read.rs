            ...
            match r.read(&mut probe) {
                Ok(n) => {
                    cfg_select! {
                        no_global_oom_handling => {
                            // SAFETY:
                            // * probe and buf are non-overlapping
                            // * buf[..len] is already initialized
                            // * buf[len..n] is initialized by copy_nonoverlapping
                            // * len + n is within the capacity of buf based on the reservation completed prior to the read
                            unsafe {
                                let len = buf.len();
                                let src = probe.as_slice().as_ptr();
