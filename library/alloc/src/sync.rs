...
unsafe impl raw_rc::RcOps for RcOps {
    ...
    unsafe fn increment_ref_count(count: &UnsafeCell<usize>) {
        let count = unsafe { Atomic::<usize>::from_ptr(count.get()) };
    ...
    unsafe fn upgrade(strong_count: &UnsafeCell<usize>) -> bool {
        let strong_count = unsafe { Atomic::<usize>::from_ptr(strong_count.get()) };
    ...
    unsafe fn downgrade(weak_count: &UnsafeCell<usize>) {
        let weak_count = unsafe { Atomic::<usize>::from_ptr(weak_count.get()) };
    ...
    unsafe fn is_unique(ref_counts: &RefCounts) -> bool {
        let strong_count = unsafe { Atomic::<usize>::from_ptr(ref_counts.strong.get()) };
        let weak_count = unsafe { Atomic::<usize>::from_ptr(ref_counts.weak.get()) };
...
impl<T: ?Sized, A: Allocator> Arc<T, A> {
    ...
    pub fn is_unique(this: &Self) -> bool {
        unsafe { this.raw_rc.is_unique::<RcOps>() }
    }
}
