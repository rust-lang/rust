...
impl<T> Arc<T> {
        ...
        F: FnOnce(&Weak<T>) -> T,
    {
        ...
        let uninit_ptr: NonNull<_> = Box::leak(Box::new(ArcInner {
            strong: atomic::AtomicUsize::new(ZERO_BIT),
            weak: atomic::AtomicUsize::new(1),
        ...
        let strong = unsafe {
            ...
            let prev_value = (*inner).strong.swap(1, Release);
            debug_assert!(is_zero(prev_value), "No prior strong references should exist");
...
impl<T, A: Allocator> Arc<T, A> {
    ...
    pub fn try_unwrap(this: Self) -> Result<T, Self> {
        if this.inner().strong.compare_exchange(1, ZERO_BIT, Relaxed, Relaxed).is_err() {
            return Err(this);
        }
...
impl<T: ?Sized, A: Allocator> Arc<T, A> {
    ...
    pub fn strong_count(this: &Self) -> usize {
        sticky_load(&this.inner().strong, Relaxed)
    }
...
impl<T: Clone, A: Allocator + Clone> Arc<T, A> {
    ...
    pub fn make_mut(this: &mut Self) -> &mut T {
        ...
        // deallocated.
        if this.inner().strong.compare_exchange(1, ZERO_BIT, Acquire, Relaxed).is_err() {
...
impl<T: ?Sized, A: Allocator> Arc<T, A> {
    ...
    fn is_unique(&mut self) -> bool {
        ...
        if self.inner().weak.compare_exchange(1, usize::MAX, Acquire, Relaxed).is_ok() {
            ...
            // is being dropped.
            let unique = sticky_load(&self.inner().strong, Acquire) == 1;
...
impl<T: ?Sized, A: Allocator> Weak<T, A> {
    ...
    pub fn strong_count(&self) -> usize {
        if let Some(inner) = self.inner() { sticky_load(&inner.strong, Relaxed) } else { 0 }
    }
    ...
    pub fn weak_count(&self) -> usize {
        if let Some(inner) = self.inner() {
            ...
            let strong = sticky_load(&inner.strong, Relaxed);
            if strong == 0 {
                0
            } else {
...
/// Loads the value of the sticky counter.
fn sticky_load(tgt: &atomic::AtomicUsize, order: atomic::Ordering) -> usize {
