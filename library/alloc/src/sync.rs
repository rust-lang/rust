...
use crate::boxed::Box;
#[cfg(not(no_global_oom_handling))]
use crate::raw_rc::MakeMut;
#[cfg(not(no_global_oom_handling))]
use crate::raw_rc::RefCounts;
use crate::raw_rc::{self, RawRc, RawWeak};
#[cfg(not(no_global_oom_handling))]
...
unsafe impl raw_rc::RcOps for RcOps {
    ...
    unsafe fn lock_strong_count(strong_count: &UnsafeCell<usize>) -> bool {
        ...
        match strong_count.compare_exchange(1, 0, Relaxed, Relaxed) {
            Ok(_) => {
                acquire!(strong_count);
    ...
    where
        ...
        A: Allocator,
    {
        let ref_counts = make_mut.ref_counts();
        let strong_count = unsafe { AtomicUsize::from_ptr(ref_counts.strong.get()) };
        let weak_count = unsafe { AtomicUsize::from_ptr(ref_counts.weak.get()) };
        ...
        if strong_count.compare_exchange(1, 0, Acquire, Relaxed).is_ok() {
            if weak_count.load(Relaxed) == 1 {
                ...
                // SAFETY: We have set strong count to 0.
                unsafe { make_mut.by_move() };
            }
        } else {
            make_mut.by_clone();
        }
...
impl<T> Arc<T> {
    ...
    where
        F: FnOnce(&Weak<T>) -> T,
    {
        Self { raw_rc: unsafe { RawRc::new_cyclic::<_, RcOps>(weak_fn_to_raw_weak_fn(data_fn)) } }
    }
...
impl<T, A: Allocator> Arc<T, A> {
        ...
        F: FnOnce(&Weak<T, A>) -> T,
    {
        Self {
            raw_rc: unsafe {
                RawRc::new_cyclic_in::<_, RcOps>(weak_fn_to_raw_weak_fn(data_fn), alloc)
            },
        }
    }
    ...
    pub fn try_unwrap(this: Self) -> Result<T, Self> {
        match unsafe { RawRc::try_unwrap::<RcOps>(Arc::into_raw_rc(this)) } {
            Ok(value) => Ok(value),
    ...
    pub fn into_inner(this: Self) -> Option<T> {
        unsafe { Arc::into_raw_rc(this).into_inner::<RcOps>() }
    }
}
...
impl<T, A: Allocator> Arc<mem::MaybeUninit<T>, A> {
    ...
    pub unsafe fn assume_init(self) -> Arc<T, A> {
        Arc { raw_rc: unsafe { Arc::into_raw_rc(self).assume_init() } }
    }
}
...
impl<T: ?Sized, A: Allocator> Arc<T, A> {
    fn into_raw_rc(this: Arc<T, A>) -> RawRc<T, A> {
        unsafe { ptr::read(&ManuallyDrop::new(this).raw_rc) }
    }

    fn raw_strong_count(&self) -> &AtomicUsize {
        unsafe { AtomicUsize::from_ptr(self.raw_rc.strong_count().get()) }
    }

    fn raw_weak_count(&self) -> &AtomicUsize {
        unsafe { AtomicUsize::from_ptr(self.raw_rc.weak_count().get()) }
    }
    ...
    pub fn into_raw(this: Self) -> *const T {
        Arc::into_raw_rc(this).into_raw().as_ptr()
    }
    ...
    pub fn into_raw_with_allocator(this: Self) -> (*const T, A) {
        let (ptr, alloc) = Arc::into_raw_rc(this).into_raw_parts();
...
impl<T: Clone, A: Allocator> Arc<T, A> {
    ...
    pub fn unwrap_or_clone(this: Self) -> T {
        unsafe { Arc::into_raw_rc(this).unwrap_or_clone::<RcOps>() }
    }
}
...
impl<A: Allocator> Arc<dyn Any + Send + Sync, A> {
    ...
    where
        T: Any + Send + Sync,
    {
        match Arc::into_raw_rc(self).downcast::<T>() {
    ...
    where
        T: Any + Send + Sync,
    {
        Arc { raw_rc: unsafe { Arc::into_raw_rc(self).downcast_unchecked() } }
    }
}
...
impl<T: ?Sized, A: Allocator> Weak<T, A> {
    fn into_raw_weak(self) -> RawWeak<T, A> {
        unsafe { ptr::read(&ManuallyDrop::new(self).raw_weak) }
    }

    fn raw_strong_count(&self) -> Option<&AtomicUsize> {
        self.raw_weak
            .strong_count()
            .map(|strong_count| unsafe { AtomicUsize::from_ptr(strong_count.get()) })
    }
...
impl<T: ?Sized, A: Allocator> Weak<T, A> {
    ...
    where
        A: Clone,
    {
        self.raw_weak.upgrade::<RcOps>().map(|raw_rc| Arc { raw_rc })
    }
    ...
    pub fn weak_count(&self) -> usize {
        if let Some(ref_counts) = self.raw_weak.ref_counts() {
            let weak = unsafe { AtomicUsize::from_ptr(ref_counts.weak.get()) }.load(Acquire);
            let strong = unsafe { AtomicUsize::from_ptr(ref_counts.strong.get()) }.load(Relaxed);
            if strong == 0 {
                0
            } else {
...
struct SliceArcInnerForStatic {
    padding:
        MaybeUninit<[u8; MAX_STATIC_INNER_SLICE_ALIGNMENT.saturating_sub(size_of::<RefCounts>())]>,
    ref_counts: RefCounts,
    value: [u8; 1],
}

#[cfg(not(no_global_oom_handling))]
unsafe impl Sync for SliceArcInnerForStatic {}
    ...
    fn default() -> Self {
        unsafe {
            let ptr = NonNull::from(core::ffi::CStr::from_bytes_with_nul_unchecked(
                &STATIC_INNER_SLICE.value,
            ));

            Self { raw_rc: RawRc::from_raw(ptr).clone::<RcOps>() }
        }
    }
}
        ...
        if mem::align_of::<T>() <= MAX_STATIC_INNER_SLICE_ALIGNMENT {
            unsafe {
                let ptr = NonNull::slice_from_raw_parts(
                    NonNull::from(&STATIC_INNER_SLICE.value).cast(),
                    0,
                );

                return Self { raw_rc: RawRc::from_raw(ptr).clone::<RcOps>() };
            }
        }
