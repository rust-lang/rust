...
impl<T> RawVec<T, Global> {
    ...
    pub(crate) fn with_capacity(capacity: usize) -> Self {
        ...
        let inner = RawVecInner::with_capacity_with_token_hint::<T>(capacity, T::LAYOUT);
...
impl RawVecInner<Global> {
    // `with_capacity_with_token_hint` is always used for LLVM AllocToken and heap partitioning
    // support.
    ...
    /// `alloc::alloc_array_with_token_hint`) to be annotated with the allocation token hint for
    /// `T` (i.e., the element type of the allocated array).
    #[cfg(all(sanitize = "alloc-token", not(any(no_global_oom_handling, test))))]
    #[must_use]
    #[inline]
    fn with_capacity_with_token_hint<T>(capacity: usize, elem_layout: Layout) -> Self {
        match Self::try_allocate_in_with_token_hint::<T>(capacity, elem_layout) {
            Ok(res) => res,
            Err(err) => handle_error(err),
        }
    }
    ...
    #[cfg(all(sanitize = "alloc-token", not(any(no_global_oom_handling, test))))]
    fn try_allocate_in_with_token_hint<T>(
        capacity: usize,
        elem_layout: Layout,
        ...
        // above).
        let ptr = unsafe { crate::alloc::alloc_array_with_token_hint::<T>(layout) };
