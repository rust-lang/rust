...
const impl RawVecInner {
    ...
    fn grow_one_zst_allocator<A: [const] Allocator>(self, elem_layout: Layout) -> Self {
        ...
        unsafe { self.grow_one(alloc, elem_layout) }
    }
    ...
    #[cfg(not(no_global_oom_handling))]
    #[inline(always)]
    unsafe fn grow_one<A: [const] Allocator + ?Sized>(
        self,
        alloc: &A,
        elem_layout: Layout,
    ) -> Self {
        match unsafe { self.grow_amortized(self.cap.as_inner(), 1, elem_layout, alloc) } {
            Ok(inner) => inner,
            Err(err) => handle_error(err),
        }
    }
    ...
    unsafe fn grow_amortized<A: [const] Allocator + ?Sized>(
        mut self,
        len: usize,
        additional: usize,
        elem_layout: Layout,
    ...
    #[cold]
    unsafe fn finish_grow<A: [const] Allocator + ?Sized>(
        &self,
        cap: usize,
        elem_layout: Layout,
        alloc: &A,
...
impl RawVecInner {
    ...
    unsafe fn reserve<A: Allocator>(
        ...
        #[cold]
        unsafe fn do_reserve_and_handle<A: Allocator + ?Sized>(
            slf: &mut RawVecInner,
            len: usize,
            additional: usize,
            elem_layout: Layout,
            ...
            // SAFETY: Precondition passed to caller
            match unsafe { slf.grow_amortized(len, additional, elem_layout, alloc) } {
                Ok(inner) => *slf = inner,
                Err(err) => handle_error(err),
            }
        }
    ...
    unsafe fn try_reserve<A: Allocator>(
        ...
        if self.needs_to_grow(len, additional, elem_layout) {
            // SAFETY: Precondition passed to caller
            let inner = unsafe { self.grow_amortized(len, additional, elem_layout, alloc)? };
            *self = inner;
        }
        unsafe {
    ...
    /// - `elem_layout`'s size must be a multiple of its alignment
    /// - The sum of `len` and `additional` must be greater than the current capacity
    unsafe fn grow_exact<A: Allocator + ?Sized>(
        &mut self,
        len: usize,
        additional: usize,
        elem_layout: Layout,
    ...
    /// # Safety
    /// `cap <= self.capacity()`
    unsafe fn shrink_unchecked<A: Allocator + ?Sized>(
        &mut self,
        cap: usize,
        elem_layout: Layout,
        alloc: &A,
