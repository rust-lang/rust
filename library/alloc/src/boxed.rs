...
/// `alloc::alloc_typed`) to be annotated with the allocation token hint for `T`.
#[inline]
#[cfg_attr(miri, track_caller)] // even without panics, this helps for Miri backtraces
#[cfg(all(sanitize = "alloc-token", not(no_global_oom_handling)))]
fn box_new_uninit_typed<T>() -> *mut u8 {
    ...
    let ptr = unsafe { crate::alloc::alloc_typed::<T>(layout) };
...
impl<T> Box<T> {
    ...
    pub fn new(x: T) -> Self {
        ...
        let ptr = box_new_uninit_typed::<T>() as *mut T;
