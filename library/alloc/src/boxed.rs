...
impl<T> Box<[T]> {
    ...
    pub fn try_new_uninit_slice(len: usize) -> Result<Box<[mem::MaybeUninit<T>]>, AllocError> {
        unsafe {
            ...
            Ok(Box::from_raw_slice_parts_in(ptr.as_mut_ptr() as *mut _, len, Global))
        }
    }
...
impl<T, A: Allocator> Box<[T], A> {
    ...
    fn allocate_in(capacity: usize, init: AllocInit, alloc: A) -> Box<[mem::MaybeUninit<T>], A> {
        ...
        if mem::size_of::<T>() == 0 || capacity == 0 {
            ...
            unsafe { Box::from_raw_slice_parts_in(ptr.as_ptr().cast(), capacity, alloc) }
        }
    }
...
impl<T, A: Allocator> Box<[mem::MaybeUninit<T>], A> {
    ...
    fn set_ptr_and_cap(&mut self, ptr: core::ptr::NonNull<[u8]>, cap: usize) {
        // Allocators currently return a `NonNull<[u8]>` whose length matches
        // the size requested. If that ever changes, the capacity here should
        // change to `ptr.len() / mem::size_of::<T>()`.
        self.0 = unsafe {
            Unique::new_unchecked(core::slice::from_raw_parts_mut(ptr.cast().as_ptr(), cap))
        }
    }
    ...
    fn grow_amortized(&mut self, len: usize, additional: usize) -> Result<(), TryReserveError> {
        ...
        let cap = core::cmp::max(Box::<[T]>::MIN_NON_ZERO_CAP, cap);

        let new_layout = Layout::array::<T>(cap);

        // `finish_grow` is non-generic over `T`.
        let ptr = finish_grow(new_layout, self.current_memory(), &mut self.1)?;
    ...
    fn current_memory(&self) -> Option<(core::ptr::NonNull<u8>, Layout)> {
    ...
    fn shrink(&mut self, cap: usize) -> Result<(), TryReserveError> {
        ...
        let ptr = unsafe {
            ...
            self.1.shrink(ptr, layout, new_layout).map_err(|_| TryReserveErrorKind::AllocError {
                layout: new_layout,
                non_exhaustive: (),
            })?
        };
...
impl<T: Copy> From<&[T]> for Box<[T]> {
    ...
    fn from(slice: &[T]) -> Box<[T]> {
        ...
        unsafe {
            ptr::copy_nonoverlapping(slice.as_ptr(), buf.0.as_ptr().cast(), len);
            buf.assume_init()
        }
    }
}
