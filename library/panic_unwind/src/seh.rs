...
//!
//! 1. The `panic` function calls the standard Windows function `_CxxThrowException` to throw a
//!    C++-like exception, triggering the unwinding process.
//! 2. All landing pads generated by the compiler use the personality function `__CxxFrameHandler3`,
//!    a function in the CRT, and the unwinding code in Windows will use this personality function
//!    to execute all cleanup code on the stack.
//! 3. All compiler-generated calls to `invoke` have a landing pad set as a `cleanuppad` LLVM
//!    instruction, which indicates the start of the cleanup routine. The personality (in step 2,
//!    defined in the CRT) is responsible for running the cleanup routines.
//! 4. Eventually the "catch" code in the `try` intrinsic (generated by the compiler) is executed
//!    and indicates that control should come back to Rust. This is done via a `catchswitch` plus a
//!    `catchpad` instruction in LLVM IR terms, finally returning normal control to the program with
//!    a `catchret` instruction.
//!
//! Some specific differences from the gcc-based exception handling are:
//!
//! * Rust has no custom personality function, it is instead *always* `__CxxFrameHandler3`.
//!   Additionally, no extra filtering is performed, so we end up catching any C++ exceptions that
//!   happen to look like the kind we're throwing. Note that throwing an exception into Rust is
//!   undefined behavior anyway, so this should be fine.
//! * We've got some data to transmit across the unwinding boundary, specifically a `Box<dyn Any +
//!   Send>`. Like with Dwarf exceptions these two pointers are stored as a payload in the exception
//!   itself. On MSVC, however, there's no need for an extra heap allocation because the call stack
//!   is preserved while filter functions are being executed. This means that the pointers are
//!   passed directly to `_CxxThrowException` which are then recovered in the filter function to be
//!   written to the stack frame of the `try` intrinsic.
//!
