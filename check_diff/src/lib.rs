...
pub enum CreateDiffError {
    /// Couldn't create a diff because the source formatter failed to format the input.
    SourceFormatterFailed(FormatCodeError),
    /// Couldn't create a diff because the target formatter failed to format the input.
    TargetFormatterFailed(FormatCodeError),
    /// Couldn't create a diff because both rustfmt binaries failed to format the input
    BothRustfmtFailed {
        source: FormatCodeError,
        target: FormatCodeError,
    },
}
...
impl Display for Diff {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let patch = diffy::create_patch(self.source.as_str(), self.target.as_str());
        write!(f, "{}", patch)
    }
}
...
where
    ...
    T: CodeFormatter,
{
    /// Creates a diff generated by running the source and target formatters on the same file path
    pub fn create_diff<P: AsRef<Path>>(&self, path: P) -> Result<Diff, CreateDiffError> {
        let source_format = self.source.format_code_from_path(&path);
        let target_format = self.target.format_code_from_path(&path);
...
pub fn compile_rustfmt<T: AsRef<str>>(
    ...
    let source_runner = build_rustfmt_from_src(
        dest.join("source_rustfmt"),
        dest,
        edition,
        style_edition,
        config,
    ...
    info!(
        ...
        dynamic_library_path_env_var, source_runner.dynamic_library_path
    );
    info!("TARGET_BIN {}", target_runner.get_binary_version()?);
    info!(
        "Runtime dependencies for ({}) rustfmt -- {}: {}",
        feature_branch, dynamic_library_path_env_var, target_runner.dynamic_library_path
    );
...
/// Calculates the number of errors when running the compiled binary and the target binary on the
/// repo specified with the specific configs.
pub fn check_diff_for_file<'repo, P: AsRef<Path>, F: AsRef<Path>>(
    diff_checker: &DiffChecker<impl CodeFormatter, impl CodeFormatter>,
    ...
    match diff_checker.create_diff(file.as_ref()) {
        Ok(diff) => {
            if !diff.is_empty() {
                Err((diff, file, repo))
            } else {
        ...
        Err(CreateDiffError::SourceFormatterFailed(e)) => {
            debug!(
        ...
        Err(CreateDiffError::TargetFormatterFailed(e)) => {
            debug!(
        ...
        Err(CreateDiffError::BothRustfmtFailed { source, target }) => {
            debug!(
        ...
        for _ in 0..u8::from(worker_threads) {
            ...
            s.spawn(move || {
                while let Ok((file, repo)) = rx.recv() {
                    if let Err(e) = check_diff_for_file(diff_checker, repo, file) {
