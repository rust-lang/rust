...
impl Display for Diff {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let patch = diffy::create_patch(self.src_format.as_str(), self.target.as_str());
        write!(f, "{}", patch)
    }
}
...
impl<T, S> CheckDiffRunners<T, S> {
    pub fn new(target: T, src_runner: S) -> Self {
        Self {
            target,
            src_runner,
        }
    ...
    S: CodeFormatter,
{
    ...
    pub fn create_diff<P: AsRef<Path>>(&self, path: P) -> Result<Diff, CreateDiffError> {
        ...
        match (src_format, target_format) {
            (Ok(s), Ok(t)) => Ok(Diff {
                src_format: s,
                target: t,
            }),
            ...
            (Err(src_error), Err(target_error)) => {
                // Both main formatting and target formatting failed
...
pub fn check_diff_for_file<'repo, P: AsRef<Path>, F: AsRef<Path>>(
    ...
    match runners.create_diff(file.as_ref()) {
        ...
        Err(CreateDiffError::BothRustfmtFailed { src, target }) => {
            debug!(
...
pub fn check_diff<'repo, P, T, M>(
    runners: &CheckDiffRunners<T, M>,
