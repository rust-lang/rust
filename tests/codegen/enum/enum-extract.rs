...
const SOME_FOUR: Option<u8> = Some(4);

#[no_mangle]
fn use_option_from_const() -> u8 {
    // CHECK-LABEL: @use_option_from_const()
    // CHECK-NEXT: start:
    // OPT-NEXT: ret i8 4
    // DBG: %[[PAYLOAD:.+]] = load i8, ptr getelementptr inbounds
    // DBG: ret i8 %[[PAYLOAD]]
    if let Some(val) = SOME_FOUR { val } else { unreachable!() }
}
