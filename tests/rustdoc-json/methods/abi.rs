...
pub trait Bar {
    ...
    //@ is "$.index[*][?(@.name=='trait_abi_c')].inner.function.header.abi" '{"C": {"can_unwind": false}}'
    extern "C" fn trait_abi_c() {}

    //@ is "$.index[*][?(@.name=='trait_abi_system')].inner.function.header.abi" '{"System": {"can_unwind": false}}'
    extern "system" fn trait_abi_system() {}

    //@ is "$.index[*][?(@.name=='trait_abi_c_unwind')].inner.function.header.abi" '{"C": {"can_unwind": true}}'
    extern "C-unwind" fn trait_abi_c_unwind() {}

    //@ is "$.index[*][?(@.name=='trait_abi_system_unwind')].inner.function.header.abi" '{"System": {"can_unwind": true}}'
