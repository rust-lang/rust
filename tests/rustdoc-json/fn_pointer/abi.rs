...
//@ is "$.index[*][?(@.name=='AbiC')].inner.type_alias.type.function_pointer.header.abi" '{"C": {"can_unwind": false}}'
pub type AbiC = extern "C" fn();

//@ is "$.index[*][?(@.name=='AbiSystem')].inner.type_alias.type.function_pointer.header.abi" '{"System": {"can_unwind": false}}'
pub type AbiSystem = extern "system" fn();

//@ is "$.index[*][?(@.name=='AbiCUnwind')].inner.type_alias.type.function_pointer.header.abi" '{"C": {"can_unwind": true}}'
pub type AbiCUnwind = extern "C-unwind" fn();

//@ is "$.index[*][?(@.name=='AbiSystemUnwind')].inner.type_alias.type.function_pointer.header.abi" '{"System": {"can_unwind": true}}'
