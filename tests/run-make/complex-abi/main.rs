...
fn main() {
    sqrt();
    #[cfg(not(any(target_family = "wasm", target_env = "msvc", target_vendor = "apple")))]
    mul();
    #[cfg(not(any(target_family = "wasm", target_env = "msvc", target_vendor = "apple")))]
    div();
    pass_simple();
...
type c_longdouble = cfg_select! {
    target_arch = "aarch64" => {
        cfg_select! {
            any(windows, target_vendor = "apple", target_os = "android") => f64,
            _ => f128,
        }
    }
    _ => (),
...
/// Test calling the complex square root libc function.
fn sqrt() {
    unsafe extern "C" {
...
/// Test calling the complex multiplication libcall.
#[cfg(not(any(target_family = "wasm", target_env = "msvc", target_vendor = "apple")))]
fn mul() {
    unsafe extern "C" {
...
/// Test calling the complex division libcall.
#[cfg(not(any(target_family = "wasm", target_env = "msvc", target_vendor = "apple")))]
fn div() {
    unsafe extern "C" {
...
/// Pass and return a complex value, check input and output are equal.
fn pass_simple() {
    #[rustfmt::skip]
...
/// Check that a complex number after a `c_int` argument is correctly aligned.
fn aligned_int() {
    #[rustfmt::skip]
    #[link(name = "test", kind = "static")]
    unsafe extern "C" {
...
/// Check that a complex number after a `c_float` argument is correctly aligned.
fn aligned_float() {
    #[rustfmt::skip]
    #[link(name = "test", kind = "static")]
    unsafe extern "C" {
...
/// Assuming 8 GPRs, put a complex number right at the boundary where it only partially fits in the
/// remaining GPRs. ABIs can either split the value or pass it completely in memory.
#[rustfmt::skip]
fn partial_gpr() {
    #[link(name = "test", kind = "static")]
    unsafe extern "C" {
...
/// Same as partial_gpr but test an argument following the complex number.
///
/// This test is based on 32-bit powerpc behavior where a complex number in this position is moved to
/// the stack (and not split), and remaining registers are consumed so that any subsequent arguments
/// are also passed via the stack.
#[rustfmt::skip]
fn spill_gpr() {
    #[link(name = "test", kind = "static")]
    unsafe extern "C" {
...
/// Same as spill_gpr but for float registers.
#[rustfmt::skip]
fn spill_fpr() {
    #[link(name = "test", kind = "static")]
    unsafe extern "C" {
