...
pub const DIFFERENT_ALLOC: usize = {
    ...
    //~| NOTE_NONVIRAL not both derived from the same allocation
    offset as usize
};
...
pub const NOT_MULTIPLE_OF_SIZE: isize = {
    ...
    //~| NOTE_NONVIRAL 1_isize cannot be divided by 2_isize without remainder
};
...
const OUT_OF_BOUNDS_1: isize = {
    ...
    //~| NOTE_NONVIRAL the memory range between them is not in-bounds of an allocation
};
...
const WRONG_ORDER_UNSIGNED: usize = {
    ...
    //~| NOTE_NONVIRAL first pointer has smaller offset than second: 0 < 8
};
