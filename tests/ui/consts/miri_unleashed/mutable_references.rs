...
//~| NOTE_NONVIRAL pointing to read-only memory
...
const SUBTLE: &mut i32 = unsafe {
    //~^ ERROR: it is undefined behavior to use this value
    //~| NOTE_NONVIRAL constructing invalid value: encountered reference to mutable memory in `const`
...
//~^ ERROR it is undefined behavior to use this value
//~| NOTE_NONVIRAL `UnsafeCell` in read-only memory
// Same with a const:
...
//~| NOTE_NONVIRAL accesses mutable global memory
