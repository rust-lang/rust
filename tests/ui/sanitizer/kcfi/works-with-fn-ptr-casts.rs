...
fn main() {
    ...
    // The methods below are transformed, but KCFI can attach one type id to a function only, so
    // they are reified, and the ReifyShims for them are not transformed, as they are created with
    // ReifyReason::FnPtr (i.e., encoded with the USE_CONCRETE_SELF option), which are the ones
    // tested at the indirect calls.
    ...
    // The ReifyShim for bar is not transformed, as it is created with ReifyReason::FnPtr
