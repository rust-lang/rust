...
fn main() {
    unsafe {
        ...
        simd_masked_load(Simd::<i8, 8>([-1, 0, -1, -1, 0, 0, 0, 0]), arr.as_ptr(), default, 1);
        //~^ ERROR expected third argument with length 8 (same as input type `Simd<i8, 8>`), found `Simd<u8, 4>` with length 4

        simd_masked_load(Simd::<i8, 4>([-1, 0, -1, -1]), arr.as_ptr() as *const i8, default, 1);
        ...
        simd_masked_load(Simd::<i8, 4>([-1, 0, -1, -1]), arr.as_ptr(), Simd::<u32, 4>([9; 4]), 1);
        ...
        simd_masked_load(Simd::<f32, 4>([1.0, 0.0, 1.0, 1.0]), arr.as_ptr(), default, 1);
        //~^ ERROR expected mask element type to be an integer, found `f32`

        simd_masked_load(Simd([!0u8; 4]), arr.as_ptr(), default, 3);
        //~^ ERROR `alignment` is not a power of two

        simd_masked_store(Simd([-1i8; 4]), arr.as_ptr(), Simd([5u32; 4]), 1);
        ...
        simd_masked_store(Simd([-1i8; 4]), arr.as_ptr(), Simd([5u8; 4]), 1);
        ...
        simd_masked_store(Simd([-1i8; 4]), arr.as_mut_ptr(), Simd([5u8; 2]), 1);
        //~^ ERROR expected third argument with length 4 (same as input type `Simd<i8, 4>`), found `Simd<u8, 2>` with length 2

        simd_masked_store(Simd([1f32; 4]), arr.as_mut_ptr(), Simd([5u8; 4]), 1);
        //~^ ERROR expected mask element type to be an integer, found `f32`

        simd_masked_store(Simd([-1i8; 4]), arr.as_mut_ptr(), Simd([5u8; 4]), 3);
        //~^ ERROR `alignment` is not a power of two
    }
}
