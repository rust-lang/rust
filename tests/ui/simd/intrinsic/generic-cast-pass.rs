...
make_runtime_and_compiletime! {
    fn main() {
        unsafe {
            ...
            assert_eq_const_safe!(i[0], u[0] as i16);
            assert_eq_const_safe!(i[1], u[1] as i16);
        }

        unsafe {
            let f: V::<f32> = Simd([i16::MIN as f32, i16::MAX as f32]);
            let i: V<i16> = simd_cast(f);
            assert_eq_const_safe!(i[0], f[0] as i16);
            assert_eq_const_safe!(i[1], f[1] as i16);
        }
        ...
        unsafe {
            // We would like to do isize::MIN..=isize::MAX, but those values are not representable
            // in an f64, so we clamp to the range of an i32 to prevent running into UB.
            let f: V::<f64> = Simd([
                max(isize::MIN, i32::MIN as isize) as f64,
                min(isize::MAX, i32::MAX as isize) as f64,
            ]);
            let i: V<isize> = simd_cast(f);
            assert_eq_const_safe!(i[0], f[0] as isize);
            assert_eq_const_safe!(i[1], f[1] as isize);
        }
        ...
        unsafe {
            ...
            assert_eq_const_safe!(u[0], f[0] as usize);
            assert_eq_const_safe!(u[1], f[1] as usize);
        }
    }
}
