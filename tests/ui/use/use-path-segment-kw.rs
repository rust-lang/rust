...
mod foo {
    pub mod bar {
        ...
        use ::self; //[e2021]~ ERROR extern prelude cannot be imported
        //[e2015]~^ ERROR imports need to be explicitly named
        //[e2015]~^^ ERROR `self` imports are only allowed within a { } list
        use ::self as _self2; //[e2021]~ ERROR extern prelude cannot be imported
        //[e2015]~^ ERROR `self` imports are only allowed within a { } list
        use ::{self}; //[e2021]~ ERROR extern prelude cannot be imported
        //[e2015]~^ ERROR imports need to be explicitly named
        pub use ::{self as _nested_self2}; //[e2021]~ ERROR extern prelude cannot be imported
...
fn main() {
    ...
    foo::bar::baz::inner();
    foo::bar::_nested_self2::outer(); //[e2021]~ ERROR failed to resolve: could not find `_nested_self2` in `bar`
