...
fn compare_const<'a, 'b>(x: *const &mut &'a i32, y: *const &mut &'b i32) { //~ ERROR one or more
    x == y;
    //~^ ERROR lifetime may not live long enough
    //~| ERROR lifetime may not live long enough
}

fn compare_mut<'a, 'b>(x: *mut &'a i32, y: *mut &'b i32) { //~ ERROR one or more
    x == y;
    //~^ ERROR lifetime may not live long enough
    //~| ERROR lifetime may not live long enough
}

fn compare_fn_ptr<'a, 'b, 'c>(f: fn(&'c mut &'a i32), g: fn(&'c mut &'b i32)) { //~ ERROR one or
    f == g;
    //~^ ERROR lifetime may not live long enough
    //~| ERROR lifetime may not live long enough
}
