...
fn test_0010_local_fully_reinit_and_use_struct() {
    ...
    drop(s);
    s.x = 10;
    //[classic]~^ ERROR assign to part of moved value: `s` [E0382]
    s.y = Box::new(20);
    use_fully!(struct s);
}

fn test_0011_local_fully_reinit_and_use_tuple() {
    let mut t: T = (0, Box::new(0));
    drop(t);
    t.0 = 10;
    //[classic]~^ ERROR assign to part of moved value: `t` [E0382]
    t.1 = Box::new(20);
    use_fully!(tuple t);
}
...
fn test_1000_field_fully_init_and_use_struct() {
    let q: Q<S<B>>;
    q.r.f.x = 10; //[classic]~ ERROR E0381
    //[classic]~^ ERROR E0658
    q.r.f.y = Box::new(20);
...
fn test_1001_field_fully_init_and_use_tuple() {
    let q: Q<T>;
    q.r.f.0 = 10; //[classic]~ ERROR E0381
    //[classic]~^ ERROR E0658
    q.r.f.1 = Box::new(20);
...
fn test_1010_field_fully_reinit_and_use_struct() {
    let mut q: Q<S<B>> = Q::new(S::new());
    drop(q.r);
    q.r.f.x = 10;
    //[classic]~^ ERROR assign to part of moved value: `q.r` [E0382]
    q.r.f.y = Box::new(20);
    use_fully!(struct q.r.f);
}

fn test_1011_field_fully_reinit_and_use_tuple() {
    let mut q: Q<T> = Q::new((0, Box::new(0)));
    drop(q.r);
    q.r.f.0 = 10;
    //[classic]~^ ERROR assign to part of moved value: `q.r` [E0382]
    q.r.f.1 = Box::new(20);
...
fn issue_26996() {
    ...
    match c {
        c2 => {
            c.0 = 2; //[classic]~ ERROR assign to part of moved value
            assert_eq!(c2.0, 1);
        }
    }
}
...
fn issue_27021() {
    ...
    match c {
        c2 => {
            (c.1).0 = 2; //[classic]~ ERROR assign to part of moved value
            assert_eq!((c2.1).0, 1);
        }
    }
    ...
    match c.1 {
        c2 => {
            ((c.1).1).0 = 3; //[classic]~ ERROR assign to part of moved value
            assert_eq!((c2.1).0, 1);
        }
    }
}
