...
impl FnMut<(isize,)> for SFn {
    extern "rust-call" fn call_mut(&mut self, args: (isize,)) -> isize {
        self.call(args)
    }
}
...
impl FnOnce<(isize,)> for SFn {
    type Output = isize;
    extern "rust-call" fn call_once(self, args: (isize,)) -> isize {
        self.call(args)
    }
}

impl std::ops::Callable<(isize,)> for SFn {}
...
impl std::ops::Callable<(isize,)> for SFnMut {}
