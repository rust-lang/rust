...
where
    ...
    #[allow(unused)] T: Trait, //~ ERROR the `allow` attribute cannot be used on where predicates
    #[allow(unused)] 'a: 'static, //~ ERROR the `allow` attribute cannot be used on where predicates
