...
fn main() {
    ...
    other_item.method();
    //~^ ERROR no method named `method` found for struct `RefCell<Struct<u32>>` in the current scope [E0599]
    //~| HELP use `.borrow()` to borrow the `Struct<u32>`, panicking if a mutable borrow exists

    other_item.some_mutable_method();
    //~^ ERROR no method named `some_mutable_method` found for struct `RefCell<Struct<u32>>` in the current scope [E0599]
    ...
    another_item.method();
    //~^ ERROR no method named `method` found for struct `std::sync::Mutex<Struct<u32>>` in the current scope [E0599]
    ...
    another_item.method();
    //~^ ERROR no method named `method` found for struct `RwLock<Struct<u32>>` in the current scope [E0599]
    ...
    //~^ ERROR no method named `some_mutable_method` found for struct `RwLock<Struct<u32>>` in the current scope [E0599]
