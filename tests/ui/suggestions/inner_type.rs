...
fn main() {
    ...
    another_item.method();
    //~^ ERROR no method named `method` found for struct `std::sync::RwLock` in the current scope [E0599]
    ...
    //~^ ERROR no method named `some_mutable_method` found for struct `std::sync::RwLock` in the current scope [E0599]
