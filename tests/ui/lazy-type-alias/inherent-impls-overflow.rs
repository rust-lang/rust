...
//[next]~| ERROR overflow evaluating the requirement `Poly1<(T,)> well-formed`
type Poly1<T> = Poly0<(T,)>;
//[current]~^ ERROR  overflow normalizing the type alias `Poly1<(((((((...,),),),),),),)>`
//[next]~^^ ERROR type parameter `T` is only used recursively
//[next]~| ERROR overflow evaluating the requirement `Poly0<(T,)> well-formed`

impl Poly0<()> {}
//[current]~^ ERROR overflow normalizing the type alias `Poly1<(((((((...,),),),),),),)>`
//[next]~^^ ERROR overflow evaluating the requirement `Poly0<()> well-formed`
