...
fn do_test() {
    let s = S;
    f(s);
    assert_eq!(1, DROP.load(Ordering::SeqCst));
    assert_eq!(1, DROP_S.load(Ordering::SeqCst));
    let t = T { i: 1 };
    g(t);
    assert_eq!(1, DROP_T.load(Ordering::SeqCst));
}

fn main() {
    do_test();
    assert_eq!(2, DROP.load(Ordering::SeqCst));
    assert_eq!(1, DROP_S.load(Ordering::SeqCst));
    assert_eq!(1, DROP_T.load(Ordering::SeqCst));
}
