...
#[rustc_dump_variances] // Combine - and + to yield o
struct Derived2<'a, 'b:'a, 'c> { //~ ERROR ['a: o, 'b: o, 'c: *]
    //~^ ERROR: `'c` is never used
    f: Base<'a, 'a, 'b, 'c>
}

#[rustc_dump_variances] // Combine + and o to yield o (just pay attention to 'a here)
struct Derived3<'a:'b, 'b, 'c> { //~ ERROR ['a: o, 'b: +, 'c: *]
    //~^ ERROR: `'c` is never used
    f: Base<'a, 'b, 'a, 'c>
}

#[rustc_dump_variances] // Combine + and * to yield + (just pay attention to 'a here)
