...
// test that the lint can be properly `allow`ed on the whole extern block
#[allow(improper_ctypes)]
extern "C" {
    pub fn attr_on_block(_: &str);
}

// test that the lint can properly be `allow`ed on the fn itself
extern "C" {
    #[allow(improper_ctypes)]
    pub fn attr_on_fn(_: &str);
}
