...
fn foo(val: MyStruct) {
    test(val);
    //[with_clause]~^     ERROR evaluate(Binder { value: TraitClause(<Helper as HelperTrait>, polarity:Positive), bound_vars: [] }) = Ok(EvaluatedToOkModuloRegions)
    //[without_clause]~^^ ERROR evaluate(Binder { value: TraitClause(<Helper as HelperTrait>, polarity:Positive), bound_vars: [] }) = Ok(EvaluatedToOk)
}
