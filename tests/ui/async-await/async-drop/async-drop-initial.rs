...
fn main() {
    ...
    let fut = pin!(async {
        ...
        test_async_drop(AsyncInt(0), [32, 48][cfg!(classic) as usize]).await;
        test_async_drop([AsyncInt(1), AsyncInt(2)], [96, 136][cfg!(classic) as usize]).await;
        test_async_drop((AsyncInt(3), AsyncInt(4)), [112, 168][cfg!(classic) as usize]).await;
        test_async_drop(5, [16, 24][cfg!(classic) as usize]).await;
        let j = 42;
        test_async_drop(&i, [16, 24][cfg!(classic) as usize]).await;
        ...
        let foo = AsyncInt(10);
        test_async_drop(AsyncReference { foo: &foo }, [32, 48][cfg!(classic) as usize]).await;
        ...
        test_async_drop(
            ...
            [48, 72][cfg!(classic) as usize],
        )
        .await;

        test_async_drop(AsyncEnum::A(AsyncInt(12)), [96, 144][cfg!(classic) as usize]).await;
        test_async_drop(AsyncEnum::B(SyncInt(13)), [96, 144][cfg!(classic) as usize]).await;

        test_async_drop(SyncInt(14), [16, 24][cfg!(classic) as usize]).await;
        test_async_drop(
            SyncThenAsync { i: 15, a: AsyncInt(16), b: SyncInt(17), c: AsyncInt(18) },
            [112, 168][cfg!(classic) as usize],
        )
        .await;
        ...
        test_async_drop(AsyncUnion { signed: 21 }, [32, 48][cfg!(classic) as usize]).await;
        test_async_drop(AsyncUnion { signed: 21 }, [32, 48][cfg!(classic) as usize]).await;
    });
