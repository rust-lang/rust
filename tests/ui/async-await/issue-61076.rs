...
struct T; //~ HELP the nightly-only, unstable trait `Branch` is not implemented for `T`
...
async fn bar() -> Result<(), ()> {
    ...
    //~| HELP the nightly-only, unstable trait `Branch` is not implemented for `impl Future<Output = Result<(), ()>>`
