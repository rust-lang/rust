...
//!
//! Under `min_generic_const_args`, a braced const argument containing an
//! associated-function call (e.g. `FieldName::len()`, as generated by `tracing`'s
//! logging macros, which expand field names to `FieldName<{ FieldName::len(name) }>`)
//! was lowered as a tuple-struct constructor. Lowering the callee's `Self` type
//! `FieldName`, written without its `const N: usize` argument, then produced a
//! spurious "missing generics" error (E0107) plus follow-on errors, which made
//! `tracing` fail to compile in any crate enabling the feature.
//!
