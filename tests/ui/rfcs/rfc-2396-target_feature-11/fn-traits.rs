...
fn main() {
    call(foo); //~ ERROR expected a `Fn()` closure, found `#[target_feature] fn() {foo}`
    call_mut(foo); //~ ERROR expected a `FnMut()` closure, found `#[target_feature] fn() {foo}`
    call_once(foo); //~ ERROR expected a `FnOnce()` closure, found `#[target_feature] fn() {foo}`
