...
impl Drop for DropChecker {
    fn drop(&mut self) {
        if CHECK.load(Ordering::SeqCst) != self.0 - 1 {
            panic!("Found {}, should have found {}", CHECK.load(Ordering::SeqCst), self.0 - 1);
        }
        CHECK.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |_c| {
            Some(self.0)
        }).unwrap();
    }
}
...
macro_rules! check_drops {
    ($l:literal) => {
        assert_eq!(CHECK.load(Ordering::SeqCst), $l)
    };
}
