...
pub fn main() {
    ...
    if let Some(Some(&&x)) = &Some(Some(&0)) {
        ...
        //[stable2021,classic2021,structural2021]~| NOTE_NONVIRAL expected integer, found `&_`
        #[cfg(any(classic2024, structural2024))] let _: u32 = x;
    }
    ...
    if let Some(Some(&x)) = &Some(&Some(0)) {
        ...
        //[stable2021]~| NOTE_NONVIRAL expected integer, found `&_`
        #[cfg(any(classic2021, structural2021, classic2024, structural2024))] let _: u32 = x;
    }
    if let Some(&Some(x)) = &Some(Some(0)) {
        //[stable2021]~^ ERROR mismatched types
        //[stable2021]~| NOTE_NONVIRAL expected `Option<{integer}>`, found `&_`
        #[cfg(any(classic2021, structural2021, classic2024, structural2024))] let _: u32 = x;
    }
    if let Some(Some(&mut x)) = &mut Some(&mut Some(0)) {
        //[stable2021]~^ ERROR mismatched types
        //[stable2021]~| NOTE_NONVIRAL expected integer, found `&mut _`
        #[cfg(any(classic2021, structural2021, classic2024, structural2024))] let _: u32 = x;
    }
    ...
    if let Some(&Some(Some(&x))) = &Some(Some(&mut Some(0))) {
        //[stable2021]~^ ERROR mismatched types
        //[stable2021]~| NOTE_NONVIRAL expected `Option<&mut Option<{integer}>>`, found `&_`
        #[cfg(any(classic2021, structural2021, classic2024, structural2024))] let _: u32 = x;
    }
    if let Some(&Some(x)) = &mut Some(Some(0)) {
        //[stable2021]~^ ERROR mismatched types
    ...
    if let Some(&Some(&x)) = &Some(&mut Some(0)) {
        //[stable2021,classic2021,structural2021]~^ ERROR mismatched types
        //[stable2021]~| NOTE_NONVIRAL types differ in mutability
        //[classic2021,structural2021]~| NOTE_NONVIRAL expected integer, found `&_`
        #[cfg(any(classic2024, structural2024))] let _: u32 = x;
    }
    if let Some(&Some(&x)) = &mut Some(&Some(0)) {
        //[stable2021,classic2021,structural2021]~^ ERROR mismatched types
