...
pub fn main() {
    ...
    if let Some(&mut Some(&_)) = &Some(&Some(0)) {
        ...
        //[stable2021,classic2021,structural2021]~| NOTE_NONVIRAL types differ in mutability
        //[classic2024,structural2024]~| NOTE_NONVIRAL cannot match inherited `&` with `&mut` pattern
    }
    if let Some(&Some(&mut x)) = &Some(&mut Some(0)) {
        ...
        //[classic2021,structural2021]~| NOTE_NONVIRAL expected integer, found `&mut _`
        //[structural2024]~| NOTE_NONVIRAL cannot match inherited `&` with `&mut` pattern
        #[cfg(classic2024)] let _: u32 = x;
    }
    if let Some(&Some(&mut _)) = &mut Some(&Some(0)) {
        //~^ ERROR: mismatched types
        //[stable2021,classic2021,structural2021]~| NOTE_NONVIRAL expected integer, found `&mut _`
        //[classic2024,structural2024]~| NOTE_NONVIRAL cannot match inherited `&` with `&mut` pattern
    }
    if let Some(&Some(Some(&mut x))) = &Some(Some(&mut Some(0))) {
        ...
        //[structural2021,structural2024]~| NOTE_NONVIRAL cannot match inherited `&` with `&mut` pattern
        #[cfg(any(classic2021, classic2024))] let _: u32 = x;
    }
    if let Some(&mut Some(x)) = &Some(Some(0)) {
        //~^ ERROR: mismatched types
        //[stable2021]~| NOTE_NONVIRAL expected `Option<{integer}>`, found `&mut _`
        //[classic2021,structural2021,classic2024,structural2024]~| NOTE_NONVIRAL cannot match inherited `&` with `&mut` pattern
    }
}
