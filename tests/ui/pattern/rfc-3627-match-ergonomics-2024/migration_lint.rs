...
fn main() {
    ...
    let Foo(mut x) = &Foo(0);
    //~^ ERROR: explicit binding modifiers may not be written within elided reference patterns in Rust 2024
    ...
    let Foo(&x) = &Foo(&0);
    //~^ ERROR: explicit reference patterns may not be written within elided reference patterns in Rust 2024
    //~| WARN: this changes meaning in Rust 2024
    assert_type_eq(x, 0u8);
    ...
    //~^ ERROR: explicit binding modifiers and reference patterns may not be written within elided reference patterns in Rust 2024
    //~| WARN: this changes meaning in Rust 2024
    assert_type_eq(a, 0u32);
    assert_type_eq(b, &&0u32);
