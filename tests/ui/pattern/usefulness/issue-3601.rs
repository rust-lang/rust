...
fn main() {
    ...
    match n.kind {
        NodeKind::Element(ed) => match ed.kind {
            //~^ ERROR non-exhaustive patterns
            //~| NOTE the matched value is of type
            //~| NOTE match arms with guards don't count towards exhaustivity
            //~| NOTE pattern `deref!(ElementKind::HTMLImageElement(_))` not covered
            //~| NOTE `Box<ElementKind>` defined here
            ElementKind::HTMLImageElement(ref d) if d.image.is_some() => true,
        },
    };
}
