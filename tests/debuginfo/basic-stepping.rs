...
//@ lldb-command: run
// In `breakpoint_callback()` in `./src/etc/debugger_tester/lldb/batchmode.py` we do
...
// `normalize_whitespace()` in `./src/etc/debugger_tester/lldb/batchmode.py` removes the newlines of
// the output. So the current line and the line before actually ends up on the same
