...
// change all the time.
// EMIT_MIR init_box_via_move.box_new.CleanupPostBorrowck.after.mir
...
fn box_new<T: Copy>(x: T) -> Box<[T; 1024]> {
    ...
    std::boxed::init_box_via_move(b, [x; 1024])
}

// EMIT_MIR init_box_via_move.vec_macro.CleanupPostBorrowck.after.mir
