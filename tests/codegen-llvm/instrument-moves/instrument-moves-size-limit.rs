//
//@ compile-flags: -Z instrument-moves -Z instrument-moves-size-limit=100 -Copt-level=0 -g
...
// Medium struct (80 bytes) should NOT be instrumented
// Large struct (160 bytes) should be instrumented

// CHECK-LABEL: instrument_moves_size_limit::test_medium_copy
// CHECK-NOT: !DISubprogram(name: "compiler_copy"
pub fn test_medium_copy() {
    let medium = MediumStruct::new();
    let _copy = medium.clone(); // Should NOT generate instrumentation (below threshold)
}

// CHECK-LABEL: instrument_moves_size_limit::test_large_copy
pub fn test_large_copy() {
    let large = LargeStruct::new();
    let _copy = large.clone(); // Should generate instrumentation (above threshold)
}
