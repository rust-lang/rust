...
pub fn bar1() {
    ...
    // CHECK:       call void %{{[0-9]}}(ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE1:[[:print:]]+]]) ]
}
...
pub fn bar2() {
    ...
    // CHECK:       call void %{{[0-9]}}(ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE2:[[:print:]]+]]) ]
}
...
pub fn foo3(a: &dyn Trait3<Type3>) {
    ...
    // CHECK:       call void %{{[0-9]}}(ptr {{%[a-z]\.0|%_[0-9]}}, ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE3:[[:print:]]+]]) ]
}
...
pub fn bar3() {
    ...
    // CHECK:       call void %{{[0-9]}}(ptr {{%[a-z]|%_[0-9]}}, ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE3:[[:print:]]+]]) ]
}
...
pub fn foo4<'a>(a: &dyn Trait4<'a, Type4, Output = &'a i32>) {
    ...
    // CHECK:       call align 4 ptr %{{[0-9]}}(ptr {{%[a-z]\.0|%_[0-9]}}, ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE4:[[:print:]]+]]) ]
}
...
pub fn bar4<'a>() {
    ...
    // CHECK:       call align 4 ptr %{{[0-9]}}(ptr {{%[a-z]|%_[0-9]}}, ptr {{%[a-z]|%_[0-9]}}){{.*}}[ "kcfi"(i32 [[TYPE4:[[:print:]]+]]) ]
}
