...
pub fn vec_extend_via_iter_repeat_n(len: usize) -> Vec<u8> {
    // CHECK: %[[ADDR:.+]] = tail call {{(noalias )?}}noundef dereferenceable_or_null(1234) ptr @{{.*}}__rust_alloc(
    // CHECK-SAME: i64 noundef {{(range\(i64 0, -9223372036854775808\) )?}}1234,
    // CHECK-SAME: i64 noundef 1)
