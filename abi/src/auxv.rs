...
//! loaders and runtimes can interoperate.  ThingOS-specific extensions occupy
...
// ── ThingOS-specific AT_* extensions (0x1000–0x100f) ──────────────────────────
...
pub const AT_THINGOS_TLS_TEMPLATE_VA: u64 = 0x1000;

/// Size of the initialized portion of the TLS template (PT_TLS `p_filesz`).
///
/// Only the first `AT_THINGOS_TLS_FILESZ` bytes of the template region contain
/// explicit values.  The remainder up to `AT_THINGOS_TLS_MEMSZ` must be
/// zero-initialized (BSS).
pub const AT_THINGOS_TLS_FILESZ: u64 = 0x1001;
...
/// `AT_THINGOS_TLS_ALIGN`) for the TLS data area, placed *before* the Thread
/// Control Block (TCB) in the ELF x86_64 "variant II" layout.
pub const AT_THINGOS_TLS_MEMSZ: u64 = 0x1002;

/// Required alignment for the per-thread TLS data block (PT_TLS `p_align`).
pub const AT_THINGOS_TLS_ALIGN: u64 = 0x1003;
